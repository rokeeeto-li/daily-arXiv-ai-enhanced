...
### [1] [Human-aware Design Generation: Adding 3D Humans into Graphic Designs](https://arxiv.org/abs/2609.17689)
*Zijin Hou,Ying Cao*

Main category: cs.GR

TL;DR: This paper introduces design-conditioned 3D human adding, a novel task where 3D human representations are added to existing partial graphic designs by predicting 3D poses, 2D composition, and spatial arrangement.


<details>
  <summary>Details</summary>
Motivation: Graphic designs predominantly contain human images, but there has been no work investigating the role of human representations in design creation. The authors seek to emphasize the importance of visual human representations in graphic design generation.

Method: A model is proposed that adds 3D human representations into existing partial designs by predicting three components: 3D poses, 2D composition in the image containing them, and spatial arrangement of the image in the input design.

Result: The model outperforms baselines by predicting 3D human poses, 2D framing, and human image layouts that better harmonize with existing design elements, and generates higher-quality holistic designs.

Conclusion: This is the first work exploring connections between 3D human generation and 2D graphic design generation, which will inspire future research in leveraging 3D information to build more powerful generative models for graphic designs in the 2D domain.
...
### [2] [PointGrade: Geometric Priors for Grading MoonBoard Problems](https://arxiv.org/abs/2609.17770)
...
### [3] [Human-Centric Grasp State Assessment: Toward Transferring Subjective Evaluation to Robots](https://arxiv.org/abs/2609.17540)
*Ryohei Kobayashi,Kosei Isomoto,Yuga Yano,Yuichiro Tanaka,Hakaru Tamukoh*

Main category: cs.RO

TL;DR: A VLM-based framework transfers human subjective criteria for grasping deformable objects using minimal human annotations, enabling online adaptation via tactile and force measurements, validated through experiments on three objects and human evaluation with 25 participants.


<details>
  <summary>Details</summary>
Motivation: Transferring tacit human subjective criteria to robotic systems for appropriate grasping of deformable objects is challenging due to the semantic gap between qualitative human expectations and quantitative robotic measurements, and conventional deep learning approaches require prohibitive amounts of manually annotated data for each new object.

Method: Integrates a Vision-Language Model (VLM)-based semi-automated supervisor generator with a lightweight grasp state predictor. Uses a minimal set of human-annotated trials as contextual anchors to propagate subjective criteria to unannotated data, enabling sequential estimation of grasp state from time-series tactile and grasping force measurements for rapid online adaptation.

Result: Experiments on three representative deformable objects and a human evaluation study with 25 participants demonstrate the framework's feasibility for adjusting grasping force according to human-perceived grasp appropriateness in the evaluated task setting.

Conclusion: The proposed framework effectively transfers human subjective criteria to robotic systems for appropriate grasping of deformable objects, enabling online adaptation with minimal human annotation and validated through human evaluation studies.
...
### [4] [Real-Time Service Robot Replanning via Simple Button Interaction for Improved Task Success and User Experience](https://arxiv.org/abs/2609.17541)
...
### [5] [Predictive Varanus: Combining CSP Conformance Monitoring with Predictive LTL Runtime Verification](https://arxiv.org/abs/2609.17625)
...
### [6] [Flexible-body Modeling, Kinematic Identification, and Assembly Accuracy of Overconstrained Spatial Linkages](https://arxiv.org/abs/2609.17627)
...
### [7] [LEAP: Learning Emergent Active Perception for Quadruped Navigation](https://arxiv.org/abs/2609.17628)
*Ü. Bora Gökbakan,Stéphane Caron,Philippe Souères*

Main category: cs.RO

TL;DR: This paper proposes LEAP (Learned emergent active perception), a method for autonomous agents to learn gaze control for goal-oriented navigation over hazardous terrains without hand-designed proxy objectives. The approach uses a gaze-invariant representation integrating depth images into egocentric belief maps, achieving 92.7% success rate—surpassing scripted (74.2%) and passive perception (34.5%) baselines—while maintaining applicability to quadrupedal locomotion policies.


<details>
  <summary>Details</summary>
Motivation: Active perception enables agents to select viewpoints that reduce environmental uncertainty, but current learned systems rely on hand-designed proxy objectives (e.g., coverage, curiosity bonuses) that can conflict with the task. The authors aim to develop a method where active perception emerges naturally from the task objective itself, without augmentation.

Method: The authors formulate goal-oriented navigation over hazardous terrains with visually discovered goals. They propose a navigation policy architecture with active perception trained on a terrain curriculum where task pressure alone drives gaze control emergence. The key innovation is a gaze-invariant representation that integrates depth images into egocentric belief maps, allowing the policy to learn effective gaze control through task experience rather than explicit proxy rewards.

Result: LEAP achieves a 92.7% success rate in held-out evaluation scenarios, compared to 74.2% for scripted approaches and 34.5% for passive perception baselines, coming within 4.6 points of a privileged oracle. Remarkably, the same LEAP navigation policies can be directly applied to steering quadrupedal locomotion policies in physics simulation without modification.

Conclusion: This work demonstrates that active perception can emerge naturally from task pressure when using appropriate representations (gaze-invariant belief maps integrating depth images), eliminating the need for hand-designed proxy objectives. The approach achieves state-of-the-art performance on hazardous terrain navigation while maintaining strong generalization to different locomotion domains (quadrupeds), suggesting a more principled foundation for embodied AI systems.
...
### [8] [Vision-Language Grounded Task-Context-Aware Imitation Learning for Robotic Disassembly](https://arxiv.org/abs/2609.17714)
*Jeon Ho Kang,Igal Tamarkin,Ethan Niu,Ian Novales,Satyandra K. Gupta*

Main category: cs.RO

TL;DR: This paper introduces a robotic disassembly framework that uses language instructions to provide task context, combining hierarchical task selection with task-context-aware imitation learning to ground language instructions in spatial visual representations.


<details>
  <summary>Details</summary>
Motivation: Real-world robotic disassembly requires long-horizon execution with ordered sequences of manipulation tasks across multiple parts. Multiple valid task goals and diverse assembly configurations make it difficult for imitation policies to infer intended skills from raw observations alone, especially when training data cannot cover the combinatorial diversity of real-world configurations and part geometries.

Method: The proposed framework combines hierarchical task selection with task-context-aware imitation learning to ground language instructions in spatial visual representations for robotic disassembly.

Result: The method improves end-to-end task success by 35 percentage points over the baseline diffusion policy and by 75 percentage points over the previous task-context-aware baseline.

Conclusion: The framework generalizes across diverse connector geometries and assembly configurations without requiring explicit object annotations.
...
### [9] [InterMASH: A Unified Geometric Representation for Grasp Synthesis](https://arxiv.org/abs/2609.18504)
*Xuanze Yang,Yumeng Liu,Haiyang Xin,Changhao Li,Haowei Shen,Kai Xu,Ligang Liu,Ruizhen Hu*

Main category: cs.RO

TL;DR: InterMASH proposes a unified geometric representation for hand-object interactions using sphere-fixed anchors with spherical harmonics encoding local geometry and contact, enabling cross-embodiment correspondence between human and robotic hands through a conditional Diffusion Transformer.


<details>
  <summary>Details</summary>
Motivation: Unified representation across human and robotic hands is lacking due to differences in hand morphology and surface modeling. Prior methods rely on contact maps or dense implicit descriptors which are often incomplete, computationally expensive, or redundant.

Method: InterMASH establishes cross-embodiment correspondence using sphere-fixed anchors where low-degree spherical harmonics compactly encode local hand geometry, object geometry, and contact at each anchor, forming an explicit and interpretable token sequence. A conditional Diffusion Transformer operates directly in this tokenized structure to jointly generate hand geometry and contact.

Result: Achieves competitive performance with state-of-the-art methods on key physical feasibility metrics in a large-scale ShadowHand benchmark, supports joint training across multiple hands, and demonstrates that cross-embodiment fine-tuning with human grasp data improves robotic grasp success and diversity.

Conclusion: The proposed InterMASH representation effectively enables cross-embodiment correspondence and joint generation of hand-object interactions, showing strong physical plausibility and improving robotic grasp capabilities when fine-tuned with human grasp data.
...
### [10] [RAF-VLA: Representation Alignment with the Future for End-to-End Autonomous Driving](https://arxiv.org/abs/2609.17728)
*Dogun Kim,Yongjae Lee,Joonhee Lim,Yeina Lee,Junhyeok Park,Moogeun Park,Dongsuk Kum*

Main category: cs.RO

TL;DR: RAF-VLA is a VLA framework for autonomous driving that uses Future-Aligned Supervised Fine-Tuning to align the policy's hidden states with future-frame representations from a pretrained world encoder, avoiding the training burden and inference latency of explicit future generation.


<details>
  <summary>Details</summary>
Motivation: Existing World-Modeling VLAs predict future driving scenes alongside driving actions for dense supervision, but this explicit future generation introduces two key limitations: additional training burden and inference latency.

Method: RAF-VLA employs Future-Aligned Supervised Fine-Tuning, where a straightforward regularization aligns the policy's hidden states with future-frame representations obtained from a pretrained world encoder while learning driving actions.

Result: On NAVSIM benchmark, RAF-VLA achieves competitive planning performance against state-of-the-art VLA planners with substantially fewer training samples. Training overhead is only 3.8% and inference overhead is negligible (1 ms).

Conclusion: RAF-VLA effectively shapes planning-relevant internal representations through direct alignment with future-frame representations, avoiding the limitations of explicit future generation while maintaining competitive performance.
...
### [11] [CALOS: Control-Affine Lyapunov On-manifold Safety Layer for Safe Deep Reinforcement Learning for Quadrotors](https://arxiv.org/abs/2609.17758)
*Fabrizio Cesareo,Sebastiano Mengozzi,Nicola Mimmo,Andrea Acquaviva*

Main category: cs.RO

TL;DR: CALOS is a runtime safety layer for quadrotor control using deep reinforcement learning that enforces attitude constraints via a quadratic programming formulation solved through active-set enumeration over the torque space.


<details>
  <summary>Details</summary>
Motivation: Deep reinforcement learning has demonstrated remarkable capability in quadrotor control, yet learned policies lack guarantees of respecting safety constraints during training or deployment. The authors seek to enforce attitude constraints without modifying the underlying learning algorithm.

Method: CALOS formulates four tilt-angle inequalities and a Lyapunov descent condition as a single quadratic program. The solution is the minimum-norm correction to the nominal torque output of the policy. The quadratic program is solved exactly via active-set enumeration over the three-dimensional torque space, with computational cost low enough for real-time enforcement across thousands of parallel simulation environments.

Result: Evaluated on trajectory-tracking tasks in NVIDIA Isaac Lab, CALOS reduces lateral tracking error by 55-60% relative to an unconstrained Proximal Policy Optimization baseline while achieving zero attitude-constraint violations on the training trajectory. The safety layer also accelerates training convergence and improves data efficiency without producing suboptimal policies.

Conclusion: The CALOS safety layer successfully enforces constraints in real time across massively parallel Deep Reinforcement Learning training environments. By restricting exploration to safe regions of the state space, it improves performance metrics while maintaining policy optimality.
...
### [12] [DRT&R: Direct Radar Teach & Repeat](https://arxiv.org/abs/2609.17766)
*Alexander Krawciw,Daniil Lisus,Cedric Le Gentil,Timothy D. Barfoot*

Main category: cs.RO

TL;DR: Direct Radar Teach & Repeat (DRT&R) is a direct radar-based navigation stack that combines direct radar processing with local mapping to maximize information retention, achieving state-of-the-art localization performance in both on-road and off-road environments.


<details>
  <summary>Details</summary>
Motivation: Radar is more robust than lidar in adverse conditions (precipitation, dust, fog, smoke). While direct methods have improved on-road global localization, they haven't been deployed in off-road or closed-loop systems. Additionally, direct global maps lose viewpoint-dependent radar artifacts that can provide useful pose information.

Method: DRT&R combines direct radar processing with local mapping to maximize retained information. It operates as a direct spinning radar-based navigation stack that processes the entire radar scan rather than sparse points.

Result: Using 344 km of on-road data and 20 km of off-road data, DRT&R achieves 4 cm localization accuracy in most conditions and 12 cm in degenerate environments. It demonstrated autonomous closed-loop navigation of over 10 km using a Clearpath Warthog off-road vehicle with real-time performance.

Conclusion: DRT&R runs in real time and achieves state-of-the-art tracking performance for off-road radar navigation, with superior localization accuracy compared to existing methods.
...
### [13] [HINT-Plan: Human Intention-Aware Robot Task Planning in Context-Rich Environments using Vision Language Models](https://arxiv.org/abs/2609.17771)
...
### [14] [SPROUT: The Open-Source Soft Growing Robot for Search and Rescue](https://arxiv.org/abs/2609.17781)
...
TL;DR: SPROUT is an open-source modular soft robotic system designed for search and rescue operations, with comprehensive supplementary materials (code, CAD assemblies, build instructions) enabling the robotics community to replicate and adapt the system for various applications.


<details>
  <summary>Details</summary>
Motivation: Soft robotics has theoretical potential for search and rescue but faces significant barriers to entry. There's a gap in replicable, reliable soft robot systems that can operate in harsh field conditions such as dusty, wet, and isolated environments.

Method: The authors present SPROUT (Soft Pathfinding Robotic Observation Unit) with a modular hardware design and ROS 2-based software stack supporting task-specific payloads. They provide extensive supplementary materials including code, parts manifests, CAD assemblies, and build instructions. The system's architecture allows adaptation for applications beyond search and rescue, including infrastructure inspection and archaeology.

Result: SPROUT successfully performs a variety of challenging inspection and traversal tasks in collapsed structure training sites used by first responders, demonstrating its operational capability in realistic field conditions.

Conclusion: The open-source design and modular architecture of SPROUT enable the robotics research community to build and adapt soft robotic systems for field deployment, significantly lowering barriers to entry and facilitating broader adoption beyond search and rescue into other domains.
...
### [15] [Learning Multi-Humanoid Pickup and Transport via Decentralized Object-Centric Control](https://arxiv.org/abs/2609.17824)
*Bikram Pandit,Mohitvishnu S. Gadde,Aayam Kumar Shrestha,Alan Fern*

Main category: cs.RO

TL;DR: A decentralized object-centric control approach using attachment-based interfaces enables cooperative multi-humanoid pickup and transport of objects of varying sizes and weights, with effective sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: The authors address the challenge of cooperative multi-humanoid pickup and transport tasks that require robot teams of varying sizes to handle objects with different sizes, weights, and geometries. They aim to create a unified control abstraction that can seamlessly transition between single-robot pickup, cooperative multi-robot transport, and robot-to-robot handover without per-task redesign.

Method: The approach employs decentralized object-centric control where each humanoid robot is assigned a local attachment region on the shared object. Robots learn to perform pickup and transport through gripperless bimanual pinching. This attachment-based interface provides a common control abstraction that spans different manipulation scenarios. The method uses reinforcement learning to train policies that can transfer between single-robot and multi-robot settings.

Result: Policies trained solely on single-robot pickup transfer nontrivially to cooperative settings, indicating the abstraction captures significant coordination structure. Explicit multi-robot training further improves performance, demonstrating that shared-object coupling introduces beneficial coordination dynamics. The approach is validated in simulation across varying team sizes (2-8 robots) and object geometries, and successfully demonstrates sim-to-real transfer on real humanoid hardware.

Conclusion: The decentralized object-centric control with attachment-based interfaces successfully enables cooperative manipulation tasks across different team sizes and object types. The results show that single-robot training provides a strong foundation for multi-robot coordination, while explicit multi-robot training offers additional performance improvements by directly learning the coordination dynamics introduced by shared-object coupling. This approach demonstrates effective sim-to-real transfer, enabling real humanoids to perform cooperative manipulation tasks.
...
### [16] [Adaptive-MHE : A Sampling-Based Adaptive MPC for Legged Loco-Manipulation via Moving Horizon Estimation](https://arxiv.org/abs/2609.17832)
*Hossein Keshavarz,Alejandro Ramirez-Serrano,Majid Khadiv*

Main category: cs.RO

TL;DR: Adaptive-MHE is an online sampling-based system identification framework that enables legged robots to adaptively estimate object and terrain physical parameters (mass, friction) and perform effective loco-manipulation in changing environments.


<details>
  <summary>Details</summary>
Motivation: Legged robots struggle with generating effective loco-manipulation behaviors due to unknown object and terrain parameters, which creates a sim-to-real gap. Classical system identification methods assume differentiable dynamics (not valid for contact-rich legged systems), while existing sampling-based approaches are typically offline and do not adapt to environmental changes.

Method: Adaptive-MHE is an online sampling-based framework based on moving horizon estimation (MHE) that estimates physical parameters (mass, friction) of objects and terrain and couples these estimates with a sampling-based model predictive controller for adaptive loco-manipulation.

Result: In both simulation and hardware experiments, Adaptive-MHE consistently outperforms baselines and achieves performance comparable to a controller with access to ground-truth parameters.

Conclusion: The proposed framework enables effective adaptive loco-manipulation in changing and uncertain environments by simultaneously identifying unknown physical parameters and adapting control policies online.
...
### [17] [VCTP: Vehicle-Conditioned Terrain Planning for Off-Road Navigation](https://arxiv.org/abs/2609.17834)
*Akshay Naik,Ramavarapu S. Sreenivas,Dustin Nottage,Ahmet Soylemezoglu*

Main category: cs.RO

TL;DR: Vehicle-Conditioned Terrain Planning (VCTP) is a method that evaluates terrain at eight vehicle headings using shared elevation and surface-ID layers, with body geometry constraints and loaded wheel contacts determining surface cost and pitch/roll predictions, enabling more accurate route planning through sampling at wheel contacts rather than vehicle center.


<details>
  <summary>Details</summary>
Motivation: A vehicle's heading affects both the surfaces beneath its tires and its pitch and roll, but traditional planning methods don't account for these relationships, leading to suboptimal route planning that doesn't consider how vehicle orientation impacts terrain interaction and stability.

Method: VCTP evaluates shared elevation and surface-ID layers at eight headings, constraining admissibility through body geometry while using loaded wheel contacts to determine modeled surface cost and predicted pitch and roll. It combines this with established D* Lite and vehicle-state search algorithms to plan routes with forward and reverse motion, and implements selective updates that recompute only affected body or contact queries when observations change.

Result: Sampling at wheel contacts rather than vehicle center lowers modeled surface cost by 39.3% while shortening routes. In offline planning on RGator recordings, VCTP reduces modeled costs over identified surfaces and observed support relative to distance-focused planning. Selective updates match full recomputation in all 474 comparisons using recorded map changes.

Conclusion: The results demonstrate that wheel-contact placement and vehicle heading significantly affect route choice, with VCTP providing more accurate terrain modeling and cost estimation than distance-focused approaches, though incomplete coverage can leave full-route rankings unresolved.
...
### [18] [Timely Activation of Safety Filters via One-Step Reachability Expansion](https://arxiv.org/abs/2609.17904)
*Javier Borquez*

Main category: cs.RO

TL;DR: This paper addresses the mismatch between continuous-time safety guarantees and discrete-time robotic control by introducing a one-step expanded Backward Reachable Tube (BRT) that enables early safety interventions, ensuring correctness under discrete-time execution.


<details>
  <summary>Details</summary>
Motivation: Existing Hamilton-Jacobi reachability-based safety filters provide strong safety guarantees under continuous-time assumptions, but discrete sampling intervals create a mismatch where the system can jump into unsafe states between control updates, leading to failures that are theoretically avoidable.

Method: The authors formulate a one-step expanded BRT that predicts all states capable of reaching the true BRT within a single timestep. This expanded boundary is used as the activation condition for the safety filter. The expanded set is formulated as a modified reachability problem and computed using standard continuous-time solvers.

Result: By using the expanded boundary as the activation condition, safety interventions occur early enough to ensure correctness under discrete-time execution, effectively bridging the gap between continuous-time theoretical guarantees and discrete-time practical implementation.

Conclusion: The proposed one-step expanded BRT approach provides a principled solution to enable early safety interventions that prevent failures in discrete-time robotic systems while maintaining the strong safety guarantees derived from continuous-time reachability analysis.
...
### [19] [Map2Route: Benchmarking Compositional Language-Grounded Route Planning over Semantic Maps](https://arxiv.org/abs/2609.17910)
...
### [20] [Multi-Session Multimodal Underwater Mapping with Acoustic and Optical Imaging](https://arxiv.org/abs/2609.17929)
...
### [21] [Feedback-Modulated Harmonic Policies for Quadruped Locomotion](https://arxiv.org/abs/2609.17946)
*Yixuan Jia,Steven Roche,Jonathan P. How*

Main category: cs.RO

TL;DR: This paper explores explicit Fourier series representations for quadruped locomotion policies, demonstrating that periodic locomotion structure emerges naturally in standard MLP policies and achieving 3.67 m/s peak speed with 5.883 kg payload capacity on a Unitree Go2 robot.


<details>
  <summary>Details</summary>
Motivation: Current learned quadruped locomotion policies map observations to joint actions, leaving periodic locomotion structure implicit. The authors investigate explicit Fourier representations to potentially improve control and understanding of periodic dynamics.

Method: A context network generates Fourier coefficients and feedback network weights to produce command-conditioned Fourier series trajectories. They compare this explicit Fourier approach with standard MLP policies using Dynamic Mode Decomposition to analyze hidden activation patterns and periodic structure.

Result: DMD analysis reveals dominant activation modes at foot-height oscillation frequency and second harmonic, showing periodic structure emerges naturally in MLP policies. The harmonic controller achieves 3.67 m/s peak speed and carries 5.883 kg loads on a real Unitree Go2 robot.

Conclusion: Explicit Fourier representations can effectively model quadruped locomotion, but periodic structure emerges naturally in standard MLP policies as demonstrated through DMD analysis of activation modes.
...
### [22] [Causal-History Test-Time Scaling for Failure Recovery in Autoregressive World-Action Models](https://arxiv.org/abs/2609.18016)
...
TL;DR: A training-free framework for failure recovery in World-action models (WAMs) that decomposes recovery into three stages: Progress-Aware Recovery Trigger, History-Prefix Recovery, and Hypothesis Verification.


<details>
  <summary>Details</summary>
Motivation: Existing WAMs trained on successful trajectories only fail when real-world execution diverges from learned dynamics. Autoregressive WAMs compound this issue as errors become part of causal history and influence subsequent predictions.

Method: Reformulates failure recovery as test-time scaling over causal histories. Implements three coupled decisions: when to revise causal history, where to recover a reliable history prefix, and which history configuration best supports execution. The framework comprises: (1) Progress-Aware Recovery Trigger detecting persistent non-progress, (2) History-Prefix Recovery identifying unreliable history suffix, retrieving anchors, and reconstructing causal KV state, and (3) Hypothesis Verification comparing complete-history, recovered-prefix, and full-reset hypotheses.

Result: Consistent improvements in task success demonstrated in both simulated and real-world manipulation settings, with ablations confirming each recovery stage contributes to performance gains.

Conclusion: The proposed framework effectively addresses failure recovery in autoregressive WAMs through test-time scaling over causal histories, enabling robust manipulation performance even when execution diverges from training data distribution.
...
### [23] [Online Multimodal Workload Assessment in Contact-Rich Physical Human-Robot Interaction](https://arxiv.org/abs/2609.18031)
*Yanyi Chen,Fan Yang,Min Deng*

Main category: cs.RO

TL;DR: An online multimodal framework integrates interaction wrench, TCP kinematics, and skin conductance level into interpretable workload factors, adjusted by path curvature, achieving 0.308 Spearman correlation with physiological reference while maintaining transparency and outperforming fixed-weight formulations.


<details>
  <summary>Details</summary>
Motivation: Contact-rich pHRI imposes time-varying demands (physical interaction, motor regulation, physiological response) requiring continuous workload assessment, motivating development of interpretable, real-time evaluation methods.

Method: The framework integrates interaction wrench, planar TCP kinematics, and skin conductance level (SCL) into four interpretable workload factors. Path curvature adjusts relative contributions to reflect motion demand and task progression. Evaluated with 24 participants across 18 environmental combinations under two admittance-control modes using strict leave-one-subject-out evaluation with standardized pupil diameter as physiological reference.

Result: Cohort-mean 30s block-wise Spearman correlation of ρ₃₀=0.308 with physiological reference, positive subject-level correspondence in 23/24 participants. Performance comparable to state-of-the-art learning-based baseline, outperformed fixed-weight formulation, and maintained consistency across two admittance-control modes.

Conclusion: The proposed framework provides a transparent and interpretable approach to continuous interaction workload assessment in contact-rich pHRI, balancing performance with interpretability through explicit workload-related factors and defined weighting rules.
...
### [24] [Geometric Shortcuts for Complex Trunk Postures: Dual-Helicity Coupling Enables Low-Dimensional Control](https://arxiv.org/abs/2609.18050)
*Huishi Huang,Danlu Chen,Matteo Lo Preti,Jun Liu,Marcelo H. Ang,Cecilia Laschi*

Main category: cs.RO

TL;DR: The paper investigates how elephant trunks achieve complex postures through dual-helicity coupling between opposite-handed oblique muscles, demonstrating that only two geometric parameters can generate a wide range of elephant-like postures in a soft-robotic prototype, suggesting a dual-layer control architecture for continuum robots.


<details>
  <summary>Details</summary>
Motivation: Elephant trunks can generate complex postures without relying solely on fine segmental activation. The authors aim to understand the underlying mechanism that enables this behavior and explore its implications for continuum robot design.

Method: A simplified soft-robotic prototype was created to test the hypothesis of dual-helicity coupling between opposite-handed oblique muscles. The researchers varied only two geometric parameters to generate a broad library of elephant-like postures.

Result: By varying just two geometric parameters in the soft-robotic prototype, a wide variety of elephant-like postures were generated. This demonstrates that dual-helicity coupling provides a low-dimensional geometric shortcut for posture generation.

Conclusion: The findings support a dual-layer control architecture for continuum robots and offer a new biological hypothesis for how elephant trunks achieve complex postures. This research has significant implications for both continuum robot design and our understanding of biological control mechanisms.
...
### [25] [CLASP: A Cluster-Level Autonomous Selective Picking Robot with a Soft Rolling-Band Gripper for Fresh-Market Blueberry Harvesting](https://arxiv.org/abs/2609.18051)
...
### [26] [An Efficient Algorithm for Minimum-Pressure Growth Planning of Vine Robots](https://arxiv.org/abs/2609.18070)
...
### [27] [Characterizing Refraction-Induced Ranging Bias in Underwater Collaborative Localization](https://arxiv.org/abs/2609.18073)
...
### [28] [Not All Layers Need Tuning: Diagnosing and Directing Adaptation in Vision-Language-Action Models](https://arxiv.org/abs/2609.18084)
...
### [29] [ReRadar: Robust Radar Global Localization via Rotation-Equivariant Descriptor Learning](https://arxiv.org/abs/2609.18092)
*Duc Manh Nguyen,Truong Giang Dao,Gia Nghiem Luong,Viet Trung Hoang,Anh Quang Nguyen*

Main category: cs.RO

TL;DR: ReRadar is a radar global localization pipeline that uses steerable CNNs to extract rotation-equivariant features, group pooling and NetVLAD for rotation-invariant descriptors, and combines descriptor retrieval with landmark-based matching to achieve high recall rates (up to 99.38%) across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Global localization with scanning millimeter-wave radar is challenging because traditional place-recognition descriptors often discard the spatial structure needed for accurate pose retrieval.

Method: ReRadar extracts rotation-equivariant intermediate features using steerable convolutional neural networks, forms rotation-invariant descriptors through group pooling and NetVLAD aggregation, and combines descriptor retrieval with landmark-based matching to estimate the robot's 3-DoF pose.

Result: With target-dataset adaptation, ReRadar achieves 99.37% Recall@1 on OORD Bellmouth, 91.44% Recall@1 with 80.99% F1_max on Mulran DCC01, and 99.38% Recall@1 on falling-snow Boreas sequence. Without target-dataset data, cross-dataset performance reaches 98.07% Recall@1 on OORD.

Conclusion: ReRadar demonstrates state-of-the-art performance in radar global localization, achieving comparable or superior results to existing methods across multiple datasets without requiring target-dataset data for adaptation.
...
### [30] [Beyond Pixel Similarity: Task-Aware Evaluation of GAN-Based Synthetic Sonar Data for Robotic Perception](https://arxiv.org/abs/2609.18100)
*Hannan Ejaz Keen,Muhammad Moazam Fraz,Karsten Berns*

Main category: cs.RO

TL;DR: This study investigates whether conventional image-fidelity metrics (SSIM, PSNR, MSE) accurately reflect downstream perception performance of GAN-generated synthetic sonar data. Using Pix2Pix with four discriminator configurations and evaluating with both pixel-level metrics and YOLOX/Faster R-CNN detectors, the authors find a discrepancy: configurations with best fidelity scores don't consistently achieve best detection performance, particularly PatchGAN configurations.


<details>
  <summary>Details</summary>
Motivation: Synthetic data reduces training data costs for robotic perception, but generating sonar imagery that preserves task-relevant characteristics remains challenging. There is uncertainty about whether conventional image-fidelity metrics adequately correlate with actual perception performance for GAN-generated synthetic sonar data.

Method: Pix2Pix conditional GAN with four discriminator configurations (PixelGAN, PatchGAN-16, PatchGAN-70, ImageGAN) trained on sonar imagery from two datasets. Evaluated using conventional metrics (SSIM, PSNR, MSE) and task-oriented evaluation with YOLOX-S, YOLOX-L, and Faster R-CNN detectors trained on real sonar and tested on GAN-generated images with identical test samples and annotations.

Result: Significant discrepancy between image-fidelity and downstream object-detection performance. The configuration achieving best SSIM, PSNR, and MSE does not consistently yield best detection performance. PatchGAN configurations achieve strong detection results despite not achieving the highest pixel-level similarity scores.

Conclusion: Pixel-level image-fidelity metrics alone may not consistently capture the task-relevant realism of synthetic sonar observations. The findings motivate the use of task-aware evaluation for synthetic sensor data intended for robotic perception applications.
...
### [31] [Technical Report: One-Step Drifting Action Heads for GR00T N1.7](https://arxiv.org/abs/2609.18108)
*Xihe Shao*
...
### [32] [A Comprehensive Review of Generative Physical Artificial Intelligence](https://arxiv.org/abs/2609.18111)
*Satyam Gaba,Krutiksinh Rana,Siva Sai,Vinay Chamola,Dusit Niyato*

Main category: cs.RO

TL;DR: This survey introduces a taxonomy of five approaches to Generative Physical Artificial Intelligence (GPAI): Robot Foundation Models (RFMs), Vision-Language Action (VLA) models, Large Behavior Models (LBMs), Diffusion Policy Models (DPMs), and World Foundation Models (WFMs). It examines their complementary roles, applications across autonomous vehicles, industrial automation, healthcare robotics, and humanoid systems, and identifies key research directions in data-efficient learning, sim-to-real transfer, edge-compatible architectures, and safety frameworks.


<details>
  <summary>Details</summary>
Motivation: The integration of large-scale foundation models with physical embodiments has advanced robotics through Generative Physical Artificial Intelligence (GPAI), enabling agentic AI systems to autonomously perceive, reason, and act in complex real-world situations.

Method: The paper introduces a taxonomy of five distinct approaches to GPAI: Robot Foundation Models (RFMs) for cross-platform skill transfer; Vision-Language Action (VLA) models for end-to-end multi-modal perception and control; Large Behavior Models (LBMs) for human-like movement generation; Diffusion Policy Models (DPMs) for temporally coherent action generation; and World Foundation Models (WFMs) for physics-compliant simulation and data generation. It examines how these approaches complement each other: WFMs generate training data for VLAs and DPMs, RFMs enable cross-platform deployment, while LBMs provide motion priors.

Result: Through examples across autonomous vehicles, industrial automation, healthcare robotics, and humanoid systems, the paper identifies significant performance improvements. It summarizes promising research directions in data-efficient learning, sim-to-real transfer, edge-compatible architectures, and safety frameworks.

Conclusion: These insights advance embodied AI for IoT-connected environments where intelligent agents interact with networked sensors, actuators, and edge devices, providing a comprehensive framework for understanding and developing GPAI systems.
...
### [33] [OpenDexGrasp: Open-vocabulary Task-Oriented Dexterous Grasping](https://arxiv.org/abs/2609.18117)
...
### [34] [Fetch My Beer: Synthetic-to-real Hierarchical Policy for Smooth Pick-and-place](https://arxiv.org/abs/2609.18119)
*Yingyue Li,Chenyangguang Zhang,Ruida Zhang,Bowen Fu,Guangyao Zhai,Xiangyang Ji*

Main category: cs.RO

TL;DR: A synthetic-to-real framework with a hierarchical, diffusion-based controller for stable transport of liquid-filled containers, addressing limitations of existing approaches in fluid simulation cost, unintended accelerations from teleoperation, and task completion vs. dynamic stability trade-offs.


<details>
  <summary>Details</summary>
Motivation: Real-world robotic applications require dynamically sensitive manipulation where success depends on maintaining stable object dynamics. The paper studies stable transport of liquid-filled containers, requiring robots to move objects while suppressing sloshing and preventing spillage. Unlike conventional pick-and-place, this task imposes stringent requirements on motion smoothness and trajectory-level stability. Existing systems have clear limitations: (1) fluid simulation remains too costly for online reinforcement learning; (2) human teleoperation introduces unintended accelerations that induce sloshing during imitation learning; (3) current policy pipelines optimize for task completion rather than dynamic stability.

Method: Proposes a synthetic-to-real framework coupling physically validated data generation with a hierarchical, diffusion-based controller. The scalable data pipeline synthesizes grasps, filters unstable poses via a vision-language model, and validates transport trajectories through fluid simulation. The policy architecture consists of: (1) a high-level module that translates language and visual observations into SE(3) control targets, and (2) a latent diffusion controller that plans efficiently in a compact latent space and decodes dense action chunks, enabling high control frequency needed for smooth and stable motion.

Result: Extensive experiments demonstrate that the proposed system outperforms state-of-the-art manipulation policies in transport smoothness and dynamic stability.

Conclusion: The proposed framework successfully addresses the limitations of existing systems by integrating physically validated synthetic data generation with a hierarchical diffusion-based controller, enabling robots to perform stable transport of liquid-filled containers with superior motion smoothness and dynamic stability compared to current approaches.
...
### [35] [Prior Evolution and Task Alignment for Aerial Grasping](https://arxiv.org/abs/2609.18153)
...
### [36] [Energy-Regularized Imitation Learning for Force- and Work-Aware Robotic Manipulation](https://arxiv.org/abs/2609.18164)
*Toshiki Otani,Hiromu Taketsugu,Norimichi Ukita*

Main category: cs.RO

TL;DR: This paper introduces an energy-aware manipulation framework that uses a differentiable energy predictor as a regularizer to fine-tune pretrained manipulation policies, reducing mechanical work by 2.1% while maintaining task success rates.


<details>
  <summary>Details</summary>
Motivation: The authors aim to improve manipulation policies by incorporating physical efficiency (mechanical work) without requiring explicit differentiable dynamics models, addressing the challenge of optimizing for physically grounded objectives in robot learning.

Method: The framework defines a joint-space mechanical-work proxy from joint torque and angular displacement, trains a differentiable energy predictor from robot states and actions, and uses this predictor to create a regularizer for fine-tuning a pretrained policy (RVT-2) on 12 RLBench manipulation tasks.

Result: The fine-tuning reduced average mechanical work from 208.8J to 204.4J (2.1% reduction) while slightly increasing mean task success rate from 86.2% to 86.9%, demonstrating that work-aware optimization can suppress inefficient motions.

Conclusion: Energy-aware policy optimization using a differentiable work proxy effectively reduces physical inefficiency in manipulation tasks without needing explicit differentiable dynamics models, showing the practical value of incorporating physically grounded regularizers into robot learning.
...
### [37] [LUMO: Designing Luminous Contact Morphology for Repeatable Whole-Finger Contact Observation](https://arxiv.org/abs/2609.18165)
...
### [38] [Characterizing Replay Retention Under Dynamics Shift in Model-Based Reinforcement Learning](https://arxiv.org/abs/2609.18167)
...
### [39] [Approximating High Dimensional Self-Motion Manifolds via Deep Generative Models](https://arxiv.org/abs/2609.18169)
*Haitao Gao,Yang Song,Liao Wu*

Main category: cs.RO

TL;DR: This paper introduces a probabilistic framework for approximating self-motion manifolds (SMMs) of redundant manipulators by treating them as the support of a conditional posterior distribution, enabling sampling and clustering to recover SMMs regardless of dimensionality.


<details>
  <summary>Details</summary>
Motivation: Existing methods (null-space continuation and learning-based approaches) assume SMMs are curves and fail to extend to higher redundancy orders; there is a need for a dimension-independent approach that can handle highly redundant manipulators.

Method: The authors adopt a probabilistic view where SMMs represent the support of the conditional posterior over configurations given a target pose. Recovering SMMs reduces to sampling from a learned distribution and separating disjoint components via clustering. This formulation is independent of manifold dimension and requires no architectural changes as redundancy increases.

Result: The method approximates 1-D SMMs with performance comparable to the latest null-space continuation and learning-based approaches, and is the first method capable of approximating highly redundant 4-D SMMs in a 7R (7-revolute) manipulator for position tasks.

Conclusion: The proposed probabilistic approach provides a dimension-independent method for SMM approximation that scales to higher redundancy orders without architectural modifications, enabling feasible and globally optimal motion planning for highly redundant manipulators.
...
### [40] [TacBPM: A Tactile-conditioned Behavior Prior Model for Dexterous Reorientation](https://arxiv.org/abs/2609.18174)
...
### [41] [OmniRisk: Omnidirectional Trajectory-Risk Learning for Agile Quadrotor Dynamic Avoidance](https://arxiv.org/abs/2609.18191)
*Yifan He,Yang Liu,Wenhao Zhao,Hai Lin,Deping Zhang,Mingze Ma,Fei Gao,Huan Yu,Zipeng Dai,Ziming Ding*

Main category: cs.RO

TL;DR: OmniRisk is an omnidirectional planning framework that learns trajectory-level risk offline for efficient onboard evasion of fast-moving obstacles by quadrotors.


<details>
  <summary>Details</summary>
Motivation: Agile quadrotor avoidance of fast-moving obstacles requires anticipating collisions and selecting feasible maneuvers within short reaction windows. Challenges include: sparse range observations do not directly reveal obstacle motion, and online trajectory optimizers either scale poorly with obstacle count or sacrifice reliability in dense, high-speed encounters.

Method: OmniRisk uses a fixed-dimensional tensor combining LiDAR range panoramas, dynamic masks, and Cartesian surface velocities. It formulates an asymmetric risk field aligned with obstacle velocity that emphasizes approaching interactions and attenuates receding ones. A dual-branch circular convolutional network predicts terminal boundary states and dynamic risks for candidate primitives over an omnidirectional anchor lattice in a single forward pass, followed by selection and closed-form reconstruction of the selected candidate primitive.

Result: Real-world flights demonstrate consecutive evasive maneuvers at relative encounter speeds up to 15 m/s without fine-tuning.

Conclusion: OmniRisk enables efficient onboard avoidance by removing online risk accumulation along trajectories and making risk-inference cost independent of obstacle count.
...
### [42] [WAVE-Go: World-Model Navigation with Adaptive Execution for Wheel-Legged Robots](https://arxiv.org/abs/2609.18193)
*Mingyi Li,Ji Li,Zhihao Ouyang,Yage He,Börje F. Karlsson*

Main category: cs.RO

TL;DR: WAVE-Go is an image-goal navigation framework that separates world-action prediction from interruptible command execution for wheel-legged robots, adaptively selecting and canceling action prefixes when observations invalidate execution.


<details>
  <summary>Details</summary>
Motivation: World models can predict action consequences but predicted sequences become invalid during execution, especially for wheel-legged robots encountering dynamic obstacles or changing locomotion modes. Existing approaches lack mechanisms to handle execution invalidation.

Method: WAVE-Go separates world-action prediction from execution. It uses a conditional-risk formulation for prefix selection based on an estimated cumulative failure budget, plus clearance, stability, and task-evidence checks for posture and locomotion-mode transitions. The executor can interrupt and cancel pending commands when observations invalidate execution.

Result: In evaluation, WAVE-Go achieved 74.1% in-distribution success and 63.3% dynamic out-of-distribution success (4.7 and 7.7 percentage points above the best baseline), reduced collisions from 4.4 to 2.9 per 100m, and outperformed interruptible fixed four-command execution with 4.0% higher success, 51.2% lower replanning frequency, and 6.5% lower collision rate.

Conclusion: Adaptive, interruptible execution effectively balances navigation performance and planning overhead, improving success rates while reducing collision frequency and replanning requirements.
...
### [43] [WholeBodyWAM: Learning Whole-Body World Action Models with Scalable Motion Priors](https://arxiv.org/abs/2609.18197)
...
### [44] [Reinforcement Learning for Real-Time Vision-Language-Action Policies](https://arxiv.org/abs/2609.18207)
*Perry Dong,Kuo-Han Hung,Dorsa Sadigh,Chelsea Finn*

Main category: cs.RO

TL;DR: Real-Time EXPO-FT is a reinforcement learning framework that decouples slow, expressive action generation from fast, reactive action edits, enabling sample-efficient fine-tuning of large Vision-Language-Action models for real-time robot manipulation.


<details>
  <summary>Details</summary>
Motivation: Large VLA models suffer from high inference latency, causing observation staleness and distribution shift that degrades reliability. Prior asynchronous methods are imitation-learning-based and lack mechanisms to move beyond the training distribution toward higher reliability.

Method: Builds on EXPO-FT framework, decoupling slow action generation from fast reactive edits: a large pretrained VLA proposes action chunks via behavior prior, while a lightweight edit policy performs fast reactive decision-making by editing actions based on latest observations.

Result: On Kinetix benchmark: best performance among delayed and non-delayed methods in 10/10 environments. On 4 dynamic real-world tasks (object passing, ball balancing, table soccer kicking, dynamic object picking): improves average performance from 42% to 97% with only 10 minutes of online robot data, requiring no human intervention.

Conclusion: Real-Time EXPO-FT enables rapid, sample-efficient adaptation to challenging real-world dynamics for robot manipulation, bridging the gap between large-scale VLA models' expressiveness and real-time control requirements.
...
### [45] [CANTABILE: Learning Expressive Dynamics for Robotic Piano Performance](https://arxiv.org/abs/2609.18213)
*Woosik Kim,Wonhyeok Choi,Sunghoon Im*

Main category: cs.RO

TL;DR: CANTABILE is a dynamics-aware framework that improves robotic piano performance by incorporating velocity fidelity and onset coverage rewards, mapping key angular velocity to MIDI velocity, and using a finger-only residual policy to refine strike-intensity adaptation, achieving a 4.6x improvement in Velocity F1 score on the EXPRESSIVE-51 dataset.


<details>
  <summary>Details</summary>
Motivation: Robotic piano playing is a standard benchmark for dexterous bimanual manipulation, but progress has been measured only by note accuracy (pitch and onset), leaving musical dynamics essential for expressive performance neither rewarded nor evaluated.

Method: Three key innovations: (1) closes score-to-contact loop by conditioning policy on velocity goals and mapping key angular velocity to MIDI velocity; (2) couples velocity-fidelity reward with onset-coverage reward to prevent dynamics improvement via note omission; (3) refines frozen dynamics-aware base policy with alpha-scaled finger-only residual for localized strike-intensity adaptation.

Result: On EXPRESSIVE-51 dataset, CANTABILE raises Velocity F1 from 0.06 to 0.34 (4.6x improvement over RoboPianist baseline), improves all 51 songs, halves matched-note velocity error, and reduces log-mel distance to reference audio by 8%. Intensity-randomized training enables runtime control of performance intensity without retraining.

Conclusion: The CANTABILE framework successfully integrates dynamics-aware training into robotic piano performance, demonstrating that incorporating intensity rewards and velocity-goal conditioning significantly improves expressive performance quality and enables runtime intensity control.
...
### [46] [UMI-Bridge: Action-Anchored Latent Alignment across Human and Robot Manipulation Data](https://arxiv.org/abs/2609.18232)
...
TL;DR: UMI-Bridge aligns representations across human and robot manipulation data using a wrist-based latent action model, enabling effective transfer of manipulation skills.


<details>
  <summary>Details</summary>
Motivation: Real-robot demonstrations are limited. Human manipulation data (egocentric videos and UMI demonstrations) have differences in viewpoint, embodiment, and action supervision that make alignment difficult.

Method: UMI-Bridge uses UMI as an intermediate domain to align representations based on action equivalence rather than visual appearance. It uses UMI action supervision to anchor the latent representation to end-effector motion and gripper behavior. A dual-view latent action model (LAM) is trained on human manipulation data without robot demonstrations. The wrist teacher and dynamics model from LAM are frozen and used to regularize VLA post-training on UMI and robot data.

Result: Across three real-robot tasks, UMI-Bridge achieves 91.7% mean success versus 73.3% for Naive Co-training. On two data-efficiency tasks, it surpasses a full-data Robot-only baseline using 25% of robot demonstrations. It also achieves 85% and 90% success on two additional tasks learned from UMI demonstrations without task-specific robot demonstrations.

Conclusion: Action-anchored latent alignment enables data-efficient robot learning and effective UMI-to-robot task transfer.
...
### [47] [ForceDelta-VLA: Distilling Force-Conditioned ActionCorrections for Contact-Rich Manipulation](https://arxiv.org/abs/2609.18242)
...
### [48] [Acting in Meters: Learning Metric Interactions for Precise Robotic Manipulation](https://arxiv.org/abs/2609.18243)
...
TL;DR: This paper introduces a metric interaction framework that explicitly models object-level and scene-level interactions in Cartesian space, using Interaction-Centric Tokens (ICTs) and a Metric Action Interaction Field (MAIF) to enhance vision-language-action and world-action models for robotic manipulation.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language-action (VLA) and world-action models (WAM) often leave metric relations among actions, objects, and scene geometry implicit. Human manipulation combines semantic understanding with spatial feedback that guides hand motion relative to objects and surroundings. The authors aim to bridge this gap by introducing a framework that models these metric interactions explicitly.

Method: The framework consists of two main components: (1) At the object level, Interaction-Centric Tokens (ICTs) explicitly represent end-effector pose trajectories relative to manipulated objects and are jointly denoised with actions, providing physically grounded interaction supervision. (2) At the scene level, the Metric Action Interaction Field (MAIF) uses action and ICT queries to attend to metric scene point-cloud features and learns geometry-conditioned action corrections. The approach uses two-stage adaptation to improve VLA and WAM baselines with minimal additional parameters and training steps.

Result: Experiments demonstrate significant performance improvements across multiple benchmarks: average success-rate gains of 0.80 percentage points on LIBERO and 3.59 percentage points on RoboTwin 2.0, gains of 6.80 percentage points on real-world tasks, and 7.45 percentage points on out-of-distribution variants.

Conclusion: The proposed metric interaction framework effectively enhances robotic manipulation performance by explicitly modeling spatial relationships at both object and scene levels. The approach demonstrates that adding a small number of parameters and training steps can significantly improve diverse VLA and WAM baselines, bridging the gap between semantic understanding and metric spatial feedback in robotic manipulation.
...
### [49] [A3P5 NEMESIS Integrated Rover Design for Environmental Reconnaissance and Robotic Sampling with Reproducible Mobility Analysis and an External Data Machine Learning Calibration Benchmark](https://arxiv.org/abs/2609.18245)
...
### [50] [${M}^2$Tok: Multi-head Multi-codebook Discrete Action Tokenization for Vision-Language-Action Models](https://arxiv.org/abs/2609.18259)
*Chunpu Xu,Zhixuan Liang,Yuhao Zhang,Chi-Min Chan,Jessie Wang,Yang Xiao,Mengkang Hu,Xiaokang Yang,Yao Mu*

Main category: cs.RO

TL;DR: M²Tok is a Multi-head Multi-codebook Action Tokenizer designed to address the discretization bottleneck in multimodal autoregressive models by decomposing latent action features into multiple heads with independent codebooks, achieving superior reconstruction fidelity and boosting VLA policy performance.


<details>
  <summary>Details</summary>
Motivation: Existing discrete action tokenizers suffer from high reconstruction loss and fail to preserve fine-grained dynamics required for precise control, creating a discretization bottleneck that limits the performance ceiling of downstream Vision-Language-Action (VLA) models.

Method: M²Tok introduces two structural innovations: (1) decomposing latent action features into multiple heads to implicitly align specific heads with distinct action dimensions; (2) assigning independent codebooks to each head for quantization, leveraging the combinatorial nature of multiple codebooks to expand representational expressivity.

Result: M²Tok-based VLA models achieve superior reconstruction fidelity and significantly boost success rates on RoboTwin, Simpler-Env, and three zero-shot real-world tasks compared to previous methods.

Conclusion: Comprehensive ablation studies confirm the effectiveness of both the multi-head and multi-codebook mechanisms, demonstrating that M²Tok effectively addresses the discretization bottleneck and enhances VLA policy performance.
...
### [51] [Function-Preserving Data Generation for Zero-Shot Real-to-Sim-to-Real Manipulation](https://arxiv.org/abs/2609.18293)
...
### [52] [RAFAIL: Relationship-Aware Failure Detection for Robotic Manipulation](https://arxiv.org/abs/2609.18324)
...
### [53] [UAVs Meet Embodied Intelligence: Bridging Human Intents and Flying Dynamics Via Harnessing Physical-Digital AI Agents](https://arxiv.org/abs/2609.18326)
...
TL;DR: This paper introduces UAV embodied intelligence (UAV EI) and embodied-intelligent UAV (EI UAV) through a unified 5+5 framework, systematically reviewing recent progress across multiple domains and identifying key challenges for advancing aerial autonomy.


<details>
  <summary>Details</summary>
Motivation: UAVs are evolving from task-specific systems to general autonomy capable of interpreting human intent, understanding open environments, reasoning about physical consequences, and organizing complex behaviors under flight-dynamic constraints. A unified framework is needed to characterize this emerging paradigm.

Method: Introduces a 5+5 framework with five capability dimensions for UAV EI and five architectural layers for EI UAVs spanning physical embodiment, general cognition, embodied skills, external interaction, and system harnessing. Systematically reviews progress in embodied morphology, perception, world models, planning, vision-language navigation, manipulation, and collaboration.

Result: Provides comprehensive overview of recent advances in UAV EI across multiple subdomains. Identifies four key challenges: long-horizon autonomy, predictive physical reasoning, test-time skill acquisition, and autonomous capability evolution.

Conclusion: Harnessing physical-digital AI agents through persistent coupling of digital intelligence with physical sensing, dynamics, action, and feedback provides a system-level pathway toward adaptive and continuously evolving UAV autonomy.
...
### [55] [RecMorph: Topology-Guided Spatial Recurrence for Generalized Morphology Control](https://arxiv.org/abs/2609.18359)
...
### [56] [Decoupling Vision, Language, and Action for Efficient Multi-Task Robot Policies](https://arxiv.org/abs/2609.18374)
...
### [57] [DistAL: Distance-based Advantage Learning for VLA Fine-Tuning](https://arxiv.org/abs/2609.18392)
*Reece O'Mahoney,Ioannis Havoutis*

Main category: cs.RO

TL;DR: DistAL uses embedding space distance as a reward signal to improve Vision-Language-Action models by creating more informative value functions, leading to higher success rates on both simulation benchmarks and real dexterous bi-manual manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Previous VLA training methods relied on simple success/failure rewards that provided limited information to distinguish states of differing quality. The authors were motivated by exploration of out-of-distribution detection methods to find a more informative reward signal.

Method: Distance-based Advantage Learning (DistAL) introduces an embedding space distance metric as a reward signal, which provides more granular information about state quality compared to binary success/failure signals. This enables the value function to better distinguish between states at different stages of task completion.

Result: DistAL achieved higher downstream task success rates. The method was validated on simulation benchmarks and demonstrated effectiveness on real dexterous bi-manual manipulation hardware tasks.

Conclusion: By replacing simple binary rewards with embedding space distance-based rewards, DistAL produces a more informative value function that leads to significant improvements in VLA performance across both simulated and real-world robotic manipulation tasks.
...
### [58] [DetAug: Obstacle-Blind Trajectory Augmentation for Zero-shot Obstacle Avoidance](https://arxiv.org/abs/2609.18395)
...
### [59] [Hardware-Free Robotics Laboratories in Mixed Reality](https://arxiv.org/abs/2609.18434)
*Santiago Berrezueta-Guzman,Habiba-Loai Khalil,Andrei Koshelev,Vanesa Metaj,Stefan Wagner*

Main category: cs.RO

TL;DR: MR-Robotics LAB is a mixed-reality platform that enables replaying MATLAB-generated robot trajectories at real scale in learners' physical spaces, validated through a formative evaluation showing high perceived usability and willingness to adopt.


<details>
  <summary>Details</summary>
Motivation: Screen-based simulation abstracts robot motion and lacks real-scale visualization, while access to physical hardware is constrained by cost, safety, and scheduling, creating barriers for robotics education.

Method: A browser-based service validates MATLAB workspace files, normalizes units, and publishes versioned JSON trajectories; a Unity application on Meta Quest 3 reproduces joint configurations under position control within a physics-enabled scene supporting collision detection and end-effector grasping.

Result: Engineering students reported low setup effort (M = 4.67/5), perceived strong support for workspace understanding from multi-viewpoint inspection (M = 4.56/5), and 83% affirmed willingness to use the platform in an introductory robotics course.

Conclusion: The contribution is a reusable simulation-to-MR trajectory pathway and design guidance for hardware-free robot visualization in engineering education, though the evaluation records only perceived outcomes without comparative advantage over desktop simulation or learning measures.
...
### [60] [VLM-MPPI: Grounding Natural Language in Behaviorally Diverse Trajectories for Aerial Navigation](https://arxiv.org/abs/2609.18451)
*Hanbing Zhang,Fangguo Zhao,Zerui Li,Xin Guan,Peng Cheng,Shuo Li*

Main category: cs.RO

TL;DR: A hierarchical UAV navigation framework that uses a parallel ensemble of behavior-conditioned MPPI planners, guided by a pretrained vision-language model, to align natural language intent with feasible flight behaviors in cluttered indoor environments.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between abstract natural language semantics and low-level flight control in cluttered indoor environments, enabling drones to follow diverse behavioral modes specified by natural language.

Method: Uses a parallelized ensemble of six behavior-conditioned MPPI planners with mode-specific guiding costs and sampling biases to induce distinct trajectory modes. Projects 3D candidates onto FPV RGB stream and employs a pretrained vision-language model (VLM) to select candidates from the overlaid FPV image and natural language prompt. MPPI replans at 20Hz with a PID-based low-level controller tracking.

Result: Achieves semantically meaningful behavior diversity, robust language alignment despite VLM latency, and safe, repeatable flight across all modes with 100% task success in evaluated scenarios.

Conclusion: The proposed framework successfully aligns natural language intent with dynamically feasible flight behaviors in cluttered indoor environments, demonstrating that intentionally diverse behavioral modes can be effectively generated and selected through the combination of behavior-conditioned MPPI planners and vision-language models.
...
### [61] [ForwardDLO: Model-Based Bimanual Shape Matching of Unconstrained Deformable Linear Objects](https://arxiv.org/abs/2609.18455)
*Tim Missal,Berk Guler,Lucas Domingues,Simon Manschitz,Jan Peters,Paula Dornhofer Paro Costa*

Main category: cs.RO

TL;DR: ForwardDLO is a recurrent latent dynamics model for controlling deformable linear objects (ropes) in unfixated bimanual settings, achieving high prediction accuracy and enabling efficient action planning.


<details>
  <summary>Details</summary>
Motivation: Controlling shape of deformable linear objects remains challenging in robot manipulation. The problem is complex because objects lie unfixated on support surfaces and two arms may grasp and move it anywhere along its length. The joint action space is combinatorially large, and the dynamics model's per-prediction cost limits how much of it a planner can search.

Method: ForwardDLO is a recurrent latent dynamics model that predicts per-segment displacements grounded in the observed rope state at every step. It contains no explicit segment-to-segment operations, which makes batched evaluation of candidate actions cheap.

Result: On open-loop prediction of real rope motion, ForwardDLO reaches 13% lower error than the strongest baseline. Within a fixed time budget, it scores 8 to 22 times more candidate actions than models of comparable accuracy while matching them in real-world shape matching. On a simulated routing task at 30Hz, it achieves 98% task success versus at most 30% for baselines.

Conclusion: ForwardDLO provides an efficient solution for model-based shape control of deformable linear objects, enabling effective planning and high success rates in real-world manipulation tasks. The authors release the model, code, and a dataset of 2.42 million simulated and 14,107 real rope transitions.
...
### [62] [Real-Time Bounded Catenary Solver for UAV Tether Modeling](https://arxiv.org/abs/2609.18482)
...
### [63] [ActionPiece: Rethinking Action Tokenization for Autoregressive Vision-Language-Action Models](https://arxiv.org/abs/2609.18487)
...
### [64] [TAO-Force: Unifying Force-Aware Perception and Fast-Slow Control for Contact-Rich Manipulation](https://arxiv.org/abs/2609.18497)
...
### [65] [ActiveScale: Scaling Active Perception for Robots across Model, Data, and Hardware](https://arxiv.org/abs/2609.18514)
*Shuai Zhou,Kaisheng Pang,Wenxuan Song,Wenjie Zhang,Xinhu Zheng,Haoang Li*

Main category: cs.RO

TL;DR: ActiveScale is a framework that advances active perception in robotic manipulation by augmenting vision-language-action (VLA) models with historical video observations and explicit camera-pose supervision. It uses per-frame pose tokens and a lightweight prediction head to associate observations across changing viewpoints, and introduces a scalable mid-training recipe with 1000 hours of egocentric and robotic data, along with the Active-perception Mobile-manipulation Platform (AMP) for coordinated viewpoint changes and manipulation.


<details>
  <summary>Details</summary>
Motivation: Active perception is essential for robotic manipulation when fixed viewpoints leave task-relevant information occluded or unobserved. However, enabling vision-language-action (VLA) models to reason across changing viewpoints and actively acquire informative observations remains challenging.

Method: The framework consists of three key components: (1) Model design that augments VLA with historical video observations and explicit camera-pose supervision, using per-frame pose tokens and a lightweight prediction head to associate observations across viewpoints; (2) Scalable human-robot mid-training recipe using 1000 hours of egocentric and robotic data to adapt the model to temporal inputs and pose supervision; (3) Active-perception Mobile-manipulation Platform (AMP) supporting single-operator teleoperation for coordinated viewpoint changes and manipulation.

Result: Experiments demonstrate improved success rates on active-perception tasks, while ablation studies validate the contributions of camera-pose-aware modeling and egocentric mid-training.

Conclusion: Together, the model, data, and hardware components provide an integrated foundation for studying and developing active perception in robotic manipulation, advancing the state of VLA models in handling dynamic viewpoints and actively acquiring informative observations.
...
### [66] [DynoFluxBench: Benchmarking Kinodynamic Space-Time Planners in Dynamic Environments](https://arxiv.org/abs/2609.18549)
...
### [67] [GroundingVLN: Reasoning and Acting with Grounding for Vision-Language Navigation](https://arxiv.org/abs/2609.18581)
*Kailing Li,Yu Han,Tianwen Qian,Yuqian Fu,Jingyu Gong,Jiangming Shi,Xiaoling Wang*

Main category: cs.RO

TL;DR: GroundingVLN uses visual grounding as a shared interface between reasoning and action in vision-and-language navigation, anchoring intermediate reasoning to visual evidence and generating progress-aligned pixel goals for precise spatial execution.


<details>
  <summary>Details</summary>
Motivation: Existing VLN agents struggle to connect semantic reasoning with spatial execution due to two gaps: intermediate reasoning not anchored to visual evidence and high-level decisions lacking precise spatial goals. Cognitive science suggests humans bridge these levels hierarchically by anchoring cognition to landmarks and guiding locomotion toward spatial goals.

Method: GroundingVLN employs visual grounding as a shared interface between reasoning and action. It reasons with grounding by anchoring task-relevant visual evidence to precise image locations throughout structured reasoning, then acts through grounding by predicting progress-aligned pixel goals translated into primitive actions via geometric planning. The authors construct GroundingCOTVLN-188K dataset (temporally aligned grounded reasoning traces) and introduce Grounded and Execution-Aware Reinforcement Learning (GEAR) to align grounded reasoning and spatial decisions with downstream execution.

Result: GroundingVLN achieves state-of-the-art performance with high sample efficiency: 69.9% SR on R2R-CE and 75.1% SR on RxR-CE, using only 0.9% as much training data as the strongest baseline. It generalizes strongly across datasets, attaining 59.9% SR on RxR-CE when trained solely on R2R (20.1% gain over strongest baseline).

Conclusion: GroundingVLN effectively bridges the gap between semantic reasoning and spatial execution in VLN through visual grounding, demonstrating superior performance, exceptional sample efficiency, and strong cross-dataset generalization capabilities.
...
### [68] [DeformSmith: Physics Harness-Guided Hierarchical Generation of Deformable Assets for Robot Manipulation](https://arxiv.org/abs/2609.18620)
*Can Li,Jie Gu,Zishun Deng,Jingmin Chen,Lei Sun*

Main category: cs.RO

TL;DR: DeformSmith is a framework for automatically generating interactive, physically credible deformable assets for robot manipulation from text or a single image, using hierarchical construction and physics-grounded methods with robot interaction feedback.


<details>
  <summary>Details</summary>
Motivation: Creating deformable assets requires jointly specifying geometry, appearance, and physical properties, but text and images provide limited evidence about how objects deform and respond to contact—critical for robotic interaction suitability. Automated generation must resolve coupled physical requirements while using interaction evidence to guide construction and refinement.

Method: DeformSmith employs hierarchical agentic construction and a shared physics-grounded harness to progressively build, test, and refine geometry, physical models, material behavior, and robot interaction. It closes the generation loop through robot interaction feedback and replayable interaction data, enabling iterative refinement until assets are ready for simulation and manipulation.

Result: DeformSmith generates assets with superior visual quality and physical plausibility compared to state-of-the-art baselines (PhysGen3D, PhysGM, PhysX-Omni), while successfully synthesizing data for robotic manipulation of deformable objects.

Conclusion: DeformSmith demonstrates effective automated generation of interactive, physically credible deformable assets, successfully addressing the coupled physical requirements challenge and enabling data synthesis for robotic manipulation applications.
...
### [69] [Benchmarking Visual-Inertial Odometry in Subterranean Environments Under Sensor Degradation, Miscalibration, and Dynamic Occlusion](https://arxiv.org/abs/2609.18628)
*Yueying Zhu,Xiang Li,Thien-Minh Nguyen,Xuehe Wang,Shenghai Yuan*

Main category: cs.RO

TL;DR: A failure-centric stress-test benchmark for visual-inertial odometry (VIO) in GPS-denied subterranean environments using the CERBERUS dataset, systematically evaluating four VIO systems across nine practical perturbation settings to reveal vulnerability patterns and provide deployment-oriented guidance.


<details>
  <summary>Details</summary>
Motivation: VIO reliability degrades sharply under sensor drift, calibration errors, and dynamic occlusion in GPS-denied underground environments; existing evaluations focus on nominal-condition accuracy but lack insight into practical deployment failures.

Method: Systematically evaluates four representative VIO systems (filtering-, optimization-, and learning-based paradigms) under nine practical perturbation settings: IMU bias and noise variation, camera intrinsic and extrinsic drift, and dynamic scene occlusion using the CERBERUS dataset. Metrics include conventional trajectory error, coverage ratio, and failure thresholds.

Result: Distinct vulnerability patterns across VIO paradigms: some methods are more sensitive to inertial degradation, while others are more affected by geometric miscalibration or dynamic interference. Failure behaviors are revealed that are not captured by nominal-condition performance alone.

Conclusion: Provides deployment-oriented guidance for VIO selection, calibration prioritization, and reliable operation in challenging underground scenarios. The study includes full benchmark scripts and evaluation pipeline for reproducible evaluation and future extensions.
...
### [70] [From Gameplay to Policy: Towards Scalable Robot Data Collection via Gamified Robot-Free Interaction](https://arxiv.org/abs/2609.18650)
...
### [71] [FIERCE: From Generalist Robot Policies to Fast Specialists via Progress-Failure Feedback](https://arxiv.org/abs/2609.18651)
*Runjia Tan,Yuang Tu,Yujie Yan,Lan Yu,Xuesong Tian,Chen Lv*

Main category: cs.RO

TL;DR: FIERCE is a generalist-initialized RL framework that uses a unified progress-failure evaluator to refine compact specialists through limited physical interaction, eliminating the need for continued generalist queries, dedicated simulators, or manual dense rewards.


<details>
  <summary>Details</summary>
Motivation: Generalist robot policies provide useful initialization, but refining compact specialists through limited physical interaction requires informative learning feedback that is difficult to obtain.

Method: FIERCE employs a unified task-adaptive progress-failure evaluator with an observation-language representation shared between an observed-progress head, action-conditioned latent predictor, and causal sequence head. Joint supervision from progress and preference labels, synchronized commands/observations, and terminal outcomes trains the evaluator, while target-task rollouts enable adaptation. Fixed evaluator snapshots provide progress shaping, failure-risk penalties, and terminal rewards. Evaluator and policy updates alternate during experience collection.

Result: The framework was evaluated across simulation and two contact-rich real-world tasks, demonstrating effectiveness in separating feedback quality, policy-learning efficiency, and deployment cost.

Conclusion: FIERCE enables refinement of compact specialists without requiring continued generalist action queries, dedicated target-task simulators, or manually annotated dense rewards. Only the compact specialist is retained at deployment, making it cost-effective and practical for real-world applications.
...
### [72] [VLA-ULAP: Interleaving Cloud VLA Calls with Ultra-Lightweight Local Action Prediction at the Edge](https://arxiv.org/abs/2609.18663)
*Deyu Cao,Ryuji Oi,Kosuke Matsushima,Yuxuan Pan,Ziheng Wang,Daichi Fujiki,Atsutake Kosuge*

Main category: cs.RO

TL;DR: VLA-ULAP interleaves remote VLA calls with an Ultra-Lightweight Local Action Predictor (ULAP) to reduce onboard power consumption and latency while maintaining high success rates in robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Billion-parameter vision-language-action (VLA) policies require substantial onboard power, and communication delays in remote inference hinder timely responses.

Method: VLA-ULAP combines remote VLA calls with ULAP (≈7.4M parameters including frozen vision encoder). ULAP predicts action chunks in one pass using current views, proprioception, and executed action history. Trained independently, it requires no VLA hidden states, online verification, or server round trips.

Result: ULAP achieves 19.9 ms/0.183 J per inference on Jetson Orin Nano vs. 284.3 ms/50.55 J for GR00T on RTX A6000. Removes 48.8-76.7% of VLA calls while retaining 95.0-97.5% baseline success rate. Uses 49.2% less time/energy than ACT, and 77.1%/79.9% less than SP-VLA. Physical SO-101 experiments retain 95.2-100% success while reducing time by 47.9-58.0% and energy by 52.1-62.5%.

Conclusion: VLA-ULAP effectively balances computational efficiency and task performance, significantly reducing power consumption and latency while maintaining high success rates across simulated and physical environments.
...
### [73] [M$^3$P-R1: Reinforcement Learning for Large Language Model Guided Multi-Modal Motion Planning via MIP Code Generation](https://arxiv.org/abs/2609.18669)
...
### [74] [WeaveRL: Weaving Reconstruction into Scene-Aware Fabrics for Perceptive Reinforcement Learning](https://arxiv.org/abs/2609.18685)
...
TL;DR: A GPU-accelerated method reconstructs scenes as surfels during RL training, enabling policies to operate on sensor-derived geometry instead of hand-specified representations, improving robustness to novel geometry from 35% to 61% in collision-dense manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning enables robots to acquire complex skills, but producing policies for geometrically complex manipulation remains difficult. Existing collision-avoidant controller approaches (like geometric fabrics) rely on static, hand-specified scene representations, limiting their effectiveness. There's a need to integrate active, online 3D perception into massively parallel RL training.

Method: The paper introduces a GPU-accelerated method that reconstructs the scene as a collection of surfels across thousands of parallel simulation instances during active rollouts. This allows policies to operate over sensor-derived geometry rather than hand-specified geometry, integrating active, online 3D perception into RL training.

Result: On a suite of collision-dense manipulation tasks, surfel fabrics enable policies to handle geometrically complex scenes where primitive-based baselines fail. Policies learned with scene-aware fabric are more robust to novel geometry at test time, improving collision-free task completion under unseen obstacles from 35% to 61%.

Conclusion: The reconstruction system, training code, and test dataset are released to spur research. The approach maintains sim-to-real transfer, enabling policies to tackle geometrically complex scenes while preserving real-world applicability.
...
### [75] [Toward 3D Printable Non-Planar Electroadhesive Structures for Active Anchoring](https://arxiv.org/abs/2609.18700)
*Mostafa A. Atalla,Carol R. Derla,Pep Canyelles Pericàs,Gijs Krijnen*

Main category: cs.RO

TL;DR: This paper investigates multi-material 3D printing to fabricate non-planar electroadhesion structures with integrated printed electrodes, demonstrating voltage-controlled adhesion in both flat and cylindrical configurations.


<details>
  <summary>Details</summary>
Motivation: To advance beyond conventional planar electroadhesion pads by developing printed parts with built-in, electrically controlled adhesion and friction capabilities using multi-material 3D printing.

Method: Multi-material 3D printing was used to fabricate flat electroadhesion pads (planar benchmark) and cylindrical pads (non-planar demonstration), with conductive interdigitated electrodes printed as part of the structure. Normal-force measurements were performed to evaluate performance at different voltages and preload conditions.

Result: Both geometries exhibited voltage-controlled modulation of normal force. At 3 kV, flat pads generated approximately 0.11-0.13 N, while cylindrical pads reached approximately 0.16 N at 5 N preload.

Conclusion: The results demonstrate that multi-material 3D printing can create structures with built-in, electrically controlled adhesion and friction, representing a significant step beyond conventional planar electroadhesion pads.
...
### [76] [Calibrated Probabilistic Obstruction Reasoning with Vision-Language Models for Grasping in Clutter](https://arxiv.org/abs/2609.18718)
*Thanh-Tuan Tran,Ngoc-Chien Chu,Thanh Nguyen Canh,Nak Young Chong,Nguyen-Viet Ha,Xiem HoangVan*

Main category: cs.RO

TL;DR: CPOR-Grasp is a calibrated probabilistic obstruction-reasoning framework for robotic grasping that fuses VLM, depth, and amodal-mask cues to estimate obstruction probabilities and makes tractable decisions with certified guarantees.


<details>
  <summary>Details</summary>
Motivation: Existing methods commit to single obstruction graphs/strategies, ignore scene interpretation uncertainty, rely on miscalibrated VLM predictions, produce inconsistent pairwise obstruction relations, and provide no guarantees about discarded hypotheses.

Method: CPOR-Grasp calibrates and fuses VLM, depth, and amodal-mask cues to estimate obstruction probabilities, induces a distribution over valid obstruction graphs, and marginalizes over these graphs. It retains only highest-probability graphs and derives a total-variation bound on discarded probability mass for tractable inference, certified decisions, adaptive stopping, and principled deferral.

Result: On synthetic and real UNOBench scenes, CPOR-Grasp outperforms SOTA baselines. Calibration error decreases from 0.1416 to 0.0185 on Gemini Robotics backbone. Graph truncation matches exact inference on 99.74% of decisions using 56× fewer graphs. Real-world experiments achieve 77.8% average success rate.

Conclusion: CPOR-Grasp provides a superior approach for obstruction reasoning in grasping tasks that enables certified decisions, adaptive stopping, and principled deferral compared to SOTA baselines.
...
### [77] [PASSAGE: Scaling Scene-Aligned Motion Learning for Perceptive Humanoid Traversal in Cluttered Environments](https://arxiv.org/abs/2609.18732)
...
### [78] [Active perception for robotic harvesting: 3D reconstruction and localisation of tomatoes hidden within clusters in a Mediterranean greenhouse](https://arxiv.org/abs/2609.18738)
*Fernando Cañadas-Aránega,Rowan Border,José C. Moreno,José L. Blanco-Claraco*

Main category: cs.RO

TL;DR: A comprehensive 5-stage pipeline for 3D reconstruction and precise localization of fruits in cluster-growing vegetables (tomatoes) within Mediterranean greenhouses, addressing the challenge of occluded fruits hidden behind visible surfaces.


<details>
  <summary>Details</summary>
Motivation: Automating robotic harvesting in intensive agriculture, particularly for cluster-growing vegetables, requires overcoming geometric complexity and occlusion challenges where fixed sensors fail to detect fruits hidden behind the visible surface. Existing solutions target isolated crops but cannot handle clustered vegetables with heavily occluded fruits.

Method: A sequential 5-stage methodology: (1) point cloud acquisition using AgriSEE Next Best View (NBV) active planner; (2) stochastic noise filtering via Statistical Outlier Removal (SOR); (3) surface classification and segmentation using Region Growing (RG); (4) isolation and recovery of occluded fruits through Density-Based Spatial Clustering of Applications with Noise (DBSCAN); (5) 3D pose estimation (position and orientation).

Result: The system achieves precision exceeding 90%, average recall of 82.8%, and mean Intersection over Union (mIoU) of 80.7%. It demonstrates high repeatability in centroid estimation with a Root Mean Square Error (RMSE) of 4.2mm.

Conclusion: The proposed approach extracts complete cluster geometry and reliably identifies partially hidden tomatoes. Evaluated across multiple scenarios with varying occlusion levels within a rigorously validated simulation framework against real-world conditions, the system demonstrates technical feasibility and high accuracy for autonomous harvesting operations.
...
### [79] [QMSR: Query-Conditioned Mask-wise Expert Routing for Robust Open-Vocabulary Underwater Object Retrieval](https://arxiv.org/abs/2609.18752)
...
### [80] [Gated Residual Body-Hand Coordination for Whole-Body Humanoid Teleoperation](https://arxiv.org/abs/2609.18763)
*Ruiming Wu,Shuang Li,Liding Zhang,Alois Knoll,Zhaopeng Chen*

Main category: cs.RO

TL;DR: A gated residual coordination framework that applies bounded corrections to motion-tracking policies and dexterous-hand retargeters in whole-body humanoid teleoperation, reducing wrist and fingertip geometry errors by 39.2-56.3% while preserving whole-body tracking accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional whole-body humanoid teleoperation independently combines motion-tracking policies with dexterous-hand retargeters, which fails to explicitly preserve body-hand geometric relations, causing mismatches in relative wrist poses and fingertip positions during bimanual interaction.

Method: A gated residual coordination framework with two key components: (1) motion-conditioned action gate that allocates correction authority across joint groups, and (2) reference-geometry-dependent reward gates that emphasize relevant interaction objectives during training. The framework keeps both modules frozen and applies bounded corrections to their outputs. The method also introduces multi-pose morphology calibration to jointly estimate triaxial scales and effector-local offsets, and staged motion dataset curation for training a SONIC-based tracker. The residual policy uses human motion references, initial commands, and robot proprioception without explicit object or contact observations.

Result: In simulation, the framework reduces wrist and fingertip geometry errors by 39.2-56.3% over direct composition on held-out GRAB motions, while preserving whole-body tracking on AMASS. Success rates improve from 89.03% (without residual coordination) to 89.29% (with it). Ablation studies characterize the contributions of reward gating, adaptive correction authority, and separate body and hand correction heads.

Conclusion: The gated residual coordination framework effectively addresses the geometric mismatch problem in whole-body humanoid teleoperation without requiring explicit object or contact observations, demonstrating significant improvements in wrist and fingertip precision while maintaining overall tracking performance.
...
### [81] [TRACER: Adaptive Multi-Robot Social Navigation via Joint Human-Response Prediction and Interaction-Aware Replanning](https://arxiv.org/abs/2609.18776)
...
### [82] [AdaGeoVLN: Selective Geometry Across Representation Depth and Navigation Time for Vision-Language Navigation](https://arxiv.org/abs/2609.18789)
...
TL;DR: A streaming vision-language navigation framework that couples hierarchical geometry foundation model representations with navigation-aware memory retention based on instruction relevance, geometric confidence, and transition novelty, achieving strong performance on R2R-CE and RxR-CE benchmarks while reducing memory usage.


<details>
  <summary>Details</summary>
Motivation: Vision-language navigation requires aligning language with visual observations while maintaining spatial understanding over time. Geometry foundation models expose intermediate representations throughout their hierarchy, but how navigation policies should use these features and retain historical geometric evidence remains unresolved.

Method: A streaming VLN framework that addresses representation depth and navigation time through: (1) Hierarchical GFM-VLM fusion coupling earlier, intermediate, and later GFM representations to successive policy stages; (2) Navigation-aware GFM memory that retains historical VGGT global-attention KV states based on instruction relevance, geometric confidence, and transition novelty under a bounded per-layer budget.

Result: Achieves strong performance on R2R-CE and RxR-CE benchmarks using a single RGB stream without additional navigation-specific external data. Multi-depth coupling substantially outperforms repeated terminal-feature injection. Bounded navigation-aware retention preserves performance while considerably reducing GFM-KV memory relative to larger-memory temporal retention.

Conclusion: These findings support jointly examining the geometric representations exposed to the policy and the historical evidence retained for future inference. The framework demonstrates that both representation depth and historical memory management are critical for effective vision-language navigation.
...
### [83] [Asymptotically Optimal Multi-Robot Task and Motion Planning](https://arxiv.org/abs/2609.18813)
*Thi Thuy Ngan Duong,Cheuk Tung Shadow Yiu,Rahul Shome,Yoonchang Sung*

Main category: cs.RO

TL;DR: This paper develops an asymptotically optimal multi-robot task and motion planning (MR-TAMP) algorithm that combines evolving individual-robot roadmaps with implicit tensor-product search, avoiding explicit composite roadmap construction while maintaining persistent exploration of task transitions.


<details>
  <summary>Details</summary>
Motivation: Extending asymptotically optimal TAMP algorithms to multi-robot settings is challenging because different task transitions involve different subsets of robots, imposing constraints of varying dimensions on the composite configuration space. An optimal planner must simultaneously optimize motion within task modes and explore different transition types connecting them.

Method: The authors characterize MR-TAMP transition structure and establish sufficient conditions for global asymptotic optimality, requiring persistent coverage of relevant transitions and asymptotically improving motion planning within connected feasible regions. The algorithm combines evolving individual-robot roadmaps with implicit tensor-product search, uses conditional transition sampling, lazy collision checking, and mode- and solution-level guidance.

Result: The framework provides asymptotic optimality guarantees for multi-robot manipulation while efficiently exploiting the structure of individual-robot motion planning, achieving both theoretical optimality and practical finite-time planning efficiency.

Conclusion: By balancing persistent exploration of task transitions with targeted sampling strategies, the proposed approach maintains asymptotic optimality guarantees while achieving practical computational efficiency for multi-robot manipulation tasks.
...
### [84] [SEAM: Submap-Anchored Evidence for Lifelong LiDAR Mapping under Trajectory Deformation](https://arxiv.org/abs/2609.18819)
*Kyuwon Kim*

Main category: cs.RO

TL;DR: SEAM is a LiDAR-based lifelong mapping framework that uses submap-level anchors, directional voxel-wise evidence, and DOP-based loop edge suppression to achieve higher accuracy and faster processing for long-term mapping.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on a single anchor spanning the entire session, making it difficult to handle trajectory modifications, suppress unreliable loop edges, and separate dynamic objects from environmental changes.

Method: SEAM optimizes trajectories with submap-level anchors, uses submap-level reprojection to maintain evidence usability after trajectory modifications, employs a DOP-based confidence measure to suppress unreliable inter-session loop edges, and utilizes a directional voxel-wise evidence model that accounts for occupancy patterns varying with ray direction.

Result: SEAM achieves higher accuracy and faster processing compared to existing methods on real construction-site and long-term multi-session datasets.

Conclusion: SEAM effectively handles dynamic object removal, change detection, and trajectory modifications in lifelong LiDAR mapping through its submap-level approach and directional evidence model.
...
### [85] [KINO: A Keyframe Interface for VLM Planning and Whole-Body Control in Humanoid Loco-Manipulation](https://arxiv.org/abs/2609.18869)
...
### [86] [Body-Motion Control of a Simulated Aerial Swarm from a First-Person View](https://arxiv.org/abs/2609.18881)
...
### [87] [SOL-SLAM: Inverse Compositional Gauss-Newton Direct Registration for Fast Sonar-Only Local SLAM](https://arxiv.org/abs/2609.18893)
*Kalvik Jakkala,Jason O'Kane*

Main category: cs.RO

TL;DR: This paper presents a dense direct registration approach for Forward-Looking Sonar (FLS)-only local SLAM in autonomous underwater vehicles, using Inverse Compositional Gauss-Newton optimization to achieve sub-meter tracking precision without expensive multi-sensor fusion.


<details>
  <summary>Details</summary>
Motivation: Existing underwater navigation systems rely on expensive multi-modal sensor suites for global SLAM, but local reactive behaviors only require local consistency. There's a critical gap in FLS-only local SLAM capabilities, and existing acoustic SLAM frameworks using sparse feature extraction discard valuable information from already sparse acoustic returns.

Method: A dense direct registration approach that aligns full acoustic intensity scans to a recursively updated local map, implemented via Inverse Compositional Gauss-Newton optimization for real-time execution on resource-constrained embedded hardware.

Result: Significant improvements in translation error compared to sparse keypoint baselines, maintaining stable sub-meter tracking precision over wide displacement gaps, and achieving odometry performance comparable to multi-sensor fusion pipelines (FLS, DVL, and IMU).

Conclusion: The approach was validated through AUV field trials, demonstrating real-world applicability and the ability to bypass expensive payload dependencies while maintaining high precision local SLAM capabilities on embedded computers.
...
### [88] [Examining the Difference in Human Behavior Between Virtual and Real-World Human-Robot Teaming](https://arxiv.org/abs/2609.18900)
*Sean Dallas,Absalat Getachew,Motaz AbuHijleh,Andrea Macklem-Zabel,Douglas Zytko,Mark Brudnak,Wing-Yue Geoffrey Louie*

Main category: cs.RO

TL;DR: This paper examines differences in human behavior, team performance, and user factors during human-robot teaming (HRT) scenarios conducted in virtual versus real environments, finding significant differences in workload, strategy, mental models of robots, and trust levels.


<details>
  <summary>Details</summary>
Motivation: Virtual simulation of HRT scenarios is increasingly used as an alternative to costly real-world user studies, but the validity of this approach depends on the consistency of human behavior between real and virtual environments. This study aims to investigate whether human behavior differs significantly when interacting with robots in virtual versus real settings.

Method: Mixed-methods user study comparing HRT scenarios in virtual and real environments. Quantitative measures examined team performance and workload, while qualitative analysis explored participants' strategies, mental models of robots, and trust relationships.

Result: Significant differences in workload were found between the two modalities. Qualitative analysis revealed differences in participants' strategies, their mental models of robots, and the type of trust they developed for robots between virtual and real environments.

Conclusion: The study demonstrates that human behavior and user factors differ meaningfully between virtual and real HRT scenarios, suggesting that virtual simulations may not fully replicate real-world human-robot interactions. This has important implications for the validity of using virtual environments for HRT research and for the design of virtual training and testing platforms.
...
### [89] [CaSCo: Cascade-Aware Soft-Collision Motion Planning](https://arxiv.org/abs/2609.18910)
*Shivaram Kumar,Gaoyuan Liu,Yoonchang Sung*

Main category: cs.RO

TL;DR: CaSCo is a motion planning framework that models soft collisions by assigning semantic risk to objects and predicting cascaded collision consequences using vision-language models and physics simulators, enabling safe robot navigation in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: Traditional motion planning treats collisions as binary constraints, but real-world contact consequences vary dramatically by object type (e.g., cardboard vs. glass). Direct robot-object collisions can displace objects and trigger secondary collisions, making risk dependent on the physical evolution of the scene rather than just geometric paths.

Method: CaSCo assigns semantic risk to objects via vision-language or language models, uses physics simulators to predict consequences of candidate motions, and searches for paths minimizing total semantic risk of displaced objects (directly and indirectly through cascaded collisions). The framework augments roadmap states with predicted object arrangements and incurred risks, featuring an optimal graph-search algorithm with cascade-relaxed heuristic, caching, and pruning mechanisms.

Result: Experiments in cluttered manipulation environments demonstrate effectiveness across semantic risk assessment, cascade reasoning, planning efficiency, and real-robot operation.

Conclusion: CaSCo successfully integrates semantic understanding of object risk and physics-based cascade reasoning into motion planning, enabling safer robot operation in complex environments where object interactions have varying consequences.
...
### [90] [Learning Holistic Whole-Body Loco-Manipulation with a Bipedal Mobile Manipulator](https://arxiv.org/abs/2609.18930)
...
### [91] ["What's going to happen after I'm gone?": Parent Perspectives on Technology in Supporting Independent Living for Adults with Intellectual Disabilities](https://arxiv.org/abs/2609.18970)
...
### [92] [Information-Based Trajectory Planning for Spacecraft-to-Spacecraft Tracking and Navigation in Cislunar Space](https://arxiv.org/abs/2609.19012)
*Trevor N. Wolf,Brandon A. Jones,Jay W. McMahon*

Main category: cs.RO

TL;DR: An information-optimal trajectory planning method for cislunar spacecraft that balances control effort and information gain to improve spacecraft-to-spacecraft absolute tracking, achieving nearly an order of magnitude improvement in tracking errors for optical observers in Distant Retrograde Orbit.


<details>
  <summary>Details</summary>
Motivation: Expanding cislunar space operations require navigation/tracking that minimizes ground-based support. Spacecraft-to-spacecraft tracking uses nonlinear dynamical tracers from relative measurements to infer absolute states, but geometric relationships between spacecraft significantly influence tracking performance, necessitating optimized trajectory design.

Method: Designs observer trajectories that jointly balance control effort and an information-theoretic quantification of expected spacecraft-to-spacecraft tracking performance. The method works for multiple low-thrust observation platforms with various sensing modalities and can incorporate multiple space object targets by leveraging information gain in the optimal control problem.

Result: Reports nearly an order of magnitude improvement in expected navigation and tracking errors for an optical observer operating in a Distant Retrograde Orbit (DRO), demonstrating the effectiveness of information-optimal low-thrust spacecraft trajectory design.

Conclusion: Demonstrates the feasibility and value of information-optimal low-thrust spacecraft trajectory design for current and upcoming cislunar missions, offering a viable approach to minimize ground-based support needs in cislunar navigation.
...
### [93] [Learning to Stack: Cube-Stacking Imitation Learning from Virtual Reality Demonstrations](https://arxiv.org/abs/2609.19040)
...
### [94] [Loco-Loco-RL: Low-Cost Terrain Mapping for Humanoid Locomotion with Reinforcement Learning](https://arxiv.org/abs/2609.19041)
*Jordan Dowdy,Gryffin Reizian,Jean Chagas Vaz*

Main category: cs.RO

TL;DR: This paper introduces a low-cost time-of-flight sensor combined with a token-compressed temporal transformer policy to enable robust humanoid locomotion over varied terrain using sparse 3D terrain representations.


<details>
  <summary>Details</summary>
Motivation: Common terrain perception sensors (depth cameras, LiDARs) incur high cost, power, and processing overhead while producing redundant, high-resolution data, making them impractical for resource-constrained robotic systems.

Method: The method employs a low-cost time-of-flight sensor for compact 3D local terrain representation, using a token-compressed temporal transformer architecture: proprioceptive and terrain observations are tokenized and processed by a self-attention multi-head transformer for within-timestep relationships, then compressed via an MLP-based latent-space token compression module before being stored in a 15-timestep rolling history. A second cross-attention multi-head transformer extracts temporal locomotion features from this compact history for policy learning.

Result: The approach achieves sim-to-real transfer on physical humanoid hardware using a terrain-based locomotion benchmark, demonstrating robust terrain walking with low-cost local terrain sensing.

Conclusion: Token compression before temporal aggregation preserves important terrain-observation structure while limiting the dimensional growth of attention over observation histories, enabling efficient processing of sparse exteroceptive inputs for robust humanoid locomotion.
...
### [95] [ElastiQP: An Always-Feasible QP Solver for Constrained Robot Control](https://arxiv.org/abs/2609.19080)
*Daniel Morton,Jon Arrizabalaga,Zachary Manchester,Marco Pavone*

Main category: cs.RO

TL;DR: ElastiQP is a modified dual active-set QP solver for robot control that relaxes inequality constraints with l1 penalties while keeping dynamics hard, achieving microsecond-level performance and 40x speedup on infeasible problems compared to alternatives.


<details>
  <summary>Details</summary>
Motivation: As robot capabilities increase, QP-based controllers must handle more constraints, which introduces more chances of momentary conflict where solvers return "infeasible" status, leaving the controller with nothing to execute.

Method: ElastiQP modifies a dual active-set QP solver to relax inequality constraints with exact per-constraint l1 penalties while keeping equality constraints (dynamics) hard. It folds slack variables analytically, maintaining a constant-size condensed linear system.

Result: On robot control benchmarks, ElastiQP achieves microsecond-level performance, matching or outperforming leading solvers on feasible problems. On infeasible problems, it handles violations gracefully and returns usable solutions up to 40x faster than best alternatives.

Conclusion: ElastiQP is open-source (C++ header-only with Python and JAX interfaces), providing a practical solution for real-time robot control with many constraints.
...
### [96] [rMuscle: Robotic Muscle Memory for Efficient Vision-Language-Action Model Inference](https://arxiv.org/abs/2609.19104)
...
### [97] [Dreaming the Sound of Contact: Leveraging Video and Audio Generation for Zero-Shot Force-Aware Manipulation and Data Generation](https://arxiv.org/abs/2609.19137)
...
### [98] [ABM-SIRTEM: A Hybrid Agent-Based and Epidemiological Model for Pandemic Response](https://arxiv.org/abs/2609.18223)
*Sheryl Paul,Samuel Williams,Preetom K. Biswas,Giulia Pedrielli,Jyotirmoy V. Deshmukh*

Main category: cs.MA

TL;DR: ABM-SIRTEM is a hybrid model combining SIR epidemiology with agent-based modeling to simulate COVID-19 spread and socioeconomic compliance dynamics.


<details>
  <summary>Details</summary>
Motivation: COVID-19 disproportionately affected lower socioeconomic groups and interaction-based jobs, creating a need for models that can simultaneously capture disease dynamics and socioeconomic impacts of interventions.

Method: ABM-SIRTEM incorporates occupation categories, economic productivity, and welfare at the individual level while dynamically modeling compliance with government interventions. The model is calibrated against historical positive and negative test counts from four U.S. states.

Result: The model successfully captures compliance dynamics when calibrated against historical data, demonstrating its utility for studying the interaction between disease spread and socioeconomic behavior.

Conclusion: ABM-SIRTEM provides a computational framework for pandemic-response planning by integrating epidemiological modeling with socioeconomic behavior dynamics, addressing the limitations of both population-level simplifications and computationally expensive detailed ABMs.
...
### [99] [Social Laws for Multi-agent Coordination in Stochastic Environments](https://arxiv.org/abs/2609.18929)
*Rolando Fernandez,Caleb Probine,Tyler Lee,Jeffrey Chen,Erez Karpas,Muhammad Arrasy Rahman,Peter Stone,Ufuk Topcu*

Main category: cs.MA

TL;DR: This paper extends social laws to stochastic, reward-based multi-agent environments by introducing α-robustness as a measure of guaranteed utility retention and developing a verification approach based on Markov decision process reduction.


<details>
  <summary>Details</summary>
Motivation: Multi-agent coordination faces challenges in preventing interference and ensuring robust individual performance. Previous social law research focused only on deterministic, goal-based settings, leaving stochastic, reward-based environments unexplored.

Method: The authors propose a formalism for defining and verifying social laws in stochastic environments. They introduce α-robustness as a measure of guaranteed utility retained by each agent while pursuing its optimal single-agent policy, assuming all agents obey the social law. The verification approach reduces the problem to solving a series of Markov decision processes.

Result: Empirical evaluations on toy environments demonstrate the framework's potential for handling stochastic multi-agent scenarios.

Conclusion: The proposed framework provides a formal approach for defining and verifying social laws in stochastic environments, with α-robustness offering a concrete measure of guaranteed utility retention for agents. The empirical results suggest the framework is viable for practical applications in stochastic multi-agent systems.
