...
TL;DR: MemBodied introduces a fixed-size episodic memory system with associative state and episode anchor components, enabling vision-language-action models to handle history-dependent manipulation tasks efficiently without bloating context or increasing inference latency.


<details>
  <summary>Details</summary>
Motivation: Most VLA policies don't preserve episode-level information beyond current observation, limiting performance in history-dependent manipulation tasks. While retaining past observations helps, it causes context bloating and inference latency.

Method: MemBodied uses two complementary memory components: (1) associative state that records interactions across policy calls, and (2) episode anchor that preserves a compact representation of the initial scene. At each policy call, the model conditions action generation on current input plus these memory components rather than directly using past observations.

Result: Across 5 RMBench tasks requiring memory, MemBodied achieved 7.81× the mean success rate of a stateless policy and 2.98× of vanilla recurrent memory. It outperformed the strongest memory-augmented baseline by 1.3× with 10× fewer parameters. On LIBERO-Long suite, it reached 90.6% (5.4% improvement over stateless π0 policy).

Conclusion: MemBodied demonstrates that a fixed-size episodic memory is a practical, parameter-efficient alternative to expanding policy context for history-dependent manipulation tasks.
...
TL;DR: A framework for generalizable robotic insertion assembly using world models that combines robot proprioceptive information with wrist-mounted camera visual observations, achieving 56% zero-shot success on unseen objects compared to 7% with model-free baselines.


<details>
  <summary>Details</summary>
Motivation: Robotic assembly in high-mixture settings requires adaptable systems that can handle diverse parts, but current approaches rely on policies specialized to each insertion task, making deployment for new problems tedious and time-consuming.

Method: A model-based approach that trains a single world model on up to 90 insertion tasks with geometrically diverse parts, combining robot proprioceptive information with raw visual observations from a wrist-mounted camera.

Result: Achieves 56% zero-shot success on unseen objects with unknown geometry (compared to 7% with model-free baseline), with performance improving as more objects are included in the training dataset. Finetuning the generalist model on held-out objects significantly enhances data-efficiency compared to training from scratch.

Conclusion: This is the first system capable of assembling unseen objects in an entirely data-driven manner, representing a significant step toward scalable, generalizable robotic assembly systems.
...
TL;DR: Least-Effort Adversarial Potentials (LEAP) combine control barrier function theory with deep reinforcement learning to create robust safety certificates that quantify disturbance effort required to cause failure, enabling effective disturbance handling in multi-agent systems and validated through simulation and hardware experiments on quadruped and quadrotor platforms.


<details>
  <summary>Details</summary>
Motivation: Robust Control Barrier Functions (CBFs) become difficult to construct and overly conservative for high-dimensional systems with input constraints and uncertainties/disturbances. Existing robust CBF approaches struggle to balance safety guarantees with performance in practical robotic applications.

Method: LEAP introduces a certificate quantifying robustness as the effort required by disturbances to cause failure. The approach constructs LEAPs using on-policy deep reinforcement learning, showing LEAPs serve as CBFs for undisturbed systems while enabling safety filters robust to disturbances with bounded cumulative effort.

Result: LEAPs demonstrated effectiveness in simulation across various multi-agent systems with disturbances and uncertainties. Hardware experiments on a quadruped and quadrotors validate that LEAPs successfully handle real-world disturbances and uncertainties, confirming suitability for practical robotic applications.

Conclusion: LEAPs provide an effective solution to the challenges of constructing robust CBFs, offering a practical method to achieve safety guarantees while maintaining performance in systems subject to disturbances and uncertainties, validated both in simulation and real-world hardware experiments.
