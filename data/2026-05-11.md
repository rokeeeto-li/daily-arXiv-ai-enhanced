...
TL;DR: A screw-theoretic Lie-algebraic framework for PDE-based control of flexible multibody robotic systems, enabling global exponential convergence of tracking errors through exact inter-link interaction cancellation and modular scalability.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of controlling flexible multibody robotic systems, specifically serial manipulators with arbitrary links in 3D space where distributed elastic deformations complicate trajectory tracking.

Method: Uses screw-theoretic Lie-algebraic model with body-fixed twists and wrenches in se(3); substitutes strain-based deformation PDE into dynamics to eliminate distributed elastic acceleration; generates twist trajectories via deflection-compensating inverse kinematics; employs nominal controllers with Lyapunov functions nu_i and adaptive modifications for online parameter estimation; constructs composite Lyapunov functions V = Σnu_i and V^a = Σnu_i^a.

Result: Inter-link interaction power terms telescope to zero, ensuring global exponential convergence of tracking errors; bounded elastic deformation guaranteed by Euler-Bernoulli energy argument; numerical simulations demonstrate physical consistency.

Conclusion: The screw-theoretic structure enables exact interaction cancellation, rendering the stability certificate modular and scalable to chains of arbitrary length while maintaining physical consistency.
...
TL;DR: This paper presents an aerial manipulator platform integrating onboard RGBD perception, MPPI-based UAV control on PX4, and a lightweight 2DoF manipulator for precise flower detection, localization, and targeting in vertical farming environments to address pollinator decline.


<details>
  <summary>Details</summary>
Motivation: The decline of natural pollinators has created major challenges for crop production in controlled indoor agriculture, particularly in vertical farming where natural insect pollination is absent. There's a need for robotic systems capable of performing precise flower targeting tasks while minimizing physical interference with delicate floral structures.

Method: The proposed system integrates onboard RGBD-based perception, model predictive path integral (MPPI) based unmanned aerial vehicle (UAV) control on a PX4 platform, and a lightweight 2DoF manipulator for precise end effector positioning. The platform was evaluated in both MuJoCo simulation and UAV lab experiments using a flower targeting testbed.

Result: Experimental results demonstrate stable UAV flight, reliable flower localization, and centimeter-level end effector positioning accuracy. In simulation, the controller achieves consistent trajectory convergence and accurate target alignment. In the real-world UAV lab environment, the integrated perception-control-manipulation framework enables stable flower targeted positioning and end effector alignment under constrained aerial operation.

Conclusion: The results validate the proposed aerial manipulator as a robust robotic carrier and positioning framework for future contactless pollination systems. The developed platform provides a practical foundation for integrating advanced contactless end effectors, including acoustic-based pollen manipulation modules, in future work.
...
TL;DR: Bi3 is a dataset of social robot navigation among groups of people in constrained lab spaces, featuring close navigation encounters, five navigation algorithms, two robot platforms, 74 diverse participants, and 10.5 hours of multimodal data (motion tracks, RGB video, user impressions).


<details>
  <summary>Details</summary>
Motivation: To address limitations of prior social robot navigation datasets by creating a resource that enables understanding how humans and robots can productively mesh their activities in constrained environments and provides data for training human motion prediction and robot control policies.

Method: An original experiment design featuring close navigation encounters between two humans and a robot; five different navigation algorithms; two robot platforms; recruitment of 74 participants from two sites (USA and France); collection of multimodal data streams including 10.5 hours of human and robot ground-truth motion tracks, RGB video, and user impressions.

Result: Analysis using metrics like interaction density and human velocity demonstrates that Bi3 represents a benchmark of unique diversity and modeling complexity, surpassing prior data collection efforts.

Conclusion: Bi3 contributes to advancing the understanding of human-robot collaboration in constrained environments and serves as a valuable resource for training models of human motion prediction and robot control policies for navigation in densely crowded spaces.
...
TL;DR: A language-in, simulation-out scenario orchestrator for closed-loop testing of autonomous vehicles that translates natural language descriptions into constraints and leverages constraint solvers to generate precise, controllable test scenarios.


<details>
  <summary>Details</summary>
Motivation: AVs require extensive simulation testing, but manual scenario programming is laborious and doesn't scale, while statistical models struggle with precise controllability for out-of-distribution scenarios that require specific interactions with AV policies.

Method: Casts scenario orchestration as a constraint-solving problem, leveraging foundation model reasoning to translate natural language descriptions into constraints, then using off-the-shelf solvers to generate actor behaviors that meet precise testing intentions in closed-loop simulation.

Result: Greatly outperforms baselines in orchestration success rate under carefully crafted and diverse scenario descriptions, with particular effectiveness for scenarios requiring ego-reactive specifications.

Conclusion: The approach successfully bridges natural language descriptions with executable test scenarios through constraint solving, enabling precise closed-loop testing that combines the scalability of statistical models with the controllability of manual programming.
...
TL;DR: An adaptive controller enables two aerial vehicles to collaboratively follow a trajectory while transporting a bendable object without explicit elasticity models, using Lyapunov analysis to guarantee asymptotic stability.


<details>
  <summary>Details</summary>
Motivation: Interacting with bendable objects in midair presents significant control challenges for aerial robots, often causing performance degradation and crashes due to their limited actuation capabilities and constant need to remain airborne.

Method: An adaptive controller is presented that allows two aerial vehicles to collaboratively follow a trajectory while transporting a bendable object without relying on explicit elasticity models, using Lyapunov analysis to demonstrate asymptotic stability.

Result: Hardware experiments in various scenarios demonstrate the capabilities of using multirotor aerial vehicles to successfully handle bendable objects.

Conclusion: The method allows on-the-fly adaptation to the object's unknown deformable properties, ensuring stability and performance in trajectory-tracking tasks.
...
TL;DR: This paper proposes an Intention Assimilation Control (IAC) strategy for teleoperation that estimates and transmits the leader's target position (rather than current position) to the follower. This approach enables high tracking accuracy without requiring high stiffness, while allowing dynamic impedance modulation for both task requirements and user intentions.


<details>
  <summary>Details</summary>
Motivation: Traditional teleoperation uses spring-like force control where the follower robot is pulled toward the leader's current position. This creates a fundamental trade-off: low follower stiffness results in poor tracking accuracy, while high stiffness poses safety risks to objects or people in the environment.

Method: The proposed Intention Assimilation Control (IAC) strategy estimates the leader's target position (rather than current position) and transmits this estimate to the follower. This allows the follower's impedance to be changed on-the-fly to continuously reflect the user's desired impedance or be modulated automatically to fulfill task requirements. The controller was validated on two 7 degree-of-freedom manipulators.

Result: Four experiments comparing IAC with Tele-Impedance Control (TIC) across free tracking, balloon interaction, peg insertion, and table polishing with force feedback showed that IAC increases tracking accuracy, improves task completion rate, and reduces completion time.

Conclusion: IAC enables the robot to accurately replicate user movements while giving them freedom to modulate impedance according to their intention, providing unprecedented control of the follower's position and impedance during both unilateral and bilateral teleoperation.
...
TL;DR: Dr-BA introduces a novel radar bundle adjustment framework that operates directly on 2D spinning radar intensity images, jointly estimating dense maps and sensor poses without requiring sparse point cloud extraction, and extending naturally to direct radar-only localization.


<details>
  <summary>Details</summary>
Motivation: Radar is unaffected by precipitation, making it critical for all-weather robustness in autonomous systems. Existing radar-based state estimation approaches extract sparse point clouds and apply point cloud alignment, which is inefficient compared to using the full radar returns directly.

Method: Dr-BA uses full radar returns from multiple scans to jointly estimate dense maps and sensor poses. The problem is formulated as separable optimization, deriving an efficient solution that decouples pose estimation from mapping. This formulation naturally extends to direct radar-only localization (DRL) within a previously built map.

Result: Dr-BA achieves state-of-the-art radar-based BA and cross-session localization performance, demonstrated on more than 200 km of on-road data across five distinct routes.

Conclusion: The implementation is publicly available at https://github.com/utiasASRL/dr_ba.
...
TL;DR: PISTO (Proximal Inference for Stochastic Trajectory Optimization) adds KL divergence regularization to STOMP, revealing its variational inference structure and enabling more stable, faster, and higher-performing trajectory optimization for robot planning with non-differentiable costs.


<details>
  <summary>Details</summary>
Motivation: STOMP enables planning with non-differentiable costs but can be unstable. The authors discovered that STOMP implicitly minimizes KL divergence from a Boltzmann trajectory distribution, revealing an elegant Variational Inference (VI) structure that was previously unexplored.

Method: PISTO augments the STOMP objective with KL divergence regularization between successive Gaussian proposals, creating a proximal formulation with a trust-region interpretation. The mean updates are computed as expectations under a surrogate distribution, estimated via importance-weighted Monte Carlo sampling, resulting in a simple, derivative-free algorithm.

Result: On robot arm motion planning benchmarks, PISTO achieves 89% success rate (vs 63% for CHOMP and 68% for STOMP), produces shorter and smoother paths, and runs twice as fast as competing stochastic methods. It also outperforms CEM and MPPI baselines on contact-rich MuJoCo locomotion and manipulation tasks.

Conclusion: PISTO inherits STOMP's ability to handle non-differentiable and discontinuous costs without modification while being more stable and faster, establishing a new state-of-the-art for stochastic trajectory optimization in robotics. The proximal inference formulation provides a principled interpretation of STOMP updates and enables practical improvements.
...
TL;DR: A lightweight autonomous exploration system for micro UAVs that leverages omnidirectional vision and sparse topological maps instead of dense occupancy grids or high-resolution point clouds.


<details>
  <summary>Details</summary>
Motivation: Classic exploration methods rely on dense occupancy maps or high-resolution point clouds, which consume substantial memory and computational resources. Micro UAVs under SWaP constraints cannot be equipped with LiDAR for accurate geometric measurements.

Method: Uses a multi-fisheye camera setup for omnidirectional Field of View (FoV) and depth estimation. Frontiers are represented as potential unexplored regions characterized by topological nodes instead of explicit boundaries. The environment is abstracted using a sparse topological map composed of key nodes and descriptors, with global path planning performed directly on the sparse graph.

Result: Validated in both simulation and real-world experiments on a palm-sized vision-based UAV (11 cm wheelbase, 400 g weight), demonstrating efficient exploration with extremely low computational consumption.

Conclusion: The proposed method achieves efficient autonomous exploration for micro UAVs while significantly reducing memory consumption and computational demands compared to classic dense representation approaches.
...
TL;DR: This paper proves that aerodynamic co-contraction in a redundant dual-rotor actuator can tune passive damping while maintaining constant net force, formalizing the mechanism as a Variable Aerodynamic Damping Actuator (VADA) that is dynamically isomorphic to stiffness modulation in variable-stiffness actuation.


<details>
  <summary>Details</summary>
Motivation: The authors aim to understand how aerodynamic co-contraction in redundant dual-rotor actuators can be used to tune passive damping while keeping commanded net force constant, enabling new control capabilities for multirotor systems.

Method: The authors define an incremental damping coefficient as the local sensitivity of net thrust to air-relative velocity at a trim point, prove it increases monotonically along constant-force fibers under a mild aerodynamic hardening condition, derive a minimal thrust model using Blade Element Theory, and formalize the mechanism as a Variable Aerodynamic Damping Actuator (VADA). They also use impedance-form representation to analyze control.

Result: The authors prove the damping tuning property, derive a minimal thrust model affine in inflow that reveals speed-inflow coupling, show VADA is dynamically isomorphic to stiffness modulation in antagonistic variable-stiffness actuation, demonstrate how fiber-density enhances aerodynamic promptness, and clarify the roles of common-mode and differential-mode actuation in controlling passive impedance.

Conclusion: The Variable Aerodynamic Damping Actuator (VADA) enables effective tuning of passive damping through aerodynamic co-contraction while maintaining constant force, similar to tendon co-contraction in biological systems, and can be effectively controlled using impedance representation. This provides a new approach for impedance control in multirotor systems.
...
TL;DR: BioProVLA-Agent is an affordable, protocol-driven, vision-enhanced embodied multi-agent system using VLA models for biological manipulation. The system employs a closed-loop workflow with a Tailored LLM Protocol Agent, VLM-RAG Verification Agent, and VLA Embodied Agent, enhanced by AugSmolVLA augmentation for robustness against wet-lab visual perturbations.


<details>
  <summary>Details</summary>
Motivation: Biological laboratory automation faces challenges due to unstructured protocols, reflective/transparent labware, and the need for state-aware execution beyond one-shot instruction following. Existing robotic systems are often costly, require fixed workflows, dedicated instruments, or robotics-oriented interfaces.

Method: BioProVLA-Agent consists of three agents: a Tailored LLM Protocol Agent that converts protocols into verifiable subtasks; a VLM-RAG Verification Agent that assesses readiness/completion using observations, retrieved knowledge, and examples; and a VLA Embodied Agent that executes verified subtasks. A closed-loop workflow integrates protocol parsing, visual state verification, and embodied execution. AugSmolVLA is an online augmentation strategy targeting transparent labware, reflections, illumination shifts, and overexposure.

Result: Evaluated on 15 atomic tasks, 6 composite workflows, and 3 bimanual tasks (tube loading, sorting, waste disposal, cap twisting, liquid pouring). AugSmolVLA improves execution stability over ACT, X-VLA, and original SmolVLA, especially for precise placement, transparent-object manipulation, composite workflows, and visually degraded scenes.

Conclusion: BioProVLA-Agent with AugSmolVLA provides a practical route toward accessible, protocol-centered, and verification-capable embodied AI for biological manipulation, demonstrating improved robustness and performance in wet-lab environments.
...
TL;DR: AT-VLA enhances Vision-Language-Action models with tactile feedback for contact-rich manipulation through adaptive tactile injection and dual-stream processing for real-time responses within 0.04s.


<details>
  <summary>Details</summary>
Motivation: VLA models struggle with contact-rich manipulation tasks requiring precise physical interactions. Previous attempts to incorporate tactile signals during fine-tuning disrupt pretrained capabilities, and VLAs' slow inference speeds hinder real-time responsiveness and effective tactile feedback utilization.

Method: AT-VLA introduces two key innovations: (1) Adaptive Tactile Injection mechanism that dynamically determines timing and locations for tactile injection, incorporating signals only when they significantly contribute to action generation to minimize interference; (2) Tactile Reaction Dual-Stream mechanism decoupling sensory processing into slow visual-language stream (low-frequency perceptual reasoning) and fast tactile control stream (high-frequency physical interaction understanding).

Result: Real-world experiments thoroughly validate AT-VLA's effectiveness in contact-rich manipulation tasks, with the dual-stream mechanism enabling rapid and accurate tactile responses while maintaining pretrained capabilities.

Conclusion: AT-VLA successfully addresses VLA limitations in contact-rich manipulation by enabling effective tactile integration without disrupting pretrained representations and achieving real-time closed-loop responses within 0.04 seconds.
...
TL;DR: MORPH-U fuses V2X and onboard sensors into a Local Dynamic Map for autonomous vehicle planning, using Hybrid-A* replanning triggered by validated hazards, with a Byzantine-inspired acceptance gate to handle message uncertainty and a multi-objective framework balancing safety, responsiveness, and comfort.


<details>
  <summary>Details</summary>
Motivation: V2X enables hazard awareness beyond line-of-sight but introduces message uncertainty (delays, drops, forgery), while dynamic map changes require real-time replanning. The authors aim to make motion planning and low-level control robust to these uncertain, event-driven updates.

Method: MORPH-U is a CARLA-based closed-loop stack that fuses LiDAR/radar/camera with V2X (CAM/DENM) into a Local Dynamic Map (LDM) and triggers Hybrid-A* replanning when validated hazards or map changes affect the planned route. A Byzantine-inspired acceptance gate combines quorum rules with on-board sensor veto to filter faulty V2X triggers. The system employs a multi-objective formulation over tracking error, safety margin (minimum TTC), responsiveness, and smoothness, with Pareto-frontier analysis to select operating points.

Result: Experiments in dynamic CARLA scenarios show that V2X-augmented LDM improves downstream safety, Pareto tuning enables controllable accuracy-comfort trade-offs, and the acceptance gate successfully prevents replanning under saturated false-DENM injection ($p_{\text{attack}}=1.0$).

Conclusion: The proposed MORPH-U framework demonstrates that fusing V2X with onboard sensors, using a Byzantine-inspired acceptance gate, and applying multi-objective planning trade-offs can effectively handle uncertain V2X messages and dynamic map changes while maintaining safety and controllable performance.
...
TL;DR: The paper introduces a Cached State Representation (CSR) framework and Asynchronous State Reconciliation (ASR) algorithm to reduce latency when deploying massive LLMs on robots by optimizing KV-cache reuse and handling state memory eviction.


<details>
  <summary>Details</summary>
Motivation: Deploying massive LLMs as continuous cognitive engines for robotics is bottlenecked by time-to-first-token (TTFT) latency when processing extensive state histories. Existing solutions like RAG or sliding windows compromise global context or incur prohibitive re-computation costs, creating a critical bottleneck for real-time embodied AI systems.

Method: The authors formalize the optimal task structure for minimizing latency and prove that prefix stability, incremental extensibility, and asynchronous state reconciliation are necessary conditions for real-time performance. They introduce the Cached State Representation (CSR) framework as a practical instantiation of these properties, ensuring optimal KV-cache reuse, and propose an Asynchronous State Reconciliation (ASR) algorithm that offloads state memory eviction to a parallel computational resource to eliminate latency spikes.

Result: On a physical robot wirelessly connected to a GPU server, CSR achieves a 26-fold latency reduction (14.67s to 0.56s) for 120K token contexts with a 235B parameter model compared to a standard baseline. On an embodied AI benchmark, the method achieves SOTA recall (0.836 vs. 0.459) while maintaining RAG-level latency. ASR sustains bounded, spike-free TTFT over 10 eviction cycles in continuous real-world operation.

Conclusion: CSR and ASR together enable massive LLMs to function as continuously operating, high-frequency embodied policies, overcoming the latency bottleneck that previously prevented real-time deployment of large models in robotics applications.
...
### [15] [Many-to-Many Multi-Agent Pickup and Delivery](https://arxiv.org/abs/2605.07835)
...
### [16] [Weather-Robust Scene Semantics with Vision-Aligned 4D Radar](https://arxiv.org/abs/2605.07367)
*Kali Hamilton,Christoffer Heckman*

Main category: cs.RO

TL;DR: A radar-VLM approach that aligns millimeter-wave radar encoders to frozen SigLIP embeddings, achieving superior weather robustness over camera baselines by resolving token-norm mismatches through projector-output LayerNorm


<details>
  <summary>Details</summary>
Motivation: Cameras and LiDAR performance degrades significantly in adverse weather conditions (rain, fog, snow), while millimeter-wave radar remains largely unaffected. There is a need for reliable perception systems that maintain accuracy in challenging environmental conditions.

Method: The researchers aligned a radar encoder with frozen SigLIP vision embeddings and used a frozen vision-language model (VLM) with approximately 7M trainable parameters to decode structured scene captions. They identified token-norm mismatch as a key failure mode and introduced projector-output LayerNorm to resolve it. They analyzed encoder complexity, caption format, and pooling strategy tradeoffs.

Result: On the K-RADAR dataset with held-out fog, light snow, and heavy snow sequences, all radar configurations outperformed a camera baseline that collapsed to over 90% hallucination. The projector-output LayerNorm approach successfully resolved the token-norm mismatch issue.

Conclusion: This work demonstrates that radar-VLM pipelines can achieve robust weather performance compared to camera-based systems. The findings provide important design insights for future radar-VLM architectures, particularly regarding normalization strategies and tradeoffs between encoder complexity, caption format, and pooling approaches.
...
TL;DR: This paper addresses the data efficiency challenge in deploying Vision-Language-Action (VLA) models by identifying and formalizing the Coverage-Density Trade-off, where maximizing diversity through single-shot demonstrations can be counterproductive due to estimation noise. The authors propose Anchor-Centric Adaptation (ACA), a two-stage framework that first stabilizes policy performance at core anchor conditions, then selectively expands coverage to high-risk boundaries, achieving superior task reliability and success rates compared to standard diverse sampling strategies under identical data budgets.


<details>
  <summary>Details</summary>
Motivation: VLA models require real-world adaptation to bridge the embodiment gap, but robot demonstrations are costly, necessitating efficient adaptation under strict data budgets. The authors observe that the standard heuristic of maximizing diversity through single-shot demonstrations can be counterproductive due to estimation noise.

Method: The authors formalize the Coverage-Density Trade-off by decomposing policy error into estimation (density) and extrapolation (coverage) terms. They propose Anchor-Centric Adaptation (ACA), a two-stage framework: (1) stabilizing a policy skeleton through repeated demonstrations at core anchors, and (2) selectively expanding coverage to high-risk boundaries via teacher-forced error mining and constrained residual updates.

Result: Real-robot experiments validate the trade-off framework and demonstrate that ACA significantly improves task reliability and success rates over standard diverse sampling strategies under the same data budget.

Conclusion: The Coverage-Density Trade-off formalization provides theoretical guidance for optimal data allocation strategies in embodied AI, and ACA offers a practical solution that outperforms standard approaches by intelligently balancing density and coverage through its two-stage adaptation framework.
...
TL;DR: A navigation system that uses BEV images as global priors, employing an image generation model to interpret human intent and generate traversability masks, with cross-view localization to align odometry and mitigate drift.


<details>
  <summary>Details</summary>
Motivation: Two key challenges remain: how to fully exploit the global information provided by BEV images and how to reliably use it during execution.

Method: A navigation system for ground and near-ground robots that uses BEV images as global priors, featuring an image generation model for natural language intent interpretation and traversability mask generation, and cross-view localization to align odometry with BEV maps to mitigate long-term drift.

Result: Extensive benchmark experiments demonstrate the method's effectiveness, and validation on a UAV platform shows successful completion of a 160-meter outdoor long-range navigation task using only a conventional local motion planner.

Conclusion: This work demonstrates how the world-understanding capabilities of foundation models can be transferred to embodied navigation, enabling robots to benefit from the strong generalization ability of existing image generation models.
...
TL;DR: PROBE is a search-based robustness testing approach that uses multi-objective optimization (NSGA-II) to identify minimal perturbations that expose failures in object detection models used in laptop refurbishing robots.


<details>
  <summary>Details</summary>
Motivation: Laptop refurbishment robots use object detection models for disassembly and sticker removal. Ensuring robustness against small input variations is critical to avoid potential damage to laptops during refurbishment.

Method: PROBE employs NSGA-II to systematically explore the perturbation space, optimizing for failure induction considering both localization/confidence and perturbation magnitude, while discovering diverse failure cases.

Result: PROBE is 3× to 7× more effective than random search in generating failure-inducing perturbations, requires smaller perturbation magnitudes, and the generated perturbations transfer across models. Metamorphic relations provide additional insights for stability assessment.

Conclusion: PROBE effectively identifies failure-inducing perturbations in object detection models used in refurbishment robots, enabling more robust software development and reducing the risk of physical damage to laptops.
...
TL;DR: A method for imitation learning that leverages failure data alongside successful demonstrations by learning latent representations of success-failure discrepancies and incorporating them into attention mechanisms, enabling direct and stable utilization of failure data during inference.


<details>
  <summary>Details</summary>
Motivation: Imitation learning primarily relies on successful demonstrations, but failures are inevitable during human data collection. Existing approaches for exploiting failure data require additional data processing or iterative policy updates through autonomous rollouts, making it difficult to directly and stably utilize failure data accumulated during data collection.

Method: The proposed method learns latent representations of success-failure discrepancies and incorporates them into the attention mechanism. During inference, an appropriate latent mode is selected from the initial observation to improve action stability. Additionally, a post-training metric quantifies the attention discrepancy between each failure sample and successful demonstrations to select failure data.

Result: Simulation results demonstrate that the proposed method improves task success rates when trained with failure data. The introduced metric successfully identifies failure samples that are beneficial for learning when combined with successful demonstrations.

Conclusion: The proposed method supports more efficient use of collected demonstrations in robotic data collection pipelines by enabling direct and stable utilization of failure data alongside successful demonstrations.
...
### [22] [MemCompiler: Compile, Don't Inject -- State-Conditioned Memory for Embodied Agents](https://arxiv.org/abs/2605.07594)
...
TL;DR: Existing embodied agent memory systems use static Ahead-of-time Monolithic Memory Injection (AMMI), which becomes misaligned with the agent's evolving state and degrades lightweight executors. The authors propose MemCompiler, which reframes memory utilization as State-Conditioned Memory Compilation, where a learned compiler dynamically selects and compiles only relevant memory based on the agent's current execution state into executable guidance via text and latent channels.


<details>
  <summary>Details</summary>
Motivation: Existing memory systems inject retrieved memory as static context at episode start (AMMI), which quickly becomes misaligned with the agent's evolving state and degrades lightweight executors below the no-memory baseline.

Method: MemCompiler reframes memory utilization as State-Conditioned Memory Compilation. A learned Memory Compiler reads a structured Brief State capturing the agent's current execution state and dynamically selects and compiles only relevant memory into executable guidance. This guidance is delivered through a text channel and a latent Soft-Mem channel that preserves perceptual information not expressible in text.

Result: Across Alf World, EmbodiedBench, and ScienceWorld, MemCompiler consistently improves over no-memory across open-source backbones (up to +129%), matches or approaches frontier closed-source systems, and reduces per-step latency by 60%.

Conclusion: State-aware memory compilation improves both effectiveness and efficiency of embodied agents' memory utilization.
...
### [23] [BrickCraft: Visuomotor Skill Composition with Situated Manual Guidance for Long-Horizon Interlocking Brick Assembly](https://arxiv.org/abs/2605.07605)
*Jichuan Yu,Bowei Li,Zhenran Tang,Guanxing Lu,Chuxiong Hu,Ruixuan Liu,Changliu Liu*

Main category: cs.RO

TL;DR: BrickCraft is a compositional framework for autonomous robotic assembly of interlocking bricks that uses relative formulations, situated manuals, and compositional execution pipelines to enable long-horizon assembly from limited demonstrations with strong generalization.


<details>
  <summary>Details</summary>
Motivation: Autonomous robotic assembly of interlocking bricks requires seamless integration of long-horizon task reasoning, spatial grounding, and fine-grained manipulation. There is a significant gap between high-level assembly plans and physical execution that needs to be bridged.

Method: BrickCraft employs three key components: (1) a relative formulation where each step is anchored to a reference brick, decomposing tasks into reusable primitive skills; (2) situated manuals that provide explicit spatial guidance for visuomotor skills by projecting assembly intent onto real-time observations; and (3) a compositional execution pipeline that chains these spatially grounded skills.

Result: Extensive experimental validations demonstrate that BrickCraft acquires proficient assembly skills from a limited set of demonstrations and exhibits strong compositional generalization to unseen structures.

Conclusion: BrickCraft successfully bridges the gap between high-level assembly plans and physical execution, demonstrating that a compositional approach can achieve proficient interlocking brick assembly through learned primitive skills with strong generalization capabilities.
...
### [24] [PhySPRING: Structure-Preserving Reduction of Physics-Informed Twins via GNN](https://arxiv.org/abs/2605.07687)
*Yixiong Jing,Xingyuan Chen,Guangming Wang,Olaf Wysocki,Haibing Wu,Brian Sheil*

Main category: cs.RO

TL;DR: PhySPRING is a fully differentiable GNN-based method that learns hierarchical coarsened graph topologies and mechanical parameters from observations, reducing complexity in spring-mass digital twins while maintaining physical and visual fidelity and achieving up to 2.30x speed-up.


<details>
  <summary>Details</summary>
Motivation: Physics-based digital twins enable real-to-sim-to-real applications in robotics. However, current approaches reconstruct twins as explicit physical models (e.g., spring-mass systems) that inherit the resolution of visual reconstruction rather than being reduced to the physical complexity needed to reproduce task-relevant dynamics. This mismatch introduces redundant topology, making forward-dynamics rollouts unnecessarily expensive.

Method: PhySPRING jointly learns a hierarchy of coarsened graph topologies and their mechanical parameters from observations using a fully differentiable GNN-based approach. At each reduction level, nodes with similar learned dynamic responses are merged to optimize the topology, while maintaining every reduced layer as an explicit spring-mass system.

Result: On the PhysTwin benchmark, PhySPRING improves dense reconstruction and prediction accuracy over the baseline PhysTwin, while reduced models retain stable physical and visual fidelity with up to a 2.30x speed-up. In Real2Sim robot policy-evaluation pipelines, reduced models substituted zero-shot into ACT and π_0 evaluations maintain comparable manipulation success rates across downsampling levels while improving action-sampling effectiveness.

Conclusion: PhySPRING enables efficient and structure-preserving spring-mass reduction without sacrificing fidelity or robotic utility, addressing the critical challenge of redundant topology in physics-based digital twins for robotics applications.
...
### [25] [Offline-Online Hierarchical 3D Global Relocalization With Synthetic LiDAR Sensing and Descriptor-Space Retrieval](https://arxiv.org/abs/2605.07741)
...
### [26] [Sensitivity-Based Robust NMPC for Close-Proximity Offshore Wind Turbine Inspection with a Tilted Multirotor](https://arxiv.org/abs/2605.07771)
*Giuseppe Silano,Martin Saska*

Main category: cs.RO

TL;DR: A sensitivity-based robust NMPC approach for offshore wind turbine inspection that uses online constraint tightening to maintain safe tower clearance under parameter uncertainty and gusts, successfully eliminating violations at moderate computational cost.


<details>
  <summary>Details</summary>
Motivation: Nominal NMPC fails to maintain safety constraints for offshore wind turbine inspection due to parameter uncertainties (mass, inertia, thrust effectiveness, drag, wind conditions) and model mismatch, risking collisions with the cylindrical tower structure.

Method: A robust NMPC formulation that augments nominal NMPC with sensitivity propagation and margin evaluation (first-order parametric state sensitivities for structured uncertainty, stage-dependent additive margins for gusts) while preserving the receding-horizon optimization structure, enabling online constraint tightening.

Result: Monte-Carlo simulation over 500 uncertainty realizations on a helical inspection trajectory shows the proposed controller eliminates clearance violations that occurred under nominal NMPC, with only a moderate increase in solve time.

Conclusion: Online constraint tightening via sensitivity-based robust NMPC effectively addresses parameter uncertainty and gusts in offshore wind turbine inspection, maintaining safety constraints without fundamentally altering the optimization framework, at the cost of moderate computational overhead.
...
### [27] [NoiseGate: Learning Per-Latent Timestep Schedules as Information Gating in World Action Models](https://arxiv.org/abs/2605.07794)
...
TL;DR: TAVIS introduces a shared benchmark and evaluation infrastructure for active-vision imitation learning, featuring two task suites (TAVIS-Head: 5 tasks with pan/tilt necks; TAVIS-Hands: 3 tasks with wrist cameras) on humanoid robots (GR1T2, Reachy2), novel GALT metric, and findings that active vision benefits are task-conditional, multi-task policies degrade under distribution shifts, and imitation alone produces anticipatory gaze comparable to human teleoperators.


<details>
  <summary>Details</summary>
Motivation: Active-vision imitation learning has demonstrated benefits across multiple independent systems, yet lacks a shared benchmark to compare approaches, quantify contributions across task types and conditions, or standardize evaluation protocols.

Method: TAVIS evaluation infrastructure built on IsaacLab with two complementary task suites: TAVIS-Head (5 tasks requiring global search via pan/tilt necks) and TAVIS-Hands (3 tasks requiring local occlusion handling via wrist cameras) on two humanoid torso embodiments. Provides three evaluation primitives: (1) paired headcam-vs-fixedcam protocol on identical demonstrations, (2) GALT (Gaze-Action Lead Time) metric grounded in cognitive science/HRI quantifying anticipatory gaze, and (3) procedural ID/OOD splits for controlled distribution shifts.

Result: Baseline experiments with Diffusion Policy and π0 reveal: (i) active vision generally helps but benefits are task-conditional rather than uniform; (ii) multi-task policies degrade sharply under controlled distribution shifts on both task suites; (iii) imitation alone yields anticipatory gaze with median lead times comparable to human teleoperator reference.

Conclusion: Active vision provides task-specific advantages that must be evaluated conditionally rather than universally, multi-task policies struggle with distribution shifts, and imitation learning alone can produce effective anticipatory gaze behavior, suggesting that gaze control is an emergent property rather than requiring explicit architectural components.
...
TL;DR: This paper empirically investigates the benefits of an actuated spine for quadruped robots in MuJoCo simulation using the Silver Badger robot. Results show the spine enhances agility and enables the robot to overcome higher stairs, steeper slopes, and smaller passages.


<details>
  <summary>Details</summary>
Motivation: The spine plays a crucial role in quadrupedal animal locomotion by improving stability, speed, and efficiency, especially during fast-paced and agile movements. This makes the spine a promising and natural approach to extend the capabilities of quadruped robots.

Method: An empirical study was conducted in MuJoCo simulation using the Silver Badger robot from MAB Robotics with an actuated 1-DOF spine in the sagittal plane. The robot was evaluated across five scenarios: high-speed running, climbing stairs, climbing high-angle slopes, hurdling, and crawling.

Result: The actuated spine provides the robot with increased agility. Specifically, the spine enables the robot to overcome higher stairs, steeper slopes, higher obstacles, and smaller passages compared to configurations without an actuated spine.

Conclusion: The findings demonstrate that incorporating an actuated spine into quadruped robots significantly enhances their locomotion capabilities, allowing them to navigate more challenging terrain and obstacles.
...
TL;DR: 123D is an open-source framework that unifies fragmented multi-modal driving sensor data (cameras, lidar, ego states, etc.) through a single API using timestamped event streams, enabling cross-dataset analysis and applications like 3D object detection transfer and reinforcement learning for planning.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving produces rich sensor data but it remains largely untapped due to fragmentation: datasets use different 2D/3D modalities, synchronization schemes, and annotation conventions, making them incompatible and preventing generalization across datasets.

Method: 123D stores each modality as an independent timestamped event stream without prescribed rates, enabling synchronous/asynchronous access across arbitrary datasets. It consolidates eight real-world driving datasets (3,300 hours, 90,000 km), a synthetic dataset, and provides tools for analysis and visualization.

Result: A systematic study of annotation statistics and pose/calibration accuracy across datasets. Two applications demonstrated: cross-dataset 3D object detection transfer and reinforcement learning for planning.

Conclusion: 123D demonstrates that unified multi-modal data access enables cross-dataset generalization. Future directions should focus on addressing annotation inconsistencies and improving dataset interoperability.
...
### [34] [Generalising Travel Time Prediction To Varying Route Choices In Urban Networks](https://arxiv.org/abs/2605.06918)
*Łukasz Gorczyca,Kacper Drozd,Michał Bujak,Rafał Kucharski*

Main category: cs.MA

TL;DR: GenTTP learns complex spatiotemporal traffic patterns and microscopic relationships between route choices and travel times to provide accurate flow and travel time predictions across varying route assignments.


<details>
  <summary>Details</summary>
Motivation: Previous GNN-based methods approximate a single demand realisation and fail to capture varying route choices, limiting their ability to generalize across different route assignments where the same demand produces substantially different network-wide outcomes.

Method: Proposes a Generalised Travel Time Predictor (GenTTP) framework that learns complex spatiotemporal traffic patterns and microscopic relationships between route choices and resulting travel times.

Result: Successfully differentiates route choices and provides accurate flow and travel time predictions.

Conclusion: Addresses the critical gap in travel time prediction models by generalizing across varying route assignments, enabling predictions of how traveler distribution over available paths affects network-wide outcomes.
...
### [35] [The Cost of Consensus: Malignant Epistemic Herding and Adaptive Gating in Distributed Multi-Agent Search](https://arxiv.org/abs/2605.06988)
*David Farr,Iain Cruickshank,Kate Starbird,Jevin West*

Main category: cs.MA

TL;DR: Distributed agents must coordinate under uncertainty with partial observations; this work introduces "epistemic alignment" to measure how compatible agents' internal beliefs are, showing that communication frequency and content jointly shape collective belief states in ways that standard coordination metrics cannot detect.


<details>
  <summary>Details</summary>
Motivation: Distributed agents need to share beliefs to coordinate effectively, but communication has costs (bandwidth, latency) and risks degrading collective reasoning. This tension is especially acute in bandwidth-constrained deployments like distributed sensing networks, autonomous reconnaissance, and collaborative cyber defense.

Method: The authors introduce "epistemic alignment" as the degree to which agents maintain compatible internal beliefs about the environment. They formalize this concept and demonstrate that it is not detectable from standard coordination metrics such as Jensen-Shannon Divergence or rate to consensus.

Result: Effective belief sharing enables agents to converge on correct hypotheses, while poorly designed communication can cause agents to confidently converge on wrong ones.

Conclusion: The work formalizes the distinction between good and bad belief sharing patterns, showing that standard coordination metrics fail to capture this crucial aspect of multi-agent coordination.
...
### [36] [Social Theory Should Be a Structural Prior for Agentic AI: A Formal Framework for Multi-Agent Social Systems](https://arxiv.org/abs/2605.07069)
*Lynnette Hui Xian Ng,Iain J. Cruickshank,Adrian Xuan Wei Lim,Kathleen M. Carley*

Main category: cs.MA

TL;DR: This position paper introduces the Multi-Agent Social Systems (MASS) framework to model agentic AI systems as dynamical systems of information generation, local influence, and interaction structure, anchored in four social theory-based structural priors: strategic heterogeneity, networked-constrained dependence, co-evolution, and distributional instability.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems are increasingly deployed in social environments populated by other agents and humans (e.g., social media platforms, multi-agent LLM pipelines, autonomous robotics fleets). In these settings, system behavior emerges not from individual agents alone but from multi-agent interactions over time, a phenomenon that social scientists have long studied in human contexts. The paper argues that agentic AI systems must be modeled with social theory as a structural prior to properly capture emergent dynamics.

Method: The authors formalize the Multi-Agent Social Systems (MASS) framework as a class of dynamical systems characterized by information generation, local influence, and interaction structure. They anchor this framework in four structural priors derived from social theory: (1) strategic heterogeneity, (2) networked-constrained dependence, (3) co-evolution, and (4) distributional instability. The importance of each prior is demonstrated through formal propositions.

Result: The paper demonstrates the importance of each of the four structural priors through formal propositions. It provides a theoretical foundation for understanding how agents interact and influence each other to generate system-level outcomes in social environments.

Conclusion: The paper articulates a comprehensive research agenda for how MASS should be modeled, evaluated, and governed. This establishes a foundation for developing agentic AI systems that account for the complex social dynamics inherent in multi-agent environments.
...
### [37] [Rethinking Priority Scheduling for Sequential Multi-Agent Decision Making in Stackelberg Games](https://arxiv.org/abs/2605.07240)
*Xiangyu Liu,Liang Zhang,Bo Jin,Ziqi Wei*

Main category: cs.MA

TL;DR: A novel Hierarchical Priority Adjustment (HPA) method optimizes decision order in N-level Stackelberg Games for multi-agent systems, outperforming benchmarks on high-precision control tasks.


<details>
  <summary>Details</summary>
Motivation: Existing N-level Stackelberg Game approaches use default decision orders from the environment, but the order significantly impacts equilibrium. This work formally analyzes this dependence and proposes a method to systematically optimize it.

Method: HPA employs a hierarchical structure with an upper policy that dynamically selects the optimal decision order based on the current state, while agents execute strategies in a Spatio-Temporal Sequential Markov Game (STMG). A slow-fast update scheme with shared intrinsic rewards derived from the advantage function coordinates learning across time scales.

Result: Experimental results on multi-agent MuJoCo tasks demonstrate HPA outperforms benchmark algorithms and robustly adapts to changing environments.

Conclusion: Optimizing agents' decision order is crucial in N-level Stackelberg Games. The proposed HPA method effectively addresses this challenge and achieves superior performance.
...
### [38] [OrchJail: Jailbreaking Tool-Calling Text-to-Image Agents by Orchestration-Guided Fuzzing](https://arxiv.org/abs/2605.07414)
*Jianming Chen,Yawen Wang,Junjie Wang,Zhe Liu,Qing Wang,Fanjiang Xu*

Main category: cs.MA

TL;DR: OrchJail is a novel orchestration-guided fuzzing framework that exploits tool-orchestration patterns to jailbreak tool-calling text-to-image agents more effectively than prompt-only techniques.


<details>
  <summary>Details</summary>
Motivation: Tool-calling T2I agents can execute multi-step tool chains that combine benign steps into harmful outputs, creating a new safety attack surface that prompt-only jailbreak techniques cannot address.

Method: OrchJail learns from successful jailbreak traces and causal relationships between prompt wording and tool orchestration to guide fuzzing search toward prompts that trigger unsafe multi-step behaviors, focusing on high-risk orchestration patterns rather than surface-level textual perturbations.

Result: OrchJail achieves higher attack success rates, better image fidelity, and lower query costs across representative tool-calling T2I agents, while remaining robust against common jailbreak defenses.

Conclusion: Tool orchestration represents a critical, previously unexplored attack surface for T2I agents, and OrchJail provides a novel framework for uncovering safety risks in these systems.
...
### [39] [Synchronizing Minds through Collective Predictive Coding: A Computational Model of Parent-Infant Homeostatic Co-Regulation](https://arxiv.org/abs/2605.07524)
*Yushi Tsubamoto,Takato Horii*

Main category: cs.MA

TL;DR: This paper proposes a constructive computational model of parent-infant co-regulation that explains how inter-brain synchrony emerges through interaction, showing that latent state alignment can occur even before fully shared world models converge.


<details>
  <summary>Details</summary>
Motivation: Inter-brain synchrony (IBS) has been observed in real-time dyadic interactions, but computational accounts of how alignment arises between agents with only local sensory access and asymmetric internal knowledge remain underdeveloped.

Method: The model integrates a POMDP formulation of active interoceptive inference with the Metropolis-Hastings Naming Game (MHNG) derived from Collective Predictive Coding (CPC). In a 6×6 visceral-state grid world, the parent observes only exteroceptive signals while the infant senses interoceptive states directly; they agree on regulatory actions through a shared communicative variable whose acceptance is determined by a locally computable MHNG probability. Agents have asymmetric generative-model knowledge: the parent knows action-state transformations but must learn what the infant communicates, while the infant perceives visceral states directly but must learn action effects.

Result: MHNG-mediated interaction regulated the infant's visceral state more adaptively than one-sided control conditions, and the two posteriors became rapidly aligned. Crucially, this latent-state alignment emerged far earlier than the convergence of learned generative matrices, indicating that representational synchrony does not presuppose fully shared world models.

Conclusion: The results offer a minimal constructive account of latent-state alignment compatible with IBS reported in hyperscanning studies and support CPC as a candidate computational basis for inter-brain alignment.
...
### [40] [Emergence of Social Reality of Emotion through a Social Allostasis Model with Dynamic Interpretants](https://arxiv.org/abs/2605.07761)
...
### [41] [Velocity-Space 3D Asset Editing](https://arxiv.org/abs/2605.07385)
*Hao Liu,Yuxuan Lin,Jingfeng Guo,Ruihang Chu,Junjie Wang,Ruotong Li,Yujiu Yang*

Main category: cs.GR

TL;DR: VS3D is a training-free, inversion-free, and mask-free framework for local editing of 3D assets that addresses identity leakage, edit amplification, and identity drag by integrating three targeted modules inside the ODE sampler: RASI for source reconstruction, PMG for edit signal amplification, and TAR for token-wise preservation decisions.


<details>
  <summary>Details</summary>
Motivation: Existing 3D editing methods enforce locality externally (via masks, voxel merging, or 2D lifting) and fail to intervene inside the ODE sampler where corruption originates, leading to three critical problems: (i) identity leakage causing non-zero edit signals on preserved regions, (ii) lack of a dedicated edit-amplification channel that forces a trade-off between edit strength and identity preservation, and (iii) identity drag at geometry and material stages where global conditions pull all tokens toward the target.

Method: VS3D intervenes inside the ODE sampler with three complementary modules: (1) Reconstruction-Anchored Source Injection (RASI) replaces unconditional embedding with asset-specific anchors calibrated through source reconstruction to absorb identity leakage; (2) Partial-Mean Guidance (PMG) contrasts high- and low-quality subsample estimates of the velocity difference to amplify the edit signal only where consistent edits exist; (3) Twin-Agreement Residual injection (TAR) enables token-by-token decision-making at geometry and material stages to preserve content selectively.

Result: The method achieves faithful local editing by addressing each problem at its source: RASI eliminates identity leakage, PMG provides a dedicated edit-amplification channel without identity trade-offs, and TAR mitigates identity drag through selective token preservation.

Conclusion: VS3D demonstrates that targeted intervention within the ODE sampler—rather than external enforcement—enables inversion-free, training-free, and mask-free local 3D editing that simultaneously preserves identity and amplifies edits, overcoming fundamental limitations of existing approaches.
...
### [42] [LoBoFit: Flexible Garment Refitting via Local Bone Mapping Blending](https://arxiv.org/abs/2605.07450)
*Meng Zhang,Yu Xin,Feiya Guo,Kaizhang Kang,Mengyu Chu,Ruizhen Hu*

Main category: cs.GR

TL;DR: LoBoFit introduces a Local Bone Mapping Blending representation for garment refitting, which expresses garment geometry as a linear blend of local bone coordinate mappings rather than manipulating global vertex positions, enabling stable convergence with fine-scale detail preservation across varying avatar shapes and poses.


<details>
  <summary>Details</summary>
Motivation: Garment refitting must preserve original design features and fine-scale wrinkles while handling significant shape variations and pose changes without registration to a shared canonical pose. Existing methods struggle to balance robustness, efficiency, and fidelity: physics-based simulation is costly, data-driven approaches lack generalizability, and geometry optimization in full vertex space is ill-conditioned and prone to local minima with unsatisfactory quality.

Method: LoBoFit employs Local Bone Mapping Blending (LoBoMap Blending), representing garment geometry as a linear blend of its mappings into local bone coordinate frames instead of global vertex positions. This representation provides pose-robust initialization, a well-conditioned parameterization, and smooth optimization landscape through blending weights. The subsequent refinement optimizes localized residuals to resolve collisions and preserve details.

Result: LoBoFit reliably refits high-resolution, single- and multi-layer garments across avatars with large shape and topological differences, faithfully preserving intricate wrinkles and intended fit style, outperforming state-of-the-art methods in robustness and output quality.

Conclusion: The Local Bone Mapping Blending representation addresses the fundamental limitation of deforming garments in global coordinates, which couples vertices non-locally and creates a complex optimization landscape. By decomposing complex global deformation into manageable subproblems, LoBoFit enables stable convergence with fine-scale detail preservation while maintaining robustness and efficiency.
