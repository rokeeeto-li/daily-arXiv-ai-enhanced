...
### [1] [HRI-SA: A Multimodal Dataset for Online Assessment of Human Situational Awareness during Remote Human-Robot Teaming](https://arxiv.org/abs/2603.18344)
*Hashini Senaratne,Richard Attfield,Samith Widhanapathirana,David Howard,Cecile Paris,Dana Kulic,Leimin Tian*

Main category: cs.RO

TL;DR: HRI-SA is the first public multimodal dataset for systematic evaluation of online situational awareness (SA) assessment in human-robot teaming, featuring 30 participants in search-and-rescue scenarios with eye-tracking, biosignals, and robot data.


<details>
  <summary>Details</summary>
Motivation: Maintaining situational awareness (SA) is critical in human-robot teams, but operators experience SA gaps under high workload and dynamic conditions. Conventional SA measures either disrupt task flow or cannot capture real-time fluctuations, limiting their operational utility. No publicly available dataset exists for systematic evaluation of online SA assessment in human-robot teaming.

Method: HRI-SA dataset collected from 30 participants in a realistic search-and-rescue human-robot teaming context, incorporating eye movements, pupil diameter, biosignals, user interactions, and robot data. Ground truth SA latency (perceptual and comprehension types) obtained by measuring time between assistance need onset and resolution. Standard machine learning models evaluated using generic eye-tracking features and contextual features.
...
Conclusion: HRI-SA is the first public dataset supporting systematic SA evaluation throughout a human-robot teaming mission, demonstrating that generic eye-tracking features have strong potential for continuous perceptual SA latency detection in remote human-robot teaming scenarios.
...
### [2] [Uncovering Latent Phase Structures and Branching Logic in Locomotion Policies: A Case Study on HalfCheetah](https://arxiv.org/abs/2603.18084)
*Daisuke Yasui,Toshitaka Matsuki,Hiroshi Sato*

Main category: cs.RO

TL;DR: This paper demonstrates that deep reinforcement learning (DRL) policies for locomotion control can autonomously acquire interpretable phase structures (periodic phase transitions and branching) similar to human-perceived motion phases, which can be analyzed using Explainable Boosting Machines.


<details>
  <summary>Details</summary>
Motivation: DRL policies in locomotion control are often treated as black boxes, making it difficult for humans to understand the decision-making process. However, human-perceived periodic motions like walking have interpretable phases (stance/swing). The authors hypothesize that DRL policies may also represent such phase structures that are interpretable to humans.

Method: In the MuJoCo HalfCheetah-v5 benchmark, state transition sequences from a trained policy were aggregated into semantic phases based on state similarity and transition consistency. Explainable Boosting Machines (EBMs) were then used to approximate the states and actions corresponding to each semantic phase to analyze phase-dependent decision making.

Result: The trained policy exhibits periodic phase transition structures and phase branching. Using EBMs, the study identified which state features the policy attends to and how it controls action outputs in each phase.

Conclusion: Neural network-based policies can autonomously acquire interpretable phase structures and logical branching mechanisms, thereby reducing the black box nature of DRL decision-making processes.
...
### [3] [Fire as a Service: Augmenting Robot Simulators with Thermally and Visually Accurate Fire Dynamics](https://arxiv.org/abs/2603.19063)
*Anton R. Wagner,Madhan Balaji Rao,Helge Wrede,Sören Pirk,Xuesu Xiao*

Main category: cs.RO

TL;DR: Fire as a Service (FaaS) is a novel co-simulation framework that integrates high-fidelity fire simulations with existing robot simulators, enabling accurate training and evaluation of firefighting robots through real-time, thermally realistic environments.


<details>
  <summary>Details</summary>
Motivation: Existing robot simulators prioritize rigid-body dynamics and photorealistic rendering but neglect the thermally and optically complex phenomena of real-world fire environments. This limitation hinders reliable capability evaluation and representative training data generation for firefighting robots operating in hazardous scenarios.

Method: FaaS is an asynchronous co-simulation framework that augments robot simulators with computationally efficient, high-fidelity fire simulations. It provides multi-species thermodynamic heat transfer and visually consistent volumetric smoke without disrupting high-frequency rigid-body control loops. The framework is designed for integration with diverse robot simulators.

Result: The framework successfully generates physically accurate fire behavior, benchmarks thermal hazards for robotic platforms, collects realistic multimodal perceptual data, supports real-time human-in-the-loop teleoperation, and enables effective training of reactive, multimodal policies via Behavioral Cloning across different robot simulators.

Conclusion: By adding fire dynamics to robot simulations, FaaS provides a scalable pathway toward safer, more reliable deployment of robots in fire scenarios, addressing the critical gap in current simulation capabilities for firefighting applications.
...
### [4] [Final Report for the Workshop on Robotics & AI in Medicine](https://arxiv.org/abs/2603.18130)
*Juan P Wachs*

Main category: cs.RO

TL;DR: This report summarizes the CARE Workshop on Robotics and AI in Medicine (Dec 1, 2025), which brought together researchers, clinicians, industry leaders, and federal stakeholders to identify gaps and establish priorities for advancing AI-enabled robotic systems in healthcare, culminating in a consensus to create a national Center for AI and Robotic Excellence in Medicine (CARE).


<details>
  <summary>Details</summary>
Motivation: To address the accelerating need for coordinated research efforts bridging engineering innovation with clinical priorities, focusing on safety, reliability, and translational readiness for intelligent robotic systems in surgical, diagnostic, rehabilitative, and assistive contexts, particularly in underserved regions, disaster relief, and military settings.

Method: The workshop convened leading researchers, clinicians, industry innovators, and federal stakeholders through keynotes, panels, and breakout sessions to collectively identify gaps and formulate a national vision for robotics and AI in healthcare.

Result: Participants identified critical gaps in data availability, standardized evaluation methods, regulatory pathways, and workforce training. They established broad consensus on establishing a national CARE center with priority research thrusts including human-robot collaboration, trustworthy autonomy, simulation/digital twins, multi-modal sensing, and ethical integration of generative AI into clinical workflows.

Conclusion: A national Center for AI and Robotic Excellence in Medicine (CARE) should be established to coordinate research, develop high-quality datasets and shared test beds, create clinically grounded benchmarks, and implement sustained interdisciplinary training mechanisms to accelerate the safe and effective deployment of AI-enabled robotic systems in healthcare.
...
### [5] [GoalVLM: VLM-driven Object Goal Navigation for Multi-Agent System](https://arxiv.org/abs/2603.18210)
*MoniJesu James,Amir Atef Habel,Aleksey Fedoseev,Dzmitry Tsetserokou*

Main category: cs.RO

TL;DR: GoalVLM is a cooperative multi-agent framework for zero-shot, open-vocabulary object navigation that integrates a Vision-Language Model (VLM) into the decision loop, enabling agents to interpret free-form language goals and score frontiers without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Traditional object-goal navigation is limited to ground robots with closed-set object vocabularies. Existing multi-agent approaches depend on precomputed probabilistic graphs tied to fixed category sets, which prevents generalization to novel goals at test time.

Method: GoalVLM integrates a VLM directly into the decision loop with SAM3 for text-prompted detection and segmentation, and SpaceOM for spatial reasoning. Each agent builds a BEV semantic map from depth-projected voxel splatting, while a Goal Projector back-projects detections through calibrated depth for reliable goal localization. A constraint-guided reasoning layer evaluates frontiers through a structured prompt chain (scene captioning, room-type classification, perception gating, multi-frontier ranking).

Result: On GOAT-Bench val_unseen (360 multi-subtask episodes with 5-7 sequential open-vocabulary targets), GoalVLM with N=2 agents achieves 55.8% subtask success rate and 18.3% SPL, competitive with state-of-the-art methods while requiring no task-specific training. Ablation studies confirm the contributions of VLM-guided frontier reasoning and depth-projected goal localization.

Conclusion: The framework successfully enables zero-shot, open-vocabulary object navigation through integration of VLMs into the decision loop, demonstrating that agents can interpret free-form language goals and score frontiers without retraining.
...
### [6] [ReDAG-RT: Global Rate-Priority Scheduling for Real-Time Multi-DAG Execution in ROS 2](https://arxiv.org/abs/2603.18238)
...
### [7] [Rapid Adaptation of Particle Dynamics for Generalized Deformable Object Mobile Manipulation](https://arxiv.org/abs/2603.18246)
...
### [8] [Manufacturing Micro-Patterned Surfaces with Multi-Robot Systems](https://arxiv.org/abs/2603.18260)
...
### [9] [SG-CoT: An Ambiguity-Aware Robotic Planning Framework using Scene Graph Representations](https://arxiv.org/abs/2603.18271)
...
### [10] [Offload or Overload: A Platform Measurement Study of Mobile Robotic Manipulation Workloads](https://arxiv.org/abs/2603.18284)
...
### [11] [Sparse3DTrack: Monocular 3D Object Tracking Using Sparse Supervision](https://arxiv.org/abs/2603.18298)
...
### [12] [Proprioceptive-only State Estimation for Legged Robots with Set-Coverage Measurements of Learned Dynamics](https://arxiv.org/abs/2603.18308)
...
### [13] [DriveVLM-RL: Neuroscience-Inspired Reinforcement Learning with Vision-Language Models for Safe and Deployable Autonomous Driving](https://arxiv.org/abs/2603.18315)
...
### [14] [ManiDreams: An Open-Source Library for Robust Object Manipulation via Uncertainty-aware Task-specific Intuitive Physics](https://arxiv.org/abs/2603.18336)
...
### [15] [Shifting Uncertainty to Critical Moments: Towards Reliable Uncertainty Quantification for VLA Model](https://arxiv.org/abs/2603.18342)
...
### [16] [Multi-material Direct Ink Writing and Embroidery for Stretchable Wearable Sensors](https://arxiv.org/abs/2603.18354)
...
### [17] [Contact Status Recognition and Slip Detection with a Bio-inspired Tactile Hand](https://arxiv.org/abs/2603.18370)
...
### [18] [Graph-of-Constraints Model Predictive Control for Reactive Multi-agent Task and Motion Planning](https://arxiv.org/abs/2603.18400)
...
### [19] [Efficient and Versatile Quadrupedal Skating: Optimal Co-design via Reinforcement Learning and Bayesian Optimization](https://arxiv.org/abs/2603.18408)
...
### [20] [MemoAct: Atkinson-Shiffrin-Inspired Memory-Augmented Visuomotor Policy for Robotic Manipulation](https://arxiv.org/abs/2603.18494)
...
### [21] [Robotic Agentic Platform for Intelligent Electric Vehicle Disassembly](https://arxiv.org/abs/2603.18520)
...
### [22] [Scaling Sim-to-Real Reinforcement Learning for Robot VLAs with Generative 3D Worlds](https://arxiv.org/abs/2603.18532)
...
### [23] [Inductance-Based Force Self-Sensing in Fiber-Reinforced Pneumatic Twisted-and-Coiled Actuators](https://arxiv.org/abs/2603.18555)
...
### [24] [TiBCLaG: A Trigger-induced Bistable Compliant Laparoscopic Grasper](https://arxiv.org/abs/2603.18559)
...
### [25] [Benchmarking Visual Feature Representations for LiDAR-Inertial-Visual Odometry Under Challenging Conditions](https://arxiv.org/abs/2603.18589)
...
### [26] [REST: Receding Horizon Explorative Steiner Tree for Zero-Shot Object-Goal Navigation](https://arxiv.org/abs/2603.18624)
...
### [27] [CSSDF-Net: Safe Motion Planning Based on Neural Implicit Representations of Configuration Space Distance Field](https://arxiv.org/abs/2603.18669)
...
### [28] [ROFT-VINS: Robust Feature Tracking-based Visual-Inertial State Estimation for Harsh Environment](https://arxiv.org/abs/2603.18746)
...
### [29] [Empathetic Motion Generation for Humanoid Educational Robots via Reasoning-Guided Vision--Language--Motion Diffusion Architecture](https://arxiv.org/abs/2603.18771)
...
### [30] [ViTac-Tracing: Visual-Tactile Imitation Learning of Deformable Object Tracing](https://arxiv.org/abs/2603.18784)
...
### [31] ["You've got a friend in me": Co-Designing a Peer Social Robot for Young Newcomers' Language and Cultural Learning](https://arxiv.org/abs/2603.18804)
...
### [32] [V-Dreamer: Automating Robotic Simulation and Trajectory Synthesis via Video Generation Priors](https://arxiv.org/abs/2603.18811)
...
### [33] [A Passive Elastic-Folding Mechanism for Stackable Airdrop Sensors](https://arxiv.org/abs/2603.18861)
...
### [34] [Safety-Guaranteed Imitation Learning from Nonlinear Model Predictive Control for Spacecraft Close Proximity Operations](https://arxiv.org/abs/2603.18910)
...
### [35] [Lightweight Model Predictive Control for Spacecraft Rendezvous Attitude Synchronization](https://arxiv.org/abs/2603.18921)
...
### [36] [PRIOR: Perceptive Learning for Humanoid Locomotion with Reference Gait Priors](https://arxiv.org/abs/2603.18979)
...
### [37] [MERGE: Guided Vision-Language Models for Multi-Actor Event Reasoning and Grounding in Human-Robot Interaction](https://arxiv.org/abs/2603.18988)
...
### [38] [ATG-MoE: Autoregressive trajectory generation with mixture-of-experts for assembly skill learning](https://arxiv.org/abs/2603.19029)
...
### [39] [CAMO: A Conditional Neural Solver for the Multi-objective Multiple Traveling Salesman Problem](https://arxiv.org/abs/2603.19074)
...
### [40] [Articulated-Body Dynamics Network: Dynamics-Grounded Prior for Robot Learning](https://arxiv.org/abs/2603.19078)
...
### [41] [Tendon-Actuated Robots with a Tapered, Flexible Polymer Backbone: Design, Fabrication, and Modeling](https://arxiv.org/abs/2603.19124)
...
### [42] [Introducing M: A Modular, Modifiable Social Robot](https://arxiv.org/abs/2603.19134)
...
### [43] [Meanings and Measurements: Multi-Agent Probabilistic Grounding for Vision-Language Navigation](https://arxiv.org/abs/2603.19166)
...
### [44] [ADMM-Based Distributed MPC with Control Barrier Functions for Safe Multi-Robot Quadrupedal Locomotion](https://arxiv.org/abs/2603.19170)
...
### [45] [Sparse Autoencoders Reveal Interpretable and Steerable Features in VLA Models](https://arxiv.org/abs/2603.19183)
...
### [46] [FASTER: Rethinking Real-Time Flow VLAs](https://arxiv.org/abs/2603.19199)
...
### [47] [OmniVTA: Visuo-Tactile World Modeling for Contact-Rich Robotic Manipulation](https://arxiv.org/abs/2603.19201)
...
### [48] [NavTrust: Benchmarking Trustworthiness for Embodied Navigation](https://arxiv.org/abs/2603.19229)
...
### [49] [Not All Features Are Created Equal: A Mechanistic Study of Vision-Language-Action Models](https://arxiv.org/abs/2603.19233)
...
### [50] [The Provenance Paradox in Multi-Agent LLM Routing: Delegation Contracts and Attested Identity in LDP](https://arxiv.org/abs/2603.18043)
*Sunil Prakash*
...
### [51] [A Trace-Based Assurance Framework for Agentic AI Orchestration: Contracts, Testing, and Governance](https://arxiv.org/abs/2603.18096)
*Ciprian Paduraru,Petru-Liviu Bouruc,Alin Stefanescu*

Main category: cs.MA

TL;DR: This paper presents an assurance framework for testing and evaluating multi-agent LLM systems that coordinate agents and interact with external services, addressing failures beyond incorrect outputs including non-termination, role drift, and side effects.


<details>
  <summary>Details</summary>
Motivation: Agentic AI systems use LLMs to orchestrate multiple agents and interact with external services. Failures extend beyond incorrect final outputs to include long-horizon interaction issues, stochastic decisions, external side effects (API calls, database writes), and attacks via untrusted context. Common failures include non-termination, role drift, propagation of unsupported claims, and security attacks.

Method: The framework instruments executions as Message-Action Traces (MAT) with explicit step and trace contracts providing machine-checkable verdicts, step localization, and deterministic replay. It includes budgeted counterexample search for stress testing over bounded perturbations, structured fault injection at service/retrieval/memory boundaries, and runtime governance enforcing per-agent capability limits and action mediation (allow/rewrite/block) at the language-to-action boundary.

Result: The paper defines trace-based metrics for task success, termination reliability, contract compliance, factuality indicators, containment rate, and governance outcome distributions to support comparative evaluations across stochastic seeds, models, and orchestration configurations.

Conclusion: The framework serves as a common abstraction for testing and evaluation of multi-agent LLM systems, enabling reproducible comparisons across orchestration designs and configurations.
