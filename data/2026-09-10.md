...
### [1] [InstantMimic: A High Performance System for Learning Physics-based Skills in Seconds](https://arxiv.org/abs/2609.09821)
...
### [2] [RealSimLoop: Online Real-to-Sim Adaptation via Differentiable Reduced-Order Simulation with Vision Feedback](https://arxiv.org/abs/2609.09828)
...
### [3] [Semi-Implicit Pairwise Descent for Nonlocal Continuum Mechanics](https://arxiv.org/abs/2609.09834)
...
### [4] [Agentic AI-enabled Semantic Commissioning of a Cognitive Digital Twin for Reconfigurable Manufacturing](https://arxiv.org/abs/2609.09503)
...
TL;DR: This paper analyzes teleoperated demonstrations, showing that multimodality in such demonstrations arises from the entanglement of operator habits, shared physics, and observation nuisance in next-observation predictors. It proposes freezing the shared physics readout and only updating a thin interface for adaptation, which improves low-shot transfer across multiple datasets including StackCube, DROID, and RH20T.


<details>
  <summary>Details</summary>
Motivation: Teleoperated demonstrations often appear multimodal even when the underlying dynamics are deterministic given the executed action. The authors aim to understand the source of this multimodality and develop better adaptation rules for robot learning from demonstrations.

Method: The authors formalize the problem using a structural causal model with action selection a = g(h, z, u), state transition z' = f(z, a), and observation o = r(z, c), where h represents operator habit. They validate this model through complementary interventions: replacing/shuffling actions at fixed states (which should increase next-state error if habits and physics are entangled) and testing appearance/camera changes (which should not affect the model). They also propose a habit-aware reverse scoring method to rank feasible pasts. The adaptation rule is to freeze the shared physics readout and only update a thin interface.

Result: The structural causal model is validated through experiments on StackCube, DROID, and RH20T datasets. The proposed adaptation rule improves low-shot transfer relative to training from scratch, retains cleaner dynamics under corrupted adaptation data, and extends from proprioception to pixel observations with multi-view and multi-step checks. Habit-aware reverse scoring improves ranking of feasible pasts.

Conclusion: The authors demonstrate that operator habits, shared physics, and observation nuisance are entangled in next-observation predictors in teleoperated demonstrations. By freezing the shared physics readout and updating only a thin interface, they achieve better low-shot transfer and cleaner dynamics modeling. This approach works across different datasets and modalities (proprioception and pixel observations) without equating latent actions with operator habit or targeting video generation benchmarks.
...
### [6] [A traffic management system for large and heterogeneous vehicles in narrow industrial environments](https://arxiv.org/abs/2609.10400)
...
### [7] [Geometry Conditioning in an Embodied SLM: Training Controls and Robustness Diagnostics in a 0.8B Hybrid Model](https://arxiv.org/abs/2609.09213)
*Hao Li,Haofei Sun,Lin He*

Main category: cs.RO

TL;DR: This paper investigates whether conditioning a small hybrid language model (0.8B parameters, 6.2M trainable) on geometric increments improves its performance on LIBERO-Spatial manipulation tasks, finding no reliable advantage while highlighting the gap between coordinate invariance and physical-layout generalization.


<details>
  <summary>Details</summary>
Motivation: The research aims to understand how physical-state inputs affect manipulation performance in language models, specifically testing whether conditioning recurrent decay gates on geometric increments provides a reliable advantage over other approaches, and to explore the relationship between coordinate invariance and physical-layout generalization in robotic manipulation tasks.

Method: A 0.8B hybrid language model with 6.2M trainable parameters was trained on three LIBERO-Spatial tasks across six conditions: geometric increments, shuffled increments, no explicit geometry, token adapter with increments, and token-clock conditioning. The model was evaluated over three seeds and 540 held-out rollouts. Robustness tests included frame relabeling (state-only policy) and 5cm object displacement (visual policies).

Result: Shuffled increments achieved 36.7% success (highest), while geometric increments with correct inputs yielded 28.9%, and no explicit geometry achieved 24.4%. Token adapter scored 27.8% (inconclusive), token-clock conditioning scored 11.1% (failed to converge in one seed). Under frame relabeling, state-only relative-coordinate policy retained 7/10 success, but all four visual policies dropped to at most 3/20 after 5cm object displacement.

Conclusion: Training-time geometric alignment provided no reliable advantage for the hybrid language model in this manipulation task setup. The results highlight a significant gap between coordinate invariance and physical-layout generalization, suggesting that visual representations alone are insufficient for robust manipulation performance under physical perturbations.
...
### [8] [Design and Attitude Control of an Underwater Quadruped Robot](https://arxiv.org/abs/2609.09217)
*Davide Molinaroli,Mohit Singh,Kostas Alexis*

Main category: cs.RO

TL;DR: This paper presents a reproducible underwater quadruped robot design with a simplified dynamics model and a closed-loop attitude controller on SO(3), validated through simulation and water tank experiments.


<details>
  <summary>Details</summary>
Motivation: Legged robots excel on land but are limited in underwater environments. Extending quadruped locomotion to water enables amphibious mobility for applications in inspection, environmental monitoring, and disaster response.

Method: The robot uses custom waterproof motor housings machined from polyoxymethylene plastic with off-the-shelf O-rings and dynamic shaft seals. A simplified dynamics model captures drag forces on spherical end effectors transmitting torque to the floating base. A closed-loop attitude controller is developed using SO(3) error formulation.

Result: The controller is validated both in simulation and experimental water tank tests, demonstrating successful tracking of desired orientation setpoints in roll, pitch, and yaw.

Conclusion: The paper successfully presents a reproducible underwater quadruped robot design with experimental validation of its attitude control system.
...
TL;DR: This paper investigates long-horizon UAV visual servoing using compact target-centric cues (image-space offsets/relative depth from lightweight segmentation) combined with proprioceptive measurements (velocity, projected-gravity) into a 12D policy observation. Comparing Direct PPO with three curriculum strategies (Visual, Dynamics, Joint), all achieve comparable performance with complementary advantages. Proprioceptive measurements are critical for stable flight, image-space cues for target alignment, while explicit depth is unnecessary. Learned policies outperform tuned classical controllers in robustness to perturbations, with the Visual curriculum showing smallest degradation under unseen target motion.


<details>
  <summary>Details</summary>
Motivation: Vision-guided reinforcement learning for UAVs faces challenges including unstable policy optimization, aggressive exploration, and the computational cost of high-dimensional visual perception. The authors investigate long-horizon UAV visual servoing using compact target-centric cues combined with low-dimensional sensor measurements to address these limitations.

Method: Using lightweight target segmentation to provide image-space offsets and relative depth, combined with quadrotor velocity and projected-gravity measurements into a 12D policy observation. Compares Direct PPO against three matched-budget curriculum strategies: Visual curriculum (progressively expands target placement difficulty), Dynamics curriculum (gradually relaxes action constraints and smoothing), and Joint curriculum (combines both progressions).

Result: All curriculum strategies achieve comparable nominal performance with complementary advantages across tracking metrics. Observation ablations confirm proprioceptive measurements are critical for stable flight and image-space cues are essential for target alignment, while explicit depth is not necessary. Learned policies demonstrate greater robustness to control and visual perturbations than tuned classical controllers, with the Visual curriculum exhibiting the smallest degradation under unseen target motion.

Conclusion: Compact target-centric representations can support robust long-horizon aerial visual servoing. Visual curriculum training improves robustness to dynamic distribution shifts despite limited gains in nominal performance. The approach effectively balances computational efficiency with strong performance in challenging UAV navigation scenarios.
...
TL;DR: This paper surveys 150+ verifiers for robot policies that evaluate candidate behaviors through scoring. It compares verifiers across availability (cost, timing, frequency) and credibility (trustworthiness of scores) properties, finding credibility decreases as availability increases - no "free checker" exists regardless of verifier type. The paper proposes nine checkable metrics for validating verifiers and identifies directions for future verifier development.


<details>
  <summary>Details</summary>
Motivation: Verifiers are essential for both evaluating and training vision-language-action policies, ranging from simple success detectors to complex safety filters and temporal-logic specifications. However, there's a need to understand how to choose and validate these verifiers, particularly trade-offs between their availability and credibility.

Method: The authors survey approximately 150 verifiers and categorize them by judgment supplier: human verifiers, rule-based/formal verifiers, learned/pretrained verifiers, and model-intrinsic verifiers. They compare verifiers along two key properties: availability (verdict cost, timing, and frequency) and credibility (trustworthiness of high scores). They examine validation measures including agreement with human labels, policy performance improvement, and behavior under reward hacking.

Result: Across the four verifier families, credibility consistently falls as availability rises. Regardless of who supplies the judgment (human, rule-based, learned, or model-intrinsic), there are unavoidable trade-offs - "no free checker" exists. This reveals fundamental limitations in verifier design.

Conclusion: The paper examines what validates a verifier and proposes nine metrics that make verifier claims checkable. It also provides coordinates (directions) for the verifiers that still need to be built, guiding future research in this important area of robot policy evaluation.
...
### [12] [A Decade of Bayesian Optimization for Controller Tuning and Robot Learning: Tutorial, Review, and Future Prospects](https://arxiv.org/abs/2609.09403)
*David Stenger,Paul Brunzema,Johanna Menn,Alexander von Rohr,Angela P. Schoellig,Sebastian Trimpe*

Main category: cs.RO

TL;DR: A comprehensive overview of Bayesian optimization methods for automatic controller tuning and robot learning, with a focus on control systems and robotics, including practical setup guidance, method classification, and introduction of a lightweight benchmark suite with evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Bayesian optimization has emerged as a powerful framework for automatic controller tuning and robot learning in the past decade. The authors aim to help researchers and practitioners understand recent advancements, practical applications, and future research directions in this field.

Method: The paper adopts a practitioner's perspective, illustrating BO setup through a representative controller tuning example. It positions BO within broader learning paradigms (deep reinforcement learning, data-driven control) and discusses diverse BO methods for complex problems. Additionally, the authors introduce a lightweight benchmark suite for control engineering and robotics with metrics and best practices for algorithm comparison.

Result: The article provides a unified perspective on the current BO landscape, emphasizing its relevance to control systems and robotics. It identifies key research challenges and promising avenues for advancing BO, and highlights the need for standardized benchmark problems in control-related applications.

Conclusion: The authors identify a significant gap in the BO landscape: the lack of standardized benchmark problems specifically for control-related applications. To foster future research and rigorous evaluation, they initiate an effort towards a lightweight benchmark suite and present metrics and best practices for direct comparisons between new BO algorithms and established state-of-the-art methods.
...
### [13] [Actuator Dynamics Curricula for Narrow-Viability Tasks in Legged Robot Learning](https://arxiv.org/abs/2609.09492)
*Kousheek Chakraborty,Chandan K. Rajendra,Ayham Alharbat,Abeje Y. Mersha*

Main category: cs.RO

TL;DR: The authors introduce an Actuator Dynamics Curriculum that initializes joint stiffness high and anneals it during training, helping RL tasks converge when exploration terminates before providing useful gradients.


<details>
  <summary>Details</summary>
Motivation: Standard RL training fails for some tasks where exploration trajectories terminate before producing useful gradient signals. This creates a bottleneck in tasks where termination conditions rather than reward signals limit exploration, preventing convergence to capable controllers.

Method: They propose an Actuator Dynamics Curriculum that initializes joint stiffness at a high value and anneals it toward the system-identified value as completed episode lengths grow. They validate the approach on a cart-pole system and apply it to a quadrupedal-to-handstand transition task on Boston Dynamics Spot.

Result: Higher closed-loop joint natural frequency under critical damping enlarges the viability kernel (fraction of initial states from which the task is feasible). The curriculum successfully enables the quadrupedal-to-handstand transition, which otherwise plateaus at a policy that never completes the transition. The trained policy executes the transition in simulation across 10 seeds and transfers to hardware.

Conclusion: Simulated actuator dynamics is a useful axis for designing curricula for tasks where exploration is bottlenecked by termination conditions rather than reward signal. This provides a general approach for improving convergence in challenging RL tasks across various robotic platforms.
...
### [14] [Compact Visuotactile World Models for Lifting: Prediction, Reward Alignment, and Force Constraints](https://arxiv.org/abs/2609.09597)
...
### [15] [MuJoCable: Reduced-Order Surface-Routed Cable Transmission for Tendon-Driven Robots](https://arxiv.org/abs/2609.09612)
...
### [16] [JEPA Policy: Diffusion-Free Imitation Learning via Paired Action and Future Representation Prediction](https://arxiv.org/abs/2609.09630)
...
### [17] [A Risk-Sensitive and Uncertainty-Aware Decision-Making and Control Framework for Safe and Robust Autonomous Driving](https://arxiv.org/abs/2609.09650)
...
### [18] [CT-SAFR: Safe and Interpretable Chain-of-Thought Reasoning for Autonomous Robots: A Multi-Layered Verification Framework for Trustworthy AI-Driven Robotic Decision Making](https://arxiv.org/abs/2609.09692)
*Cagri Temel*
...
### [19] [PccDiffuser: Multi-solution Motion Planning for Continuum Robots](https://arxiv.org/abs/2609.09745)
...
### [20] [HiRAD: A Flexible Large-Scale AGV Routing System](https://arxiv.org/abs/2609.09752)
...
### [21] [GTA-2: A Multi-VLM Framework for Synthesizing Robot Manipulation Skills via Grounded Task Axes](https://arxiv.org/abs/2609.09808)
*M. Yunus Seker,Shobhit Aggarwal,Ruwan Wickramarachchi,Jonathan Francis,Oliver Kroemer*

Main category: cs.RO

TL;DR: GTA-2 is a modular multi-VLM framework that constructs executable, task-bespoke manipulation skills from reusable object-centric task-axis components, enabling zero-shot skill generation with 73.9% average success rate (90.7% with targeted refinement) on 14 real-robot tasks.


<details>
  <summary>Details</summary>
Motivation: Existing robotic manipulation skills are often too coarse, lacking the geometric, control, and scene-dependent decisions required for execution. Current approaches either predefine behaviors for specific tasks or use generic skills that cannot be easily refined.

Method: GTA-2 uses four specialized VLM agents to: (1) decompose tasks, (2) construct abstract task-axis skills with keypoints/axes and controller compositions, (3) assign controller parameters, and (4) ground visual features from RGB-D. Skills are represented as semantic subtasks rather than end-to-end actions.

Result: GTA-2 achieves 73.9% average zero-shot success rate on 14 real-robot manipulation tasks, exceeding the strongest baseline by 31.4 percentage points. With targeted human feedback refinement, success rate reaches 90.7%.

Conclusion: The abstraction-to-grounding factorization enables zero-shot skill generation without task-specific demonstrations or fine-tuning while keeping intermediate decisions explicit for targeted human feedback, demonstrating superior performance over VLA and Code-as-Policies baselines.
...
### [22] [ViBe: Visual Behavior Adaptation for Perceptive Humanoid Whole-Body Control](https://arxiv.org/abs/2609.09918)
...
### [23] [HaWMPO: Hallucination-Aware World Model-based Policy Optimization for Generalist Robot Policy](https://arxiv.org/abs/2609.09941)
...
### [24] [RoboDrop: Curating VLA Post-Training Data via Local Gradient Compatibility](https://arxiv.org/abs/2609.10021)
...
### [26] [What Symmetry Buys a Learned Motion Planner](https://arxiv.org/abs/2609.10033)
*Andrea Emir Sevincel*

Main category: cs.RO

TL;DR: Learning-based motion planners trained in world coordinates lack rigid-body equivariance. The authors show that expressing trajectories and obstacles in a frame defined by start and goal positions (removing 3 translations and 2 rotations from SE(3)) significantly improves performance, raising collision-free rate from 14.60% to 51.10% on a cluttered 3D benchmark, demonstrating that the representation supplied dominates what any mechanism enforces.


<details>
  <summary>Details</summary>
Motivation: Learning-based motion planners pay at training what classical planners pay per query, and trained in world coordinates they relearn the same motion at every position and orientation. Existing work restores missing rigid-body equivariance in training data, inference operator, or weights, but each carries a cost. The authors ask how much equivariance the planning query supplies for free.

Method: The authors define a frame in closed form with origin at the midpoint of start s and goal g and first axis along g-s. Expressing trajectory and obstacles in this frame removes three translations and two rotations from SE(3) during initialization, requiring only one cross product per query with no constraint on architecture. Three mechanisms for the residual rotation about the start-goal axis are built and evaluated.

Result: On a cluttered 3D benchmark, holding architecture, data, and budget fixed, the frame raises the held-out collision-free rate from 14.60% to 51.10%, where a straight segment from start to goal scores only 15.6% and the world-frame model does not beat it. Local geometry is worth +40.0 where the frame is worth +36.5.

Conclusion: The representation supplied dominates what any mechanism enforces, and the standard diagnostic does not see the difference: two models with indistinguishable non-equivariance residuals differ by 28 points. The equivariant backbone reaches any given level two to three times sooner than individual mechanisms, and the frame is not even the largest effect available.
...
### [27] [Grounding Generated Video Plans in Simulation Towards Versatile Dexterous Controllers](https://arxiv.org/abs/2609.10050)
...
### [28] [Automatic Reproducible Camera Intrinsic Calibration](https://arxiv.org/abs/2609.10082)
*Xiangcheng Hu*
...
### [29] [Assembling Two Parts in One Hand](https://arxiv.org/abs/2609.10137)
...
### [30] [Future-Aware Flow Planning for Safe UAV Target Following](https://arxiv.org/abs/2609.10166)
...
### [31] [Multi-Robot Scanner for Automated Full-Body Dermoscopic Imaging](https://arxiv.org/abs/2609.10169)
...
### [32] [Adaptive Shared Control with Online Bounded-Rational Human Behavior Estimation](https://arxiv.org/abs/2609.10215)
...
### [33] [CougarTail & CUB: A General-Purpose Mast and Central Utility Board for Cylindrical Underwater Enclosures](https://arxiv.org/abs/2609.10230)
...
### [34] [FolDeX: A Physical-World Benchmark for Long-Horizon Robotic Manipulation of Deformable Objects](https://arxiv.org/abs/2609.10243)
...
### [35] [SwingBot: Learning Whole-Body Brachiation for Humanoid Robots](https://arxiv.org/abs/2609.10283)
...
### [36] [Learning Terrain-Adaptive Humanoid Locomotion on Granular Terrain](https://arxiv.org/abs/2609.10286)
...
### [37] [Deformable Object Manipulation under Partial Observability via Real-Time Full-Shape Estimation](https://arxiv.org/abs/2609.10308)
...
### [38] [Odometer-Agnostic Drift Correction Using OpenStreetMap Lane Geometry](https://arxiv.org/abs/2609.10336)
*Joaquin Caballero,Emilio Garcia-Fidalgo,Alberto Ortiz,Jarno Ralli*

Main category: cs.RO

TL;DR: A lightweight, odometry-agnostic method that aligns short trajectory segments to OpenStreetMap lane centerlines to reduce long-term drift in odometry estimation, avoiding dense maps and complex preprocessing.


<details>
  <summary>Details</summary>
Motivation: Long-term drift remains a fundamental limitation of incremental pose integration in odometry estimation, especially in large-scale or loop-free environments. Existing map-assisted methods can reduce drift but depend on dense maps, sensor-specific processing, or complex matching pipelines.

Method: A lightweight open-source, odometry-agnostic correction method that aligns short trajectory segments to OpenStreetMap (OSM) lane centerlines. The approach formulates drift correction as a direct alignment between recent odometry and sparse lane geometry, enabling efficient online operation without dense priors or expensive preprocessing.

Result: Experiments with LiDAR and visual odometry backends demonstrate consistent improvements, with particularly strong gains under severe drift conditions.

Conclusion: The method effectively reduces long-term drift in odometry estimation while maintaining efficiency and generality across different sensor types, offering a practical solution for large-scale navigation without requiring complex infrastructure or dense map representations.
...
### [39] [A Confidence-Aware Multimodal Fusion Framework for Industrial Human-Robot Collaboration](https://arxiv.org/abs/2609.10339)
...
### [40] [Data-Driven Risk Fields for Safer End-to-End Autonomous Driving](https://arxiv.org/abs/2609.10377)
...
TL;DR: A deep reinforcement learning framework for training UAV agents to navigate and monitor simulated wildfire environments, demonstrating stable and effective learning behaviors over time.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of deep reinforcement learning (DRL) based UAV systems for autonomous wildfire monitoring, and to understand how environmental structure and reward design influence policy effectiveness.

Method: Developed a deep reinforcement learning framework for training Unmanned Aerial Vehicle (UAV) agents to navigate and monitor simulated wildfire environments.

Result: Agents learned increasingly stable and effective behaviors over time, evidenced by converging loss trends, improved reward signals, and more consistent navigation patterns such as fire-boundary tracking.

Conclusion: Deep reinforcement learning-based UAV systems show promise for autonomous wildfire monitoring, and findings suggest that both environmental structure and reward design significantly influence policy effectiveness.
...
TL;DR: This paper introduces an Unreal Engine 5 pipeline for automatic generation of coastal environments in the HoloOcean marine robotics simulation, addressing limitations in existing simulation tools that require manual editing or specialized knowledge.


<details>
  <summary>Details</summary>
Motivation: Marine robotics simulations are essential for safe and cost-effective development of UUV and USV autonomy and perception algorithms. However, current simulation suites are limited by the availability of simulated environments, requiring manual creation or editing that demands existing data or specialized knowledge of environment systems.

Method: The researchers developed a novel Unreal Engine 5 level generation pipeline that takes a user-provided overhead image of a coastal scene as input. The pipeline processes the image to generate height map data, automatically selects appropriate assets, and places them within the environment.

Result: The pipeline successfully enables automatic creation of coastal environments for HoloOcean, overcoming the limitations of manual environment creation methods.

Conclusion: This automated pipeline provides a more accessible and efficient approach to creating realistic marine simulation environments, reducing the technical barriers for researchers developing UUV and USV autonomy and perception algorithms.
...
TL;DR: Show-Harness is an embodied harness that enables vision-language models (VLMs) to control robots through a compact semantic interface, bridging high-level intent with low-level actions while maintaining the VLM's responsibility for fine-grained decisions.


<details>
  <summary>Details</summary>
Motivation: Foundation VLMs possess broad world intelligence but struggle to translate this into effective robot control. The challenge lies in creating an interface that allows VLMs to reason over actions while preserving the VLM's decision-making responsibility.

Method: Show-Harness exposes discrete semantic action units that VLMs can naturally reason over, with embodiment-specific interpreters deterministically grounding these units into local robot actions. A GUI Manipulation Interface (GUMI) extends this approach to GUI-based demonstration collection, enabling cross-embodiment teleoperation without specialized hardware.

Result: Show-Harness enables zero-shot control with closed-source frontier VLMs and low-cost adaptation of small open-source VLMs (requiring only a few GPU-hours of fine-tuning). GUMI allows humans and agents to "play" robots across embodiments. Show-Harness-equipped VLM agents achieve robust generalization across tasks, embodiments, and environments, outperforming representative agentic and VLA paradigms.

Conclusion: The right interface can unlock substantial embodied capability from foundation VLMs without requiring additional model capacity or costly embodiment-specific pretraining, demonstrating that semantic action interfaces are a viable pathway for VLM-based robot control.
...
### [46] [Scores Alone Do Not Prove Discovery: The Discovery Certification Protocol for Auditing AI Research Agents](https://arxiv.org/abs/2609.09219)
...
### [47] [Who Are They to Each Other? Multi-Agent Reasoning for Speaker Relationship Inference](https://arxiv.org/abs/2609.09628)
*Yaohan Guan,Yen-Ju Lu,Yuzhe Wang,Junhyeok Lee,Jesus Villalba,Laureano Moro Velazquez,Thomas Thebaud,Najim Dehak*

Main category: cs.MA

TL;DR: A training-free multi-agent reasoning framework for inferring speaker relationships from conversations, using Multi-Role Multi-Agent Debate and Multi-Agent Compete designs that improve over zero-shot baselines on the Seamless Interaction dataset.


<details>
  <summary>Details</summary>
Motivation: Inferring speaker relationships is important but underexplored; supervised modeling is costly and scaling is challenging. Existing inference-time LLM approaches lack structure for handling subtle, distributed, and multimodal relational cues.

Method: A training-free multi-agent reasoning framework with two complementary designs: (1) Multi-Role Multi-Agent Debate - assigning agents complementary roles or social-theory-grounded perspectives rather than a single viewpoint; (2) Multi-Agent Compete - a competition-based protocol using pairwise adjudication to compare and eliminate weaker candidates.

Result: Both methods improve over zero-shot and existing multi-agent baselines in most cases. Human evaluation shows the task is challenging even for people. LLMs outperform human annotators in text-included settings but perform less competitively in audio settings.

Conclusion: Relationship inference benefits from structured inference-time interaction among agents. Current models do not yet fully capture acoustic cues, indicating room for improvement in multimodal relationship understanding.
...
### [48] [Glyph: A Multi-Strategy Agentic System for Column Description and Sensitivity-Ontology Tagging of Enterprise Data Catalogs](https://arxiv.org/abs/2609.10430)
