...
TL;DR: This paper introduces a modular multi-level agent-based architecture for modeling democratic climate governance by integrating individual cognitive decision-making models (HUMAT/MOA) with strategic institutional behavior (NGOs, media, politicians) within a unified simulation framework.


<details>
  <summary>Details</summary>
Motivation: Climate governance involves complex interactions between heterogeneous actors (citizens, advocacy groups, media, politicians). Existing ABM approaches often focus on either institutional dynamics or individual behavioral mechanisms in isolation, creating a gap in understanding how these levels interact to produce political decisions.

Method: The architecture integrates three core components: (1) motive-based individual decision-making via HUMAT and MOA frameworks; (2) socially embedded influence processes through demographic homophily networks; (3) institutional strategy modules for NGOs, media agents, and politicians. Political decisions emerge from aggregating multiple signals (expert input, public mobilization, party alignment, media framing). The model is designed for empirical calibration using synthetic populations from survey data and Living Lab stakeholder engagement.

Result: The paper presents the architectural design principles, modular structure, and integration logic of the model rather than empirical results. It demonstrates how the multi-layered approach creates a framework where individual-level decision-making and institutional-level strategies interact to produce emergent political decisions.

Conclusion: The multi-layered architecture contributes to modeling democratic climate governance by bridging individual cognitive processes and strategic institutional behavior. Future work will focus on generalization, validation, and scenario-based exploration of climate-relevant land-use governance processes.
...
### [2] [M2Note: Continual Evolution of Vision Language Models via Mistake Notebook Learning](https://arxiv.org/abs/2607.00685)
...
### [3] [Calibrating the Instrument: Controllability of an LLM-Driven Synthetic Population](https://arxiv.org/abs/2607.00910)
*Mirko Degli Esposti*

Main category: cs.MA

TL;DR: SIVE tests the controllability of Generative Synthetic Populations by validating that synthetic personas respond to stimuli in an ordered, replicable, group-structured way across seven conditions spanning strongly positive to strongly negative institutional communications.


<details>
  <summary>Details</summary>
Motivation: Generative Synthetic Populations (GSP) are gaining interest for urban simulation and institutional communication research, but a fundamental question must be answered before real-world use: does the instrument respond to stimuli of known valence in an ordered, replicable, group-structured way (controllability)? This internal-validity question is logically prior to any claim about external validity.

Method: SIVE (Synthetic Instrument Validation Experiment) created a fictional municipality (Montelago) with 120 synthetic personas of known latent structure. The personas were exposed to seven conditions spanning strongly positive to strongly negative institutional communications about a water network. Seven pre-registered criteria (fidelity, stability, noise floor, specificity, sensitivity, and ordering) were evaluated across a temperature sweep.

Result: All seven criteria passed at every temperature. A calibration failure was turned into a diagnostic success: a message designed as "weakly positive" was identified as functionally negative due to unresolved problems, uncertainty, and institutional passivity in the text. A redesigned version restored the expected ordering. A noise sub-experiment showed the instrument's intrinsic noise is roughly half the cross-agent estimate and stable across temperatures.

Conclusion: The instrument demonstrates internal validity—the latent structure imposed on synthetic populations is recovered in their responses. Calibration failures can serve as diagnostic successes rather than mere errors. Individual trajectories reveal coherent micro-dynamics that summary statistics obscure, and full data are available via an interactive explorer.
...
### [4] [Decentralized Geometric Control for Cable-Suspended Payload Transport with Adaptive Mass Estimation](https://arxiv.org/abs/2607.00024)
*Hadi Hajieghrary,Benedikt Walter,Paul Schmitt,Miguel Hurtado*

Main category: cs.RO

TL;DR: GPAC is a decentralized four-layer hierarchical architecture for N quadrotors to cooperatively transport a cable-suspended payload without central coordination, achieving payload tracking RMSE of 33.8 cm in high-fidelity simulations with wind turbulence.


<details>
  <summary>Details</summary>
Motivation: Cooperative aerial transport requires controllers that respect nonlinear manifold geometry, operate without centralized coordination, and respect operational safety constraints. Existing approaches typically require centralized coordination or exchange of cable states and adaptive parameters.

Method: GPAC consists of four layers: 1) Implicit coordination via local cable measurements for load sharing estimation without knowledge of N or payload mass; 2) Local payload reconstruction from cable geometry; 3) Low-rate neighbor-position broadcast for collision avoidance; 4) Integrated control components including geometric position/attitude control, anti-swing regulation, extended-state observer for wind rejection, concurrent learning-based mass estimation, and priority-ordered CBF-inspired safety filter with input-to-state safety (ISSf) margins. The system operates directly on the full nonlinear configuration manifold.

Result: High-fidelity simulation with flexible cables, onboard sensor fusion, and wind turbulence achieved payload-tracking RMSE of 33.8 cm with 2.8% coefficient of variation over 13 seeds, demonstrating the method's robustness and reliability under realistic conditions.

Conclusion: GPAC enables fully decentralized cooperative aerial transport through implicit coordination and local information processing, achieving robust payload tracking performance while maintaining safety margins and operating at low per-agent computational cost.
...
### [5] [EmbodimentSemantic: A Spatial Scene-Graph Dataset and Benchmark for Vision-Language Models on Embodied Manipulation Trajectories](https://arxiv.org/abs/2607.00020)
*Hassan Jaber,Refinath S N,Luca Cagliero,Christopher E. Mower,Haitham Bou-Ammar*

Main category: cs.RO

TL;DR: The paper introduces EmbodimentSemantic, a spatial scene-graph dataset and benchmark for evaluating relational grounding in embodied manipulation, containing real-world observations from a low-cost robot arm and a simulator-grounded LIBERO benchmark with 60K+ frames and 120K+ scene graphs. Experiments show current VLMs predict plausible relations but struggle with exact depth-aware and viewpoint-dependent spatial structure.


<details>
  <summary>Details</summary>
Motivation: Spatial grounding is a key limitation of vision-language-action (VLA) systems for robotic manipulation. Current models can recognize objects and follow language instructions but lack explicit representations of spatial arrangements including support, containment, ordering, occlusion, and depth-sensitive relations.

Method: The authors introduce EmbodimentSemantic as a spatial scene-graph dataset and benchmark where scenes are represented as directed object-relation-object triplets with a fixed set of relations. The dataset includes real-world manipulation observations from the SO101 robot arm and a simulator-grounded LIBERO benchmark with 60K+ manipulation frames and 120K+ camera-specific scene graphs across paired third-person and wrist views. Ground-truth relations are derived automatically from MuJoCo geometry, world coordinates, camera projections, and visibility constraints.

Result: Experiments across open-source and commercial VLMs show that current models often predict plausible relations but struggle with exact depth-aware and viewpoint-dependent spatial structure. Scene graphs improve downstream control when injected into VLA policy prompts.

Conclusion: EmbodimentSemantic provides a unified framework for diagnosing spatial grounding in VLM perception and testing its utility for VLA manipulation. The benchmark reveals significant gaps in current models' ability to capture precise spatial relationships.
...
### [6] [Memory-Native Non-Terrestrial Networks for Embodied Intelligence](https://arxiv.org/abs/2607.00029)
*Chengyang Li,Yikun Wang,Jiahui He,Yujie Wan,Shuai Wang,Yuan Wu,Yik-Chung Wu,Chengzhong Xu,Huseyin Arslan*

Main category: cs.RO

TL;DR: MemNTN proposes a memory-native NTN paradigm with dual-memory architecture (physical world state + digital historical experience) and cross-layer mechanisms for memory-augmented decision-making, outperforming stateless protocols in satellite embodied question answering.


<details>
  <summary>Details</summary>
Motivation: NTN enables embodied intelligence connectivity but synergy is challenging due to dynamic, resource-constrained, topology-varying, and task-oriented environments. Existing memoryless protocols become inefficient as decisions are driven only by local channel conditions and instantaneous service demands.

Method: Proposes memory-native NTN (MemNTN) paradigm leveraging long-horizon contexts for memory-augmented optimization. Establishes dual-memory architecture: physical memory (world state) and digital memory (historical network experience). Develops memory acquisition, compression, valuation, update, and utilization mechanisms enabling cross-layer memory-native decision-making from physical/access to network/application layers.

Result: Experiments in satellite embodied question answering (SEQA) demonstrate MemNTN significantly outperforms conventional stateless NTN and terrestrial approaches.

Conclusion: The proposed MemNTN paradigm successfully shifts from memoryless to memory-native NTN by establishing cross-layer memory mechanisms that enable effective long-horizon context utilization for robust embodied intelligence connectivity in challenging environments.
...
### [7] [When to Personalize Household Object Search: A Rigidity-Gated Hybrid Policy](https://arxiv.org/abs/2607.00022)
...
### [8] [HydraCollab: Adaptive Collaborative-Perception for Distributed Autonomous Systems](https://arxiv.org/abs/2607.00191)
*Luke Chen,Cheng-Ju Wu,David R. Martin,Qilin Ye,Pramod Khargonekar,Mohammad Abdullah Al Faruque*

Main category: cs.RO

TL;DR: HydraCollab is an adaptive collaborative-perception framework that selectively transmits informative sensor features and dynamically employs collaboration strategies (intermediate or late) based on spatial confidence maps, achieving superior accuracy-communication cost trade-off compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing collaborative-perception systems face an inherent trade-off: more information exchange improves accuracy but increases communication overhead. Real-world networks impose bandwidth constraints that require minimizing communication without sacrificing perception performance.

Method: HydraCollab proposes two key innovations: (1) selectively transmitting the most informative sensor features, and (2) dynamically employing collaboration strategies (intermediate or late fusion) based on spatial confidence maps.

Result: Evaluated on V2X-R, V2X-Radar, and UAV3D-mini datasets, HydraCollab achieves the best accuracy-communication trade-off. Compared to SOTA Where2comm, it uses only 41% bandwidth on V2X-R (improving performance by 0.78%) and 26% bandwidth on V2X-Radar (improving performance by 0.75%).

Conclusion: HydraCollab effectively addresses the bandwidth-accuracy trade-off challenge in collaborative perception, significantly reducing communication overhead while maintaining or improving perception performance across multiple benchmark datasets.
...
### [10] [FLYNN: Robust Neural Network for Robot Navigation using Fly Brain Topology](https://arxiv.org/abs/2607.00025)
*Benquan Wang,Jingdao Chen*

Main category: cs.RO

TL;DR: The paper develops FLYNN, a recurrent neural network architecture directly derived from the fruit fly's synaptic-resolution connectome, demonstrating that biologically-inspired topologies can achieve vision-based navigation performance comparable to modern hand-crafted networks while exhibiting superior robustness to out-of-distribution data and sensory loss without further training.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are brittle when faced with new environments or sensory deprivation, whereas biological systems demonstrate remarkable tolerance to such challenges. The authors seek to address this vulnerability by leveraging the biological brain's architecture to design more resilient artificial agents.

Method: The researchers develop FLYNN (Fly Connectome Neural Network), a recurrent neural network whose architecture is directly derived from the synaptic-resolution brain connectome of Drosophila melanogaster (fruit fly). The network is trained to perform vision-based navigation tasks in MuJoCo simulations.

Result: FLYNN achieves performance comparable to modern hand-crafted networks of similar parameter counts in vision-based navigation. Crucially, it exhibits superior resistance to out-of-distribution (OOD) data and tolerance to sensory loss without further training—remaining functional even under total vision loss while hand-crafted networks largely failed. Principal Component Analysis reveals that FLYNN exhibits a high degree of representational modularity, which may explain its robustness.

Conclusion: The work provides a new direction for designing resilient artificial agents by following the topology of biological brains, suggesting that biologically-inspired architectures may offer inherent robustness advantages over conventional neural network designs.
...
### [11] [From Real-Time Planning to Reliable Execution:Scalable Coordination for Heterogeneous Multi-Robot Fleets in Industrial Environments](https://arxiv.org/abs/2607.00591)
*Bo Cao,Zhe Liu,Hesheng Wang*

Main category: cs.RO

TL;DR: SCALE is a reactive online coordination framework for heterogeneous robot fleets that addresses real-time path planning challenges with motion-induced conflict reduction and adaptive precedence relations via a generalized Conjugate Action-Precedence Hypergraph (CAPH).


<details>
  <summary>Details</summary>
Motivation: Efficient coordination of heterogeneous robot fleets in industrial environments requires real-time path planning that handles high robot densities, diverse motion capabilities, communication delays, and execution uncertainties, which can cause path deviations leading to excessive waiting and congestion.

Method: SCALE introduces a motion-induced conflict reduction mechanism for online feasible path generation and a generalized Conjugate Action-Precedence Hypergraph (CAPH) that adaptively adjusts precedence relations among robots to mitigate disturbances.

Result: Extensive validation experiments and a three-day warehouse deployment demonstrate the framework's effectiveness.

Conclusion: SCALE enables robust real-time planning and execution for heterogeneous robot fleets in dynamic industrial environments.
...
### [12] [Invariant Stochastic Filtering on SE(3) for Inertial-Encoder State Estimation of Serial Rigid Manipulators](https://arxiv.org/abs/2607.00026)
*S. Yaqubi,J. Mattila*
...
### [13] [Urban Deceleration Behavior Modes Under Scene Context: An Early-Kinematic Classifier from Argoverse 2 Multi-Agent Trajectories](https://arxiv.org/abs/2607.00027)
...
### [14] [Trajectory Learning with Graph Representations for Social Robot Navigation](https://arxiv.org/abs/2607.00028)
...
### [15] [A Unified Benchmark for RCM-Constrained Visual Servoing: Modeling-Controller Interaction and Robustness Analysis in Laparoscopic Robots](https://arxiv.org/abs/2607.00030)
...
### [16] [Joint Discovery of Object and Action Symbols through Effect Prediction for Robotic Manipulation Planning](https://arxiv.org/abs/2607.00031)
*Burcu Kilic,Berke Kartal,Fatih Dogangun,Erhan Oztop,Emre Ugur*

Main category: cs.RO

TL;DR: The paper proposes a binary bottleneck model that jointly discovers high-level manipulation primitives and object categories from random interaction data, enabling effect-driven planning for precise manipulation across both seen and novel objects.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots need to abstract continuous sensorimotor interactions into discrete representations. Previous approaches either used visual appearance (failing to distinguish similar-looking objects with different behaviors) or effects under predefined actions (limited scope), requiring a method that can discover both object categories and manipulation primitives jointly.

Method: The researchers propose a model with a binary bottleneck layer trained to predict multi-modal outcomes (object motion, contact, force feedback) from random interaction data. Building on discovered binary representations, they leverage a discrete planning method using intermediate steps in predicted effect trajectories for partial action executions. They evaluate generalization on novel objects by comparing interaction effects with learned object symbols.

Result: Experiments on tabletop repositioning and stacking tasks show that the effect-driven planning approach outperforms both a state-of-the-art method and a visual-based alternative in planning precision across seen and novel objects.

Conclusion: The approach enables few-shot generalization based on behavioral similarity rather than visual appearance, demonstrating that effect-driven planning provides superior precision in complex manipulation tasks.
...
### [17] [Learning Dexterous Manipulation Using Contact Wrench Guidance From Human Demonstration](https://arxiv.org/abs/2607.00033)
...
### [18] [Optimal any-angle path planning in static and dynamic environments](https://arxiv.org/abs/2607.00065)
...
### [19] [Learning Expert Strategy for Autonomous Robotic Endovascular Intervention via Decoupled Procedural Execution](https://arxiv.org/abs/2607.00066)
...
### [20] [AD-MPCC: Adaptive Differentiable Model Predictive Contouring Control for Autonomous Racing](https://arxiv.org/abs/2607.00141)
...
### [21] [Stop Pretending Social Robots Are Inevitable](https://arxiv.org/abs/2607.00142)
*Serge Thill*

Main category: cs.RO

TL;DR: This paper critiques the inevitable human-robot society narrative in RO-MAN and HRI conferences, arguing that this framing distorts research priorities by deprioritizing technical development and ecological relevance in favor of scenario design and techno-solutionism.


<details>
  <summary>Details</summary>
Motivation: The authors take issue with recent conference themes portraying a future human-robot society as inevitable, recognizing that such framing shapes research directions, promotes techno-solutionism, and deprioritizes actual societal needs and technical development.

Method: The authors analyze how the "inevitable human-robot society" frame influences research practices, particularly in user studies and technical development, by examining the consequences of treating this future as a fait accompli.

Result: The authors demonstrate that assuming an inevitable robot future leads to research that emphasizes scenario design over actual robot abilities, deprioritizes societal needs and technical development, and promotes techno-solutionism. This approach is self-defeating as it risks stifling the very research needed to achieve the assumed future.

Conclusion: The paper recommends rejecting attempts to frame and promote the field in terms of inevitable social robots, instead focusing on a field that facilitates advances regardless of the future, with a suggested starting point being renewed focus on cognitive mechanisms necessary for the "I" in HRI.
...
### [22] [Iterated Invariant EKF for 3D Landmark-Aided Inertial Navigation](https://arxiv.org/abs/2607.00145)
*Hilton Marques Souza Santana,João Carlos Virgolino Soares,Marco Antonio Meggiolaro*

Main category: cs.RO

TL;DR: The paper applies the Iterated Invariant EKF (IterIEKF) to landmark-based inertial 3D localization for the first time, demonstrating superior estimation accuracy and consistency compared to classical SO(3)-EKF, Iterated SO(3)-EKF, and IEKF through numerical simulations.


<details>
  <summary>Details</summary>
Motivation: Classical SO(3)-based EKF suffers from the false observability problem, where the filter becomes overconfident in unobservable directions. While IEKF addresses some limitations by reformulating system dynamics as a group-affine system on a Lie group, it does not fully satisfy certain state compatibility properties. The IterIEKF was proposed to improve upon IEKF by ensuring the estimated state remains on the observed state manifold while uncertainty is confined to the tangent space.

Method: The paper formulates and applies the Iterated Invariant EKF (IterIEKF) to landmark-based inertial 3D localization. The IterIEKF reformulates the system dynamics as a group-affine system on a Lie group and ensures that in the low-noise regime, the estimated state remains on the observed state manifold while the uncertainty is confined to its tangent space.

Result: Numerical simulations demonstrate that the proposed IterIEKF approach outperforms the classical SO(3)-EKF, Iterated SO(3)-EKF, and IEKF in terms of both estimation accuracy and consistency.

Conclusion: The IterIEKF formulation for landmark-based inertial 3D localization provides improved performance, establishing it as a valuable approach for robotic perception and state estimation problems involving landmark measurements and inertial navigation.
...
### [23] [3D Point World Models: Point Completion Enables More Accurate Dynamics Learning](https://arxiv.org/abs/2607.00148)
...
### [24] [Dual-Informed Vertical Expansion for Multi-Objective Node Selection in Anytime Conflict-Based Search](https://arxiv.org/abs/2607.00156)
*Willem van Osselaer,Jiarui Li,Meshal Alharbi,Gioele Zardini*

Main category: cs.RO

TL;DR: This paper introduces Dual-Informed Vertical Expansion (DIVE), a node selection policy for Conflict-Based Search (CBS) that balances depth-oriented and best-first approaches, improving dive efficiency while maintaining node efficiency and providing early incumbents.


<details>
  <summary>Details</summary>
Motivation: Standard CBS uses fixed best-first node selection which maintains large frontiers, interrupts parent-child expansion sequences, and provides no feasible incumbent until termination. The paper treats node selection as a first-class design choice to address these limitations.

Method: The paper formalizes CBS node selection through a branch-and-bound view and introduces DIVE, which starts each dive from the current best-bound frontier, follows promising children to exploit parent-child locality, and uses incumbent pruning to limit unproductive excursions. DIVE is best-bound between dives and depth-oriented within a dive.

Result: The analysis identifies three complementary extremes: best-first search (node efficient), iterative deepening (memory efficient), and DIVE (dive efficient). Experiments show DIVE reduces dive breaks, provides early incumbents with certified gaps, uses substantially less queue memory, and benefits from warm starts and responsive variants.

Conclusion: DIVE provides an effective trade-off map between expanded nodes, dive breaks, queue size, and primal-dual bound progress, being particularly beneficial in dense or memory-limited regimes.
...
### [25] [Distributed Multi Robot Lunar Cargo Transportation via Phase Decomposed Reinforcement Learning](https://arxiv.org/abs/2607.00160)
...
### [26] [ELMP: Efficient Learning for Motion Planning via Analytical Policy Gradients](https://arxiv.org/abs/2607.00215)
*Yixiao Li,Tifanny Portela,Jordis Herrmann,René Zurbrügg,Marco Hutter*

Main category: cs.RO

TL;DR: ELMP is a self-supervised fine-tuning framework that enables data-efficient adaptation of Neural Motion Planners by optimizing policies through differentiable kinematic layers with dense objectives, achieving 84.8% success rate and eliminating the need for large expert datasets.


<details>
  <summary>Details</summary>
Motivation: Neural Motion Planners (NMPs) enable fast reactive motion generation, but adapting them to new environments requires collecting large expert datasets, which is computationally prohibitive and creates a bottleneck for practical deployment.

Method: ELMP replaces expensive expert data generation with rapid problem sampling via a differentiable kinematic layer that optimizes policies using dense collision, target-reaching, and smoothness objectives. It also introduces a mechanism to encode tool geometry via point clouds for robust generalization across different kinematic chains.

Result: ELMP achieves 84.8% average success rate with orders-of-magnitude lower cold-start latency than classical methods. In unseen environments, self-supervised fine-tuning improves success rate from 57.3% (zero-shot) to 89.8%, while maintaining millisecond-level inference latency. Validated on a physical Franka Emika Panda robot.

Conclusion: ELMP effectively removes the data collection bottleneck by enabling data-efficient adaptation through self-supervised fine-tuning, achieving strong generalization across environments and kinematic chains without sacrificing inference latency.
...
TL;DR: The paper introduces a Planning KL-divergence (PKL)-guided framework that identifies and ranks occluded agents based on their impact on the ego vehicle's trajectory, then uses expert VLMs to generate annotations for training VLMs to handle planning-critical occlusions in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Current approaches either treat all occlusions uniformly with conservatism (yielding defensive driving) or infer hidden spaces without estimating the impact on the planner. There's a critical gap between perception and planning that needs to be bridged for autonomous vehicles to safely navigate complex environments with hidden agents.

Method: The framework introduces Planning KL-divergence (PKL), an information-theoretic metric to systematically identify and rank occluded agents based on their impact on the ego vehicle's plan. An expert VLM (GPT-5) generates rich, structured annotations capturing visual evidence and reasoning. The approach is applied to the nuScenes dataset to create a new benchmark focused on high-impact scenarios.

Result: Fine-tuning on PKL-guided data yields dramatic performance improvements across all models. Smaller, fine-tuned models significantly outperform larger zero-shot counterparts. The PKL-guided data selection strategy improves performance by approximately 30% over random sampling.

Conclusion: This work presents the first systematic approach for training VLMs to focus on planning-critical occlusions, enabling more semantically grounded and efficient risk assessment in autonomous driving.
...
TL;DR: ACT-VLA is an offline data augmentation framework that leverages latent task representations to synthesize novel demonstrations from existing tasks, enabling VLA models to compose sub-skills and generalize better without additional manual data collection.


<details>
  <summary>Details</summary>
Motivation: Standard VLA models severely overfit to specific behavioral patterns and cannot generalize to out-of-distribution scenarios requiring novel combinations of sub-skills. Expanding datasets can mitigate overfitting, but acquiring high-quality robot data is labor-intensive and cost-prohibitive.

Method: ACT-VLA uses a VLA model's latent task representations to synthesize novel, physically valid demonstrations directly from existing tasks, allowing the model to learn to compose known sub-skills into new executable behaviors through offline data augmentation.

Result: On challenging manipulation tasks in simulation, policies trained with ACT-VLA's synthesized data achieve substantially higher success rates compared to baseline VLA models that overfit to the original distribution, validating the method's effectiveness.

Conclusion: Leveraging existing tasks for automated demonstration synthesis provides an effective, scalable, and data-efficient route to broadening VLA generalization without requiring expensive human teleoperation or additional manual data collection.
...
### [30] [Robust Operational Space Control with Conformal Disturbance Bounds for Safe Redundant Manipulation](https://arxiv.org/abs/2607.00424)
...
### [31] [Learning Gait-Aware Quadruped Locomotion with Temporal Logic Specifications](https://arxiv.org/abs/2607.00442)
...
### [33] [VLM-AR3L: Vision-Language Models for Absolute and Relative Rewards in Reinforcement Learning](https://arxiv.org/abs/2607.00483)
...
### [34] [From Technical Metrics to User Perception: A User Study of a Multimodal Human-Robot Interaction System for Object Detection and Grasping](https://arxiv.org/abs/2607.00530)
...
### [35] [Learning from Demonstration via Spatiotemporal Tubes for Unknown Euler-Lagrange Systems](https://arxiv.org/abs/2607.00534)
...
### [36] [[Preprint] Dynamic Modeling, Gait Synthesis, and Control of a Novel Subsurface Bore Propagator](https://arxiv.org/abs/2607.00569)
...
### [37] [Enhancing Robustness in Robot-Environment Interactions through Passive Compliant Degrees of Freedom: A Hybrid Position-Force Control Approach with Feedback Linearization](https://arxiv.org/abs/2607.00571)
...
### [38] [Domain Arithmetic: One-Shot VLA Adaptation under Environmental Shifts](https://arxiv.org/abs/2607.00666)
...
### [39] [Path Planning in Physically Viable World Models](https://arxiv.org/abs/2607.00673)
...
### [40] [From Prediction Uncertainty to Conformalized Distance Fields for Safe Motion Planning](https://arxiv.org/abs/2607.00776)
...
### [41] [From World Models to World Action Models: A Concise Tutorial for Robotics](https://arxiv.org/abs/2607.00836)
...
### [44] [DART-VLN: Test-Time Memory Decay and Anti-Loop Regularization for Discrete Vision-Language Navigation](https://arxiv.org/abs/2607.01043)
...
### [45] [Robots Ask the Way: Communication-Enabled Social Navigation](https://arxiv.org/abs/2607.01044)
...
### [46] [AutoSpeed: Annotation-Free Stage-Adaptive Motion Speed Learning for Robot Manipulation](https://arxiv.org/abs/2607.01051)
*Qingda Hu,Ziheng Qiu,Jieru Zhao,Zhongxue Gan,Wenchao Ding*

Main category: cs.RO

TL;DR: AutoSpeed introduces a model-agnostic learning framework that enables visuomotor policies to adapt motion speeds to task difficulty stages without annotations, using a composite cost function and DCT-based frequency domain speed modulation.


<details>
  <summary>Details</summary>
Motivation: Different manipulation task stages require varying difficulty levels, motion speeds, and temporal prediction horizons. Existing IL-based visuomotor policies imitate fixed demonstration speeds and temporal horizons, limiting flexibility and task throughput.

Method: AutoSpeed treats trajectories at different speeds as optimization targets, evaluating each candidate via a composite cost balancing prediction error and horizon. The framework uses DCT in the frequency domain for smooth speed scaling, adjusting effective temporal prediction horizons through speed modulation: simple stages execute faster with longer horizons, complex stages execute slower with shorter horizons.

Result: AutoSpeed substantially reduces task execution time while improving success rates. Inferred motion speeds strongly correlate with task stages, demonstrating effective stage-adaptive behavior.

Conclusion: AutoSpeed provides a simple, annotation-free method to enhance visuomotor policy adaptability, enabling flexible stage-dependent speed control that improves both efficiency and task success.
...
### [47] [RoboWorld: Fast and Reliable Neural Simulators for Generalist Robot Policy Evaluation](https://arxiv.org/abs/2607.01060)
*Byeongguk Jeon,Seonghyeon Ye,JaeHyeok Doo,Sungdong Kim,Minjoon Seo,Hyungmok Son,Kimin Lee*

Main category: cs.RO

TL;DR: RoboWorld is an automated evaluation pipeline for robot policies using a fast autoregressive video world model paired with task-progress-aware vision-language model scoring. The key innovation is Step Forcing, which combines anchored and one-step self-forwarded contexts to reduce train-test mismatch while preserving action-observation dynamics. The system achieves strong correlation (Pearson's r = 0.989, Spearman's rho = 0.970) with real-world robot evaluation across multiple tasks and environments.


<details>
  <summary>Details</summary>
Motivation: Video world models offer a scalable alternative for evaluating generalist robot policies by bypassing physical constraints and engineering burdens of real-world deployment. However, existing approaches face two critical challenges: (1) world-model errors that make generated rollouts unreliable, and (2) slow inference speeds that limit large-scale throughput. These limitations prevent reliable evaluation of long-horizon robot policies.

Method: RoboWorld introduces an automated evaluation pipeline consisting of two main components: (1) a fast autoregressive video world model paired with a task-progress-aware vision-language model scoring system, and (2) Step Forcing, a novel technique that combines anchored and one-step self-forwarded contexts. Step Forcing reduces train-test mismatch while preserving action-observation dynamics, enabling reliable long-horizon autoregressive world-model rollouts.

Result: RoboWorld achieves strong alignment with real-world robot evaluation across tasks and environments, demonstrating Pearson correlation coefficient r = 0.989 and Spearman's rho = 0.970. This indicates near-perfect correlation between the model's evaluation scores and actual real-world robot performance, validating the effectiveness of the proposed approach.

Conclusion: The research demonstrates that automated evaluation pipelines combining autoregressive video world models with task-aware scoring can reliably evaluate robot policies at scale. The Step Forcing technique successfully addresses the train-test mismatch problem while maintaining action-observation dynamics, making it a practical solution for evaluating generalist robot policies without requiring physical robot deployment. This enables researchers to efficiently assess robot performance across diverse tasks and environments.
...
### [48] [Human-Centric Transferable Tactile Pre-Training for Dexterous Robotic Manipulation](https://arxiv.org/abs/2607.01067)
*Chi Zhang,Penglin Cai,Ziheng Xi,Haoqi Yuan,Hao Luo,Wanpeng Zhang,Sipeng Zheng,Chaoyi Xu,Zongqing Lu*

Main category: cs.RO

TL;DR: H-Tac presents a large-scale tactile-action dataset (160h, 300 tasks, 135k episodes) and Transferable Tactile Pre-Training (TTP) framework that enables scalable human-to-robot tactile pre-training with unified tactile and action spaces, achieving superior fine-grained manipulation performance in both simulation and real robots.


<details>
  <summary>Details</summary>
Motivation: Existing tactile datasets are limited in scale and contact coverage due to hardware/data collection constraints. Vision-Language-Action models with tactile are constrained by dynamics-agnostic post-training, which limits their performance ceiling on downstream tasks.

Method: H-Tac dataset: 160-hour egocentric human videos containing 300+ tasks and 135k episodes. TTP framework: tactile-based pre-training on human data using unified tactile and action spaces for human-to-robot transfer. Leverages a tactile expert for future tactile prediction to explicitly model contact dynamics and precise physical interactions.

Result: Extensive experiments in simulation and on real robots demonstrate superior performance with robust generalization and fine-grained manipulation capabilities.

Conclusion: TTP enables scalable tactile pre-training via human-to-robot transfer, bridging the gap between human and robot tactile capabilities for dexterous contact-rich tasks.
...
### [49] [Where Am I? Semantic Map Grounding via Vision-Language Models for Multi-Modal Localization](https://arxiv.org/abs/2607.01079)
...
### [50] [ROSA: A Robotics Foundation Model Serving System for Robot Factories](https://arxiv.org/abs/2607.01088)
...
TL;DR: ROSA is a Robotics Foundation Models serving system for factories that uses shared GPU-pool serving, robotics-aware programming abstractions, and factory-objective-driven scheduling, achieving up to 12.06x improvement in factory productivity over conventional single-robot serving systems.


<details>
  <summary>Details</summary>
Motivation: Existing RFM serving systems are designed for single-robot, single-model scenarios, focusing on minimizing latency of individual actions. This approach fails to scale for factory deployments with multiple robots, limiting GPU utilization, battery duration, and overall factory productivity.

Method: ROSA implements three key principles: (1) shared GPU-pool serving for fleet-wide GPU access via network, (2) robotics-aware programming abstraction supporting multi-model pipelines, per-task performance requirements, and failure handling, and (3) factory-objective-driven scheduling to maximize SLO-qualified factory productivity rather than minimizing individual request latency. Implementation uses Ray Serve for orchestration with vLLM, PyTorch, and JAX backends.

Result: ROSA improves factory productivity by up to 12.06x over conventional dedicated serving systems on both real robots and synthetic large-scale workloads.

Conclusion: A factory-oriented serving architecture that prioritizes aggregate factory productivity over individual request latency can dramatically outperform traditional single-robot serving approaches, making general-purpose robots more practical for factory deployments.
...
### [51] [Technical Report: Asynchronous Distributed Trajectory Estimation of Multi-Robot Systems](https://arxiv.org/abs/2607.01106)
...
### [52] [FAR: Failure-Aware Retry for Test-Time Recovery and Continual Policy Improvement](https://arxiv.org/abs/2607.01111)
*Haoran Hao,Shahram Najam Syed,Jeffrey Ichnowski,Jeff Schneider*

Main category: cs.RO

TL;DR: FAR is a test-time learning framework that enables robots to autonomously recover from failures by combining failure-based preference learning with exploration, achieving 11.7-17.6% success rate improvements over baseline diffusion policies.


<details>
  <summary>Details</summary>
Motivation: Robot policies inevitably encounter failures when deployed in real environments. Naive retries often repeat the same mistakes, while many existing recovery methods rely on human intervention, limiting autonomous operation.

Method: FAR combines two key components: (1) Failure-Contrastive Preference Adaptation, which constructs preference learning data from failures to steer policies away from unsuccessful behaviors, and (2) lightweight action perturbations during retries to encourage local exploration. Successful recovery trajectories are incorporated into a training loop for continual policy improvement.

Result: FAR substantially improves success rates and robustness: 17.6% over standard diffusion policy in simulation and 11.7% in the real world. It also significantly enhances data efficiency under both reset and timestep budgets during continual policy improvement.

Conclusion: By exploiting informative failure cases and enabling autonomous recovery without human intervention, FAR achieves substantial performance gains while improving data efficiency, demonstrating a practical approach to enhancing robot robustness in real-world deployment scenarios.
...
### [53] [Structured 4D Latent Predictive Model for Robot Planning](https://arxiv.org/abs/2607.01166)
*Zhiyi Li,Peilin Wu,Xiaoshen Han,Ruojin Cai,Yilun Du*

Main category: cs.RO

TL;DR: A Structured 4D Latent Predictive Model that predicts 3D scene evolution in a structured latent space, enabling precise spatial reasoning and effective robotic planning for complex manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing video predictive models operate on 2D video sequences, lacking the 3D geometric understanding necessary for precise spatial reasoning and physical consistency in robotics applications.

Method: The paper introduces a Structured 4D Latent Predictive Model that predicts scene 3D structure evolution in a structured latent space conditioned on observations and textual instructions. The model encodes scenes holistically and decodes into diverse 3D formats. It serves as a planner generating future scenes, which are translated into executable actions by a goal-conditioned inverse dynamics module.
...
Conclusion: This structured 4D latent predictive model offers a powerful paradigm for robotics, enabling precise spatial reasoning, physical consistency, and effective planning for complex manipulation tasks through its holistic scene encoding and 3D consistent representation.
...
### [54] [FastBridge: Closing the Model-Based Realization Gap in Safety Filters on 3D Gaussian Splatting for Fast Quadrotor Flight](https://arxiv.org/abs/2607.01200)
...
### [55] [Sensorless Four-Channel Control Architecture Using Inverse Dynamics Modeling for Human-Scale Bilateral Teleoperation](https://arxiv.org/abs/2607.01201)
*Amir Noohian,Dylan Miller,Justin Valentine,Alan Lynch,Martin Jagersand*

Main category: cs.RO

TL;DR: A sensorless four-channel teleoperation architecture based on inverse dynamics modeling that eliminates the need for force/torque sensors while improving position/force tracking and operator effort compared to conventional schemes.


<details>
  <summary>Details</summary>
Motivation: Four-channel teleoperation is limited in human-scale applications by high inertia, modeling challenges, and reliance on noisy, costly force/torque sensors.

Method: Introduces a sensorless four-channel architecture based on inverse dynamics modeling, implemented and validated on a customized WAM bilateral teleoperation setup.

Result: Experiments show the approach outperforms conventional two- and four-channel schemes and transparency-enhancement methods, improving position/force tracking, reducing operator effort, and increasing maximum transmittable impedance without external sensors. A door-opening case study demonstrates effectiveness in sustained whole-body contact tasks.

Conclusion: The proposed sensorless inverse dynamics-based architecture effectively enhances teleoperation performance without external sensors, making it suitable for realistic human-scale manipulation tasks.
...
### [56] [FurnitureVLA: Learning Long-Horizon Bimanual Furniture Assembly with Vision-Language-Action Model](https://arxiv.org/abs/2607.01212)
...
### [57] [Geometric Shape Optimization for Limbless Locomotion](https://arxiv.org/abs/2607.00524)
*Utpal Khanal,Avirup Mandal*

Main category: cs.GR

TL;DR: This paper presents a differential-geometric framework for simulating locomotion in slender soft bodies like snakes by representing the body as a Fourier-Chebyshev polynomial parametric curve, solving an optimization problem to determine environmental interaction, incorporating bending and torsional energy terms for physically plausible behavior, and using interpolation to drive surface visualization.


<details>
  <summary>Details</summary>
Motivation: The simulation of locomotion in limbless, deformable organisms remains challenging across computer graphics, soft robotics, and computational modeling, particularly for slender soft bodies like snakes.

Method: The body is represented as a three-dimensional parametric curve using a Fourier-Chebyshev polynomial basis. Motion is computed by solving an optimization problem that determines the curve-environment interaction by estimating polynomial coefficients. Bending and torsional energy terms are incorporated to ensure physically plausible and non-self-intersecting behavior. The resulting curve drives a surface representation via interpolation.

Result: The approach is evaluated across complex scenarios and parameter settings, demonstrating robustness and versatility. Comparative analysis with state-of-the-art methods shows improved simulation quality and more physically realistic motion generation.

Conclusion: The proposed differential-geometric framework achieves superior simulation quality and generates more physically realistic motion compared to state-of-the-art methods.
