...
TL;DR: Real-Time AttentionBender is a plugin tool that enables live, interactive manipulation of video diffusion transformer (DiT) internals, exposing self-attention, cross-attention, and feed-forward networks at granular levels for "material intimacy" with generative video models.


<details>
  <summary>Details</summary>
Motivation: Generative video models achieve remarkable visual fidelity but offer limited creative agency and obscure their material process from artists, requiring a tool that bridges the gap between black-box models and creative control.

Method: A plugin built within the DayDream Scope ecosystem that wraps open-source real-time Wan pipelines, exposing self-attention, cross-attention, and feed-forward networks as independently manipulable surfaces with targeting precision down to individual diffusion steps, DiT layers, prompt tokens, and hidden neurons.

Result: Provides "material intimacy" with the model—a responsive, near-mechanistic feel for how specific layers and neurons shape generated video through live manipulation.

Conclusion: The tool functions simultaneously as an XAIxArts probe into transformer internals and an expressive instrument for discovering aesthetics outside the model's default representational space.
...
TL;DR: A dual-conditioned generative framework using Multi-Modal Diffusion Transformer with structural and semantic alignment to create visually appealing yet statistically accurate pictorial charts.


<details>
  <summary>Details</summary>
Motivation: Traditional statistical graphics are precise but lack visual appeal, memorability, and engagement compared to pictorial charts. The authors aim to bridge the gap between semantic expression and structural faithfulness in automated chart generation.

Method: A dual-conditioned generation task using Multi-Modal Diffusion Transformer guided by text prompts (semantic context) and context images (global structure). Two feature-level mechanisms: structural alignment for spatial layouts and semantic alignment for expressive textures.

Result: The method generalizes across visual channels (length, area, angle, position) producing artistically compelling and structurally consistent pictorial charts. Outperforms traditional controllable generation and image editing baselines in quantitative evaluations and perceptual user studies.

Conclusion: The framework provides a foundation for high-fidelity, data-driven generative modeling in expressive visual storytelling, successfully merging artistic appeal with statistical accuracy.
...
TL;DR: This paper presents an implicit modeling method for lattice structures that uses cubic Hermite curve-controlled convolution surfaces over skeletal graphs, enabling efficient and robust modeling of complex strut shapes while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Lattice structures are crucial in additive manufacturing for achieving lightweight designs with high surface-to-volume ratios and superior mechanical properties. However, conventional lattice modeling using simple primitives (cylinders/cones) limits geometric diversity, and recent efforts to increase strut complexity often sacrifice computational efficiency and modeling robustness. There is a need for a method that balances rich geometric expressiveness with efficient computation.

Method: The proposed method represents each strut as a convolution surface over a skeletal graph, with profile shape controlled by a cubic Hermite curve. By exploiting the polynomial structure of both the convolution kernel and the cubic Hermite curve-controlled profile, the authors derive analytical expressions for efficient field evaluation, avoiding costly numerical computations.

Result: Four case studies validated the method in terms of profile shape diversity, graded lattice modeling, slicing robustness, and efficiency. The method successfully expands the design and optimization space of lattice structures while preserving the robustness and efficiency of implicit representations.

Conclusion: The proposed implicit modeling method achieves an optimal balance between geometric expressiveness and computational efficiency for lattice structures, enabling more complex and optimized designs in additive manufacturing applications.
...
### [4] [Compatibility and Accuracy Verification of CADmesh-Based Complex Geometry Modeling in Geant4](https://arxiv.org/abs/2606.06508)
...
TL;DR: This paper evaluates STL/OBJ format compatibility in Geant4 Monte Carlo simulation, finding 100% import success rate and volume errors ≤0.288%, while designing an interface that reduces parsing code by 70% and cuts simulation time from 15194.3s to 77.28s.


<details>
  <summary>Details</summary>
Motivation: CSG method in Geant4 Monte Carlo simulation has low efficiency and high application threshold, making triangular facet formats (STL/OBJ) via CADmesh a promising alternative that requires systematic evaluation of format compatibility, geometric accuracy, and physical simulation deviations.

Method: Built an open-source experimental environment based on Geant4 11.0, CADmesh 1.3.0, and FreeCAD 1.0; designed high and low precision gradient test cases using simple geometric bodies and complex engineering models; systematically evaluated import success rate, facet loss rate, volume error, and particle transport dose deviation for STL and OBJ formats; designed a general adaptive interface.

Result: 100% import success rate for both STL and OBJ formats; volume error rate ≤0.018% for high-precision models and ≤0.288% for low-precision models; both formats share the same vertex facet data structure; interface reduces parsing code lines by ~70% while maintaining geometric accuracy; tetrahedral mesh loading takes 3.1 times longer than tessellated solids, but simulation time is reduced from 15194.3s to 77.28s.

Conclusion: STL and OBJ formats are reliable alternatives to CSG in Geant4 Monte Carlo simulation, offering high geometric accuracy and significant performance improvements through the developed adaptive interface, though tetrahedral mesh loading has a time overhead.
...
### [5] [Differentiable 3D Triangle-Triangle Intersection Energy](https://arxiv.org/abs/2606.06511)
*Tianyu Wang*

Main category: cs.GR

TL;DR: A GPU-based inexact Newton optimization method using a second-order differentiable energy from 3D triangle-triangle intersection testing enables intersection-freeness in non-oriented deformation primitives without requiring initialization, user interaction, or history information.


<details>
  <summary>Details</summary>
Motivation: Intersection-freeness and global injectivity are crucial in computer graphics, but challenging for non-oriented deformation primitives. Most existing methods rely on intersection-free initialization and continuous trajectory tracking, making them unusable for tasks lacking such initialization.

Method: The authors introduce a novel second-order differentiable energy defined from 3D triangle-triangle intersection testing, combined with a GPU-based inexact Newton optimization route to resolve intersections.

Result: Intersection can be efficiently resolved integrated with this method, requiring no user interaction, history information, or valid initialization.

Conclusion: The proposed approach enables solving intersection-freeness problems for non-oriented deformation primitives in 3D space without the limitations of traditional methods that require initialization or continuous trajectory tracking.
...
### [6] [Agentic Large Language Models for Automated Structural Analysis of 3D Frame Systems](https://arxiv.org/abs/2606.06525)
...
### [7] [AI Level of Detail: Distance-Aware ML Model Precision Selection for Real-Time Human Motion Prediction in Games](https://arxiv.org/abs/2606.06565)
*Mathew Varghese*
...
### [8] [Skeletal-Anchored Dual Harmonics for Structured 3D Modeling](https://arxiv.org/abs/2606.07337)
...
### [9] [MADRAG: Multi-Agent Debate with Retrieval-Augmented Generation for Training-Free Analytic Essay Scoring](https://arxiv.org/abs/2606.06754)
*Ali Keramati,Shiyuan Zhou,Sharad Mehrotra,Mark Warschauer*

Main category: cs.MA

TL;DR: MADRAG is a training-free framework for analytic essay scoring that uses multi-agent reasoning (Advocate, Skeptic, Judge) combined with retrieval-augmented grounding to achieve performance approaching supervised systems without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Standard LLM-as-judge approaches suffer from bias and unstable scoring; the authors propose a more reliable evaluation framework that avoids task-specific training.

Method: MADRAG decomposes evaluation into an interactive process: an Advocate identifies strengths, a Skeptic critiques weaknesses, and a Judge aggregates arguments into a final score. The Judge is augmented with rubric-aligned exemplar retrieval for calibration through comparison with scored examples.

Result: MADRAG significantly outperforms prompt-based baselines while approaching the performance of supervised systems without requiring task-specific training.

Conclusion: Retrieval drives calibration gains, while debate improves reasoning on higher-level traits. Structured interaction and external memory play complementary roles in reliable LLM-based evaluation.
...
### [10] [Modeling U.S. Attitudes Toward China via an Event-Steered Multi-Agent Simulator](https://arxiv.org/abs/2606.06971)
*Chenxu Zhu,Hantao Yao,Wu Liu,Junbo Guo,Yongdong Zhang*

Main category: cs.MA

TL;DR: The paper introduces ES-MAS, an event-steered multi-agent simulator that uses continuous events and news to drive dynamic opinion evolution (U.S. public attitudes toward China), outperforming existing static simulators.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based multiagent simulators rely on static rules and fixed datasets, limiting their ability to capture the dynamic, event-driven nature of macro-level opinion shifts in real-world settings like geopolitical risks.

Method: 1. Constructs CURE dataset (2021-2025, 20 quarters, 258 events, 14,000+ news articles) as temporal foundation. 2. Proposes Dual-Stream Data Integration Engine (DSDIE) for timeline alignment and personalized information exposure. 3. Designs News-Driven Dynamic Interaction (NDDI) module for adaptive agent grouping and consensus formation.
...
Conclusion: ES-MAS provides a scalable and effective framework for modeling dynamic opinion evolution in real-world settings.
...
### [11] [Learning Multi-Agent Communication Protocol: Study on Information Entropy Efficiency in MARL](https://arxiv.org/abs/2606.07200)
*Xinren Zhang,Zixin Zhong,Jiadong Yu*

Main category: cs.MA

TL;DR: This paper introduces the Information Entropy Efficiency Index (IEI) to enable multi-agent communication protocols that balance task performance with information compactness, demonstrating equivalent or superior performance while improving communication efficiency across diverse MARL algorithms.


<details>
  <summary>Details</summary>
Motivation: Existing Multi-Agent Reinforcement Learning (MARL) approaches with communication prioritize performance gains through increasingly complex architectures and expanding communication overhead, lacking principled metrics to evaluate information exchange efficiency, which hinders the development of truly scalable MAS systems.

Method: The authors propose the Information Entropy Efficiency Index (IEI), which quantifies the ratio between message entropy and task performance, and incorporate IEI into training loss functions to encourage agents to develop communication protocols that achieve high performance with improved communication efficiency.

Result: Extensive experiments across diverse MARL algorithms demonstrate that the proposed approach achieves equivalent or superior task performance compared to baseline methods while significantly improving communication efficiency.

Conclusion: These findings challenge the prevailing assumption that performance improvements require complex architectures or increased communication overhead, highlighting the potential of simultaneously improving both task success and communication efficiency to enable scalable Multi-Agent Systems.
...
### [12] [Hierarchical Certified Semantic Commitment for Byzantine-Resilient LLM-Agent Collaboration](https://arxiv.org/abs/2606.07316)
...
### [13] [Modelling Opinion Dynamics at Scale with Deep MARL](https://arxiv.org/abs/2606.07487)
*Lukas Seier,Brandon Kaplowitz,Sebastian Towers,Richard Bailey,Jakob Foerster*

Main category: cs.MA

TL;DR: MARL-based GPU-accelerated consensus and truth-finding game (1000 agents) applied to Bluesky network; high conformity in large social media reduces collective accuracy and promotes dishonesty, while small dynamic networks benefit from conformity, suggesting a mismatch between evolved heuristics and modern platforms contributing to misinformation.


<details>
  <summary>Details</summary>
Motivation: Hand-crafted local interaction rules are conventional for modeling opinion dynamics; MARL enables direct learning of behaviors through reward optimization, offering a more flexible alternative to explore emergent phenomena like consensus and polarization.

Method: Introduced a GPU-accelerated consensus and truth-finding game scaling to 1000 agents, extended other-play to general-sum social interactions, and used a learned attention layer to recover agent importance structures from graph topology; validated on Bluesky network and compared with small dynamic hunter-gatherer networks.

Result: High conformity in large social media networks (Bluesky) significantly reduces collective accuracy and promotes dishonest agents who lie to fit in; small, dynamic hunter-gatherer networks are less affected, where conformity can even improve collective agreement.

Conclusion: Evolved human conformity heuristics are mismatched with modern social media environments, potentially contributing to misinformation spread; MARL provides a flexible framework for modeling such dynamics at scale.
...
### [14] [Robots Need More than VLA and World Models](https://arxiv.org/abs/2606.06556)
...
TL;DR: This position paper critiques the current framing of generalist robot intelligence as solely a policy-scaling problem and argues that the central bottleneck is the absence of mechanisms to convert unstructured behavioral data into grounded robot supervision. It identifies four missing components: data interfaces for autolabeling unstructured behavior, embodiment interfaces for retargeting human motion to robot actions, world-model interfaces for physics-grounded 3D reasoning, and reward interfaces for inferring task progress and success from video and language.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the prevailing view that generalist robot intelligence can be achieved simply by collecting more robot demonstrations and training larger Vision-Language-Action (VLA) models. While human motion, internet video, simulation rollouts, and interactive demonstrations contain rich information about tasks, goals, contacts, failures, and physical constraints, this information is not directly usable by robot policies due to the lack of embodiment-specific action labels, task semantics, and reward structure.

Method: The paper surveys recent progress in robot foundation models, cross-embodiment datasets, learning from video, world models, and reward modelling, and proposes a research agenda for building robotics systems that can learn from the broader physical world rather than just robot demonstrations.

Result: The paper identifies four missing components for the next generation of robotics: (1) data interfaces for autolabeling unstructured behavior, (2) embodiment interfaces for retargeting human motion to robot actions, (3) world-model interfaces for physics-grounded 3D reasoning, and (4) reward interfaces for inferring task progress and success from video and language.

Conclusion: To build robotics systems capable of generalist intelligence, researchers must develop mechanisms that convert the world's abundant unstructured behavioral data into grounded robot supervision through these four interfaces, enabling robots to learn from not just their own demonstrations but from the broader physical world.
...
### [15] [ChronoForest: Closed-Loop Multi-Tree Diffusion Planning for Efficient Bridge Search and Route Composition](https://arxiv.org/abs/2606.06618)
*Jungmin Seo,Jaesik Park*

Main category: cs.RO

TL;DR: ChronoForest is a closed-loop planning system that combines local bridge search with online route re-solving to efficiently plan long-horizon routes using only short-horizon offline trajectories, achieving 99.3-99.8% success on AntMaze benchmarks and correcting poor temporal orderings on Hamiltonian route-composition tasks.


<details>
  <summary>Details</summary>
Motivation: Planning long-horizon routes is critical in offline navigation, but collecting rich long-range data is difficult. The challenge involves composing many short-horizon segments (microscopic trade-off between search cost and path quality) and ordering waypoints when anchor travel costs are unknown and unreliable when estimated only from temporal distance.

Method: ChronoForest couples local bridge search with online route re-solving through an anchor-chaining tree diffusion planner and online multi-tree orchestrator. It uses temporal distance for short-range guidance and node evaluation, while search-time bridge evidence validates long-range anchor connectivity and enables repeated route re-solving.

Result: On OGBench AntMaze-Stitch, ChronoForest achieves 99.8%, 99.3%, and 99.5% success on medium, large, and giant splits respectively, improving giant-stitch success by up to 34.5 points over prior diffusion-based results. On Hamiltonian route-composition benchmarks, online re-solving corrects poor temporal orderings and improves route quality while being substantially cheaper than exhaustive planning.

Conclusion: ChronoForest effectively addresses the long-horizon route planning challenge by bridging microscopic local search with macroscopic online re-solving, demonstrating that temporal distance can serve as effective guidance when combined with search-time bridge evidence for long-range validation.
...
### [16] [What Matters When Cotraining Robot Manipulation Policies on Everyday Human Videos?](https://arxiv.org/abs/2606.06627)
*Richard Li,Aditya Prakash,Andrew Wen,Saurabh Gupta,Yilun Du,Pulkit Agrawal*

Main category: cs.RO

TL;DR: The paper investigates transferring robot manipulation policies from everyday Internet videos to robots, finding that hand pose quality and embodiment specialization are critical factors, with a co-training approach achieving 29.7% absolute success rate improvement across six manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Current human video datasets for robot manipulation co-training are limited: they require curated demonstrations with orchestrated motions and specialized hardware for 3D hand pose capture. Everyday Internet video offers a much more plentiful data source, but it's unclear what factors enable effective transfer from such videos to robots.

Method: The authors created a new dataset of 532 human videos containing 28 hours of high-quality triangulated hand labels and natural motions. They investigated the transferability of manipulation policies from this dataset to robots, examining the effects of hand pose quality and motion gaps. Their approach involved co-training vision and policy networks to specialize to each embodiment.

Result: Hand pose quality affects transfer, but even with accurate hand labels, the inherent motion gap hinders transfer unless the vision and policy networks specialize to each embodiment. The co-training recipe yields consistent improvements, with an absolute success rate gain of 29.7% in the low-robot-data regime across six manipulation tasks.

Conclusion: Successful transfer from everyday Internet videos to robots requires both accurate hand pose annotations and network specialization to different embodiments. The motion gap between humans and robots cannot be overcome without specialized co-training approaches, which can significantly improve robot manipulation performance.
...
### [17] [On the Hardness of Optimal Motion on Trees](https://arxiv.org/abs/2606.06686)
*Tzvika Geft*

Main category: cs.RO

TL;DR: This paper proves NP-hardness of Multi-Agent Path Finding (MAPF) on trees across standard objectives (distance, makespan, flowtime) for both labeled and 2-colored variants, resolving the long-standing Pebble Motion problem on trees and establishing a tractability barrier in fundamental motion models.


<details>
  <summary>Details</summary>
Motivation: While many MAPF variants are intractable, several basic cases on trees remained open for decades, particularly the classical Pebble Motion problem. The authors aim to establish hardness results that unify and strengthen prior knowledge by connecting MAPF to the Stack Rearrangement problem.

Method: The authors establish NP-hardness by reducing from the Stack Rearrangement problem, which optimally rearranges items in stacks. This reduction works on simple tree structures (subdivided stars) and proves hardness for both labeled and 2-colored MAPF across all three standard objectives (distance, makespan, flowtime).

Result: The paper proves NP-hardness on trees for: (1) labeled MAPF under all three objectives, (2) 2-colored MAPF under all three objectives, (3) classical Pebble Motion on trees, and (4) colored Pebble Motion on any graph class (first result with two colors). All results are established via Stack Rearrangement reduction on subdivided stars.

Conclusion: The results reveal a common tractability barrier permeating several fundamental motion models, unifying and strengthening prior hardness results. The connection to stacks demonstrates that hardness emerges even on very simple tree structures, suggesting fundamental limitations in solving these problems optimally.
...
### [18] [Optimal Control Approach for Non-prehensile Ball Juggling Using a 7-DoF Manipulator](https://arxiv.org/abs/2606.06704)
*Joel Ramadani,Vasilije Rakčević,Riddhiman Laha,Arne Sachtler,Valentin Le Mesle,Achim J. Lilienthal,Sami Haddadin*

Main category: cs.RO

TL;DR: This paper presents a model-based framework for controlling a 7-DOF manipulator to perform non-prehensile ball juggling with a tool. The approach uses a two-stage optimal control method to generate feasible motion patterns for stable periodic juggling, with offline-computed trajectories enabling real-time error correction without online optimization.


<details>
  <summary>Details</summary>
Motivation: Non-prehensile manipulation tasks like juggling require dynamic stabilization of underactuated objects where stability depends entirely on applied forces. High-speed manipulation with sensitive hybrid dynamics makes juggling particularly challenging for robots, as timing is critical to counteract deviations and maintain desired behavior.

Method: A model-based framework incorporating a two-stage optimal control approach to compute feasible motion patterns for stable juggling. The method organizes offline-computed trajectories to enable real-time error correction without solving optimal control problems online.

Result: The controller was successfully evaluated in a simulation environment and demonstrated experimentally on a Franka Emika Panda robot, showing effectiveness in performing 7-DOF manipulator ball juggling with a tool.

Conclusion: The proposed framework enables dynamic stabilization of underactuated objects through systematic trajectory generation and periodic motion stabilization, making high-speed non-prehensile manipulation tasks like juggling achievable for robots.
...
### [19] [SCOUT: Semantic scene COverage via Uncertainty-guided Traversal](https://arxiv.org/abs/2606.06721)
*Junyu Mao,Sara Ayoubi,Vishnu D. Sharma,Ilija Hadžić,Matthew Andrews*

Main category: cs.RO

TL;DR: SCOUT is an online semantic exploration framework that couples active traversal with probabilistic scene graph construction, enabling robots to incrementally build uncertainty-aware 3D scene graphs and select viewpoints by balancing semantic certainty gain, geometric coverage gain, and travel cost.


<details>
  <summary>Details</summary>
Motivation: Most 3D scene graph pipelines treat perception as a post-processing stage over fixed datasets, decoupling scene representation from the decisions that determine what is observed. This separation prevents robots from progressively understanding space over extended operations.

Method: SCOUT incrementally builds an uncertainty-aware 3D scene graph where nodes maintain fused geometry and posterior beliefs over open-vocabulary object labels, while edges encode structural relations (on, inside, belong, next to). These beliefs feed an uncertainty-guided traversal planner that selects viewpoints balancing expected semantic certainty gain, geometric coverage gain, and travel cost.

Result: The system enables robots to revisit ambiguous objects for additional evidence and expand into unseen free space when the scene remains incomplete, treating semantic scene completeness as an operational objective rather than a passive by-product.

Conclusion: SCOUT moves toward autonomous agents capable of patrolling, updating, and reasoning about evolving indoor environments with minimal human intervention, closing the loop between perception and exploration decisions.
...
### [20] [IDDMBSE: Integrating Data-Driven and Model-Based Systems Engineering for Trusted Autonomous Cyber-Physical Systems](https://arxiv.org/abs/2606.06727)
*John S. Baras,Sai Sandeep Damera,Ryan Matheu,Clinton Enwerem,Praveen M. S. Kumar*

Main category: cs.RO

TL;DR: IDDMBSE is an integrated methodology that combines Model-Based Systems Engineering (MBSE) with data-driven ML/AI, implemented through three interoperable tools (PERFECT, TRADES-X, VERITAS) and demonstrated on a Trusted Autonomous Ground Robot across its complete development lifecycle.


<details>
  <summary>Details</summary>
Motivation: Autonomous cyber-physical systems (CPS) require integration of Model-Based Systems Engineering (MBSE) and data-driven Machine Learning/AI (ML/AI), but no existing SE methodology natively spans both domains. The gap exists between the rigorous, model-driven MBSE V-process and the data-driven ML/AI approaches.

Method: IDDMBSE extends the MBSE V-process with a data-driven loop at every step, anchored in SysML, the autonomy stack, and a hybrid model-based plus data-driven trade-off architecture. The methodology is instantiated through three interoperable open-source tools: PERFECT (maps SysML to ROS stacks for performance evaluation), TRADES-X (decomposes design-space exploration into model-based optimization followed by data-driven evaluation), and VERITAS (combines formal, data-driven, and runtime verification).

Result: The methodology was demonstrated on a Trusted Autonomous Ground Robot across its development lifecycle, including sensor-suite selection, risk-sensitive path planning, behavior-tree task verification, conformal-prediction-based robust perception, and assured multi-robot coordination. All aspects were exercised in a contested-terrain Isaac Sim test range that was released with the tool chain.

Conclusion: IDDMBSE is being re-formulated on SysML v2 / KerML foundations to enable language-native composability and tighter ML/AI integration. This future work aims to improve the integration between the modeling language and ML/AI components.
...
### [21] [AxisGuide: Grounding Robot Action Coordinate System in RGB Observations for Robust Visuomotor Manipulation](https://arxiv.org/abs/2606.06761)
...
### [22] [Multi-Robot Planning and Control from CCTV Camera Networks in a Real Warehouse](https://arxiv.org/abs/2606.06762)
...
### [23] [Learning All-Terrain Locomotion for a Planetary Rover with Actively Articulated Suspension](https://arxiv.org/abs/2606.06790)
*Arthur Bouton,Tristan D. Hasseler,Michael Paton,Travis Brown,Jacob Levy,William Reid,Joshua Martin,Hari Nayar*

Main category: cs.RO

TL;DR: ERNEST is a four-wheeled planetary rover with a two-degree-of-freedom Active Gimbal Suspension. A single neural network controller, trained via reinforcement learning in DARTS simulation with policy consolidation, enables autonomous traversal of challenging terrains including rock fields, sand ripples, and slopes, reducing cost of transport by 37% on dry sand and outperforming passive suspension on wet sand.


<details>
  <summary>Details</summary>
Motivation: Planetary rovers face mobility challenges on loose, heterogeneous terrains where passive suspensions are often insufficient. This work aims to unlock the full potential of an actuated suspension system through a unified neural network controller that can autonomously negotiate obstacles and adapt to varying soil conditions without explicit terrain classification.

Method: A reinforcement learning framework was developed using the DARTS simulation engine with rigid-contact dynamics and Bekker-Wong terramechanics. A policy consolidation strategy merged terrain-specialized agent experiences into a single neural network controller. Training employed domain randomization, sensor noise injection, and model-to-real system identification for zero-shot transfer.

Result: The learned controller enables autonomous traversal of rock fields, bump traps, wheel-high steps, sand ripples, and sandy slopes. On a 20° sandy slope, it reduces cost of transport by 37% on dry sand and achieves superior performance on wet sand where the passive suspension becomes immobilized.

Conclusion: The integration of a two-degree-of-freedom Active Gimbal Suspension with a unified neural network controller successfully enables autonomous obstacle negotiation across diverse terrains. The policy consolidation approach eliminates the need for explicit terrain classification, while domain randomization enables effective zero-shot transfer from simulation to physical hardware, demonstrating significant improvements in locomotion efficiency and capability.
...
### [24] [Lane Change Trajectory Planning for Personalized Driving Comfort and Mobility Efficiency](https://arxiv.org/abs/2606.06805)
*Haoxuan Dong,Dongjun Li,Ziyou Song*

Main category: cs.RO

TL;DR: A neural network-driven lane-change planner that integrates a third-order polynomial trajectory generator with a dual-head architecture (one for general operational guarantees, one for personalized comfort/efficiency preferences), using a statistical gate mechanism to adaptively select the appropriate head based on driving conditions, achieving personalized comfort and mobility while maintaining feasibility across diverse scenarios.


<details>
  <summary>Details</summary>
Motivation: Lane-changing involves tightly coupled longitudinal and lateral motions that significantly impact driving comfort and mobility efficiency. Due to substantial inter-vehicle variability and the personalized nature of driver preferences, trajectory planning for lane-change maneuvers is challenging to generalize across different drivers and conditions.

Method: The proposed system comprises: (1) a neural network-driven planner integrating a third-order polynomial trajectory generator; (2) a learning module that infers optimal trajectory parameters; (3) a shared backbone architecture with dual heads—one head ensures all-condition operational guarantees, the other captures driver-specific preferences for comfort or mobility efficiency; (4) a head-gated switching mechanism based on error-winner logistic regression statistical gate that adaptively selects the appropriate head under varying driving conditions.

Result: Representative cases and Monte Carlo simulations demonstrate that the proposed planner successfully achieves personalized comfort and mobility during lane changes, while the baseline approach ensures feasible trajectories in scenarios where personalized data are insufficient or inaccessible.

Conclusion: The proposed context-aware lane-change trajectory planning approach effectively balances personalized driver preferences (comfort vs. mobility efficiency) with robust operational guarantees across diverse driving conditions, demonstrating the feasibility of adaptive, personalized autonomous driving trajectory planning.
...
### [25] [Three-dimensional hydro-cluttered locomotion by an undulatory robot](https://arxiv.org/abs/2606.06829)
...
TL;DR: AquaMILR is a cable-driven, programmable-compliance aquatic robot that navigates hydro-cluttered environments through regulated body deformation, depth control, and emergent rolling recovery mechanisms.


<details>
  <summary>Details</summary>
Motivation: Aquatic robots face challenges in "hydro-cluttered" environments with rigid and flexible obstacles. Existing models struggle with body-obstacle contact, and there's a need for robots that can exploit rather than avoid contact to navigate complex underwater spaces.

Method: The researchers developed AquaMILR, an elongate limbless robot featuring bilateral cable-driven actuation, programmable body compliance, distributed depth regulation, corrosion-resistant enclosures, and onboard power/electronics for untethered operation.

Result: Programmable body compliance regulates deformation and converts body-environment interactions into forward progression. Depth regulation enables 3D navigation and recovery from obstruction. Emergent inertia-induced rolling acts as a spontaneous recovery mechanism in jamming scenarios. Field tests in mangrove environments demonstrate practical navigation and onboard visual inspection.

Conclusion: These principles establish a design paradigm where aquatic robots exploit environmental complexity as a locomotor resource, enabling navigation of previously inaccessible underwater spaces.
...
### [26] [STRIPS-WM: Learning Grounded Propositional STRIPS-style World Models from Images](https://arxiv.org/abs/2606.06832)
*Abhiroop Ajith,Constantinos Chamzas*

Main category: cs.RO

TL;DR: STRIPS-WM is a framework that learns image-grounded STRIPS-style world models from visual transitions, enabling robots to plan high-level actions from start and goal images.


<details>
  <summary>Details</summary>
Motivation: Robots performing long-horizon visual manipulation must interpret high-dimensional images while focusing on action-relevant facts (what can be done now and what changes afterward). Classical task planners use symbolic operators with preconditions and effects, but obtaining such structured representations from raw visual experience remains challenging.

Method: STRIPS-WM induces a finite abstract transition graph from images, learns latent binary predicates and one grounded propositional operator per action label, forms a symbolic action model with sparse preconditions and effects, and distills the learned predicates into a visual encoder for classical planning from novel images.

Result: Experiments on visual rearrangement tasks demonstrate that STRIPS-WM improves image-to-plan success compared to visual rollout, latent graph-search, and latent-symbolic baselines.

Conclusion: STRIPS-WM successfully learns interpretable, symbolic action models directly from visual transitions, enabling classical planning with novel start and goal images in visual manipulation tasks.
...
### [27] [Think Like a Pilot: Fine-Grained Long-Horizon UAV Navigation](https://arxiv.org/abs/2606.06836)
...
### [28] [What Is My Robot Thinking? Design Considerations for Transparent and Trustworthy Shared Autonomy](https://arxiv.org/abs/2606.06870)
...
### [29] [Neuro-Symbolic Learning for Long-Horizon Task Planning Under Complex Logical Constraints](https://arxiv.org/abs/2606.06877)
*Qiwei Du,Zitong Zhan,Shaoshu Su,Bowen Li,Yi Du,Zhipeng Zhao,Taimeng Fu,Sebastian Scherer,Jiaoyang Li,Chen Wang*

Main category: cs.RO

TL;DR: A neuro-symbolic approach to task planning that formulates object-importance learning as a bilevel optimization problem with a 3R (Repair, Restart, Rollback) strategy to address exposure bias from imperfect pruning, achieving 80.04% failure rate reduction and 57.14% planning time reduction.


<details>
  <summary>Details</summary>
Motivation: Task planning for robots suffers from efficiency bottlenecks when reasoning over long-horizon action sequences with complex logical constraints. Existing neuro-symbolic methods prune task-irrelevant objects but rely on fixed offline supervision from full search spaces, creating a train-test mismatch where deployment uses pruned spaces from imperfect predictions, leading to exposure bias and degraded planning performance.

Method: Formulates object-importance learning as an imperative learning-based bilevel optimization problem: the upper level optimizes a neural scorer, while the lower level solves a symbolic planning problem in the score-pruned search space. To stabilize training, introduces a 3R strategy (Repair, Restart, Rollback) that provides reliable and adaptive feedback for upper-level learning.

Result: Achieves state-of-the-art performance on three challenging benchmarks with 80.04% reduction in failure rate and 57.14% reduction in planning time. Validated on a quadruped-based mobile manipulator in both simulation and real-world deployment.

Conclusion: Demonstrates the potential for efficient and deployable neuro-symbolic task planning, addressing the exposure bias problem through the 3R recovery strategy and bilevel optimization framework.
...
### [30] [A Cross-view Fusion Framework for Robust 6-DoF Grasp Pose Estimation](https://arxiv.org/abs/2606.06878)
*Kangjian Zhu,Haobo Jiang,Jianjun Qian,Jin Xie*

Main category: cs.RO

TL;DR: A cross-view fusion framework for 6-DoF grasp pose estimation that uses self-supervised contrastive learning and a cylindrical coordinate integration module to enhance robustness in corner views.


<details>
  <summary>Details</summary>
Motivation: Enhance robustness of 6-DoF grasp pose estimation in corner views by alleviating occlusion and avoiding time-consuming task-agnostic multi-view reconstruction through a post-fusion strategy.

Method: Proposes a cross-view fusion framework featuring: (1) self-supervised contrastive learning using cross-view point pair matching (same 3D location = match, different directions = non-match) to regularize point cloud features; (2) cross-view-aligned cylinder integration module that aligns points/features by similarity, registers into cylindrical coordinate frame to emphasize rotation-symmetric geometry, and uses alternating local self-attention and seed cross-attention layers for fine-grained representation.

Result: Achieves strong performance on the GraspNet-1Billion benchmark and in real-world applications.

Conclusion: The self-supervised contrastive learning strategy significantly enhances spatial consistency and direction distinctiveness of point features, while the cylinder integration module effectively fuses grasp-relevant geometry, thereby facilitating cross-view fusion and improving estimation robustness against occlusion and noise.
...
### [31] [ActionMap: Robot Policy Learning via Voxel Action Heatmap](https://arxiv.org/abs/2606.06904)
*Pei Yang,Hai Ci,Yanzhe Chen,Qi Lv,Han Cai,Mike Zheng Shou*

Main category: cs.RO

TL;DR: ActionMap introduces a voxel heatmap action head that replaces the traditional unstructured action decoders in VLA models, enabling them to exploit geometric relationships in action space for better performance and data efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLA models use action decoders that treat action space as unstructured (via autoregressive token bins, L1 regression, or flow-matching denoising), failing to leverage the geometric proximity of neighboring actions. Despite rapid advances in backbones and training recipes, the action decoder architecture has remained virtually unchanged.

Method: ActionMap is a voxel heatmap action head that replaces the native action decoder in existing VLA models. For each action, it predicts a voxel heatmap over the continuous action space where each voxel directly stores the probability of the corresponding action, exploiting the geometric structure of action space.

Result: Across LIBERO simulation and real-world Franka manipulation, ActionMap surpasses two architecturally distinct backbones at matched training steps (+8.2% over OpenVLA-OFT's L1 regression head on LIBERO four-suite average), converges at comparable or faster rates, and shows marked data efficiency at low training data. The cross-backbone consistency demonstrates that action representation is a key performance lever distinct from backbone or recipe scaling.

Conclusion: ActionMap demonstrates that action representation is a critical factor for VLA performance, independent of backbone architecture and training recipe. By exploiting geometric relationships in action space through voxel heatmaps, ActionMap provides a more effective and data-efficient alternative to traditional unstructured action decoders.
...
TL;DR: A terrain-conditioned generative motion prior (T-GMP) combined with adversarial learning and foothold penalty enables humanoid robots to achieve both anthropomorphic naturalness and robust terrain traversal across varying environments.


<details>
  <summary>Details</summary>
Motivation: Achieving both anthropomorphic naturalness and robust terrain traversal remains a fundamental challenge in humanoid locomotion. Existing Reinforcement Learning (RL) approaches typically rely on fixed motion priors, limiting their adaptability to varying environments.

Method: The authors propose Terrain-conditioned Generative Motion Priors (T-GMP), a Conditional Variational Autoencoder (CVAE) module that captures terrain-conditioned latent motion manifolds from expert state-terrain demonstrations. This is integrated into an adversarial learning pipeline with a Foothold Penalty, where a discriminator dynamically modulates naturalness constraints conditioned on local terrain features.

Result: Experimental results demonstrate that the method outperforms existing baselines in traversal success rate and motion smoothness, while preserving biomimetically natural and physically coordinated motions.

Conclusion: The proposed approach successfully enables unified policies that adapt to terrain variations with smooth style transitions, addressing the core challenge of combining natural humanoid motion with robust terrain traversal capabilities.
...
### [33] [LIMMT: Less is More for Motion Tracking](https://arxiv.org/abs/2606.06953)
*Yu Guan,Zekun Qi,Chenghuai Lin,Xuchuan Chen,Dairu Liu,Wenyao Zhang,Jilong Wang,Xinqiang Yu,He Wang,Li Yi*

Main category: cs.RO

TL;DR: LIMMT is a data-centric framework that defines motion data quality through physics feasibility, diversity, and complexity dimensions. The authors demonstrate that training with under 3% of AMASS outperforms full-dataset training for physics-based humanoid motion tracking.


<details>
  <summary>Details</summary>
Motivation: High-quality motion data can steer tracking policies toward better optimization trajectories early in training. This is the first data-centric study for physics-based humanoid motion tracking, which traditionally focuses on model-centric approaches.

Method: The authors introduce LIMMT (Less Is More for Motion Tracking) which defines motion data quality through three dimensions: physics feasibility, diversity, and complexity. They also perform data cleaning on web-sourced mocap data and compare performance across different data subset sizes.

Result: Training with under 3% of AMASS yields better tracking performance than training with the full dataset. Extensive experiments validate the effectiveness of their quality-based data selection framework.

Conclusion: Quality-focused data selection and cleaning is more effective than using all available data for physics-based humanoid motion tracking. A small, high-quality subset of motion data can achieve superior results compared to using the full dataset.
...
### [34] [Compliance-Based Sensor Placement for Force Sensing on a Sensorized Prostate Phantom](https://arxiv.org/abs/2606.06977)
...
### [35] [Mission-Level Runtime Assurance Framework for Autonomous Driving](https://arxiv.org/abs/2606.06996)
...
### [36] [Task Editing for Generalizable 3D Visuomotor Policy Learning](https://arxiv.org/abs/2606.07012)
...
### [37] [A Multi-Operator Mixed-Reality Interface for Multi-Robot Control and Coordination: Co-Located and Private Workspace Collaboration](https://arxiv.org/abs/2606.07013)
*Omotoye Shamsudeen Adekoya,Antonio Sgorbissa,Carmine Tommaso Recchiuto*

Main category: cs.RO

TL;DR: A mixed-reality interface extends HORUS to support collaborative multi-operator control of robot teams through co-located shared workspace and private-workspace modes, with registration-driven scene construction and per-robot control leases preventing conflicting commands.


<details>
  <summary>Details</summary>
Motivation: Multi-operator control of robot teams requires not only shared mission information but also mechanisms for maintaining shared awareness and preventing conflicting interventions, extending single-operator multi-robot supervision to collaborative multi-operator use.

Method: The system features two complementary modes: co-located shared workspace (operators observe and manipulate the same mini-map in the same physical location) and private-workspace mode (independently placed local workspaces). Architecture combines registration-driven scene construction, lightweight shared-session synchronization, and per-robot control leases. Evaluated in a human-subject study with 36 participants (18 pairs) controlling three Nova Carter mobile robots in two search environments.

Result: Objective task performance was comparable across both modes, indicating both supported effective mission execution. However, the co-located shared workspace significantly improved perceived collaboration, shared understanding, and handoff clarity, and was the preferred collaborative mode.

Conclusion: Physically co-locating the MR workspace improves how operators coordinate even when the underlying robot-control tools remain unchanged.
...
### [38] [Extending Responsibility-Sensitive Safety for the Assessment of Offloaded Autonomous Driving Services](https://arxiv.org/abs/2606.07067)
...
### [39] [Predictive Style Matching: Natural and Robust Humanoid Locomotion](https://arxiv.org/abs/2606.07083)
*Simeon Nedelchev,Ekaterina Chaikovskaia,Egor Davydenko,Eduard Zaliaev,Roman Gorbachev*

Main category: cs.RO

TL;DR: Predictive Style Matching (PSM) improves humanoid locomotion quality by using an offline predictor to generate interpretable upper-body targets from state history, achieving ~10x lower style error than task-only RL while maintaining robust fall recovery.


<details>
  <summary>Details</summary>
Motivation: Task-only RL produces stiff, asymmetric gaits, while motion imitation improves appearance but reduces disturbance recovery because reference signals oppose transient balance poses.

Method: An offline predictor maps the robot's lower-body state history and velocity commands to interpretable upper-body joint and gait targets that shape rewards during training. The targets are state-conditioned rather than time-indexed, and the predictor is only used during training.

Result: On the Unitree G1, PSM reduces upper-body style error by roughly an order of magnitude over task-only RL while preserving its fall-recovery rate, whereas the motion-imitation baseline attains the lowest style error but fails to recover from disturbances about five times as often.

Conclusion: PSM successfully balances motion quality with robustness, outperforming both task-only RL (better style) and motion imitation (better recovery) by providing a middle ground that maintains the deployed controller's simple proprioceptive interface and inference cost.
...
### [40] [Dreaming when Necessary: Advancing World Action Models with Adaptive Multi-Modal Reasoning](https://arxiv.org/abs/2606.07089)
*Yinzhou Tang,Jingbo Xu,Yu Shang,Zihao Song,Chen Gao,Wei Wu,Yong Li*

Main category: cs.RO

TL;DR: AdaWAM is a world action model with adaptive multimodal reasoning abilities, integrating a lightweight dynamic router that autonomously switches between textual and visual reasoning during task execution to improve efficiency and performance on complex embodied tasks.


<details>
  <summary>Details</summary>
Motivation: Existing world action models rely heavily on video prediction as action priors and lack adaptive multimodal reasoning, limiting their effectiveness on long-horizon, complex tasks. The authors observe that different execution contexts require different reasoning modes: textual reasoning for high-level action prediction during task transitions, and visual reasoning for precise control during fine-grained manipulation.

Method: AdaWAM integrates a lightweight dynamic router that autonomously triggers textual or visual reasoning as needed during task execution, enabling adaptive multimodal reasoning based on contextual requirements.

Result: Experiments on both simulated and real-world embodied tasks demonstrate that AdaWAM substantially improves inference efficiency while outperforming state-of-the-art embodied policies.

Conclusion: AdaWAM demonstrates that adaptive multimodal reasoning significantly enhances world action models, making them more effective for long-horizon, complex embodied tasks by dynamically selecting the appropriate reasoning mode (textual vs. visual) based on execution context.
...
### [41] [Coarse-to-Control: Action-Token Planning for Vision-Language-Action Models](https://arxiv.org/abs/2606.07107)
...
### [42] [QuadVerse: An Integrated Framework Aligning Visual-Physical Reality for Quadruped Simulation](https://arxiv.org/abs/2606.07118)
*Yuxiang Chen,Yuanhao Wang,Ziheng Zhang,Meng Zhang,Yu Liu,Yufei Jia,Tiancai Wang,Erjin Zhou,Jin Xie*

Main category: cs.RO

TL;DR: QuadVerse is an integrated framework that uses reconstructed 3D Gaussian Splatting scenes to align visual perception, contact dynamics, and actuator dynamics, bridging the sim-to-real gap more effectively than separate approaches.


<details>
  <summary>Details</summary>
Motivation: Simulation is central to robot learning, but the sim-to-real gap remains a major bottleneck. Existing approaches tackle visual or dynamic gaps separately, overlooking how these individual mismatches accumulate and propagate throughout the robot's state evolution.

Method: QuadVerse reconstructs geometry-constrained 3D Gaussian Splatting scenes from RGB videos to enable batched photorealistic ego-view rendering and collision-ready semantic mesh extraction. Meshes initialize spatially varying friction priors refined via trajectory-based posterior search. A residual dynamics compensator is trained by replaying real-world trajectories on the contact-calibrated terrain.
...
Conclusion: The integrated framework addresses the sim-to-real gap more effectively by simultaneously aligning visual perception, physical interaction, and actuator dynamics, rather than treating them separately.
...
### [43] [Test-Time Trajectory Optimization for Autonomous Driving](https://arxiv.org/abs/2606.07170)
*Yihong Xu,Eloi Zablocki,Yuan Yin,Elias Ramzi,Ellington Kirby,Alexandre Boulch,Matthieu Cord*

Main category: cs.RO

TL;DR: TOAD is a plug-and-play method that improves autonomous driving trajectory planners by treating the scoring function as a learned reward and optimizing trajectories to maximize it using the Cross-Entropy Method, requiring no retraining.


<details>
  <summary>Details</summary>
Motivation: Traditional end-to-end planners generate candidate trajectories first, then score them, but the scorer cannot influence the set of generated candidates. A weak set of candidates limits planning performance regardless of the scorer's quality, so the authors aim to make the scorer actively guide trajectory generation.

Method: Treat the scorer as a learned trajectory-level reward function and search for trajectories that maximize it using the Cross-Entropy Method. The method is warm-started from the planner's proposals, requires no retraining, and is plug-and-play for existing planners.

Result: Across six base planners, TOAD improved NAVSIM-v1 (94.7 PDMS), NAVSIM-v2 (56.3 EPDMS), and the closed-loop HUGSIM benchmark.

Conclusion: TOAD is a simple, effective enhancement that can significantly improve existing trajectory planners without requiring retraining, demonstrating that optimizing the scorer during planning can overcome the limitations of weak candidate sets.
...
### [44] [A Causal Probabilistic Framework for Perception-Informed Closed-Loop Simulation of Autonomous Driving](https://arxiv.org/abs/2606.07186)
...
TL;DR: A perception-informed SIL testing methodology that bridges ground-truth simulation and real-world perception behavior by incorporating causal probabilistic models into standardized scenario-based simulation toolchains, enabling systematic injection of realistic perception errors for SOTIF validation.


<details>
  <summary>Details</summary>
Motivation: Current SIL simulation frameworks use ideal sensing, which bypasses the functional insufficiencies of perception algorithms, leading to over-optimistic safety assessments and missing latent operational risks.

Method: A framework for incorporating causal probabilistic models into standardized, scenario-based simulation toolchains applicable to ADAS and ADS. The approach systematically injects realistic perception errors (loss of detection, sizing inaccuracies, positioning offsets) derived from physical triggering conditions like fog, rain, and object-merging scenarios.

Result: Perception-informed testing reveals latent operational risks that ideal SIL environments fail to capture, demonstrating the effectiveness of the approach for SOTIF (ISO 21448) validation.

Conclusion: The methodology provides a scalable pathway for SOTIF validation by systematically injecting realistic perception faults into standardized simulation environments, bridging the gap between ground-truth simulation and real-world perception behavior.
...
### [45] [Shield-Loco: Shielding Locomotion Policies with Predictive Safety Filtering](https://arxiv.org/abs/2606.07193)
*Aditya Shirwatkar,Sebastian Sanokowski,Shishir Kolathaya,Aaron Johnson,Majid Khadiv*

Main category: cs.RO

TL;DR: This paper proposes a predictive safety filter that post-hoc filters contact locations for RL policies to prevent collisions, using a sampling-based optimizer with full-physics model and value function to find safer contact sequences.


<details>
  <summary>Details</summary>
Motivation: RL policies enable dynamic legged locomotion but lack mechanisms to avoid safety constraints absent during training. Existing solutions are impractical (offline learning) or degrade performance (conservative controllers) or use reduced-order models that cannot reason about whole-body behaviors.

Method: A predictive safety filter that post-hoc filters nominal contact locations fed to RL policy. When collision is predicted, a sampling-based optimizer asynchronously searches for safer contact sequences using a full-physics model, while a learned value function bootstraps long-horizon returns. Three algorithmic components make optimization tractable: geometric projection of sampled contacts, momentum-augmented updates, and replica-exchange.

Result: Validated on quadruped robot in dense, cluttered environments (simulation and real-world). Shows substantial reductions in safety violations with minimal deviation from nominal input.

Conclusion: The proposed predictive safety filter effectively prevents safety violations while maintaining task performance, offering a practical solution for deploying RL policies in real-world environments with safety constraints.
...
### [46] [An Abstract Architecture for Explainable Autonomy in Hazardous Environments](https://arxiv.org/abs/2606.07211)
*Matt Luckcuck,Hazel M Taylor,Marie Farrell*

Main category: cs.RO

TL;DR: This paper proposes an abstract architecture for "explainable autonomy" that enables autonomous systems to explain their decision-making behavior, providing a design template for implementing explainable autonomous systems, with a focus on applications in the civil nuclear industry.


<details>
  <summary>Details</summary>
Motivation: Human workers will continue to use and direct autonomous robots in hazardous environments, making trust in these systems crucial. Recent literature indicates that explainability is closely related to trustworthiness, and explainability should be designed into systems rather than added as an afterthought.

Method: The authors present an abstract architecture that supports autonomous systems explaining their behavior (explainable autonomy), providing a design template for implementing explainable autonomous systems. The paper demonstrates how this architecture could be applied in the civil nuclear industry.

Result: The architecture provides a framework for implementing explainable autonomous systems that can communicate their decision-making processes, thereby enhancing trust in autonomous systems operating in hazardous environments.

Conclusion: The proposed abstract architecture offers a design template for building explainable autonomous systems, which is essential for ensuring trust in systems that make sophisticated decisions in dangerous contexts such as the civil nuclear industry.
...
### [47] [Robotic Policy Adaptation via Weight-Space Meta-Learning](https://arxiv.org/abs/2606.07217)
*Christian Bianchi,Siamak Yousefi,Alessio Sampieri,Andrea Roberti,Luca Rigazio,Fabio Galasso,Luca Franco*

Main category: cs.RO

TL;DR: WIZARD is a weight-space meta-learning framework that generates task-specific LoRA parameters for frozen VLA policies using only language instructions and short demonstration videos, enabling zero-shot adaptation without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: VLA models require costly task-specific demonstrations, action annotations, and fine-tuning for adaptation, limiting their scalability and deployment.

Method: WIZARD learns to map task evidence (language instruction + short demonstration video) to expert LoRA updates during meta-training, predicting adaptation weights in a single forward pass without target-task action labels or test-time optimization.

Result: On LIBERO, WIZARD improves performance by ~2x on unseen dataset collections and up to ~14x on unseen tasks. On a real Franka Emika Panda, it consistently outperforms a real-domain adapted baseline.

Conclusion: Generated adapters provide task-level specialization beyond simulation, enabling efficient, zero-shot adaptation of VLA models to new tasks without additional fine-tuning.
...
### [48] [Beyond Waypoints: A Trajectory-Centric Waypointing Paradigm for Vision-Language Navigation](https://arxiv.org/abs/2606.07244)
...
### [49] [CAPE: Contrastive Action-conditioned Parallel Encoding for Embodied Planning](https://arxiv.org/abs/2606.07304)
*Cong Chen,Haowen Wang,Zhixiang Zhang,Pei Ren,Zhengping Che*

Main category: cs.RO

TL;DR: CAPE (Contrastive Action-conditioned Parallel Encoding) is a framework that learns visual dynamics by distinguishing future outcomes induced by different action sequences. It decodes full future latent trajectories in a single forward pass and uses a Goal-Convergent Contrastive Objective to align predictions of the same outcome while separating different outcomes.


<details>
  <summary>Details</summary>
Motivation: Existing visual dynamics models focus on reconstructing future visual states or dense latent representations, which spreads learning capacity across visually salient but planning-irrelevant content rather than the action-conditioned changes that drive manipulation outcomes.

Method: CAPE uses a Contrastive Action-conditioned Parallel Encoding framework that decodes the full future latent trajectory in a single forward pass. It is trained with a Goal-Convergent Contrastive Objective that aligns predictions corresponding to the same future outcome while separating those corresponding to different outcomes.

Result: CAPE substantially outperforms prior baselines on future-state retrieval, offline action matching, and closed-loop planning. It notably reduces planning-time inference cost at long prediction horizons.

Conclusion: CAPE provides a more efficient and effective approach to learning visual dynamics for embodied agents by focusing on action-conditioned changes rather than visually salient but irrelevant content, while maintaining lower inference costs compared to previous methods.
...
### [50] [RhinoVLA Technical Report](https://arxiv.org/abs/2606.07383)
...
TL;DR: RhinoVLA is a deployment-optimized VLA model co-designed with the Huixi R1 edge SoC that achieves 11.69 Hz inference (meeting 10 Hz real-time control target) while maintaining performance comparable to π0.5 through token-efficient architecture, continuous action experts, and hardware-aware optimizations.


<details>
  <summary>Details</summary>
Motivation: Real-time deployment of VLA models on edge hardware is challenging due to linear computation growth with VLM tokens; existing models lack deployment-aware design for edge SoCs like Huixi R1.

Method: RhinoVLA co-designed with Huixi R1 edge SoC featuring: (1) token-efficient Qwen3-VL backbone with continuous Action Expert; (2) unified interface combining View Registry, 72D physical state-action slot space, and robotinstance LoRA for cross-robot learning; (3) hardware-aware compilation, mixed-precision execution, and parallel visual encoding.

Result: RhinoVLA achieves downstream performance comparable to π0.5 at similar parameter scale, with 11.69 Hz end-to-end inference on Huixi R1, meeting the 10 Hz real-time closed-loop control target.

Conclusion: Token-efficient architecture design and hardware-aware optimizations enable VLA models to achieve real-time deployment on edge hardware while maintaining multimodal capability and supporting cross-robot learning through unified interfaces.
...
### [51] [Spline Policy: A Structured Representation for Robot Policies](https://arxiv.org/abs/2606.07386)
*Mengze Tian,Yiming Li,Sichao Liu,Auke Ijspeert,Sylvain Calinon*

Main category: cs.RO

TL;DR: This paper proposes Spline Policy (SP), a structured representation that replaces action chunks with spline parameters while preserving the policy backbone, enabling compact decoding, temporal resampling, local correction, uncertainty propagation, and compatibility with classical control mechanisms.


<details>
  <summary>Details</summary>
Motivation: Modern imitation-learning policies for robot manipulation typically use fixed-resolution action chunks, which are simple but expose limited geometric and temporal structure before execution. This limits the ability to perform operations like temporal resampling, local correction, and uncertainty evaluation.

Method: SP replaces action chunks with spline parameters while keeping the policy backbone unchanged. The predicted spline can be decoded as a compact continuous trajectory, queried at different temporal resolutions, and edited in parameter space. For quadratic splines, an analytical distance-field construction converts the output into a state-dependent vector field, yielding a local corrective mechanism. SP also supports uncertainty propagation and can be combined with classical control mechanisms like null-space collision avoidance.

Result: Experiments across low-dimensional motion learning, simulated manipulation, dexterous manipulation, and real-robot case studies demonstrate that SP remains compatible with modern policy learners (diffusion, flow-matching, transformer-based, and vision-language-action backbones) while exposing useful motion-structure properties.

Conclusion: SP provides a structured representation that preserves compatibility with modern policy learners while enabling compact decoding, temporal resampling, local correction around predicted motions, uncertainty evaluation, and controller compatibility.
...
### [52] [Simulation-Driven Imitation Learning for Biosignals-Free Shared-Autonomy Prosthetic Grasping](https://arxiv.org/abs/2606.07389)
*Kaijie Shi,Wanglong Lu,Huiling Chen,Vinicius Prado da Fonseca,Ting Zou,Hanli Zhao,Xianta Jiang*

Main category: cs.RO

TL;DR: This paper presents a scalable simulation framework that automatically generates diverse reach-to-grasp demonstrations from a wrist-mounted virtual camera, enabling effective sim-to-real policy learning for biosignals-free shared-autonomy control of upper-limb prosthetic hands.


<details>
  <summary>Details</summary>
Motivation: Recent imitation-learning approaches for biosignals-free prosthetic control show promise but are limited by the high cost and variability of collecting large-scale real-world human demonstration data. The authors aim to address this bottleneck through simulation-driven data generation.

Method: The framework combines three key components: (1) physically feasible grasp synthesis, (2) natural reaching trajectories retargeting, and (3) reach-grasp-lift execution in procedurally generated indoor environments. It records wrist-view observations, proprioception, and actions to create a large-scale demonstration dataset.

Result: Simulation benchmarks demonstrate that generated demonstrations are sufficiently rich and consistent for effective policy learning. In three realistic settings, the learned sim-to-real policy achieves over 90% grasp success rate, surpasses baseline methods, and exhibits stronger generalization capabilities.

Conclusion: The results validate simulation-driven training as a promising approach for biosignals-free shared-autonomy prosthetic grasping, showing that synthetic demonstrations can effectively train policies that generalize well to real-world scenarios without requiring expensive and variable real-world data collection.
...
### [53] [Rapid co-design of Buoyancy-assisted robots for Challenging Locomotion using Gaussian Evolutionary Specialists](https://arxiv.org/abs/2606.07424)
*Ankit Sinha,Nitish Sontakke,Dennis Hong,Yusuke Tanaka,Sehoon Ha*

Main category: cs.RO

TL;DR: Gaussian Evolutionary Specialists (GES) decouples design-space partitioning from policy learning by assigning specialist policies to evolving Gaussian regions, iteratively refining them via training and probing to avoid behavioral diversity collapse. This framework replaces costly re-training with direct evaluation in design sampling loops, achieving 5-25% performance gains and 3x obstacle climbing on hardware while reducing optimization time by 37%.


<details>
  <summary>Details</summary>
Motivation: Designing high-performance legged robots requires joint morphology-control optimization. Model-free RL enables robust controllers without explicit dynamics, but expensive repeated policy training in co-design loops is impractical. Universal policies suffer from behavioral diversity collapse, while end-to-end Mixture-of-Experts (MoE) fail due to representation collapse.

Method: GES framework decouples design-space partitioning from policy learning. It assigns specialist policies to evolving Gaussian regions and iteratively refines them via training, probing, and territory expansion. Specialists are integrated into a design sampling loop, replacing costly re-training with direct evaluation.

Result: GES discovers designs with 5-25% higher performance than naive universal policies on BALLU. On hardware, GES optimized design overcomes a 24 cm obstacle (3x improvement over baseline). Optimization time is reduced by 37%.

Conclusion: GES effectively captures diverse behaviors through decoupled design-space partitioning and specialist policies, enabling efficient morphology-control co-optimization that achieves significant performance gains and faster optimization compared to existing approaches.
...
### [54] [Re-imagining ISO 26262 in the Age of Autonomous Vehicles: Enhancing Controllability through Transferability and Predictability](https://arxiv.org/abs/2606.07437)
*Chaitanya Shinde,Hadi Hajieghrary,Paul Schmitt,Adam Shoemaker,Bodo Seifert,Steve Kenner*

Main category: cs.RO

TL;DR: This paper extends ISO 26262's Controllability assessment for autonomous vehicles by introducing Transferability and Predictability as measurable sub-concepts, enabling falsifiable fallback and interaction claims for SAE Levels 4-5 driverless systems.


<details>
  <summary>Details</summary>
Motivation: ISO 26262's risk assessment framework (Severity, Exposure, Controllability) was designed for human-driven vehicles. With the rise of autonomous vehicles lacking human drivers, this framework requires adaptation to assess safety in driverless systems.

Method: The paper decomposes Controllability into two measurable dimensions: Transferability (ability to hand off control to fallback mechanisms) and Predictability (ease of external agents anticipating AV behavior). Predictability is formally defined using human-robot interaction principles with a mathematical framework. A "designed-versus-achievable gap" metric distinguishes architectural fallback claims from scene-conditioned achievable fallback capability.

Result: The proposed metrics align with ISO 26262 and ISO/PAS 21448 (SOTIF), making fallback and interaction claims falsifiable and traceable across Operational Design Domain (ODD) slices.

Conclusion: These dimensions complement rather than replace existing standards, preserving ISO 26262's structure while extending its applicability to SAE Levels 4 and 5 autonomous vehicles.
...
### [55] [Planning-aligned Token Compression for Long-Context Autonomous Driving](https://arxiv.org/abs/2606.07464)
...
### [56] [Affordance-Based Hierarchical Reinforcement Learning for Quadruped Pedipulation](https://arxiv.org/abs/2606.07506)
*Tuba Girgin,Jose Castelblanco,Gabriel Rodriguez,Emre Girgin,Cagri Kilic*

Main category: cs.RO

TL;DR: A three-level hierarchical reinforcement learning framework for quadruped robots that uses pose affordances for navigation and interaction-point affordances for object manipulation, enabling autonomous object interaction without pre-designed trajectories.


<details>
  <summary>Details</summary>
Motivation: Quadruped robot manipulation is challenging because existing approaches focus on low-level policy learning while task execution still requires expert-designed high-level trajectories. The study aims to remove this reliance on pre-designed trajectories by enabling autonomous selection of affordable interaction points and robot base poses.

Method: A three-level hierarchical reinforcement learning framework that: 1) uses pose affordances to guide navigation policy, 2) has navigation policy drive locomotion policy, and 3) uses interaction-point affordances to guide pedipulation policy for object-centric pose alignment. The framework is trained in the IsaacSim ecosystem.

Result: The framework successfully identifies candidate poses based on affordance values and executes object manipulation tasks in real-world settings without human guidance. The researchers created an object-interaction dataset through various object interaction tasks and validated the approach both in simulation and real-world environments.

Conclusion: Pose affordances are effective for guiding both navigation and manipulation policies in hierarchical RL frameworks, enabling quadruped robots to autonomously perform object manipulation tasks without pre-designed trajectories or human guidance.
