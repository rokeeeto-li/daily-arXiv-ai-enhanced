...
TL;DR: BayesBeliefAgent uses a hierarchical LLM planner paired with Bayesian tracking to dynamically adapt to teammates whose strategies shift mid-task, interrupting skills only when partner actions contradict inferred skills, reducing replans by an order of magnitude while narrowing the belief-action gap.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems struggle to collaborate with new teammates whose strategies shift mid-task. Because agents execute multi-step or temporally extended skills, they frequently continue executing outdated plans long after public evidence shows that a partner has changed its skill. Existing methods either treat partner tracking as passive context (aware of the shift but slow to act) or replan indiscriminately.

Method: BayesBeliefAgent pairs a hierarchical LLM planner with a Bayesian tracking module. Rather than replanning constantly, the agent interrupts its current skill only when a partner's actions directly contradict the inferred skill.

Result: Across benchmark Overcooked environments, contradiction-conditioned control drastically narrows the belief-action gap (the fraction of decisions where a correct partner estimate leads to a non-complementary skill) while requiring an order of magnitude fewer replans than heuristic methods.

Conclusion: The approach balances belief accuracy with action alignment, achieving efficient collaboration with dynamic partners by interrupting skills only when necessary rather than replanning indiscriminately.
...
### [5] [Trust as a Field: A Macroscopic Representation for Vehicular Networks](https://arxiv.org/abs/2608.18178)
...
### [6] [Revisiting the "Push-T" Robot Manipulation Task with Agentic Robotics](https://arxiv.org/abs/2608.18227)
...
### [7] [GigaBrain-WBC-0.5: A Behavior World Model for Robust Whole-Body Control with Environment Interaction](https://arxiv.org/abs/2608.18234)
...
TL;DR: GigaBrain-WBC-0.5 introduces a Behavior World Model for humanoid whole-body control that jointly predicts actions, states, and latent behavior distributions, enabling terrain interaction, handling implausible commands, and robust fall recovery.


<details>
  <summary>Details</summary>
Motivation: Existing whole-body motion tracking policies trained on flat ground fail to handle varied terrain and environment-dependent behaviors. They attempt to handle all commands by expanding reference-motion corpora, which doesn't scale when feasible behaviors become environment-dependent.

Method: Trains a causal Transformer to jointly predict next action, next state, and distribution over next latent behavior command, modeling how the environment shapes future capabilities. Uses an automatic terrain-annotation pipeline to recover full 3D contact geometry from retargeted motion at dataset scale. The predicted distribution is reused at deployment to detect and retract implausible commands onto learned behaviors.

Result: Achieves highest success rates among three large-scale tracker baselines: 81.3% on terrain interaction (4.3x strongest baseline), 83.1% under implausible commands, and 99.3% fall recovery (16.8x strongest baseline). Hardware trials demonstrate robust interaction under missing supports and disturbances; transfers to Unitree G1 checkpoint with simple fine-tuning.

Conclusion: A unified policy that takes real-time commands, interacts with the environment, and remains robust to implausible commands, falls, and disturbances, addressing the core limitation of existing trackers that fail on complex terrains and environment-dependent behaviors.
...
### [8] [GAPL: Grounded Action-effect Policy Learning for LLM-Based Trajectory Planning](https://arxiv.org/abs/2608.18254)
*Zhihong Cui,Hengyu Liu,Zhangkai Wu,Yushuai Li,Tianyi Li,Peiyuan Guan,Amir Taherkordi,Tor Skeie*

Main category: cs.RO

TL;DR: GAPL is a unified framework for autonomous driving trajectory planning that integrates LLM-based effect estimation, simulation-based effect grounding, and policy optimization into a closed-loop system to address LLM limitations of hallucinated reasoning and poor numerical precision.


<details>
  <summary>Details</summary>
Motivation: LLMs offer semantic-rich planning capabilities but are limited by hallucinated reasoning, poor grounding in environment dynamics, and limited numerical precision in control, making them unsuitable for precise autonomous driving trajectory planning.

Method: GAPL consists of three modules: (1) LLM-based Effect Evaluator for structured multi-dimensional action-effect estimation, (2) Simulation-based Effect Grounder that predicts dynamics-consistent effects from simulator rollouts, and (3) Effect-Aware Decision Maker that grounds LLM effect estimates against simulation via a distiller to guide Proximal Policy Optimization (PPO)-based policy learning.

Result: On four highway scenarios, GAPL consistently outperforms baselines with average reductions of 0.76 in collision rate, 0.86 in average displacement error (ADE), and 2.00 in final displacement error (FDE), along with an average reward gain of 1.44.

Conclusion: By integrating effect estimation, grounding, and policy optimization into a closed-loop system, GAPL effectively addresses LLM limitations in autonomous driving, achieving significant performance improvements over baseline methods.
...
### [9] [VERAGMIL: Virtual Environment for Scooping Granular Foods with Imitation Learning Models](https://arxiv.org/abs/2608.18258)
*Amanuel Ergogo,Diego Dall'Alba,Przemyslaw Korzeniowski*

Main category: cs.RO

TL;DR: VERAGMIL is a VR-based simulation framework for training Robot-Assisted Feeding systems to handle granular food items like rice and beans using imitation learning from human demonstrations.


<details>
  <summary>Details</summary>
Motivation: Manipulating granular food items presents significant physical challenges for Robot-Assisted Feeding systems. While learning from human demonstrations is promising, acquiring high-quality demonstrations is complex. The paper aims to address this challenge by creating a framework that combines high-fidelity simulation with intuitive VR interfaces for demonstration collection.

Method: VERAGMIL integrates a high-fidelity simulator with a Virtual Reality interface to enable recording demonstrations and supporting different imitation learning methods. The framework includes realistic environments with robots, sensors, and various food items with distinct physical characteristics. Three imitation learning models (Behavior Cloning, BC-RNN, and BCQ) were trained on granular scooping and transporting tasks using both VR interface and 3D space mouse demonstrations.

Result: VR-based demonstrations significantly outperformed 3D space mouse data across all metrics. BCQ achieved the best overall performance, particularly in reducing spillage and approaching human expert performance. The models were evaluated on success rate, spillage, generalization to unseen food items, and task completion time.

Conclusion: VERAGMIL demonstrates the effectiveness of combining VR-based demonstrations with imitation learning for training Robot-Assisted Feeding systems to handle granular materials. The framework successfully enables systems to approach human performance levels, particularly in spillage reduction, and the code is publicly available for further research.
...
### [10] [Transferable Tool-Tissue Contact Detection from Stereo Depth in Robot-Assisted Surgery](https://arxiv.org/abs/2608.18270)
...
### [11] [GuideFetch: A Task Coordination Framework for Concurrent Navigation and Object Retrieval in Assistive Robot Dogs](https://arxiv.org/abs/2608.18292)
...
### [12] [A Task-Agnostic Control Strategy for Dynamic Assistance with Pneumatically Actuated Soft Exosuits](https://arxiv.org/abs/2608.18364)
...
### [13] [The Embodiment Gap in Robot Foundation Models](https://arxiv.org/abs/2608.18433)
...
### [14] [HarvestPoint-ACT: Explicit Target Selection and Harvest-Point Conditioning for Robotic Fruit Harvesting under Occlusion](https://arxiv.org/abs/2608.18446)
...
### [15] [Backward Layout Search for Sequence-Constrained Robotic Assembly](https://arxiv.org/abs/2608.18454)
*Xi Zhang,Jiancong Dai,Hao Chen,Zhengtao Hu,Changcai Yang,Weiwei Wan*

Main category: cs.RO

TL;DR: The paper proposes Backward Layout Search (BLS), a reverse-order pose assignment algorithm for robotic assembly layout planning that reduces search complexity by focusing on feasible assembly steps.


<details>
  <summary>Details</summary>
Motivation: Robotic assembly layout planning is challenging because the obstacle environment changes after each assembly step, and unassembled parts remaining in the workspace may block robot motions.

Method: Backward Layout Search (BLS) assigns initial part poses in reverse assembly order, using geometric, kinematic, grasp, and prescribed-motion checks, collision masks, candidate set filtering, and beam selection to maintain promising partial layouts. Complete layouts are validated by forward motion planning.

Result: Experiments on five assembly models show BLS produces collision-free executable layouts and reduces step evaluations and search time compared to a matched forward search.

Conclusion: BLS effectively addresses the challenge of robotic assembly layout planning by exploiting the dependency of assembly step feasibility on initial poses, achieving better performance than forward search approaches.
...
### [16] [DevGRU: Depth-guided Visual Navigation using a Collision-aware Recurrent Model](https://arxiv.org/abs/2608.18470)
...
### [17] [An Experimental Study of Downwash Effects on a Continuum Manipulator Integrated with a Multirotor UAV](https://arxiv.org/abs/2608.18507)
...
TL;DR: A real-time control-constrained DDP framework (ABC-DDP) for underactuated legged robots, integrating virtual constraints into a feasibility-driven multiple-shooting framework to enable static two-leg standing, walking, and running via finite-horizon MPC.


<details>
  <summary>Details</summary>
Motivation: Classical DDP struggles with control constraints; the paper aims to develop a real-time control-constrained DDP that can handle underactuated legged robots with stable optimization from dynamically infeasible initializations.

Method: Accelerated Projected Gradient (APG)-based control-constrained DDP (ABC-DDP) that computes constrained solutions and identifies active sets without repeated KKT inversions, combined with a virtual constraint integrated into a feasibility-driven multiple-shooting framework.

Result: Successfully demonstrated static two-leg standing, slow catwalk, upright walking, and high-speed running in simulation under strong underactuation, achieving the first real-time finite-horizon MPC demonstration of static two-leg standing for quadruped robots.

Conclusion: The proposed ABC-DDP framework effectively addresses control constraints in DDP for underactuated legged robots, enabling diverse dynamic behaviors including the challenging static two-leg standing from dynamically infeasible initializations.
...
### [19] [LabDex: A Hierarchical Benchmark for Dexterous Manipulation in Laboratories](https://arxiv.org/abs/2608.18618)
...
### [20] [Evaluation of Monocular SLAM Systems on High-Altitude Nadir UAV Footage](https://arxiv.org/abs/2608.18632)
*Gašper Spagnolo,Matej Dobrevski,Danijel Skočaj*

Main category: cs.RO

TL;DR: This paper benchmarks five monocular SLAM systems on aerial nadir video across local UAV flights, synthetic imagery, and long-range sequences, finding that current methods are insufficient for reliable visual-only aerial navigation due to poor vertical position accuracy and trajectory distortion despite loop-closure capability.


<details>
  <summary>Details</summary>
Motivation: Aerial nadir video presents unique challenges for monocular SLAM due to weak geometric constraints combined with severe perceptual aliasing, making it a difficult regime that requires systematic evaluation of existing methods.

Method: The authors benchmark five monocular SLAM systems on three test scenarios: local UAV flights, synthetic city-scale imagery, and long-range aerial sequences, with no inertial or GNSS aiding to isolate visual performance.

Result: Performance varies strongly with environment and trajectory scale: MASt3R-SLAM achieves the lowest mean horizontal MAE on DJI flights (0.53% of reference path length), while DROID-SLAM performs best overall (2.88% average). No system consistently preserves global trajectory shape on long sequences, and vertical position remains poor with large-area trajectories remaining highly distorted despite loop-closure capability.

Conclusion: Current monocular SLAM methods are by themselves insufficient for reliable visual-only aerial navigation, indicating a need for improved approaches or additional aiding systems.
...
### [21] [Progressive Experience Fusion for Multi-Task World Model Control in Endovascular Navigation](https://arxiv.org/abs/2608.18647)
...
### [22] [Orienteering Problem with Uncertain Time-Varying Rewards: Framework and Benchmark for Everyday Service Robotics](https://arxiv.org/abs/2608.18672)
...
### [23] [SoftVTBench: A Deformation-Aware Visuo-Tactile Dataset and Benchmark for Deformable-Object Manipulation](https://arxiv.org/abs/2608.18701)
...
### [24] [Dream2Reward: Transition-Alignment Reward Models from Positive Demonstrations for Robotic Manipulation](https://arxiv.org/abs/2608.18787)
...
### [25] [Beyond Placement and Articulation: Usage-Driven Code Scenes for Embodied Interaction](https://arxiv.org/abs/2608.18840)
*Zijian Xiao,Zipeng Ye,Jinkun Hao,Xiong Yang,Yuchen Xie,Ran Yi*

Main category: cs.RO

TL;DR: RoomWright is a code-based 3D scene generation framework for embodied AI that focuses on usage-driven object reasoning and multi-part interaction through trigger-condition-effect rules, producing executable and editable simulation-ready environments.


<details>
  <summary>Details</summary>
Motivation: Recent code-based scene generation methods focus on visual construction and object-level articulation but leave functional usage of scenes unmodeled. Indoor scene synthesis is essential for embodied AI, robotic manipulation, and simulation-based policy learning.

Method: RoomWright is an agentic usage-driven framework that represents 3D scenes entirely as code. Key components include: (1) usage-driven object reasoning where each anchor is treated as a task centre with task-required objects and affordances; (2) a code agent that compiles interactions into trigger-condition-effect rules updating structured object states; (3) annotation-informed usage-guided orientation to handle manipuland orientation ambiguity.

Result: Extensive experiments demonstrate the method's effectiveness. The resulting scenes are executable, editable, and simulation-ready, providing interactive environments for embodied AI and policy learning.

Conclusion: RoomWright addresses the gap in functional usage modeling in scene generation, providing executable and editable 3D scenes with multi-part interaction capabilities for embodied AI applications.
...
### [26] [RoboEdit: Turning Human Manipulation Videos into Scalable Robot Experience](https://arxiv.org/abs/2608.18948)
...
### [27] [Autonomous Agricultural Tractor: Integrated Weed Detection and LiDAR Navigation for Precision Paddy Farming](https://arxiv.org/abs/2608.19004)
...
### [28] [LT-Mem: Volatility-Aware Spatio-Temporal Memory for Lifelong Scene Understanding](https://arxiv.org/abs/2608.19059)
*Yumin Lee,Hyoseok Ju,Giseop Kim*

Main category: cs.RO

TL;DR: LT-Mem is a volatility-aware memory evolution framework for long-term robot operation in evolving environments, addressing temporal amnesia in object-level understanding by unifying spatially aligned instance-level 3D perception with volatility-conditioned temporal reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing systems either overwrite history to maintain up-to-date maps or store semantic snapshots without consistent cross-session object identity, resulting in temporal amnesia—the systematic loss of object history that prevents answering queries about object trajectories across sessions.

Method: The framework consists of: (1) a multi-session SLAM backbone providing spatially aligned per-object observations across sessions, (2) a reasoning layer with deterministic evidence scoring for cross-session identity preservation and a volatility-aware policy selecting among overwrite, hold, and multi-hypothesis actions, and (3) a Tri-Memory structure (Live, Delta, Meta) that preserves both current states and event histories.

Result: LT-Mem consistently outperforms baselines across all metrics while consuming an order of magnitude fewer tokens. Ablations confirm that gains are driven by the structured memory architecture rather than LLM capacity.

Conclusion: The paper introduces LT-VQA, a dataset and evaluation suite with multi-session recordings, persistent identity annotations, and temporal QA pairs, demonstrating that LT-Mem effectively addresses temporal amnesia in object-level understanding for long-term robot operation.
...
### [29] [DA-WAM: Decision-Aligned Future Latents for Driving World Models](https://arxiv.org/abs/2608.19085)
...
### [30] [ADEPT: Accelerating Dexterity via Pre-Training and Post-Training using Reinforcement Learning](https://arxiv.org/abs/2608.19182)
...
TL;DR: ADEPT is a large-scale RL framework for sim-to-real transferable dexterity that pre-trains a dexterous policy on generic object reposing, then uses stable post-training (behavior cloning, critic warm-up, conservative updates) to enable multi-fingered robots to solve long-horizon tasks from raw visuo-tactile perception at human-level speed.


<details>
  <summary>Details</summary>
Motivation: Learning dexterity across high DoF robot embodiments is challenging; discovering behaviors from scratch for each task is inefficient; naive RL fine-tuning after pre-training rapidly degrades the pretrained capability; need to safely exploit full kinematic dexterity.

Method: Pre-trains policy on generic object reposing task; post-trains using pretrained behavior as prior; combines behavior-cloning distillation, critic warm-up, and conservative on-policy updates; introduces joint-space Geometric Fabric to mediate policy-robot interaction; distills teachers into perceptive students.

Result: Zero-shots the reposing phase of downstream tasks; achieves sim-to-real transfer on 23 DoF Kuka-Allegro and 29 DoF Flexiv-Sharpa with two RGB cameras and five vision-based tactile sensors; solves long-horizon tasks from challenging initial states at human-level speed.

Conclusion: ADEPT enables learning behaviors difficult to discover from scratch, avoids relearning same skills for each task, and provides a stable post-training recipe that preserves pretrained capabilities while enabling robust sim-to-real transfer.
...
### [31] [PartialBiGrasp: Inferring Hidden Local Geometry for Bimanual Grasping from Partial Views](https://arxiv.org/abs/2608.19188)
*Ayush Kaura,Vignesh Vembar,Md Faizal Karim,Keshab Patra,K Madhava Krishna*

Main category: cs.RO

TL;DR: PartialBiGrasp is a dual-arm robotic grasping framework that generates force-closure compliant grasp pairs directly from noisy partial point clouds without requiring full object geometry.


<details>
  <summary>Details</summary>
Motivation: Single-arm manipulation cannot reliably handle large, heavy, or complex objects with sparse graspable regions. Prior bimanual methods require full point clouds, which may not be accessible in real scenarios where only partial observations are available.

Method: The framework employs convolutional occupancy networks to learn implicit geometric features from partial point clouds, enabling local reasoning about graspability, collision-free contact regions, and object thickness. It generates initial grasp pairs and refines them using sampling-based optimization to correct ambiguity from incomplete geometry.

Result: The approach demonstrates robust and physically stable dual-arm grasp generation on noisy partial point clouds of novel objects through analytical force-closure metrics, large-scale simulation experiments, and real-world robot evaluations.

Conclusion: PartialBiGrasp enables reliable dual-arm grasping in real-world scenarios where full object geometry is not available, overcoming limitations of prior methods that require complete point clouds.
