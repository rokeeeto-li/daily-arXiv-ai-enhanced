...
TL;DR: AstraTag is a recursive fiducial marker designed for autonomous on-orbit robotic operations, using a square Spidron pattern with multi-scale detection capability and GRS encoding, achieving higher detection rates than existing markers on curved spacecraft surfaces.


<details>
  <summary>Details</summary>
Motivation: Space robotic operations face challenges from harsh environments and high failure costs. Existing fiducial markers are mostly single-scale and terrestrial-designed, leaving the camera's field of view at close range during critical proximity and docking phases.

Method: Marker template based on square Spidron pattern with recursive self-similar structure enabling multi-scale detection. Uses 48-bit signature from triangular sub-regions encoded with Generalized Reed-Solomon (GRS) code. Detection pipeline includes contour-based quadrilateral localisation, perspective normalisation, signature matching against a pre-computed dictionary. Includes Thin-Plate Spline (TPS) re-warp fallback for curved surfaces using internal rectangular borders as geometric correspondences.

Result: Benchmarked against Fractal ArUco and AprilTag on spacecraft mockups with flat and curved surfaces. On curved surfaces, AstraTag achieves higher detection rate than both baselines.

Conclusion: AstraTag provides a robust recursive-marker option for space robotics, particularly effective on curved surfaces where existing markers struggle.
...
TL;DR: SceneBot is a unified humanoid motion-tracking framework that combines free-space locomotion with contact-rich tasks using a single policy conditioned on reference motions and contact labels, trained via hindsight scene reconstruction to infer interaction graphs from human motion data.


<details>
  <summary>Details</summary>
Motivation: Pure kinematic tracking fails to resolve physical ambiguities in contact-rich tasks, and there is a lack of annotated interaction data for training humanoid RL policies to handle complex interactions with objects and uneven terrain.

Method: SceneBot conditions a single policy on both reference motions and per-link contact labels to explicitly define expected environmental interactions. It uses a hindsight scene reconstruction approach to infer scene-interaction graphs from retargeted human motion, trained on 7.5 hours of reconstructed contact-rich data to enable unified handling of free-space locomotion, terrain traversal, and whole-body manipulation.

Result: SceneBot successfully generalizes to unseen motions and environments, executing complex long-horizon tasks such as carrying a box upstairs, demonstrating that contact conditioning is a powerful interface for humanoid control.

Conclusion: SceneBot represents the first general framework to seamlessly unify free-space and contact-rich behaviors in humanoid robots, establishing contact conditioning as an effective approach for handling complex physical interactions.
...
TL;DR: A hybrid simulation framework combining analytical quadrotor models with physics solvers enables deep reinforcement learning policies for agile, accurate payload throws, outperforming model-based baselines by 50% in landing error reduction and 30% in throw duration reduction.


<details>
  <summary>Details</summary>
Motivation: While suspended-payload transport is well studied, dynamic targeted payload release remains underexplored. State-of-the-art model-based approaches suffer from conservative constraints, tracking errors, and difficulties in analytically modeling flexible rope dynamics, limiting their performance in time-critical applications like search-and-rescue and medical delivery.

Method: The researchers developed a hybrid simulation framework that couples a high-fidelity analytical quadrotor model with a physics solver for rope-payload interactions, exchanging forces between domains at each step. They used this environment to train a deep reinforcement learning policy for payload throws, and conducted ablation studies to validate the approach. They also demonstrated training with visual observations instead of explicit state estimates.

Result: The RL policy achieved significant improvements over model-based baselines: reducing landing error by up to 50% and throw duration by up to 30%. Ablation studies confirmed the coupled simulation was essential for these gains, and visual policies achieved comparable accuracy to state-based policies.

Conclusion: The proposed hybrid simulation framework and RL approach successfully overcome limitations of model-based trajectory optimization for dynamic aerial manipulation. The open-source simulator accelerates future research in this domain, with the method pushing the boundaries of the agility-accuracy trade-off in quadrotor payload delivery.
...
TL;DR: Composite Whole-Body Imitation (CWI) is a framework that decouples upper-body manipulation from lower-body locomotion using MoCap data for manipulation and AMP (Adversarial Motion Prior) for locomotion, enabling robust humanoid robot control with competitive loco-manipulation performance.


<details>
  <summary>Details</summary>
Motivation: Existing whole-body controllers face two major challenges: (1) command-sampling methods struggle with sparse rewards and require carefully tuned curricula, while upper-body control produces motions that deviate from human-like statistics; (2) MoCap data imitation suffers from dataset imbalance where locomotion trajectories are overly aggressive for stable-locomotion scenarios, requiring extensive filtering and augmentation.

Method: CWI decouples MoCap data usage: (1) full MoCap dataset for diverse manipulation references, (2) dual discriminators trained on curated expert-quality walking and squatting clips via Adversarial Motion Prior (AMP) for stable command-conditioned locomotion. The framework uses a multi-critic architecture to reduce conflicts among locomotion, manipulation, and motion-style objectives, followed by teacher-student distillation to produce a whole-body policy conditioned only on bimanual hand poses and velocity/height commands.

Result: CWI achieves competitive loco-manipulation performance, robust whole-body coordination, and practical teleoperation without full-body motion-capture equipment. The method was evaluated through simulation experiments and real-world deployment on a full-size LimX Oli humanoid robot.

Conclusion: By decoupling upper-body manipulation from lower-body locomotion and leveraging AMP for stable locomotion, CWI successfully addresses the challenges of dataset imbalance and sparse rewards, demonstrating that effective humanoid robot control can be achieved without extensive data filtering or full-body motion-capture equipment.
...
TL;DR: This study analyzes architectural redundancy in Vision-Language-Action (VLA) models through transformer block removal experiments. The authors find that language backbones are highly redundant for robotic manipulation tasks while vision and action pathways are less tolerant to removal, suggesting that current VLA benchmarks may not sufficiently pressure deep language grounding.


<details>
  <summary>Details</summary>
Motivation: VLA models inherit oversized language backbones from pretrained vision-language models that exceed the capacity needed for short robotic instructions. The authors investigate how much of a VLA model is actually necessary for closed-loop control, addressing the question of architectural redundancy.

Method: The authors introduce Drop-Then-Recovery (DTR), an analysis protocol that removes selected transformer blocks from pretrained VLA models and fine-tunes to measure whether removed capacity was necessary. To enable reliable intervention, they propose GateProbe, a one-shot virtual-gate sensitivity metric that ranks blocks by their contribution to downstream action loss. This method is applied across multiple VLA architectures, manipulation benchmarks, and real-robot industrial scenarios.

Result: The study reveals a strong asymmetry in post-removal recoverability: language backbones are highly redundant for standard robotic manipulation tasks, whereas vision and action pathways are substantially less tolerant to removal. On the LIBERO benchmark, removing half of the LLM blocks improved OpenVLA-OFT from 95.0% to 98.3% under the same downstream fine-tuning budget, and retaining only two language blocks still recovered baseline-level performance.

Conclusion: Current VLA benchmarks may exert limited pressure on deep language grounding and compositional instruction understanding. Future VLA architectures should allocate capacity more deliberately across language, vision, and action components to improve efficiency and reduce redundancy.
...
TL;DR: SpikeVLA is an energy-efficient spiking Vision-Language-Action architecture that replaces transformer-based approaches with event-driven spiking neural networks, reducing computational and energy costs while maintaining performance for embodied navigation and robotic control.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language-Action (VLA) models rely on large-scale transformers, which suffer from high inference latency and energy consumption that limits practical deployment in low-power, real-time scenarios.

Method: SpikeVLA consists of three components: (1) Spike-V, a spiking vision encoder with event-driven spiking layers; (2) Spike-L, a multi-modal spiking large language model with spiking dynamics and token-level event-driven sparsity; (3) Spike-A, a spiking action policy network using Laplacian-kernel population coding with a multi-layer fully connected SNN.

Result: Experiments on navigation and robotic control tasks demonstrate that SpikeVLA significantly reduces energy consumption and computational cost while maintaining competitive performance.

Conclusion: SpikeVLA shows strong potential for enabling low-power, real-time embodied intelligence applications.
...
TL;DR: A data-centric pipeline integrating motion data curation, real-to-sim adaptation, AMP-based reinforcement learning, and sim-to-real deployment for humanoid robot motion learning, validated on Booster T1 with preliminary cross-platform testing on K1.


<details>
  <summary>Details</summary>
Motivation: Humanoid robot motion learning requires both task-oriented control policies and physically feasible, natural behaviors, but robot-feasible motion data is scarce due to incompatibility between raw human demonstrations and robot morphology, variable quality of open-source clips, and the need for feasibility checking in simulation-collected trajectories.

Method: The proposed framework comprises four integrated components: (1) motion data curation, (2) real-to-sim model adaptation, (3) AMP-based reinforcement learning, and (4) sim-to-real deployment.

Result: The framework is validated on the Booster T1 robot and further provides preliminary cross-platform validation on the Booster K1.

Conclusion: The data-centric training and deployment pipeline successfully addresses the challenge of scarce robot-feasible motion data and enables effective motion learning for humanoid robots with physically feasible and natural behaviors.
...
TL;DR: PPO-EAL integrates exact augmented Lagrangian optimization with proximal policy optimization to enable safe robotic control with precise constraint satisfaction, demonstrated across multiple benchmarks including sim-to-real deployment.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning is promising for robotic control but typically ignores safety requirements. Current safe RL algorithms struggle to learn policies efficiently while satisfying explicit physical constraints precisely.

Method: PPO-EAL combines clipped policy updates with exact quadratic penalty terms and momentum-regulated multiplier updates to integrate exact augmented Lagrangian optimization into proximal policy optimization, providing theoretically grounded constraint enforcement.

Result: Achieves superior safety precision and reward performance compared to state-of-the-art first-order safe RL baselines across diverse benchmarks (cart-pole, cart-double-pendulum, Franka reaching, quadrupedal locomotion). Demonstrates zero-shot sim-to-real deployment in gear assembly with improved success, reduced peak contact force, and enhanced operational robustness.

Conclusion: PPO-EAL establishes a general and practically deployable safe RL framework suitable for diverse safety-critical robotic systems.
...
TL;DR: A distillation strategy that transfers spatial-semantic reasoning from large frontier models to a lightweight 4B-parameter local VLM for edge execution, combined with token generation regularization and quantization to achieve 82.8% latency reduction without performance degradation.


<details>
  <summary>Details</summary>
Motivation: VLMs enable powerful environmental perception with language context for robotic tasks like ObjectNav, but their computational footprint typically confines them to cloud execution, hindering low-latency inference with local deployment on resource-constrained robots.

Method: 1) Established a SotA Scene Graph (SG)-based pipeline using Claude Sonnet 4.6; 2) Fine-tuned Qwen3.5-4B on just 500 frontier reasoning traces; 3) Introduced E-RLVR with Token Generation (TG) regularization to compress output sequence lengths; 4) Combined with quantization.

Result: SG-based pipeline achieved 39.7% SR on HM3D OVON benchmark; fine-tuned Qwen3.5-4B achieved 34.5% SR (narrowing gap to large cloud models); TG overhead reduced by 72.1% and latency by 71.8%; combined with quantization, achieved 82.8% overall inference latency reduction without significant performance sacrifice.

Conclusion: This joint strategy presents a viable paradigm for local, low-latency VLM execution on mobile robots, enabling edge deployment while maintaining performance on resource-constrained embedded GPU devices like Jetson Orin.
...
TL;DR: S²-VLA is a compact 2B parameter framework with State-Space Guided Adaptive Attention (SSGAA) that dynamically fuses visual, language, and action representations based on task progression to improve long-horizon robotic manipulation performance.


<details>
  <summary>Details</summary>
Motivation: VLA models degrade in long-horizon tasks due to static feature fusion mechanisms using fixed weights, preventing adaptation to different task execution phases.

Method: Introduces State-Space Guided Adaptive Attention (SSGAA) mechanism that maintains a belief state tracking task progression and generates dynamic gating weights to adaptively fuse visual features (spatial perception), task intents (high-level planning), and temporal action sequences (execution consistency).

Result: Despite being only 2B parameters, S²-VLA consistently outperforms larger 7B-scale models and achieves state-of-the-art performance on LIBERO and SimplerEnv long-horizon manipulation benchmarks.

Conclusion: Adaptive feature fusion is critical for long-horizon robotic manipulation, demonstrating that compact models with dynamic fusion mechanisms can outperform larger models with static fusion.
...
TL;DR: This paper investigates motion-based visual communication for stealth-constrained swarm robotics, using modular planar trajectories as visual cues and a custom 3DTrajDecoder for pose estimation and trajectory classification.


<details>
  <summary>Details</summary>
Motivation: In stealth-constrained swarm robotics, visual communication provides a critical alternative to active radio transmissions, which might be jammed, enabling non-active information exchange.

Method: The research utilizes modular, dynamically feasible planar trajectories as visual cues. The receiver drone employs a pose estimator to track the transmitting drone's pose, which is fed into a custom 3DTrajDecoder that classifies and segments spatiotemporal sequences while regressing size and normal vector. A configurable online procedural generation pipeline was developed to train the decoder on both communicative and non-communicative trajectories.

Result: The system was validated through real-world testing and simulation to define its operating domain, supported by an extensive ablation study detailing architectural choices and system limitations.

Conclusion: The paper presents a robust motion-based communication system for stealth-constrained swarm robotics that enables non-active information exchange through visual cues, with demonstrated validation through real-world testing and simulation.
...
TL;DR: This paper presents a control framework for autonomous drifting on production vehicles by compensating for actuator delays and mechanical constraints, enabling a production sports car to sustain drifts with lateral error ≤1.1 m despite 250+ ms delays.


<details>
  <summary>Details</summary>
Motivation: Prior autonomous drifting demonstrations were limited to research platforms with instantaneous torque delivery and independently actuated wheels, making their applicability to production vehicles with actuator latencies and mechanically coupled axles uncertain.

Method: The authors developed a predictor to compensate for powertrain delays, a revised control formulation accommodating higher actuation latencies and differential coupling on the driven axle, and brake-based velocity stabilization.

Result: The controller enables a production sports car with a combustion engine to robustly sustain circular and figure-eight drifts, limiting lateral error to 1.1 m and sideslip overshoot to 0.06 rad despite actuator delays exceeding 250 ms, while mitigating oscillations and maintaining stable path and sideslip tracking.
...
TL;DR: This paper introduces Embodied Collective Intelligence (ECI), a multi-robot paradigm where robot teams share context, task progress, and skill experience as collective resources through co-perception, co-action, and co-evolution components.


<details>
  <summary>Details</summary>
Motivation: As embodied AI evolves from simple perception-control pipelines to agentic closed-loop systems and robotics shifts from single-robot to multi-robot autonomy, there's a growing need to move beyond sharing static resources (maps, tasks, datasets) toward sharing dynamic states produced by embodied agent loops. Multi-agent collaboration requires robot teams to accumulate and utilize world context, task progress, and skill experience as shared resources.

Method: The authors review the evolution of embodied AI toward agentic systems and multi-robot cooperation. They present Embodied Collective Intelligence (ECI) as a conceptual framework comprising three core components: Co-Perception, Co-Action, and Co-Evolution. A navigation study is conducted to examine one concrete aspect—shared world-memory inheritance—to demonstrate how newly added robots benefit from merged team memory.

Result: The study demonstrates that newly added robots can benefit from merged team memory, validating the concept of shared world-memory inheritance as a component of ECI. However, the authors clarify this is not a full evaluation of the entire ECI framework but serves as an illustrative case study grounding one measurable aspect of the concept.

Conclusion: Embodied Collective Intelligence is proposed as a promising direction for embodied multi-agent intelligence. The framework motivates future research by establishing a conceptual foundation, while the navigation case study provides empirical grounding for one component. This work bridges the gap between agentic embodied AI and multi-robot collaboration by proposing that robot teams should share and accumulate dynamic states rather than just static resources.
...
TL;DR: A cloud-native simulation infrastructure framework for embodied intelligence that unifies simulation environment generation, task execution, trajectory collection, model evaluation, data management, and cloud services into a scalable and reproducible platform for large-scale training and standardized evaluation.
...
Method: Adopts cloud-native technologies including elastic resource scheduling, containerized simulation, unified data management, and service-oriented system design. Built on a four-layer architecture providing standardized environment assets, automated task generation, trajectory collection, benchmark evaluation, and closed-loop data optimization.

Result: The framework successfully integrates with representative systems including D-VLA, RL-VLA3, Sword, and Pre-VLA to support scalable simulation, dynamic scheduling, visual augmentation, and real-time data filtering.

Conclusion: Cloud-native simulation infrastructure provides a unified foundation for data generation, model training, standardized evaluation, and real-world deployment, playing a key role in the future development of embodied intelligence.
...
TL;DR: The paper studies learning novel bi-manual manipulation skills from human actions for a robot with parallel grippers. Instead of using noisy 6DoF human hand poses, they propose a bridging action representation (relative wrist translation within the initial head-camera frame) shared by humans and robots, implemented via a π0-like vision-language-action model with interleaved action tokens and attention masking.


<details>
  <summary>Details</summary>
Motivation: Human action data is cheap, abundant, and diverse, making it a promising resource for scaling robot learning. However, transferring skills from humans to robots is challenging because most prior work treats humans as just another bi-manual 6DoF embodiment, where hand-pose estimates are noisy and human finger contact patterns differ fundamentally from parallel grippers. Learning rotation-inclusive action signals from human data is therefore sub-optimal.

Method: The authors propose a bridging action representation: relative wrist translation within the initial head-camera frame, an action space shared by humans and robots. They build a π0-like vision-language-action model with interleaved action tokens and attention masking to handle potential absence of certain action components in different embodiments.

Result: On a suite of novel bi-manual manipulation tasks, the bridging action representation transfers human manipulation knowledge to robots far more effectively than noisy 6DoF human actions. The method scales with the amount of human data available.

Conclusion: The bridging action representation effectively transfers human manipulation knowledge to robots for bi-manual manipulation tasks, overcoming limitations of using raw 6DoF human hand poses that suffer from noise and embodiment differences.
...
TL;DR: PA-BiCoop is a single-model bimanual cooperation framework that dynamically assigns primary and auxiliary roles to arms based on task stages, using shared encoders and specialized decoders to enable inter-arm knowledge sharing for superior manipulation performance.


<details>
  <summary>Details</summary>
Motivation: Existing bimanual manipulation approaches either lack inter-arm interaction or treat arms as functionally equivalent, ignoring the human-like dynamic division of labor where one arm handles core operations while the other provides auxiliary support.

Method: PA-BiCoop proposes a dynamic primary-auxiliary arm differentiation framework with: 1) Two specialized decoders sharing a global feature encoder, where the primary decoder generates base-coordinate pose and affordance heatmaps while the auxiliary decoder outputs relative pose; 2) A dynamic role assignment module that automatically maps roles to left/right arms without manual pre-definition.

Result: PA-BiCoop outperforms state-of-the-art baselines by 48% on average in RLBench2 simulation tasks and over 50% on average in real-world tasks, demonstrating significant improvements in bimanual manipulation performance.

Conclusion: The proposed PA-BiCoop framework effectively addresses the limitations of existing approaches by enabling dynamic arm role assignment and inter-arm knowledge sharing, achieving superior performance in both simulated and real-world bimanual manipulation tasks.
...
TL;DR: This paper decomposes dexterous manipulation skills into simpler, analyzable components and learns them with constraints and guidance from classical physics and control theory, achieving efficient and stable skill learning across various challenging scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional dexterous manipulation controllers rely on analytic models constrained by strong assumptions about hands and objects, while reinforcement learning (RL) explores skills end-to-end but suffers from inefficiency due to unnoticeable instability and conflicting learning objectives.

Method: The authors decompose dexterous skills into multiple simpler/analyzable components. Each skill component is learned with constraints and guidance from classical physics and control theory, combining the strengths of both approaches.

Result: Skill learning becomes efficient and stable when incorporating prior knowledge from theory across various challenging conditions: stable grasp, in-grasp reposition/reorientation with different objects, sensor/motor noise, latency, and varying frictional conditions.

Conclusion: Leveraging prior knowledge from classical physics and control theory enables more efficient and stable skill learning for dexterous manipulation, bridging the gap between model-based approaches and end-to-end reinforcement learning.
...
TL;DR: Uni-Mo is a fully automated pipeline that generates quadruped robot motion data using LLMs and video diffusion models, eliminating the need for animal data collection. It introduces the Quad-Imaginarium dataset with 7,488 language-annotated motions (18.5 hours) and achieves 96.7% deployment success rate on real Unitree Go2 robots.


<details>
  <summary>Details</summary>
Motivation: Quadruped robots have limited behavioral repertoire despite remarkable locomotion capabilities. Existing approaches that import humanoid motion data require cooperative animals, suffer from fragile reconstruction across species, and face ill-posed retargeting across incompatible morphologies.

Method: Uni-Mo reframes data scarcity as a generation problem: an LLM proposes motion prompts, a video diffusion model synthesizes corresponding robot behaviors, and these are lifted into 3D reference trajectories for training tracking policies on real robots. The pipeline introduces an Identity Consistency Loss to enforce appearance coherence across frames.

Result: Released Quad-Imaginarium dataset containing 7,488 language-annotated quadruped motions spanning acrobatic and performative behaviors (18.5 hours). Validation shows 96.7% deployment success rate on real Unitree Go2 for 392 sampled motions and 97.6% success rate across the full dataset in simulation.

Conclusion: The paper demonstrates that fully automated motion generation for quadrupeds is feasible, overcoming traditional limitations of animal-based data collection. This approach enables more diverse behavioral capabilities for quadruped robots without requiring animal cooperation or species-specific reconstruction.
...
TL;DR: SimFoundry is a modular system for zero-shot real-to-sim scene construction from videos, generating sim-ready digital twins and affordance-preserving variations (digital cousins) that enable zero-shot policy transfer and significant real-world generalization.


<details>
  <summary>Details</summary>
Motivation: Training and evaluating robot policies in the real world is costly and difficult to scale, creating a need for effective simulation-based approaches that can enable broader deployment.

Method: SimFoundry is a modular and automated system that performs zero-shot real-to-sim scene construction from videos, generating sim-ready digital twins and supporting object, scene, and task editing to create affordance-preserving variations (digital cousins) of reconstructed scenes.

Result: Across 7 manipulation tasks and 5 policy architectures, SimFoundry simulation evaluations strongly predict real-world performance (Pearson correlation 0.911, max ranking violation 0.018). Zero-shot real-world evaluation shows task success rate improvements of 17% with object cousins, 21% with scene cousins, and 40% with task cousins.

Conclusion: SimFoundry enables automated generation of diverse digital twins and cousins that facilitate zero-shot policy transfer to challenging real tasks (multi-step manipulation, articulated object interaction, bimanual interaction) and significantly improve real-world generalization, with strong correlation between simulation and real-world performance metrics.
...
TL;DR: Certified CacheMPC uses a locality-sensitive hashed cache of horizon contact-force trajectories with a-posteriori feasibility and optimality certificates, achieving 25× simulation and 18.7× hardware speedups over baseline MPC while maintaining comparable closed-loop stability at n=50.


<details>
  <summary>Details</summary>
Motivation: MPC is the standard predictive layer in hierarchical quadruped controllers, but per-cycle QP solve limits update rates on embedded processors. Legged gaits revisit bounded regions of state space, suggesting MPC solutions can be cached and reused.

Method: A Locality-Sensitive-Hashed cache partitions horizon contact-force trajectories by contact mode. At query time, cached solutions are retrieved and accepted only when an a-posteriori certificate confirms primal feasibility and provides a Lagrangian dual-gap upper bound on cost suboptimality. A bounded-budget controller schedule combines top-K certified retrieval, a deadline-bounded QP solve, and a shifted last-certified fallback.

Result: Evaluated on Unitree Go2 across 2,038 MuJoCo trials (including 600-trial n=50 campaign at failure-boundary cells) and on-robot NVIDIA Orin NX. Ungated cache delivers 25× median solve-time speedup in simulation and 18.7× on hardware. At n=50, no statistically significant difference in closed-loop stable rate between cache variants and no-cache baseline. Certificate's contribution to closed-loop safety not resolvable at present sample size.

Conclusion: Certified CacheMPC provides substantial computational efficiency gains while maintaining comparable closed-loop stability performance. The certification framework ensures safety through feasibility and optimality guarantees, though the quantifiable safety contribution requires larger sample sizes to determine.
...
TL;DR: WARP (Warp-Augmented Relative Progress) is a self-supervised algorithm that learns dense relative progress signals from demonstrations via time-warp augmentations, enabling behavior cloning to maintain high performance (19/20 success) even with noisy, suboptimal teleoperation data, outperforming vanilla BC (2/20 success) by up to 18x throughput.


<details>
  <summary>Details</summary>
Motivation: Scaling imitation learning requires large datasets, but human teleoperation produces mixed-quality demonstrations with hesitations and recoveries. Prior frame-level progress reward models have limitations: they supervise on absolute temporal progress proxies that suffer from label noise, or require costly human annotations to define subtask boundaries.

Method: WARP generates per-frame progress targets through time-warp augmentations of demonstrations (variable playback speeds and reversals), training a progress reward model (WARP-RM) to predict normalized elapsed time between input frames. Aggregating these predictions across overlapping windows yields a dense frame-level progress signal. WARP-BC then leverages these scalar reward estimates to upweight high-advantage action chunks during behavior cloning, where chunk-level advantage is obtained by aggregating per-frame rewards.

Result: Evaluated on a physical bimanual robot performing T-shirt folding, with training datasets of varying quality constructed using episode length as a proxy for teleoperation sub-optimality. WARP-BC maintains a 19/20 success rate compared to vanilla BC's 2/20, and improves throughput by up to 18x.

Conclusion: WARP-BC demonstrates superior robustness against suboptimal data compared to vanilla behavior cloning. By learning dense relative progress signals in a fully self-supervised manner, the approach maintains high performance even as datasets include more inefficiencies, suggesting that self-supervised progress signals can effectively handle noisy human demonstrations.
...
TL;DR: DexCompose is a role-aware residual composition framework that enables multi-task dexterous manipulation by reusing pretrained policies through explicit finger-level action ownership and dual asymmetric residual modules, achieving 77.4% success rate on 16 composite tasks.


<details>
  <summary>Details</summary>
Motivation: Existing dexterous manipulation policies struggle to compose multiple tasks in a single hand because new tasks impose conflicting demands on overlapping fingers and contact modes, causing destructive interference with preserving established skill states.

Method: DexCompose collects successful post-task states from a pretrained skill, performs release tests on finger masks to identify necessary fingers, and trains two asymmetric residual modules: a bounded residual stabilizer for task preservation and a context-aware residual for adapting the frozen downstream policy within the action subspace assigned to the new task.

Result: Evaluated on 16 composite dexterous manipulation tasks spanning four object-retention skills and four downstream interactions, DexCompose achieves a 77.4% average composite success rate.

Conclusion: Structural action ownership with dual residuals demonstrates a promising direction for composing dexterous skills beyond conventional policy chaining, effectively resolving conflicts between preserving existing manipulation outcomes and executing new tasks.
...
### [30] [SidConArena: An Environment Evaluating Agents in Open-Ended,Positive-Sum Bargaining Game](https://arxiv.org/abs/2606.27397)
*Yeqi Feng,Yuxin Chen,Tianxing He*

Main category: cs.MA

TL;DR: SidConArena is a new benchmark for evaluating LLM agents in open-ended, positive-sum bargaining scenarios, formalized as a multi-player economy with negotiation, production, and auction phases.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent evaluation lacks dynamic environments beyond static reasoning and zero-sum games. Real-world economic interactions require open-ended, mixed-motive capabilities: negotiation, positive-sum surplus creation, competition for scarce assets, and planning under delayed returns.

Method: SidConArena formalizes a multi-player economy as a finite-horizon partially observable stochastic game with three coupled phases: (1) natural-language negotiation with binding trades, (2) deterministic converter-based production, and (3) sealed-bid auctions for long-term assets. The framework combines structured observations, phase-aware agent dispatching, a neural-symbolic action interface, and asynchronous execution.

Result: Across homogeneous and heterogeneous tournaments, stronger frontier models achieve higher economic outcomes. However, agents still misvalue resources, bargain passively, and remain limited in long-horizon investment planning.

Conclusion: SidConArena successfully enables rule-grounded evaluation of LLM agents in complex economic interactions while preserving free-form interaction. The benchmark reveals that current LLM agents struggle with economic reasoning, resource valuation, and long-term strategic planning despite improvements in economic performance.
...
### [31] [Delayed Verification Destabilizes Multi-Agent LLM Belief: Instability Thresholds and Optimal Corrector Placement](https://arxiv.org/abs/2606.27409)
*Igor Itkin*

Main category: cs.MA

TL;DR: Multi-agent LLM systems use verifier/critic agents to reduce hallucinations, but verification delays cause false claims to propagate. The authors model this as delayed consensus on a graph with grounded corrector nodes and use spectral decomposition to derive stability thresholds, showing that too strong or delayed correction can turn consensus into oscillation.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems rely on verifier and critic agents to suppress hallucinations, but verification is delayed. During this delay, false claims can propagate through the agent network before they can be corrected.

Method: Model the verification process as delayed consensus on a graph with grounded corrector nodes. Use spectral decomposition by the grounded Laplacian to derive a closed-form stability threshold for the verification dose. Show that correction that is too strong or too delayed can turn consensus into oscillation. Propose a supermodular placement objective and a greedy (1-1/e)-approximation rule for assigning a limited corrector budget to influential nodes.

Result: The most unstable regime occurs when communication and verification delays coincide; for delay 2, the threshold is the inverse golden ratio. Experiments across five open models confirm the predicted dose-delay oscillations. Grounded factual answering makes truth an absorbing boundary and eliminates the effect, suggesting the instability is specific to signed-belief tasks while grounded verification remains stabilizing.

Conclusion: The framework provides theoretical insights into the stability of multi-agent LLM consensus mechanisms, demonstrating that verification delays can cause oscillations in consensus outcomes, and that this effect is task-specific—eliminated in grounded factual answering but present in signed-belief tasks.
...
### [32] [Glite ARF: Verifier-Driven Research with Parallel LLM Coding Agents](https://arxiv.org/abs/2606.27416)
*Vassili Philippov,Pavel Katunin,Dmitry Andreev,Igor Ostanin,Anton Nikolaev*

Main category: cs.MA

TL;DR: Glite ARF is an open-source Python framework that enables parallel execution of multiple LLM coding agents on research repositories using a verifier-driven approach (three-role stack: researcher, coding agents, deterministic Python verifiers) to ensure reproducibility and prevent instruction lapses from compounding into broken artifacts.


<details>
  <summary>Details</summary>
Motivation: LLM coding agents are tempting for automating empirical research, but naive delegation doesn't scale to large projects because low-rate instruction lapses compound into broken, irreproducible artifacts. The framework addresses the need for reproducibility and auditability when scaling LLM agents across complex research workflows.

Method: A three-role stack architecture: (1) human researcher selects hypotheses, (2) coding agents (Claude Code, Codex CLI) implement tasks under a fixed structure, (3) deterministic Python verifier scripts enforce task isolation, immutability of completed work, corrections overlay, and materialized project overview. The "verifier-driven research" approach places rules in code that fail loudly when violated, rather than relying on prose that agents merely follow.

Result: Applied to BEA 2026 vocabulary-difficulty shared task, achieving first place in closed track and second in open track across Spanish, German, and Mandarin. Reduced official baseline RMSE by 29.9% (closed) and 35.9% (open). Campaign involved 273 tracked tasks, 146 experiment runs, 129 feature sets, up to 12 parallel agents on a single laptop with rented A100s for model training. Total cost: ~$450 LLM API spend, $498 total third-party cost. The framework's structural machinery adds only ~1% wall-clock time overhead.

Conclusion: The verifier-driven approach successfully scales LLM coding agents to large research projects while maintaining reproducibility and auditability. The framework proved effective across three domains, enabling significant improvements in a shared task competition while maintaining low overhead (1% wall-clock time) and enabling detection/correction of data issues (four target-leaking feature sets identified).
...
### [33] [QueenBee Planner: Skill-Evolving Communication Topologies for Token-Efficient LLM Multi-Agent Systems](https://arxiv.org/abs/2606.27492)
*Congjia Tian,Yuhang Yao,Jiaming Cui*

Main category: cs.MA

TL;DR: QueenBee Planner is a framework that uses an LLM planner to self-improve inter-agent communication topology in multi-agent systems by generating temporal communication DAGs, learning evidence-backed design rules, and employing mechanisms like acceptance gates and variance-aware credit to prevent self-evolution issues.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems depend not only on individual agent reasoning but also on how agents are connected. The paper treats inter-agent communication topology as a retrievable and self-improving design skill, addressing the gap in learning architectural design knowledge for multi-agent systems.

Method: The framework freezes worker agents, task adapter, and scoring function, while an outer LLM planner learns to generate temporal communication DAGs specifying who sends information to whom in each round, who merges messages, and who emits the final answer. Execution traces are distilled into evidence-backed design rules with three actions (Preserve, Modify, Avoid). The method employs multiple mechanisms to prevent self-evolution issues: held-out acceptance gates, variance-aware credit, motif-level attribution, transfer trust, insight falsification, and structural deduplication.

Result: Self-evolved graph generation improves over fixed topologies and cold generation. In the Count-Frequency fulltest setting, the best generated graph reduces RMSE from 12.53 (strongest fixed topology) to 7.87 while also reducing messages, model calls, and token costs. Silo-style tasks show the same direction of improvement over cold and fixed-topology baselines.

Conclusion: These results demonstrate that multi-agent systems can learn reusable architectural design knowledge rather than merely memorizing task answers, showing that communication topology itself can be effectively learned and optimized through self-evolution mechanisms.
...
### [34] [GenWorld: Empirically Grounded Urban Simulation Infrastructure for Scalable LLM-Agent Studies](https://arxiv.org/abs/2606.27650)
*Gen Li,Jieyuan Lan,Pengcheng Xu,Zongyuan Wu,Masaki Ogura,Tao Feng*

Main category: cs.MA

TL;DR: GenWorld is a simulation infrastructure that addresses the joint grounding and scaling problem in LLM-agent simulation by combining a building-level synthetic city, a structured agent-environment interface, and offline compilation of LLM-derived decision signals into lookup policies for scalable rollout.


<details>
  <summary>Details</summary>
Motivation: LLM-agent simulation faces two key challenges: agents need to act in environments that reflect real urban constraints, and direct online LLM calls for city-scale populations are computationally prohibitive.

Method: GenWorld comprises three components: (1) a building-level synthetic city, (2) a structured agent-environment interface, and (3) offline compilation of LLM-derived decision signals into lookup policies for scalable rollout.

Result: In Higashihiroshima, Japan, GenWorld grounds 196,608 synthetic residents using census and geospatial data, validates demographic consistency against census tabulations, and uses YJMob100K mobile-phone data as a commuting-distance diagnostic. Three reproducible cases demonstrate the infrastructure: a full-city weekday rollout, a weekday-weekend behavioral contrast, and a warning-response perturbation with auditable replanning traces.

Conclusion: GenWorld serves as a reproducible platform for grounded and scalable LLM-agent studies, with calibrated forecasting for traffic, evacuation, or policy outcomes remaining future work.
...
### [35] [GBC: Gradient-Based Connections for Optimizing Multi-Agent Systems](https://arxiv.org/abs/2606.28187)
*Xiaocheng Yang,Abdulrahman Alrabah,Dilek Hakkani-Tür,Gokhan Tur*

Main category: cs.MA

TL;DR: Gradient-Based Connections (GBC) models multi-agent systems as computational graphs and uses gradient-based connection weights to enable fine-grained attribution and targeted optimization of LLM-based MAS by precisely identifying error sources at the token level.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems (MAS) built on LLMs can solve complex tasks through role specialization, but they suffer from miscoordination and lack of fine-grained credit assignment. Existing approaches rely on coarse-grained feedback, making it difficult to identify which agents or interaction steps are responsible for errors.

Method: GBC models a MAS as a computational graph and introduces gradient-based connection weights to quantify each agent's output influence on downstream agents at the token level. It constructs an attribution graph and propagates task-specific loss signals backward, enabling precise error source identification and targeted prompt optimization. An efficient implementation called AgentChord leverages prefix-based gradient computation.

Result: Experiments on MultiWOZ and τ-bench demonstrate that GBC improves multi-agent performance and outperforms strong single-agent and multi-agent baselines. Higher attribution quality is associated with greater optimization effectiveness.

Conclusion: GBC provides an effective approach for fine-grained attribution and optimization of multi-agent systems, enabling precise identification of error sources and targeted prompt improvements through gradient-based connection weights.
...
### [36] [Spectral Subsurface Scattering from RGB via Biophysical Skin Inversion](https://arxiv.org/abs/2606.27604)
