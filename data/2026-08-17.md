...
TL;DR: A-Fiedler proposes replacing the Fiedler vector with the dominant adjacency eigenvector as a distributed graph embedding for controlling communication agent repositioning in heterogeneous mobile-agent networks, achieving comparable performance to the classical Fiedler-gradient controller while improving robustness under distributed estimation constraints.


<details>
  <summary>Details</summary>
Motivation: Controlling communication agents to maximize algebraic connectivity (a spectral graph metric) requires distributed estimation of the Fiedler vector, which converges only with unbounded communication rounds, making it unsuitable for real-time control under local communication constraints.

Method: The authors decompose the Fiedler-gradient controller into a local interaction rule and graph embedding component, and propose A-Fiedler which replaces the Fiedler embedding with the dominant eigenvector of the adjacency matrix, a representation better suited for distributed implementation under local communication constraints.

Result: A-Fiedler achieves comparable network performance to the classical Fiedler-gradient controller in the absence of communication constraints, and shows improved robustness under distributed estimation, maintaining performance even when the classical method may converge to disconnected configurations.

Conclusion: A-Fiedler provides a simpler path toward distributed network control by using an alternative graph embedding that can be estimated distributively without requiring unbounded communication rounds, offering both comparable performance and enhanced robustness.
...
TL;DR: hint^2 guides short-horizon manipulation policies toward satisfying complex Linear Temporal Logic (LTL) specifications at inference time using hierarchical world models with separate high-level (progress through LTL automaton) and low-level (local safety) guidance objectives.


<details>
  <summary>Details</summary>
Motivation: Large-scale language-conditioned policies enable robots to execute runtime instructions but struggle with temporal structure and safety constraints. LTL provides a powerful language for complex, non-Markovian instructions, yet guiding policies toward LTL satisfaction is challenging because policies generate short-horizon action chunks while LTL specifications are evaluated over long-horizon trajectories.

Method: hint^2 introduces a method for guiding short-horizon policies toward LTL satisfaction using hierarchical world models. It derives two separate guidance objectives: a high-level model that predicts future action-induced transitions in task-relevant atomic propositions to guide progress through the LTL automaton, and a low-level dynamics model that predicts immediate state evolution for accurate local safety guidance.

Result: hint^2 overcomes limitations of current LTL-guided diffusion methods, outperforms existing inference-time steering methods in CALVIN, successfully completes instructions with complex liveness and safety constraints more elegantly than language-conditioned alternatives, and successfully handles complex instructions on a real UR5e manipulator.

Conclusion: hint^2 effectively bridges the gap between short-horizon policies and long-horizon LTL specifications, providing an elegant solution for complex manipulation tasks with temporal and safety constraints.
...
TL;DR: Graph-MambaNav is a target-aware spatial-temporal graph encoding framework for object-goal navigation that introduces heuristic node ordering based on target relevance and uses LLM-derived commonsense relationships, combining local message passing with global GraphMamba-based selective scanning and Mamba-based sequence modeling to achieve improved navigation performance with better generalization.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based navigation methods are permutation-invariant and lack explicit control over information propagation order, limiting their ability to model target-dependent importance and long-range dependencies.

Method: Graph-MambaNav introduces heuristic node ordering based on target relevance (processed later to aggregate richer context), initializes node ordering and edge weights from LLM-derived commonsense relationships, and employs a spatial module (local message passing + global GraphMamba selective scanning) and a temporal module (Mamba-based sequence modeling over object-wise temporal orders) for selective aggregation of historical context.

Result: Experiments on AI2-THOR and RoboTHOR demonstrate improved navigation performance with generalization, validated further through real-world robot deployment.

Conclusion: The proposed approach effectively addresses the limitations of existing methods by incorporating node prioritization through sequence ordering, enabling better global reasoning and target-dependent importance modeling for object-goal navigation.
...
TL;DR: This paper proposes Knowledge-Data Dual-driven Reinforcement Learning (KDDRL) to address three key challenges in autonomous vehicle decision-making: latent driver intention uncertainty, non-stationarity from abrupt maneuvers, and hybrid action space instability. The method uses a conditional deep generative model to synthesize intention-aware trajectories, a knowledge-data dual-driven paradigm to fuse probabilistic insights with physical constraints, and a coupling module to create unified embeddings for multi-timescale optimization of continuous car-following and discrete lane-changing.


<details>
  <summary>Details</summary>
Motivation: In mixed traffic environments, AV decision-making faces three interconnected challenges: (1) physics-based RL priors fail to capture latent vehicle intentions and diverse driver behaviors, limiting proactive reasoning; (2) abrupt maneuvers by surrounding vehicles create non-stationarity that leaves long-tail safety events under-explored; (3) hybrid action spaces (continuous car-following and discrete lane-changing) with different temporal scales destabilize unified RL training.

Method: KDDRL consists of three main components: (1) A conditional deep generative model that synthesizes intention-aware future trajectories, converting passive perception into proactive predictive states; (2) A knowledge-data dual-driven paradigm that operates on predictive states, fusing probabilistic data-driven insights with physical constraints to guide safe exploration through safety-critical scenarios; (3) A coupling module that compresses intention-aware trajectories and physical constraints into compact shared embeddings, enabling asynchronous multi-timescale optimization of continuous and discrete actions while preserving mutual information.

Result: Evaluations on dataset-calibrated simulations demonstrate that KDDRL effectively handles intention uncertainty, accelerates training convergence, and outperforms conventional baseline methods in terms of safety, efficiency, and comfort metrics.

Conclusion: KDDRL successfully addresses the three interrelated challenges of mixed traffic AV decision-making by integrating generative modeling, knowledge-data fusion, and unified representation learning. The proposed framework enables proactive reasoning about vehicle intentions, robust handling of non-stationary environments, and stable training with hybrid action spaces, achieving superior performance across safety, efficiency, and comfort dimensions compared to baseline approaches.
...
TL;DR: Onto-EV-WM extends EV-WM with an ontology-grounded interface that explicitly represents unmet task predicates, route labels, and verification-gated corrections, improving final-state accuracy on PointMaze and LIBERO benchmarks while maintaining competitive success rates.


<details>
  <summary>Details</summary>
Motivation: EV-WM uses feature and event scores for candidate quality but lacks explicit representation of unmet task predicates, route labels for correction mechanisms, and post-correction acceptance results, making it difficult to diagnose and correct failures.

Method: Onto-EV-WM is a layered interface built atop EV-WM featuring: (1) task-local TBox defining entity types, predicate signatures, and constraints; (2) source-specific grounding maps for state representation; (3) deterministic rules that retain missing predicates and arguments when assigning route labels; (4) separation of learned/heuristic proposers from the symbolic interface; (5) native task predicates for acceptance decisions; (6) bounded protocol for retrying failed verifications.

Result: PointMaze: Onto-EV-WM achieves 94% success with mean final-state distance of 0.61177 (vs 0.90573 for EV-WM); LIBERO-Goal: 93.8% corrected-window success on seed 0, 94.05 ± 0.30% across four seeds; LIBERO-Plus (10,030 tasks): 85.00% task success (8,526/10,030), with suite-level rates of 65.98% (LIBERO-10), 91.39% (LIBERO-Goal), and 91.38% (LIBERO-Object, LIBERO-Spatial).

Conclusion: The ontology-grounded configuration significantly improves final-state accuracy and success rates compared to EV-WM baseline, demonstrating the value of explicit symbolic representation for diagnosis and correction mechanisms. However, ontology-only causal contributions are not measured separately, and real-robot recovery performance remains untested.
...
TL;DR: A biologically inspired modular recurrent controller with two GRU-based modules connected by a learnable delayed inter-hemispheric channel outperforms monolithic baselines in reaching and holding tasks, demonstrating that inter-module communication improves endpoint precision, reduces energetic cost, and lowers muscle co-contraction.


<details>
  <summary>Details</summary>
Motivation: Robotic motor control requires balancing fast, accurate movement with robust postural stabilization under signal-dependent noise and energetic cost. While modular controllers can distribute these competing demands, it remains unclear whether they outperform monolithic architectures under realistic constraints and how inter-module communication shapes the resulting strategy.

Method: Inspired by the bilateral hemispheric organization of the brain, a recurrent controller with two GRU-based modules connected by a learnable delayed inter-hemispheric channel was designed. The controller was trained end-to-end in a differentiable two-arm musculoskeletal simulator across reaching and holding tasks.

Result: The modular architecture substantially outperforms a capacity-matched monolithic baseline. Compared to a matched modular controller without communication, learned inter-hemispheric communication improves endpoint precision, reduces energetic cost in non-zero-delay regimes, and decreases muscle co-contraction.

Conclusion: Biologically inspired modular controllers offer a practical route to robust movement under noise and energetic constraints, with inter-module communication providing a mechanism to tune trade-offs between precision, stability, and actuation cost.
...
TL;DR: BICPO-VLA addresses the request-to-handoff gap in robot control by sequentially handling ambiguity about intended behavior, physical-state drift, and residual incompatibility through a causal history encoder, sequential Haar subspace generation, and Flow-DPO adaptation.


<details>
  <summary>Details</summary>
Motivation: The request-to-handoff gap has three coupled sources: ambiguity about behavior intended at request time, physical-state drift accumulated during action generation, and residual incompatibility when the new action assumes control.

Method: 1) Instruction-aware causal history encoder identifies behavior supported by command and task progress; 2) Sequential Haar subspace generation decomposes each action chunk into complementary pairwise scaffold and residual coefficients, enabling two specialized generation stages followed by exact reconstruction; 3) Flow-DPO rolls known outgoing actions to actual handoff state and applies reference-relative Flow-DPO among behaviorally matched candidates to adapt the generated chunk.

Result: By reducing iterative refinement in the original action space, BICPO-VLA shortens the interval over which the robot continues moving before the new chunk becomes available, while adapting the generated chunk to remaining request-to-handoff mismatch without changing intended behavior.

Conclusion: BICPO-VLA provides a sequential solution addressing all three sources of the request-to-handoff gap in robot control systems.
...
TL;DR: AdvDex is a unified Vision-Language-Action framework that enables dexterous manipulation learning from human and robot demonstrations by combining a large-scale human demonstration dataset (OmniShare), a canonical Joint-Aligned Action Space (JAAS), and domain-adversarial learning for cross-embodiment generalization.


<details>
  <summary>Details</summary>
Motivation: Dexterous manipulation is fundamental for embodied intelligence, but scaling is challenging due to: 1) Expensive robot demonstration collection; 2) Varying action spaces across different robot embodiments; 3) Policies trained on heterogeneous data entangle task-relevant visual cues with embodiment-specific appearance, limiting cross-embodiment generalization.

Method: AdvDex comprises three key innovations: (1) OmniShare - a large-scale multimodal dataset of human manipulation demonstrations providing high-quality kinematic supervision and tactile measurements while reducing reliance on expensive robot teleoperation; (2) JAAS (Joint-Aligned Action Space) - a canonical action representation using SE(3) wrist pose and 15 finger joints to functionally align human hands, dexterous robot hands, and parallel grippers; (3) Domain-adversarial learning to reduce embodiment-specific information in visual representations.

Result: Experiments demonstrate consistent improvements over baselines in hand-action prediction and real-world dexterous manipulation, with effective zero-shot human-to-robot skill transfer, generalization to unseen objects and environments, and data-efficient few-shot adaptation capabilities.

Conclusion: AdvDex successfully addresses the core challenge of cross-embodiment generalization in dexterous manipulation by decoupling task-relevant visual cues from embodiment-specific appearance through its unified framework, demonstrating that effective human-to-robot skill transfer and generalization are achievable with properly designed action spaces and domain-adversarial learning techniques.
...
TL;DR: ART is a tool-injection framework that integrates VLA models with agentic tool-use, reducing action space complexity through modular tool utilization to improve generalizability and reduce data dependency.


<details>
  <summary>Details</summary>
Motivation: Compared to vanilla VLA models with a whole continuous action solution space, ART reduces complexity through tool-use, which improves generalizability across different tasks and reduces data dependency.

Method: ART tunes any VLA model to leverage off-the-shelf tool modules for low-level vision, high-level affordance, and embodiment enhancement. It includes a dataset of 30K tool-use trajectories and action demonstrations, and a training regimen for long-trajectory tool-use reasoning in challenging environments.

Result: ART achieves a 20% higher success rate than mainstream baselines on simulation and real-world tasks, such as pick-and-place in the dark at novel viewpoints.

Conclusion: Modular tool utilization enables more efficient training, lightweight deployment, and scalable integration of new tools, fostering robustness, adaptability, and extensibility for practical deployment of VLA systems in complex real-world scenarios.
...
TL;DR: AgilePE is a self-play reinforcement learning system for autonomous UAV pursuit-evasion that enables zero-shot transfer to real quadrotors without task-specific tuning, using competitive self-play with Prioritized Fictitious Self-Play (PFSP) and hardware-aligned simulation pipelines.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based or differential-game approaches struggle with high-dimensional aerial interactions and agile maneuvering, requiring rapid decision-making under tightly coupled dynamics and continuously changing opponent behaviors.

Method: AgilePE integrates agile low-level control, competitive policy optimization, and sim-to-real deployment. It uses competitive self-play with Prioritized Fictitious Self-Play (PFSP) and diversified opponent pool. The policy maps onboard state observations to Collective Thrust and Body Rates (CTBR) commands. A hardware-aligned simulation pipeline models actuator-response dynamics, communication latency, and domain randomization.

Result: The learned policies transfer zero-shot to real quadrotors without task-specific tuning. Real-world experiments reproduce pursuit-evasion tactics including rapid dodging and flanking, and demonstrate interactive two-agent zero-shot deployment.

Conclusion: Competitive self-play enables sophisticated pursuit and evasion strategies to emerge, and the system demonstrates effective zero-shot transfer from simulation to real quadrotors for autonomous aerial pursuit-evasion tasks.
...
TL;DR: OccPlanner is a goal-aware occupancy-conditioned diffusion planner that addresses pixel-goal navigation challenges by grounding pixel targets in egocentric metric space through temporal visual context and learned local 3D occupancy features, achieving significant improvements in success rates (20.81%→71.55%) and demonstrating initial sim-to-real transfer capabilities.


<details>
  <summary>Details</summary>
Motivation: Pixel-goal navigation specifies targets directly in the agent's camera view, but pixel targets lack metric depth and traversability information, making 3D goal grounding and collision-free continuous planning challenging.

Method: OccPlanner employs a goal-aware occupancy-conditioned diffusion planner that sequentially conditions goal representation on temporal visual context and learned local 3D occupancy features. To provide occupancy supervision at scale, L3ROcc converts monocular RGB navigation videos into robot-centric local 3D occupancy annotations via geometric reconstruction and ray-based visibility reasoning. Training is performed on InternData-N1 dataset.

Result: In closed-loop simulation across four unseen scene categories and two goal-distance ranges, OccPlanner increased average success rate from 20.81% to 71.55% in the 5-8m setting. Performance reached 86.20% and 84.92% in cluttered-easy and cluttered-hard scenes, respectively. Real-world open-loop experiments on Unitree Go2 provide initial evidence of sim-to-real transfer and adaptation with L3ROcc-generated supervision.

Conclusion: OccPlanner effectively addresses the fundamental challenges of pixel-goal navigation by integrating occupancy-conditioned diffusion planning with temporal visual context, demonstrating substantial improvements in navigation success rates and initial feasibility for real-world deployment.
...
TL;DR: MMUSV-Sim is a perception-oriented maritime simulation platform built on Unreal Engine 5 and Project AirSim, designed to generate multi-USV cooperative perception datasets with multimodal observations (RGB, depth, semantic, LiDAR, radar).


<details>
  <summary>Details</summary>
Motivation: Cooperative perception among multiple unmanned surface vehicles (USVs) can extend maritime target sensing beyond a single platform's view range and field. Developing such systems at scale requires a unified workflow for configurable multi-USV scenarios, multimodal acquisition, and shared annotations.

Method: MMUSV-Sim is a perception-oriented maritime simulation and data-generation platform built on Unreal Engine 5 and Project AirSim. It provides island, open-sea, and port environments; configurable weather, time of day, and wave conditions; a diverse vessel asset library; and spline-based multi-vessel motion. It acquires RGB, depth, semantic, LiDAR, and radar observations across multiple USVs and captures a common world state for per-agent annotation export.

Result: The configured wave settings produce intended changes in vessel heave, roll, and pitch. Geometric consistency between projected annotations and semantic renderings is verified. In LiDAR-based cooperative BEV vessel detection experiments, Early Fusion achieves an AP@0.5 of 72.74, compared with 45.54 using a single USV.

Conclusion: MMUSV-Sim provides a scalable solution for multi-USV cooperative perception scenarios, enabling effective multi-USV dataset generation that significantly improves detection performance (AP@0.5 increased by ~27%) through cooperative perception.
...
TL;DR: A collaborative leader-follower robot control strategy is proposed to suppress transverse vibrations in flexible payloads during motion, enabling manipulation of heavier and larger flexible objects compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Vibrations in large and heavy structures during motion pose challenges for precise manipulation, particularly in critical applications like remote maintenance of future fusion energy reactors (tokamaks). Existing methods struggle with heavier and larger flexible objects due to challenges such as vibration suppression and heterogeneous robot specifications.

Method: Two industrial robot arms are arranged in a leader-follower configuration. The leader robot guides motion using shaped velocity commands, while the follower robot ensures compliance with estimated external forces through an admittance controller. The system dynamics are modeled using an equivalent mass-spring-damper model, with stability analysis provided.

Result: With appropriate admittance parameters, the total energy of the system is passively dissipated. Numerical simulations validate the method, and experimental results demonstrate its effectiveness in suppressing transverse vibrations during payload manipulation.

Conclusion: The proposed collaborative payload manipulation approach successfully addresses the challenges of vibration suppression and heterogeneous robot specifications, enabling the manipulation of heavier and larger flexible objects that existing methods cannot handle effectively.
...
TL;DR: A first fully parallel computing framework for LiDAR bundle adjustment in large-scale mapping, achieving up to tenfold computational efficiency improvement while preserving accuracy.


<details>
  <summary>Details</summary>
Motivation: LiDAR bundle adjustment is widely used for constructing globally consistent point cloud maps but is computationally intensive for large-scale datasets; existing methods struggle with efficiency on memory-constrained GPUs.

Method: Three key techniques: (1) adaptive asynchronous data loading strategy for memory-constrained GPUs, (2) novel bottom-up voxelization method for parallel planar feature extraction, and (3) majorization-minimization formulation with parallel computation for residuals, Jacobian, Hessian matrices, and increment solver.

Result: Extensive benchmarking on large-scale public datasets across various platforms demonstrates up to tenfold improvement in computational efficiency while maintaining accuracy comparable to state-of-the-art methods.

Conclusion: The proposed framework is robust and adaptable, with implementation code available on GitHub to benefit future research.
...
TL;DR: CORAL is a reinforcement learning approach for autonomous urban driving that uses a five-stage curriculum learning schedule and a stage-aware reward to simultaneously learn competing behaviors (goal reaching, route following, obstacle avoidance, signal obeying) across progressively harder tasks, achieving 100% success on the longest routes and zero-shot transfer to unseen environments.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning is promising for autonomous driving but long-horizon goal-directed navigation requires acquiring multiple competing behaviors simultaneously (reaching a distant goal, tracking a route, avoiding obstacles, obeying signals), and a fixed objective does not provide an order for learning these behaviors effectively.

Method: CORAL advances two schedules: (1) a five-stage curriculum that progressively lengthens routes and tightens behavioral constraints, and (2) a stage-aware reward whose component weights shift emphasis from mission progress toward route following, safety, smoothness, and rule compliance as the task hardens. The policy is a multi-stream actor-critic network trained with PPO in CARLA using a 99-dimensional state (polar LiDAR histogram + vehicle telemetry + ego-frame route geometry + traffic-rule indicators).

Result: Against two PPO baselines under identical protocol, CORAL reaches the goal in all 20 evaluation episodes on the longest routes with full behavioral constraints (baselines: 5% and 10%). Ablation shows neither schedule alone matches their combination—removing either lowers both success and route completion, and disabling both drops success to 55%. Zero-shot transfer to seven unseen towns achieves 68-98% success on routes of 100-150m length with mean lateral deviation below 0.35m.

Conclusion: CORAL effectively learns multiple competing behaviors for autonomous driving through its curriculum learning and stage-aware reward approach, demonstrating superior performance over baselines and good zero-shot transfer across different environments without requiring point-cloud encoders or bird's-eye-view rasterization.
...
TL;DR: ReflexBench is a new benchmark for reaction-critical manipulation tasks in robotics that addresses the gap between static and dynamic task evaluation. ReflexVLA is a VLA model built on this benchmark that improves performance on dynamic manipulation through temporal reasoning techniques including latent future prediction and multi-frame temporal fusion.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models and benchmarks focus mainly on static manipulation tasks and overlook dynamic interaction scenarios, creating a gap in evaluating generalization for reaction-critical manipulation contexts.

Method: ReflexBench introduces six dynamic tasks with an evaluation framework that decouples simulator stepping from robot control and supports configurable latency under synchronous/asynchronous inference. ReflexVLA is an efficient VLA model designed without large-scale robot-data pretraining, featuring latent future prediction, multi-frame temporal fusion in the vision backbone, batched visual encoding, and CUDA Graph replay.

Result: ReflexVLA consistently improves dynamic manipulation performance while maintaining competitive accuracy on standard static manipulation benchmarks. Real-world experiments demonstrate effectiveness under practical deployment conditions.

Conclusion: The proposed benchmark and model successfully address the gap in evaluating reaction-critical manipulation and demonstrate that temporal reasoning techniques can enhance VLA performance in dynamic scenarios.
...
TL;DR: A control-informed online trajectory planning framework that learns from execution errors to dynamically expand free-space planning areas, reducing lap time by 1.8 seconds without increasing computational burden.


<details>
  <summary>Details</summary>
Motivation: Autonomous racecars operate at vehicle dynamics limits where small control errors cause safety-critical behavior. Current trajectory planners assume perfect tracking, remain blind to execution errors, and use conservative spatial margins, leaving usable track space untapped.

Method: The framework measures systematic tracking deviations during runtime, dynamically adapts spatial track constraints, and iteratively expands the free-space planning area while maintaining time-optimal planning and compensating for accumulated execution errors.

Result: In a high-fidelity closed-loop simulation with autonomous racecars, the approach reduced lap time by 1.8 seconds without increasing computational burden, maintaining a median runtime of 25 ms.

Conclusion: Feeding control-induced deviations back into the planning layer unlocks performance previously inaccessible to modular architectures, enabling autonomous vehicles to systematically exploit track limits.
...
TL;DR: THRIVE is a modular at-home rehabilitation platform combining virtual-reality games, real-time camera-based motion tracking, and an adaptive robot therapist for children with upper-limb motor impairments.


<details>
  <summary>Details</summary>
Motivation: Children with upper-limb motor impairments require consistent, task-specific practice to improve function, but current therapeutic solutions are often expensive, not scalable, or difficult to adapt for sustained home use.

Method: The system integrates newly designed VR games targeting functional reaching, grasping, and object manipulation through customizable tasks (popping, hitting, catching, grabbing). A camera-based motion-tracking system captures kinematic performance, while a robot therapist delivers adaptive feedback. The design decouples games from robot embodiment, creating a robot-agnostic modular architecture.

Result: The robot-agnostic platform is affordable, scalable, and adaptable for sustained home use, enabling consistent and engaging therapy sessions.

Conclusion: THRIVE offers a practical pathway to more consistent and engaging upper-limb rehabilitation for children with motor function impairments by combining engaging gameplay with precise therapeutic feedback in a home setting.
...
TL;DR: This paper proposes Expected Free Energy (EFE) as a unified criterion for budgeted robotic informative path planning, enabling autonomous robots to simultaneously build accurate information maps and locate high-value regions while accounting for travel and measurement costs under resource constraints.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots exploring unknown environments face competing demands: building accurate information maps, quickly finding regions of greatest value, and managing costs for travel distance and measurements. Classical information-seeking and reward-seeking criteria address only one objective at a time, creating a need for a unified framework that balances multiple objectives simultaneously.

Method: The authors propose Expected Free Energy (EFE) from active inference as a unifying action-selection objective. The agent maintains a Gaussian-process belief over the information field and plans continuous trajectories that minimize expected free energy under hard path-length constraints.

Result: Multiple realizations demonstrate that EFE-based planning yields accurate posterior maps and simultaneously locates the highest-value regions, outperforming information-theoretic baselines under identical settings.

Conclusion: Unified, easy-to-tune principled information-gathering strategies facilitate autonomous deployment while enforcing efficiency and resource constraints, making EFE a practical solution for robotic exploration applications.
...
TL;DR: A spatiotemporal tube-based approach for autonomous navigation of articulated vehicles (truck-trailers, road trains, towing AGVs) on narrow routes, generating certified path plans that ensure trailers remain within road corridors using kinematics and sway constraints.


<details>
  <summary>Details</summary>
Motivation: Articulated vehicles are essential for freight transportation, but their autonomous navigation is challenging due to physical characteristics and motion constraints when manoeuvring on narrow routes.

Method: A spatiotemporal tube-based planning approach that leverages kinematics of linked elements and sway constraints to create a safe tube for the actuated prime mover. The method modifies the spatiotemporal tube using permissible corrections to provide a route safety certificate.

Result: The proposed method is verified on a truck-trailer navigation simulation for a complex route, demonstrating its effectiveness for autonomous navigation of articulated vehicles.

Conclusion: The approach provides a certified path plan ensuring towed trailers always remain within the road corridor, limited by permissible corrections, making it suitable for safe autonomous navigation of articulated vehicles on narrow routes.
...
### [27] [Strand-based Hairstyle Generation via Large Reconstruction and Multimodal Models](https://arxiv.org/abs/2608.13679)
*Conghui Hao,Tao Huang,Yuefan Shen,Tongtong Wang,Zhongtian Zheng,Kui Wu*

Main category: cs.GR

TL;DR: This paper presents an automatic pipeline for generating high-quality strand-based hairstyles from single-view images by combining Large Reconstruction Models (LRMs), Large Multimodal Models (LMMs), and classical geometry processing, enabling production-ready strand geometry without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Current strand-based hairstyle creation relies on skilled artists and manual authoring, making it costly and difficult to scale. Existing learning-based methods have limitations: they require large diverse training datasets, struggle to generalize to complex styles like buns and ponytails, and operate in representations incompatible with strand-based modeling, editing, and simulation.

Method: The approach integrates Large Reconstruction Models (LRMs), Large Multimodal Models (LMMs), and classical geometry processing to generate strand-based hairstyles from single-view images. The method operates without task-specific training or data collection, leveraging the combined capabilities of these models to produce detailed, production-ready strand geometry.

Result: The method generates visually compelling strand-level reconstructions within a few minutes, handling a wide variety of hairstyles including straight and curly hair, short and long styles, and challenging structured configurations such as ponytails and buns. The output is production-ready strand geometry suitable for digital human workflows.

Conclusion: This novel pipeline effectively automates a time-consuming and costly process that previously required skilled manual authoring, making it well-suited for integration into modern digital human workflows while eliminating the need for task-specific training and data collection.
...
### [28] [Uncertainty-Aware Jacobi Set Computation](https://arxiv.org/abs/2608.14409)
*Daniel Klötzl,Daniel Weiskopf*

Main category: cs.GR

TL;DR: This paper presents an uncertainty-aware Jacobi set computation method that extends existing edge-based Jacobi set computation to handle uncertainty in multi-field scalar fields. The approach uses a spatially coherent uncertainty model based on multivariate normal distributions to propagate uncertainty analytically, enabling visualization of both Jacobi sets and uncertainty information simultaneously.


<details>
  <summary>Details</summary>
Motivation: Jacobi sets are topological descriptors that capture gradient alignments between two scalar fields, commonly used in multi-field visualization. The original computation method does not account for uncertainty in the input data, which limits its applicability in real-world scenarios where measurements and models contain uncertainty. The authors aim to extend Jacobi set computation to be uncertainty-aware to better represent the reliability of visualizations.

Method: The authors reformulate an existing edge-based identification of Jacobi set edges on a triangulation. They extend this with a versatile, spatially coherent uncertainty model for pairs of scalar fields based on multivariate normal distributions. The uncertainty is propagated analytically to lift the Jacobi set computation to uncertain inputs. They also design a specific visual mapping overlay to display Jacobi sets alongside different facets of uncertainty information.

Result: The uncertainty model and uncertainty-aware method were validated against a Monte Carlo approach on an analytic dataset and successfully applied to two practical use cases: fluid dynamics and weather ensembles. The results demonstrate that the method can effectively handle uncertainty while preserving the topological structure of Jacobi sets.

Conclusion: The proposed uncertainty-aware Jacobi set computation method successfully extends the traditional Jacobi set computation to handle uncertain inputs using an analytical uncertainty propagation approach. The combination of a spatially coherent uncertainty model and tailored visual mappings enables effective visualization of both gradient alignment structures and uncertainty information, validated through Monte Carlo comparisons and demonstrated in practical applications.
...
### [29] [XAI-Guided Conservative Decentralized Execution for Offline Multi-Agent Network Slicing](https://arxiv.org/abs/2608.13982)
*Eslam Eldeeb,Hatim Chergui,Merouane Debbah*

Main category: cs.MA

TL;DR: X-CODE is an explainable offline multi-agent reinforcement learning method for network slicing resource allocation that operates without environmental interaction or inter-agent communication by using explainability-aware reward shaping during centralized training.


<details>
  <summary>Details</summary>
Motivation: 6G and beyond-6G networks require intelligent resource management for network slicing under shared infrastructures. Resource allocation forms a resource-coupled cooperative optimization problem with competing slice demands. Existing online MARL formulations are costly, unsafe, and difficult to deploy due to reliance on environmental interactions and inter-agent communication.

Method: X-CODE (explainable artificial intelligence-guided conservative decentralized execution) employs explainability-aware reward shaping to modify relative preferences among joint offline transitions during centralized training. Agents operate independently without signaling exchange during deployment, making it a fully offline MARL approach that requires no environmental interaction or inter-agent communication.

Result: The proposed approach achieves zero observed resource-conflict events while minimizing per-slice latencies. It exhibits lower signaling overhead and reduces effective inference latency by 88% compared to online baselines under the considered communication-delay model.

Conclusion: X-CODE successfully addresses the challenges of network slicing resource allocation by providing an effective offline MARL framework that eliminates resource conflicts, reduces latency, and significantly lowers signaling overhead compared to online approaches. The method enables practical deployment in resource-constrained environments without the safety and communication costs associated with online MARL.
...
### [30] [Submodular Policy Learning for Distributed Task Allocation in Open Multi-Agent Systems](https://arxiv.org/abs/2608.14390)
