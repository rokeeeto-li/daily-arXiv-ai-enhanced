...
TL;DR: GaussVLA is a parameter-efficient Mamba-based Vision-Language-Action model that uses Gaussian Spatial Tokenizer and Depth-Aware Chain-of-Thought to enable effective spatial reasoning for robotic manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models encode visual observations as flat 2D patch tokens lacking geometric structure. Monocular depth provides only scalar pixel values that don't encode surface orientation or geometric confidence, limiting the policy's ability to perform structured spatial reasoning for action prediction.

Method: GaussVLA incorporates two custom modules: 1) Gaussian Spatial Tokenizer (GST) that lifts frozen semantic and depth features into compact 3D Gaussian tokens and pools geometrically salient regions using learned queries; 2) Depth-Aware Chain-of-Thought (DA-CoT) that performs structured, non-autoregressive geometric reasoning under language and flow-time conditioning.

Result: On LIBERO dataset: 93.5% average success rate and 100% success on the Spatial suite with only 200M parameters. Outperforms SpatialVLA by 19.7% relative average success while being significantly more parameter-efficient.

Conclusion: GaussVLA demonstrates that incorporating geometric structure through Gaussian tokens and structured reasoning enables strong spatial-manipulation performance with minimal parameters, effectively addressing the geometric reasoning limitations of prior VLA approaches.
...
TL;DR: SOPO-CD is a sequential optimization framework for robotic object packing that uses differentiable nonlinear optimization in decomposed free space, achieving 100x speedup over classical grid search while eliminating assumptions about spatial discretization resolution.


<details>
  <summary>Details</summary>
Motivation: Robotic object packing is challenging due to the curse of dimensionality in combinatorial search, difficulty dealing with dynamic and contact constraints for irregularly shaped objects, and inefficiency of current heuristic and learning-based methods that assume limited spatial discretization resolution.

Method: SOPO-CD frames object placement as a differentiable nonlinear optimization problem in decomposed free space. The method proves that placing a convex object inside a convex hull constrains its vertices within the hull, allowing constraints and derivatives to be expressed in closed form (calculated within 200ns) and implemented via a custom solver.

Result: The framework achieves optimal placement within tightly constrained space in milliseconds, representing a 100x speedup over classical grid search. It generalizes to 2D Tangram, 2D Tetris, and 3D Bin Packing with strong computational performance and packing utility. Real-world demonstration solved a Tangram puzzle online using an Allegro Hand and Xarm.

Conclusion: SOPO-CD eliminates assumptions about spatial discretization resolution in robotic object packing, providing an efficient solution that achieves significant computational speedup and optimal placement accuracy. The framework demonstrates strong generalization capability across different packing domains.
...
TL;DR: A control-oriented data-driven framework that decomposes soft pneumatic actuator behavior into a nonlinear static equilibrium model and linear residual dynamics using EDMDc, enabling accurate trajectory tracking and stability analysis.


<details>
  <summary>Details</summary>
Motivation: Soft pneumatic actuators offer inherent compliance and safe human interaction but are difficult to model and control due to their highly nonlinear and distributed dynamics.

Method: The authors developed a control-oriented approach that decomposes actuator behavior into a nonlinear static equilibrium model and a linear residual dynamics model identified using Extended Dynamic Mode Decomposition with control (EDMDc). This representation enables feedforward compensation, task-space feedback control, and local closed-loop stability analysis through an augmented linear model.

Result: Experiments achieved approximately 1 mm RMSE during low-speed (≈10 mm/s) trajectory tracking and below 10 mm RMSE at higher speeds (≈100 mm/s). The framework successfully tracked highly dynamic user-generated references with peak accelerations exceeding 25 m/s² while performing real-time obstacle avoidance. The stability analysis accurately predicted stable, marginal, and unstable operating regimes.

Conclusion: Structured, control-oriented learning provides an accurate and practical framework for soft actuator control, demonstrating that data-driven approaches can effectively address the modeling and control challenges of soft robotic systems.
...
TL;DR: CRESSim-Neo is a batched GPU simulation engine that integrates position-based physics (rigid bodies, deformable tissues, fluids, strands) with batched rendering, surgery-specific sensing, and a GPU-resident data pipeline. It supports tissue manipulation, fluid suction, suturing, cable-driven robots, and ultrasound image synthesis, enabling GPU-resident robot learning with zero-copy PyTorch integration via DLPack.


<details>
  <summary>Details</summary>
Motivation: To provide a unified and scalable platform for surgical simulation, synthetic data generation, and surgical robot learning that can efficiently handle complex surgical tasks like tissue manipulation, fluid interaction, and ultrasound sensing.

Method: Combines position-based simulation for rigid bodies, deformable tissues, fluids, and strands with batched rendering, surgery-specific sensing, and a GPU-resident data pipeline. Supports applications including tissue manipulation, fluid suction, suturing, cable-driven robots, and ultrasound image synthesis. Enables GPU-resident robot learning and zero-copy PyTorch integration using DLPack.

Result: Achieves up to 2.03 million environment steps per second for 8192 parallel CartPole environments on an NVIDIA RTX 4090. Scales effectively to batched surgical scenarios involving tissue deformation, fluid interaction, and ultrasound sensing.

Conclusion: CRESSim-Neo provides a unified and scalable platform for surgical simulation, synthetic data generation, and surgical robot learning, with direct access to physics and rendering buffers for efficient GPU-resident learning.
...
TL;DR: This study investigates key barriers non-expert users face when teaching robots via demonstration in home environments without live expert feedback, using pre-training and adaptive guidance with care providers across multiple visits, and proposes an open-source LfD dataset.


<details>
  <summary>Details</summary>
Motivation: Most LfD evaluations with non-experts are conducted in controlled laboratory environments with robotics experimenter presence, lacking ecological validity for real-world home settings where users must teach robots without live expert feedback.

Method: Human subjects experiment with care providers as non-expert end users, employing two forms of demonstrator guidance (pre-training and adaptive feedback) developed in prior work, conducted over multiple visits to simulate realistic home environment conditions.

Result: The study identifies key usability barriers that non-expert users encounter when teaching robots via demonstration without live robotics expert feedback in home environments.

Conclusion: The researchers propose to open source the resulting LfD dataset capturing care providers teaching assistive tasks over multiple visits to a home environment.
...
TL;DR: TARCAT is a human-interpretable taxonomy of 41 action primitives for construction work, organized into 12 groups and 3 classes, derived from O*NET tasks and instructional videos, enabling robots to perform broader range of construction tasks.


<details>
  <summary>Details</summary>
Motivation: Conventional task-specific robots have limited repertoires, and while vision-language-action models offer broader capabilities, they require a precise inventory of worker activities and robot capabilities needed for construction deployment.

Method: TARCAT was derived from 91 O*NET tasks across seven high-employment construction occupations and 30 instructional videos. The taxonomy defines 41 action primitives in 12 groups and 3 classes with a mechanism for composing parameterized primitive sequences into reusable skills. Selected primitives were demonstrated on a DOBOT CR3 arm with a CRAFT hand.

Result: TARCAT provides a common vocabulary for analyzing human work and developing general-purpose construction robots, enabling it to organize demonstrations, specify robot requirements, and support coding agents that retrieve and extend skill libraries. Annotations are available on GitHub.

Conclusion: TARCAT establishes a standardized framework for construction robotics that bridges human work analysis and robot development, facilitating the creation of robots with broader construction capabilities through a human-interpretable, reusable skill library.
...
TL;DR: LAC is a general whole-body controller for humanoid robots that achieves commanded Linear and Angular Compliance for wrenches applied to the upper body, enabling robust physical interaction with objects and humans.


<details>
  <summary>Details</summary>
Motivation: Real-world humanoid tasks require physical interaction with objects and humans. Current controllers either reject external forces as disturbances or restrict compliance to limited body links while ignoring angular effects.

Method: The approach synthesizes whole-body compliant responses into a large-scale augmented dataset by imposing sampled force and couple events on contact frames from human interaction data. At each contact link, external force and virtual torque from a passively yielding kinematic chain drive a virtual admittance under commanded stiffness. Teacher-student reinforcement learning then trains a single policy to track compliant motions under external wrenches.
...
Conclusion: LAC effectively enables humanoid robots to handle physical interactions in real-world scenarios through simultaneous linear and angular compliance across the entire upper body.
...
TL;DR: A constrained multibody dynamic model is developed to analyze the dynamic influence of welding umbilicals on collaborative robots, enabling efficient computation of reaction forces without Lagrange multipliers.


<details>
  <summary>Details</summary>
Motivation: With increasing deployment of lightweight and collaborative robots in welding applications, the external umbilical connection introduces significant dynamic effects that can affect robot motion and actuation forces. Understanding and modeling these dynamics is crucial for accurate control and performance optimization.

Method: The paper proposes a constrained multibody dynamic model representing the welding umbilical as a serial chain of rigid bodies interconnected by passive joints with elastic and dissipative properties. Holonomic kinematic constraints are applied at the distal anchor point. The equations of motion are reduced by projecting the dynamics onto the subspace of admissible velocities, yielding an efficient formulation that eliminates the need for Lagrange multipliers.

Result: The approach provides an efficient dynamic formulation for computing the reaction wrench exerted by the umbilical on the robot. A planar case study demonstrates the effectiveness of the method, showing that the reaction forces can be explicitly recovered without computational overhead from multiplier terms.

Conclusion: This work establishes a computationally efficient framework for analyzing umbilical dynamics in robotic welding systems, enabling better understanding of how external umbilical connections affect robot behavior and facilitating improved control strategies for collaborative robots in industrial applications.
...
TL;DR: RA-VLA is a retrieval-augmented Vision-Language-Action framework that enables training-free robotic task adaptation through behavior-aligned context retrieval and a grounded execution pipeline, achieving superior success rates and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action (VLA) models exhibit brittleness when confronted with novel task distributions. Existing In-Context Imitation Learning (ICIL) frameworks suffer from an adaptation bottleneck due to superficial retrieval mechanisms and behavioral inertia that anchors the policy to its pre-trained priors, hindering effective translation of expert context to executable actions.

Method: RA-VLA integrates behavior-aligned context retrieval with a grounded execution pipeline. It enforces faithful adherence to functional cues within a scalable architecture, addressing the limitations of superficial retrieval and behavioral inertia in existing ICIL frameworks.

Result: Empirical evaluations across the LIBERO benchmark and a real-world UR5e environment demonstrate that RA-VLA achieves superior success rates and computational efficiency compared to existing approaches.

Conclusion: RA-VLA establishes a robust framework for training-free robotic adaptation that facilitates seamless task adaptation while preserving inference efficiency, offering a promising solution to the brittleness problem in VLA models.
...
TL;DR: A navigation agent with non-parametric memory integrated into a recurrent PPO architecture, which explicitly indexes prior steps to critical events to improve retention of sparse, high-cost failures like human collisions and partially address the sim-to-real gap.


<details>
  <summary>Details</summary>
Motivation: Current robot policies rely exclusively on network parameters for behavior storage, creating a heavy burden on representation learning. There's a need to outsource some behavior to explicit memory, enable continual learning for OOD generalization, retain critical failure information in social navigation, and address the sim-to-real gap.

Method: Proposed navigation agent with non-parametric memory that explicitly indexes prior steps leading to critical events, integrated into a recurrent PPO architecture using hidden states for memory retrieval. Training leverages RL advantage signals to exploit rare, high-impact events and uses photorealistic rendering combined with non-visual crowd simulation.

Result: The agent improves capability to retain sparse, high-cost failures (e.g., human collisions), naturally addresses the sim-to-real gap by basing some decisions on real data, and demonstrates robustness to OOD social behavior.

Conclusion: Non-parametric memory allows policies to outsource behavior to explicit storage, enables continual learning through deployment data collection, and effectively handles social navigation challenges while partially mitigating sim-to-real transfer difficulties.
...
TL;DR: U-TAMP leverages Vision-Language Models to generate object affordance abstractions for task-and-motion planning in contact-rich scenarios with heterogeneous objects, achieving significantly higher success rates and faster planning times.


<details>
  <summary>Details</summary>
Motivation: Traditional TAMP approaches use simplified object models and sub-symbolic reasoning, limiting their applicability to real-world contact-rich tasks with heterogeneous object properties. These methods overlook key physical characteristics and suffer from increased planning time and decreased success rates.

Method: The paper proposes U-TAMP (Unified TAMP) which uses a Vision-Language Model to generate object-centric abstractions of inter-object affordances (specifically grasp and support constraints) for characterizing physical interaction constraints. These affordance abstractions enrich the planning domain to handle objects with variable physical properties.

Result: The approach achieves significantly higher planning success rates and improves planning times by 1-2 orders of magnitude compared to both the original U-TAMP baseline and a state-of-the-art VLM-based planner in simulated kitchen table organization scenarios.

Conclusion: By leveraging VLM-generated affordance abstractions, the method effectively handles contact-rich tasks with heterogeneous objects, offering both superior success rates and dramatically faster planning times than existing approaches.
...
TL;DR: EgoNav: A hierarchical system for image-goal navigation that combines learned waypoint predictors with geometric safety constraints and adaptive local path planning to improve success rate and path efficiency.


<details>
  <summary>Details</summary>
Motivation: Image-goal navigation using lightweight topological maps is practical but suffers from learned waypoint predictors that can violate geometric constraints or deviate from global paths. Existing systems either lack collision avoidance capabilities or rely on fixed parameters that cannot adapt to confined spaces, limiting their effectiveness.

Method: EgoNav implements a hierarchical approach that: (1) generates candidates from semantically segmented traversable regions, (2) scores candidates alongside learned waypoints for geometric safety, directional coherence, and fidelity to learned prior, and (3) employs an adaptive local path planner with parameters modulated based on refinement outcomes.

Result: Experiments in Habitat-sim and on a physical humanoid robot demonstrate that EgoNav consistently outperforms contemporary baselines in both success rate and path efficiency compared to existing approaches.

Conclusion: EgoNav addresses the limitations of existing systems by retaining the navigational intuition of learned predictors while introducing geometric constraints and adaptive planning, enabling safer and more efficient execution of image-goal navigation tasks in confined spaces.
...
TL;DR: This paper introduces \methodname, a compact extension to Vision-Language-Action (VLA) policies that inserts World State and Prediction Tokens directly into the backbone, enabling efficient 3D world representation and prediction without heavy computational overhead.


<details>
  <summary>Details</summary>
Motivation: While VLA policies have advanced language-conditioned robotic manipulation, action-imitation objectives provide weak supervision for metric 3D structure. Geometry-enhanced policies mainly improve current-scene grounding, while predictive policies often incur substantial deployment cost. GaussianDream provides 3D supervision but uses a dense VGGT/TGE-based prefix.

Method: The method inserts World State Tokens and World Prediction Tokens directly into the VLA backbone. A training-only World Representation Head decodes these tokens into a Current World and Future Prediction over shared Gaussian primitives, using static-dynamic factorization to preserve persistent structure and focus on interaction-relevant regions. At inference, the head, renderer, auxiliary objectives, and VGGT/TGE pathway are removed, leaving only 20 world tokens without online Gaussian decoding or rollout.

Result: \method achieves 98.6% on LIBERO and 87.8% on LIBERO-Plus, with clear gains under Camera and Layout shifts. Real-robot experiments improve average success from 29.2% to 52.5% over reproduced π_0.5 while maintaining efficient closed-loop control.

Conclusion: The proposed method provides effective 3D supervision through compact world tokens, outperforming baseline methods while maintaining deployment efficiency. The static-dynamic factorization enables persistent structure preservation and focused motion modeling, leading to significant performance improvements in both simulation and real-robot experiments.
...
TL;DR: VISTA-Policy leverages a Visual Deformation Field (VDF) from a compliant gripper as high-dimensional visuo-physical feedback for contact-rich manipulation, combining vision and physical deformation cues to achieve precise closed-loop control without expensive tactile sensors.


<details>
  <summary>Details</summary>
Motivation: Contact-rich manipulation requires precise interaction feedback. Vision-centric methods provide indirect, ambiguous cues under occlusion or subtle interactions, while tactile/force sensors offer rich contact information but introduce hardware complexity, calibration requirements, and deployment costs.

Method: VISTA-Policy integrates three components: 1) Physics-Aware Encoding Engine for real-time VDF decoding; 2) Energy Aggregation Denoising Mechanism to isolate true interaction signals; 3) Deformation-Augmented Policy Network with incremental gripper actions for precise closed-loop correction.

Result: Outperforms strong pure-vision baseline (3D Diffusion Policy) and tactile baseline on Cross-Scale Object Grasping, Cap Unscrewing, and Calligraphy Writing tasks. Demonstrates substantial out-of-distribution generalization to unseen object scales and robustness against dynamic disturbances.

Conclusion: VISTA-Policy offers a durable and cost-effective route toward general-purpose fine-grained manipulation in unstructured environments, bridging the gap between vision and tactile approaches.
...
TL;DR: VirTooS is a Python/C# toolkit for fleet management of Autonomous Mobile Robots (AMRs) that enables realistic, scalable virtual experiments in mixed-reality environments using ROS 2 and Unity, allowing seamless interaction between real and virtual robots.


<details>
  <summary>Details</summary>
Motivation: The authors address the need for realistic, scalable virtual experiments for distributed robotics, particularly for fleet-management tasks on AMRs. They aim to enable seamless interaction between real and virtual robots to facilitate task assignment and other distributed robotics experiments.

Method: VirTooS leverages ROS 2 and Unity game engine to create a mixed-reality environment. It provides tools for generating and customizing virtual scenarios, supports both virtual and real sensors (e.g., LiDARs) for mapping and navigation, and proposes tailored routines based on the ChoiRbot framework. The toolkit includes containerized deployment for easy setup across different machines.

Result: The toolkit successfully enables users to generate and customize virtual scenarios, demonstrating task assignment problems with seamless real-virtual robot interaction. It provides a containerized suite for easy deployment, and the source code will be publicly available on GitHub.

Conclusion: VirTooS provides a practical solution for conducting distributed robotics experiments in mixed-reality environments, bridging the gap between simulation and real-world applications through its comprehensive toolkit and seamless real-virtual integration capabilities.
...
TL;DR: This paper introduces a language-guided intent module that uses intent-geometry divergence scoring to detect and correct intent misinterpretation in autonomous vehicle planning. Gating the planned maneuver before commitment (upstream of corridor envelope) successfully prevents trajectory excursions. The system detects conflicts quickly (72ms after drift onset) while maintaining low false trigger rates (0.341/min after redesign). Ablation studies confirm the gating mechanism's effectiveness, particularly for detecting failures and providing uncertainty-based vetoing of the geometric rule.


<details>
  <summary>Details</summary>
Motivation: Intent misinterpretation during vehicle interactions causes recurring planning failures. The authors address this by implementing a decision layer that gates planned maneuvers before commitment, upstream of the corridor envelope.

Method: A decision layer with a language-guided intent module that:
1. Reads structured descriptors
2. Computes a smoothed intent-geometry divergence score
3. Gates the planned maneuver before commitment (upstream of corridor envelope)

The system includes a geometric rule for conflict detection and a confidence-based veto mechanism. Calibration is performed with uncertainty treated as abstention.

Result: On replays of an off-road departure and four crash clips:
- Gating is the only layer that repairs the plan
- Fires 72ms after drift onset but 161ms before corridor exit
- Keeps trajectory in corridor in all ten replays
- False triggers reduced from 9/min to 0.341/min by redesigning uncertainty handling

Ablation results:
- Full score detects fastest on 4/5 failures under deployed eligibility
- Detects 3/5 against unvetoed rule
- On in-domain tracks with equal false positives, geometric rule more than triples detection

Conclusion: The evidence supports the gating mechanism. The model's demonstrated roles are: (1) fastest detection of failures, and (2) uncertainty veto on the geometric rule. The approach successfully prevents trajectory excursions while maintaining low false positive rates through proper uncertainty handling.
...
TL;DR: A Lie Group-constrained MeanFlow approach for fast generative robotic grasping on SO(3) × R^3 that achieves 39× speed-up over diffusion/flow-based models while maintaining reliable grasp synthesis.


<details>
  <summary>Details</summary>
Motivation: Grasp synthesis in robotics requires modeling multimodal distributions rather than point estimates. While deep generative models (diffusion, flow-based) can learn these distributions, their iterative sampling nature makes them unsuitable for time-critical robotic operations.

Method: A fast generative grasping approach based on MeanFlow on the product Lie group G = SO(3) × R^3. The training objective couples algebraic semigroup consistency with Riemannian Conditional Flow Matching on G, anchoring the average velocity to the data distribution.

Result: The Lie Group-constrained MeanFlow samples reliable grasps in ≤5 network evaluations, matching SOTA diffusion/flow-based models on ACRONYM dataset with millisecond-scale inference latency (up to 39× speed-up). The approach directly translates to real-world robotic grasping without additional training or domain adaptation, exhibiting robustness under observation noise.

Conclusion: Lie Group-constrained MeanFlow enables fast, reliable generative grasping that overcomes the latency barrier of iterative generative models while maintaining performance, and demonstrates direct transferability to real-world robotic systems without domain adaptation.
...
### [44] [Hamiltonian Two-Way Coupling of Nonlinear Waves and 3D Flows](https://arxiv.org/abs/2608.25203)
*Sinan Wang,Ruicheng Wang,Taiyuan Zhang,Fan Feng,Jinjin He,Yuchen Sun,Zhiqi Li,Bo Zhu*

Main category: cs.GR

TL;DR: A nonlinear and dispersive 2D wave model based on the Zakharov formulation with canonical Hamiltonian structure enables consistent two-way coupling with 3D fluid solvers, reducing wave-height errors by 1.7-5× while running over 1,000× faster than BEM and 4× faster than pure GPU NB-FLIP.


<details>
  <summary>Details</summary>
Motivation: Existing 2D wave models in graphics are either linear or non-dispersive, causing visible reflections and artifacts at the 2D-3D interface when coupled with nonlinear 3D fluid solvers. This mismatch in wave dynamics limits accurate simulation of large-scale free-surface water.

Method: A nonlinear and dispersive 2D wave model is introduced based on the canonical Zakharov formulation, featuring a Hamiltonian structure where surface elevation (η) and surface potential (ψ) form a canonical pair. A canonically consistent two-way coupling scheme enables smooth information transfer across the 2D-3D interface.

Result: The 2D solver reduces mean wave-height error by 1.7-5× over SWE, BEM, and Airy baselines while running more than 1,000× faster than BEM. It achieves greater nonlinear accuracy and coupling fidelity than SWE and Airy with minor speed/stability trade-offs. Coupling with 3D Navier-Stokes suppresses visible seam artifacts across dispersion-matching and Kelvin-wake tests, running over 4× faster than pure GPU NB-FLIP.

Conclusion: The proposed nonlinear, dispersive 2D wave model with canonical Hamiltonian structure provides an effective solution for coupling with 3D fluid solvers, significantly reducing wave-height errors and interface artifacts while maintaining high computational efficiency, enabling accurate and efficient simulation of large-scale free-surface water dynamics.
...
### [45] [GLOSS: Geometric Local Self-Similarity Learning for Faithful Reference-Guided Texture Fill](https://arxiv.org/abs/2608.25461)
...
### [46] [BVR Sim: An Open and High-Throughput Environment for Heterogeneous Air-Combat Reinforcement Learning](https://arxiv.org/abs/2608.25419)
...
### [47] [Trust-Aware Sequential Decision Making and Rollout Planning for Resilient Multi-Robot Systems](https://arxiv.org/abs/2608.25690)
*Roee M. Francos,Daniel Garces,Orhan Eren Akgün,Nathaniel D. Bastian,Stephanie Gil*

Main category: cs.MA

TL;DR: Multi-robot routing systems can be compromised by GPS spoofing attacks; this paper introduces a distance-constrained spoofing model, tiered bipartite matching strategy, and trust-aware monitor to detect adversaries and maintain planner-execution consistency, demonstrating effective detection and resilient routing across various attack scenarios.


<details>
  <summary>Details</summary>
Motivation: Multi-robot sequential decision-making relies on reliable planning information and agents executing anticipated actions. Compromised agents can violate both assumptions through GPS spoofing, creating mismatches between planning models and physical execution that undermine system reliability.

Method: Researchers introduce a distance-constrained spoofing model for monitor-aware adversaries and a tiered bipartite matching strategy that maximizes assignment influence while limiting spoofing magnitude. They develop a trust-aware monitor combining probabilistic localization trust (calibrated with real GPS spoofing data) with behavioral evidence from task execution to classify and remove adversaries.

Result: Undetected adversaries cause rollout algorithms to lose expected cost-improvement behavior by violating planner-execution consistency. The trust-aware removal restores this consistency, enabling stable routing and recovery of the rollout's empirical advantage over base policies. Experiments demonstrate effective detection and resilient routing across varying spoofing capabilities, adversarial fleet sizes, adaptive attacks, monitoring configurations, and rollout horizons.

Conclusion: The trust-aware monitoring approach effectively identifies and removes compromised agents in multi-robot routing systems under GPS spoofing attacks. By restoring planner-execution consistency, the system maintains routing stability and recovers the empirical advantage of rollout algorithms, demonstrating robustness across diverse attack scenarios and system configurations.
...
### [48] [HypoForge: A Self-Improving Multi-Agent Framework for Automated Hypothesis Generation and Testing via Scientific Skill Learning](https://arxiv.org/abs/2608.25770)
*Ziqing Qian,Jiaying Lei,Yifang Wang,Nan Cao*

Main category: cs.MA

TL;DR: HypoForge is a multi-agent framework that learns reusable scientific skills for automated hypothesis generation and testing through stage-specific supervision mechanisms: adversarial generator-discriminator for hypothesis generation (without explicit feedback) and execution outcome-based learning for hypothesis testing (with empirical feedback).


<details>
  <summary>Details</summary>
Motivation: Existing AI scientist systems rely on static prompting or fixed workflows and cannot accumulate experience for continual improvement. The paper observes that hypothesis generation and testing involve different supervision signals - hypothesis generation lacks explicit feedback while hypothesis testing has empirical feedback available.

Method: HypoForge adopts an adversarial generator-discriminator mechanism for hypothesis generation to improve reasoning through comparative critique, and learns testing skills from execution outcomes and ground-truth results for hypothesis testing. It matches skill learning strategies with stage-specific supervision signals, enabling continual improvement without fine-tuning foundation models.

Result: Experiments on hypothesis generation and testing benchmarks demonstrate that HypoForge consistently outperforms existing AI scientist frameworks and skill-level variants, validating the effectiveness of the proposed stage-specific skill learning paradigms.

Conclusion: By matching skill learning strategies with stage-specific supervision (adversarial critique for generation, outcome-based learning for testing), HypoForge enables continual improvement of AI scientist systems without requiring fine-tuning of foundation models.
...
### [49] [AI Agentic Selective Laser Sintering Process Optimization](https://arxiv.org/abs/2608.25928)
*Peter Pak,Victor Alvarado,Amir Barati Farimani*

Main category: cs.MA

TL;DR: An AI-enabled agentic system for Selective Laser Sintering (SLS) that iteratively optimizes process parameters to improve tensile and flexural properties of three materials (PA12 GF, PA11 Onyx, and PA12 Blend) on the Inova Mk1, achieving comparable TDS specifications with minimal user guidance.


<details>
  <summary>Details</summary>
Motivation: Agentic systems enable intelligent automation of complex workflows, particularly for process parameter optimization to achieve desired mechanical properties in additive manufacturing.

Method: The agentic system uses knowledge from previous builds and minimal user guidance to iteratively optimize process parameters for three SLS materials (PA12 GF, PA11 Onyx, and PA12 Blend) on the Inova Mk1 platform.

Result: The system achieved comparable TDS-specified mechanical properties (tensile and flexural) over a small number of iterations, demonstrating the ability to continually learn from updated data.

Conclusion: This work showcases the capability of agentic systems to enable intelligent automation of complex tasks like process parameter optimization in selective laser sintering through continual learning from updated data.
...
### [50] [Praxist: From Experimental Artifacts to Solution Lineages](https://arxiv.org/abs/2608.25955)
