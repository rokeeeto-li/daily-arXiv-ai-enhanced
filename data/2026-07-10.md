...
TL;DR: A multi-agent framework for cooperative reasoning systems where diverse models collaborate to generate, critique, and synthesize solutions, demonstrating that model heterogeneity rather than framework structure is the critical factor driving substantial performance improvements in step-wise reasoning accuracy and explainability.


<details>
  <summary>Details</summary>
Motivation: As foundation models grow in scale and diversity, coordinating multiple models into cooperative reasoning systems offers a path toward safer, more reliable AI.

Method: A multi-agent framework where solver models generate independent drafts, critic agents provide structured critique and revision, and an aggregator synthesizes a final consensus solution. A scoring module evaluates across semantic, numerical, and procedural dimensions. Four configurations compared: (1) Individual Baseline, (2) Homogeneous Framework using one shared model, (3) Redundant Homogeneous Solvers using multiple instances of the same model, and (4) Heterogeneous Framework with diverse specialized models.

Result: Model heterogeneity is the critical factor for performance improvements. The heterogeneous configuration achieves superior step-wise accuracy (0.64 vs. 0.54 for individual models; 2.3x improvement over homogeneous configurations) with reduced variance across categories and difficulty levels. Step-wise reasoning quality improves dramatically only with model diversity, demonstrating that heterogeneous agents provide complementary error detection and reasoning refinement.

Conclusion: Heterogeneous multi-agent coordination supports transparent, auditable, high-confidence decision-making across scientific and industrial domains. The framework demonstrates that model diversity provides complementary error detection and reasoning refinement essential for explainability and auditability, establishing architectural principles for Global Applied AI systems.
...
TL;DR: A CPM-grounded multi-agent framework that models dynamic emotional evolution in persona-based dialogue systems by representing emotions as latent states continuously reshaped by dialogue triggers through affective trigger extraction, collaborative appraisal, and emotion state updating.


<details>
  <summary>Details</summary>
Motivation: LLMs have advanced persona-based dialogue agents, but current approaches encode emotions as static traits or surface-level cues and focus on user empathy rather than modeling the agent's own evolving emotional state, leaving trigger-driven emotional evolution within characters underexplored.

Method: Inspired by the Component Process Model (CPM) psychological theory, CPM-MultiAgent represents character emotion as a latent state continuously reshaped by dialogue triggers. The framework includes affective trigger extraction, CPM-based collaborative appraisal, and emotion state updating components.

Result: Experiments with baseline comparisons, ablation studies, human evaluation, and case analyses demonstrate that CPM-MultiAgent effectively models dynamic emotional evolution and enables more emotionally consistent role simulation in multi-turn interactions.

Conclusion: The framework successfully addresses the gap in modeling trigger-driven emotional evolution by providing a psychologically grounded approach to dynamic emotion modeling, showing effectiveness through comprehensive evaluation across multiple metrics and analyses.
...
TL;DR: Bandwidth-constrained cooperative VLN introduces hindsight gating, a lightweight post-hoc supervised gate that fires early and when agents are confident to synchronize representations via recurrent hidden-state alignment, achieving 260-320% higher alignment efficiency than random or entropy-based gating.


<details>
  <summary>Details</summary>
Motivation: Most cooperative VLN methods assume unlimited communication, ignoring real-world bandwidth constraints and information efficiency needs.

Method: Hindsight gating: a lightweight supervised gate that labels communication-critical steps post-hoc from navigation failures (avoiding REINFORCE variance). The gate fires in early steps and when agents are confident, explained by recurrent hidden-state alignment where early communication injects grounded trajectory representations that persist through GRU updates.

Result: Trained gates fire predominantly in early episode steps and when agents are confident across all budget levels. Achieves +0.072 cumulative alignment gain with B=3 transmissions, approaching unconstrained communication (+0.078) at 260% greater efficiency than random gating and 320% greater efficiency than entropy-based gating.

Conclusion: Establishes a new communication regime for bandwidth-limited embodied agents: synchronize representations early, navigate independently later.
...
### [4] [SASGeo: Stability-Aware Semantic Map Localization for GNSS-Denied UAVs -- A Framework and Synthetic Proof of Concept](https://arxiv.org/abs/2607.07737)
*Natalia Trukhina,Vadim Vashkelis*

Main category: cs.RO

TL;DR: A semantic map-localization framework for GNSS-denied UAVs that combines semantic raster alignment, relational graph evidence, feature stability, geographic distinctiveness, and integrity-aware rejection to achieve robust cross-view image retrieval.


<details>
  <summary>Details</summary>
Motivation: GNSS-denied UAVs require absolute position fixes to bound visual-inertial odometry drift. Cross-view image retrieval provides such fixes but raw appearance is sensitive to season, illumination, viewpoint, map age, and sensor modality.

Method: Represents the environment through persistent structures (roads, buildings, waterways, railways, intersections, field boundaries). Combines semantic raster alignment, relational graph evidence, feature stability and geographic distinctiveness, explicit positive/contradictory/unknown observations, and integrity-aware rejection of ambiguous fixes.

Result: In 220 randomized retrieval trials, global semantic descriptor achieved 58.6% Recall@1, while spatial semantic matching variants achieved 94.5-95.5%. Wilson 95% intervals separate global from spatial variants but overlap among spatial variants.

Conclusion: Structured semantic geometry can discriminate locations under controlled cross-view perturbations. The experiment supports semantic geometry over definitive benefits from individual modules. Further work is needed on aliasing, map-aging, and rejection tests for real-flight navigation.
...
### [5] [Physics-Guided Biomechanical Gait Adaptation for Humanoid Locomotion on Extreme Sloped Terrains](https://arxiv.org/abs/2607.07830)
*Xuanyu Chen,Mohan Liu,Dengchen Mei,Zhihao Gu,Haitian Zhang,Kaimin Mao,Haiyue Zhu,Shijun Yan,Lin Wang*

Main category: cs.RO

TL;DR: HumoSlope is a two-stage physics-guided framework for humanoid locomotion on steep slopes (up to 32.1°), using a slope-adaptive ZMP regularizer and a Biomechanical Slope Gait Adapter to prevent posture degeneration and enable blind continuous traversal.


<details>
  <summary>Details</summary>
Motivation: Model-free RL achieves impressive humanoid locomotion on flat/discrete terrains but struggles with steep slopes due to persistent gravitational bias. Generic reward formulations cause policies to converge to slow, conservative crouched gaits instead of proper climbing/braking dynamics.

Method: Two-stage framework: (I) Slope-adaptive Zero Moment Point (ZMP) regularizer on local inclined support plane for terrain-consistent balance prior; (II) Biomechanical Slope Gait Adapter (BSGA) that uses terrain descriptors as privileged signals to gate reward priors, modulating CoM height and lower-limb coordination for hip-dominant uphill propulsion and knee-oriented downhill braking.

Result: Extensive Sim-to-Real experiments demonstrate effective posture mitigation and enable blind continuous traversal of outdoor grass slopes up to 62.7% (32.1°).

Conclusion: Validates physics-guided approach for challenging slope terrain adaptation, with deployed actor remaining entirely proprioceptive without online exteroceptive sensing.
...
### [6] [Shift & Drift: A Zero-Shot Benchmark for Generalizable and Robust Autonomous Driving Motion Planning](https://arxiv.org/abs/2607.07844)
*Alessandro Canevaro,Hang Yu,Julian Schmidt,Peizheng Li,Silvan Lindner,Wilhelm Stork,Georg Martius,Julian Jordan*

Main category: cs.RO

TL;DR: Shift & Drift introduces a dual-track benchmark for evaluating motion planners under semantic shift (cross-city evaluation with nuPlan) and state-distribution drift (stochastic perturbations). RL-based planners show better generalization and robustness than imitation learning methods.


<details>
  <summary>Details</summary>
Motivation: Closed-loop motion planners trained on nuPlan-like datasets show strong in-distribution performance but struggle with novel urban topologies and recovery after execution perturbations, particularly regarding semantic shift (different cities/cultures) and state-distribution drift (compounding execution errors).

Method: A novel dual-track benchmark called Shift & Drift: (1) Semantic Shift Track transforms the DeepScenario Open 3D dataset into nuPlan, enabling zero-shot evaluation against 1,182 scenarios across German cities and San Francisco; (2) State-Distribution Drift Track injects stochastic perturbations into ego vehicle dynamics to test robustness against temporally correlated actuation noise. The paper systematically evaluates diverse planning paradigms including imitation learning and RL-based methods.

Result: Imitation learning methods achieve high scores in in-distribution benchmarks but fail significantly under semantic shift, especially in pedestrian-dense environments, and suffer persistent drift from temporally correlated actuation noise. In contrast, the RL-based planner demonstrates more graceful degradation, maintaining higher safety and progress metrics across both tracks.

Conclusion: The study reveals an empirical trade-off between imitation fidelity and closed-loop resilience, providing a rigorous benchmark for the community to evaluate progress toward reliable deployment of motion planners in real-world scenarios with distribution shifts.
...
### [7] [STEMbot: A Compliant Robot for Under-Canopy Plant Navigation](https://arxiv.org/abs/2607.07873)
...
### [8] [Time-to-Collision Based Dynamic Obstacle Avoidance Using Pretrained Vision Models for Robots in Unstructured Environments](https://arxiv.org/abs/2607.07885)
*Erik Jagnandan,Mulugeta Haile,Gregory Barber,Pratik Chaudhari*

Main category: cs.RO

TL;DR: A data-efficient, interpretable vision-based dynamic obstacle avoidance method that uses UniDepth for monocular depth estimation and SuperPoint/SuperGlue for feature tracking, eliminating the need for sim-to-real transfer and requiring minimal training data.


<details>
  <summary>Details</summary>
Motivation: Dynamic obstacle avoidance in unstructured outdoor environments is challenging due to the impracticality of large-scale robot-specific training data and simulation-based policies, with the sim-to-real transfer problem being inherent in simulation-trained approaches.

Method: Uses UniDepth (pretrained monocular depth estimation) to produce dense depth maps from RGB video; extends SuperPoint and SuperGlue feature correspondence pipeline to track keypoints across long frame sequences; projects 2D pixel positions into 3D using camera intrinsics and predicted depth; runs bundle adjustment; computes per-keypoint time-to-collision (TTC); selects a 2D motion primitive based on minimum-TTC keypoint.

Result: Achieves precision of 0.49 and recall of 0.38 in identifying frames with TTC below 1 second on M3ED dataset; correctly generates evasive motion direction in 84% of true positive detections; detects at least one frame with TTC less than 1 second for 20 out of 22 unique physical obstacles; requires only 74 seconds of data for hyperparameter tuning.

Conclusion: Unlike end-to-end learned methods requiring thousands of hours of robot-specific training data, this approach eliminates model training entirely, demonstrating exceptional data efficiency while preserving interpretable and generalizable behavior across diverse obstacle types.
...
### [9] [Monocular Vision Based Control Framework for Grasping](https://arxiv.org/abs/2607.07897)
*Shail Jadav,Dongheui Lee*

Main category: cs.RO

TL;DR: A unified monocular vision-based grasping framework that handles both soft and rigid objects using only RGB input and a position-controlled gripper, with language-based stiffness estimation and visual tracking.


<details>
  <summary>Details</summary>
Motivation: Existing approaches address soft and rigid object grasping separately and often rely on tactile sensing, object-specific models, or specialized grippers. There is a need for a unified, sensor-efficient approach that can handle objects with widely different mechanical properties in unstructured environments.

Method: The system combines open-vocabulary object detection, image segmentation, boundary-aware point assignment, real-time point tracking, and monocular depth estimation. It includes a language-based stiffness estimation model that infers object compliance from semantic descriptions, a Procrustes-based dissimilarity measure for deformable object grasp adaptation, and scaling of tracked point distances for rigid object gripper width regulation.

Result: Validated on a Franka Emika Research 3 arm using lettuce, fresh mozzarella cheese, croissants, paper towels, and hard plastic bottles. The framework achieves stable grasping across both soft and rigid objects using only visual feedback.

Conclusion: The proposed method demonstrates a practical, sensor-efficient, and generalizable approach for food handling and household manipulation, eliminating the need for tactile sensing, object-specific models, or specialized grippers.
...
### [10] [Towards Soft Robotic Exogloves for Musculoskeletal Manipulation to Reduce Pain and Spasticity](https://arxiv.org/abs/2607.07958)
*Antonia Salluce,Maeryn Erdheim,Gailen Davis,Lauren H. Sullivan,Max-William Kanz,Jacqueline Libby*

Main category: cs.RO

TL;DR: A soft robotic exoglove is developed that combines mobility assistance with massage-like compression therapy to address both spasticity and pain simultaneously.


<details>
  <summary>Details</summary>
Motivation: Hand spasticity and pain affect 12 million people worldwide, yet existing soft robotic exogloves only address mobility or pain separately, with no solution providing both benefits.

Method: Novel soft pneumatic actuators were designed, modeled with finite element analysis at 100 and 200 kPa, and fabricated using stereolithography. The glove features dorsal finger actuators customized to individual hand topology for conformal contact, ventral finger actuators capable of drastic compression for hyperflexed spastic fingers, and a palmar actuator printed via stereolithography. The glove was assembled and tested on a pilot user.

Result: Successfully fabricated and customized actuators including a dorsal finger actuator with full conformal contact and force distribution, a ventral finger actuator that compresses to fit tight spaces in hyperflexed spastic fingers, and a stereolithography-printed palmar actuator. The glove was successfully worn by a pilot user, validating comfort and effectiveness.

Conclusion: The preliminary development demonstrates the feasibility of a soft robotic exoglove that simultaneously addresses both mobility enhancement and pain management through massage-like compression, providing a novel solution for patients with hand spasticity and related pain conditions.
...
### [11] [Soft Robotic Exogloves for Dexterous Mobility -- Towards Personalized Rehabilitation](https://arxiv.org/abs/2607.07968)
*Paul Dela Cruz,Mostafa Mo. Massoud,Jacqueline Libby*

Main category: cs.RO

TL;DR: This paper presents a personalized pneumatically-actuated soft robotic exoglove for hand rehabilitation and assistance, designed through topological scanning, silicone mold casting, and validated through FEA and pneumatic testing.


<details>
  <summary>Details</summary>
Motivation: Soft robotic exogloves for hand rehabilitation and assistance often rely on standardized measurements not tailored to individuals, limiting their effectiveness for fine articulation needed in dexterous manipulation.

Method: The authors designed and fabricated a personalized pneumatically-actuated soft robotic exoglove by fitting the glove to a user's hand via topological scans and silicone mold casting. They performed finite element analysis (FEA) to evaluate actuator bending and physical human-robot interaction (pHRI) forces between an actuator and a simplified personalized biomechanical finger model. Pneumatic pressure control experiments were conducted with static and dynamic references. Testing of multiple designs explored the effect of relaxing the strain-limiting layer on actuator-to-finger joint alignment.

Result: Topological scans enable precise tailoring to hand anatomy. Anatomical personalization allows analysis of pHRI contact forces and achieves sufficient joint mobilization with non-ideal compression on the proximal phalanx. Pneumatic pressure control enables accurate and targeted mobility of the metacarpophalangeal (MCP) and proximal interphalangeal (PIP) joints with intrinsic stiffness. Relaxing the strain-limiting layer improves actuator-to-finger joint alignment during actuation.

Conclusion: This work presents personalization in structural conformability, joint topology, modeling of pHRI contact, and time-dependent actuation-deformation profiles. It lays a groundwork for informing exoglove design optimization to enable assistance in dexterous manipulation and neuromuscular rehabilitation of fine motor skills.
...
### [12] [In vivo feasibility study of humanoid robots in surgery](https://arxiv.org/abs/2607.07972)
...
TL;DR: A systematic evaluation of contemporary humanoid technology for laparoscopic surgical tasks, developing a humanoid-based teleoperation framework and assessing its capabilities through benchtop, user, and in vivo studies.


<details>
  <summary>Details</summary>
Motivation: Healthcare faces staffing shortages and increasing care demand. Current automation focuses on digital/logistical tasks, but hospital work remains embodied, requiring mobility, manipulation, and safe interaction. Humanoid robots could assist with surgical tasks, but it's unclear if current systems meet the precision, control, and safety requirements of minimally invasive surgery.

Method: Developed a humanoid-based laparoscopic teleoperation framework using general-purpose instruments. Assessed abilities through three evaluation methods: benchtop characterization, dry-laboratory user studies spanning diverse surgical experience levels, and in vivo porcine studies. Quantified technical feasibility, task performance, and clinical readiness relative to established surgical platforms.

Result: The study quantifies technical feasibility, task performance, and clinical readiness across all evaluation methods, providing an evidence-based assessment of current humanoid abilities and limitations for surgical applications.

Conclusion: Humanoid robots show promise for surgical applications, but significant technical challenges remain that must be addressed before clinical deployment. The study provides a comprehensive evidence-based assessment to guide future development.
...
### [13] [D-CLIPSE: Distributed Consensus-based Localization with Passive Listening on Shared State Exchange](https://arxiv.org/abs/2607.07995)
*Kyle Biron-Gricken,James Richard Forbes*

Main category: cs.RO

TL;DR: The paper proposes a consensus-based distributed filtering framework for multi-robot localization that shares preintegrated odometry and shared states among robots, achieving near centralized performance in accuracy and consistency compared to state-of-the-art decentralized approaches.


<details>
  <summary>Details</summary>
Motivation: Accurate and consistent multi-robot localization is crucial for downstream tasks like planning and control. While centralized filtering optimally fuses all measurements, it's rarely implementable due to hardware, communication, and computational constraints. Existing distributed approaches have limitations that this paper addresses.

Method: A consistent, communication-efficient, consensus-based distributed filtering framework that enables robots to share both preintegrated odometry and relevant shared states during inter-robot communication.

Result: Validated in simulated and experimental scenarios, the proposed method demonstrates near centralized performance in accuracy, with especially improved consistency compared to the current state-of-the-art decentralized approach.

Conclusion: The proposed framework achieves near centralized performance while remaining distributed, making it practical for real-world implementation where centralized solutions are infeasible.
...
### [14] [Factors Influencing Conversational Engagement in Robot-Delivered Individual Cognitive Stimulation Therapy (iCST) for Dementia in Home Settings](https://arxiv.org/abs/2607.07998)
*Emmanuel Akinrintoyo,Nicole Salomons*

Main category: cs.RO

TL;DR: This study analyzed conversational dynamics in robot-delivered Cognitive Stimulation Therapy (iCST) for people living with dementia in home settings, using the Co-STAR system with 8 participants over one week.


<details>
  <summary>Details</summary>
Motivation: Social robots offer promising support for dementia cognitive therapies by guiding structured conversation and activities, but conversational dynamics during robot-delivered cognitive stimulation therapy (CST) sessions are poorly understood.

Method: The Co-STAR system was deployed in homes of 8 people living with dementia for 30-minute sessions. Conversational metrics including words per turn, speech production rate, response duration, response latency, and self-referential language were analyzed to examine how engagement is shaped by prompt personalization, interaction phase, and participant characteristics.

Result: Three key findings: (1) Personalized prompts significantly increase response duration, self-referential language, and overall engagement; (2) Conversational behavior changes within sessions with reduced verbal output and autobiographical engagement in later phases, suggesting cognitive fatigue; (3) First-session metrics predicted long-term participation, while living situation influenced engagement patterns.

Conclusion: These findings provide empirical insights into factors shaping conversational engagement in robot-delivered iCST, informing the design of adaptive conversational robots for dementia therapy.
...
### [15] [RadLoc: Radar-based 3-DoF Global Localization via Fast, Robust, and Lightweight Spatial Descriptor Across Diverse Environmental Scenarios](https://arxiv.org/abs/2607.08115)
*Hogyun Kim,Jiwon Choi,Jungwoo Lee,Younggun Cho*

Main category: cs.RO

TL;DR: RadLoc is an end-to-end pipeline for radar-based global localization that accelerates preprocessing with 1D CA-CFAR filtering, leverages near-range dominance for compact descriptors, implements hierarchical coarse-to-fine retrieval, and combines phase correlation for 3-DoF pose estimation, achieving robust performance with the smallest descriptor size and fastest retrieval time among state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Global localization using spinning radar is gaining attention for its robustness to adverse weather and challenging environments, but existing studies focus on individual components (place recognition, pose estimation) rather than a holistic end-to-end pipeline. A fast, robust, and lightweight solution is needed.

Method: RadLoc implements a holistic pipeline from place recognition to 3-DoF pose estimation using 1D CA-CFAR filtering for preprocessing acceleration, a compact descriptor based on near-range dominance in radar images, an efficient hierarchical coarse-to-fine retrieval strategy, and phase correlation-based 3-DoF pose estimation.

Result: Extensive experiments on 15 sequences across 5 datasets demonstrate RadLoc achieves robust performance while maintaining the smallest descriptor size and fastest retrieval time among state-of-the-art approaches.

Conclusion: RadLoc presents a versatile global localization module applicable to SLAM and multi-session SLAM systems, demonstrating that a holistic approach to radar sensor-based localization can outperform individual-component methods in terms of descriptor efficiency and retrieval speed while preserving robustness.
...
### [16] [X-ACTA: eXtended Analytic Center Tension distribution Algorithm for fixed and mobile cable-driven-parallel-robot](https://arxiv.org/abs/2607.08265)
*Domenico Dona',Vincenzo Di Paola,Alberto Trevisani,Matteo Zoppi*

Main category: cs.RO

TL;DR: This paper develops an extended Analytic Center method for determining cable tensions in cable-driven parallel robots (CDPRs) when operating beyond their wrench-feasible workspace (WFW), enabling the robots to handle aggressive maneuvers and cable failure scenarios while maintaining smooth, differentiable tension profiles.


<details>
  <summary>Details</summary>
Motivation: While cable tension determination is well-studied within the wrench-feasible workspace (WFW), few approaches address scenarios where CDPRs must operate outside this boundary, such as during aggressive maneuvers or cable failures. This limitation restricts the robots' capabilities in challenging operational conditions.

Method: The authors introduce an extended version of the Analytic Center method that: (1) determines cable tensions outside the WFW, (2) maintains differentiability, (3) includes non-linear constraints, (4) preserves continuous and differentiable tension profiles, (5) ensures fast real-time convergence to a unique solution, and (6) relegates wrench errors to a negligible area of the WFW.

Result: The proposed method demonstrates: (1) continuous and differentiable tension profiles, (2) fast real-time convergence to a unique solution, (3) superior smoothness and wrench error compared to other slack-based formulations, and (4) Pareto dominance over the leading state-of-the-art method. Effectiveness is validated through numerical experiments.

Conclusion: The extended Analytic Center method provides a practical solution for enabling CDPRs to operate beyond their WFW, with significant advantages in smoothness and error minimization. This enables safer and more capable operation during aggressive maneuvers and cable failure scenarios.
...
### [17] [TFP: Temporally Conditioned Memory-Fusion Policies for Visuomotor Learning](https://arxiv.org/abs/2607.08283)
*Yushen Liang,Yue Peng,Baosheng Jin,Tianluo Zhang,Xinyu Zhang,Shuyi Zhou,Zhuoran Chen,Xinqi Liu,Shenji Wan*

Main category: cs.RO

TL;DR: TFP introduces event-aware memory dynamics to VLA policies, enabling them to preserve task progress and adapt beliefs during occluded or stage-dependent manipulation phases, improving success rates on LIBERO tasks by ~2% and achieving 75% on ShellGameTouch.


<details>
  <summary>Details</summary>
Motivation: VLA policies like π₀.₅ and OpenVLA are reactive (action from current observation only), which breaks down in stage-dependent manipulation where visually similar states require different actions based on task progress and interaction outcomes. Tasks require not just memory, but dynamics-aware belief updates that preserve task progress during stable/occluded phases and revise beliefs near contact/release/subgoal transitions.

Method: TFP is a lightweight memory-action framework for VLA backbones that maintains an episode-local task-progress belief using Liquid Time-Constant dynamics and injects the updated belief into a flow-matching action decoder via adaptive modulation, allowing temporally accumulated context to shape action chunks rather than serving only as passive history.

Result: With a 3.3B-parameter model, TFP improves LIBERO success from 96.9% to 98.75% and LIBERO-plus from 91.4% to 93.77%, achieves 75% success on MIKASA ShellGameTouch, and shows write-gain changes near manipulation events are ~6× larger than non-event phases, with hidden-state interventions confirming belief causally modulates action chunks.

Conclusion: Compact, event-sensitive memory dynamics can significantly improve VLA policies under occlusion, visual perturbation, and stage-dependent task structure by enabling temporally conditioned belief updates that adapt to manipulation events rather than just reactive current observations.
...
### [18] [AnyDexRT: Calibration-Free Dexterous Hand Retargeting with Few-Shot Human Guidance](https://arxiv.org/abs/2607.08341)
*Chenxi Wang,Ying Feng,Hongjie Fang,Shangning Xia,Lixin Yang,Chuan Wen,Cewu Lu*

Main category: cs.RO

TL;DR: AnyDexRT is a calibration-free retargeting method for intuitive dexterous teleoperation across different human-like robotic hands, combining self-supervised fingertip correspondence learning with few-shot human guidance and a contact classifier.


<details>
  <summary>Details</summary>
Motivation: Teleoperation is critical for controlling dexterous robotic hands and collecting demonstrations for imitation learning. Existing retargeting methods require hand-crafted objectives, precise calibration, or global shape matching, making them sensitive to hand-specific tuning and less reliable across different dexterous hands.

Method: AnyDexRT integrates three key components: (1) self-supervised fingertip correspondence learning, (2) few-shot human guidance to anchor mappings in task-relevant regions, and (3) a contact classifier to refine pinch-related poses.

Result: Experiments across diverse dexterous hands and real-world teleoperation tasks demonstrate that AnyDexRT improves retargeting quality, significantly reduces manual tuning requirements, and provides more intuitive and efficient control compared to prior retargeting methods.

Conclusion: AnyDexRT enables calibration-free, intuitive dexterous teleoperation across different robotic hand types with minimal manual tuning, making it a more reliable and adaptable solution for practical applications.
...
### [19] [SkillPlug: Unsupervised Skill Mining for Few-Shot Adaptation in Robotic Manipulation](https://arxiv.org/abs/2607.08354)
*Zi-han Ding,Ziwei Wang*

Main category: cs.RO

TL;DR: SkillPlug is a plug-in framework that augments existing visuomotor policies with skill-conditioning modules and mines transferable behavior primitives from multi-task demonstrations. It enables data-efficient generalization and rapid adaptation to new tasks by fine-tuning only lightweight components while keeping learned skills fixed.


<details>
  <summary>Details</summary>
Motivation: Learning transferable visuomotor policies that generalize across diverse tasks and adapt from few demonstrations is challenging. Existing end-to-end policies are data-inefficient under limited supervision and lack explicit structure for reusing and recombining behaviors across tasks.

Method: SkillPlug adds a skill-conditioning module to existing policies and mines a shared skill library from raw multi-task demonstrations via self-supervised objectives. It promotes compact, reusable, and non-redundant behavior-level primitives. After skill mining, it specializes to unseen tasks by fine-tuning only lightweight router and action head components while keeping learned skills fixed.

Result: Evaluated on two simulation benchmarks and a real robot, SkillPlug shows consistent improvement in both multi-task performance and few-shot adaptation compared to baseline approaches.

Conclusion: SkillPlug provides a scalable approach to mine reusable skills that enhance data-efficient generalization in robotic manipulation, enabling efficient adaptation without full end-to-end retraining.
...
### [20] [FSD-VLN: Fast-Slow Dual-System Modeling for Aerial Long-Horizon Vision-Language Navigation](https://arxiv.org/abs/2607.08359)
*Xueke Zhu,Qingyan Meng,Liutao Yu,Wei Zhang,Zhengyu Ma,Huihui Zhou,Yonghong Tian*

Main category: cs.RO

TL;DR: FSD-VLN is a fast-slow dual-system architecture for UAV Vision-Language Navigation that decouples semantic reasoning and low-latency flight command generation, achieving 2X higher navigation success rates on unseen scenes and cutting inference delay and runtime by over 50%.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods suffer from structural misalignment between global multimodal understanding and sequential action generation, causing jittery trajectories and severe decision latency for long-horizon aerial navigation. VLN requires tight integration of high-level semantic reasoning and low-latency flight control.

Method: FSD-VLN proposes a fast-slow dual-system architecture with two asynchronous branches: (1) a slow stream extracting stable semantic priors from pre-trained vision-language models, and (2) a Diffusion Transformer (DiT) fast stream modeling cross-temporal action distributions to produce consistent flight outputs. A time-aware adaptive optimizer is introduced to stabilize long-sequence training and reduce gradient oscillation.

Result: Large-scale low-altitude simulation experiments demonstrate that FSD-VLN achieves up to 2X higher navigation success rates on unseen scenes compared to SOTA methods, while cutting single-action inference delay by over 50% and total task runtime by more than 50%.

Conclusion: This work validates the benefit of decoupled semantic-control modeling and provides a practical paradigm for long-horizon aerial Vision-Language Navigation.
...
### [21] [On Exploring Input Resolution Scaling For Anytime LiDAR Object Detection](https://arxiv.org/abs/2607.08391)
*Ahmet Soyyigit,Shuochao Yao,Heechul Yun*

Main category: cs.RO

TL;DR: The paper introduces a multi-resolution inference method with a deadline-aware scheduler to enable anytime computing for DNN-based 3D object detection on LiDAR point clouds, using only a single model deployment.


<details>
  <summary>Details</summary>
Motivation: Tradeoffs between execution latency and result utility (anytime computing) enhance performance of cyber-physical systems, particularly for dynamic operational requirements in LiDAR-based 3D object detection.

Method: Multi-resolution inference for point cloud models (pillars or voxels) with dynamic input scaling; memory-efficient single-model deployment; deadline-aware scheduler that predicts execution time for all possible resolutions at runtime to handle LiDAR irregularity.

Result: Significantly outperforms existing anytime computing approaches on nuScenes dataset; enables collision-free navigation in simulated autonomous driving while avoiding stalls from environmental complexity.

Conclusion: The proposed method effectively enables anytime computing for LiDAR-based 3D object detection, balancing latency and utility through intelligent resolution selection and execution time prediction.
...
### [22] [EgoWAM: World Action Models Beyond Pixels with In-the-Wild Egocentric Human Data](https://arxiv.org/abs/2607.08436)
*Baoyu Li,Xinchen Yin,Mengying Lin,Yixin Zhang,Danfei Xu*

Main category: cs.RO

TL;DR: This paper introduces EgoWAM, a framework for training robot manipulation policies from egocentric human data using World Action Models (WAMs) that predict scene evolution alongside actions. It compares Pixel, DINO, and 3D motion flow as world representations, finding that DINO and 3D flow significantly outperform behavior cloning and Pixel-based approaches.


<details>
  <summary>Details</summary>
Motivation: Behavior cloning from egocentric human data is challenging because it entangles transferable content (objects, scenes, task semantics) with non-transferable factors (human morphology, head motion, behavioral style). The authors aim to understand what world representation best enables effective human-to-robot transfer, hypothesizing that an effective world target should abstract appearance, capture agent-invariant physical effects, and separate camera motion from environment change.

Method: The researchers introduce EgoWAM, a controlled human-robot co-training framework that fixes the policy backbone, action head, and data mixture while varying only the world prediction target. They compare three world representations: Pixel-based prediction, DINO (a self-supervised vision transformer), and 3D motion flow. The framework is evaluated across three real-world bimanual tasks using in-the-wild egocentric human data.

Result: WAM co-training scales more effectively with egocentric human data than behavior cloning. Pixel-based prediction transfers weakly, while DINO and 3D flow yield substantial gains: DINO improves out-of-distribution object and scene generalization by up to 4x, and 3D flow improves in-domain performance by 20-30%.

Conclusion: The findings demonstrate that incorporating scene evolution prediction (World Action Models) significantly enhances robot manipulation learning from human demonstrations. Both DINO and 3D flow representations provide effective training signals that separate transferable content from non-transferable factors, making human-to-robot transfer more scalable and robust.
...
### [23] [Harness VLA: Steering Frozen VLAs into Reliable Manipulation Primitives via Memory-Guided Agents](https://arxiv.org/abs/2607.08448)
...
TL;DR: Harness VLA is a memory-augmented framework that combines a frozen VLA model with analytic primitives to extend pretrained VLA capabilities beyond their original training distribution for robust language-conditioned manipulation.


<details>
  <summary>Details</summary>
Motivation: Language-conditioned manipulation requires precise contact-rich control and robust reasoning over language, scenes, and long horizons. Current approaches have limitations: end-to-end VLA models fail under deployment perturbations (semantic retargeting, goal re-binding, spatial-layout shifts, unstable local contacts), while LLM coding agents provide semantic reasoning but struggle with irregular grasping, constrained placement, and articulated-object interaction.

Method: Harness VLA exposes a frozen VLA as a retryable contact-rich primitive composed with a small fixed library of analytic primitives for grounding, staging, transport, navigation, and release. The framework learns operating ranges from execution traces, success rules, and failure models. It lifts semantic re-grounding, non-contact execution, and VLA re-staging to the planner while reserving the frozen VLA for local contact-rich phases, extending pretrained VLAs without finetuning.

Result: Across perturbed tabletop, household kitchen, and clean-to-randomized bimanual manipulation, Harness VLA improves over baselines by 38.6 percentage points on LIBERO-Pro and 25.4 percentage points on RoboCasa365, reaching 58.4% on RoboTwin C2R.

Conclusion: By composing frozen VLA models with analytic primitives and handling semantic re-grounding at the planner level, Harness VLA successfully extends pretrained VLAs beyond their original trajectory distribution, achieving significant performance improvements across multiple benchmarks without requiring finetuning.
...
### [24] [FabriVLA: A Lightweight Vision-Language-Action Model for Precise Multi-Task Manipulation](https://arxiv.org/abs/2607.08575)
...
### [25] [A New Human-Likeness and Comfort Index for Robot Movements Along Prescribed Paths](https://arxiv.org/abs/2607.08620)
*Rosanna Coccaro,Enrico Ferrentino,Antonio Parziale,Angelo Marcelli,Pasquale Chiacchio*

Main category: cs.RO

TL;DR: This paper defines a human-likeness index based on the lognormality principle to characterize robot trajectories and evaluates its correlation with human-perceived comfort during physical interaction, demonstrating that comfort preference aligns with the index values.


<details>
  <summary>Details</summary>
Motivation: Visual similarity alone is insufficient to ensure robot acceptance in physical interaction; acceptance depends on comfort and ergonomics, which are measured by how humans perceive robot movement quality.

Method: The study investigates the relationship between comfort and movement similarity by analyzing kinematic characterization of human movement, focusing on time laws of movements with prescribed end-effector paths. Based on the lognormality principle for modeling human movements, a human-likeness index is defined to provide an a priori characterization of trajectories. This index enables evaluation of trajectory generation algorithms' performance in producing human-like movements.

Result: Three experimental campaigns with 68 subjects physically interacting with a robot show a globally consistent trend between perceived comfort preference and the distribution of the suggested human-likeness index.

Conclusion: The human-likeness index can effectively evaluate trajectory generation algorithms' performance in creating human-like movements before actual execution, providing a useful tool for assessing robot movement comfort and ergonomics.
...
### [26] [Native Video-Action Pretraining for Generalizable Robot Control](https://arxiv.org/abs/2607.08639)
...
TL;DR: LingBot-VA 2.0 is a video-action foundation model specifically designed for robot control, addressing limitations of repurposing video generative models by introducing a semantic visual-action tokenizer, causal pretraining, sparse MoE backbone, and enhanced asynchronous inference scheme, achieving robust few-shot generalization across complex manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: The authors argue that repurposing video generative models designed for digital content creation is inherently inadequate for physical environments, creating a gap that needs to be bridged with a model built from the ground up for embodiment.

Method: Four core design principles: (1) Semantic visual-action tokenizer aligning visual representations with both semantics and actions (replacing reconstruction-focused VAEs), (2) Causal pretraining paradigm for strictly causal temporal dynamics, (3) Sparse Mixture-of-Experts (MoE) backbone for high-frequency inference, (4) Enhanced asynchronous inference scheme for real-time closed-loop control with parallel latent prediction and action execution.

Result: Real-world deployment validates LingBot-VA 2.0 as a robust foundation model with few-shot generalization across complex manipulation tasks.

Conclusion: LingBot-VA 2.0 successfully demonstrates that a purpose-built video-action foundation model can achieve robust performance in embodied control tasks, overcoming the limitations of adapting video generative models for physical environments.
...
### [27] [Learning Adaptive Solvers for Distributed Factor Graph Optimization on Matrix Lie Groups](https://arxiv.org/abs/2607.08735)
*Jaeho Shin,Maani Ghaffari,Yulun Tian*

Main category: cs.RO

TL;DR: DeepCORD is a learning-augmented framework for distributed factor graph optimization on general matrix Lie groups (e.g., SE(3), SL(4)). It learns a self-supervised feedback policy to dynamically adapt solver parameters based on optimization phase and communication status, outperforming existing distributed baselines in SE(3) pose graph optimization and SL(4) projective submap alignment.


<details>
  <summary>Details</summary>
Motivation: Modern robotic perception requires large-scale geometric optimization across distributed robots/sessions. Existing distributed solvers rely on brittle hand-tuning and primarily target rigid body pose graphs (SE(3)), lacking adaptability to general matrix Lie groups and communication regimes.

Method: DeepCORD unfolds a parallel, accelerated Riemannian optimizer into differentiable iterations and learns a self-supervised feedback policy. This policy dynamically adjusts solver parameters according to optimization phase and communication status, supporting both synchronous and asynchronous communication regimes for general matrix Lie groups.

Result: Extensive experiments on SE(3) pose graph optimization and SL(4) projective submap alignment demonstrate that DeepCORD achieves lower objective values than existing distributed baselines across most benchmarks in realistic operating scenarios.

Conclusion: DeepCORD successfully enables adaptive distributed optimization over general matrix Lie groups under both synchronous and asynchronous communication regimes, outperforming traditional distributed solvers in objective value minimization for robotic perception tasks.
...
### [28] [ContactMimic: Humanoid Object Interaction via Contact Control](https://arxiv.org/abs/2607.08742)
*Xinyao Li,Xialin He,Runpei Dong,Saurabh Gupta*

Main category: cs.RO

TL;DR: CONTACTMIMIC is a learning framework that jointly tracks keypoint trajectories and part-level binary contact commands, enabling robots to achieve precise, controllable physical contact for manipulation tasks like sitting, wiping, and pushing objects.


<details>
  <summary>Details</summary>
Motivation: Keypoint tracking alone is insufficient for object interaction tasks where robots must make meaningful physical contact (e.g., sitting on a chair, wiping a board, pushing furniture) but can reach correct poses without contact.

Method: A learning framework that tracks explicit part-level binary contact commands alongside keypoint trajectories, using contact-following rewards and a trajectory augmentation scheme designed to break correlations between keypoint trajectories and contact labels.

Result: The resulting policy successfully decouples contact behavior from keypoint geometry, achieving precise physical contact and contact-controllability (ability to produce or suppress contact as desired). It outperforms keypoint-only trackers on contact-relevant tasks across 10 diverse motions in simulation and validates controllability in the real world across 5 different motions.

Conclusion: CONTACTMIMIC enables robots to complete manipulation tasks without task-specific rewards while maintaining precise physical contact, demonstrating the importance of explicit contact modeling for object interaction tasks.
...
### [29] [DexVerse: A Modular Benchmark for Multi-Task, Multi-Embodiment Dexterous Manipulation](https://arxiv.org/abs/2607.08751)
...
TL;DR: DexVerse is a large-scale, modular benchmark for dexterous manipulation comprising 100 diverse tasks across multiple robot embodiments (3 arms + 6 hands), configurable visual variations, and 3,180 demonstrations with multi-modal observations. Benchmarking state-of-the-art methods reveals significant challenges in task generalization and visuomotor robustness, establishing it as a comprehensive testbed for general-purpose dexterous manipulation research.


<details>
  <summary>Details</summary>
Motivation: General-purpose dexterous manipulation policies require benchmarks that systematically evaluate policies across diverse interaction modes, sensory conditions, and robot embodiments. Existing benchmarks are limited in task and data diversity, embodiment coverage, and controllable visual variation, hindering studies of cross-task and cross-embodiment generalization.

Method: DexVerse is a large-scale, modular benchmark featuring: (1) 100 tasks spanning grasping, articulated interaction, tool use, bimanual coordination, non-prehensile control, contact-rich behaviors, multi-goal execution, and long-horizon tasks; (2) support for 3 robot arms and 6 dexterous hands; (3) configurable visual variations in textures, background, lighting, and camera viewpoints; (4) VR-based teleoperation interface; (5) 3,180 demonstrations with synchronized proprioceptive, RGB, depth, point-cloud, and state observations.

Result: Benchmarking representative methods (Diffusion Policy, DP3, OpenVLA, π0.5) across 19 tasks reveals substantial challenges in task generalization and visuomotor robustness, demonstrating the complexity of achieving true general-purpose dexterous manipulation capabilities.

Conclusion: DexVerse establishes itself as a comprehensive and promising testbed for general-purpose dexterous manipulation research, providing the necessary diversity and controllability to advance the field toward robust, cross-task, and cross-embodiment generalization.
...
### [30] [HoloTetSphere: Unified TetSphere Mesh Reconstruction for Physical Simulations](https://arxiv.org/abs/2607.08398)
*YaQiao Dai,Renjiao Yi,Zhirui Gao,Wei Chen,Kai Xu,Chenyang Zhu*

Main category: cs.GR

TL;DR: A topology-adaptive framework for holistic tetrahedral mesh reconstruction that directly optimizes tetrahedral elements end-to-end through coupling Gaussian spheres to tetrahedral elements, continuous opacity field estimation for differentiable pruning, and joint minimization of mesh smoothing energy and multi-view Gaussian rendering error.


<details>
  <summary>Details</summary>
Motivation: Standard pipelines for physics-ready 3D reconstruction rely on a decoupled two-stage paradigm (surface geometry extraction followed by error-prone tetrahedralization). Recent Lagrangian methods like TetSphere Splatting attempt to bypass this by directly optimizing volumetric primitives, but their homeomorphic constraints prevent topology-adaptive optimization, resulting in disjoint tetrahedra rather than a single connected mesh.

Method: The authors propose a topology-adaptive framework for holistic tetrahedral mesh reconstruction through end-to-end topological and geometric optimization. First, Gaussian spheres are coupled to tetrahedral elements and edge connections are leveraged to estimate a continuous opacity field for differentiable element pruning. Next, mesh smoothing energy and multi-view Gaussian rendering error are jointly minimized to drive alternating geometric refinement while preserving topological adaptivity.

Result: The method effectively constructs a unified and topologically coherent tetrahedral mesh, outperforms state-of-the-art techniques in geometric accuracy, and produces coherent, single-connected tetrahedral meshes.

Conclusion: The approach effectively bypasses the error-prone conventional tetrahedralization step for reconstructed surface meshes and streamlines downstream physical simulation, producing meshes suitable for further physical simulations.
...
### [31] [ARDY: Autoregressive Diffusion with Hybrid Representation for Interactive Human Motion Generation](https://arxiv.org/abs/2607.08741)
*Kaifeng Zhao,Mathis Petrovich,Haotian Zhang,Tingwu Wang,Siyu Tang,Davis Rempe*

Main category: cs.GR

TL;DR: ARDY is a streaming framework for real-time, high-fidelity 3D human motion generation that combines explicit root features with latent body embeddings and uses a two-stage autoregressive transformer denoiser to enable text and kinematic constraint control, bridging the gap between offline precision and online speed.


<details>
  <summary>Details</summary>
Motivation: Real-time 3D human motion generation is critical for animation, simulation, and humanoid robotics. Offline methods lack the inference speed needed for interactive settings, while online methods sacrifice controllability or struggle with complex text semantics and long-horizon goals due to limited context windows.

Method: ARDY employs a hybrid representation combining explicit root features with latent body embeddings, and a two-stage autoregressive transformer denoiser with variable history context. It supports conditioning on flexible, long-horizon kinematic constraints and is trained on large-scale motion capture data conditioned on both text labels and ground truth kinematic constraints.

Result: Extensive evaluations on HumanML3D and the Bones Rigplay dataset demonstrate ARDY's high motion quality and constraint adherence. An interactive demo shows practical versatility with dynamic text control, keyframe pose constraints, path following, and mouse/keyboard locomotion control.

Conclusion: ARDY effectively bridges the gap between real-time generation speed and precise control capabilities, validating the efficacy of its key architectural decisions for interactive applications requiring text and kinematic constraint control.
