...
TL;DR: APPROVE is an LLM-based multi-modal end-user programming framework that combines natural language input with a block-based interface and explicit user confirmation, enabling transparent, aligned, and reusable robot programming for non-experts.


<details>
  <summary>Details</summary>
Motivation: Programming robots remains challenging for non-experts as traditional methods require expert knowledge and even block-based interfaces lack flexibility. Existing LLM-based systems suffer from lack of transparency, missing mechanisms to ensure alignment with user intent, and little support for program reuse.

Method: APPROVE integrates natural language input with a block-based interface (Blockly) and an explicit user confirmation step. Generated programs are visualized, allowing users to confirm, modify, or reject them before execution. Confirmed functions are stored in a library for reuse, gradually building a set of reliable program components.

Result: The approach demonstrates a human-centered design for LLM-based robot programming that emphasizes user trust, intent alignment, and reusability through its integrated framework.

Conclusion: APPROVE addresses key limitations of current LLM-based robot programming systems by providing transparency through visual program representation, ensuring alignment with user intent through explicit confirmation steps, and enabling program reuse through a component library system.
...
TL;DR: This paper demonstrates that spatially distributed tactile feedback significantly improves teleoperation performance by enabling operators to feel remote contact information, reducing task completion steps and bringing teleoperated movements closer to natural human behavior.


<details>
  <summary>Details</summary>
Motivation: Teleoperated robot performance falls short of human dexterity even for basic tasks, and a key contributing factor is the absence of spatially distributed tactile feedback, which limits operators' ability to control robots intuitively.

Method: Researchers used a two-degree-of-freedom bilateral force-feedback telemanipulator paired with a 32-DoF tactile fingertip display to test how reproducing localized deformations on remote manipulators affects operator performance across various teleoperation tasks.

Result: Operator performance improved significantly with spatially distributed tactile feedback: task performance accelerated, corrective actions and completion steps were minimized, and deviation between teleoperated and natural trajectories was reduced by 29-79%. Higher tactile resolution compressed the state-space distribution of teleoperated motions.

Conclusion: Spatially distributed tactile feedback is essential for closing the gap between human and teleoperated dexterity and for training the next generation of autonomous robots.
...
TL;DR: A neural reduced dynamics (NRD) framework learns a control-relevant abstraction that preserves physics from high-fidelity simulators, enabling high-throughput policy training in fast models that transfer back to the original simulator.


<details>
  <summary>Details</summary>
Motivation: High-fidelity embodied AI simulators are computationally expensive and limit large-scale reinforcement learning campaigns; the authors advocate for data-driven, faster simulations using neural dynamics models.

Method: Develops a neural reduced dynamics (NRD) framework that learns a reduced state abstraction preserving control-relevant physics, separates propagated state from input/recoverable states, trains policies in a frozen learned model, and validates them in the high-fidelity simulator.

Result: A single policy trained in the terrain-conditioned dynamics model achieves lower tracking error than single-terrain specialists on all terrains (including zero-shot on bumpy terrain); tracked vehicle reaches 100/100 goals, arm reaches 97/100 goals with zero violations; NRD models run ~4 orders of magnitude faster than high-fidelity simulators.

Conclusion: NRD models provide a practical bridge between accurate but expensive physics simulation and scalable robot learning, making iterative on-policy learning feasible.
...
TL;DR: SCAPE is a scenario-conditioned simulation-augmented policy evaluation framework that predicts real-world policy performance using limited paired sim-and-real samples and large-scale simulation rollouts, correcting sim-to-real bias and calibrating uncertainty through conformal prediction to enable fine-grained deployment strategies.


<details>
  <summary>Details</summary>
Motivation: Reliable performance evaluation is a bottleneck for deploying robot-learning policies in real-world conditions. Real-world testing is faithful but costly and difficult to scale, while simulation-based testing scales easily but is inevitably biased by the sim-to-real gap. Existing simulation-augmented methods focus on population-level averages, obscuring scenario-specific variation and providing limited guidance about when and where a policy can be safely deployed.

Method: SCAPE uses limited paired sim-and-real samples and large-scale simulation rollouts to predict scenario-conditioned real-world policy performance. The framework corrects sim-to-real bias in simulation labels before training the prediction model and calibrates prediction uncertainty through conformal prediction.

Result: In sim-to-sim studies, SCAPE reduces scenario-level prediction error by 4.9%/34.7% (driving) and 14.5%/27.7% (quadruped) relative to scene-conditioned neural and aggregate statistical baselines on average. On a physical Unitree Go2 robot, SCAPE improves testing sample efficiency, produces narrower calibrated prediction intervals, generalizes better to out-of-distribution scenarios, and enables fine-grained deployment strategies.

Conclusion: SCAPE effectively reduces prediction error compared to baselines, provides better sample efficiency, delivers better calibrated uncertainty estimates, and offers more precise deployment guidance for real-world robot policies.
...
TL;DR: Feedback Sampling MPC (FS-MPC) improves sample efficiency and stability of sampling-based MPC for high-dimensional unstable systems by using an optimized feedback policy to guide sampling, balancing local and global search based on stability and computation budget. It achieves faster convergence than MPPI and better optimality than standard feedback sampling, outperforming on contact-rich tasks including humanoid manipulation.


<details>
  <summary>Details</summary>
Motivation: Sampling-based MPC is popular for robotics but suffers from exponential sample growth and numerical instability for high-dimensional, open-loop unstable systems, requiring poor sample efficiency.

Method: Proposes Feedback Sampling MPC (FS-MPC) that uses an optimized feedback policy to realize optimal sampling proposal distribution. Implements a hybrid sampling design balancing local and global search based on system stability and computation budget.

Result: Theoretically, hybrid sampling achieves faster convergence than standard MPPI and better optimality than standard feedback sampling. Empirically, FS-MPC handles dynamically unstable contact-rich tasks (humanoid loco-manipulation, dexterous manipulation) where standard approaches struggle and strictly outperforms feedback policies alone.

Conclusion: FS-MPC successfully tackles dynamically unstable tasks with superior sample efficiency and optimality compared to standard sample-based approaches and feedback policies, validated on real-world humanoid robot locomotion and manipulation tasks.
...
TL;DR: SAM-TD is a compilation method that enforces Linear Temporal Logic (LTL) constraints in stream-based robotics Task and Motion Planning by embedding automaton guards into action schemas and using shared validity tokens to prune constraint-violating branches.


<details>
  <summary>Details</summary>
Motivation: Stream-based TAMP typically only considers goal reachability, missing temporal specifications like safety-critical ordering, invariance, and liveness constraints. Existing temporal-logic compilation techniques are incompatible because streams generate expanding geometric object sets via iterative stream refinement loops.

Method: SAM-TD translates arbitrary LTL_f constraints into automata and embeds regressed automaton guards into pre-specified action schemas. During search, it synchronously updates automaton states and uses a shared validity token across all automata to prune constraint-violating branches, handling dynamically generated stream objects without enumerating a fixed object set or modifying the underlying planner.

Result: SAM-TD demonstrates the first stream-based TAMP under LTL_f constraints in three robotics PDDLStream environments. On standard discrete PDDL benchmarks, SAM-TD is competitive with state-of-the-art temporal-constraint compilation methods.

Conclusion: SAM-TD successfully supports dynamically generated stream objects from iterative stream refinements during plan search, effectively extending stream-based TAMP to support arbitrary LTL_f temporal constraints while maintaining competitive performance on standard benchmarks.
...
TL;DR: This paper presents a self-supervised finetuning method for VLA models that generates online interaction rollouts as training data, enabling policies that maintain prior knowledge while learning new skills with improved sample efficiency.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art VLA models like π_0.5 perform well but suffer severe performance drops on new robots due to hardware mismatches. Fine-tuning on in-domain expert data improves task performance but degrades the model's original instruction following and behavioral priors.

Method: A self-supervised method that generates online interaction rollouts from the zero-shot VLA model as additional training data for fine-tuning.

Result: The approach yields strong multi-task policies that (1) inherit prior tasks distilled from the zero-shot model, (2) enable generalist instruction following, and (3) learn new skills from expert data with improved sample efficiency. Validated on a real ALOHA robot and RoboTwin simulation benchmark.

Conclusion: The self-supervised finetuning scheme successfully balances the trade-off between maintaining original behavioral priors and learning new skills, demonstrating strong multi-task capabilities and generalization across different embodiments.
...
TL;DR: A parameter-free LiDAR odometry approach (LF-GICP) using a voxel-normal localizability field with two statistics (normalized fraction f0 and absolute per-voxel mass λ0) to handle degeneracy in tunnels/corridors without environment-specific tuning.


<details>
  <summary>Details</summary>
Motivation: Scan-to-map LiDAR odometry drifts unboundedly in geometrically degenerate environments (tunnels, corridors). Existing degeneracy handling methods require environment-specific parameter tuning and fail to distinguish between different types of degeneracy (information absence vs dilution).

Method: LF-GICP uses a voxel-normal localizability field with two statistics: f0 (detects directional anisotropy) and λ0 (distinguishes information absence from dilution). A temporal-median gate combines these to trigger Fisher-information correspondence weighting. Calibrated once on short sequences, then frozen for parameter-free operation.

Result: LF-GICP achieves the lowest KITTI relative translation error (0.865%), outperforms baselines on GEODE tunnels and MulRan, leads HeLiPR mean, and generalizes across four sensor types without re-tuning.

Conclusion: The approach successfully handles different types of degeneracy (unobservable tunnels vs dilution in dense scenes) and generalizes well across environments and sensor types without requiring parameter tuning.
...
TL;DR: This paper presents a guide path-free multimodal trajectory planning framework for autonomous surface vehicles in dynamic environments, integrating MPC with a turning circle-based CBF that accounts for nonholonomic motion and finite turning capability.


<details>
  <summary>Details</summary>
Motivation: Conventional Euclidean distance-based CBFs (ED-CBFs) evaluate safety solely based on proximity, which fails to account for surface vehicles' nonholonomic motion and finite turning capability. Additionally, many conventional multimodal planning approaches rely on globally planned guide paths, which can cause local-minimum and deadlock problems in single-mode MPC.

Method: The method integrates model predictive control (MPC) with a turning circle-based control barrier function (TC-CBF). Unlike ED-CBFs, TC-CBF accounts for nonholonomic motion and finite turning capability through a geometric formulation that identifies feasible avoidance regions according to the vehicle's turning circles and generates distinct left- and right-turning avoidance modes. The avoidance direction is embedded directly into the safety constraint.

Result: Extensive simulations with multiple moving vessels demonstrate that the proposed method achieves higher success rates, fewer safety violations, and smaller residual violations than single-mode baselines across all tested traffic densities.

Conclusion: The framework alleviates local-minimum and deadlock problems of single-mode MPC while maintaining computational efficiency, providing superior performance in dynamic maritime environments.
...
TL;DR: HiTac-WAM is a hierarchical tactile world action model that forecasts future tactile states (contact, deformation, slip) for action planning and execution, improving real-robot success rates from 31.1% to 61.1% (full system 72.2%).


<details>
  <summary>Details</summary>
Motivation: Existing tactile-aware action models represent touch as images or latent streams without modeling the physical dependencies that organize tactile states hierarchically. World action models jointly predict future visual observations and actions, but tactile variants lack this hierarchical modeling of tactile states.

Method: HiTac-WAM forecasts future tactile states for each candidate action chunk before execution, factorizing into contact state, 3D deformation field, and slip risk organized as a directed hierarchy. Uses stop-gradient signals for conditioning, directed attention masks for query relationships, and ranks action chunks using tactile forecasts and task-progress estimates. Execution uses the selected forecast as a reference and triggers corrective replanning upon discrepancies.

Result: Achieves mean contact F1 of 0.921. Directed hierarchy reduces 3D displacement L2 error by 17.6% vs deformation-only predictor and improves slip AUPRC by 60.4% vs slip-only predictor. Selection guided by hierarchical forecasts increases average real-robot success rate from 31.1% to 61.1%; full system achieves 72.2% success rate across chip grasping, blackboard erasing, and USB insertion tasks.

Conclusion: Hierarchical tactile modeling significantly improves tactile forecasting accuracy and real-robot manipulation performance by explicitly modeling the physical dependencies among tactile states, enabling more effective action selection and execution.
...
TL;DR: OrthoSkillVLA is a parameter-efficient framework for continual skill learning in pretrained Vision-Language-Action (VLA) models without demonstration replay. It uses separate subspace constraints for the Vision-Language Model (VLM) and ActionHead to preserve semantic capacity while protecting localized velocity patterns, and introduces a lightweight feature-aware Mixture-of-Experts (MoE) decoder for the output layer with a training-free router based on feature-space affinity.


<details>
  <summary>Details</summary>
Motivation: Pretrained Vision-Language-Action (VLA) models provide strong foundations for robot learning but suffer from catastrophic forgetting when sequentially adapted to diverse skills. Existing solutions have limitations: architecture-based approaches increase inference footprint, while subspace-constrained methods impose unified constraints on the entire model. The authors identify two VLA-specific challenges: (1) the VLM maintains broad semantic representations vulnerable to capacity exhaustion, while the ActionHead refines semantics into localized velocity patterns sensitive to perturbations; (2) the final velocity decoder serves as a readout layer where freezing creates an output-stage expressivity bottleneck while updating risks overwriting previous velocity mappings.

Method: OrthoSkillVLA is proposed as a parameter-efficient framework for continual skill learning in pretrained VLA models without demonstration replay. The method consists of: (1) Separate subspace constraints on the VLM and ActionHead to address representation heterogeneity - preserving reusable semantic capacity in the VLM while protecting localized velocity patterns in the ActionHead; (2) A lightweight feature-aware Mixture-of-Experts (MoE) decoder for the output layer, where each skill is allocated a compact expert and a training-free router selects the expert according to feature-space affinity.

Result: Extensive simulated and real-world evaluations, together with ablation studies, demonstrate that OrthoSkillVLA effectively preserves prior skills while acquiring new ones, showing superior performance compared to baseline methods.

Conclusion: The proposed OrthoSkillVLA framework successfully addresses the challenge of catastrophic forgetting in pretrained VLA models through its dual approach of component-specific subspace constraints and a feature-aware MoE output decoder. This enables continual skill learning without demonstration replay, balancing the preservation of previously learned skills with the acquisition of new capabilities.
...
TL;DR: This paper introduces a Post-Semantic Communication Framework to address the verification gap in networked Physical AI, where proposals require additional evidence and authorization before becoming physical actions.


<details>
  <summary>Details</summary>
Motivation: In networked Physical AI, a task-effective proposal is not yet a justified physical action because the required evidence or authority for finalization remains unavailable. This mismatch is termed the "verification gap" between proposal formation and physical execution.

Method: A Post-Semantic Communication Framework is proposed with the following components: application-declared evidence requirements, evidence records representing qualifying observations, validation of supporting/conflicting records through a single path, separation of evidence sufficiency from authorized finalization and a downstream runtime gate, and a distinction between evidence transfer (expands record set) and evidence coordination (suppresses redundant payload). Finite-state framework checks ensure consistent implementation.

Result: A controlled communication study reveals a finalizer-dependent asymmetry: sender-finalized Feedback uses evidence transfer to expand evidence reachability throughout the feasible plotted region, while receiver-finalized Feedback uses coordination to suppress redundant payload. Selection shifts to One-way when loss, latency, freshness, and deadline costs become significant.

Conclusion: An episode-level reporting schema is defined to establish common denominators for future measured Physical-AI studies, enabling consistent evaluation across different systems.
...
TL;DR: An integrated magnetorheological fluid (MRF)-based haptic system featuring a compact magnetically self-sealed rotary actuator, low-hysteresis PWM operation, and high-fidelity model-based torque rendering for safe human-machine interaction.


<details>
  <summary>Details</summary>
Motivation: Accurate and stable torque rendering is essential for safe and perceptive human-machine interaction. MRF-based actuators offer a compact and rapidly controllable solution but require reliable fluid sealing, low-hysteresis excitation, accurate torque control, and stable long-duration operation.

Method: Magnetostatic simulation guides material arrangement to focus flux in multidisk torque and permanent-magnet sealing regions. Low-hysteresis PWM operation is implemented. Real-time controller combines feedforward, hysteresis compensation, PI feedback, and sliding-mode correction.

Result: Maximum 600 N·mm/A output achieved. Higher PWM frequencies reduce hysteresis and improve repeatability. At 10 kHz, the response is represented by a nonlinear model varying with torque direction and speed. Compared with PID, the proposed controller reduces square-wave overshoot (77.4%), undershoot (61.9%), and steady-state RMSE (68.3%). A 1.5-hour test shows only 2.5°C temperature rise with no tracking loss.

Conclusion: This high-fidelity torque rendering will fundamentally transform human-robot collaboration by making interactions safer, more efficient, and more intuitive.
...
TL;DR: This paper proposes a physics-grounded neural world model to enhance large language model-based robot planners, enabling marine vehicles (AUV and ASV) to execute collision-free navigation missions near offshore wind infrastructure with 70-93% improvement in goal accuracy over ungrounded baselines.


<details>
  <summary>Details</summary>
Motivation: Large language models can generate robot action sequences from natural language but lack physical grounding - they cannot judge command duration or predict collisions. The authors aim to give these planners a sense of physics to enable safe, accurate robot operation.

Method: Three components: 1) Physics-grounded neural world model, 2) Three-phase gradient-based trajectory optimizer, 3) MPC-style closed-loop replanner with trust-region guard. The language model decides what to do while the world model determines timing and physics constraints.

Result: Evaluated on 6-DOF AUV and 3-DOF ASV with five benchmark missions each. Both reached every goal with zero predicted collisions, transferred successfully to GazeboSim with ocean currents/waves, reduced goal-distance error by 70-93% vs baseline, and after fine-tuning reduced surrogate rollout RMSE by 60-69%. VLM-assisted semantic-mapping achieved 96% navigability accuracy.

Conclusion: The physics-grounded approach significantly outperforms ungrounded large language model planners across different marine vehicle platforms, enabling robust navigation in complex offshore wind infrastructure environments with precise timing and collision avoidance.
...
TL;DR: SAGE (Semantic and Adaptive Generative Ergodicity) is an ergodic-control architecture that dynamically allocates inspection time to underwater valve structures based on real-time risk assessments rather than fixed routes. In a five-valve scenario, SAGE revisits high-risk valves every 5.8 seconds compared to a fixed 8.1-second interval for all valves under A* planning, enabling faster leak detection while requiring no operator re-planning.


<details>
  <summary>Details</summary>
Motivation: Subsea Christmas Trees (XTs) require constant inspection, but risk varies across valves over time—valves with suspected leaks need more frequent revisits than those with clean histories, and risk patterns change dynamically during missions as new leaks are discovered. Fixed inspection routes cannot adapt to this evolving risk distribution efficiently.

Method: SAGE implements an ergodic-control architecture that allocates vehicle time proportionally to a live, sensor-derived risk distribution instead of scripted routes. The authors study a two-XT scenario with five valves, comparing SAGE against a fixed-loop A* tour and deriving the underlying ergodic control law.

Result: Simulation results show that SAGE revisits the dominant-risk valve every 5.8 seconds versus 8.1 seconds for all valves under A* planning. Newly detected leaks cause immediate behavior changes in the next control cycle without explicit re-planning, whereas fixed routes require discrete re-routing. The ergodic approach enables faster leak detection (62% faster) while maintaining similar total inspection time near high-risk valves.

Conclusion: Ergodic control provides a robust, operator-independent mechanism for adaptive subsea inspection that dynamically responds to changing risk distributions. By eliminating the need for re-planning or operator intervention when new information emerges, SAGE achieves faster leak detection while maintaining efficient resource allocation, making it particularly valuable for long-duration underwater missions where conditions evolve.
...
TL;DR: This paper presents an open-source ROS 2 software stack for the Franka Emika Panda robot that fixes unreliable external position control by addressing timing issues through an asynchronous hardware interface, rate-matching mechanism, and position-domain reference generation.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the long-standing unreliability of the Franka Emika Panda robot's external position control interface when using ROS 2, which hinders the development of modern, reliable human-robot collaboration applications.

Method: The authors analyze root causes of unstable position control (timing of external control loop and sampling jitter), then introduce an asynchronous hardware interface to decouple real-time communication from the ROS 2 control loop, a rate-matching mechanism for slower command sources, and a position-domain reference generation strategy for reliable, smooth commands.

Result: Experimental validation demonstrates reliable velocity reference tracking with reduced motion artifacts. The stack is validated across motion planning, compliance control, position-controlled manipulation, and haptic teleoperation on two independent Panda platforms.

Conclusion: By restoring a modern, reliable, and open ROS 2 ecosystem for the Panda, this work lowers the barrier to developing safe, responsive, and reproducible human-robot collaboration applications that integrate planning, perception, interaction, and shared autonomy.
...
TL;DR: GOAG is a generative grasp planner that learns gripper contact surface distributions in a latent space, enabling object-agnostic grasp generation by only introducing object features at inference time, achieving 86.93% success on MultiDex dataset without object-specific training data.


<details>
  <summary>Details</summary>
Motivation: Current deep-learning grasp planners struggle to generalize to new objects because they require object-specific training datasets; the authors observe that gripper and object share identical surface geometry at contact points, which suggests a more fundamental approach to grasp planning.

Method: GOAG learns a compact latent representation of a gripper's contact surface distribution using a deep generative model. At inference time, object features are introduced only to retrieve admissible contact areas compatible with gripper capabilities, without requiring object-specific training data.

Result: GOAG achieves state-of-the-art results on the MultiDex dataset with 86.93% average success rate, processes grasp generation significantly faster than trained methods, and demonstrates effectiveness across different grippers in both simulated and real-world scenarios.

Conclusion: Object-agnostic learning via generative models effectively addresses generalization challenges in robotic grasping, providing a more flexible approach than traditional data-driven methods that require object-specific training data.
...
TL;DR: CoToGrasp is a novel object-agnostic generative framework that synthesizes diverse, stable grasps conditioned on specific contact topologies via a feature-based canonical workspace, enabling zero-shot generalization to unseen objects and outperforming existing taxonomy-guided planners on the DexGraspNet dataset.


<details>
  <summary>Details</summary>
Motivation: Current dexterous grasp planners primarily optimize for physical stability (whether an object can be grasped) rather than functional tasks. Taxonomy-guided grasp synthesis typically requires prohibitively expensive, object-annotated datasets, creating a significant data collection bottleneck.

Method: CoToGrasp proposes a novel generative framework trained entirely in an object-agnostic manner. Key innovations include: (1) a feature-based canonical workspace that projects local object features into a unified gripper-centric domain, effectively decoupling semantic functional intent from arbitrary object geometry; (2) learning the intrinsic contact manifold of the gripper within this workspace; and (3) synthesizing grasps strictly conditioned on specific contact topologies.

Result: Extensive evaluations on the large-scale DexGraspNet dataset demonstrate that CoToGrasp achieves state-of-the-art performance, outperforming existing taxonomy-guided planners. The model achieves zero-shot generalization to unseen objects at inference. Physical viability and kinematic feasibility of the synthesized contact topologies are verified on a physical robot platform.

Conclusion: CoToGrasp effectively decouples semantic functional intent from arbitrary object geometry and achieves superior grasp synthesis performance without requiring expensive object-annotated datasets, demonstrating practical viability on physical robots and establishing a new benchmark for taxonomy-guided grasp planning.
...
TL;DR: MILD is a physics-grounded discrete-element contact solver combined with a terrain-aware locomotion controller trained via deep reinforcement learning, enabling bipedal robots to walk on yielding terrain with effective online terrain identification and adaptation.


<details>
  <summary>Details</summary>
Motivation: Walking on yielding terrain is critical for disaster response and planetary exploration, but current simulators fail to capture the spatiotemporal heterogeneity of deformable surfaces, limiting bipedal robots' locomotion capabilities on such terrain.

Method: MILD combines a physics-grounded discrete-element contact solver for accurate foot-terrain interaction simulation with a terrain-aware locomotion controller trained via deep reinforcement learning using latent modulation and proprioceptive estimation.

Result: The approach generates more diverse and realistic contact scenarios during training, producing controllers that exhibit natural adaptation on real deformable surfaces. Hardware experiments demonstrate successful online terrain identification and adaptation across various surface stiffness levels.

Conclusion: The system effectively enables bipedal robots to navigate yielding terrain by combining physics-based simulation with reinforcement learning, achieving superior adaptability compared to state-of-the-art methods.
...
TL;DR: A 3D keypoint-based modular learning framework for autonomous assembly manipulation that learns assembly dependencies from RGB-D input to infer actionable outputs, evaluated against object-centric baselines on an assembly pose estimation dataset.


<details>
  <summary>Details</summary>
Motivation: Modern computer vision has enabled partial autonomy in robotic assembly manipulation, but autonomous manipulation of progressive assemblies requires more specific skills beyond object perception. Through comparative analysis, the authors deduce that object-centric perception must advance to learn assembly dependencies for meaningful actionable outputs.

Method: A 3D keypoint-based modular learning framework is presented that learns assembly dependencies from RGB-D input of assembly scenes to infer actionable outputs. The framework is evaluated on an assembly pose estimation dataset.

Result: The network is trained and evaluated against object-centric baselines using an augmented set of metrics specifically designed for progressive assemblies.

Conclusion: The proposed framework enables learning of assembly dependencies from RGB-D input to infer actionable outputs, addressing the gap in autonomous manipulation capabilities for progressive assemblies.
...
TL;DR: This paper surveys the evolution of end-to-end autonomous driving systems, examining the shift from simple camera-to-control regression to sophisticated planning-oriented architectures with structured representations and trajectory-level outputs.


<details>
  <summary>Details</summary>
Motivation: The authors motivate their work by addressing the transition in autonomous driving from basic regression approaches to more advanced planning-oriented systems, arguing that the key distinction is not whether intermediate representations are used, but how they are learned, supervised, and evaluated for safe, feasible, and route-compliant planning.

Method: The authors synthesize existing methods across four axes: input representation, planning output, supervision signal, and evaluation protocol. They review behavior cloning, conditional imitation learning, privileged distillation, BEV and vectorized planning, unified perception-prediction-planning architectures, world-model-based planners, and vision-language-action systems. They also analyze the benchmark shift from open-loop trajectory matching to closed-loop simulation, non-reactive real-log evaluation, long-tail testing, and human-preference-aware metrics.

Result: The analysis highlights that architectural progress is difficult to interpret without benchmark-consistent evaluation, and that displacement-based open-loop metrics alone provide limited evidence for safe and human-aligned driving.

Conclusion: The paper concludes with open challenges in uncertainty-aware planning, learner-expert mismatch, runtime safety assurance, language-action grounding, world-model validation, and reproducible benchmarking.
...
TL;DR: Video2DoorTraversal is a real-to-sim-to-real framework that enables wheel-legged mobile manipulators to autonomously open and traverse through doors by reconstructing door geometry from a single RGB video, generating executable demonstrations in simulation, and training ArticuACT, an onboard dual-depth policy for coordinated base-arm-gripper control.


<details>
  <summary>Details</summary>
Motivation: Door opening and traversal is a challenging long-horizon loco-manipulation task that requires precise handle interaction and coordinated control of the robot's base, arm, and gripper. Existing methods struggle with this task due to the need for both spatial perception and complex manipulation planning.

Method: The framework consists of three main components: (1) DoorTwin reconstructs an instance-aligned, articulated, simulation-ready door twin from a single RGB video with realistic geometry and appearance; (2) A simulation-in-the-loop agent converts the door articulation into a parameterized skill program and iteratively refines failed rollouts to generate physically executable demonstrations; (3) ArticuACT is a dual-depth policy trained on these demonstrations that predicts coordinated base, arm, and gripper commands using robot-centric camera conditioning and interaction-aware supervision.

Result: The system achieves a 96.57% average success rate across five real doors and an 80.95% zero-shot success rate on structurally similar unseen doors. The full approach, opening, and traversal sequence is completed in approximately 13 seconds on average, with all perception and policy inference running onboard.

Conclusion: This work demonstrates that a single-video real-to-sim-to-real pipeline can achieve high-performance door traversal with onboard perception and control, enabling mobile manipulators to navigate through doors in real-world environments without requiring extensive pre-collected datasets or specialized hardware.
...
### [28] [An Evidence-Grounded Multi-Agent System for High-Level Bio-Robot Design](https://arxiv.org/abs/2608.19699)
...
### [29] [Distilling Aggregate Mobility Statistics into a Language Model Policy for Post-Event Crowd Simulation](https://arxiv.org/abs/2608.19778)
...
### [30] [Reward-Guided Autoregressive Graph Generation for Efficient Multi-Agent Communication Topology Design](https://arxiv.org/abs/2608.20099)
...
### [31] [Multi-Agent Orchestration with the Common-Sense Reasoning Capabilities of LLMs for Autonomous Driving](https://arxiv.org/abs/2608.20129)
