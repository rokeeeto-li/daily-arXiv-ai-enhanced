...
TL;DR: GovSim-SelfGovern extends the GovSim common-pool resource environment to enable agents to author executable Python governance rules, vote on laws, and experience self-governance outcomes across multiple rounds, revealing how scarcity transforms institutional authorship into a political and ethical challenge.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of multi-agent LLM systems in social dilemmas treat governance as externally imposed, rhetorical, or restricted to fixed mechanisms. This research addresses the gap by enabling agents to author executable governance rules and experience the consequences of their self-governance decisions in common-pool resource scenarios.

Method: GovSim-SelfGovern extends GovSim to allow agents to author executable Python governance rules, receive sandbox validation feedback, vote on proposed laws, and live under the rules they enact across rounds. The system evaluates agents in three scenarios ranging from stable abundance to a fatal resource wall where five agents cannot all survive through harvest alone.

Result: Executable governance expands the space of possible interventions for agents, but survival depends on agents discovering the right institutional mechanisms in time. Fiscal capacity enables redistribution, while deeper reasoning and removal of democratic veto make exile more feasible. Agents may hesitate to propose exile for normative reasons or because it never enters their candidate set.

Conclusion: GovSim-SelfGovern demonstrates how executable code actions can be adapted to common-pool governance, showing that scarcity transforms institutional authorship into a political and ethical problem rather than merely a technical or economic challenge.
...
TL;DR: The paper examines limitations in aggregate descriptions of communication gain in distributed inference, showing that spectral summaries can miss directional information affecting accuracy, and that community-shared bias can lead to higher individual accuracy harming communities or lowering global accuracy.


<details>
  <summary>Details</summary>
Motivation: Communication can bring agents into agreement while making their decisions worse; standard aggregate descriptions of communication gain may be insufficient to capture directional information that affects accuracy.

Method: Analyzing stable linear systems with fixed evidence and network configurations, using task-projected local-response approximations and labeled calibration data to predict multi-round gains in trained nonlinear agents.

Result: Systems with identical eigenvalue and singular-value spectra can produce gains of opposite sign depending on message orientation (accuracy varying from 72.6% to 91.2% or to 65.9%). Under community-shared bias, calibration constraints can reduce observed community harm while retaining much of the mean benefit.

Conclusion: Spectral summaries are insufficient for capturing directional information; task-aware prediction and calibration constraints can help mitigate issues but don't guarantee protection. Broad transfer and practical superiority remain open questions.
...
### [6] [CHOREO: Every Humanoid Skill as a Trajectory](https://arxiv.org/abs/2609.22274)
*Ziyi Sun,Jingwen Chen,Yuxi Wang,Xiuze Xia,Long Cheng,Zhaoxiang Zhang,Yujun Dong*

Main category: cs.RO

TL;DR: CHOREO is a training-free framework that enables composition of heterogeneous humanoid robot skills by converting them into unified SkillMotion representations (combining motion states, contacts, semantics, and boundary conditions), then composing skills via direct continuation, cubic Hermite blending, or validated bridge motions without retraining.


<details>
  <summary>Details</summary>
Motivation: Humanoid robotics skills acquired through reinforcement learning, motion imitation, and generative modeling are currently siloed due to incompatible representations, interfaces, and controllers, preventing seamless combination of diverse capabilities.

Method: CHOREO converts any learned skill into SkillMotion representation (motion states, contacts, semantics, boundary conditions). Skills are composed through three methods: direct continuation, cubic Hermite blending, and validated bridge motions—all without retraining source models or updating models at test time.

Result: On Unitree G1 in MuJoCo, CHOREO organizes 2,950 admitted SkillMotion assets from heterogeneous sources and achieves 95.4% sequence success across 130 multi-action tasks, including 93.8% success on eight-action sequences.

Conclusion: Executable trajectories provide a scalable interface for accumulating and composing pretrained humanoid capabilities, demonstrating that training-free skill composition is viable across heterogeneous skill sources.
...
### [7] [D3DWA: Adaptive Weight and Prediction-Horizon for Dynamic Window Approach via Dueling Double Deep Q-Network](https://arxiv.org/abs/2609.22276)
...
### [8] [Density-Driven Area Coverage for Nonholonomic Multi-Robot Systems with Safety Guarantee](https://arxiv.org/abs/2609.22668)
...
### [9] [DeViGrasp: Robust Visual Mobile Grasping for Quadruped Manipulators under Degraded Perception](https://arxiv.org/abs/2609.22278)
*Liang Zhou,Jiaming Su,Yancong Wei,Kangkang Dong,Houde Liu*

Main category: cs.RO

TL;DR: A benchmark (DeViGrasp-Bench) and method (DeViGrasp-Net) for mobile grasping under degraded vision conditions on quadruped manipulators, outperforming existing methods like VBC and DQ-Net.


<details>
  <summary>Details</summary>
Motivation: Quadruped manipulators enable mobile grasping, but their control policies are vulnerable to unreliable onboard visual perception. Existing methods haven't systematically examined how visual degradations (occlusion, segmentation-mask dropout, depth noise, target-localization jitter) affect grasp reasoning and target tracking.

Method: DeViGrasp-Bench: benchmark with controlled visual degradations, seen/unseen objects, multiple difficulty levels, complex terrains. DeViGrasp-Net: teacher-student framework combining state-conditioned grasp reasoning with reliability-aware temporal target estimation. Teacher attends to offline grasp candidates conditioned on object, robot, end-effector, and task states. Student fuses dual-view segmented-depth observations with current, memory, and recovery target hypotheses via Target Hold Memory and Temporal Memory Attention.

Result: DeViGrasp-Net outperforms VBC and adapted DQ-Net across degradation levels, unseen objects, and complex terrains. Under Difficult setting: 62.3% success rate (16.1 points higher than VBC, 4.3 points higher than DQ-Net). Under Hard setting: margin over DQ-Net increases to 10.5 percentage points. Ablation studies confirm complementary benefits of grasp-aware supervision and reliability-aware temporal memory.

Conclusion: DeViGrasp-Net effectively handles degraded vision in mobile grasping tasks, demonstrating superior performance over existing methods across various degradation scenarios and outperforming baselines by significant margins under challenging conditions.
...
### [10] [Decentralized Multi-Robot Exploration with Probabilistic Peer Intent and Multi-hop Plan Propagation](https://arxiv.org/abs/2609.22726)
*Saurbh Singh Jamwal,Nived Chebrolu,Shivaram Kalyanakrishnan*

Main category: cs.RO

TL;DR: This paper introduces Probabilistic Peer Intent (PPI) to enhance decentralized multi-robot exploration by converting peer trajectories into a continuous spatial representation of predicted intent and incorporating it into local MCTS action evaluation. It also studies multi-hop plan propagation and demonstrates real-world deployment on three robots.


<details>
  <summary>Details</summary>
Motivation: Efficient coordination under limited communication remains a key challenge in decentralized multi-robot exploration. While centralized approaches benefit from global information sharing, they are impractical in large-scale or communication-constrained environments. Existing Monte Carlo Tree Search (MCTS)-based approaches, such as Decentralized Monte Carlo Exploration (DMCE), enable decentralized planning but have limitations in peer intent representation and communication range.

Method: The paper introduces Probabilistic Peer Intent (PPI), which converts peer trajectories into a continuous spatial representation of predicted intent and incorporates it into local MCTS action evaluation. It also studies the effects of sharing peer intent beyond direct communication range by propagating plans over multiple hops.

Result: Experiments across multiple simulated environments and team sizes show that PPI and Multi-hop propagation can each improve decentralized exploration, with their relative benefits depending on environment structure and team size. The method was also successfully demonstrated on three real robots operating in different environment types.

Conclusion: The proposed method improves decentralized exploration through probabilistic peer intent and multi-hop propagation, with the relative benefits varying based on environment structure and team size. The approach demonstrates practical viability for real-world deployment.
...
### [11] [ORDER: A Fictitious-World Benchmark for Domain-Adaptive Embodied AI](https://arxiv.org/abs/2609.22285)
*Sai Krishna Reddy Sathi,Anuj Tiwari*

Main category: cs.RO

TL;DR: ORDER introduces a novel benchmark for evaluating continual pre-training on language models in knowledge-intensive, task-light robot deployments by testing on a synthetic world with invented physics.


<details>
  <summary>Details</summary>
Motivation: Continual pre-training evaluation is confounded when training corpora overlap with existing model knowledge, making it difficult to attribute performance gains to genuine learning. This is particularly critical for KHTL robot deployments (e.g., pharmaceutical dispensing, hazardous-material handling) where physical tasks are simple but governing rules are proprietary, safety-critical, and testing is costly/unsafe.

Method: ORDER (Ontology-driven Decision-making for Embodied Reasoning) benchmarks are built on a 342,069-token synthetic corpus defining a self-consistent physics that cannot appear in any model's pre-training data. The benchmark includes ORDER-BENCH (a 500-question knowledge test) and ORDER-SPATIAL (a harder compositional task for ordering objects in familiar and novel scenes). A robot pipeline with a simulated iiwa7 arm demonstrates perception-to-execution with human-in-the-loop correction.

Result: GPT-4.1 without adaptation scores below chance on ORDER-SPATIAL (Kendall's tau = 0.441), showing its priors conflict with the invented physics. After continual pre-training, small models improve substantially on both familiar and novel scenes. Fully offline small models outperform GPT-4.1 with retrieval access (Kendall's tau = 0.848 vs. 0.606) on the complete perception-to-execution robot pipeline.

Conclusion: ORDER-SPATIAL performance, not knowledge-test accuracy, is the key predictor of real plan quality. This demonstrates that genuine world-model induction (rather than memorization) is achievable through continual pre-training on novel domains, enabling small offline models to surpass large models with access to external knowledge retrieval.
...
### [12] [Latent Telepathy: Multi-Robot Communication with Self-Supervised Perceptual Latents](https://arxiv.org/abs/2609.23269)
...
### [13] [OJOx: Specification-Conditioned Demonstrations for Embodied AI in Construction](https://arxiv.org/abs/2609.22289)
*Mohamed Dawod*
...
### [14] [MR-SPITE: Accelerating Multi-Robot Conflict Scans via Hierarchical Swept-Volume Approximations](https://arxiv.org/abs/2609.23928)
...
### [15] [When Does Test-Time Physical Diagnosis Pay? A Frozen Policy Buys Evidence It Never Reads](https://arxiv.org/abs/2609.22299)
*Zhengshu Zhang*

Main category: cs.RO

TL;DR: This paper tests whether six ordered empirical conditions must hold for robot diagnosis and adaptation to unfamiliar physical conditions to improve behavior, finding that the chain breaks at evidence use when transferred to unseen mechanisms.


<details>
  <summary>Details</summary>
Motivation: When robots encounter unfamiliar physical conditions, a common approach is to collect evidence about what changed and adapt. The authors aim to identify the necessary conditions for such diagnosis to lead to improved behavior.

Method: The authors test a chain of six ordered empirical conditions (meaningful reference, identifiability, use of acquired evidence, decision value, selection value over a fixed alternative, and safe realization) in controlled environments, then transfer to unseen mechanisms. They compare a frozen decoder against a linear model using only trace increments.

Result: The chain holds in controlled environments but breaks at evidence use when transferred to unseen mechanisms. Decisions requiring the full trace fail (frozen decoder doesn't change choice), while decisions settled with lower-cost evidence remain correct. This failure is concentrated at the richest evidence level and hidden by aggregate accuracy.

Conclusion: Successful physical identification does not guarantee evidence use or useful adaptation. Evaluation should identify where the chain breaks rather than relying solely on recovery accuracy or aggregate performance.
...
### [16] [EditWM: Event-Decomposed World Modeling with Incremental Correction for End-to-End Autonomous Driving](https://arxiv.org/abs/2609.22317)
...
### [17] [Embedding Physics Priors in Robot Learning: A Survey](https://arxiv.org/abs/2609.22319)
...
TL;DR: A comprehensive survey of physics-embedded robot learning, which reviews methods that incorporate physics priors into machine learning algorithms for robotics applications, providing a unified taxonomy and identifying open challenges and future directions.


<details>
  <summary>Details</summary>
Motivation: Robotics faces challenges of limited data, complex real-world interactions, and the need for reliable operation. Purely data-driven methods have succeeded in vision and NLP but struggle in robotics due to these constraints.

Method: The authors employ a unified taxonomy classifying approaches by physics embedding: physics-guided inputs/data/representations; physics-encoded model architectures; and physics-informed training loss functions. They review methods across robot dynamics learning, trajectory planning, prediction, control, and estimation.

Result: The survey provides a broad review of physics-embedded robot learning across various physics priors, robotics applications, and machine learning models (from single-layer perceptrons to generative foundation models), including the corresponding open-source software ecosystem.

Conclusion: Physics priors provide a robotics-specific inductive bias that complements rather than replaces data-driven learning, enabling more generalizable, data-efficient, and trustworthy robotic systems.
...
### [18] [ReliCAD: From Uncertain LLM Generation to Reliable Parametric CAD Modeling](https://arxiv.org/abs/2609.22325)
...
### [19] [AffordanceWAM: Affordance-Aware Joint World-Action Modeling for Robot Manipulation](https://arxiv.org/abs/2609.22332)
...
### [20] [Active Spatial Inspection for Effective and Efficient Embodied Exploration](https://arxiv.org/abs/2609.22385)
*Wenbin Wang,Xiang Bai,Yizhao Wang,Hang Sun,Dong Ren,Jie Qin,Qingquan Li,Bing Wang*

Main category: cs.RO

TL;DR: This paper introduces the spatial-inspection-guided ACtive Exploration (ACE) framework, which couples evidence-grounded perception with exposure-informed movement to establish a spatially resolved decision paradigm for embodied exploration, improving both task success and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing frameworks typically guide agent behavior through spatially coarse and indirect assessments of suggestive cues and directions, which struggle to judge cue sufficiency and the need for further inspection, leading to early termination or excessive continuation and ultimately reducing task success and efficiency.

Method: ACE couples evidence-grounded perception (integrating granular localization with focused verification to turn suggestive cues into precise decisive visual support) with exposure-informed movement (using prospective prioritization and retrospective suppression to select promising directions for efficient spatial progress), establishing a spatially resolved decision paradigm.

Result: ACE achieves 18.0% higher navigation task success and 10.3% higher exploration efficiency for question answering compared to prior state-of-the-art baselines.

Conclusion: ACE advances effective and efficient embodied exploration by alleviating the tension between preventing early termination and avoiding unnecessary continuation.
...
### [21] [Behavior Trees for Robotic Systems: An Empirical Study on Practices and Experiences](https://arxiv.org/abs/2609.22404)
...
### [22] [VLPSA: Vision-Language-Poisson-Safe Actions for Full-Body Safety of Learned Policies](https://arxiv.org/abs/2609.22462)
...
### [23] [Tracker-Free Robotic Ultrasound Calibration with a Spherical-Marker Phantom and Threshold-Free Center Localization](https://arxiv.org/abs/2609.22483)
...
### [24] [Layered e-skin for Shear Sensing](https://arxiv.org/abs/2609.22493)
...
### [26] [FRAMES: Failure Recovery And Monitoring of Embodied Skills for Humanoid Loco-Manipulation](https://arxiv.org/abs/2609.22538)
...
### [27] [React When You Need To: Event-Triggered Asynchronous Inference for VLA Policies](https://arxiv.org/abs/2609.22587)
...
### [28] [REBOOT: From Failure to Recovery - A Dataset and Benchmark for Precision Assembly](https://arxiv.org/abs/2609.22591)
...
### [29] [Characterizing Wildlife Response to Biomimetic and Conventional Underwater Vehicles](https://arxiv.org/abs/2609.22594)
...
### [30] [VT-Bridge: Bridging Pretrained Foundation VLAs to VTLAs via Lightweight Residual Adaptation](https://arxiv.org/abs/2609.22606)
...
### [31] [Becoming a Fruit Ninja: Real-Time Probabilistic Kinodynamic Planning for Manipulator Projectile Interception](https://arxiv.org/abs/2609.22608)
*Lucas Chen,Austin Garrett,Andrew Niu,Zachary Kingston*

Main category: cs.RO

TL;DR: FRUITNINJA is an anytime GPU-based sampling planner for projectile interception that dynamically generates safe trajectories to cut falling objects with a robot arm, achieving 96.7% success in open scenarios and 68.3% with obstacles.


<details>
  <summary>Details</summary>
Motivation: Projectile interception requires precise timing to reach a point on an object's trajectory as it falls, with the robot arm's blade velocity and orientation fixed at contact. The challenge lies in finding trajectories within milliseconds that satisfy actuator limits while navigating uncertainty in the object's position.

Method: FRUITNINJA uses an anytime sampling-based planner that grows a tree on the GPU in batches toward the interception manifold. Each trajectory edge is an exact cubic spline whose travel time is computed via parallel search against the arm's dynamics, guaranteeing actuator limit compliance. Plans are ranked by a risk-aware objective considering uncertainty in object position and arrival time.

Result: Evaluated on a Franka Research 3 robot in a calibrated real-time simulator against six baselines, FRUITNINJA achieved 96.7% interception rate in open scenarios and 68.3% with five obstacles, outperforming the best baseline's 68.3% (open) and 35.0% (obstacles).

Conclusion: FRUITNINJA demonstrates that GPU-accelerated sampling-based planning with exact cubic trajectory edges and risk-aware ranking can effectively solve the challenging problem of real-time projectile interception, significantly improving success rates over traditional methods.
...
### [32] [From Documented Strengths to Force Limits: Material-Informed Robotic Insertion for Construction Assembly](https://arxiv.org/abs/2609.22609)
...
### [33] [HIGenNTO: Scalable Humanoid Interaction Generation via Noise-Space Trajectory Optimization](https://arxiv.org/abs/2609.22611)
...
### [34] [Simultaneous Forward and Inverse Human-in-the-Loop Optimization](https://arxiv.org/abs/2609.22630)
*Kyeongwon Park,Steven H. Collins*

Main category: cs.RO

TL;DR: SFIHILO is a simultaneous forward and inverse human-in-the-loop optimization method that efficiently learns individual-specific reward functions from user preferences using active querying and uncertainty reduction, significantly improving sample efficiency compared to mutual information approaches while preserving forward model accuracy.


<details>
  <summary>Details</summary>
Motivation: Subjective user experience is crucial for human-robot interaction, but user preferences across individuals and contexts are often unknown. Inverse learning approaches that identify user rewards are limited by high experimental costs (control policy execution, biomechanical/physiological measurements, user feedback), which restrict the number of queries and optimization iterations possible.

Method: SFIHILO bootstraps a forward model to predict user outcomes from control policies, uses this model for active querying to accelerate inverse reward learning, and optimizes the final control policy without additional user trials. It scores candidate policies by expected reduction in uncertainty across both forward and inverse beliefs and targets a regional preference boundary for robust learning.

Result: In simulation, SFIHILO was effective across user heterogeneity, outcome dimensionalities, precision requirements, noise levels, and nonstationarity. Compared to mutual information approaches, it significantly improved sample efficiency in inverse learning while preserving forward model accuracy.

Conclusion: The approach demonstrates the potential to infer latent human goals, enabling more transferable and effective human-robot interaction by efficiently learning individual-specific reward functions with minimal user trials.
...
### [35] [AquaWorld: Structure-Consistent Underwater World Generation for Robot Simulation](https://arxiv.org/abs/2609.22670)
...
### [36] [SHAFT: A Slack-Compensating, Helical-Buckling-Attenuating Flexible-Shaft Transmission for Lightweight Multi-DoF Manipulation](https://arxiv.org/abs/2609.22677)
...
### [37] [A Direct Rigid Transmission 2-DoF Wrist Extension for Tendon-Driven Hand](https://arxiv.org/abs/2609.22681)
...
### [38] [StateMem: Single-State Residual Memory with Adaptive Inference for Vision-Language-Action Policies](https://arxiv.org/abs/2609.22684)
...
### [39] [BEACON: Belief-Enabled Adaptive CONtrol for Imitation Learning under Uncertainty](https://arxiv.org/abs/2609.22730)
...
### [40] [Rapidly-Iterating Grid-Based Near-Optimal Kinodynamic Motion Planning](https://arxiv.org/abs/2609.22733)
...
### [41] [Distributed Infrastructure Sensors and Cues for Robotic Fire-Fighting and Safety System on a Lunar Base](https://arxiv.org/abs/2609.22786)
*Alexis Munguia,Eryc Rodriguez,Drake Russ,Jekan Thangavelautham*

Main category: cs.RO

TL;DR: A prototype fire-fighting technology using thin ceramic QR codes embedded in lunar habitats, enabling rapid response via low-power rover that decodes local hazard information distributed across passive plates.


<details>
  <summary>Details</summary>
Motivation: Need for rapid-response fire-hazard management in early crewed lunar habitats where suited astronauts and Earth support cannot meet response time requirements for handling pressurized cylinders within minutes.

Method: Embedding thin ceramic QR codes every few meters containing local floor maps, safe sensor limits, and step-by-step hazard responses, with a low-power rover that decodes plates in seconds, polls temperature-gas sensor clusters, and acts immediately without requiring global maps.

Result: Simulations show this approach simplifies rapid-response firefighting, avoids costly resource use, localizes damage, allows procedure updates by replacing single codes, and maintains minimal processor demands.

Conclusion: The distributed knowledge approach through passive plates effectively addresses fire safety challenges but important implementation challenges remain for rapid-response fire-hazard management in early crewed lunar habitats.
...
### [42] [Task-Oriented Co-Design and Optimization of Geared Actuators for Robotic Applications](https://arxiv.org/abs/2609.22795)
*Xuanyu Huang,Jianqiang Dong,Hang Zhao*

Main category: cs.RO

TL;DR: A systematic optimization framework for task-oriented co-design of actuator hardware and control, enabling legged robots to achieve significantly improved performance through surrogate-based mixed-variable optimization and validated with a 35.7 N·m/kg torque density jumping leg prototype.


<details>
  <summary>Details</summary>
Motivation: Legged robots require distinct torque and speed capabilities across tasks, yet existing actuator designs are optimized at the component level without task guidance. System-level optimization across coupled mechanical, electrical, and electromagnetic components is computationally challenging, causing individual component improvements to not translate into better robot performance.

Method: A hierarchical mixed-variable optimization strategy combining surrogate models for accelerated motor evaluation with discrete enumeration and continuous search over design dimensions. The framework jointly optimizes rated output torque density and task performance using Bezier-parameterized joint torque profiles, with indices rounded to select admissible discrete choices before evaluation.

Result: Fabricated a 2-DOF jumping leg prototype achieving 35.7 N·m/kg rated output torque density (60% higher than a commercial geared joint actuator) while being 18.6% lighter. Under matched bench conditions, the prototype achieved 12.0% greater jump height at twice-rated torque compared to the baseline.

Conclusion: Demonstrates a systematic route from task requirements to actuator design and control, showing that integrated hardware-control co-design can significantly improve legged robot performance beyond what component-level optimizations achieve alone.
...
### [43] [Expert-Play Contouring Control: Faster-than-Demonstration Planning from Slow Expert and Fast Play](https://arxiv.org/abs/2609.22798)
...
### [44] [ARCGym: Benchmarking Deep Reinforcement Learning in Autonomous Robotic Colonoscopy](https://arxiv.org/abs/2609.22803)
*Guanglin Ji,Martina Finocchiaro,Kenny Erleben,Hang Yin*

Main category: cs.RO

TL;DR: ARCGym introduces an open-source reinforcement learning environment and benchmark for image-based navigation in clinically derived deformable colon anatomies, supporting both capsule robots and flexible endoscopes with magnetic-driven tip actuation and proximally translational actuation.


<details>
  <summary>Details</summary>
Motivation: Prior learning-based autonomous colonoscopic navigation simulations focused exclusively on fully actuated capsule robots, failing to capture the contact-rich navigation challenges of long, flexible clinical colonoscopes.

Method: ARCGym provides a unified RL environment with (1) 5 CT-reconstructed colons representing typical clinical scenarios, (2) multiple robot types including flexible endoscopes with magnetic-driven tip actuation and proximally translational actuation, (3) clinically meaningful navigation subtasks, (4) unified success metrics, and (5) a novel reward combining depth-based lumen alignment with a lumen-visibility score to improve learning under occlusions.

Result: Experiments across tasks, robots, and anatomies demonstrate that autonomous navigation remains challenging for both magnetic-driven and proximal-insertion flexible robots, with proximal-insertion actuation proving particularly difficult.

Conclusion: ARCGym establishes a comprehensive benchmark for image-based colonoscopic navigation research, highlighting proximal-insertion actuation as an open problem and providing a foundation for future advancements in flexible endoscopy automation.
...
### [45] [Kinematic Interface for the Wild: Modular Bimanual Loco-Manipulation Capture from 360$^{\circ}$ Cameras Alone](https://arxiv.org/abs/2609.22809)
...
### [46] [Commonsense-Grounded Path Planning from Abstract Instructions](https://arxiv.org/abs/2609.22813)
...
### [47] [Whole-Body UMI: Transferring UMI Manipulation Skills to Humanoid Whole-Body Manipulation via Real-Time Motion Generation](https://arxiv.org/abs/2609.22829)
*Yuxuan Nai,Leixin Chang,Liangjing Yang,Shuo Yang,Zhongyu Li*

Main category: cs.RO

TL;DR: WB-UMI is a task-agnostic, real-time motion generator that decouples whole-body coordination learning from task semantics learning through a shared end-effector interface, enabling scalable humanoid manipulation data collection without body trackers.


<details>
  <summary>Details</summary>
Motivation: Current whole-body demonstration collection for humanoid manipulation relies on costly teleoperation. While UMI provides scalability, using only end-effector trajectories underdetermines whole-body coordination, which is insufficient for comprehensive whole-body demonstration collection.

Method: WB-UMI introduces a diffusion policy that learns from native UMI demonstrations and a motion generator that learns independently from retargeted motion capture data. An asynchronous hierarchy integrates the diffusion policy, motion generator, and whole-body controller with latency compensation and measured-state feedback.

Result: Real-robot experiments on G1 achieve 90% success in drawer closing, 80% in shelf pick-and-place, 30% in ball toss, and 40% in Loco-PnP across four tasks, demonstrating effective closed-loop transfer of native UMI skills to humanoid whole-body manipulation.

Conclusion: The asynchronous hierarchy successfully decouples whole-body coordination learning from task semantics learning through a shared end-effector interface, providing an effective approach for transferring skills to humanoid manipulation without requiring body trackers or paired image-whole-body demonstrations.
...
### [48] [ForceRFT: Refining VLA Actions through Force-Guided Residual Reinforcement Learning](https://arxiv.org/abs/2609.22840)
*Yichen Wang,Chaoyang Zhang,Xuqi Su,Jun Ma,Haiyue Zhu,Xiaocong Li*

Main category: cs.RO

TL;DR: ForceRFT is a force-guided residual reinforcement learning framework that enhances force-conditioned VLA policies by learning contact-dependent corrections from both human corrective imitation and autonomous task outcomes, achieving higher autonomous success rates on real robot tasks.


<details>
  <summary>Details</summary>
Motivation: Force-conditioned VLA policies are limited by demonstration coverage and don't learn from deployment outcomes. Human corrective imitation provides recovery examples but doesn't explicitly optimize task return. The authors aim to create a framework that learns from both human supervision and autonomous task outcomes while optimizing task return.

Method: ForceRFT uses a frozen SmolVLA-based prior to generate force-conditioned action chunks, with a lightweight residual actor refining end-effector pose commands using wrist feedback (wrench, temporal change, base motion). Human corrections supervise the residual actor, while verified autonomous transitions train twin critics for value-guided updates. Bootstrapping is restricted to autonomous segments to prevent TD credit from crossing human-intervention boundaries.

Result: Real-robot experiments on plug insertion, ring-on-peg assembly, and whiteboard wiping show higher autonomous success rates than demonstration-trained and residual-imitation baselines. Plug-insertion ablations demonstrate the benefit of direct execution-time wrist feedback, and comparisons with residual imitation support value-guided residual optimization.

Conclusion: ForceRFT effectively combines human corrective imitation with residual RL, learning contact-dependent corrections from both supervision types. The value-guided residual optimization approach outperforms residual imitation alone, and direct execution-time wrist feedback provides significant benefits for force-conditioned manipulation tasks.
...
### [49] [Physical-Touch Observability from Wrist Wrench in Granular Scooping](https://arxiv.org/abs/2609.22852)
...
### [50] [SmoLSTM: A Compact Vision-Language-Action Model with Recurrent Memory that Persists](https://arxiv.org/abs/2609.22854)
...
### [51] [PileBelief: Persistent Physical State for Interaction-Driven World Modeling](https://arxiv.org/abs/2609.22858)
...
### [52] [A Reconfigurable Dual-Opposition Architecture for Single-Hand Assembly and Manipulation](https://arxiv.org/abs/2609.22871)
*William Su,Yunosuke Nakamura,Yixiao Wang,Yitong Li,Mingrui Yu,Huanan Qi,Boyuan Liang,Masayoshi Tomizuka,Jianshu Zhou*

Main category: cs.RO

TL;DR: A reconfigurable dual-opposition architecture with four independently actuated fingers organized into two virtual finger oppositions and a reconfigurable palm, enabling simultaneous grasping of two parts and coordinated in-hand manipulation with 98.6% sphere rotation success rate compared to 73.8% for LEAP Hand.


<details>
  <summary>Details</summary>
Motivation: In-hand assembly is constrained by the need to maintain grasps on two separate parts while controlling their relative motion within a single hand. The authors aim to enable both in-hand assembly and manipulation through an improved hand architecture.

Method: Four independently actuated fingers organized into two virtual finger (VF) oppositions, with relative configuration controlled by a reconfigurable palm. A kinematic model of the fingers and palm and an object-size-conditioned workspace formulation were built. Finger-joint motion and palm tracking were characterized, and ablation experiments tested the importance of finger abduction/adduction and palm reconfiguration.

Result: The hand achieved a mean continuous sphere rotation success rate of 98.6% over diameters of 40-230 mm in simulation (vs. 73.8% for LEAP Hand). After policy fine-tuning with external disturbances, success rate was 92.8% (vs. 45.2% for LEAP Hand). Hardware demonstrations showed successful in-hand rotation of objects of different sizes using simulation-trained policies.

Conclusion: The proposed dual-opposition architecture successfully supports both assembly of two separately held parts and coordinated manipulation of a single object within one hand, with finger abduction/adduction and palm reconfiguration being critical for successful in-hand assembly.
...
### [53] [Barrier Certificate Synthesis for Non-Polynomial Robotic Dynamics via Polynomial Lifting](https://arxiv.org/abs/2609.22885)
...
### [54] [Stable and Efficient Real-World Online VLA Post-Training via Asynchronous Replay-Anchored Policy Improvement](https://arxiv.org/abs/2609.22888)
*Jiarui Yang,Jiajin Zhang,Bin Zhu,Jingjing Chen,Yu-Gang Jiang*

Main category: cs.RO

TL;DR: RAPolicy is an asynchronous framework for online post-training of vision-language-action (VLA) models that improves policy efficiency by performing rollout and learning concurrently while grounding both critic and actor updates in replayed behavior.


<details>
  <summary>Details</summary>
Motivation: Online post-training of VLA models requires efficient use of robot interaction and reliable policy improvement from continually collected experience.

Method: RAPolicy performs rollout and learning concurrently. The critic learns chunk-level values from recorded actions and constructs Bellman targets without predicting next actions, reducing computation and dependence on action-value estimates outside replay coverage. The one-step flow actor reuses the initial noise stored during rollout and learns through advantage-weighted conditional likelihood, directly supervising the action mapping used for execution.

Result: Evaluated across four single-task settings and one joint five-task setting in the real world with 1-2 hour online training budgets. Starting from policies fine-tuned on just 10 demonstrations per task, RAPolicy achieves an average 86.3% success rate in single-task settings and improves overall success rate from 52% to 88% in the joint multi-task setting, while preserving performance on already reliable tasks and improving weaker capabilities.

Conclusion: RAPolicy substantially outperforms baselines in aggregate task success while requiring fewer human interventions, demonstrating stable policy improvement and high online training efficiency.
...
### [55] [H-VLA: Hierarchical Vision-Language-Action Model with Key-Action Reasoning and Motion Planning in a Unified Action Space](https://arxiv.org/abs/2609.22895)
...
TL;DR: H-VLA is a hierarchical Vision-Language-Action framework that decouples high-level key-action reasoning from low-level motion generation, using a Key-Action Model and Motion Planning Model with a unified camera-centric action space and two-stage training.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models directly map language and visual observations to dense actions, which weakens semantic reasoning from pre-trained VLMs and becomes fragile under spatial variations like object positions, scene layouts, robot embodiments, and camera viewpoints.

Method: H-VLA combines a Key-Action Model for predicting manipulation subgoals and a Motion Planning Model for generating dense actions conditioned on those subgoals, using a unified camera-centric action space. It employs a two-stage training strategy emphasizing key-action reasoning during pre-training and dense motion generation during fine-tuning.

Result: On SimplerEnv: 91% Google Robot visual matching, 84% variant aggregation, 81% WidowX visual matching. On Agilex real-robot tasks: improves over strongest baseline by 10%, 47%, and 16 percentage points under in-distribution, out-of-distribution position, and out-of-distribution scene/object settings, respectively.

Conclusion: H-VLA effectively addresses VLA limitations by decoupling high-level reasoning from low-level control, achieving strong performance on both simulation and real-world robotic manipulation tasks with robustness to spatial variations.
...
### [56] ["Dear LLaVA, Please Drive": A Depth-Aware Vision-Language Agent for Closed-Loop Robotic Control](https://arxiv.org/abs/2609.22925)
...
### [57] [Embodied Snap: Octopus-Inspired Distributed Reach-and-Attach with a Speed-Limited Soft Arm](https://arxiv.org/abs/2609.22926)
*Linxin Hou,Zhihang Qin,Heyang Zou,Qirui Wu,Peiyi Wang,Muhammad Sunny Nazeer,Yongxin Guo,Cecilia Laschi*

Main category: cs.RO

TL;DR: This paper proposes an embodied snap controller for soft robotic arms that separates slow servo-driven preloading from rapid elastic release, enabling tip speeds 108% above direct tendon-driven limits. Using a learned policy shared across three sections, the controller achieves 73±4% attachment success across simulated targets and demonstrates successful hardware retrieval in 9 of 18 trials.


<details>
  <summary>Details</summary>
Motivation: Soft robotic arms with passive suction require accurate targeting and sufficient contact speed for reach-and-attach tasks, but geared actuators impose speed limits that trajectory tracking improvements alone cannot overcome.

Method: The embodied snap controller is inspired by octopus biology (section-wise organizational prior). It separates slow servo-driven preloading from rapid elastic release. A learned policy shared across three sections selects preloads, aim, tendon slack, and release timing. The policy is optimized offline using a hardware-validated recurrent model within experimentally supported bounds.

Result: Across 5 optimization seeds and 400 unseen simulated targets, attachment success is 73±4% at a 5 cm lateral tolerance. The shared policy reaches the matched centralized controller's mean final reward after a median 17% of the common evaluation budget. Hardware characterization achieves tip speeds of 1.56-1.64 m/s (≥108% above direct tendon-driven release). In 18 open-loop hardware trials across six placements, 17 exceed the 1 m/s snap threshold and nine retrieve the object, with successful retrieval at five placements.

Conclusion: The results demonstrate a practical division of responsibility in the control problem: learned control prepares the body, and passive body mechanics execute the rapid movement needed for dynamic reach-and-attach.
...
### [58] [MIRA-PRM: Mission-Informed Reusable Roadmap Planning for Mobile Gas-Sensing Inspection](https://arxiv.org/abs/2609.22954)
...
### [59] [Prescribed-Time Contracting-Boundary Control of a Tendon-Driven Flexible Arm](https://arxiv.org/abs/2609.22963)
...
### [60] [Transferring the Intelligence of VLMs to Robotic Control](https://arxiv.org/abs/2609.22966)
*Meng-Hao Guo,Zhe-Han Mo,Jia-Jun Wang,Yi Zhang,Kejin Wang,Yi-Xuan Deng,Jia-Peng Zhang,Yongming Rao,Shi-Min Hu*

Main category: cs.RO

TL;DR: RoboDawn is a framework that enables vision-language models (VLMs) to control robots without task-specific training, using a human-intuitive interface with discrete commands and in-context learning. It achieves strong zero-shot performance and SOTA results with just one demonstration.


<details>
  <summary>Details</summary>
Motivation: Humans can transfer intelligence across physical and digital worlds. The fundamental question is whether VLMs, which excel in the digital world, can similarly generalize to control robots in the physical world without task-specific robot training.

Method: RoboDawn introduces a human-intuitive interface exposing robotic control to VLMs through a compact set of discrete translation, rotation, and gripper commands. The VLM operates in closed loop (observe → reason → execute → adapt) and uses an in-context learning scheme with few demonstrations to ground the model in interface usage and task-solving strategies.

Result: RoboDawn outperforms several benchmark-specific trained policies in zero-shot settings. With a single in-context demonstration, it achieves substantial performance gains: RoboTwin 2.0 C2R (53.2% → 73.6%, exceeding π0.5 baseline of 46.0%) and RoboDojo (35.67% → 47.17%). The framework also transfers successfully to real-world robots (Franka) for block-in-basket and block stacking tasks.

Conclusion: RoboDawn demonstrates that VLM intelligence can effectively generalize to robotic control without task-specific training. The framework achieves state-of-the-art results with minimal demonstrations, proving that digital-to-physical transfer of intelligence is possible and practical for robotic applications.
...
### [61] [An Empirical Study and Open Testbed for Federated Fine-Tuning of Vision-Language-Action Models](https://arxiv.org/abs/2609.22973)
...
### [62] [A Horizon-slicing Approach to Minimum Obstacle Displacement Planning for Robot Navigation](https://arxiv.org/abs/2609.22974)
...
### [63] [Connectivity-Aware Exploration of Robotic Grasp Spaces](https://arxiv.org/abs/2609.22983)
*Maksim A Kazanskii*
...
### [64] [Fast and Robust Temporal Logic Planning via ADMM-based Trajectory Optimization](https://arxiv.org/abs/2609.23037)
...
### [66] [On the Control of Mobile Ad-Hoc Agent Deployments in Partially Observed Space](https://arxiv.org/abs/2609.23060)
...
### [67] [Splat-CBF: Safe Next-Best-View Control in 3D Gaussian-Splat Maps](https://arxiv.org/abs/2609.23100)
...
### [68] [DiagGen: Agentic Generation of Deformable Assets with Sim-based Diagnostics for Robotic Simulation](https://arxiv.org/abs/2609.23103)
...
### [71] [Selective Commitment for Language-Guided Object Retrieval under Partial Observability](https://arxiv.org/abs/2609.23131)
*Wonhee Koh,Sushil Samuel Dinesh,Hansol Ko,Shinkyu Park,Eungjoo Lee*

Main category: cs.RO

TL;DR: A closed-loop framework for language-guided object retrieval under partial observability that maintains a joint belief over target identity, container relation, and presence through tracked-object, unobserved-target, and target-absent hypotheses, using view-conditioned VLM observations, conformal grasp eligibility, and finite-horizon belief-space planning.


<details>
  <summary>Details</summary>
Motivation: Language-guided object retrieval under partial observability requires coordinating decisions about gathering more evidence, interacting with the scene, grasping a candidate, or abstaining to retrieve a target specified in relation to a reference container.

Method: The framework maintains a persistent joint belief over target identity, container relation, and presence through tracked-object, unobserved-target, and target-absent hypotheses. It uses view-conditioned categorical VLM observations to update the belief, conformal grasp eligibility and robot feasibility to govern commitment decisions, and finite-horizon belief-space planning to select information-gathering actions.

Result: Across five scenarios, the method succeeds in 19/25 simulation episodes vs 12/25 for the best-performing baseline and is the only evaluated policy to achieve at least one success in each scenario. Cross-view memory improves success under partial occlusion. Real-robot trials demonstrate closed-loop re-observation and autonomous recovery from grasp failures, though injected viewpoint failures result in false defer.

Conclusion: The proposed framework demonstrates the feasibility of coordinating evidence gathering and selective grasp commitment within a unified system for language-guided object retrieval under partial observability, achieving superior performance over baselines and enabling autonomous recovery from failures in real-world experiments.
...
### [72] [Tip Manipulation in Soft Everting Robots via Wall Retraction and Deployable Fingers](https://arxiv.org/abs/2609.23132)
*Nelson Badillo Perez,Niccolo Pagliarani,Matteo Cianchetti,Robert D. Howe*

Main category: cs.RO

TL;DR: Soft everting robots with wall retraction and coordinated actuation enable repositionable tip manipulation using ultralight inflatable fingers for adaptive grasping in confined environments.


<details>
  <summary>Details</summary>
Motivation: Soft everting robots traverse long confined paths by growing, but current designs have limited active interaction with only a single fixed tool at the tip that cannot be repositioned and is difficult to integrate with the soft body.

Method: A tip-manipulation strategy based on wall retraction using a base roller assembly to independently meter membrane flow in the outer wall and a tail spool to regulate growth in the internal tail section, decoupling robot length from membrane-material position. This enables membrane-mounted devices to be transported, exposed, and repositioned at selected locations near the distal tip.

Result: Ultralight pleated inflatable fingers integrated into the membrane (TPU-coated nylon with internal airtight bladder) achieve large bending (~100° at 50 kPa) and blocking forces up to 1.9 N while remaining limp during transport. The system demonstrates adaptive grasping across diverse objects (21-550 g, 16-200 mm), 3D manipulation and stacking, environmentally braced extension, distal camera panning for inspection, and controlled sequential payload delivery.

Conclusion: This work enables embodied and reversible tip manipulation for soft growing robots in cluttered and tortuous environments, overcoming the limitation of fixed single-tool interaction.
...
### [73] [AquaCap: A Training-Free Underwater Embodied Agent with Code-as-Policy](https://arxiv.org/abs/2609.23133)
*Xiaoshi Li,Yule Xu,Chunghiu Kong,Yizhou Zhou,Yang Liu,Hao Yang,Zihao Huang,Yunxiao Shan*

Main category: cs.RO

TL;DR: AquaCap is a training-free Code-as-Policy framework for underwater navigation and manipulation that employs a dual-layer agent, structured perception, and failure-aware memory to achieve autonomous grasping and object transport with 66.43% simulation success rate.


<details>
  <summary>Details</summary>
Motivation: Recent vision-language-action models rely on large-scale interaction data, which is scarce and costly to collect underwater, limiting their applicability in underwater embodied intelligence tasks.

Method: AquaCap features a dual-layer agent that translates task instructions and environmental observations into condition-aware plans and executable control programs. Structured perception provides semantic, geometric, and reliability-aware observations under degraded underwater conditions. A failure-aware memory diagnoses unsuccessful actions to support closed-loop replanning and code revision, enabling online adaptation without task-specific training or parameter updates.

Result: AquaCap achieves a 66.43% success rate in simulation. Real-world experiments demonstrate autonomous grasping and object transport with an ROV, including manipulation of targets displaced by hydrodynamic disturbances.

Conclusion: The framework enables online adaptation without task-specific training or parameter updates, demonstrating effectiveness for underwater navigation and manipulation tasks even in dynamic environments.
...
### [74] [Probabilistic Scene Graphs: Hierarchical Representation and Real-time System](https://arxiv.org/abs/2609.23144)
...
### [75] [Robot World Models Are Not Invariant to How the Actions Are Written](https://arxiv.org/abs/2609.23252)
*Ahmed Karim,Leon Chlon*

Main category: cs.RO

TL;DR: This paper identifies a catastrophic issue in robot learning where the choice of action parameterization (absolute joint targets vs. deltas) silently affects world models, causing severe performance degradation when models trained on one parameterization are given trajectories in another.


<details>
  <summary>Details</summary>
Motivation: In robot learning, engineers choose between absolute joint targets and delta actions relative to current state. This choice is silently inherited by world models trained on robot data. The authors argue this inheritance is "catastrophic" and needs investigation because the invariance literature focuses on visual models (crops, jitter, camera pose) but neglects the parameterization of commands.

Method: The researchers train latent dynamics models on robot datasets using two action parameterizations and test how they handle trajectories written in the other parameterization. They also propose a mathematical test to distinguish valid re-parameterizations from lossy summaries or sensor swaps, which rejected three of four proposed axes.

Result: When a model trained on one parameterization is given trajectories in another, retrieval degrades by 2.6-13.4x across datasets, goal-conditioned action selection drops from 53% to 15%, and beliefs become near-orthogonal (cosine similarity ~0.067, worst case -0.377). The two encodings are mutually reconstructible at R² = 0.996, so no information is lost—this is a defect in the action channel, not a distribution shift.

Conclusion: The fix is averaging over the two encodings, but where you average matters: averaging the objective restores task performance, but averaging outputs can fail for direction-valued prediction due to normalization issues. Averaging objectives leaves behind the tail where worst-case agreement stays at 0.78; adding a disagreement penalty closes it to 0.995. Over a latent rollout, this distinction determines whether worst-case performance erodes or holds. On PushT, averaging alone fails to repair the axis.
...
### [76] [Scenario MPC with STL Specifications and Pareto-Based Feasibility Repair](https://arxiv.org/abs/2609.23263)
...
### [77] [Steering Through Contact: A Finite-Support Motion Model for Single-Track Center-Articulated Robots](https://arxiv.org/abs/2609.23271)
...
### [78] [SCULPT-VLA: Learning Structured Control through Staged Action Grounding](https://arxiv.org/abs/2609.23275)
...
### [79] [Safety-Critical Control under Uncertainty via Adaptive Conformal Quantile Prediction Intervals](https://arxiv.org/abs/2609.23280)
...
### [80] [Shared Execution-Clock Drifting Policy for Dynamic Precision Manipulation](https://arxiv.org/abs/2609.23305)
...
### [81] [Manipulation Feasible Navigation Among Movable Obstacles with Discrete Contact Pushing](https://arxiv.org/abs/2609.23312)
...
### [82] [Towards Reliable Underwater Diver-Robot Interaction: Gesture Design, Interaction Logic, and Real-World Evaluation](https://arxiv.org/abs/2609.23392)
...
### [83] [EmoPose: Vision-Language Model Guided Emotion-Aware Gesture Generation for Humanoid Robots](https://arxiv.org/abs/2609.23414)
*Daojie Peng,Bingtao Wang,Fulong Ma,Wenjun Yue,Liang Zhang,Jun Ma*

Main category: cs.RO

TL;DR: EmoPose is a vision-language model (VLM)-guided framework that bridges semantic flexibility and deterministic robot control for expressive humanoid robot gesture generation. It uses a VLM to generate executable gesture plans from language and dialogue context, while a robot-owned motion library provides the physical embodiment and a deterministic robot-side module validates and schedules gestures.


<details>
  <summary>Details</summary>
Motivation: Socially competent humanoid robots need to communicate affect and intent through both gesture and speech in open-ended interactions. However, there is a gap between the semantic flexibility needed for contextual social intent and the deterministic, embodiment-aware control required for physical execution. Existing approaches often either lack semantic flexibility or cannot guarantee executable motion on a specific robot body.

Method: EmoPose introduces an executable semantic interface that uses a VLM to generate ordered gesture plans containing communicative class, library variant, intensity, and speech anchor. A scalable robot-owned motion library defines the expressive vocabulary and source of 14-DoF joint targets. Pose Studio provides automatic trajectory generation, MuJoCo simulation preview, and synchronization of new library entries with the VLM guide. Deterministic robot-side modules validate plans, construct trajectories, schedule gestures, and manage queueing and interruption.

Result: On EmoPose-Bench, structured GPT-5.5 planning achieves 98.25±0.52% accuracy on the Easy tier and 76.50±0.54% overall, exceeding same-model direct-label prompting. The system completes the MuJoCo suite and realizes all 29 authored variants on the physical Unitree G1 robot. Tests validate dialogue-context use and ordered multi-action composition.

Conclusion: EmoPose successfully bridges the gap between semantic flexibility and deterministic robot control, enabling expressive humanoid robot interaction without changing the control interface or delegating raw joint commands to foundation models. The framework demonstrates practical applicability through successful execution on both simulation (MuJoCo) and a physical robot (Unitree G1), with a four-stop laboratory tour showcasing expressive narration, interruption handling, camera-grounded dialogue, and navigation capabilities.
...
### [84] [HEARTH: An Object-Centric RGB-Thermal-3D Dataset for Temperature-Aware Robot Manipulation](https://arxiv.org/abs/2609.23418)
*Yuning Su,Borui Li,Yonghao Shi,Bofei Liu,Xing-Dong Yang*

Main category: cs.RO

TL;DR: HEARTH is an RGB-thermal-3D dataset of 90 objects with 145 states that enables robots to perform temperature-dependent manipulation tasks by providing thermal data alongside visual and geometric information.


<details>
  <summary>Details</summary>
Motivation: Language-guided robot manipulation often depends on physical properties (like temperature) not visible in appearance alone. Existing object datasets lack associations between measured temperatures and visual/geometric properties, limiting robots' ability to follow temperature-related instructions.

Method: HEARTH pipeline captures 90 physical objects from 18 categories (145 states), mapping apparent surface temperatures onto reconstructed meshes via camera calibration and pose transfer. The dataset provides raw temperature measurements, camera parameters, RGB-textured meshes, and thermal textures for simulation. Three LIBERO-derived tasks were created with 1,200 demonstrations used to fine-tune the pretrained VLA model π₀.₅.

Result: Ablation study shows adding thermal observations to the VLA increased success rates on temperature-dependent object-selection tasks from 35.0% (RGB-only baseline) to 75.0%.

Conclusion: HEARTH demonstrates significant utility for training robot policies to follow temperature-related instructions, proving that thermal sensing data substantially improves performance on manipulation tasks requiring temperature awareness.
...
### [85] [RiverVLN: Phase-Grounded Temporal Vision--Language Navigation for Unmanned Surface Vehicles](https://arxiv.org/abs/2609.23423)
...
### [86] [RopeFormer: Cross-Trial Adaptation from Interaction History for Dynamic Rope Manipulation](https://arxiv.org/abs/2609.23432)
*Menglin Wu,Kaixiang Yao,Shangbo Luan,Masayoshi Tomizuka,Yuxin Chen*

Main category: cs.RO

TL;DR: RopeFormer is a history-conditioned framework for dynamic rope manipulation that retains cross-trial action-response history without explicit parameter estimation, enabling effective control of previously unseen ropes.


<details>
  <summary>Details</summary>
Motivation: Dynamic rope manipulation is highly sensitive to unknown object dynamics; the same robot motion produces different responses across ropes, and explicitly identifying relevant physical properties is difficult.

Method: A history-conditioned framework that uses prior task interaction as context for subsequent control. The policy retains cross-trial action-response history while keeping its weights fixed and requires no explicit online rope-parameter estimation.

Result: In simulation, retaining context improved subsequent control relative to resetting checkpoints across all tasks. On the Unitree H1-2 robot with previously unseen ropes, target-acquisition time decreased by 30.9% for Rope Swing and 33.9% for Rope Twirl, while Rope Whip target hits increased from 0.2 to 2.3 out of 3.

Conclusion: Prior interaction provides effective control context for dynamic deformable-object manipulation, demonstrating the value of history conditioning in handling unknown object dynamics.
...
### [87] [Receding-Horizon Pushing with Composable Object-Centric Policies](https://arxiv.org/abs/2609.23439)
...
### [88] [BiRoAD: Learning Shared and Role-Adaptive Representations for Bimanual Manipulation](https://arxiv.org/abs/2609.23445)
...
### [89] [Smartphone GNSS Booster: Centimeter-Level Pedestrian Positioning Using a Portable Signal Re-Radiator](https://arxiv.org/abs/2609.23456)
*Taro Suzuki*
...
### [90] [STRIDER: Stepping-Enabled Multi-Gait Hierarchical 3D Loco-Manipulation Framework for Humanoid Robots](https://arxiv.org/abs/2609.23483)
...
### [91] [Cognitive Action Reasoning for Proactive Robots from Human-Centered Multimodal Observations](https://arxiv.org/abs/2609.23486)
...
### [92] [Design and Control of a Cable-Driven Switchable Actuator with Torque/Tension Dual Modes for Exoskeletons](https://arxiv.org/abs/2609.23487)
...
TL;DR: FeasibleFlow is a one-step end-to-end generative framework that jointly transports configuration-space feasibility fields and multimodal ego trajectories using Asymmetric Joint MeanFlow, Anchor-relative ranker (ARR), and Pareto-ReinFlow to balance safety and progress.


<details>
  <summary>Details</summary>
Motivation: End-to-end autonomous driving maps observations to trajectories, but trajectories must remain valid as scenes evolve. Future state modeling addresses temporal mismatch, but general representations contain irrelevant information and only affect trajectory generation through auxiliary supervision, static conditioning, or proposal evaluation.

Method: FeasibleFlow jointly transports feasibility field and trajectories. Uses Asymmetric Joint MeanFlow with pathwise Jacobian-vector product in MeanFlow identity to incorporate field evolution. Introduces Anchor-relative ranker (ARR) and Pareto-ReinFlow to balance safety and progress in candidate selection and generation.

Result: Experiments on NAVSIM benchmark demonstrate strong performance of FeasibleFlow, validating both the joint transport of feasibility and trajectories and the proposed safety-first mechanisms.

Conclusion: FeasibleFlow shows strong performance on NAVSIM benchmark, validating the joint transport approach and safety-first mechanisms.
...
TL;DR: UniPoint is a humanoid whole-body locomotion framework that fuses 360° LiDAR and depth cameras into a unified point-level representation using self-attention. It maintains wide coverage, local accuracy, and redundancy against sensor failure while decoupling computational cost from sensor count, achieving robust zero-shot generalization across diverse terrains.


<details>
  <summary>Details</summary>
Motivation: Open-world humanoid robot deployment requires perception that simultaneously provides wide coverage, local accuracy, and redundancy against sensor failure. Existing approaches fail to satisfy all three: single forward depth camera or nearby height sampling covers too little; odometry-corrected elevation maps drift under aggressive motion and miss thin vertical structures; image-level encoding costs grow with camera count.

Method: Multi-source point-level sensor fusion where 360° LiDAR and two depth cameras are early-fused into a base-frame point set. Voxelization resamples to a fixed number of tokens encoded by linear self-attention and proprioception-queried cross-attention, decoupling forward cost from sensor count. Training uses terrain-aware rewards, perception-degradation injection, and domain randomization to produce one policy for all terrain types.

Result: A single training run produces a deployable policy on RK3588 without fine-tuning. On a DR02 humanoid, validated across 70-cm-high platforms, 100-cm gaps, thin barriers, and sparse/narrow footholds (20 trials per setting, 7 terrain types), with zero-shot generalization outdoors. Single-modality failures degrade policy gracefully by removing only part of tokens.

Conclusion: UniPoint successfully addresses the limitations of existing approaches by providing a unified framework that maintains coverage, accuracy, and redundancy while being computationally efficient and deployable on real hardware, enabling safe humanoid locomotion across highly heterogeneous terrains without fine-tuning.
...
TL;DR: EgoWild2Dex transfers in-the-wild egocentric human experience to dual-arm dexterous robots by using GeoFormer to align human observations with robot views and a human-robot training scheme, achieving 96.7% task success rate on real robots.


<details>
  <summary>Details</summary>
Motivation: Prior approaches collect robot manipulation data in constrained environments, but the authors collect in-the-wild egocentric demonstrations in real settings (homes, factories, pharmacies) to capture diverse workflows and hand-object interactions across long-tailed distributions, though these yield visually challenging observations due to scene clutter and head-motion-induced viewpoint changes.

Method: EgoWild2Dex jointly aligns unstable egocentric views with robot observations and human motions with robot actions. Key components include: (1) GeoFormer, a differentiable geometric transformer that warps noisy human observations toward robot observations; (2) a human-robot training scheme to bridge the embodiment gap; (3) EgoWild dataset (538.9 hours, 179,049 episodes, 125,961 unique task descriptions, 1,282 object categories).

Result: On real robots, EgoWild2Dex achieves 96.7% average success rate across three long-horizon bimanual dexterous manipulation tasks and 33.3% average object-level zero-shot success rate.

Conclusion: The approach successfully transfers in-the-wild human experience to robots with high task success using limited robot supervision, demonstrating the value of diverse, uncontrolled egocentric data for dexterous robot manipulation learning.
...
TL;DR: PackLab introduces a comprehensive framework using multimodal large language models (MLLMs) for closed-loop robotic bin packing, outperforming traditional heuristics and reinforcement learning methods.


<details>
  <summary>Details</summary>
Motivation: Existing robotic bin packing methods rely on hand-crafted geometric heuristics or reinforcement learning with trial-and-error. While MLLMs show promise, their potential for closed-loop sequential decisions across heterogeneous packing configurations remains underexplored.

Method: PackLab comprises three components: PackLab-Suite (physics-based simulation for generating diverse training trajectories), PackLab-VLM (packing-specialized MLLM for jointly selecting objects and predicting placements), and PackLab-Bench (standardized scenarios at multiple difficulty levels for evaluation).

Result: PackLab-VLM outperforms conventional packing heuristics, traditional reinforcement learning methods, and general-purpose MLLMs across various object sets and container configurations, demonstrating superior performance in long-horizon robotic packing tasks.

Conclusion: The study demonstrates that specialized MLLMs can effectively handle the sequential decision-making challenges of robotic bin packing, highlighting their potential for long-horizon robotic manipulation tasks.
...
### [109] [Risk-Aware Motion Planning and Control under Unknown Dynamics with Hybrid Observations](https://arxiv.org/abs/2609.23792)
...
### [110] [ContactDP: Contact-Guided Diffusion Policy for Tight Insertion Tasks](https://arxiv.org/abs/2609.23800)
...
### [111] [Structured World-State Reasoning for Agentic Robotic Search](https://arxiv.org/abs/2609.23841)
...
### [113] [Grounded Action Model: 3D Grounding as a Foundation for Robotics](https://arxiv.org/abs/2609.23863)
*Gehao Zhang,Weikai Huang,Shailesh Shailesh,Yiyan Peng,Jiafei Duan,Ranjay Krishna*

Main category: cs.RO

TL;DR: GAMs introduce 3D grounding into robot foundation models, conditioning on language/points/box prompts to predict actions, achieving SOTA on RoboTwin 2.0 (55.3%), LIBERO-PRO (61%), and strong real-world performance under visual shift (17/20 vs 4/20).


<details>
  <summary>Details</summary>
Motivation: Current robot foundation models (VLAs, WAMs) use pretrained backbones that don't require metric grounding, forcing implicit learning from demonstrations which is inefficient and limits performance in challenging scenarios.

Method: GAMs use 3D grounding to transform language/points/box prompts into a shared object-centric representation capturing target-focused visual features and metric object geometry, then mix this with robot state history via multi-stream transformer to predict action chunks. Can serve as autonomous controller or low-level controller for high-level planners.

Result: 55.3% success on RoboTwin 2.0 (vs 52.0% Spatial Forcing), 47.6% under scene randomization (vs 30.4% Abot-M0); 61% on LIBERO-PRO (vs 53% π0.5, SOTA); 17/20 successes under visual shift on real bimanual YAM (vs 4/20 for π0.5); 64.7% ID and 49.8% OOD step completion with Molmo2 planner on Franka.

Conclusion: GAMs provide explicit metric grounding through 3D representation learning, achieving superior performance across simulation benchmarks and real robots, especially in challenging scenarios like scene randomization, target relocation, and visual shifts, demonstrating the value of grounded representations for robust manipulation.
...
### [114] [HumynexSurg-1: A Curated Expert Liposuction Dataset](https://arxiv.org/abs/2609.23885)
...
### [115] [HapticWAM: Distilling Imagined Touch into a World-Action Model without Inference-Time Tactile Sensing](https://arxiv.org/abs/2609.23888)
...
### [116] [BarrierFormer: Transformer-Guided Predictive Barrier Enforcement for Safe Robot Control](https://arxiv.org/abs/2609.23896)
...
### [117] [ReVeal: A Reconstruction-Aware Real-to-Sim Framework for VLA Policy Evaluation](https://arxiv.org/abs/2609.23910)
...
### [118] [FinsSim: A Reality-Aligned Integrated Simulation Platform for Underwater Robot Learning](https://arxiv.org/abs/2609.23943)
...
### [119] [Topology-Informed Visual Prompting For Vision Language Action Policies](https://arxiv.org/abs/2609.23944)
...
### [120] [Opt2VLA: Force-Aware Vision-Language-Action for Contact-Rich Humanoid Whole-Body Manipulation](https://arxiv.org/abs/2609.23968)
...
### [121] [Anticipatory Robot Goalkeeping via Monotone Optimal Stopping](https://arxiv.org/abs/2609.23976)
...
### [122] [RoboTalk: Learning Multi-Robot Communication and Coordination from Multimodal Demonstrations](https://arxiv.org/abs/2609.23997)
*Dorian Benhamou Goldfajn,Mason Nakamura,Saaduddin Mahmud,Justin Svegliato,Kyle H. Wray,Shlomo Zilberstein*

Main category: cs.RO

TL;DR: RoboTalk introduces a synthetic data pipeline and dataset (7,950 multimodal trajectories across 53 mobile-manipulation kitchen tasks) to train small vision-language models for multi-robot collaboration via natural-language communication, achieving 77% success on novel tasks vs ~2% for untuned models.


<details>
  <summary>Details</summary>
Motivation: Multi-robot collaboration under partial observability is challenging. While natural-language communication offers promising coordination approaches, decentralized manipulation with joint learning of explicit inter-robot communication and skill-level action selection from multimodal demonstrations remains underexplored for small VLMs intended for on-device deployment.

Method: RoboTalk includes a synthetic data-generation pipeline and dataset featuring leader-follower planning protocol, tool calls (perception, manipulation, navigation, communication), rationale traces, and diversified natural-language communication across 53 mobile-manipulation kitchen tasks.

Result: Fine-tuning open-source models on the RoboTalk dataset achieves 77% success rate on novel held-out tasks, representing a significant improvement over untuned open-source models which had approximately 2% success rate.

Conclusion: The synthetic data approach effectively enables small VLMs to learn inter-robot communication and coordinated action selection for decentralized manipulation, substantially enhancing multi-robot collaboration performance in partially observable environments.
...
### [123] [Imagine-RL: Residual-Confidence-Guided Cross-Attention for World-Model-Augmented VLA Reinforcement Learning](https://arxiv.org/abs/2609.24033)
*Kejia Hu,Wentong Zhai,Bo Zhao,Shuai Liang*

Main category: cs.RO

TL;DR: Imagine-RL enhances action evaluation for contact-rich manipulation by augmenting frozen VLA policies with action-conditioned visual-torque imagination via a latent world model, enabling better policy steering with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Contact-rich manipulation requires evaluating actions based on future visual and contact consequences, not just current observations. Existing noise-space reinforcement learning methods (e.g., DSRL) that steer frozen Vision-Language-Action (VLA) policies ignore these future consequences, limiting their effectiveness.

Method: Imagine-RL augments noise-space VLA post-training with action-conditioned visual-torque imagination using a frozen Visual-Torque Latent World Model (VTLWM). The method employs: (1) a VTLWM that autoregressively predicts compact future representations without pixel reconstruction; (2) a current image-state-action query that attends to observed histories and predicted futures; (3) previous-window prediction residuals providing token-wise confidence priors to suppress unreliable future tokens. The action critic combines current evidence with predicted consequences to evaluate candidate actions and supervise the actor, while both VLA and VTLWM remain frozen.

Result: Across four real-robot tasks with 50 evaluation trials per task, Imagine-RL achieves significant improvements: (1) uses only 100 RL trajectories for training; (2) improves average success rate by 23.6% over DSRL; (3) improves average success rate by 60% over VLA baselines.

Conclusion: By incorporating future consequence prediction through a latent world model, Imagine-RL enables more reliable action evaluation and policy steering for contact-rich manipulation tasks while maintaining frozen VLA and VTLWM models, demonstrating that future-aware evaluation can substantially enhance performance with minimal training data.
...
### [124] [What Matters in Designing World Action Models: An Empirical Study](https://arxiv.org/abs/2609.24048)
...
### [125] [AquaOrbit: Sim-to-Real Reinforcement Learning for Underwater Target Orbiting under Intermittent Visual Feedback](https://arxiv.org/abs/2609.24054)
...
### [126] [Toward Human-in-the-Loop Robot Failure Recovery: Bridging Communication Gaps in Human-Robot Collaboration](https://arxiv.org/abs/2609.24055)
...
### [127] [Automatic Labelling for Bimanual Mobile Manipulation](https://arxiv.org/abs/2609.24059)
*Yupu Lu,Jia Pan*
...
### [128] [Safety Control of a Hyper-redundant Robot via Adaptive Weighted Control Barrier Functions](https://arxiv.org/abs/2609.24062)
*Zijian Cai,Kiwan Wong,Wenci Xin,Wei Xiao,Daniela Rus,Cecilia Laschi*

Main category: cs.RO

TL;DR: This paper proposes a weighted control barrier functions (W-CBFs) framework to enhance safe operation and tracking performance of hyper-redundant robots in confined spaces with uneven load distributions.


<details>
  <summary>Details</summary>
Motivation: Hyper-redundant robots excel at confined-space manipulation due to high dexterity, but they face challenges in safe operation within cluttered environments. Their slender structures also cause uneven load distributions and nonuniform tracking errors along the body, limiting their effectiveness.

Method: The authors develop a weighted control barrier functions (W-CBFs) framework that enforces safety constraints while reducing tracking errors caused by uneven loading. The method employs fixed weights initially, then an adaptive weighting strategy based on discrepancies between simulated and experimental performance using different mapping functions.

Result: The controller achieved significant improvements: under fixed weights, RMS tracking error was reduced by 59.6% in simulation and 87.7% in physical experiments compared to non-weighted methods. The adaptive weighting strategy further reduced errors by 21.9% (simulation) and 8.5% (experiments). In a cleaning task, the autonomous strategy achieved comparable or better area coverage than manual teleoperation while successfully avoiding collisions.

Conclusion: The W-CBF framework effectively addresses both safety constraints and tracking error issues in hyper-redundant robot manipulation. While performance degrades when obstacles are close to the robot body, the adaptive weighting strategy provides additional error reduction. The method demonstrates practical applicability in complex manipulation tasks like cleaning, outperforming manual teleoperation in collision avoidance.
...
### [129] [When Does Touch Matter? Charting the Vision-Interaction Gap in Cluttered Dexterous Grasping](https://arxiv.org/abs/2609.24068)
...
### [130] [Dexterous Robot Manipulation from Human Demonstrations via Contact-Anchored Retargeting and Residual Policy Learning](https://arxiv.org/abs/2609.24093)
...
### [131] [Ask Before It Tells: Benchmark-to-Robot Body-Cue Transfer for a Question-First Bedside Robot](https://arxiv.org/abs/2609.24099)
*Dongsik Yoon*

Main category: cs.RO

TL;DR: Nuni is a bedside robot prototype using a pose-centric hybrid pipeline with X3D-UGT RGB classifiers to detect distress cues, triggering questions rather than alerts. The hybrid approach achieved 0.71 macro recall versus 0.25-0.29 for RGB variants, and event injection tests showed successful state transitions.


<details>
  <summary>Details</summary>
Motivation: Body-cue recognition can support assistive robots, but benchmark accuracy doesn't guarantee reliable behavior from a robot-camera viewpoint. The research aims to evaluate how interaction policy can limit the consequences of uncertain perception.

Method: Two X3D-UGT RGB appearance classifiers (97.7% and 94.8% accuracy on NTU RGB+D) were compared with a pose-centric hybrid pipeline on 28 single-actor scripted clips. A question-first controller was tested through event injection.

Result: The hybrid pipeline achieved 0.71 six-way macro recall versus 0.25 and 0.29 for RGB variants. It produced question-triggering cues in 12/16 distress clips vs 2/16 and 3/16 for RGB variants, with 2/8 normal clips triggering unnecessarily. All 13 event injection state-transition trials passed.

Conclusion: Interaction policy can effectively limit the consequences of uncertain perception. While preliminary (no user study or medical validation), the results demonstrate how designing interaction strategies that work with imperfect perception can improve robot behavior reliability.
...
### [132] [CARE: Experience-Guided Atomic Corrective Execution for Vision-Language-Action Policies](https://arxiv.org/abs/2609.24118)
...
### [133] [ActiveArena: Benchmarking and Understanding Active Perception in Robotic Manipulation](https://arxiv.org/abs/2609.24124)
*Yibo Li,Enshen Zhou,Rui Chen,Yanjun Ding,Mengzhen Liu,Yi Han,Jiabo Zhan,Lipeng Wang,Shanghang Zhang,Lu Sheng*

Main category: cs.RO

TL;DR: The paper introduces ActiveArena-Sim (active-perception simulator), ActiveArena-Bench (35 tasks across 5 categories for visual exploration and interactive information acquisition), and ActiveArena-VLA (13 vision-language-action configurations). The benchmark reveals a substantial ID-OOD gap, showing that uniform memory sampling, increased memory capacity, proprioceptive inputs, and subtask supervision improve OOD generalization, while planner-guided memory management achieves performance close to best-performing configuration using only sparse memory.


<details>
  <summary>Details</summary>
Motivation: Active perception and manipulation are crucial for robots to interact with complex scenes. Existing benchmarks struggle to evaluate how robots effectively acquire and maintain information in memory in an active manner.

Method: ActiveArena-Sim (active-perception simulator with controllable viewpoints and large-scale workspaces), ActiveArena-Bench (35 tasks across 5 fine-grained categories covering visual exploration and interactive information acquisition), ActiveArena-VLA (13 vision-language-action configurations for controlled studies of memory writing, memory capacity, proprioceptive state, subtask supervision, and high-level planning).

Result: Benchmark results reveal a substantial ID-OOD gap. Uniform memory sampling, increased memory capacity under reliable write policies, proprioceptive inputs, and subtask supervision improve OOD generalization, while planner-guided memory management and decision-making achieve performance close to the best-performing configuration using only sparse memory.

Conclusion: ActiveArena provides a unified testbed to develop and diagnose models for active perception and manipulation.
...
### [134] [Phrase-Level Robotic Guqin Performance: Bimanual Motion Planning and Audio-Tactile Interaction Monitoring](https://arxiv.org/abs/2609.24133)
...
### [135] [BayesianGS-SLAM: Uncertainty-Aware Neural Rendering SLAM via Probabilistic Formulation](https://arxiv.org/abs/2609.24140)
...
### [136] [MimicAgent: Quadruped Skills via Prompt-to-Trajectory Generation](https://arxiv.org/abs/2609.24145)
...
### [137] [Object-Centric Conditioning for Visuomotor Flow Matching](https://arxiv.org/abs/2609.24155)
...
### [138] [GraspTune: Tactile-Driven Execution Refinement for Robust Grasping](https://arxiv.org/abs/2609.24180)
...
### [139] [StenoVLA-3D: 3D-Aware Reasoning VLA for Navigation Through Gastrointestinal Stenoses](https://arxiv.org/abs/2609.24187)
...
TL;DR: StenoVLA-3D is a 3D-aware vision-language-action framework for autonomous endoscopic navigation in stenotic regions, integrating point-maps, temporal modeling, and reasoning capabilities to improve geometric grounding and lesion reporting compared to existing VLA models.


<details>
  <summary>Details</summary>
Motivation: Autonomous endoscopic navigation requires predicting safe actions from texture-poor monocular observations and retaining lesion evidence after they leave the field of view. Existing vision-language-action (VLA) models are limited by their reliance on visual appearance and short-term context, which restricts geometric grounding and episode-level reporting capabilities.

Method: StenoVLA-3D introduces several key components: 1) Integration of point-maps into the Cosmos-Reason 2 backbone via learned geometry-gated fusion; 2) A temporal state branch to model traversal progress; 3) A reasoning-and-action backbone that predicts grounded reasoning alongside actions; 4) Dedicated heads for stenosis shape estimation and lesion report generation. Additionally, the authors introduce EndoCausal, a new episode-level dataset with lesion annotations, actions, and temporally grounded reasoning.

Result: On 40 held-out test episodes, StenoVLA-3D achieves 95.2% semantic accuracy and 83.4% action accuracy. On a physical 3-DoF endoscope with 36 trials each, it attains 88.9% task success in esophageal phantoms and 77.8% in colonic phantoms, substantially outperforming the evaluated baselines.

Conclusion: StenoVLA-3D demonstrates that integrating 3D-aware geometry, temporal modeling, and explicit reasoning capabilities into VLA frameworks significantly improves autonomous endoscopic navigation performance, particularly in challenging stenotic regions where texture-poor monocular observations are common.
...
### [140] [A Topological Representation with Object-Path Graphs for Open-Vocabulary Instance Navigation](https://arxiv.org/abs/2609.24189)
...
### [141] [Odometry-Aided Real-Time Mapping for Underwater Robots Using Forward-Looking Sonar](https://arxiv.org/abs/2609.24195)
...
### [142] [Audio-based UAV Localization with Adaptive Temporal Correspondence via Reinforcement Learning](https://arxiv.org/abs/2609.24218)
*Haoxiang Lei,Mingzheng Feng,Daotong Wang,Shenghai Yuan*

Main category: cs.RO

TL;DR: An audio-based localization framework with adaptive temporal correspondence that uses a reinforcement learning controller to dynamically determine audio window size and a Mamba-based network for 3D position estimation, achieving competitive accuracy with reduced latency compared to SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing audio-based localization methods rely on predefined fixed audio segment lengths, which limits temporal correspondence and creates a trade-off between sufficient acoustic evidence and timely localization.

Method: A probe segment extracts a compact acoustic state characterizing reliability and consistency; a reinforcement learning controller dynamically determines the required audio window size; a Mamba-based localization network with adaptive temporal feature modulation processes the selected segment for 3D position estimation.

Result: The method achieves competitive 3D localization accuracy with substantially reduced temporal correspondence latency compared to SOTA methods and exhibits strong generalization across scenarios.

Conclusion: The proposed adaptive temporal correspondence framework effectively addresses the trade-off between acoustic evidence and timely localization in audio-based anti-UAV early warning systems.
...
### [143] [OpenFlyScan: A Quality-Guided Aerial Reconstruction System for Consumer Drones](https://arxiv.org/abs/2609.24253)
*Zhongrui You,Zhen Li,Junli Liu,Zhigang Wang,Bin Zhao*

Main category: cs.RO

TL;DR: OpenFlyScan is a quality-guided aerial reconstruction system for consumer drones that integrates a Gaussian Splatting quality model, a reacquisition planner, and a mobile app to enable rapid, low-cost urban asset creation through targeted view selection.


<details>
  <summary>Details</summary>
Motivation: Constructing large-scale urban assets with 3D Gaussian Splatting (3DGS) is constrained by expensive equipment and delayed quality feedback, as preset surveys often leave complex surfaces insufficiently observed, with defects discovered only after reconstruction requiring return visits.

Method: OpenFlyScan integrates a GS quality model (trained on rendering errors to predict regional reconstruction quality), a reacquisition planner (generating complementary reacquisition strips), and a custom mobile app (supporting automated oblique surveys and data transfer without additional hardware).

Result: The quality model effectively identifies poorly reconstructed regions. In the Expo West field experiment, targeted reacquisition improved PSNR at additional views by 10.95 dB, demonstrating that OpenFlyScan enables rapid, low-cost urban asset creation with consumer drones.

Conclusion: OpenFlyScan successfully integrates capture, targeted reacquisition, and reconstruction to address the limitations of traditional 3DGS-based urban asset construction, providing a practical solution for rapid, cost-effective scene reconstruction with consumer drones.
...
### [144] [ME-Brain-1.0: Memory, Cognition and Action for Evolving Embodied Intelligence](https://arxiv.org/abs/2609.24271)
...
### [145] [vla.simd: Efficient CPU Inference for Language-Conditioned Manipulation](https://arxiv.org/abs/2609.24274)
...
### [146] [Performance-Preserving Online Adaptation in Social Navigation via Diffusion Steering](https://arxiv.org/abs/2609.24317)
...
### [147] [LIBERO-VPro: Benchmarking Closed-Loop Visual Robustness of Robotic Foundation Models](https://arxiv.org/abs/2609.24350)
*Huiqiong Li,Zhiting Mei,Anirudha Majumdar,Jingjing Chen,Yu-Gang Jiang,Bin Zhu*

Main category: cs.RO

TL;DR: LIBERO-VPro is a benchmark for systematically evaluating closed-loop visual robustness of robotic foundation models by systematically perturbing visual evidence during execution across four dimensions: Visual Evidence Degradation, Camera Staleness, Visual Source Consistency, and Task-Relevant Scene Variation.


<details>
  <summary>Details</summary>
Motivation: Robotic foundation models achieve strong performance on standard manipulation benchmarks, but these evaluations typically assume clean, timely, and consistent visual observations. There is a need to evaluate models under degraded or inconsistent visual conditions to assess their true visual grounding and adaptation capabilities.

Method: LIBERO-VPro introduces four complementary dimensions for perturbing visual evidence: Visual Evidence Degradation, Camera Staleness, Visual Source Consistency, and Task-Relevant Scene Variation. The benchmark spans 12 challenge categories, 96 experimental settings, and 3,296 task-condition cases, covering approximately 196,000 simulated episodes and 200 real-world rollouts on a Franka Research 3.

Result: Three vision-language-action models and three world-action models were evaluated. Results reveal that strong nominal performance often masks substantial weaknesses in visual grounding and adaptation. Models succeed despite severe object-level occlusion but degrade sharply when local interaction cues are disrupted or familiar spatial priors are violated. Models are highly sensitive to stale or missing observations and struggle when task preconditions change. Vision-language-action models and world-action models exhibit distinct robustness profiles.

Conclusion: LIBERO-VPro provides a systematic diagnostic framework for developing robotic foundation models that can more reliably ground and adapt their actions under challenging visual conditions. The benchmark reveals that visual robustness is multi-dimensional and architecture-dependent, highlighting the need for models that can maintain performance despite visual evidence degradation.
...
### [148] [Multi-Agent Transportation of Free-Flyers in Microgravity Via Pushing Interaction Under Human-in-the-Loop Control](https://arxiv.org/abs/2609.24376)
...
TL;DR: A safety-critical framework for cooperative transportation of passive targets in microgravity using chaser robots that push targets to track desired motions while ensuring safety. The approach uses delay-aware feedback control with Control Lyapunov Function (CLF) and Control Barrier Function (CBF) constraints within a mixed-integer thrust allocation program, validated through Gazebo simulations.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenges of cooperative transportation in microgravity where a team of chaser robots use only unilateral pushing contacts, requiring sparse, configuration-dependent actuation constraints. Chasers must physically relocate on the target body when pushing allocation changes. The framework needs to ensure safe target motion while tracking human-provided desired twists, handle intermittent control authority, and guarantee obstacle avoidance.

Method: A delay-aware feedback control architecture leveraging Control Lyapunov Function (CLF) constraints for stability and Control Barrier Function (CBF) constraints for safety, implemented within a mixed-integer thrust allocation program. This formulation enforces stability and safety of the target while enabling reference tracking.

Result: The framework enables reference tracking while guaranteeing obstacle avoidance with a circular obstacle despite intermittent control authority. It provides a foundation for human-supervised cooperative transportation of free-flyers in space environments, validated through Gazebo simulations.

Conclusion: The proposed framework demonstrates feasibility for cooperative transportation of passive targets in microgravity environments. By combining CLF/CBF constraints with mixed-integer thrust allocation, it successfully addresses the sparse actuation challenges of pushing-only interactions and provides a practical foundation for human-supervised space operations.
...
### [149] [Tactile-JEPA: Topology-Aware Self-Supervised Representation Learning for Distributed Tactile Sensors](https://arxiv.org/abs/2609.24385)
*Elizaveta Kovtun,Matvey Konovalov,Andrey Sakhovskiy,Semen Budennyy*

Main category: cs.RO

TL;DR: Tactile-JEPA is an efficient self-supervised pre-training method for tactile encoders that uses spatial arrangement and dual-scale masking to learn topology-aware representations, improving force estimation by 6.3% and orientation error by 20.8% across diverse tactile datasets.


<details>
  <summary>Details</summary>
Motivation: Tactile sensing is essential for contact-rich manipulation, especially with visual occlusion. Existing tactile encoders are trained from scratch from raw signals, limiting expressivity. Existing SSL approaches focus on vision-based tactile sensors, leaving distributed electronic skins (with sparse, irregular sensor arrangements) unaddressed, making direct reuse of visual SSL methods suboptimal.

Method: Tactile-JEPA uses spatial arrangement of tactile sensors to learn topology-aware representations. It predicts embeddings of masked sensing elements from unmasked remainder using sensor connectivity graph for spatial masking. Dual-scale masking captures both local contact details and global tactile surface state.

Result: Across three diverse datasets spanning magnetic and piezoresistive sensors, different robot embodiments, and single/paired-sensor configurations, Tactile-JEPA reduces force estimation error by 6.3% and in-hand orientation error by 20.8% over prior SOTA, with consistent gains in other downstream applications including policy learning.

Conclusion: The benefit of tactile sensing depends critically on the quality of encoder pre-training, a problem Tactile-JEPA addresses directly. By learning topology-aware representations, it significantly improves performance across diverse tactile sensing applications.
...
### [150] [Zeva-Ego: Egocentric Mid-Training with In-Context Causal Learning for Robot Manipulation](https://arxiv.org/abs/2609.24411)
*Bingjia Huang,Xin Ding,Fu Chen,Kun Li,Wei Sun,Hao Wu,Yunxin Liu,Ting Cao*

Main category: cs.RO

TL;DR: Zeva-Ego is a unified framework that learns physical priors from egocentric human videos (via Action-Centric Encoder ACE) and enables continual adaptation through robot interaction (via In-Context Causal Learning ICCL), demonstrating a scalable path toward embodied intelligence with human-to-robot data ratios of 4-5:1 and significant performance improvements through interaction.


<details>
  <summary>Details</summary>
Motivation: Egocentric videos provide a scalable source of physical interaction experience, but translating them into robot-executable knowledge and enabling continual adaptation remain challenging. The authors aim to bridge the gap between human demonstration data and robot-executable policies through a unified framework.

Method: Zeva-Ego consists of two key components: (1) Action-Centric Encoder (ACE) that converts egocentric visual transitions into action-centered supervision for Vision-Language-Action (VLA) model mid-training, and (2) In-Context Causal Learning (ICCL) that enables parameter-free adaptation from action-effect feedback during deployment. The framework learns physical priors from human experience and evolves through robot interaction.

Result: Scaling egocentric data to 10K hours improved RoboTwin success from 63.8% to 75.3%, matching 2K hours of robot demonstrations (74.7%), corresponding to an empirical human-to-robot data ratio of roughly 4-5:1. With accumulated interaction experience, ICCL further improved success from 58% to 89% within four attempts without any parameter updates.

Conclusion: These results demonstrate a scalable path toward embodied intelligence that can learn from human experience and continuously improve through its own interaction, showing that egocentric video data can effectively approximate robot demonstration data and that continual adaptation through interaction significantly enhances performance.
...
### [151] [Robotic Valve Turning: Axial Misalignment Correction Using Reaction Torque Feedback](https://arxiv.org/abs/2609.24413)
*Amit Kumar,Sri Harsha Turlapati,Gautami Golani,Yang Lin,Ravi N. Banavar,Domenico Campolo*

Main category: cs.RO

TL;DR: This paper proposes a haptic control law using reaction torques to correct axial misalignment during robotic valve manipulation. The method exploits the geometric relationship between misalignment error and reaction torques, achieving uniformly asymptotically stable control.


<details>
  <summary>Details</summary>
Motivation: Vision-based misalignment estimation suffers from calibration errors, occlusion, and uncertainty in contact geometry. Reaction torques arise directly from physical interaction between gripper and valve, providing a more reliable feedback signal.

Method: A stable controller is proposed that exploits the geometric relationship between the misalignment error vector and reaction torques. The control law is mathematically proven to be uniformly asymptotically stable.

Result: Simulations and experiments using a Kinova Gen3 robotic arm for misalignments ranging from -15° to 15° at three valve positions. The median absolute misalignment across 18 test cases was within 2.46° and median reaction torque within 0.23 Nm.

Conclusion: The proposed haptic control method is robust and effective for correcting axial misalignment during robotic valve manipulation, with significantly improved accuracy compared to vision-based approaches.
...
### [152] [FoldQuantVLA: Native Low-Bit Quantization of Vision-Language-Action Models via Consistent Folding](https://arxiv.org/abs/2609.24433)
*Hung T. Ho,Khanh D. Nguyen,Quang D. Nguyen,Thanh Q. Duong,Ngan Le,Meng Guo,Vien A. Ngo,An T. Le*

Main category: cs.RO

TL;DR: FoldQuantVLA is a post-training quantization framework that achieves 1.2-1.52× speedup with W4A4 quantization while improving robot task success from 80% to 92.5% with only 1ms additional latency.


<details>
  <summary>Details</summary>
Motivation: Low-bit vision-language-action inference must reduce observation-to-action latency while preserving robot behavior.

Method: FoldQuantVLA combines channel scaling and block Hadamard transforms with dynamic per-token quantization, using custom TensorRT plugins for W4A4 execution on Ada GPUs and Jetson AGX Orin. Language attention-output and feed-forward down projections retained at W8A8 without policy retraining.

Result: Achieves 1.20-1.33× speedup on Orin and 1.25-1.52× on desktop. Retains W8A8 for critical projections. Across four real-robot tasks, success rate improves from 80% to 92.5% over 80 trials per configuration.

Conclusion: Post-training quantization can significantly improve efficiency without retraining, maintaining or improving robot behavior with minimal additional latency.
...
### [153] [ScaleMPA: Rethinking Scalable RRT* Acceleration With a Grid-Native Representation](https://arxiv.org/abs/2609.24497)
*Zilong Wang,Yuzhou Chen,Xinyue He,Chen Zhang,Guanghui He*

Main category: cs.RO

TL;DR: ScaleMPA is a motion-planning accelerator for RRT* that uses a grid-native representation instead of tree-centric organization, reducing critical path and exposing fine-grained parallelism through multi-resolution grid search and hash-grid memory.


<details>
  <summary>Details</summary>
Motivation: Prior RRT* accelerators use tree-centric state organization, which reduces per-query cost but preserves superlinear end-to-end complexity and limits parallelism through structural dependencies, making real-time motion planning challenging in large and high-dimensional environments.

Method: ScaleMPA rethinks RRT* with a grid-native representation that replaces hierarchical traversal with direct grid-based access. It proposes a multi-resolution grid search engine and a hash-grid memory system to make this practical for sparse high-dimensional planning.

Result: Implemented in 28 nm CMOS, ScaleMPA achieves millisecond-level planning latency and delivers 4.7×--44.4× speedup over state-of-the-art motion-planning accelerators.

Conclusion: By using grid-native representation and eliminating hierarchical traversal, ScaleMPA reduces the planner critical path and exposes fine-grained parallelism, enabling practical real-time motion planning in high-dimensional sparse environments.
...
### [154] [A Monolithic Force-Proprioception Soft Acutuator Enabled by Single-Material 3D printing](https://arxiv.org/abs/2609.24499)
*Nan Huang,Lele Liu,Junfeng Lu,Yipan Zhu,Jiansheng Dai,Sicong Liu*

Main category: cs.RO

TL;DR: The Monolithic Force-Proprioception Soft (MFPS) design integrates actuation and sensing capabilities into a single-material soft actuator using one-step FDM fabrication, enabling self-sensing performance for soft robotics applications.


<details>
  <summary>Details</summary>
Motivation: Existing pneumatic proprioceptive actuators for soft robots suffer from assembly errors and stress concentrations caused by heterogeneous materials, limiting their reliability and performance.

Method: The researchers propose the Monolithic Force-Proprioception Soft (MFPS) design, which combines an Asymmetric Origami Bending (AOB) chamber and a Force-Proprioception Soft (FPS) sensor into a single-material structure fabricated via one-step Fused Deposition Modeling (FDM). The FPS sensor utilizes conductive thermoplastic polyurethane (TPU) whose resistance changes with strain, with comprehensive parametric analysis of sensing characteristics and FDM fabrication parameters.

Result: Experimental results demonstrate that the MFPS actuator achieves a 40° bending angle and 12.5 N output force, with a resistance change of 26.9% as the applied external force increases from 0 to 45 N. A two-finger force-proprioception gripper built with MFPS actuators successfully demonstrates both grasping and force-proprioception capabilities.

Conclusion: The MFPS design method provides a novel approach for developing self-sensing actuators, overcoming the limitations of assembly errors and stress concentrations associated with heterogeneous materials, while enabling integrated actuation and proprioception through single-material FDM fabrication.
...
### [155] [TACIT: Tactile Contact Supervision for Spatial Attention in Dexterous Manipulation](https://arxiv.org/abs/2609.24507)
*Yanhou Lai,Fucai Zhu,Ruiqiang Wang,Koichi Hashimoto*

Main category: cs.RO

TL;DR: TACIT uses measured tactile contacts from teleoperated demonstrations to supervise spatial attention for visuomotor policies trained from few demonstrations, achieving higher success rates on ball placement (66.7%) and peg insertion (73.3%) compared to vision-only and fusion approaches.


<details>
  <summary>Details</summary>
Motivation: Visuomotor policies trained from few demonstrations often fail to follow changes in object position because they lack spatial priors. Existing attention-based approaches obtain spatial priors from human annotations or visual models, requiring additional point annotations that are difficult to obtain.

Method: TACIT uses measured tactile contacts from teleoperated demonstrations to supervise spatial attention. Gaussian targets over preceding camera point clouds supervise an attention head whose pooled output conditions a visuotactile diffusion policy. Targets are used only during training; tactile observations remain inputs at inference.

Result: On a real-robot benchmark with ten demonstrations per task and five demonstrated placement regions, TACIT achieves 66.7% success on ball placement and 73.3% on peg insertion, compared to 10.0% and 20.0% for input-matched 3D visuotactile fusion and 20.0% and 43.3% for vision-only DP3. TACIT enters the 150 mm palm-to-object approach region within 12 seconds in all trials.

Conclusion: Measured tactile contact provides effective spatial supervision for approach behavior from few demonstrations within the evaluated workspace, demonstrating that supervision beyond branch capacity contributes to performance.
...
### [156] [InsertAnything: Generalizable Contact-Rich Precision Insertion from Simulation to Reality](https://arxiv.org/abs/2609.24511)
*Zhenghua Ma,Xinpan Meng,Zeyu Liu,Muyuan Ma,Hengdi Zhang,Houcheng Li,Long Cheng*

Main category: cs.RO

TL;DR: A reinforcement learning framework that trains precision insertion policies entirely in simulation for direct real-world deployment, achieving 95% success across multiple geometries and perfect scores on the ManipulationNet benchmark.


<details>
  <summary>Details</summary>
Motivation: Contact-rich precision insertion is critical for robotic assembly but challenging due to tight clearances (0.02 mm), alignment errors, and variations in geometry and clearance across parts that complicate policy reuse.

Method: Combines target poses with compact 3D fingertip force feedback for alignment search, uses a decoupled gated reward to coordinate alignment and insertion, and employs force-signal smoothing with state-independent standard deviations to stabilize learning.

Result: Policies achieved 20/20 perfect score on ManipulationNet peg-in-hole benchmark and 95% success across eight unseen real-world tasks with hexagonal training geometry, reducing peak contact forces under position errors.

Conclusion: Learning entirely in simulation yields precision insertion skills that can be directly deployed and generalized across different hole geometries and clearances, demonstrating the viability of simulation-to-reality transfer for complex manipulation tasks.
...
### [157] [Bridge3D: Enabling Vision-Language-Action Models to See and Act in 3D](https://arxiv.org/abs/2609.24525)
*Haoxuan Li,Sixu Yan,Lianghui Zhu,Xuanlai Tang,Shikang Wang,Xinggang Wang*

Main category: cs.RO

TL;DR: Bridge3D enhances 2D Vision-Language-Action models with both implicit and explicit 3D geometry guidance for precise robotic manipulation in 3D spaces, introducing Implicit Fusion and Explicit Conditioning strategies.


<details>
  <summary>Details</summary>
Motivation: VLA models are trained on 2D-centric observations, limiting their capacity for precise spatial manipulation. Previous methods enhance 3D awareness through implicit spatial priors but lack explicit geometry guidance.

Method: Bridge3D introduces two key strategies: 1) Implicit Fusion - enriches visual tokens with features from 3D foundation models; 2) Explicit Conditioning - integrates action denoising with explicit 3D semantic field. Also utilizes layer-wise linear probing to improve learning efficiency.

Result: Bridge3D achieves superior performance against state-of-the-art methods: 14.0 percentage point improvement over π0 on RoboTwin 2.0 benchmark, and 11.7 percentage point improvement over Spatial Forcing in real-world experiments.

Conclusion: Bridge3D demonstrates strong capabilities in high-precision and spatial-sensitive manipulation tasks, effectively enabling 2D pre-trained VLA models to "see" and "act" in 3D environments.
...
### [158] [Estimation and Control of Tensegrity Manipulator Kinematics based on Strut Inclination Angles](https://arxiv.org/abs/2609.24535)
*Tufail Ahmad Bhat,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: This paper presents a reduced-order model for shape and posture control of tensegrity-based continuum manipulators using an optimization approach with geometric constraints and IMU sensors, demonstrating the first experimental real-time IMU-based shape estimation on a full-scale tensegrity manipulator.


<details>
  <summary>Details</summary>
Motivation: Tensegrity-based continuum robots pose a fundamental challenge for closed-loop control due to their complex continuous bending configurations. Most existing continuum robot modeling approaches assume a continuous elastic backbone, which is not applicable to tensegrity manipulators composed of rigid struts and tensioned cables. Tensegrity-based continuum robots remain largely unaddressed in the literature.

Method: The manipulator is modeled as a serially connected parallel-link mechanism. The method is formulated as an optimization problem that uses geometric constraints of the tensegrity structure together with information from IMU sensors embedded in strut elements. Posture control is achieved using a simple Proportional-Integral (PI) controller.

Result: The proposed method can estimate the shape of both single-module tensegrity structures and multi-module tensegrity manipulators from arbitrary static configurations and successfully achieve desired postures.

Conclusion: This is the first experimental demonstration of a real-time IMU-based shape estimation method on a full-scale tensegrity manipulator, showing that the proposed method can effectively estimate shape and achieve posture control.
...
### [159] [MIRA: Real-Time Full-Duplex Human-Robot Interaction for Embodied Companions](https://arxiv.org/abs/2609.24547)
*Lijian Lin,Ye Zhu,Fan Zhang,Yunfei Liu,Baofeng Li,Xianwen Zeng,Jianan Wang,Yu Li*

Main category: cs.RO

TL;DR: MIRA is a unified framework for real-time embodied companion interaction that simultaneously predicts response text and expressive, streaming co-speech motion from user inputs, using a sliding window approach and dual-timescale policy to maintain temporal alignment while ensuring physical safety.


<details>
  <summary>Details</summary>
Motivation: Real-time embodied companions must infer intent from streaming speech, generate timely responses, and execute expressive, interruptible motions. Existing systems decouple dialogue orchestration from gesture synthesis using offline motion generation, leaving open how robots can dynamically synchronize response content, prosodic timing, and physical safety under incremental inputs and uncertain turn boundaries.

Method: MIRA predicts response text and embodiment cues from streaming user speech, dialogue history, and vocal affect. Discrete social behaviors are mapped to validated trajectories, while open-ended speaking uses streaming co-speech motion governed by a predict-more-than-commit sliding window for temporal look-ahead with limited physical commitment. CORTEX is a dual-timescale interaction policy managing low-latency streaming and deliberative turn decisions, backed by a robot-side execution layer enforcing physical safety constraints.

Result: Quantitative evaluations show competitive audio-motion alignment with state-of-the-art baselines, while real-robot deployment measurements characterize streaming responsiveness and interruption handling.

Conclusion: MIRA demonstrates a viable unified framework for real-time embodied companion interaction that successfully handles streaming inputs, generates synchronized responses and motions, and maintains physical safety during deployment.
...
### [160] [Smoothness as a Constraint for Stable Humanoid Locomotion](https://arxiv.org/abs/2609.24552)
*Utsav Panchal,Denis Kleyko,Unal Artan,Amy Loutfi*

Main category: cs.RO

TL;DR: DeCap is a constrained reinforcement learning algorithm for humanoid whole-body control that decouples smoothness into separate upper- and lower-body constraint groups using explicit physical motion limits, achieving 2.5x and 2.18x reductions in upper-body action rate and acceleration respectively while maintaining lower-body reactivity and transferring across diverse terrains.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots require whole-body control policies that are both task-responsive and physically smooth, with different smoothness requirements for upper vs lower body. Existing RL approaches impose smoothness through reward-based auxiliary terms that compete with task objectives, treat the body as uniform, and lack direct control over physical quantities responsible for smooth behavior.

Method: DeCap (Decoupled Constraint-aware policy) decouples whole-body smoothness into separate upper- and lower-body constraint groups, formulating smoothness as explicit constraints on physical motion limits rather than reward terms. It incorporates a bounded barrier penalty that activates proactively near feasibility boundaries while remaining bounded at the constraint limit to improve constraint satisfaction.

Result: DeCap reduces upper-body action rate by 2.50x and acceleration by 2.18x relative to reward-based smoothness policies, improves lower-body smoothness, reduces transient motion, and demonstrates that fixed smoothness constraints transfer across diverse terrains without requiring extensive reward tuning.

Conclusion: By using explicit physical constraints instead of reward-based smoothness, DeCap successfully addresses the different smoothness requirements of upper and lower body in humanoid control, achieving significant performance improvements while maintaining transferability and reducing the need for reward engineering.
...
### [161] [ARSTAG: An Agentic Real2Sim2Real System for Task-Specific Robot Data Generation](https://arxiv.org/abs/2609.24563)
...
### [162] [What do VLM-Based Vision-Language Navigation Models Rely on: Interpreting and Steering Policy Behavior](https://arxiv.org/abs/2609.24576)
...
### [163] [Learning tactile perception from high-bandwidth single-point sensing](https://arxiv.org/abs/2609.24621)
*Joseph Rigal,Emmanuel Virot,Caroline Pascal*

Main category: cs.RO

TL;DR: SpectRobot transforms single-point tactile signals into time-frequency spectrograms that preserve high-bandwidth temporal and frequency information, enabling effective robotic manipulation with sparse, off-the-shelf sensors.


<details>
  <summary>Details</summary>
Motivation: Existing tactile sensing approaches rely on spatially distributed sensor arrays, which are expensive, complex, and prone to wear. The authors aim to enable effective tactile sensing using only single-point measurements that preserve rich dynamic information.

Method: SpectRobot converts single-point tactile signals (vibration, force, strain) into compact time-frequency spectrograms using standard vision encoders. Sensors are mounted away from the contact surface but mechanically coupled to preserve dynamics. The framework processes these spectrograms using existing vision-based learning pipelines.

Result: Three key findings: (1) Single-point vibration signals enable solving visually occluded manipulation tasks; (2) Temporal history strongly influences policy performance, while bandwidth controls spectral information (up to 100kHz); (3) The same representation works across different tactile sensing technologies. High-bandwidth capabilities are achievable with off-the-shelf hardware.

Conclusion: SpectRobot demonstrates that sparse, high-bandwidth single-point tactile sensing can match or complement spatially dense arrays for embodied learning systems. The approach provides broader access to high-frequency tactile information using affordable hardware, potentially facilitating integration of contact dynamics into robotic manipulation.
...
### [164] [From Semantic Decisions to Feasible Trajectories: Self-Evolving LLM-Guided Optimal Control for Narrow-Space Parking](https://arxiv.org/abs/2609.24631)
*Zhengbao Yao,Yuanfu Luo,Kehan Xue*

Main category: cs.RO

TL;DR: SE-LLM-OCP integrates LLMs with optimal control for autonomous parking in narrow environments, using LLMs for high-level discrete decisions and optimal control for low-level feasibility, with online replanning and offline knowledge base evolution.


<details>
  <summary>Details</summary>
Motivation: Autonomous parking in nonconvex and narrow environments remains challenging. Optimal-control methods can enforce vehicle dynamics but suffer from nonconvexity issues; LLMs have strong semantic reasoning but struggle with dense trajectory generation that guarantees physical feasibility.

Method: SE-LLM-OCP is a unified framework where LLMs make high-level discrete maneuver decisions while an optimal-control module enforces low-level vehicle dynamics and collision constraints. Online, LLM proposes sparse maneuver plans decomposing tasks into short-horizon trajectory optimization problems. If the solver fails, LLM aggregates failure evidence for replanning. Offline, a structured decision-making knowledge base evolves from accumulated online failures.

Result: Experimental validation in simulation on both a car-like vehicle model and a differential-drive robot demonstrates that SE-LLM-OCP enables safer autonomous parking in narrow scenarios and successfully transfers the same maneuver representation to a different kinematic platform.

Conclusion: The hybrid approach combining LLMs' semantic reasoning with optimal control's physical feasibility provides a robust solution for autonomous parking in challenging nonconvex environments, with the added benefit of knowledge base evolution from failure analysis.
...
### [165] [Feasibility Distance Fields for Heterogeneous Constraints in Robot Configuration Space](https://arxiv.org/abs/2609.24632)
...
### [166] [Touch2Robot: Robot Touch in the Human Demonstration Loop](https://arxiv.org/abs/2609.24660)
...
### [167] [Think Like a World Model, Act Like a VLA: Distilling World-Model Representations into Compact Robot Policies](https://arxiv.org/abs/2609.24682)
...
### [168] [AC-DC: Adaptive Communication for Scalable Dynamic Average Consensus in Multi-Robot Ergodic Search](https://arxiv.org/abs/2609.24702)
...
### [169] [SPARSER: Sparse Variable Projection by Exploiting Separable Structure in Robotic Perception](https://arxiv.org/abs/2609.24708)
...
### [170] [LLM-based Conversational AI Knowledge Assistant for MyBuddy Humanoid Robot](https://arxiv.org/abs/2609.24742)
*Hanxiao Chen*
...
### [171] [Beyond Visual Quality: A Study of Test-Time Planning with World Action Models](https://arxiv.org/abs/2609.24745)
...
### [172] [D-JEPA: A Decision-Aligned Latent World Model](https://arxiv.org/abs/2609.24749)
...
### [173] [A Switched Adaptive Control Framework for Aerial Manipulators Under Dynamic Transitions](https://arxiv.org/abs/2609.24761)
...
### [174] [H2RBench: A Real-to-Sim Benchmark for Evaluating Human-to-Robot Transfer](https://arxiv.org/abs/2609.24778)
*Chuyang Xiao,Haotian Zhan,Sriram Krishna,Peilin Meng,Muhammad Zubair Irshad,Sergey Zakharov,David Held*

Main category: cs.RO

TL;DR: H2RBench is a Real2Sim benchmark for standardizing and comparing human-to-robot (H2R) transfer methods, built on real human video demonstrations and simulated robot demonstrations across four diverse manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Comparing different H2R transfer methods is challenging due to inconsistent evaluation settings (different task suites, scene layouts, object instances, and amounts of robot supervision).

Method: H2RBench provides a standardized Real2Sim protocol with real human video demonstrations and simulated robot demonstrations, covering four manipulation tasks with diverse interaction requirements. Multiple representative H2R transfer methods were evaluated to characterize their scaling behavior with human demonstration amounts and their ability to bridge the embodiment gap.

Result: Methods differ substantially in their ability to leverage additional human data. Simulation performance is broadly predictive of real-world robot performance with Pearson correlation r = 0.89, Spearman correlation ρ = 0.85, and Mean Maximum Rank Violation (MMRV) of 0.06 across method-task configurations.

Conclusion: H2RBench establishes a practical and scalable benchmark for comparative H2R evaluation prior to real-world deployment, enabling consistent assessment of different transfer strategies across standardized tasks and conditions.
...
### [175] [Uranus: Building the Next-Generation Simulation Infrastructure for Embodied AI](https://arxiv.org/abs/2609.24815)
...
### [176] [Minimum Time Trajectories for a Car-Like Mobile Robot Moving with Rigid Wheels Under Non-Sliding Constraints](https://arxiv.org/abs/2609.24832)
*Joseph Ben-Asher,Elon Rimon,Leeor Ravina*

Main category: cs.RO

TL;DR: This paper extends time-optimal trajectory planning for car-like mobile robots by incorporating dynamic non-sliding wheel constraints into the kinematic model, introducing three new path primitives and providing approximate analytic solutions.


<details>
  <summary>Details</summary>
Motivation: Previous work solved the time-optimal trajectory problem for kinematic car-like robots assuming pure rolling (non-sliding constraints), but these constraints require proper handling through the robot's dynamics rather than being addressed solely by kinematic models.

Method: The paper formulates non-sliding constraints based on robot dynamics and augments the kinematic model's 12 time-optimal path primitives with three new primitives associated with these constraints. Approximate analytic solutions are derived for the new primitives.

Result: The three non-sliding path primitives combined with the original 12 kinematic primitives constitute the complete set of car-like robot time-optimal trajectories. Examples along representative maneuvers demonstrate how the non-sliding constraints influence time-optimal trajectories.

Conclusion: Incorporating non-sliding wheel constraints through the three new path primitives provides a complete solution framework for time-optimal trajectories of car-like robots, with examples illustrating the constraints' impact on trajectory planning.
...
### [177] [PredActor: Predictive Action Diffusion for Steerable Onboard Humanoid Control](https://arxiv.org/abs/2609.24840)
...
### [178] [CAST: Collision-Aware Assembly with Construction Robots using Simultaneous Trajectory Estimation and Planning](https://arxiv.org/abs/2609.24841)
*Karthik Shaji,Chisung Kim,John D'Amato,Edvard Bruun,Frank Dellaert*

Main category: cs.RO

TL;DR: A single factor graph framework for trajectory estimation and planning in multi-robot construction systems, incorporating measured states, collision constraints, and learned cable constraints to enable synchronized, high-dimensional motion planning with handling of stiff, vibration-induced uncertainty.


<details>
  <summary>Details</summary>
Motivation: Multi-robot systems offer precision and safety benefits in construction, but face significant challenges including high-dimensional configuration spaces, substantial collision-avoidance constraints (robots, objects, boundaries), and stiff, vibration-induced uncertainty of heavy robotic systems.

Method: A single factor graph approach for trajectory estimation and planning that incorporates measured robot states, explicit collision constraints, and learned cable constraints, enabling handling of changing workspaces and vibration-induced uncertainty in heavy robotic systems.

Result: Successfully demonstrated on construction of a post-and-lintel structure using one robot arm as timber gripper and a second robot as nail-fastener.

Conclusion: The framework enables synchronized, high-dimensional robot motion planning that accounts for stiffness and vibration uncertainty, supporting adaptable construction scenarios with changing workspaces.
...
### [179] [Range-Aided SLAM Initialization Exploiting Accurate Heading Information](https://arxiv.org/abs/2609.24846)
...
### [180] [SE(3) Neural Potential Fields for 6-DoF Trajectory Planning Directly from Images Without Explicit 3D Reconstruction](https://arxiv.org/abs/2609.24864)
...
### [181] [DualWAM: Dual-System World Action Models for Asynchronous Global Planning and Local Refinement](https://arxiv.org/abs/2609.24868)
...
### [182] [Steerable and Reactive Grasping Through Modular Design with a Three-Point Interface](https://arxiv.org/abs/2609.24896)
...
### [183] [Visuomotor Robotic Pruning in Planar Orchards Using Hybrid Reinforcement Learning](https://arxiv.org/abs/2609.24906)
...
### [184] [Learning to Drive on Mars: Visual Multimodal Traversability Estimation for Off-World Navigation](https://arxiv.org/abs/2609.24952)
...
### [185] [DexTacWAM: A Visuo-Tactile World-Action Model for Dexterous Manipulation](https://arxiv.org/abs/2609.24976)
...
### [186] [MIGU: Multimodal Instruction Grounding under Uncertainty for Manipulation Planning](https://arxiv.org/abs/2609.24995)
*Mingke Lu,Anxing Xiao,David Hsu*

Main category: cs.RO

TL;DR: MIGU is a modular framework for multimodal instruction grounding that fuses geometric likelihood from eye-finger geometry with semantic priors from a VLM via Bayesian fusion, enabling robots to ground human instructions in 3D space and perform appropriate planning behaviors.


<details>
  <summary>Details</summary>
Motivation: Understanding natural human instructions combining language and gesture is crucial for deploying robots in human-centric environments, as these modalities provide complementary but uncertain cues.

Method: MIGU constructs a 3D geometric likelihood by propagating viewing-direction and depth uncertainty through eye-finger geometry while accounting for hand-direction estimation error. A vision-language model (VLM) provides semantic priors over candidate objects and regions, which are fused via Bayes-inspired fusion. The resulting belief supports behavior planning (direct or clarification) and connects to manipulation planning for mobile manipulation and tabletop task-and-motion planning.

Result: On a real-world benchmark, MIGU outperforms all evaluated baselines, demonstrating superior multimodal instruction grounding performance.

Conclusion: Ablation studies support the benefit of explicit multimodal uncertainty modeling, highlighting the importance of properly handling uncertainty when fusing geometric and semantic information for instruction grounding.
...
### [187] [Learning Beyond What Humans Can Demonstrate](https://arxiv.org/abs/2609.24996)
...
### [188] [Edge-centric Brain Transformer: An Edge-centric Functional Connectivity Learning Framework for fMRI-based Brain Disorder Diagnosis](https://arxiv.org/abs/2609.23782)
...
### [189] [MoSAT: Human Motion Generation from Spatial Audio and Textual Description](https://arxiv.org/abs/2609.23797)
*Shuyang Xu,Zhiyang Dou,Yiduo Hao,Zekun Li,Liang Pan,Jingbo Wang,Cheng Lin,Yuan Liu,Wenping Wang,Mingmin Zhao,Taku Komura*

Main category: cs.GR

TL;DR: The paper introduces a novel task of human motion synthesis jointly conditioned on spatial audio and natural language, introduces STAM dataset and MoSAT framework, and achieves state-of-the-art performance through hierarchical cross-attention design.


<details>
  <summary>Details</summary>
Motivation: Human motion is shaped by both external acoustic events (spatial audio conveying environmental cues) and behavioral intent (text specifying desired action and performance). This paper addresses the largely overlooked task of human motion synthesis jointly conditioned on spatial audio and natural language.

Method: The authors introduce STAM dataset (motion sequences paired with spatial audio and detailed textual annotations) and MoSAT framework (a latent flow-matching framework for full-body motion generation). MoSAT uses hierarchical cross-attention to jointly condition on natural language intent and directional spatial-audio cues. They also develop tri-modal evaluators for comprehensive evaluation.

Result: Extensive experiments demonstrate that MoSAT achieves state-of-the-art (SOTA) performance by leveraging spatial audio's intrinsic motion-shaping properties alongside textual semantics, enabling precise and diverse motion generation in various scenarios.

Conclusion: The hierarchical cross-attention design enhances temporally coherent and semantically aligned motion sequences, successfully demonstrating that incorporating spatial audio into motion generation significantly improves the precision and diversity of generated human motions.
...
### [190] [Constrained Program Generation for 3D Reaction Animation with a 0.8B Model](https://arxiv.org/abs/2609.24457)
