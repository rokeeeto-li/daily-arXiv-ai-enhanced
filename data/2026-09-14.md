...
TL;DR: Pelican-Sim 1.0 is a general world model simulator for embodied intelligence that predicts future observations from visual context and robot actions. It achieves 5.67× speedup and 4.6-10.3 PSNR improvements across benchmarks, enabling 70-93% policy success and 0.994 Pearson correlation in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Develop a general world model simulator for embodied intelligence that predicts future observations from visual context and robot actions to support downstream learning and decision making.

Method: Four key design features: (1) Unified 28-dimensional action value space; (2) Action-visual injection using URDF- and camera-rendered action videos; (3) Sparse mixture-of-experts (MoE) layers; (4) Efficient rollout generation via causal adaptation and few-step distillation (4-step autoregressive).

Result: Trained on ~1M trajectories, achieving +4.636 PSNR on AgiBotWorld Beta, +2.080 on RoboMIND, +10.343 on RoboTwin; 5.67× speedup; downstream: 70%→93% policy success, 0.994 Pearson correlation, 47.7% action selection gain, 20.3% policy improvement gain.

Conclusion: Qualitative generalization across trajectory, scene, object, embodiment, and viewpoint shifts demonstrates potential as a general-purpose world model simulator.
...
### [2] [FoldNet++: a Large-Scale Synthetic Dataset for Robotic T-Shirt Folding and Unfolding](https://arxiv.org/abs/2609.12433)
...
### [3] [ProClosure: Hierarchical Room-Object Assignment using Progressive Boundary Closure from Monocular Video](https://arxiv.org/abs/2609.12614)
*Vinoth Kumar Muthuraj,Soumyadeep Banik,Kushal Sharma,Hardik Jain*

Main category: cs.RO

TL;DR: Progressive Boundary Closure is a method for recovering room layer from monocular RGB video using 3D scene graphs, improving room segmentation and object-to-room assignment accuracy by progressively sealing openings at their own scale rather than using fixed radius.


<details>
  <summary>Details</summary>
Motivation: 3D scene graphs group objects into rooms for robot navigation, but objects recorded in wrong rooms are not retrievable. Prior methods treat both doorways and unobserved wall gaps as passages, merging rooms that should remain separate. Need to properly distinguish and handle these openings.

Method: Uses SLAM front end and open-vocabulary segmenter to get structural point cloud, camera trajectory, and object tracks. Rasterizes cloud into top-down map, recovers rooms from it, and assigns objects to rooms holding most of their extent. Progressively thickens boundary inward and freezes free-space regions once enclosed. Uses camera poses as seeds instead of sampling heuristics, sealing openings at their own scale.

Result: Tested on 10 floors of 6 HM3D-Semantics scenes against HOV-SG on identical top-down maps: recovered 74 rooms for 72 annotated regions (HOV-SG: 44), Room F_1 increased from 0.741 to 0.890 at IoU 0.25, Object-to-room ARI increased from 0.488 to 0.696 (p=0.002).

Conclusion: Progressive Boundary Closure significantly improves room segmentation and object-to-room assignment, outperforming prior methods on all floors and achieving better room F_1 score (0.890 vs 0.741) and object-to-room ARI (0.696 vs 0.488).
...
### [4] [Improving Imitation Learning Efficiency for Manipulation through Geometric Prior Pretraining](https://arxiv.org/abs/2609.12721)
...
### [5] [Parameter Sensitivity Analysis for Aerial LiDAR-Inertial Odometries in low-altitude flights](https://arxiv.org/abs/2609.12837)
...
TL;DR: This paper enhances global path planning using a multi-model kinematic switching pure-pursuit controller that adapts to diverse terrains, employing a traversability graph, Heading-Aware A* algorithm, and real-time kinematic model switching to optimize path efficiency and energy use.


<details>
  <summary>Details</summary>
Motivation: The goal is to improve path planning performance across diverse terrains, particularly in challenging scenarios where standard approaches may struggle, by optimizing path efficiency and energy consumption.

Method: The system integrates a traversability graph for terrain analysis, a Heading-Aware A* algorithm for path generation, and a multi-model Pure Pursuit controller with adaptive kinematic modeling that enables real-time switching between kinematic models based on terrain features and robot states.

Result: Validated in simulation on the Artaban quadruped and X3 quadrotor drone, the approach demonstrates improved performance, robustness, and adaptability over standard baselines.

Conclusion: Adaptive kinematic modeling optimizes path efficiency and energy use in challenging scenarios, with the proposed multi-model pure-pursuit controller maintaining plan fidelity across diverse terrains.
