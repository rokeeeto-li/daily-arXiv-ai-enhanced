...
TL;DR: VLAct is a representation-centric VLM backbone trained on broad robot data that outperforms industrial VLA systems across simulation, real-world, and unseen-embodiment transfer tasks with minimal compute budget.


<details>
  <summary>Details</summary>
Motivation: Robot trajectories are costly and sparsely distributed compared to web-scale data, making representation quality the central bottleneck. Under fixed robot-data budgets, continued pre-training must convert limited trajectories into transferable visual-action knowledge rather than merely fitting actions.

Method: VLAct is a VLA-oriented VLM backbone trained on broad, heterogeneous, multi-embodiment robot data before task-specific fine-tuning. It preserves broad VLM priors and encourages shared action semantics through: (1) VLM-prior preservation, (2) multi-head continuous action co-supervision, (3) partially unified cross-embodiment action layout, while allowing task-specific action heads during fine-tuning.

Result: VLAct consistently improves downstream performance under fixed fine-tuning protocols: LIBERO-Plus (82.6% success rate, surpassing ABot-M0 and LingBot-VLA), RoboTwin 2.0 (92.5% success rate), RoboDojo (6th rank, outperforms all WAM entries), and RoboCasa-GR1 (using only 20% of downstream trajectories, outperforms full-data GR00T-N1.6 baseline).

Conclusion: Representation-centric continued pre-training can deliver highly competitive performance under modest compute budgets (16-GPU setup) and represents an important independent axis of VLA progress beyond data scaling, using fully open-source data.
...
TL;DR: A year-long field deployment in a subarctic boreal forest evaluates 64 km of data using nine odometry, localization, and mapping methods across seasonal changes, revealing that complex SLAM algorithms offer limited accuracy gains over proprioceptive baselines while increasing fragility, with lidar-based methods outperforming radar and visual approaches for cross-season localization.


<details>
  <summary>Details</summary>
Motivation: Subarctic regions face unique challenges for autonomous robots due to dense tree canopies and atmospheric attenuation that make GNSS and cloud computing unreliable. Existing exteroceptive modalities (cameras, lidars, radars) are typically evaluated in structured urban settings or environments without significant seasonal variations, leaving a gap in understanding performance in dynamic, seasonally-changing subarctic boreal forests.

Method: A mobile robot was deployed for one year in a subarctic boreal forest, collecting 64 km of data. The researchers evaluated nine odometry, localization, and mapping methods across seasonal changes. The evaluation included assessing performance degradation in self-similar scenes and tall snowbanks, comparing complex SLAM algorithms against a proprioceptive baseline, investigating cross-season localization in prior maps, and conducting multi-season Teach and Repeat (T&R) evaluations using both radar and lidar-based pipelines.

Result: Seasonal changes significantly hinder state-of-the-art techniques, increasing fragility in conditions with self-similar scenes or tall snowbanks. Complex SLAM algorithms provide limited accuracy gains over proprioceptive baselines while significantly increasing system fragility. Visual-based SLAM methods are particularly affected by seasonal changes. Lidar-based methods successfully completed localization runs between seasons, while radar and visual methods are prone to failure due to few matching features.

Conclusion: The year-long trial revealed critical challenges and lessons learned for deploying autonomous robots in subarctic environments, including the limitations of current SLAM algorithms in dynamic seasonal conditions and the relative advantages of lidar over radar and visual approaches for cross-season navigation. The multi-season Teach and Repeat evaluation demonstrates the practical difficulties and necessary adaptations for sustained autonomous operation in these extreme environments.
...
TL;DR: An iterative Linear Quadratic (LQ) game framework for multi-manipulator motion planning that enables coordinated, collision-aware trajectory generation by solving local LQ games with feedback Nash strategies and differentiable collision penalties.


<details>
  <summary>Details</summary>
Motivation: Multi-agent motion planning for high-DOF robotics manipulators is challenging: centralized planners suffer from poor scalability, while decentralized approaches face robustness and safety concerns. Game-theoretic formulations offer promise for modeling agent interactions but have limited application to articulated multi-arm systems.

Method: The method models each manipulator as an independent agent optimizing its own objective while interacting with other agents based on shared global states and collision constraints. It solves a series of local LQ games by linearizing dynamics and approximating the cost around a nominal trajectory, using Riccati backward recursions to yield feedback Nash strategies. Differentiable penalties for self-collision and inter-arm collision are incorporated into the optimization pipeline.

Result: Experiments demonstrate that the framework produces smooth, safe, and efficient trajectories in high-dimensional settings, outperforming traditional methods.

Conclusion: The differential game formulation effectively addresses the challenges of multi-robot manipulation, overcoming the limitations of centralized and decentralized approaches while enabling coordinated, collision-aware trajectory generation.
...
TL;DR: ROBUST TAMP is a modular LLM/VLM-guided reactive task and motion planning framework for partially observable environments. It constrains foundation-model planners to visible states, validates actions against executable interfaces, and triggers distinct replanning events for object discovery, enabling robust manipulation with hidden and non-target objects across six RLBench/CoppeliaSim scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing foundation-model-guided TAMP systems assume fully specified scene states or invoke replanning only after failures, making them brittle for partially observable manipulation where initially valid plans may succeed yet remain insufficient for task completion. The authors seek a reactive planning approach that handles dynamic visibility changes (object discovery) and incomplete scene information.

Method: The framework adopts a modular architecture: (1) restricts the foundation-model planner to the currently visible relational scene state; (2) validates generated task-level actions against a strict executable interface; (3) routes accepted actions to scene-specific execution adapters; (4) treats object discovery as a distinct replanning event; (5) after a stable execution horizon, reconstructs the visible scene state and replans using completed-action history and structured replanning context. Evaluations compare text-only LLM and VLM planners of varying sizes under identical validation, execution, monitoring, and replanning pipelines.

Result: Evaluations on six RLBench/CoppeliaSim kitchen and grill variants (involving hidden objects, non-target object discovery, articulated-container interaction, and temporal manipulation procedures) demonstrate that ROBUST TAMP enables successful task execution with appropriate discovery- and failure-triggered replanning behaviors, implicit non-target-object handling, and provides comparative insights into planner inference costs across different foundation model sizes.

Conclusion: The ROBUST TAMP framework demonstrates that constraining foundation-model planners to visible states, validating against strict interfaces, and triggering distinct replanning events for object discovery enables robust reactive TAMP in partially observable environments. The approach successfully handles hidden objects, non-target objects, and temporal manipulation procedures while providing quantitative analysis of planner performance across different model sizes.
...
TL;DR: The study explores how robot self-disclosure in small talk affects human-robot collaboration, finding that low-disclosure strategies enhance teaming and coordination, especially for users with prior HRI experience.


<details>
  <summary>Details</summary>
Motivation: People naturally self-disclose to build rapport; as robots increasingly collaborate with humans, understanding how to design effective social behaviors—particularly in non-anthropomorphic robots—is critical for engagement and teamwork.

Method: An LLM-driven industrial manipulator was programmed to engage in small talk with two self-disclosure strategies (low vs. high). A user study (N=50) examined how these strategies influence human-robot dynamics, with attention to prior HRI experience.

Result: Participants disclosed more in the low-disclosure condition and reported stronger teaming and coordination than in the high-disclosure condition. The effect was amplified for users with prior robot collaboration experience.

Conclusion: Increasing robot self-disclosure does not automatically improve rapport, social connection, or reciprocal disclosure. Designers should consider factors such as user prior HRI experience when planning robot-initiated small talk.
...
TL;DR: A hierarchical hybrid approach combining model-based bin-picking with a model-free exploration agent that uses gripper-stroke feedback and Wilson score intervals for autonomous grasp ranking, achieving 100% bin clearance on automotive parts.


<details>
  <summary>Details</summary>
Motivation: Model-based bin-picking suffers from deadlocks when predefined grasps are occluded, while model-free methods lack reliability. Existing approaches treat these techniques in isolation, and manual fine-tuning of grasp points is labor-intensive.

Method: Four-tiered hierarchical hybrid framework: model-based pipeline as robust backbone + model-free exploration agent for deadlock resolution and grasp discovery. Online self-learning mechanism uses gripper-stroke feedback and Wilson score intervals to autonomously rank grasp candidates, reducing manual commissioning.

Result: On three automotive parts: significantly outperforms model-free baseline in grasp success rate; improves model-based baseline bin clearance from 50.9% to 100%.

Conclusion: Combines precision of model-based methods with flexibility of model-free approaches, enabling truly autonomous, intervention-free industrial bin-picking operations.
...
TL;DR: A training-free suction grasping system for sorting deformed aseptic beverage cartons, using open-vocabulary detection, SAM2 masking, and geometric scoring to achieve 88.2% single-object grasp success and 72.6% end-to-end retrieval in clutter.


<details>
  <summary>Details</summary>
Motivation: Robotic sorting of recyclable waste is challenging due to the deformable and geometrically inconsistent nature of target objects, making effective grasping difficult.

Method: An open-vocabulary vision-language model detects cartons from text prompts, SAM2 refines detections into instance masks, and a geometric scoring method selects suction points by combining surface flatness with normal alignment. Three geometric methods are compared: k-nearest-neighbour PCA, Sobel cross-product, and RANSAC plane fitting.

Result: Evaluated on a real robot across three deformation levels and 35 cluttered scenes, achieving 88.2% single-object grasp success rate and 72.6% end-to-end retrieval success rate.

Conclusion: The proposed training-free approach effectively decouples target identification from grasp-point selection, demonstrating strong performance on challenging deformable objects despite their geometric inconsistency.
...
TL;DR: CoCoBench is a construct-level benchmark for evaluating multi-agent coordination in embodied agents, featuring 897 oracle-validated instances across four coordination constructs (task allocation, sequential ordering, mutual exclusion, handoff coordination). It reveals that coordination ability is construct-specific rather than uniform across MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing embodied-agent benchmarks lack fine-grained diagnostics for multi-agent coordination, focusing instead on single-agent tasks or overall success rates that obscure critical failures like duplicated work, ordering violations, resource contention, and desynchronized handoffs.

Method: CoCoBench introduces 897 oracle-validated instances organized around four recurring coordination constructs: task allocation, sequential ordering, mutual exclusion, and handoff coordination. It provides construct-level scores alongside task success rates to enable granular evaluation.

Result: Evaluating 11 leading MLLMs demonstrates that coordination ability is highly construct-specific: strong overall performance does not imply balanced competence across different coordination types.

Conclusion: These findings highlight the need for targeted model architectures and interventions to improve multi-agent coordination capabilities rather than relying on general task performance metrics.
...
TL;DR: A real-time semantic navigation framework for UAVs that uses a Large Language Model to interpret natural language instructions, perform semantic reasoning over detected objects and spatial context, and generate efficient trajectories for the Object Goal Navigation task.


<details>
  <summary>Details</summary>
Motivation: To improve time efficiency in the Object Goal Navigation (ObjectNav) task for Unmanned Aerial Vehicles (UAVs), addressing limitations of prior methods that rely on offline reasoning or simulator-constrained action spaces.

Method: Combines real-time object detection, 3D spatial mapping, and polynomial spline interpolation for trajectory planning. A Large Language Model (LLM) interprets natural language instructions and performs semantic reasoning over detected objects and spatial context to prioritize high-probability search regions, enabling continuous semantic relevance updates in real time.

Result: Experiments in both simulated and real-world settings demonstrate reductions in mission duration while maintaining high search accuracy, validating the effectiveness of LLM-guided reasoning for time-efficient UAV-based ObjectNav.

Conclusion: The framework successfully operates in real time with continuous semantic updates, offering a significant improvement over prior offline methods in the ObjectNav task for UAVs.
...
TL;DR: PanelShield is a verifiable closed-loop safety planning framework for manual-guided industrial panel operation that uses LTL and Safety FSM to enforce constraints, reducing violations to 2.7% with 4.1s latency.


<details>
  <summary>Details</summary>
Motivation: Industrial panel operation is knowledge-intensive and safety-critical. Foundation-model-based planners lack computable, localizable, and reproducible mechanisms for violation detection and repair, despite strong semantic capabilities.

Method: PanelShield generates parameterized action primitive sequences from task-relevant manual evidence and applies dual formal verification with LTL and Safety FSM to enforce cross-step temporal correctness and local transition legality. When violations occur, it outputs structured counterexamples with the earliest violating step and cause for targeted repair.

Result: PanelShield improves complex safety-constrained task performance over foundation-model-only baselines, reduces violation rate to 2.7% with 4.1s total latency, and demonstrates end-to-end feasibility in real-world robotic experiments.

Conclusion: PanelShield offers a verifiable approach to robotic panel operation that balances flexibility, safety, and auditability through formal verification mechanisms.
...
TL;DR: This paper develops a game-theoretic framework for cooperative risk-aware exploration in multi-robot systems, where robots use altruistic behavior to distribute risk based on their value, reducing redundancy while maintaining map coverage.


<details>
  <summary>Details</summary>
Motivation: Effective deployment of multi-robot systems in hazardous environments requires not only determining where robots should gather information but also how risk should be distributed across heterogeneous team members to optimize team performance.

Method: The approach introduces a game-theoretic structure for trajectory planning that defines a Social Nash Equilibrium, where utility is shaped according to agent relatedness (inspired by Hamilton's rule). Each robot selects finite-horizon trajectories to maximize information gain while penalizing redundant exploration and expected hazard exposure. Heterogeneity is encoded through agent-specific value parameters. The exploration utility rewards area coverage and uncertainty reduction while penalizing redundancy and risk, enabling projected gradient-based waypoint optimization in a receding-horizon planner.

Result: Simulations demonstrate that altruistic planning reduces redundant exploration, improves inter-robot separation, and reallocates risk according to agent value while maintaining comparable map coverage. Hardware experiments with wheeled robots using single-integrator controllers and barrier certificates validate the approach, showing that planned waypoints can be successfully tracked.

Conclusion: The proposed game-theoretic framework effectively integrates ecologically inspired altruistic behavior into multi-robot exploration, enabling intelligent risk distribution that benefits team performance without sacrificing exploration efficiency. The Social Nash Equilibrium successfully encourages lower-valued robots to accept risk when it benefits higher-valued agents and improves overall team performance.
...
TL;DR: ChainSplat introduces a physics-inspired framework for jointly learning 3D geometry, appearance, kinematics, and dynamics of deformable linear objects (DLOs) like cables and ropes from multi-view RGB videos, enabling real-time robotic manipulation capabilities.


<details>
  <summary>Details</summary>
Motivation: Deformable linear objects (DLOs) such as cables, ropes, and hoses present significant challenges for robotic manipulation due to their high-dimensional configuration spaces and diverse behaviors arising from varying material properties. Existing methods relying on multi-stage pipelines and auxiliary depth inputs are error-prone under dynamic interactions, while their high-dimensional state representations make model-based control computationally expensive.

Method: ChainSplat represents a DLO as an open-chain structure of rigid links connected by revolute joints, yielding an analytic screw-theoretic model with a compact state representation parameterized by joint configurations. By integrating this formulation with Gaussian splatting, ChainSplat jointly recovers DLO dynamics, kinematics-aware 3D geometry, and appearance from multi-view RGB videos alone.

Result: ChainSplat achieves state-of-the-art performance in dynamics predictions, 3D geometry reconstruction, and RGB rendering across dynamic interactions. It enables real-time state and force estimation, as well as accurate model-based trajectory optimization, demonstrating practical utility for real-world robotic manipulation of DLOs.

Conclusion: ChainSplat's compact screw-theoretic representation combined with Gaussian splatting provides an efficient solution for DLO modeling that reduces computational overhead while improving accuracy, making it suitable for real-time robotic manipulation applications.
...
TL;DR: Aero Hand Open is a simulation-ready tendon-driven anthropomorphic hand that enables reinforcement learning policies to be trained entirely in simulation and deployed on the physical hand without fine-tuning or state estimation.


<details>
  <summary>Details</summary>
Motivation: Tendon-driven hands are more affordable than direct-drive hands because motors can be placed off the joints, but they are harder to learn due to complex cable transmission and non-independent joint control. The paper aims to address the difficulty of simulating and controlling these underactuated systems.

Method: The researchers developed Aero Hand Open, which includes: (1) a simulation model reproducing cable transmission mechanics, (2) an identified actuation map connecting motor commands to joint movements (including thumb's three-way coupling), and (3) a reinforcement learning package for policy training.

Result: Policies can be trained entirely in simulation and successfully deployed on the physical hand without fine-tuning or state estimation, demonstrating the effectiveness of their simulation-ready approach.

Conclusion: The paper releases the mechanical design, simulation model, actuation mapping, training environment, and deployment stack to enable reproducible research and development of tendon-driven robotic hands.
...
### [27] [Visual Cue Interactions in AR-Guided Needle Insertion: A Prostate Biopsy-Inspired Phantom Study](https://arxiv.org/abs/2608.27620)
*Xinrui Zou,Mingxu Liu,Thomas T. Jones,Braden Millan,Sandeep Gurram,Peter A. Pinto,Raisa Z. Freidlin,Alejandro Martin-Gomez*

Main category: cs.GR

TL;DR: This paper introduces and evaluates three AR visualization techniques (focus-and-context window, color-based proximity encoding, and explicit trajectory overlay) for needle alignment in ultrasound-guided percutaneous procedures, finding that their effectiveness depends on user expertise and visual context configuration.


<details>
  <summary>Details</summary>
Motivation: Ultrasound-guided percutaneous needle procedures are challenging because clinicians cannot directly visualize internal anatomy, increasing cognitive demand. While augmented reality (AR) offers promise by providing pertinent visual information, simply adding visual information can complicate depth perception and spatial understanding, making it essential to develop effective AR guidance strategies that consider meaningful visual cues.

Method: Three visualization techniques were developed and evaluated in a user study (n=26, including n=7 clinical experts) using a prostate-biopsy-inspired phantom: (1) a localized focus-and-context window, (2) a color-based proximity encoding, and (3) an explicit trajectory overlay. The study examined how these techniques affected targeting accuracy, completion time, retreat behavior, and subjective perception across different cue configurations.

Result: Cue effects depended on surrounding cue configuration and user expertise. For novices, explicit trajectory overlay improved targeting accuracy and reduced retreat behavior but slowed completion time when presented alone. For experts, the focus-and-context window reduced completion time and retreat events, while color-based proximity overlay improved completion time. Color cues were subjectively perceived as helpful even when accuracy effects were inconsistent.

Conclusion: AR guidance strategies for percutaneous interventions should consider both user expertise and visual context configuration, as different techniques benefit different user groups and their effectiveness varies based on how visual cues are arranged together.
...
### [28] [The Role of Mixed and Augmented Reality in Medical Visualization: Literature Review and A Context-Aware Taxonomy](https://arxiv.org/abs/2608.27644)
...
### [29] [What Will This Copper Look Like Later? Forecasting Surface Appearance and Rendering It as a PBR Material](https://arxiv.org/abs/2608.28102)
*Teejuta Sriwaranon,Borworntat Dendumrongkul,Tanapat Chamted,Pizzanu Kanongchaiyos*

Main category: cs.GR

TL;DR: A digital design pipeline predicts copper oxidation appearance 10 units ahead for rendering. Surprisingly, a learned spatio-temporal model fails to transfer to unseen specimens (less accurate than copying the last frame), while a simple closed-form global color extrapolation (no trained parameters) successfully generalizes, improving prediction accuracy by 13.4-55.5% depending on forecast horizon.


<details>
  <summary>Details</summary>
Motivation: Digital design requires accurate prediction of metal surface oxidation appearance to enable proper material rendering. Copper oxidation is complex and time-varying, making accurate forecasting essential for visual fidelity in design workflows.

Method: The paper presents a pipeline that forecasts copper appearance 10 accelerated units ahead, converting predictions into albedo, normal, roughness, and metallic maps for rendering. Training uses one specimen for checkpoint selection, while test set is a completely held-out specimen from a different day/condition. Three trained architectures (including a learned spatio-temporal model with monotone oxidation state) and a closed-form global color extrapolator are compared. Controls include photometric drift correction using a non-oxidizing reference region and moving-block bootstrap on 6 independent recording windows.

Result: The learned spatio-temporal model, most accurate within a single recording, performs worse than copying the last observed frame on unseen specimens. The closed-form global color extrapolation is the only forecaster that successfully transfers, improving over copy-last-frame by 13.4% (t+1) and 50.6% (t+10), with margins increasing to +16.7% and +55.5% at longer horizons. Photometric drift correction leaves these margins intact, and bootstrap analysis confirms significance of the larger margin.

Conclusion: The pipeline should use the closed-form global color forecaster for unseen specimens and reserve the learned model only for continuing an already-observed specimen. This is because learned susceptibility maps (indicating where corrosion begins) are specimen-specific and mislead on new specimens, whereas the global color trajectory represents shared characteristics across specimens.
...
### [30] [Transparency Rendering in Computer-Aided Design: Methodologies, Trade-offs, and Challenges](https://arxiv.org/abs/2608.28310)
*Grigoris Tsopouridis,Ioannis Fudos*

Main category: cs.GR

TL;DR: A comprehensive survey of transparency rendering techniques in Computer-Aided Design (CAD) environments, covering practical deployment strategies, algorithmic approaches, and future research directions.


<details>
  <summary>Details</summary>
Motivation: To review current transparency rendering approaches in CAD and identify application scenarios where accurate, performant transparency is critical for design inspection, communication, and decision-making processes.

Method: The paper systematically analyzes trade-offs between approximate and exact transparency techniques, compares performance-quality balance across desktop and mobile platforms, establishes criteria for method selection, identifies needs for CAD-specific benchmarks and quality metrics, examines importance-driven transparency and silhouette-based methods for emphasizing interior components, and discusses challenges in rendering transparent constructive solid geometry (CSG) objects.

Result: The survey characterizes current capabilities in CAD transparency rendering, identifies open problems, and establishes criteria for selecting appropriate rendering techniques based on performance-quality trade-offs across different platforms.

Conclusion: Transparency rendering in CAD visualization presents unique challenges including robustness, correctness, and integration with modern rendering pipelines, with clear directions for future research development.
