...
TL;DR: ITAS is a multi-agent Intelligent Teaching Assistant System with three layers (teaching, operational, feedback) designed to run LLM-based tutoring in real courses, handling 334 chat turns and 10,628 events without hallucinations.


<details>
  <summary>Details</summary>
Motivation: Large language model tutors are easy to build in notebooks but hard to run in real courses; addresses the "Blind Instructor Problem" where LLM tutors accumulate more data about students than instructors can access through routine channels.

Method: Three-layer architecture: (1) Teaching layer with Spoke-and-Wheel of specialist agents (Video, Code, Guidance) + Synthesizer + autograder; (2) Operational layer with four Cloud Run microservices, Cloud SQL session state, and Pub/Sub streaming to BigQuery; (3) Feedback layer with narrow-scope conversational agent for instructor questions using pseudonymized event streams.

Result: Pilot deployment (5 students, 1 course, 1 semester) demonstrated: 334 chat turns handled without task-boundary hallucinations; 10,628 events captured across five modules; two findings surfaced that the instructor acted on mid-semester.

Conclusion: The system is a workable end-to-end solution for running LLM-based Intelligent Tutoring Systems in real courses, though pilot results are interpreted as system-behavior evidence rather than learning-outcome evidence and do not claim generalizability.
...
TL;DR: MultiHedge is a hybrid LLM architecture that uses retrieval-augmented decision making conditioned on historical precedents, demonstrating that memory-augmented retrieval provides greater robustness and stability than increasing model scale alone in modular decision pipelines.


<details>
  <summary>Details</summary>
Motivation: Decision-making under changing conditions is challenging; existing approaches fail to generalize across shifting regimes and exhibit unstable behavior under uncertainty.

Method: MultiHedge combines an LLM for structured allocation decisions with retrieval-augmented historical precedents and canonical option strategies for execution, creating a hybrid memory-augmented architecture.

Result: Memory-augmented retrieval confers greater robustness and stability than increasing model scale alone in a controlled U.S. equities evaluation.

Conclusion: The paper contributes a controlled computational study demonstrating that memory and architectural design play a central role in robustness within modular decision systems.
...
TL;DR: A new benchmark measuring AI's ability to autonomously implement end-to-end machine learning pipelines from minimal task descriptions, finding Claude Opus 4.7 significantly outperforms others in implementing AlphaZero-style Connect Four pipelines, while GPT-5.4 shows anomalous time-budget usage suggesting potential prompt sensitivity rather than intentional sandbagging.


<details>
  <summary>Details</summary>
Motivation: Forecasting when AI systems will meaningfully accelerate AI research is critical for AI safety. Existing benchmarks measure broad capability but lack early warning signals for recursive self-improvement, requiring new metrics that assess autonomous research implementation capabilities.

Method: Proposed benchmark: measure AI's capability to autonomously implement end-to-end ML pipelines from past breakthroughs using only minimal task descriptions. Proof-of-concept implementation with AlphaZero-style pipeline for Connect Four on consumer hardware within 3-hour budget. Evaluated four frontier coding agents across eight trials each, plus GPT-5.4 time-budget anomaly investigation with 16-trial probe.

Result: Claude Opus 4.7 won 7/8 trials against Pascal Pons solver (statistically significantly better), others achieved ≤2/8 wins. Task went from impossible (Jan 2026) to near-saturation. GPT-5.4 showed anomalous low time-budget usage; follow-up probe increased usage but Bradley-Terry ratings showed only directional differences, not definitive sandbagging evidence.

Conclusion: The benchmark effectively differentiates AI agents in autonomous research implementation capabilities, with Claude Opus 4.7 demonstrating superior performance. The task's rapid progression from impossible to near-saturation suggests emerging AI research taste and autonomous pipeline implementation capability. GPT-5.4's time-budget behavior requires further investigation but doesn't conclusively indicate intentional sandbagging.
...
TL;DR: A capability-oriented testing approach for embodied agents in safety-critical applications like VLN that enables failure detection and attribution by combining adaptive test case generation, capability oracles, and a feedback mechanism.


<details>
  <summary>Details</summary>
Motivation: Embodied agents in safety-critical applications rely on multiple interdependent capabilities (perception, memory, planning, decision). Existing testing methods are system-level and provide limited insight into which capability deficiencies cause task failures, making failures difficult to localize and attribute.

Method: The authors propose a three-component approach: (1) adaptive test case generation via seed selection and mutation; (2) capability oracles for identifying capability-specific errors; and (3) a feedback mechanism that attributes failures to capabilities and guides further test generation.

Result: Experiments demonstrate that the proposed method discovers more failure cases and more accurately pinpoints capability-level deficiencies compared to state-of-the-art baselines.

Conclusion: The capability-oriented testing approach provides more interpretable and actionable guidance for improving embodied agents, enabling better understanding of capability-specific deficiencies.
...
TL;DR: The paper proposes a neural network-based method to estimate replanning benefits in Multi-Agent Path Finding (MAPF) scenarios with delayed agents using Action Dependency Graph (ADG) features, achieving up to 94.6% reduction in delay impact.


<details>
  <summary>Details</summary>
Motivation: Real-world MAPF execution faces desynchronization issues where delayed agents cause robust methods (like ADG) to require waiting, increasing execution cost. While rescheduling/replanning can reduce cost, it's expensive and may not always provide benefits. The paper aims to estimate when replanning is worthwhile to balance safety and efficiency.

Method: A fully connected feed-forward neural network is trained to estimate replanning benefits. The network takes as input a set of newly designed ADG-based features describing the robust execution state and potential delay impacts, and outputs an estimated benefit achievable by replanning. The model is trained and tested on a labeled dataset of 12,000 experiments.

Result: The neural network achieves up to 94.6% of the achievable reduction in delay impact, demonstrating effective estimation of replanning benefits in scenarios with delayed agents.

Conclusion: The proposed method successfully estimates replanning benefits in delayed MAPF scenarios, showing that neural networks can effectively identify when replanning operations are worthwhile, thereby reducing execution costs while maintaining safety.
...
TL;DR: Pythia is a multi-agent serving system that exploits workflow semantics in LLM applications to optimize performance, improving throughput and job completion time over state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Multi-agent architectures are increasingly used to decompose complex LLM workflows, introducing structured topology that could reduce runtime uncertainty. However, existing systems treat these workloads as generic traffic, missing optimization opportunities that arise from the predictable workflow patterns.

Method: Pythia captures workflow semantics through a simple interface at the serving layer, enabling new optimization opportunities that existing systems cannot exploit. This approach allows the system to understand and optimize based on the structured nature of multi-agent interactions.

Result: Pythia substantially improves throughput and job completion time over state-of-the-art baselines, addressing key bottlenecks including low prefix cache hit rates, severe resource contention from long-context requests, and suboptimal scaling that causes queuing delays.

Conclusion: By exploiting the structured nature of multi-agent workflows rather than treating them as generic traffic, Pythia provides a significant performance improvement over traditional LLM serving approaches, demonstrating that workflow semantics can be leveraged to substantially reduce runtime uncertainty and improve system efficiency.
...
### [7] [Power Foam: Unifying Real-Time Differentiable Ray Tracing and Rasterization](https://arxiv.org/abs/2604.24994)
*Shrisudhan Govindarajan,Daniel Rebain,Dor Verbin,Kwang Moo Yi,Anish Prabhu,Andrea Tagliasacchi*

Main category: cs.GR

TL;DR: A differentiable 3D representation that combines constant-time ray tracing efficiency with rasterization performance by generalizing Voronoi foams to bounded power diagrams and introducing an oriented surface formulation for decoupling geometry from appearance.


<details>
  <summary>Details</summary>
Motivation: Prior foam-based ray tracing representations enable constant-time ray traversal through explicit volumetric partitions, but their potentially unbounded cells hinder efficient tile-based rasterization.

Method: Generalizes Voronoi foams to bounded power diagrams with controllable cell extents to enable spatially bounded primitives without expensive Delaunay triangulations; introduces an oriented surface formulation that explicitly models interfaces between interior and exterior regions and decouples geometry from appearance by embedding differentiable texture directly on surfaces.

Result: The representation preserves state-of-the-art ray tracing efficiency while achieving rasterization performance competitive with current generation 3DGS (3D Gaussian Splatting).

Conclusion: Provides a practical path toward unified real-time differentiable rendering by bridging the gap between ray tracing and rasterization approaches.
...
### [8] [8DNA: 8D Neural Asset Light Transport by Distribution Learning](https://arxiv.org/abs/2604.25129)
...
### [9] [Cutscene Agent: An LLM Agent Framework for Automated 3D Cutscene Generation](https://arxiv.org/abs/2604.25318)
...
TL;DR: Cutscene Agent is an LLM agent framework for automated end-to-end cutscene generation in video games, featuring bidirectional engine integration via MCP, a multi-agent orchestration system, and a new hierarchical evaluation benchmark called CutsceneBench.


<details>
  <summary>Details</summary>
Motivation: Cutscenes are complex productions requiring seamless coordination across screenwriting, cinematography, animation, voice acting, and technical direction from multidisciplinary teams, taking days to weeks to produce minutes of content. This complexity necessitates automated solutions to streamline the generation process.

Method: The framework introduces three key components: (1) Cutscene Toolkit built on Model Context Protocol (MCP) for bidirectional integration between LLM agents and game engine, enabling agents to invoke operations and observe real-time scene state for closed-loop generation; (2) Multi-agent system with a director agent orchestrating specialist subagents for animation, cinematography, and sound design, augmented by visual reasoning feedback loops; (3) CutsceneBench, a hierarchical evaluation benchmark for cutscene generation that evaluates long-horizon, multi-step orchestration with strict ordering constraints.

Result: The paper evaluates a range of LLMs on CutsceneBench, demonstrating their capabilities in handling the complex, multi-step orchestration required for cutscene generation, which differs significantly from typical tool-use benchmarks that evaluate isolated function calls.

Conclusion: The Cutscene Agent framework successfully enables automated end-to-end cutscene generation with closed-loop capabilities, and CutsceneBench provides a necessary evaluation framework for assessing LLMs on this challenging task that requires multi-step tool orchestration with strict ordering constraints.
...
### [10] [MotionBricks: Scalable Real-Time Motions with Modular Latent Generative Model and Smart Primitives](https://arxiv.org/abs/2604.24833)
...
### [11] [VISION-SLS: Safe Perception-Based Control from Learned Visual Representations via System Level Synthesis](https://arxiv.org/abs/2604.24894)
*Antoine P. Leeman,Shuyu Zhan,Melanie N. Zeilinger,Glen Chou*

Main category: cs.RO

TL;DR: VISION-SLS is a method for nonlinear output-feedback control from RGB images that provides robust constraint satisfaction guarantees under uncertainty despite partial observability, sensor noise, and nonlinear dynamics.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of safe visuomotor control from high-resolution RGB images while providing robust constraint satisfaction guarantees. Key challenges include partial observability, sensor noise, nonlinear dynamics, and the need for scalability while maintaining safety guarantees.

Method: The method proposes: (i) a learned low-dimensional observation map from pretrained visual features with state-dependent error bounds, and (ii) a causal affine time-varying output-feedback policy optimized via System Level Synthesis (SLS). A scalable novel solver for the resulting nonconvex program leverages sequential convex programming coupled with efficient Riccati recursions.

Result: On two simulated visuomotor tasks (4D car and 10D quadrotor) with ≥512×512 pixels and a 59D humanoid task with partial observability, the method enabled safe, information-gathering behavior that reduces uncertainty while guaranteeing constraint satisfaction with empirically-calibrated error bounds. Hardware validation on a ground vehicle from onboard images showed outperformance of baselines in safety rate and solve times.

Conclusion: Learned visual abstractions coupled with an efficient solver make SLS-based safe visuomotor output-feedback practical at scale. The code implementation is available at https://github.com/trustworthyrobotics/VISION-SLS.
...
### [12] [An analysis of sensor selection for fruit picking with suction-based grippers](https://arxiv.org/abs/2604.24906)
*Eva Krueger,Marcus Rosette,Joseph R. Davidson*

Main category: cs.RO

TL;DR: This paper presents a phase-dependent multimodal sensing suite for a compliant suction-based apple gripper that identifies optimal sensors for different pick phases, achieving over 90% pick state classification accuracy and predicting failures within 0.09s.


<details>
  <summary>Details</summary>
Motivation: Robotic fruit harvesting often fails to reliably detect pick success due to compliant fruit and grippers, variable stem attachment, and occlusions in orchard environments, with minimal research on minimal sensor sets and phase-dependent sensing strategies for pick/slip detection.

Method: A multimodal sensing suite integrated into a compliant suction-based apple gripper that identifies which sensors are most informative at different phases of the pick, enabling predictive detection of failures. The approach evaluates phase-dependent multimodal sensors and identifies minimal sensor sets for reliable pick state classification using Random Forest and Multilayer Perceptron classifiers.

Result: Experiments in a real apple orchard show that Random Forest and Multilayer Perceptron classifiers detect successful picks and impending failures with over 90% accuracy. Random Forest predicts pick/slip events within 0.09 seconds of human-annotated ground truth.

Conclusion: The phase-dependent multimodal sensing approach with minimal sensor sets enables reliable pick state classification and early failure prediction, demonstrating significant improvement in robotic fruit harvesting reliability.
...
### [13] [asRoBallet: Closing the Sim2Real Gap via Friction-Aware Reinforcement Learning for Underactuated Spherical Dynamics](https://arxiv.org/abs/2604.24916)
*Fang Wan,Guangyi Huang,Tianyu Wu,Zishang Zhang,Bangchao Huang,Haoran Sun,Mingdong Chen,Chaoyang Song*

Main category: cs.RO

TL;DR: asRoBallet is the first successful deployment of reinforcement learning on humanoid ballbot hardware, featuring a high-fidelity simulation of omni-wheel mechanics and a friction-aware RL framework enabling zero-shot Sim2Real transfer.


<details>
  <summary>Details</summary>
Motivation: Ballbots have served as a canonical benchmark for underactuated and nonholonomic control, but transitioning from simulation (LQR/MPC) to actual hardware using RL is hindered by critical gaps in contact modeling, actuator latency, and safe hardware exploration.

Method: The study proposes: (1) a high-fidelity MuJoCo simulation explicitly modeling discrete roller mechanics of ETH-type omni-wheels to capture parasitic vibrations and contact discontinuities; (2) a Friction-Aware Reinforcement Learning framework for zero-shot Sim2Real transfer; (3) asRoBallet designed via subtractive reconfiguration repurposing components from an overconstrained quadruped; (4) a generalized iOS ecosystem transforming consumer electronics into a low-latency interface.

Result: The Friction-Aware RL framework achieves zero-shot Sim2Real transfer by mastering coupled rolling, lateral, and torsional friction channels at wheel-sphere and sphere-ground interfaces, enabling expressive humanoid maneuvers on the actual ballbot hardware.

Conclusion: asRoBallet demonstrates that RL can successfully address the reality gap challenges in humanoid ballbot control through proper contact modeling and friction-aware learning, providing a robust, low-cost research platform and an intuitive natural motion interface for complex nonholonomic systems.
...
### [14] [Libra-VLA: Achieving Learning Equilibrium via Asynchronous Coarse-to-Fine Dual-System](https://arxiv.org/abs/2604.24921)
...
### [15] [TEACar: An Open-Source Autonomous Driving Platform](https://arxiv.org/abs/2604.24934)
*Zhongzheng Zhang,Maxwell Ruyle,Andrew Kappes,Tyler Ruble,William Shaoul,Dana Moreno,Jack Penn,Ivan Ruchkin*

Main category: cs.RO

TL;DR: TEACAR is a 1/14- to 1/16-scale autonomous driving platform with modular mechanical architecture, hardware abstraction, and ROS 2-based software, featuring a four-layer deck structure that physically decouples sensing, computation, actuation, and power subsystems for scalable ITS validation.


<details>
  <summary>Details</summary>
Motivation: ITS increasingly rely on vision-based perception and learning-based control, requiring experimental platforms for realistic hardware-in-the-loop validation. Small-scale autonomous racing platforms are practical but suffer from limited modularity, high integration complexity, and restricted extensibility.

Method: TEACAR adopts a modular mechanical architecture with hardware abstraction and ROS 2 software, utilizing a four-layer deck structure that physically decouples sensing, computation, actuation, and power subsystems. The system was comprehensively evaluated using three CNN-based steering controllers, measuring mechanical stability, structural characteristics, inference latency, power consumption, and system operating time.

Result: The prototype demonstrated that TEACAR offers a scalable, modular, and cost-effective testbed for ITS research, education, and development, successfully supporting hardware-in-the-loop validation of learning-based control systems.

Conclusion: TEACAR provides a practical platform for ITS research and development, with its modular design and ROS 2 integration enabling flexible reconfiguration and extensibility for various perception and control algorithms.
...
### [16] [DiscreteRTC: Discrete Diffusion Policies are Natural Asynchronous Executors](https://arxiv.org/abs/2604.25050)
*Pengcheng Wang,Kaiwen Hong,Chensheng Peng,Katherine Driggs-Campbell,Masayoshi Tomizuka,Chenfeng Xu,Chen Tang*

Main category: cs.RO

TL;DR: DiscreteRTC introduces discrete diffusion policies as natural asynchronous executors for physical AI, replacing external corrections with native unmasking to achieve better performance with less computation and no fine-tuning requirements.


<details>
  <summary>Details</summary>
Motivation: Physical AI requires asynchronous execution to act while the world evolves, but flow-matching-based real-time chunking (RTC) is structurally suboptimal: its inpainting comes from inference-time corrections rather than base policy, yielding little pre-training benefit, requiring specific fine-tuning, heuristic guidance, and extra computation that inflates latency.

Method: DiscreteRTC replaces external corrections with native unmasking using discrete diffusion policies, which generate actions by iteratively unmasking. Early stopping provides adaptive guidance and reduces inference cost. The approach requires 0 lines of code for async inpainting and only 0.7x computation compared to generating actions from scratch.

Result: DiscreteRTC achieves higher success rates than continuous RTC and other baselines on dynamic simulated benchmarks and real-world dynamic manipulation tasks, with 50% higher success rate in real-world dynamic pick task compared to flow-matching-based RTC.

Conclusion: DiscreteRTC is simpler to implement, faster at inference, and better at execution. It is fine-tuning-free since inpainting is native, and early stopping provides adaptive guidance and reduces inference cost, making it a more efficient approach for physical AI.
...
### [17] [HANDFUL: Sequential Grasp-Conditioned Dexterous Manipulation with Resource Awareness](https://arxiv.org/abs/2604.25126)
*Ethan Foong,Yunshuang Li,Hao Jiang,Gaurav S. Sukhatme,Daniel Seita*

Main category: cs.RO

TL;DR: HANDFUL is a learning framework for sequential grasp-conditioned dexterous manipulation that models finger usage as a limited resource, using finger-level contact rewards and curriculum-based policy learning to prioritize resource-aware grasps that preserve finger functionality for subsequent subtasks.


<details>
  <summary>Details</summary>
Motivation: Prior dexterous manipulation research focuses on single-object, single-skill tasks, but many real-world scenarios require sequential tasks where robots must execute multiple skills while maintaining control over previously grasped objects. The authors identify that resource-aware grasps that conserve fingers for future actions are essential for successful sequential manipulation.

Method: HANDFUL models finger usage as a limited resource and uses finger-level contact rewards to encourage exploration of resource-aware grasps. These grasps are selected for downstream tasks via curriculum-based policy learning. The authors also introduce HANDFUL-Bench, a simulation benchmark with sequential dexterous manipulation tasks across multiple second-subtask objectives (pushing, pulling, pressing) under a shared grasp-conditioned setup.

Result: Extensive simulation results demonstrate that prioritizing resource-aware grasps significantly improves second-subtask success rates and robustness compared to a baseline approach that greedily optimizes the initial grasp before attempting the second subtask. The approach was validated on a real dexterous LEAP hand.

Conclusion: Resource-aware grasp planning is established as a key principle for multifunctional dexterous manipulation, enabling robots to maintain control over grasped objects while executing sequential manipulation tasks. The HANDFUL framework and HANDFUL-Bench provide a foundation for future research in this area.
...
### [18] [Dynamic UGV-UAV Cooperative Path Planning in Uncertain Environments](https://arxiv.org/abs/2604.25267)
*Ninh Nguyen,Srinivas Akella*

Main category: cs.RO

TL;DR: The paper presents a Dynamic UGV-UAV Cooperative Path Planning (DUCPP) framework where UAVs dynamically inspect and prune impassable edges in an uncertain road network to guide a UGV to its destination, with bidirectional strategies and multiple UAVs being evaluated for optimal performance.


<details>
  <summary>Details</summary>
Motivation: DUCPP addresses the need for autonomous ground vehicles to navigate uncertain road networks (with potentially impassable edges) in disaster response, emergency supply transport, and rescue operations where road conditions are partially unknown.

Method: The authors propose multiple cooperation strategies including a bidirectional approach, evaluating how multiple UAVs impact UGV path planning and computation time. The strategies are implemented and tested on 100 urban road networks.

Result: The bidirectional strategy achieves the best performance in most instances. Using multiple UAVs further reduces UGV travel time but increases computation time. The framework demonstrates effectiveness across the tested urban road networks.

Conclusion: The paper presents a robust framework for DUCPP that achieves efficient UGV-UAV cooperation for path planning in challenging and uncertain conditions, offering practical solutions for navigation where road conditions are not fully known.
...
### [19] [Optimal UGV-UAV Cooperative Partitioning and Inspection of Shortest Paths](https://arxiv.org/abs/2604.25284)
*Ninh Nguyen,Srinivas Akella*

Main category: cs.RO

TL;DR: This paper studies cooperative shortest path planning for a UGV assisted by a UAV in environments with unknown road blockages, generalizing the Canadian Traveller Problem and proposing an optimal path partitioning strategy that reduces UGV travel times by up to 30%.


<details>
  <summary>Details</summary>
Motivation: The original Canadian Traveller Problem (CTP) assumes all traversability status is revealed upon arrival at a vertex, which is unrealistic for UGVs. The researchers aim to investigate how UAV assistance can improve path planning efficiency when road blockages are only discovered when a robot reaches a damaged point.

Method: 1. Analyzes the case with k disjoint paths between start and goal, proving the worst-case competitive ratio for single UGV is 2k-1. 2. With UAV assistance and negligible initial transit/deadheading costs, shows ratio improves to 2(v_G/(v_A+v_G))k - 1. 3. For general graphs with non-negligible UAV costs, presents an optimal path partitioning strategy (UGV does prefix inspection, UAV does suffix inspection) and proves its optimality. 4. Evaluates on road networks from 50 most populous cities with randomized blockages.

Result: The proposed path partitioning strategy is proven optimal for general graphs. Experimental evaluation on road networks shows the proposed method reduces UGV travel times by up to 30%.

Conclusion: UAV assistance significantly improves path planning efficiency for UGVs in environments with unknown blockages, with the path partitioning strategy delivering substantial time savings while maintaining optimality guarantees.
...
### [20] [Slot-hopping Enabled Loiter Guidance and Automation for Fixed-wing UAV Corridors](https://arxiv.org/abs/2604.25292)
*Pradeep J,Siddhardha Kedarisetty,Ashwini Ratnoo*

Main category: cs.RO

TL;DR: This paper develops a semi-cooperative guidance strategy for inserting fixed-wing UAVs into loiter lanes with minimal disruption, enabling more compact UAV corridors through either direct slot rendezvous or coordinated slot hopping when necessary.


<details>
  <summary>Details</summary>
Motivation: Traffic congestion management in fixed-wing UAV corridors is challenging, and existing methods may not achieve optimal corridor compactness while minimizing disruption to UAVs already operating within the corridor.

Method: Building on a loiter-lane framework, the proposed semi-cooperative guidance strategy has incoming UAVs first attempt to rendezvous with existing empty loiter slots within speed bounds. If direct insertion is infeasible, a minimal number of loitering UAVs perform coordinated slot hopping to create suitably positioned empty slots.

Result: The feasibility and performance of the method are demonstrated through numerical simulations, showing that the approach enables more compact UAV corridors while minimizing disruption to already operating UAVs.

Conclusion: The proposed semi-cooperative guidance strategy successfully addresses traffic congestion management in fixed-wing UAV corridors by enabling efficient and non-disruptive UAV insertion through either direct slot rendezvous or coordinated slot hopping maneuvers.
...
### [21] [ANCHOR: A Physically Grounded Closed-Loop Framework for Robust Home-Service Mobile Manipulation](https://arxiv.org/abs/2604.25323)
*Jinhao Jiang,Shengyu Fang,Sibo Zuo,Yujie Tang,Yirui Li*

Main category: cs.RO

TL;DR: ANCHOR is a physically grounded closed-loop framework for robust mobile manipulation that aligns symbolic reasoning with verifiable physical state through three key mechanisms: physically anchored task planning, operability-aware base alignment, and minimum-responsible-layer hierarchical recovery.


<details>
  <summary>Details</summary>
Motivation: Existing mobile manipulation systems fail not due to semantic misunderstanding but due to inconsistencies between symbolic plans and evolving physical worlds. Three recurring limitations: (i) reliance on pre-scanned semantic maps that become inconsistent after scene changes; (ii) navigation endpoints selected without considering downstream manipulation feasibility (arrived but inoperable problem); (iii) undifferentiated global replanning that fails to contain local errors.
...
Result: Across 60 real-robot trials in previously unseen environments, ANCHOR improved task success from 53.3% to 71.7% and achieved a 71.4% recovery rate under perturbations.

Conclusion: Explicit physical grounding and structured failure containment are critical for robust mobile manipulation, demonstrating that aligning symbolic reasoning with verifiable physical state during execution enables reliable long-horizon operation in real domestic environments.
...
### [22] [ProDrive: Proactive Planning for Autonomous Driving via Ego-Environment Co-Evolution](https://arxiv.org/abs/2604.25329)
*Chuyao Fu,Shengzhe Gan,Zhuoli Ouyang,Yuhan Rui,Xiaowei Chi,Sirui Han,Jiankun Wang,Hong Zhang*

Main category: cs.RO

TL;DR: ProDrive is a world-model-based proactive planning framework for autonomous driving that enables ego-environment co-evolution through end-to-end training of a query-centric trajectory planner and a BEV world model.


<details>
  <summary>Details</summary>
Motivation: End-to-end autonomous driving planners typically generate trajectories from current observations alone, but real-world driving is highly dynamic and reactive planning cannot anticipate future scene evolution, often leading to myopic decisions and safety-critical failures.

Method: ProDrive jointly trains a query-centric trajectory planner and a bird's-eye-view (BEV) world model end-to-end: the planner generates diverse candidate trajectories and planning-aware ego tokens, while the world model predicts future scene evolution conditioned on them. Planner features are injected into the world model and all candidates are evaluated in parallel, preserving end-to-end gradient flow and allowing future outcome assessment to directly shape planning.

Result: On NAVSIM v1, ProDrive outperforms strong baselines in both safety and planning efficiency. Ablation studies validate the effectiveness of the ego-environment coupling design.

Conclusion: The bidirectional coupling enables proactive planning beyond current-observation-driven decision-making, effectively addressing the limitations of reactive planning in dynamic environments.
...
### [23] [Robust Graph Matching through Semantic Relationship Generation for SLAM](https://arxiv.org/abs/2604.25404)
...
### [24] [GS-Playground: A High-Throughput Photorealistic Simulator for Vision-Informed Robot Learning](https://arxiv.org/abs/2604.25459)
...
TL;DR: GS-Playground is a multi-modal simulation framework that integrates a high-performance parallel physics engine with batch 3D Gaussian Splatting rendering, achieving 10^4 FPS throughput and an automated Real2Sim workflow to address bottlenecks in embodied AI.


<details>
  <summary>Details</summary>
Motivation: Embodied AI is shifting toward vision-centric paradigms, but existing simulators face prohibitive computational overhead for photorealistic rendering and rely on labor-intensive manual modeling; the sim-to-real physical gap hinders transfer of contact-rich manipulation policies.

Method: GS-Playground comprises a novel high-performance parallel physics engine integrated with batch 3D Gaussian Splatting rendering for high-fidelity synchronization, and an automated Real2Sim workflow for reconstructing photorealistic, physically consistent, memory-efficient environments.

Result: The system achieves breakthrough throughput of 10^4 FPS at 640x480 resolution, significantly lowering barriers for large-scale visual RL; experiments on locomotion, navigation, and manipulation demonstrate effective bridging of perceptual and physical gaps across diverse embodied tasks.

Conclusion: GS-Playground effectively bridges the perceptual and physical gaps across diverse embodied tasks, providing a solution to computational, modeling, and sim-to-real transfer bottlenecks in embodied AI research.
...
### [25] [Egocentric Tactile and Proximity Sensors as Observation Priors for Humanoid Collision Avoidance](https://arxiv.org/abs/2604.25554)
*Carson Kohlbrenner,Niraj Pudasaini,William Xie,Naren Sivagnanadasan,Nikolaus Correll,Alessandro Roncone*

Main category: cs.RO

TL;DR: This paper explores how sensor properties (coverage, type, range) affect whole-body collision avoidance in humanoid robots using reinforcement learning, finding that raw proximity measurements can replace explicit object localization and that sparse non-directional signals are more sample-efficient than dense directional alternatives.


<details>
  <summary>Details</summary>
Motivation: Collision-free motion on humanoid robots requires effective sensor design. While tactile and proximity sensors resist occlusion better than cameras, there is no clear understanding of how to shape sensor properties (coverage, type, and range) to enable effective avoidant behavior.

Method: A reinforcement learning framework is developed for whole-body collision avoidance on the H1-2 humanoid robot. The researchers use dodgeball as a benchmark task and systematically ablate various sensor properties distributed across the robot's upper body to characterize their impact on learned behavior.

Result: The study finds that raw proximity measurements can substitute for explicit object localization provided the sensing range is sufficient. More importantly, sparse non-directional proximity signals demonstrate superior sample efficiency compared to dense directional alternatives.

Conclusion: This work characterizes how sensor properties shape learned avoidance behavior, providing insights that can guide the design of effective sensor configurations for collision avoidance in humanoid robots.
...
### [26] [Improving Sensing Coverage and Compliance of 3D-Printed Artificial Skins Through Multi-Modal Sensing and Soft Materials](https://arxiv.org/abs/2604.25563)
...
TL;DR: A hybrid time-of-flight (ToF) and self-capacitance (SC) sensing skin is presented for 3D-printed artificial skins, enabling multi-modal sensing integration with soft compliant coverings that support contact detection, scene reconstruction, and pressure-correlated tactile responses.


<details>
  <summary>Details</summary>
Motivation: While 3D-printed artificial skins offer scalable approaches for whole-body tactile and proximity coverage, prior implementations were limited to unimodal sensing and rigid materials, restricting their practical usability.

Method: The researchers developed a hybrid ToF and SC sensing skin featuring multi-modal sensing integration, soft compliant coverings for impact absorption and pressure sensing, and a streamlined electrical interface between printed conductive traces and external electronics. The system was implemented using six artificial skin units with 40 sensing elements over an FR3 robot arm.

Result: The combination of ToF and SC modalities successfully enables contact detection, scene reconstruction, and pressure-correlated tactile responses with the compliant covering.

Conclusion: Hybridizing ToF and SC modalities with compliant coverings demonstrates that 3D-printed artificial skins can achieve comprehensive tactile and proximity sensing capabilities, significantly improving upon prior unimodal, rigid implementations.
...
### [27] [SlicerRoboTMS: An Open-Source 3D Slicer Extension for Robot-Assisted Transcranial Magnetic Stimulation](https://arxiv.org/abs/2604.25661)
...
### [28] [GEGLU-Transformer for IMU-to-EMG Estimation with Few-Shot Adaptation](https://arxiv.org/abs/2604.25670)
*Miroljub Mihailovic,Luca Tonin,Stefano Tortora,Emanuele Menegatti*

Main category: cs.RO

TL;DR: An adaptive IMU-to-EMG learning framework that reconstructs muscle activation envelopes from inertial measurements using a Transformer encoder with GEGLU units, achieving strong cross-subject generalization and rapid personalization with minimal adaptation data.


<details>
  <summary>Details</summary>
Motivation: Reliable neuromuscular activation estimation is crucial for adaptive and personalized control in wearable robotics, but surface EMG is difficult to deploy robustly outside laboratories due to electrode sensitivity, signal non-stationarity, and strong subject dependence.

Method: The approach combines a Transformer encoder with Gaussian Error Gated Linear Units (GEGLU-Transformer) to reconstruct continuous muscle activation envelopes from wearable inertial measurements across heterogeneous movement conditions, enabling cross-subject generalization and rapid subject-specific personalization using minimal adaptation data.

Result: Under a strict leave-one-subject-out (LOSO) protocol on a multi-condition lower-limb biomechanics dataset, the method achieves r = 0.706 +/- 0.139 and R^2 = 0.474 +/- 0.208 without subject-specific adaptation. With only 0.5% adaptation data, performance increases to r = 0.761 +/- 0.030 and R^2 = 0.559 +/- 0.047, demonstrating rapid adaptation and early performance saturation.

Conclusion: Attention-based architectures combined with lightweight adaptation provide a practical and scalable alternative to direct EMG sensing for real-world wearable robotic applications.
...
### [29] [Learning-Based Dynamics Modeling and Robust Control for Tendon-Driven Continuum Robots](https://arxiv.org/abs/2604.25691)
*Ziqing Zou,Ke Qiu,Fei Wang,Haojian Lu,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: A differentiable learning framework combining high-fidelity GRU-based dynamics modeling with end-to-end neural control for tendon-driven continuum robots, achieving accurate tracking and robustness against unseen payloads by eliminating self-excited oscillations.


<details>
  <summary>Details</summary>
Motivation: Tendon-Driven Continuum Robots (TDCRs) present significant modeling and control challenges due to complex nonlinearities including frictional hysteresis and transmission compliance, which complicate long-horizon prediction and control.

Method: Develops a GRU-based dynamics model with bidirectional multi-channel connectivity and residual prediction to suppress compounding errors during auto-regressive prediction. Treats this model as a gradient bridge to optimize an end-to-end neural control policy via backpropagation, enabling implicit internalization of nonlinearity compensation.

Result: Experimental validation on a physical three-section TDCR demonstrates accurate tracking and superior robustness against unseen payloads, outperforming Jacobian-based methods by eliminating self-excited oscillations.

Conclusion: The proposed framework effectively addresses TDCR control challenges through integration of high-fidelity dynamics modeling and neural control, achieving superior performance compared to traditional Jacobian-based approaches.
...
### [30] [Reference-Augmented Learning for Precise Tracking Policy of Tendon-Driven Continuum Robots](https://arxiv.org/abs/2604.25698)
*Ziqing Zou,Ke Qiu,Haojian Lu,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: A reference-augmented offline learning framework using a differentiable RNN-based dynamics surrogate as a gradient bridge to optimize a control policy through multi-scale augmentation (stochastic bias, harmonic perturbations, random walks) for precise 6-DOF tracking control of Tendon-Driven Continuum Robots (TDCRs).


<details>
  <summary>Details</summary>
Motivation: TDCRs present control challenges due to highly nonlinear, path-dependent dynamics and non-Markovian characteristics. Traditional Jacobian-based controllers struggle with hysteresis-induced oscillations, while conventional learning-based approaches suffer from poor generalization to out-of-distribution trajectories.

Method: The proposed framework employs a differentiable RNN-based dynamics surrogate as a gradient bridge to optimize a control policy through an augmented reference distribution. A multi-scale augmentation scheme incorporates stochastic bias, harmonic perturbations, and random walks to force the policy to internalize diverse tracking error recovery mechanisms without additional hardware interaction.

Result: Experimental results on a three-section TDCR platform demonstrate a 50.9% reduction in average position error compared to non-augmented baselines, and significant outperformance of Jacobian-based methods in both precision and stability across various speeds.

Conclusion: The proposed reference-augmented offline learning framework achieves precise 6-DOF tracking control for TDCRs, effectively addressing the limitations of traditional Jacobian-based controllers and learning-based approaches by internalizing diverse tracking error recovery mechanisms through multi-scale augmentation.
...
### [31] [Sensitivity-Based Tube NMPC for Cooperative Aerial Structures Under Parametric Uncertainty](https://arxiv.org/abs/2604.25766)
*Giuseppe Silano,Quentin Sablé,Marco Tognon,Luigi Iannelli,Antonio Franchi*

Main category: cs.RO

TL;DR: A sensitivity-based tube Nonlinear Model Predictive Control (NMPC) framework for cooperative aerial chains under bounded parametric uncertainty, with input-rate actuation constraints.


<details>
  <summary>Details</summary>
Motivation: Cooperative aerial chains (multi-vehicle systems with rigid links) require robust control despite uncertainties in link parameters (mass, length, inertia). Traditional NMPC may fail under such uncertainties due to constraint violations.

Method: A planar two-vehicle chain with input-rate actuation (thrust and torque limits) is modeled. First-order parametric state sensitivities are propagated along the prediction horizon and used to compute online constraint-tightening margins. The inter-link separation constraint (smooth cosine embedding) and thrust-magnitude bounds are robustified. The method is implemented in MATLAB.

Result: Improved constraint margins under uncertainty while maintaining tracking performance comparable to nominal NMPC. Monte-Carlo uncertainty sampling demonstrates robustness.

Conclusion: The sensitivity-based tube NMPC approach effectively handles parametric uncertainties in cooperative aerial chains while preserving control performance and meeting actuation constraints.
...
### [32] [KinDER: A Physical Reasoning Benchmark for Robot Learning and Planning](https://arxiv.org/abs/2604.25788)
...
TL;DR: KinDER is an open-source benchmark for Kinematic and Dynamic Embodied Reasoning in robotics, featuring 25 procedurally generated environments that isolate five core physical reasoning challenges (spatial relations, nonprehensile manipulation, tool use, combinatorial geometric constraints, dynamic constraints), a Gymnasium-compatible Python library with parameterized skills/demonstrations, and an evaluation suite with 13 baselines spanning task/motion planning, imitation learning, reinforcement learning, and foundation-model-based approaches.


<details>
  <summary>Details</summary>
Motivation: Robotic systems must reason about kinematic and dynamic constraints from their embodiment, environment, and task. Current approaches to physical reasoning in robotics have substantial gaps, necessitating a standardized benchmark for systematic comparison.

Method: KinDER comprises 25 procedurally generated environments designed to isolate five core physical reasoning challenges (basic spatial relations, nonprehensile multi-object manipulation, tool use, combinatorial geometric constraints, dynamic constraints), a Gymnasium-compatible Python library with parameterized skills and demonstrations, and a standardized evaluation suite with 13 implemented baselines spanning task and motion planning, imitation learning, reinforcement learning, and foundation-model-based approaches.

Result: Empirical evaluation demonstrates that existing methods struggle to solve many KinDER environments, indicating substantial gaps in current approaches to physical reasoning. Real-to-sim-to-real experiments on a mobile manipulator confirm correspondence between simulation and real-world physical interaction.

Conclusion: KinDER is fully open-sourced to enable systematic comparison across diverse paradigms for advancing physical reasoning in robotics.
...
### [33] [Privileged Foresight Distillation: Zero-Cost Future Correction for World Action Models](https://arxiv.org/abs/2604.25859)
*Pengcheng Fang,Hongli Chen,Xiaohao Cai*

Main category: cs.RO

TL;DR: Privileged Foresight Distillation (PFD) transfers future-conditioned correction from a world action model teacher to a current-only student model by distilling the residual between predictions using true future vs. current frames only, enabling performance improvements at negligible inference latency.


<details>
  <summary>Details</summary>
Motivation: World action models jointly predict future video and action during training. Recent findings show the future-prediction branch can be removed at inference with little loss, suggesting future information may act merely as a regularizer. The authors propose instead that joint training induces an action-conditioned correction that privileged future observations impose on action denoising, and that current-only policies capture this correction only partially.

Method: Privileged foresight is formulated as a residual in the action-denoising direction—the difference between what a model predicts given true future versus only the current frame. Privileged Foresight Distillation (PFD) transfers this residual from a training-time teacher into a small adapter on a current-only student. Teacher and student share the same backbone and differ only in attention mask over video tokens; future video is never generated at inference.

Result: Controlled experiments verify that gains reflect genuine future-conditioned correction rather than side effects of capacity or regularization. PFD achieves consistent improvements on LIBERO and RoboTwin manipulation benchmarks while preserving current-only inference interface at negligible added latency.

Conclusion: Future information in world action models should be reframed not as a target to predict, nor as a regularizer to absorb, but as a compressible correction to be distilled.
...
### [34] [Variational Neural Belief Parameterizations for Robust Dexterous Grasping under Multimodal Uncertainty](https://arxiv.org/abs/2604.25897)
*Clinton Enwerem,Shreya Kalyanaraman,John S. Baras,Calin Belta*

Main category: cs.RO

TL;DR: This paper introduces a variational inference framework for risk-sensitive grasp planning that uses differentiable Gaussian mixture models and Gumbel-Softmax reparameterization to optimize tail robustness through differentiable CVaR surrogates, achieving faster planning and better calibration than particle-filter approaches.


<details>
  <summary>Details</summary>
Motivation: Contact variability, sensing uncertainty, and external disturbances make grasp execution stochastic; expected-quality objectives ignore tail outcomes leading to failures under adverse contact realizations. Existing risk-sensitive POMDP approaches use particle-filter beliefs that scale poorly, obstruct gradient-based optimization, and estimate CVaR with high-variance approximations.

Method: Formulate grasp acquisition as variational inference over latent contact parameters and object pose, representing belief with a differentiable Gaussian mixture. Use Gumbel-Softmax component selection and location-scale reparameterization to express samples as smooth functions of belief parameters, enabling pathwise gradients through a differentiable CVaR surrogate for direct optimization of tail robustness.

Result: In simulation, variational neural belief improves robust grasp success under contact-parameter uncertainty and exogenous force perturbations while reducing planning time by ~10x vs particle-filter MPC. On a serial-chain robot arm with multifingered hand, controller terminates in fewer steps and less wall-clock time while achieving higher tactile grasp-quality proxy. Risk calibration error is 0.14 vs 0.58 for Cross-Entropy Method planner.

Conclusion: The proposed differentiable variational inference approach for risk-sensitive grasp planning provides superior robustness and efficiency compared to particle-filter methods and baselines, with accurate risk calibration and better performance under uncertainty.
