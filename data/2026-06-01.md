...
TL;DR: This thesis simulates the Argumentative Theory of Reasoning (ATR) through multi-agent debate (MAD) of large language models (LLMs), demonstrating that collective adversarial discourse significantly improves truth-seeking performance and establishing a new benchmarking methodology for measuring model properties like hallucination propensity.


<details>
  <summary>Details</summary>
Motivation: Human reasoning has been theorized to operate socially through collective adversarial discourse (ATR) rather than isolated individual cognition. Rather than relying on individual "intellectualist reasoners," ATR reconceptualizes truth as an emergent property of social epistemology - the product of imperfect individual reasoning refined under debate. This distributed method underpins democratic systems and has guided humanity to greater epistemic heights.

Method: The researchers implemented the Argumentative Theory of Reasoning through multi-agent debate (MAD) of large language models. They engineered an epistemically diverse set of models to participate in adversarial debates on questionnaire-based tasks. They analyzed debate dynamics and developed a novel benchmarking methodology leveraging LLM-MAD to measure intrinsic model properties like hallucination propensity.

Result: LLM-MAD significantly improves truth-seeking performance on questionnaire-based tasks, even when individual debate participants exhibit limited standalone performance. The performance gain is mechanistically grounded in the central principles of ATR. Strong empirical evidence suggests collective reasoning may be universally favorable over individualist reasoning, rather than a biological or evolutionary quirk.

Conclusion: Collective adversarial discourse may be universally superior to individualist reasoning for truth-seeking. The research establishes that LLM-MAD can simulate ATR principles effectively and introduces a novel benchmarking methodology that leverages debate dynamics to measure intrinsic model properties (such as hallucination propensity) in ways that current static benchmarking approaches cannot support.
...
TL;DR: The paper investigates whether institutional processing delays can destabilize multi-agent systems, finding that reactive agents (not learning) are most vulnerable to delay-induced oscillations, while non-reactive agents remain stable and Q-learning provides partial resilience.


<details>
  <summary>Details</summary>
Motivation: Regulatory institutions observe, deliberate, and intervene with characteristic delays. The core question is whether this processing lag alone can destabilize a system that would otherwise remain stable without exogenous shocks, coordination, or malicious actors.

Method: Two-stage analysis: (1) Mathematical analysis of a delayed replicator equation with autonomous agents receiving benefits from radical behavior but facing punishment based on lagged signals; derives critical delay threshold and proves supercritical Hopf bifurcation. (2) Simulation study with N=240 agents on a network using tabular Q-learning, comparing non-reactive (fixed policy), reactive (threshold heuristic without memory), and Q-learning (adaptive with cumulative value estimates) agents in a factorial design.

Result: Non-reactive agents are immune to delay (0% runaway), reactive agents collapse catastrophically (96% runaway at delay ≥ 8 steps), and Q-learning agents achieve partial resilience (66% runaway at delay = 20). The destabilizing ingredient is reactivity to delayed signals, not learning. Learning buffers instability through implicit punishment memory encoded in Q-values.

Conclusion: The destabilizing mechanism is reactivity to delayed signals: agents that immediately exploit low-alarm windows trigger oscillatory feedback loops. Learning provides resilience by creating an implicit memory of punishment, creating a hierarchy opposite to naive expectations that learning amplifies instability.
...
TL;DR: This paper introduces Copa, a theory-guided agentic LLM peer agent for STEM+C learning using the Evidence-Decision-Feedback framework grounded in Social Cognitive Theory and Social Constructivism, demonstrating in a high school study that it supports confidence building and conceptual understanding without causing dependence or over-reliance.


<details>
  <summary>Details</summary>
Motivation: LLM pedagogical agents are proliferating but raise concerns about adherence to learning theories, including cognitive offloading, over-reliance, and "gaming" behaviors that remain largely unaddressed in the literature.

Method: The researchers developed Copa, an agentic, multi-agent, multimodal Collaborative Peer Agent for STEM+C learning built on the Evidence-Decision-Feedback (EDF) framework, grounded in Social Cognitive Theory and Social Constructivism to promote sense-making through adaptive dialogic support rather than answer-seeking. They conducted an authentic high school computational-modeling study with 33 student dyads to evaluate its effectiveness.

Result: The study demonstrated that Copa (1) successfully supports students' confidence building and ability to verbalize conceptual understanding without causing dependence or over-reliance, and (2) provides adaptive feedback personalized to learners that is interpretable with respect to students' multimodal input data.

Conclusion: Theory-guided, multimodal LLM agents represent a promising path toward classroom AI integration that amplifies students' reasoning rather than replacing it, addressing concerns about cognitive offloading and over-reliance through principled design grounded in established learning theories.
...
TL;DR: MATraM ABM introduces dynamic activity adaptation in activity-based transport modeling, enabling agents to modify their schedules in response to sub-optimal travel conditions, bridging activity-based and interaction-based mobility simulation.


<details>
  <summary>Details</summary>
Motivation: Traditional transport models rely on pre-defined trip patterns from fixed daily schedules, limiting their ability to capture behavioral flexibility and uncertainty in response to changing conditions.

Method: MATraM enables agents to flag activity modification requests when facing sub-optimal travel conditions (e.g., increased travel times). The model couples an activity scheduling and modification framework with adaptive decision-making for generating and executing daily activity schedules.

Result: By enabling adaptive decision-making, MATraM provides a more realistic representation of how individuals adjust behavior in response to transport system dynamics, leading to emergent mobility and congestion patterns.

Conclusion: MATraM bridges activity-based modelling with interaction-based mobility simulation, providing a flexible and extensible platform for exploring transport dynamics under uncertainty, contributing to next-generation transport models capable of capturing the complex interplay between individual behavior and system-level outcomes.
...
TL;DR: Multi-agent LLM architectures for prediction market oracle resolution are evaluated against single-model baselines, finding that independent aggregation improves accuracy (83.43%) but deliberative consensus degrades performance; error correlations limit ensemble gains, motivating hybrid AI-human systems with confidence-based routing.


<details>
  <summary>Details</summary>
Motivation: Prediction markets require reliable outcome resolution, but existing systems trade off fast automation against costly human arbitration. Single-LLM oracles inherit all failure modes of their underlying models with no self-correction mechanism, creating a need to evaluate whether multi-agent architectures can improve oracle resolution accuracy.

Method: The authors compare independent aggregation and deliberative consensus against single-LLM baselines (GPT-5 Nano, DeepSeek V3, and Llama-3.3-70B) on 1,189 resolved prediction market questions from KalshiBench. All agents share a common evidence layer through Exa, with retrieval filtered by publication date to isolate reasoning from retrieval quality. Independent aggregation uses confidence-weighted voting, while deliberative consensus employs multi-agent debate.

Result: Independent aggregation with confidence-weighted voting achieves the highest accuracy at 83.43%, outperforming the best individual model by 1.01 percentage points. Deliberative consensus degrades accuracy to approximately 76%, below every single-model baseline. Error correlations across models range from 0.529 to 0.689, explaining why aggregation gains fall short of the theoretical Condorcet ceiling.

Conclusion: Many questions resist correction by any multi-agent architecture, motivating escalation to human arbitration. The authors propose routing criteria for hybrid AI-human oracle systems: auto-resolving only unanimous, high-confidence questions yields 97.87% accuracy on 47% of the dataset, with inter-agent disagreement flagging the remainder for human review.
...
TL;DR: SEPO augments RL training with penalties for exploitability, collusion, and externality cost, achieving zero exploit-pool advantage in Kuhn Poker and correcting over-cooperative behavior across multiple strategic domains.


<details>
  <summary>Details</summary>
Motivation: Language models fine-tuned with RL optimize for task reward, ignoring multi-agent strategic structure. Because agents use natural language game-state descriptions and free-form generation, strategic failure modes (exploiting weaker opponents, harmful equilibria, externalizing costs) are inseparable from the language interface itself.

Method: Safe Equilibrium Policy Optimization (SEPO) augments expected payoff with explicit penalties for exploitability, collusion risk, and externality cost. Implemented as a reward signal for Group Relative Policy Optimization (GRPO) applied to Gemma 4 E4B-it and Qwen 3.5-4B after supervised fine-tuning (SFT).

Result: Achieves zero exploit-pool advantage in Kuhn Poker for both models, outperforms base model on safety in four domains, and corrects over-cooperative behavior introduced by SFT. In negotiation, achieves positive-safety outcome with only positive normalized relative advantage. Ablation confirms per-rollout exploit computation is necessary (constant penalties cancel in GRPO advantage normalization).

Conclusion: Per-rollout exploit computation is essential for effective gradient signal; the authors release code and SFT datasets to support further research in strategic safety for agents.
...
### [7] [Learning-Based Navigation for Indoor Mobile Robots](https://arxiv.org/abs/2605.30468)
*Tri-Tin Nguyen,Tien-Dat Nguyen,Gia-Uy Le,Vinh Nguyen,Vinh-Hao Nguyen*

Main category: cs.RO

TL;DR: A learning-based navigation framework for indoor mobile robots that combines a supervised neural global planner (trained from cost-aware A* trajectories) with a PPO-refined local planner (Behavior Cloning → feasibility-aware PPO) to generate safe, feasible routes and motion commands.


<details>
  <summary>Details</summary>
Motivation: Indoor mobile robots require safe, reliable goal-directed navigation in obstacle-rich environments; integrating learning-based global planning with RL-refined local control offers a promising approach to address navigation challenges.

Method: Two-stage learning approach: (1) Supervised neural global planner trained from cost-aware A* expert trajectories; (2) Learning-Based DWA local planner formulated as discrete candidate selection over DWA action lattice, trained via behavior cloning followed by PPO refinement with feasibility-aware masking. Framework implemented in both simulation and real-world environments.

Result: The method generates feasible global routes and reliable local motion commands for safe goal-directed navigation in indoor environments, demonstrating that learning-based global planning combined with RL-refined local control is effective for mobile robot navigation.

Conclusion: The integration of learning-based global planning with reinforcement-learning-refined local control successfully enables safe and effective indoor mobile robot navigation, with the approach validated through both simulated and real-world experiments. The source code is available at https://ntdathp.github.io/rl_robot_web/.
...
### [8] [ELAN4D: Embodiment-Centric 4D Supervision for Vision-Language-Action Models via Plug-and-Play Adaptation](https://arxiv.org/abs/2605.30484)
...
TL;DR: ELAN4D is an embodiment-centric, 4D-aware training framework for Vision-Language-Action (VLA) models that uses robot keypoint tracks as predictive spatio-temporal supervision to improve generalization under out-of-distribution perturbations.


<details>
  <summary>Details</summary>
Motivation: Existing VLA policies operate reactively by directly regressing actions from current observations without explicitly modeling future dynamics, limiting their ability to generalize under out-of-distribution perturbations such as camera, background, and layout shifts.

Method: ELAN4D derives 3D displacement tracks of robot keypoints (joints and end-effector) using only forward kinematics from proprioceptive states, providing metric and compact supervision without external trackers. It employs a plug-and-play auxiliary branch with a lightweight track decoder that injects the 4D signal into the action expert through gradient isolation, preserving the pretrained vision-language backbone. The track decoder is discarded during inference.

Result: ELAN4D consistently improves over strong VLA baselines, achieving the best overall performance and substantial gains under out-of-distribution perturbations. Experiments on LIBERO, LIBERO-Plus, RoboTwin2.0, and real-world manipulation tasks demonstrate significant improvements across camera, background, and layout shifts.

Conclusion: Embodiment-centric 4D supervision is effective for building more robust and generalizable manipulation policies, as demonstrated by ELAN4D's consistent performance gains across diverse environments and out-of-distribution scenarios.
...
### [9] [LiftNav: Path Planning via Semantic Lifting in TSDF-Guided Gaussian Splatting](https://arxiv.org/abs/2605.31376)
*Hannah Schieber,Dominik Frischmann,Victor Schaack,Angela P. Schoellig,Daniel Roth*

Main category: cs.RO

TL;DR: LiftNav is a hybrid navigation framework combining TSDF+GS dual maps with YOLO detection and B-spline trajectory optimization, achieving 100% feasibility and shorter trajectories in indoor environments.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots in unknown indoor environments require both reliable collision avoidance and object-level understanding. Classical representations (TSDF) lack semantics, while photorealistic methods (Gaussian Splatting) suffer from soft geometry, limiting precise obstacle avoidance.

Method: LiftNav builds on GSFusion's TSDF+GS dual map, augmented with a real-time pipeline comprising YOLO-based detection, TSDF-based 3D lifting, and B-spline trajectory optimization. A hinge-loss-based collision penalty is introduced to improve trajectory smoothness and safety.

Result: Evaluated on the Replica dataset simulation, LiftNav achieves 100% feasibility rate and produces shorter trajectories compared to a state-of-the-art radiance field baseline.

Conclusion: LiftNav enables flexible semantic navigation without dense 3D embeddings, successfully combining the geometric precision of TSDF with the rich appearance of Gaussian Splatting while maintaining safety through optimized trajectory planning.
...
### [10] [CoMo3R-SLAM: Collaborative Monocular Dense SLAM with Learned 3D Reconstruction Priors for Outdoor Multi-Agent Systems](https://arxiv.org/abs/2605.30488)
*Zhihao Cao,Qi Shao,Shuhao Zhai,Feng Tian,Anh Nguyen,Hesheng Wang,Baoru Huang*

Main category: cs.RO

TL;DR: CoMo3R-SLAM introduces the first collaborative monocular dense RGB SLAM system that uses learned feed-forward 3D reconstruction priors to enable multi-robot mapping without depth sensors. It achieves globally consistent metric maps at 8 FPS, outperforming RGB-D methods on Tanks and Temples benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multi-robot collaborative dense SLAM is essential for scalable 3D perception, but existing systems require depth sensors (payload, power, calibration costs) while monocular approaches suffer from scale ambiguity and unreliable inter-agent data association, especially in outdoor scenes with low overlap and repetitive structures.

Method: Each agent runs a prior-guided front-end for real-time tracking and local dense fusion. A coordinator performs dense pointmap matching for cross-agent verification, closed-form Sim(3) gauge synchronization, and GPU-accelerated global bundle adjustment with segment-level depth optimization. The system uses learned feed-forward 3D reconstruction priors and requires neither depth sensors nor parametric intrinsics.

Result: On Tanks and Temples, CoMo3R-SLAM achieves the best ATE on three of four scenes and competitive Waymo accuracy, matching or exceeding state-of-the-art RGB-D methods while running online at 8 FPS.

Conclusion: CoMo3R-SLAM demonstrates that robust learned priors can enable collaborative monocular dense SLAM, producing globally consistent metric maps without depth sensors or parametric intrinsics, establishing a new benchmark for lightweight multi-robot mapping systems.
...
### [11] [Physics-informed Goal-Conditioned Reinforcement Learning under Hybrid Contact Dynamics](https://arxiv.org/abs/2605.30503)
*Vittorio Giammarino,Anastasios Manganaris,Ahmed H. Qureshi*

Main category: cs.RO

TL;DR: This paper introduces contact-aware and hierarchical formulations to extend physics-informed goal-conditioned reinforcement learning (Pi-GCRL) to contact-rich manipulation tasks, addressing issues caused by hybrid dynamics and nonsmooth value landscapes.


<details>
  <summary>Details</summary>
Motivation: Goal-conditioned reinforcement learning (GCRL) struggles with high-dimensional, hybrid, or contact-dependent dynamics. Existing physics-informed GCRL (Pi-GCRL) methods work well in navigation but fail in contact-rich manipulation tasks due to hybrid dynamics, mode-dependent controllability, and nonsmooth value landscapes.

Method: The authors first analyze why existing Pi-GCRL methods degrade when applied naively to contact-rich manipulation. They then introduce contact-aware and hierarchical formulations that apply physics-informed inductive biases selectively across the manipulation problem.

Result: The analysis demonstrates that existing Pi-GCRL methods degrade when applied naively to contact-rich manipulation, and the proposed contact-aware and hierarchical formulations address this challenge.

Conclusion: This work provides a principled step toward extending Pi-GCRL to contact-rich manipulation tasks, offering a pathway for more reliable robotic manipulation in complex physical environments.
...
### [12] [VLM-GLoc: Vision-Language Model Enhanced Monte Carlo Localization for Robust Semantic Global Localization in Cluttered Quasi-Static Environments](https://arxiv.org/abs/2605.30506)
*Shivendra Agrawal,Bradley Hayes*

Main category: cs.RO

TL;DR: VLM-GLoc: Hierarchical semantic Monte Carlo Localization using Vision-Language Models for global localization in geometrically and semantically ambiguous indoor environments.


<details>
  <summary>Details</summary>
Motivation: Global localization in geometrically aliased, quasi-static indoor environments (grocery stores, offices, schools, hospitals) is challenging due to geometric and semantic ambiguity. Traditional approaches relying on distinct geometric features or domain-specific vision pipelines struggle with long-tail semantic distributions and transient visual clutter.

Method: Hierarchical semantic Monte Carlo Localization (MCL) that leverages open-vocabulary Vision-Language Models (VLMs) as a unified semantic observation front-end. The method introduces an inverse semantic proposal mechanism that seeds particles via text-to-map retrieval, with three hypothesized benefits: extracting discriminative rich text features, implicit quality filtering of blurry/dynamic objects, and permanence reasoning for targeted data augmentation.

Result: Evaluated across two real-world environments (3,500 sq. ft. grocery store and 3,700 sq. ft. lab space) using different platforms (cellphone and quadruped). Achieved 70% and 74% global localization success respectively, substantially outperforming traditional geometry-only and domain-specific baselines.

Conclusion: VLM-GLoc effectively addresses global localization challenges in geometrically and semantically ambiguous indoor environments by leveraging VLMs, demonstrating superior performance over traditional approaches.
...
### [13] [ARISTO Hand: Sensing-Driven Distal Hyperextension for Fine-Grained Manipulation](https://arxiv.org/abs/2605.30508)
*Aaron Kim,Dong Ho Kang,Mark Helwig,Mingyo Seo,Kazuto Yokoyama,Tetsuya Narita,Luis Sentis*

Main category: cs.RO

TL;DR: The ARISTO Hand is a tendon-driven robotic hand with active distal hyperextension and a hybrid fingertip-sensing architecture combining a rigid nail-mounted force-torque sensor and soft capacitive tactile array, achieving 2.76x increased pull-out force for thin objects (1-20mm) while preserving grasp capability.


<details>
  <summary>Details</summary>
Motivation: Manipulating thin objects requires precise contact geometry and reliable force perception, yet many anthropomorphic robotic hands lack the mechanical and sensing capabilities needed for such interactions.

Method: A tendon-driven robotic hand (ARISTO Hand) with active distal hyperextension enabling controlled fingertip engagement beyond standard flexion kinematic limits. The hybrid fingertip-sensing architecture combines a rigid nail-mounted force-torque sensor and a soft capacitive tactile array. Validation is performed through quantitative force characterization and a multi-stage SD card extraction and insertion task.

Result: Active hyperextension increases pull-out force by 2.76x for object thicknesses of 1-20mm while preserving nominal grasp capability. The rigid nail-mounted sensor provides reliable force measurements during edge contacts, where proprioceptive force estimation degrades as contact geometry approaches kinematic singularities.

Conclusion: The proposed architecture effectively addresses the challenges of manipulating thin objects by combining mechanical hyperextension with dual-sensing capabilities that provide reliable force perception across various contact geometries.
...
### [14] [Any-ttach: Quick End-effector Swapping Enables Manipulation Dexterity with Simplicity](https://arxiv.org/abs/2605.30569)
*Weizhe Ni,Jinzhou Li,Haoyu Li,Cody Andres Alessio-Bunnell,Wenjing Pan,Xianyi Cheng*

Main category: cs.RO

TL;DR: Any-ttach is a tool-centric robotic manipulation framework that achieves dexterity through rapid end-effector swapping rather than complex multi-fingered hands. It combines automatic tool-swapping mechanisms with learned and planned tool-use skills to perform complex tasks like sandwich making and cucumber preparation.


<details>
  <summary>Details</summary>
Motivation: The authors observe that human hand complexity primarily serves tool use and tool making rather than pure manipulation, suggesting robots could achieve dexterity through rapid tool exchange rather than increasingly complex multi-fingered hands.

Method: Any-ttach integrates three components: 1) a low-cost automatic swapping mechanism for an open-close robot interface, 2) a handheld device for collecting human demonstrations, and 3) a task planning framework that composes learned, parameterized, and planned tool-use skills. The system supports diverse tools (daily tools, scissors, Fin Ray fingers, anthropomorphic hand) through a shared interface.

Result: Experiments demonstrate improved tool-swapping reliability, increased demonstration efficiency, reduced tool-pose variability, and support for diverse tool-use skills. In long-horizon tasks (sandwich making, cucumber preparation), Any-ttach executes six tool-use subskills through end-effector switching and execution monitoring.

Conclusion: Robots can expand manipulation capability through rapidly exchangeable tools and end-effector modules rather than solely through increasingly complex end-effectors, offering a more practical approach to robotic dexterity.
...
### [15] [Caspar: CUDA Accelerator for Symbolic Programming with Adaptive Reordering](https://arxiv.org/abs/2605.30583)
*Emil Martens,Aaron Miller,Matias Varnum,Annette Stahl*

Main category: cs.RO

TL;DR: Caspar is a library that bridges Python symbolic programming and GPU computing in robotics, automatically generating optimized CUDA kernels for high-performance nonlinear optimization problems like bundle adjustment.


<details>
  <summary>Details</summary>
Motivation: The authors aim to make modern GPU power more accessible to robotics researchers and practitioners. There is a gap between expressive symbolic programming in Python and high-performance GPU runtimes in C++, and the library needs to enable users to leverage GPU acceleration for nonlinear optimization problems without needing to manually write CUDA code.

Method: Caspar is built on the SymForce library. Users define symbolic residual functions using expressive symbolic programming (including Lie group operations), and Caspar uses symbolic differentiation to automatically generate optimized CUDA kernels. The system handles the interface between symbolic definitions and GPU execution, allowing users to focus only on defining the residual functions.

Result: The authors benchmark Caspar on the Bundle Adjustment in the Large (BAL) dataset against state-of-the-art bundle adjusters. Caspar achieves 5-20 times faster performance, requires less memory, and maintains similar accuracy to the best alternatives, demonstrating the effectiveness of the symbolic GPU programming approach.

Conclusion: The symbolic GPU programming approach provides substantial performance benefits for robotics optimization problems. Caspar successfully demonstrates that symbolic programming can be combined with GPU acceleration to create efficient, accessible tools for complex nonlinear optimization tasks in robotics. The library is released as open-source software.
...
### [16] [ZAPS-DA: Zero-Phase Action Policy Smoothing with Decoupled Actor for Continuous Control in Reinforcement Learning](https://arxiv.org/abs/2605.30612)
*Faiq Shamass*

Main category: cs.RO

TL;DR: ZAPS-DA is a framework that reduces high-frequency action jitter in continuous control policies by causal distillation of a zero-phase filter, avoiding post-processing and phase lag.


<details>
  <summary>Details</summary>
Motivation: Off-policy RL policies exhibit high-frequency action jitter that makes direct deployment impractical. Existing solutions have limitations: post-hoc filtering introduces phase lag, and embedding smoothness penalties in the actor's loss conflates reward regression with over-aggressive smoothing.

Method: ZAPS-DA pairs an unmodified main actor (trained by base RL loss) with a separate decoupled actor trained via supervised imitation of zero-phase filtered targets from the replay buffer. The deployed policy is the decoupled actor—a feed-forward map from observation to smooth action (causal distillation of a non-causal filter). A magnitude-matched MSE loss provides zero-hyperparameter portability across optimizers.

Result: Validated with Soft Actor-Critic and Savitzky-Golay filter in two driving simulators: (1) MetaDrive: 14-21x steering jitter reduction and 3-5x throttle jitter reduction at 6.3% reward cost; (2) Webots adaptive cruise control: 8-45x steering jitter reduction and task-failure rate reduced from 2.0% to 0.7% (Pareto improvement).

Conclusion: ZAPS-DA effectively reduces action jitter without phase lag or post-processing, matching task completion while achieving significant jitter reduction across different environments and demonstrating zero-hyperparameter portability.
...
### [17] [Exploiting Chordal Sparsity for Globally Optimal Estimation with Factor Graphs](https://arxiv.org/abs/2605.30617)
*Avinash Subramanian,Connor Holmes,Timothy D. Barfoot,Frank Dellaert,Frederike Dümbgen*

Main category: cs.RO

TL;DR: This paper presents a GTSAM framework extension that automatically constructs convex SDP relaxations for factor graphs and exploits Bayes tree decomposition for efficient global state estimation in robotics, outperforming local solvers in both accuracy and computational efficiency for chordally sparse problems.


<details>
  <summary>Details</summary>
Motivation: Standard local solvers in factor-graph frameworks (e.g., GTSAM) often converge to poor local minima, compromising safety in critical robotics applications. While convex relaxations can provide global solutions or certification, they typically require manual formulation and incur high computational costs due to large semidefinite programs (SDPs).

Method: The authors developed two key contributions: (1) an automated procedure within GTSAM to construct convex SDP relaxations for arbitrary factor graphs with common factor and variable types, and (2) a Bayes tree-based decomposition technique that exploits chordal sparsity to significantly speed up SDP solving.

Result: Demonstrated favorable scaling compared to standard local solvers in two case studies: a 3D pose-graph SLAM with ring structure and a 2D localization with chain structure. The structure-exploiting global estimator showed improved performance on chordally sparse problems.

Conclusion: The proposed framework successfully addresses both the formulation complexity and computational cost limitations of convex relaxation methods, providing a practical tool for safe global state estimation in robotics applications. The software is publicly available at https://github.com/borglab/gtsam.
...
### [18] [Bidirectional Incremental Generalized Hybrid A*](https://arxiv.org/abs/2605.30647)
*Sidharth Talia,Oren Salzman,Siddhartha Srinivasa*

Main category: cs.RO

TL;DR: Bi-IGHA* is a bidirectional variant of Incremental Generalized Hybrid A* (IGHA*) that improves kinodynamic planning efficiency by mitigating the "hiding" effect of frozen vertices, reducing expansion costs on R3, R4, and R6 planning problems while maintaining solution quality.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of efficient anytime kinodynamic planning for systems with complex dynamics in unstructured environments where precomputing motion primitives is infeasible. Direct A* is computationally infeasible due to curse of dimensionality. Hybrid A* approaches use state space discretization but create coupling between tree discovery and discretization resolution. IGHA* addresses this by performing search over a hierarchy of resolutions in an anytime fashion, freezing vertices rather than pruning them. However, the frozen vertices can hide solution-supporting vertices from the search at a particular iteration.

Method: The authors extend IGHA* into a bidirectional setting (termed Bi-IGHA*). While classical bidirectional search is motivated by search depth reduction, Bi-IGHA* obtains additional benefit by fundamentally mitigating the behavior induced by frozen vertices hiding solutions. Bi-IGHA* preserves IGHA*'s guarantees on monotonic cost improvement and termination.

Result: Bi-IGHA* substantially reduces expansions on R3, R4, and R6 planning problems. It achieves equivalent closed-loop performance with kinodynamic planning for high-speed off-road autonomy while requiring significantly fewer expansions.

Conclusion: Bi-IGHA* effectively addresses the limitations of IGHA* by mitigating frozen vertex hiding effects, providing substantial computational savings while maintaining solution quality, particularly beneficial for high-speed off-road autonomy scenarios.
...
### [19] [Primitive Subspaces Mediate Few-Shot Transfer in VLAs](https://arxiv.org/abs/2605.30695)
*Anya Singh,Cabrel Happi,Jai Relan,Varun Nair,Vidyut Baradwaj*

Main category: cs.RO

TL;DR: This paper investigates whether primitive-aware training in vision-language-action (VLA) policies can produce transferable sub-skills that enable few-shot task performance without fine-tuning, demonstrating a 3× improvement in sample efficiency over flat training.


<details>
  <summary>Details</summary>
Motivation: Current VLA policies lack low-cost task adaptation capabilities since each new task requires fine-tuning. The researchers aim to determine if training with primitive awareness can create a learned library of sub-skills that can be composed at inference time with few demonstrations.

Method: Training two VLA architectures (OpenVLA and π₀.₅) on the REASSEMBLE dataset with matched LoRA fine-tuning recipes, comparing flat trajectories vs primitive-segmented episodes with primitive-specific language prompts. Evaluated few-shot transfer with m ∈ {0, 1, 3, 5, 10} demonstrations on held-out tasks, replicated across three seeds, and validated on LIBERO-Long. Conducted ablation studies on primitive-decodable subspace vs random subspace of equal dimensionality. Identified and corrected a methodological pitfall in evaluating chunked policies regarding action-range gates.

Result: Primitive-trained models reached 78% of fine-tuned upper-bound performance with only m=3 demonstrations, while flat-trained models required m=10 demonstrations to achieve the same level—a 3× sample efficiency gap that replicated across seeds, architectures, and datasets. Ablation showed few-shot transfer degraded by 32 percentage points when ablating the primitive-decodable subspace, while ablating a random subspace had no effect, indicating primitive representations are causally necessary for transfer.

Conclusion: Primitive-aware training produces causally necessary transferable sub-skills that significantly improve few-shot task adaptation, enabling VLA policies to perform new tasks through composition of primitive skills rather than requiring full fine-tuning. The research establishes that primitive representations are a key mechanism for enabling low-cost task learning in VLA policies.
...
### [20] [Geometry-Aware Control Barrier Functions for Collision Avoidance via Bernstein Polynomial Approximations](https://arxiv.org/abs/2605.30696)
*Siwon Jo,Yanze Zhang,Yupeng Yang,Wenhao Luo*

Main category: cs.RO

TL;DR: A novel geometry-aware Control Barrier Function (CBF) based on Bernstein-Polynomial Signed Distance Fields (BP-SDFs) is introduced to represent irregular geometries more accurately for safe robot navigation and collision avoidance.


<details>
  <summary>Details</summary>
Motivation: Safe navigation with irregular geometries is challenging because common shape surrogates (spheres, super-ellipsoids) are overly conservative or require many local primitives, increasing constraint counts and degrading real-time performance.

Method: A geometry-aware CBF based on Bernstein-Polynomial Signed Distance Fields (BP-SDFs) is proposed, providing a unified representation of obstacles and robots with a unified minimum distance. The differentiability of Bernstein polynomials enables easy enforcement of control constraints in a closed loop.

Result: The method is validated through simulations showing efficiency and performance for single-robot navigation and heterogeneous multi-robot collision avoidance in various environments.

Conclusion: BP-SDF-based CBFs provide an effective solution for safe navigation that addresses geometry representation challenges while maintaining efficiency and real-time performance.
...
TL;DR: GSAM is a generalizable and safe robotic framework for articulated object manipulation that uses vision-based perception, VLM-based refinement with chain-of-thought reasoning, interaction constraint generation, and kinematic-aware planning.


<details>
  <summary>Details</summary>
Motivation: Articulated object manipulation is challenging. Existing methods (end-to-end policy learning, vision-motor planning, LLM/VLM) overlook object diversity and handle interaction complexity, leading to limited generalization and destructive collisions.

Method: GSAM framework comprising: (1) vision-based perceiver generating kinematic parameters; (2) fine-tuned VLM-based refiner using chain-of-thought commonsense reasoning; (3) interaction constraint function generator integrating articulated object, interaction pose, and obstacle avoidance knowledge; (4) LLM functionalizing constraints for trajectory and posture planning; (5) kinematic-aware manipulation planner verifying reachability.

Result: Experiments on 50 hinge tasks across 5 object categories and 50 random end-effector-handle configurations show 3.1% reduction in standard deviation and 36.0% improvement in manipulation success rate compared to best baseline.

Conclusion: GSAM demonstrates superior object generalization and interaction safety in practical scenarios, addressing limitations of existing methods in handling diverse articulated objects and preventing destructive collisions.
...
### [22] [SSR: Scaling Surefooted and Symmetric Humanoid Traversal to the Open World](https://arxiv.org/abs/2605.30770)
...
### [23] [Object-Informed Model Predictive Path Integral Control for Non-Prehensile Robot Manipulation](https://arxiv.org/abs/2605.30778)
*Nikola Raicevic,Bharath Raam Radhakrishnan,Chenbin Yu,Ki Myung Brian Lee,Nikolay Atanasov*

Main category: cs.RO

TL;DR: A hierarchical MPPI formulation that separates object-level planning from robot-level planning to improve long-horizon non-prehensile manipulation success rates while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Long-horizon planning for non-prehensile robot manipulation is challenging due to underactuated and discontinuous object-robot interactions.

Method: A hierarchical MPPI approach where a simplified object-only planning problem (assuming direct actuation) is solved first, and the resulting object trajectory is used as a reference for solving the joint robot-object planning problem.

Result: In simulation, object-informed MPPI increases task success by 40% with 26% faster control frequency. In real experiments with a 6-DoF xArm6 manipulator, it improves success by 20% with similar computation to regular MPPI.

Conclusion: The hierarchical MPPI approach effectively addresses the challenges of non-prehensile manipulation by enabling non-myopic reasoning while maintaining computational efficiency.
...
### [24] [Two Degree-of-Freedom Vibratory Transport in a Grasp](https://arxiv.org/abs/2605.30780)
*C. L. Yako,Shenli Yuan,Kenneth Salisbury*

Main category: cs.RO

TL;DR: This paper demonstrates 2-DoF in-hand manipulation using asymmetric stick-slip vibrations applied to grasped parts through closed-loop position control of a moving surface.


<details>
  <summary>Details</summary>
Motivation: To enable two-degree-of-freedom (translation and rotation) manipulation of grasped parts using asymmetric vibrations, which is valuable for dexterous in-hand manipulation.

Method: 1) Closed-loop position control of a moving surface creates periodic stick-slip waveforms; 2) Analytical derivation of how sticking/slipping accelerations affect average velocity against gravity; 3) Experimental validation with controlled squeeze force and high-resolution encoder; 4) Development of 2-DoF vibratory surface (translation + rotation about surface normal); 5) Parallel jaw gripper configuration using two such surfaces.

Result: Theoretical velocity trends were experimentally validated. The system can bidirectionally translate and rotate various grasped parts, with consistent waveform behavior for both translation and in-plane rotation.

Conclusion: Asymmetric vibrations provide a reliable method for 2-DoF in-hand manipulation, with translation and rotation governed by the same fundamental waveform parameters.
...
### [25] [Feat2Go: Visual Feature-Grounded Value Estimation for Embodied Reinforcement Learning](https://arxiv.org/abs/2605.30795)
*Junyang Shu,Zhiwei Lin,Bingqing Wei,Yongtao Wang*

Main category: cs.RO

TL;DR: Feat2Go is a fine-grained value estimation framework for embodied reinforcement learning in vision-language-action (VLA) models that derives continuous progress targets from a pretrained visual world model and uses them to reshape terminal rewards, improving performance across multiple robotic manipulation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning offers a promising approach for improving VLA models without heavy data requirements of imitation learning, but its effectiveness is constrained by sparse supervision and the difficulty of designing informative reward signals for long-horizon manipulation tasks.

Method: Feat2Go first derives continuous progress targets from a pretrained visual world model by measuring patch-level similarity to subgoal states and partitioning episodes into semantic stages using trend-based clustering. An embodied value model is then trained to predict this structural progress from current observation and task instruction, and the predicted value is used to reshape terminal rewards during policy optimization. The framework is compatible with existing VLA policy reinforcement learning pipelines like PPO and GRPO.

Result: Extensive experiments on ManiSkill3 and RoboTwin 2.0 demonstrate consistent performance improvements for existing VLA models. On ManiSkill3, Feat2Go improves OpenVLAOFT from 17.5% to 82.9% average out-of-distribution success while retaining 96.9% in-distribution performance. On RoboTwin 2.0, it achieves an average success rate of 88.8% in domain-randomized task settings, outperforming prior reinforcement learning methods.

Conclusion: Feat2Go consistently improves the performance of existing VLA models under both single-arm and bimanual manipulation settings without relying on manual reward engineering, making it a valuable addition to VLA policy reinforcement learning pipelines.
...
### [26] [Hide-and-Seek in Trajectories: Discovering Failure Signals for VLA Runtime Monitoring](https://arxiv.org/abs/2605.30834)
*Seongheon Park,Wendi Li,Changdae Oh,Samuel Yeh,Zsolt Kira,Michael Hagenow,Sharon Li*

Main category: cs.RO

TL;DR: Hide-and-Seek is a coarsely supervised learning framework for VLA failure detection that uses inter- and intra-trajectory contrastive objectives to localize failure-indicative actions from trajectory-level supervision alone, achieving state-of-the-art performance on LIBERO, VLABench, and real-world platforms.


<details>
  <summary>Details</summary>
Motivation: VLA models enable robots to follow natural language instructions but remain vulnerable to execution failures in real-world deployment. Existing failure detection methods are either expensive (action resampling) or rely on external models, while alternatives propagate trajectory-level labels uniformly across timesteps, obscuring localized failure signals.

Method: Hide-and-Seek formulates VLA failure detection as a coarsely supervised learning problem. It combines inter-trajectory and intra-trajectory contrastive objectives to localize failure-indicative actions and induce temporally structured failure signals from trajectory-level supervision alone, requiring no step-level annotation.

Result: Hide-and-Seek achieves state-of-the-art multi-task failure detection performance with a practical accuracy-timeliness trade-off under conformal prediction. It generalizes well to both seen and unseen tasks when evaluated on LIBERO, VLABench, and a real-world robotic platform across OpenVLA, π0, and π0.5 policies.

Conclusion: Hide-and-Seek enables robust, efficient failure detection for VLA models by localizing failure signals from coarse supervision alone, demonstrating that trajectory-level labels can effectively capture temporally structured failure patterns without expensive resampling or external models.
...
### [27] [High-Load-Density Electro-Permanent Magnetic Foot with Controllable Adhesion for Quadruped Wall-Climbing Robots](https://arxiv.org/abs/2605.30849)
*An Li,Bo Tao,I-Ming Chen,Han Ding*

Main category: cs.RO

TL;DR: A high-load-density electro-permanent magnetic foot with force-feedback circular Halbach-net electro-permanent magnet (CHN-EPM) adhesion units enables quadruped robots to reliably climb on ferromagnetic surfaces, achieving over 1000 N adhesion with a 200:1 load-to-weight ratio.
...
Method: The paper presents a high-load-density electro-permanent magnetic foot featuring CHN-EPM adhesion units with a three-dimensional magnetic circuit structure and flux-concentration effect for enhanced flux utilization. A magnetization driver and two-stage pulse current control strategy regulate excitation current, while a flexible pressure sensor provides contact force feedback for attachment/detachment monitoring.

Result: The CHN-EPM achieves maximum adhesion force exceeding 1000 N with a load-to-weight ratio over 200:1, reduced sensitivity to air-gap variations, and effective adhesion under partial contact conditions. The system demonstrates high-load adhesion on ceiling and vertical-wall surfaces and stable locomotion on painted, perforated, and curved ferromagnetic surfaces.

Conclusion: The proposed CHN-EPM-based system enables robust adhesion switching and stable locomotion on various ferromagnetic surfaces, effectively addressing the challenges of climbing locomotion on ferromagnetic environments.
...
### [28] [Wall-OSS-0.5 Technical Report](https://arxiv.org/abs/2605.30877)
...
TL;DR: Wall-OSS-0.5 is a 4B Vision-Language-Action model that demonstrates meaningful robot capabilities after pretraining alone, not just serving as an initialization for downstream fine-tuning. The pretrained checkpoint achieves non-trivial zero-shot performance on a 17-task suite and reaches 60.5% average task progress after fine-tuning, outperforming π_0.5 by 17.5%.


<details>
  <summary>Details</summary>
Motivation: Large-scale VLA pretraining is increasingly used as a foundation for robot policies, but evidence is almost always reported after task-specific fine-tuning. This creates an unanswered question: does VLA pretraining itself yield executable robot behavior, or does it merely provide a better initialization for downstream policy learning?

Method: Wall-OSS-0.5 is a 4B VLA built on a 3B VLM backbone augmented with action-generation components. It is pretrained across 20+ embodiments, processing over 1 million robot trajectories per epoch alongside a grounded multimodal corpus. The model uses a gradient-bridged co-training recipe with three objectives: discrete action prediction (routes strong VLM-native gradients), multimodal prediction (preserves grounded vision-language understanding), and continuous flow matching (serves as the deployment-time action interface).

Result: Before task-specific fine-tuning, the pretrained checkpoint achieved non-trivial zero-shot real-robot behavior on a 17-task suite, completing several tasks including a held-out deformable manipulation task. After fine-tuning, it reached 60.5% average task progress on 15 real-robot tasks and outperformed π_0.5 by 17.5%. Multimodal evaluations confirmed that action training does not erode grounded vision-language competence.

Conclusion: The results reposition VLA pretraining from merely an initialization strategy to a directly testable, already useful source of robot capability. VLA pretraining yields executable behavior that is beneficial both as a standalone policy and as a stronger adaptation prior for downstream fine-tuning.
...
### [29] [Trajectory Planning for Non-Communicating Mobile Robots using Inverse Optimal Control](https://arxiv.org/abs/2605.30906)
*Nina Majer,Yannick Epple,Xin Ye,Stefan Schwab,Sören Hohmann*

Main category: cs.RO

TL;DR: A novel combined trajectory planning and prediction algorithm for non-communicating mobile robots in collision avoidance scenarios, using inverse optimal control and perspective-taking to estimate goal states and solve joint prediction problems.
...
Method: Inverse optimal control estimates unknown goal states based on observed past trajectories. Each robot takes the perspective of other robots, solving a joint prediction problem using estimated goal states, which are then used for planning.

Result: Simulation results with 2-8 robots show the median duration until all vehicles reach their goals is 9.8% faster compared to planning with constant acceleration based estimated goal states. The proposed approach never leads to the solver being unable to find a solution.

Conclusion: The proposed approach improves efficiency and guarantees solution existence in collision avoidance scenarios for non-communicating mobile robots.
...
### [30] [Enhancing Human-Likeness in Reinforcement Learning Agents via Hierarchical Macro Action Quantization](https://arxiv.org/abs/2605.30928)
*Usman Nizamani,M. Shaheer Luqman,Fawad Javed Fateh,Ali Shah Ali,Murad Popattia,M. Zeeshan Zia,Quoc-Huy Tran*

Main category: cs.RO

TL;DR: This paper introduces HiMAQ (Hierarchical Macro Action Quantization), a novel RL framework that uses a two-level vector quantization approach to encode human demonstrations into macro actions. The method aligns agent behavior with human actions while maximizing rewards, outperforming non-hierarchical baselines on D4RL benchmarks across multiple RL algorithms.


<details>
  <summary>Details</summary>
Motivation: Current RL agents are primarily reward-driven and often behave differently from humans, limiting their interpretability and reliability. There is a need for agents that can predict action sequences aligned with human behaviors while still maximizing rewards.

Method: HiMAQ (Hierarchical Macro Action Quantization) uses a two-level vector quantization approach: the lower level maps input actions to fine-grained subaction clusters, while the higher level aggregates these into action clusters. This hierarchical encoding captures human demonstrations into macro actions that can be integrated with various RL algorithms (IQL, SAC, RLPD).

Result: Extensive evaluations on D4RL benchmarks demonstrate that HiMAQ outperforms the non-hierarchical MAQ baseline, achieving better human-likeness scores while maintaining comparable or better success rates. The improvements generalize across integration with different RL algorithms.

Conclusion: The hierarchical macro action quantization framework successfully balances human-likeness with performance, providing a pathway to more interpretable and reliable RL agents that align with human behaviors while maximizing rewards.
...
### [31] [RDGen: Demonstration Generation for High-Quality Robot Learning via Reinforcement Learning](https://arxiv.org/abs/2605.30957)
*Zijian Zhu,Menglin Zou,Zhuang Li,Yaojie Tu,Xinhai Sun*

Main category: cs.RO

TL;DR: RDGen is a sim-to-real RL framework that generates high-quality robot demonstrations using a VLM-based task parser, Grounding DINO object localizer, and simulation-to-real RL policy transfer, producing smoother trajectories and superior VLA performance compared to human teleoperation.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models are promising for robot control but are limited by the scarcity of high-quality robot trajectory data. Current data collection via human teleoperation is labor-intensive, costly, and difficult to scale, creating a bottleneck for developing effective VLA models.

Method: RDGen employs a VLM-based task parser to identify task-relevant objects, Grounding DINO for object localization, and an RL policy transferred from simulation to the real robot. Successful RL rollouts are harvested as clean demonstrations for downstream VLA training, while the simulation stage provides scalable additional trajectories.

Result: On a pick-and-place task, the transferred RL policy achieves high task success rate. RDGen produces significantly smoother trajectories compared to human teleoperation and yields superior downstream VLA performance.

Conclusion: RL-generated demonstrations can serve as more reliable and consistent supervisory signals for robot policy learning, overcoming the limitations of human teleoperation data collection.
...
TL;DR: A VR teleoperation framework that uses GPU-accelerated inverse kinematics and trajectory optimization to generate safe, collision-aware motion for a 7-DoF manipulator in real-time.


<details>
  <summary>Details</summary>
Motivation: VR teleoperation enables safe, non-contact task execution in hazardous environments. Existing VR teleoperation studies primarily serve as data-collection tools for imitation learning and often lack explicit handling of dynamic obstacles, workspace changes, or collision risks. For real deployment, teleoperation must react to dynamic situations with low latency and remain robust to operator mistakes.

Method: A VR teleoperation framework integrating GPU-accelerated inverse kinematics and trajectory optimization within a VR interface. The system generates feasible joint commands at each control cycle under robot constraints, supporting real-time manipulation while handling collisions with both static and moving obstacles.

Result: Experiments with a 7-DoF manipulator demonstrate stable online behavior across three scenarios: obstacle-free, static-obstacle, and moving-obstacle environments. The proposed approach generates motion consistent with operator commands while producing safe detours when obstacles interfere with the commanded path.

Conclusion: The framework successfully enables collision-aware motion generation for VR teleoperation, providing stable real-time control and safety through obstacle avoidance. This addresses the limitations of existing VR teleoperation systems that primarily serve as data-collection tools rather than deployment-ready solutions.
...
### [33] [Can Aerial VLA Models Cooperate? Evaluating Closed-Loop Air-Ground Coordination with CARLA-Air](https://arxiv.org/abs/2605.31066)
*Tianle Zeng,Yanci Wen,Xueang Yu,Hong Zhang*

Main category: cs.RO

TL;DR: This paper introduces CARLA-Air, a unified air-ground simulation environment, and evaluates current aerial vision-language-action models on cooperative tasks. Results show these models struggle to achieve stable cooperation despite good single-agent performance, requiring explicit partner-state grounding, low-latency action coordination, and team-level objective alignment.


<details>
  <summary>Details</summary>
Motivation: Recent aerial vision-language-action (VLA) models show promising single-UAV capabilities (tracking moving objects, language-specified navigation), but it remains unclear if these capabilities can transfer to air-ground cooperation where a UAV and UGV must act jointly in a shared, closed-loop physical world.

Method: CARLA-Air: a single-process evaluation environment unifying CARLA and AirSim within Unreal Engine runtime, sharing world state, physics tick, and sensing pipeline. Enables physically consistent UAV-UGV interaction and precise measurement of simulation-timestamp alignment and effective coordination latency. Evaluated representative aerial VLA and planning baselines on two diagnostic tasks: moving-platform landing and occlusion-recovery escort.

Result: Current aerial VLA models can often track or follow a ground partner but struggle to convert single-agent competence into stable cooperative behavior. State prompting provides limited benefit, and naive bidirectional interaction fails to consistently improve performance and can amplify errors for most baselines.

Conclusion: Under text-based cue interfaces, zero-shot cooperative air-ground VLA requires three components beyond the current paradigm: explicit partner-state grounding, low-latency action coordination, and team-level objective alignment.
...
### [34] [Seeing Fast and Slow: Bimodal 3D Scene Graphs for Open-set Tasks](https://arxiv.org/abs/2605.31067)
*Marcel Bartholomeus Prasetyo,Shrutika Vishal Thengane,A Manicka Praveen,Yi Loo,Malika Meghjani*

Main category: cs.RO

TL;DR: BiMoSG is a bimodal 3D scene graph generation approach for open-set tasks that can efficiently switch between fast (coarse) and slow (fine) modes, significantly outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Open-set task execution benefits from adaptively switching between coarse and fine scene representations based on context and exploration progress. Starting with a coarse representation is efficient, while finer representations are only needed when encountering task-relevant regions.

Method: BiMoSG employs a bimodal approach with a default "fast" mode for efficient coarse 3D scene graph generation and a switchable "slow" mode for generating finer, open-vocabulary 3D scene graphs of task-relevant objects.

Result: BiMoSG is significantly faster than open-source state-of-the-art approaches, enabling integration with task execution for real-time deployment.

Conclusion: The proposed scene graph generation approach successfully balances efficiency and detail, allowing robots to perform open-set tasks in real-time by dynamically adjusting scene representation granularity.
...
### [35] [Building Generalization Into Behavior Generation Via Adaptive Compositions of Regularities](https://arxiv.org/abs/2605.31110)
*Aravind Battaje,Malte Bernhard,Vito Mengers,Oliver Brock*

Main category: cs.RO

TL;DR: This paper proposes that generalization in robotics arises from adaptively composing regularities (predictable relationships) into situation-appropriate structures for behavior generation, examining this mechanism in the AICON framework where sensory feedback realizes composition and gradient descent generates behavior.


<details>
  <summary>Details</summary>
Motivation: Generalization in robotics requires prior knowledge about world structure, but this structure changes from situation to situation, posing a challenge for building generalizable behavior systems.

Method: The authors use AICON (Active InterCONnect), a framework representing regularities as interacting processes in a differentiable network, where sensory feedback realizes composition and gradient descent generates behavior. They isolate this mechanism by studying a simple simulated problem where all relevant regularities can be identified, then exposing the model to novel conditions not considered during design.

Result: The model generates context-appropriate behavior in all but one case (where encoded regularities are provably insufficient). Ablation studies reveal the network automatically modulates which regularities influence behavior based on their informativeness.

Conclusion: Adaptive composition of regularities constitutes a powerful inductive bias for building generalization into behavior generation systems.
...
### [36] [Don't Fool Me Twice: Adapting to Adversity in the Wild with Experience-Driven Reasoning](https://arxiv.org/abs/2605.31119)
...
### [37] [TARIC: Memory-Augmented Traversability-Aware Outdoor VLN under Interrupted Semantic Cues](https://arxiv.org/abs/2605.31121)
*Tianle Zeng,Hanjing Ye,Jianwei Peng,Jingwen Yu,Hanxuan Chen,Hong Zhang*

Main category: cs.RO

TL;DR: A unified outdoor vision-language navigation framework that maintains traversability-consistent executable guidance during prolonged semantic-cue interruptions, improving real-world success rates from 17.5% to 40% and simulation success by over 10 percentage points.


<details>
  <summary>Details</summary>
Motivation: Outdoor VLN in long-range environments suffers from semantic-cue interruptions where goal cues become sparse, occluded, or leave the field of view. When cues disappear, agents degrade into backtracking, oscillatory headings, or aimless exploration. Memory-based methods fail under traversability-driven detours because remembered cue directions may be infeasible, forcing detours that prolong cue-free phases and render robot-centric cues stale. Traversability is thus a stability condition for maintaining goal-directed guidance.

Method: The proposed framework extracts semantic bearings from visibility-gated goal/exploration cues and grounds them into executable headings using a real-time near-field traversability profile. It lifts intermittent 2D evidence into a world-aligned 3D cue memory with an uncertainty-aware readout mechanism to prevent guidance degradation during detours.

Result: Evaluated on quadrupedal and wheeled platforms over 600-1000 m routes. Improved simulation success rate by over 10 percentage points over the strongest baseline. Achieved real-world success rate of 40% compared to 17.5% for the strongest baseline, with substantially higher robustness during prolonged cue-free intervals.

Conclusion: The proposed framework successfully maintains traversability-consistent executable guidance during cue-free phases, significantly improving navigation success rates and robustness in both simulation and real-world scenarios by preventing guidance degradation through 3D cue memory with uncertainty-aware readout.
...
### [38] [Simulation of collision avoidance behavior in crowd movement by data-driven approach](https://arxiv.org/abs/2605.31210)
...
### [39] [HARP-VLA: Human-Robot Aligned Representation Learning for Vision-Language-Action Model](https://arxiv.org/abs/2605.31234)
*Xiang Zhu,Puzhen Yuan,Yichen Liu,Jianyu Chen*

Main category: cs.RO

TL;DR: HARP aligns human and robot visual representations using paired human-robot demonstrations and unpaired videos to train a robot-adapted visual encoder and latent action model, improving VLA pretraining from human videos.


<details>
  <summary>Details</summary>
Motivation: Learning generalizable VLA models from human videos is challenging due to cross-embodiment discrepancies in visual observations and executable actions. Existing latent action models rely on visual features, and misaligned human and robot visual representations lead to inconsistencies in policy inputs and domain-dependent latent actions.

Method: HARP uses limited paired human-robot demonstrations as cross-embodiment bridges and abundant unpaired human and robot videos as a scalable dynamics supervision data source. It trains a robot-adapted visual encoder and latent action model with manipulation-centric auxiliary cues and a source-relative pair-discriminative alignment loss.

Result: Experiments on feature visualization, simulation, and real-world manipulation show improved human-robot alignment and downstream policy performance, achieving 4.481 average length on CALVIN ABC→D and a 7.1% real-world success rate gain over the strongest baseline.

Conclusion: The learned aligned vision encoder and latent action model provide a unified vision and action representation for VLA-style policy learning, enabling effective co-training with human videos where human and robot videos provide vision-language-to-latent-action supervision and a lightweight robot action head grounds latent actions into executable commands.
...
### [40] [Before Parc Fermé: RL-Time Pruning for Efficient Embodied LLMs in Autonomous Driving](https://arxiv.org/abs/2605.31256)
*Luca Benfenati,Ali Azimi,Matteo Risso,Fabio Carapellese,Daniele Jahier Pagliari,Alessio Burrello*

Main category: cs.RO

TL;DR: The paper proposes Before Parc Fermé (BPF), a pruning strategy for embodied LLMs that compresses models during reinforcement learning to achieve better task-performance vs. memory trade-offs compared to post-training pruning or using smaller dense models.


<details>
  <summary>Details</summary>
Motivation: Embodied LLMs used in robotic control pipelines have high memory and generation latency, making real-time deployment difficult. While pruning can reduce these costs, the key challenge is determining when pruning should occur in the training pipeline—specifically, whether to prune pre-training, during SFT, during RL, or post-training.

Method: The researchers propose Before Parc Fermé (BPF), a pruning strategy performed during RL that compresses embodied LLM controllers while they are still being optimized for closed-loop behavior. Two variants are proposed: BPF-RL (iterative pruning during RL at predefined intervals) and BPF-SFT/RL (first prune during SFT, then further compress during RL). The approach is evaluated on RobotxR1 (an autonomous-driving LLM-based control pipeline) using LLM-Pruner framework, comparing against post-training pruning, post-training pruning with RL recovery, SFT-stage pruning, and smaller dense models.

Result: BPF provides the best task-performance vs. memory and throughput trade-off among considered strategies. BPF-SFT/RL achieves a 1.69× better size-end-to-end performance trade-off than directly selecting a smaller dense model from the same family. On Jetson AGX Orin, compact models improve decode throughput by up to 27%.

Conclusion: BPF is an effective strategy for compressing embodied LLM controllers, offering superior performance-cost trade-offs compared to post-training pruning approaches and dense models from the same family. The key insight is that pruning during RL allows decisions to account for task-specific supervision and closed-loop feedback that shape the final controller.
...
### [41] [DeMaVLA: A Vision-Language-Action Foundation Model for Generalizable Deformable Manipulation](https://arxiv.org/abs/2605.31286)
...
TL;DR: DeMaVLA is a Vision-Language-Action foundation model for generalizable deformable manipulation, using a VLM backbone with an action expert, flow matching for continuous action generation, and layer-wise pruning for efficiency.


<details>
  <summary>Details</summary>
Motivation: Real-world household robots need reusable VLA models for deformable-object folding across diverse objects, conditions, and environments. Existing systems train separate policies per object category, while naive multi-task training suffers from task interference and degraded performance.

Method: DeMaVLA adopts a VLM backbone with an action expert, formulates continuous action generation using flow matching, and prunes every other transformer layer to reduce training/inference cost while preserving layer-wise alignment. It is pre-trained on ~5,000 hours of real-world dual-arm demonstrations, then post-trained on mixed folding data with DAgger to incorporate corrective trajectories from real-robot failures.

Result: DeMaVLA achieves competitive performance on RoboTwin and strong real-world results on a household folding benchmark.

Conclusion: These results demonstrate the value of scalable real-world data, efficient action generation, and corrective learning for developing general-purpose VLA policies in deformable-object manipulation.
...
### [42] [AR Forcing: Towards Long-Horizon Robot Navigation World Model](https://arxiv.org/abs/2605.31314)
...
TL;DR: AR Forcing integrates diffusion loss into autoregressive training, reducing distribution shift and improving long-horizon navigation prediction consistency and accuracy across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based robot navigation world models trained with parallel supervision but used autoregressive inference during path planning, causing distribution shift that destabilizes long-horizon performance.

Method: AR Forcing: an autoregressive training strategy that integrates standard diffusion loss into the autoregressive training loop, using its own predictions to update context and optimize single-step noise prediction, exposing model to inference distribution without additional discriminators or matching losses.

Result: Experiments on RECON, SCAND, HuRoN, and TartanDrive datasets showed improved consistency of generated images during long-horizon navigation and enhanced accuracy of predicted trajectories, demonstrating robustness in both complex known and unknown environments.

Conclusion: AR Forcing successfully bridges the training-inference gap in diffusion-based navigation models while preserving the original diffusion framework and sampler, making it easy to integrate with existing systems.
...
### [43] [Surface Constraint Policy for Learning Surface-Constrained and Dynamically Feasible Robot Skills](https://arxiv.org/abs/2605.31321)
*Shuai Ke,Jiexin Zhang,Huan Zhao,Zhiao Wei,Yikun Guo,Jie Pan,Han Ding*

Main category: cs.RO

TL;DR: A novel surface constraint policy (SCP) for robot manipulation tasks involving free-form surface constraints, combining diffusion-based imitation learning with surface-constrained dynamic movement primitives


<details>
  <summary>Details</summary>
Motivation: Diffusion-based imitation learning methods face limitations in tasks with complex free-form surface constraints due to lack of explicit surface geometry constraint modeling and dynamic feasibility issues, resulting in stochastic action generation that fails to achieve reliable surface alignment and maintain stable contact

Method: Surface geometry constraint is encoded using a 2D weighted Gaussian kernel function derived from demonstrations. A diffusion-based policy infers task-level action intentions from multimodal sensory inputs (visual observations and robot state feedback), which are then transformed into surface-constrained dynamic movement primitives (DMPs) through similarity-based action mapping for smooth and compliant motion execution

Result: The proposed SCP achieves generation of structured surface geometric intent and dynamically admissible actions, demonstrating superior task success rates and contact stability under surface constraints in multiple surface manipulation tasks compared to existing techniques

Conclusion: The method is validated on multiple surface manipulation tasks and shows superior performance in terms of task success rates and contact stability under surface constraints
...
### [44] [Learning Terrain-Aware Whole-Body Control for Perceptive Legged Loco-Manipulation](https://arxiv.org/abs/2605.31343)
*Sikai Guo,Yudong Zhong,Guoyang Zhao,Botao Dang,Zhihai Bi,Jun Ma*

Main category: cs.RO

TL;DR: TA-WBC is a terrain-aware whole-body control framework for legged manipulators that integrates exteroception through a hybrid encoder and dual-policy distillation to enable robust loco-manipulation across complex terrains.


<details>
  <summary>Details</summary>
Motivation: Legged manipulators offer exceptional terrain adaptability and mobile manipulation, but existing whole-body controllers rely primarily on proprioception without exteroception for terrain topology perception, limiting their ability to adapt to varying environmental conditions and navigate complex terrains effectively.

Method: The authors propose TA-WBC, an RL-based unified policy for whole-body loco-manipulation that employs a hybrid exteroception encoder to extract terrain features, a foot contact plane-based end-effector sampling method to decouple manipulation targets from base fluctuations, and a dual-policy distillation module to integrate expansive whole-body motion with terrain adaptability while preventing catastrophic forgetting.

Result: Simulation and real-world experiments validate the controller's robustness, demonstrating a larger reachable workspace, reduced tracking error, and fewer unexpected stumbles compared to approaches without terrain awareness.

Conclusion: The unified policy highlights the promising capabilities of legged manipulators in performing loco-manipulation tasks across complex terrains, effectively bridging the gap between terrain perception and whole-body control.
...
### [45] [Haptic Sorter: A Unified Planning Framework for Online Shape Estimation and Real-Time Pose Inference](https://arxiv.org/abs/2605.31352)
*Zhuoyi Lu,Lin Yang,Sri Harsha Turlapati,Domenico Campolo*

Main category: cs.RO

TL;DR: A unified model-based geometric framework for robotic manipulation that integrates haptic perception, modeling, and manipulation planning, using Bayesian Optimization for shape inference with superellipse approximations, adaptive manipulation potential encoding, and online ODE for real-time pose inference.


<details>
  <summary>Details</summary>
Motivation: Traditional robotics manipulation assumes known object shape and pose, but precise geometric information is often unavailable in practice due to sensor uncertainties and view occlusion, causing pose inference to suffer from these limitations.

Method: Introduces Bayesian Optimization (BO) to guide haptic exploration for object shape inference using superellipses to approximate geometric boundaries; proposes adaptive formulation of manipulation potential encoding object geometry for quasi-static robot-object interaction; develops an online Ordinary Differential Equation (ODE) for real-time pose inference based on model prediction and tactile feedback.

Result: Deployed on a 2D robotic sorting task with varied object geometries, demonstrating robustness and generalizability in both simulation and a real-world multi-arm setup.

Conclusion: The proposed framework successfully integrates haptic perception, modeling, and manipulation planning for robotic manipulation tasks when geometric information is uncertain, providing a unified approach that addresses the limitations of traditional methods.
...
### [46] [Adaptive Artificial Time-Delay Control with Barrier Lyapunov Constraints for Euler-Lagrange Robots](https://arxiv.org/abs/2605.31405)
...
### [47] [Shaft-integrated Force Sensing with Transformer-based Dynamics Compensation for Telesurgery](https://arxiv.org/abs/2605.31434)
*Shuyuan Yang,Grant Boone,Timo Markert,Sebastian Matich,Andreas Theissler,Martin Atzmueller,Zonghe Chua*

Main category: cs.RO

TL;DR: This paper presents a method to integrate a six-axis force sensor into the distal end of a standard cable-driven surgical instrument for RAMIS, using a transformer neural network to compensate for internal cable forces and achieve end-effector force measurement with normalized errors below 6%.


<details>
  <summary>Details</summary>
Motivation: Robot-Assisted Minimally Invasive Surgery (RAMIS) enhances surgeon dexterity, and haptic feedback improves performance. Force information has broader potential to inform performance assessment, tactile localization, and surgical autonomy. The need exists for accessible approaches to integrate force sensing into RAMIS tools without requiring specialized manufacturing tools.

Method: The proposed design integrates a six-axis commercial force sensor into the distal end of a standard cable-driven surgical instrument, preserving original mechanical functionality. A transformer neural network integrates force sensor measurements with robot state information to estimate applied forces at the end-effector while compensating for internal cable forces from actuation. The approach emphasizes reproducibility and accessibility for research applications.

Result: The approach achieved normalized errors below 6% and generalized better to unseen conditions than purely proximal data-driven sensing approaches. However, high internal cable forces caused sensor saturation and reduced axial force observability, degrading performance along the tool's major axis and under higher load conditions.

Conclusion: The balance of system integrability and performance enables applications and research into haptic feedback, skill assessment, and force-informed autonomy in RAMIS. Videos and code are available at https://enhanced-telerobotics.github.io/shaft force sensing.
...
### [48] [Actuator-Aware Inverse Kinematics with Joint-Limit Admissibility for Torque-Controlled Redundant Robots](https://arxiv.org/abs/2605.31436)
*Mohammad Dastranj,Mahdi Hejrati,Jouni Mattila*

Main category: cs.RO

TL;DR: This paper proposes actuator-aware inverse kinematics for torque-controlled redundant robots under joint-limit constraints, addressing the issue that small commanded task residuals may not improve realized motion in this context.


<details>
  <summary>Details</summary>
Motivation: In torque-controlled redundant robots, the inverse-kinematic output is a required joint velocity supplied to a downstream torque-level controller, not just a purely kinematic joint-velocity command. Therefore, small commanded task residuals may not necessarily improve realized motion.

Method: The method formulates a convex quadratic programming problem where the decision variable is the joint-level required velocity. Control barrier function style bounds impose reference-level joint-limit admissibility, while the task equation is handled through a penalized slack variable. Redundancy is resolved using a controller-compatibility objective that accounts for previous-command consistency and actuator torque-capacity weighting.

Result: Experiments on a virtual-decomposition-controlled seven-degree-of-freedom upper-limb exoskeleton show that the method produces lower limit-pushing commands, bounded admissible required velocities, and improved realized task behavior compared to standard inverse-kinematic baselines and a constrained task-preserving quadratic programming baseline.

Conclusion: The method is independent of the particular torque-level controller and can serve as an intermediate IK layer between an endpoint trajectory and a redundant robot controller.
...
### [49] [On-Device Robotic Planning: Eliminating Inference Redundancy for Efficient Decision-Making](https://arxiv.org/abs/2605.31460)
*Joonhee Lee,Hyunseung Shin,Hyunmi Kim,Pei Zhang,Jeonggil Ko*

Main category: cs.RO

TL;DR: REIS is a human cognition-inspired robotic decision-making framework that reduces inference latency by addressing temporal redundancy in robotic reasoning workloads, combining lightweight scene gating, KV-steered affordance routing, and deliberative reasoning.


<details>
  <summary>Details</summary>
Motivation: Reasoning-based robotic policies using large language and vision-language models achieve strong semantic planning capabilities but suffer from high inference latency that limits practical real-time deployment.

Method: The authors observe that robotic reasoning workloads contain temporal redundancy where consecutive observations frequently produce identical actions and subgoals. REIS combines lightweight scene gating, KV-steered affordance routing, and deliberative reasoning to accelerate robotic control under embodied constraints.

Result: Experiments on ALFRED and real-world robotic tasks demonstrate that REIS significantly suppresses reasoning overhead while maintaining competitive task performance.

Conclusion: REIS presents a framework that minimizes unnecessary reasoning while preserving semantic adaptability, enabling faster robotic control without sacrificing task performance.
...
### [50] [IDOL: Inverse-Dynamics-Guided Future Prediction for End-to-End Autonomous Driving](https://arxiv.org/abs/2605.31476)
*Chenghao Zhang,Timin Li,Dongmei Li*

Main category: cs.RO

TL;DR: IDOL is an inverse-dynamics-guided future prediction framework for end-to-end autonomous driving that bridges world modeling and planning by decoding motion implications from latent future scene states.


<details>
  <summary>Details</summary>
Motivation: End-to-end autonomous driving with world-model-based approaches enables explicit future reasoning, but current methods forecast future scene states without explicitly decoding the motion implications hidden in state transitions. This results in future reasoning being descriptively useful but weakly coupled to executable motion generation. Future prediction alone does not guarantee better planning unless the predicted evolution can be converted into planning-relevant trajectory updates.

Method: IDOL (Inverse-Dynamics-Guided Future Prediction) operates in latent BEV space. It first predicts multiple future latent scene states using a BEV world model, then applies an inverse dynamics model to adjacent latent futures to decode transition-aware trajectory features and recover planning-relevant motion deltas. These inverse-dynamics-derived signals optimize the planned trajectory, transforming future forecasting into actionable planning guidance. A lightweight closed-loop refinement module further improves long-horizon consistency by reusing the optimized trajectory for another round of future-aware reasoning.

Result: Extensive experiments on the NAVSIM v1 and NAVSIM v2 benchmarks demonstrate that IDOL achieves state-of-the-art performance among comparable methods.

Conclusion: By introducing inverse dynamics into latent future reasoning, IDOL tightens the coupling between world modeling and planning, transforming future forecasting from passive scene anticipation into actionable planning guidance.
...
### [51] [Batched Differentiable Rigid Body Dynamics in PyTorch for GPU-Accelerated Robot Learning](https://arxiv.org/abs/2605.31481)
...
### [52] [Learning Controlled Separation of Small Objects Between Two Fingers with a Tactile Skin](https://arxiv.org/abs/2605.31486)
...
### [53] [Smaller and Faster 3DGS via Post-Training Dictionary Learning](https://arxiv.org/abs/2605.30396)
*Jiarong Gong,Jonas Unger,Ehsan Miandji*

Main category: cs.GR

TL;DR: A dictionary-learning-based post-training compression framework for 3D Gaussian Splatting that reduces memory footprint without retraining, achieving 3.95x-4.55x compression ratios and 23.3%-25.3% rendering speedup across 13 benchmark scenes.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) has large memory footprints that limit deployment on less powerful devices. Existing compression techniques require additional trainable parameters and cause noticeable image quality degradation.

Method: The first dictionary-learning-based compression framework for 3DGS, using a post-training compression pipeline that can be deployed in virtually any 3DGS model without retraining or modifications to existing models.

Result: Achieves average compression ratios of 3.95x, 3.10x, and 4.55x for 3DGS, 3DGS-MCMC, and PixelGS respectively, with consistent rendering speedups of 23.3%, 24.3%, and 25.3% while maintaining image quality across 13 benchmark scenes.

Conclusion: The proposed framework provides significant compression capabilities, preserves image quality, and improves real-time rendering performance without requiring model retraining or architectural modifications.
...
### [54] [BijectiveRemesh: Maintaining Bijective Mappings for Data Transfer Across Remeshed Manifolds](https://arxiv.org/abs/2605.30744)
*Leyi Zhu,Michael Tao,Yixin Hu,Daniele Panozzo,Denis Zorin*

Main category: cs.GR

TL;DR: BijectiveRemesh is a robust algorithm for maintaining bijective mappings across complex remeshing sequences on 2D triangle surfaces and 3D tetrahedral meshes, constructing mathematically rigorous composite maps by chaining local bijective atlases for each remeshing operation.


<details>
  <summary>Details</summary>
Motivation: Traditional data transfer methods rely on interpolation or projection, which may not preserve bijectivity. The authors seek a mathematically rigorous approach to maintain continuous, bijective mappings across complex remeshing sequences on both 2D triangle surfaces and 3D tetrahedral meshes.

Method: The framework represents the overall mapping as a composition of local bijective atlases (one per remeshing operation). For 2D triangle meshes, a Shared Scaffold structure enforces global bijectivity through local orientation preservation, supporting edge splits, edge swaps, and vertex smoothing beyond edge collapses. For 3D tetrahedral meshes, the method generalizes local atlas construction using Steinitz's Theorem and Maxwell-Cremona lifting to ensure valid embeddings.

Result: The approach enables exact tracking of geometric entities (points, curves, and surfaces) across remeshing operations, providing robust data transfer capabilities.

Conclusion: This bijective remeshing framework has practical applications in texture transfer and volumetric simulations, where maintaining accurate geometric correspondence during mesh deformation and refinement is essential.
...
### [55] [SWIM: Single-Instance Whole-Body Imitation for swiMming](https://arxiv.org/abs/2605.31120)
*Binglun Wang,Edmond S. L. Ho,He Wang*

Main category: cs.GR

TL;DR: SWIM is a new imitation method for synthesizing physically-based swimming motions that learns from a single reference and generalizes to unseen environments, body conditions, and swimming styles.


<details>
  <summary>Details</summary>
Motivation: Swimming motion synthesis is challenging due to full-body coordination requirements, continuous fluid interactions, volatile environmental forces, generalization difficulties, lack of data references, and slow physical simulation during control learning.

Method: SWIM is an imitation learning method that can learn from a single swimming motion sample and generalize to diverse environments, body conditions, and swimming styles.

Result: Extensive evaluation demonstrates SWIM is data-efficient, stable, robust, and generalizable, outperforming alternative methods across multiple task classes and metrics.

Conclusion: SWIM successfully addresses the complex challenges of physically-based swimming motion synthesis by enabling effective learning from limited data and robust generalization capabilities.
