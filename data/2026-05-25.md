...
TL;DR: A framework that detects underspecified features in demonstration-based reward learning by analyzing variation across demonstrations, then actively queries for targeted corrective demonstrations using natural language explanations.


<details>
  <summary>Details</summary>
Motivation: Learning reward functions from demonstrations assumes demonstrations provide adequate supervision over all task-relevant features. However, demonstrations are often imperfect: humans may under-emphasize certain features due to cognitive load or physical difficulty, or training may not cover all relevant situations. This leads to ambiguous learned reward functions and misaligned behavior at deployment.

Method: The framework detects underspecified features by analyzing statistical variation across demonstrations—features with little variation are well-specified, while those with wide variation are underspecified. It infers which features may have been insufficiently demonstrated, explains uncertainties in natural language, and actively queries for demonstrations that address identified gaps.

Result: In a simulated tabletop manipulation domain and a user study with a real Franka robot, targeted, explanation-guided queries significantly improve reward recovery compared to random querying and passive data collection.

Conclusion: The approach reduces ambiguity that would otherwise persist in learning from imperfect demonstrations, demonstrating that targeted, explanation-guided queries can effectively recover underspecified features.
...
### [4] [PIMbot: A Self-Adaptive Attack Framework for Adversarial Manipulation of Multi-Robot Reinforcement Learning](https://arxiv.org/abs/2605.23027)
*Zexin Li,Ziliang Zhang,Hyoseung Kim,Cong Liu*

Main category: cs.RO

TL;DR: PIMbot is a framework that manipulates multi-robot collaboration outcomes through dual levers—reward channel incentive manipulation and policy manipulation—balanced by an adaptive multi-objective controller, validated in both simulation and real embedded systems.


<details>
  <summary>Details</summary>
Motivation: Multi-robot collaboration in social dilemmas faces challenges from environmental factors like miscommunication and adversarial robots. Understanding how communication can be manipulated to achieve different outcomes is crucial for robust cooperative systems.

Method: PIMbot employs two complementary manipulation levers: (i) incentive manipulation via reward channel modification, and (ii) policy manipulation of an agent's own actions. An adaptive multi-objective controller balances these levers online. The framework is evaluated in Gazebo simulation and on NVIDIA Jetson Orin Nano embedded devices.

Result: Comprehensive experiments demonstrate PIMbot's effectiveness in manipulating social dilemma environments. Real embedded device case studies quantify system costs and validate performance on realistic autonomous embedded systems beyond simulation.

Conclusion: PIMbot serves as a rigorous stress-test tool that exposes critical vulnerabilities in multi-robot cooperative tasks, providing insights into system robustness against manipulation.
...
### [5] [Direct Dynamic Retargeting for Humanoid Imitation Learning from Videos](https://arxiv.org/abs/2605.23762)
*Constant Roux,Ludovic De Matteïs,Armand Jordana,Valentin Guillet,Nicolas Mansard,Olivier Stasse,Philippe Souères*

Main category: cs.RO

TL;DR: Direct Dynamic Retargeting (DDR) is a single-stage framework that generates dynamically feasible humanoid trajectories directly from monocular video demonstrations, bypassing geometric bias and improving tracking accuracy and RL training.


<details>
  <summary>Details</summary>
Motivation: Standard approaches for translating human motion to humanoids rely on intermediate kinematic projections (Geometric/Indirect Dynamic Retargeting) that introduce geometric bias, restricting the search space and yielding suboptimal dynamic behaviors.

Method: DDR is a novel single-stage framework that formulates the problem in task space and leverages a sampling-based Model Predictive Control (MPC) solver within a physics simulator. It generates high-fidelity, dynamically feasible trajectories directly from expert videos, natively optimizing over complex contact sequences while mitigating input drift.

Result: Experiments demonstrate DDR outperforms state-of-the-art baselines in demonstration tracking accuracy. Providing DDR-generated physically viable references to RL agents accelerates training convergence and enhances final execution of agile and balancing behaviors.

Conclusion: Bypassing the geometric bias through direct dynamic retargeting enables superior performance in both imitation learning and RL-based training, establishing DDR as an effective approach for teaching complex skills to humanoid robots.
...
### [6] [Instrumentation for Imitation Learning: Enhancing Training Datasets for Clothes Hanger Insertion](https://arxiv.org/abs/2605.23847)
*Remko Proesmans,Thomas Lips,Francis wyffels*

Main category: cs.RO

TL;DR: This paper explores using sensor instrumentation in objects to provide valuable state information for robotic manipulation, demonstrating improved imitation learning performance for clothes hanger insertion tasks.


<details>
  <summary>Details</summary>
Motivation: Large behavior models require prohibitive data amounts, preventing the same revolution in robotic manipulation that vision language models achieved. The authors believe instrumentation (sensor integration in objects) can provide valuable state information to enable efficient learning.

Method: The authors implement instrumented imitation learning for clothes hanger insertion tasks. They train diffusion policies with and without instrumentation data using 180 teleoperated demonstrations. They also test a black-box imitation learning policy and enhance the dataset with rollouts from an instrumented expert policy.

Result: Policies leveraging instrumentation outperform vision-only counterparts by 14-25 percentage points. A black-box policy learns to prioritize instrumentation signals without explicit guidance. A vision-only student policy enhanced with instrumented expert rollouts achieves performance comparable to the instrumented expert.

Conclusion: These findings establish instrumentation as a promising strategy to enhance imitation learning for robotic manipulation.
...
### [7] [Self-Refining Topology Optimization via an LLM-Based Multi-Agent Framework](https://arxiv.org/abs/2605.23273)
...
### [8] [ARMS: Automatic Reward Shaping for Sparse-Reward Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.23562)
...
### [9] [SCRIPT: Scalable Diffusion Policy with Multi-stage Training for Language-driven Physics-Based Humanoid Control](https://arxiv.org/abs/2605.22894)
...
### [10] [Closing Trajectories: Equation-Free Cyclic Animation via Koopman Surrogates](https://arxiv.org/abs/2605.23462)
...
### [11] [DrawVideo: Generating Long Video from Storyboard Keyframe Sketches](https://arxiv.org/abs/2605.23508)
