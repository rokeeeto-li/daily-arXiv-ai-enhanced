...
### [1] [Comparative Evaluation of 3D Reconstruction Methods for Immersive Visualization of Laboratory Objects](https://arxiv.org/abs/2608.27301)
...
### [2] [Zero-Shot Self-Orchestration with Ledger-Based Control for Improved LLM Coding Performance](https://arxiv.org/abs/2608.26480)
...
### [3] [Risks and Controls for Multi-Agent Systems: an analytical framework for deployment of AI agents across organisational boundaries](https://arxiv.org/abs/2608.26626)
*Alistair Reid,Simon O'Callaghan,Dustin Venini,Liam Carroll,Tiberio Caetano*

Main category: cs.MA

TL;DR: A framework for analyzing risks and governance controls when AI agents interact across organizational boundaries, organized into three deployment tiers: singular, federated, and open environments.


<details>
  <summary>Details</summary>
Motivation: As organizations deploy AI agents, these agents increasingly interact with each other across organizational perimeters (within organizations, with partners, customers, suppliers, and on the open internet). Failures can emerge from these interactions, and once interactions cross organizational boundaries, no single organization can fully see, control, or govern them.

Method: The report introduces three deployment tiers defined by governance structures: (1) singular governance where one organization governs every agent; (2) federated governance where multiple organizations operate in a shared environment under agreed rules; and (3) open environments with no central authority and voluntary standards. Within each tier, it examines risk factors, failure modes, available controls, and identifies who is positioned to apply them.

Result: The framework identifies who is positioned to apply controls in each governance tier and characterizes gaps where no actor is positioned to act, along with the collective action required to close those gaps.

Conclusion: This framework helps organizations, policymakers, and researchers reason about emerging risks in AI agent interactions and determine appropriate governance controls across different deployment scenarios.
...
### [4] [Fixed-Haven Reservation for Online Multi-Agent Pickup and Delivery in Dense Warehouses](https://arxiv.org/abs/2608.26759)
*Taisei Hirayama,Kohei Yoshida,Hiroki Sakaji,Itsuki Noda*

Main category: cs.MA

TL;DR: This paper introduces fixed-Haven reservation for Multi-Agent Pickup and Delivery (MAPD) in dense warehouse layouts with single-lane aisles, dead ends, and tree-like guidepaths. Each agent owns a fixed Safe Haven (typically its start cell) that only the owner can occupy. Implemented as SHARP (Safe-Haven Retreat Planner), this approach achieves 100% success on all tested configurations, though with higher computational cost.


<details>
  <summary>Details</summary>
Motivation: Dense warehouses often contain layouts with single-lane aisles, dead ends, and tree-like guidepaths that leave little room for idle agents to wait without blocking others. Existing MAPD guarantees typically rely on extra waiting endpoints or biconnected topologies, which fail in these challenging layouts.

Method: The researchers study fixed-Haven reservation for online MAPD with time-release tasks. Each agent owns a fixed Safe Haven (Haven) - usually its start cell - that only the owner can occupy and others treat as blocked. They implement this in SHARP (Safe-Haven Retreat Planner), which keeps every busy or retreating agent on a collision-free reserved route ending at its Haven. They compare SHARP with Token Passing (TP), Priority Inheritance with Backtracking (PIBT), and PIBT with Temporary Priority and Temporary Avoidance (PIBTTP-TA).

Result: For finite task releases, the fixed-Haven contract completes all tasks under Haven-Reachability and explicit planning/progress assumptions. SHARP achieves 100% success on all tested configurations in tree-like layouts. A fixed-home-return counterfactual shows that fixed return is a central robustness mechanism. In high-load tree conditions, disabling mid-retreat reassignment worsens service time by 1.89× and makespan by 1.53×.

Conclusion: Fixed-Haven reservation provides robustness for MAPD in tree-like layouts, with SHARP achieving complete success. Fixed return is a central robustness mechanism in these environments, and disabling mid-retreat reassignment significantly degrades performance, suggesting that keeping agents on their fixed Haven routes is crucial.
...
### [5] [Dynamic Haven Selection for Multi-Agent Pickup and Delivery in Constrained Warehouses](https://arxiv.org/abs/2608.26939)
*Taisei Hirayama,Kohei Yoshida,Hiroki Sakaji,Itsuki Noda*
...
### [6] [One Model, Many Minds: Unlocking Multi-Agent Synergy in a Single Agent via Mixture of Roles](https://arxiv.org/abs/2608.27338)
...
### [7] [WALL-SS: Scaling Long-horizon World Models via Next-Scale Autoregression](https://arxiv.org/abs/2608.26239)
...
### [8] [Constraint-Aware Physics-Informed Neural Networks for Static Shape Estimation of Co-Manipulative Continuum Robots](https://arxiv.org/abs/2608.26273)
...
### [9] [Dispersive Forward Tree Search for Optimal Control: Coverage, Complexity, and Computation](https://arxiv.org/abs/2608.26314)
*Shashank A. Deshpande,Jonathan P. How*

Main category: cs.RO

TL;DR: This paper introduces DFT* (Dispersive Forward Tree search), a propagation-based kinodynamic planner with deterministic finite-sample near-optimality guarantees for differentially flat nonlinear systems. The algorithm constructs forward trees using locally dispersive control commands with cost-conditioned dominance pruning to maintain polynomial tree size relative to the horizon.


<details>
  <summary>Details</summary>
Motivation: Steering-based planners require state-to-state boundary value problems that are inaccessible for nonlinear platforms. While forward propagation avoids this requirement, existing forward-based planners have uncharacterized finite-sample behavior and poor practical performance.

Method: The authors work with differentially flat nonlinear systems and prove that a forward tree of locally dispersive control commands contains a near-optimal trajectory. They provide a general mechanism to construct dispersive command sets for control-affine systems and implement DFT* as breadth-first expansion of the forward tree with cost-conditioned dominance pruning to keep tree size polynomial in the horizon.

Result: DFT* delivers consistently competitive and often substantially better solution quality than state-of-the-art kinodynamic planners at comparable solution times on embedded-tier processors. It accelerates with parallel compute scaling and achieves real-time planning in dynamic environments when implemented in a receding-horizon loop.

Conclusion: The proposed propagation-based kinodynamic planner with near-optimality guarantees outperforms existing methods in both solution quality and computational efficiency, achieving practical performance that matches theoretical guarantees.
...
### [11] [RTNav: Towards Real-Time Zero-Shot Object Navigation](https://arxiv.org/abs/2608.26496)
*Easop Lee,Lingyu Zhang,Boyuan Chen*

Main category: cs.RO

TL;DR: RTNav is a real-time object navigation architecture that explicitly accounts for inference latency and asynchronous environment stepping, improving performance under realistic timing conditions.


<details>
  <summary>Details</summary>
Motivation: Vision and language foundation models enable object navigation in unknown environments but introduce non-negligible inference latency. Most prior work operates in synchronous simulators where inference time is effectively free, leading to architectures designed for sequential execution without time constraints. Real-time execution reveals inefficiencies and performance degradation.

Method: RTNav is a simple but effective architecture that treats inference latency, asynchronous environment stepping, and bounded compute as explicit design considerations, moving beyond the sequential perception-reasoning-action paradigm.

Result: Evaluated on real-time variants of HM3D-v1, HM3D-v2, and HM3D-OVON datasets, RTNav improves success rate by up to 11% and Success weighted by Completion Time by up to 5.1 points over prior work.

Conclusion: Explicitly designing for inference latency and asynchronous execution yields significant performance gains in real-time object navigation scenarios, demonstrating the importance of accounting for wall-clock time constraints in embodied AI systems.
...
### [12] [Closing the Loop on the Poppy Humanoid: Bipedal Locomotion with Linear-Quadratic Control and Learned Cost Functions](https://arxiv.org/abs/2608.26505)
*Xulin Chen,Borui He,Ruipeng Liu,Naveed Tahir,Zhenyu Gan,Garrett E. Katz*

Main category: cs.RO

TL;DR: This paper presents a closed-loop walking controller for the Poppy humanoid robot based on the linear-quadratic regulator (LQR) framework, achieving statistically significant improvements in walking reliability compared to open-loop trajectory playback.


<details>
  <summary>Details</summary>
Motivation: The Poppy humanoid robot is an open-source, low-cost platform for AI research and education, but there was no published methodology for achieving reliable, unassisted bipedal locomotion on this hardware.

Method: The method uses the linear-quadratic regulator (LQR) framework for trajectory tracking. Starting from data collected during open-loop playback of a nominal walking trajectory, the approach learns a quadratic cost function for the LQR controller.

Result: The closed-loop controller was validated empirically, demonstrating statistically significant improvements in walking performance compared to the open-loop approach.

Conclusion: The proposed LQR-based closed-loop controller substantially improves the reliability of the Poppy humanoid's bipedal locomotion, providing a functional solution for unassisted walking on the standard Poppy hardware.
...
### [13] [Memory Anchors for Continual Robot Learning](https://arxiv.org/abs/2608.26545)
...
### [15] [SOLO: Stable Omni-terrain Long-Horizon Perceptive Humanoid Locomotion](https://arxiv.org/abs/2608.26583)
...
TL;DR: SOLO is a unified framework for humanoid robots to navigate complex terrains over long distances without losing balance. It addresses two key issues: terrain reconstruction that smooths action-critical details (solved with Query Reconstructor using Fourier-encoded queries), and pointwise imitation lacking temporal credit assignment (solved with Trajectory-Aware MSE Distillation).


<details>
  <summary>Details</summary>
Motivation: Humans can traverse complex terrain over long distances without losing balance, but perceptive humanoid policies become fragile as perception and control errors accumulate. The paper identifies two compounding causes: (1) dense terrain reconstruction smooths out action-critical details, and (2) pointwise imitation lacks temporal credit assignment.

Method: SOLO introduces two key components: (1) Query Reconstructor (QR) - uses Fourier-encoded cell queries to retrieve spatially specific evidence from depth-proprioception tokens, preserving sharp terrain boundaries; (2) Trajectory-Aware MSE (TA-MSE) Distillation - adds next-state teacher-student disagreement to PPO reward, enabling Generalized Advantage Estimation to propagate future disagreement penalties to preceding actions.

Result: QR reduces height-map L1 error by factors of 3.3-4.0; TA-MSE surpasses PPO and MSE+PPO in curriculum progression. On stress-test terrains, SOLO achieves 97.5% mean traversal success and 96% stepping-stone success versus 75.0-75.6% and 0-3% for dense-reconstructor variants. Zero-shot deployment completes a 1.5-km outdoor route and indoor mixed-terrain course using only a chest-mounted depth camera and proprioception.

Conclusion: SOLO successfully enables humanoid robots to navigate complex terrains over long distances with high success rates, outperforming dense-reconstructor variants by a significant margin. The framework demonstrates practical deployment capabilities, completing continuous outdoor routes (1.5 km) and indoor courses with only basic sensing (chest-mounted depth camera + proprioception), proving that addressing both reconstruction quality and temporal credit assignment can overcome long-horizon fragility in humanoid navigation.
...
### [16] [Relaxation-Aware Multimodal Sensing of Soft Gripper Driven by Structure-Perception-Learning](https://arxiv.org/abs/2608.26622)
*Yanzhe Wang,Hao Wu,Ziyi Zheng,Huixu Dong*

Main category: cs.RO

TL;DR: A variable-stiffness soft gripper with onboard vision and thermography tracks deformation in real time, while a temperature-coupled physics-informed learning model compensates for viscoelastic force decay, achieving stable force control with 0.066N MAE in 280s grasp-and-hold tasks.


<details>
  <summary>Details</summary>
Motivation: Soft robotic hands suffer from intrinsic viscoelasticity causing stress relaxation and continuous force decay during holding, unlike human grasping which combines phase-dependent stiffness regulation with continuous sensing and feedback.

Method: Develops an integrated mechanism-perception-learning framework featuring a variable-stiffness soft gripper with onboard vision and infrared thermography for real-time deformation and temperature field tracking. Uses a temperature-coupled viscoelastic force representation combined with a physics-informed learning model to reconstruct force trends and provide explicit compensation during holding.

Result: In a 280-second force-controlled grasp-and-hold task, the proposed method maintains desired force with a mean absolute error of 0.066N, outperforming fixed-aperture baselines by 80% and instantaneous-only baselines by 95%.

Conclusion: Supports a mechanism-AI co-design perspective where physical mechanisms shape feasible interactions while learning compensates remaining uncertainty in viscoelastic dynamics, together enabling stable, sustained grasping.
...
### [17] [FLARE: A Failure-Aware Framework for Autonomous Correction and Recovery in Visual-Language Robotic Manipulation](https://arxiv.org/abs/2608.26645)
...
### [18] [Beyond the Proving Ground: Independent Public-Road Testing of Assisted Lane Change Systems using LiDAR](https://arxiv.org/abs/2608.26669)
...
TL;DR: This study presents the first public road campaign to independently test Assisted Lane Change systems against UNECE Regulation 79 safety distance requirements using a LiDAR-based vehicle detection system on the A31 French motorway.


<details>
  <summary>Details</summary>
Motivation: Commercial ADAS testing requires safety and compliance verification, but proving ground scenarios may not reflect real-world driving complexity, and geo-fencing approaches require manufacturer collaboration that limits assessment independence.

Method: A test vehicle equipped with a LiDAR-based vehicle detection and tracking system was used on the A31 French motorway to test Assisted Lane Change systems. Tests covered combinations of inter-vehicle distance and speed between the test vehicle and takeover vehicle. Real-time kinematic global navigation satellite system receivers assessed detection and tracking performance, and recorded lane change trajectories were compared with UNECE Regulation Number 79 requirements.

Result: Of 27 predefined lane change maneuvers, 18 were completed and 9 were suppressed. The system allowed 6 maneuvers that did not meet regulatory minimum distance requirements, and in 3 cases the deviation remained statistically significant after accounting for measurement uncertainty.

Conclusion: LiDAR-based sensing is suitable for independent public road assessment of Assisted Lane Change compliance. This methodology can support market surveillance and future regulatory revisions by revealing real-world behaviors not covered by approval procedures.
...
### [19] [PredVLA: A Sub-Million-Parameter Predictive-Coding Policy for Robot Manipulation](https://arxiv.org/abs/2608.26673)
...
TL;DR: This paper proposes a residual deep reinforcement learning-enhanced computed torque control framework for cable-driven lower-limb rehabilitation robots, combining a model-based nominal command with a bounded DDPG policy for compensating torque, demonstrating improved tracking and disturbance rejection while preserving interpretability and constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: Accurate trajectory tracking in cable-driven lower-limb rehabilitation robots is challenging due to model uncertainty, external disturbances, joint constraints, and pull-only cable actuation. Conventional model-based controllers are interpretable but sensitive to model mismatch, while fully learning-based control reduces transparency and complicates constraint-aware operation.

Method: The proposed framework combines computed torque control (generating nominal commands) with a bounded Deep Deterministic Policy Gradient (DDPG) policy that supplies only an additional compensating torque. Evaluation covers nominal, uncertain, disturbed, combined, and generalization conditions, with diagnostics for trajectory tracking, joint-limit, cable-demand, workspace-feasibility, and cable-Jacobian constraints.

Result: The residual controller improves tracking and disturbance rejection relative to computed torque control while preserving the interpretable model-based command structure and satisfying feasibility checks. Tracking improvements persist beyond the representative case, though trajectory-dependent constraint limitations are also exposed.

Conclusion: Bounded residual learning is validated as a practical robustness-enhancement strategy for simulation-based rehabilitation robot control, supporting further constraint-aware and experimental validation efforts.
...
### [21] [MeshPriorDiT: Hierarchical Modeling for Action-Conditioned Cloth Dynamics](https://arxiv.org/abs/2608.26766)
*Zihang Wang,Jianming Hu,Shang Su,Hao Huang,Mengkai Shi,Jun Gao,Shuo Feng*

Main category: cs.RO

TL;DR: MeshPriorDiT is a hierarchical cloth dynamics model combining mesh GNN priors with Residual DiT generation to achieve both local plausibility and long-range coordination in action-conditioned cloth manipulation.


<details>
  <summary>Details</summary>
Motivation: Action-conditioned cloth dynamics prediction requires balancing local physical plausibility (material connectivity) with long-range coordination. Existing mesh GNNs have limited message-passing range, while Transformer-based models lack explicit material topology and must learn local responses from data. Both approaches suffer from autoregressive error accumulation or lack of structural priors.

Method: MeshPriorDiT decomposes future cloth motion into a structured mesh prior and generative residual. An action-conditioned mesh GNN predicts multi-step vertex displacements as a reference trajectory respecting material topology and grasp constraints. A Residual DiT (conditional flow matching) then generates the residual motion not captured by the prior. The residual is rescaled and decoded using material adjacency for cross-neighborhood coordination.

Result: On 15-step autoregressive rollouts across three cloth manipulation tasks, MeshPriorDiT reduced average Global MSE by 43.42% vs GNN-Only baseline and by 75.03% vs DiT-DDPM baseline, while maintaining favorable Edge-strain MSE comparable to GNN-Only.

Conclusion: The hierarchical approach combining explicit mesh topology priors with conditional flow matching effectively balances local deformation plausibility and long-range coordination, significantly outperforming both pure GNN and pure Transformer baselines in cloth manipulation tasks.
...
### [22] [Online Joint Calibration of Steering Offset and Planar LiDAR Extrinsics for Wheeled Mobile Robots](https://arxiv.org/abs/2608.26789)
...
### [23] [Rapid On-Robot Learning for Dynamic Manipulation Skills: Robot Juggling](https://arxiv.org/abs/2608.26800)
*Taeyoon Lee,Chunpeng Wang,Christopher G. Atkeson,Alfred A. Rizzi,Nicolas Rojas*

Main category: cs.RO

TL;DR: A bimanual robot learns diverse juggling patterns in minutes using regularized memory-based learning that combines local experience with global prior models, ensuring safety through mutually reachable sets.


<details>
  <summary>Details</summary>
Motivation: Bimanual robots need to acquire complex manipulation skills like juggling directly on physical hardware despite sim2real gaps, requiring an approach that builds upon existing knowledge rather than starting from scratch.

Method: The framework employs regularized memory-based learning where the robot learns local models from accumulated experience while retaining global prior models for extrapolation. Safety is ensured through mutually reachable sets that constrain transitions between throws and catches, preventing joint or actuator limit violations.

Result: The robot successfully learned five canonical three-ball juggling patterns (cascade, tennis, half-shower, shower, and box) in under 5 minutes of real-world interaction, demonstrating efficient and stable online learning.

Conclusion: This work demonstrates that robots can effectively build upon imperfect prior knowledge and continually refine their behavior through real-world experience, providing a foundation for more capable continual learning systems.
...
### [25] [TemporalFlow-VLA: Learning Physically Grounded Execution History for Long-Horizon Robot Manipulation](https://arxiv.org/abs/2608.26821)
...
### [26] [Active Surface-Driven Reconfigurable Gripper: Robust Grasping and Sequential Manipulation of Thin Objects](https://arxiv.org/abs/2608.26883)
...
### [27] [Beyond Shallow-Water Photorealism: Physically and Sensor-Grounded Simulation for Deep-Sea Robotics](https://arxiv.org/abs/2608.26888)
*Michele Grimaldi,Enrico Di Maria,Ignacio Carlucho,Yvan R. Petillot*

Main category: cs.RO

TL;DR: This paper extends the Stonefish underwater simulator to better represent deep-sea physical and sensor realism, adding stochastic sensor drift, higher-order hydrodynamics, terramechanics, pressure-driven environmental variability, and physically based underwater optics while maintaining real-time simulation compatibility.


<details>
  <summary>Details</summary>
Motivation: Many recent underwater simulators prioritize visual realism over physical fidelity, focusing on shallow-water effects with limited relevance to deep-water environments. This lack of physical accuracy introduces non-physical artifacts and overly optimistic performance estimates, which is particularly problematic for long-duration navigation and learning-based autonomy in deep-sea conditions.

Method: The authors augment the Stonefish simulator with: (1) stochastic IMU and DVL drift models, (2) magnetometer disturbances, (3) higher-order hydrodynamic terms, (4) terramechanics for ground interactions, (5) pressure-driven environmental variability, and (6) physically based underwater optical models. These additions are designed to capture the forces and measurements that shape deep-ocean vehicle behavior while maintaining real-time simulation performance.

Result: The proposed extension enables more accurate representation of deep-sea operating conditions for AUVs, ROVs, landers, ASVs, and gliders. The framework provides a practical foundation for navigation, perception, and autonomy research by eliminating non-physical artifacts caused by inaccurate sensor and environmental models, though challenges remain in modeling complex fluid-structure interactions and full environmental stochasticity.

Conclusion: While challenges such as complex fluid-structure interactions and complete environmental stochasticity persist, this work advances underwater simulation toward more representative deep-sea operating conditions. The approach is particularly valuable for long-duration navigation and learning-based autonomy applications where accurate physical modeling is critical.
...
### [28] [Contact-Aided Factor-Graph Localization for Underwater Sampling](https://arxiv.org/abs/2608.26932)
...
### [29] [4DSynth: Controllable Procedural World Synthesis for Dynamic Embodied Simulation](https://arxiv.org/abs/2608.26947)
...
### [30] [Arbitrary-Order Hermite Interpolation of Rigid-Motion Jets via Hyper-Multidual Quaternions](https://arxiv.org/abs/2608.27000)
*Daniel Condurache*

Main category: cs.RO

TL;DR: The paper studies interpolation of finite-order rigid-motion jets using hyper-multidual (HMD) quaternions, extends screw linear interpolation (ScLERP) to this framework, and shows that direct HMD-ScLERP is generally non-holonomic. A holomorphic alternative using logarithmic dual-quaternion coordinates and Hermite polynomials is proposed and validated.


<details>
  <summary>Details</summary>
Motivation: Develop a mathematical framework for interpolating finite-order rigid-motion trajectories (poses and their derivatives) represented by unit dual quaternions, and characterize holonomic (i.e., consistent with the underlying pose projection) interpolation methods for rigid-body motions at higher orders.

Method: 1. Introduce hyper-multidual (HMD) quaternions as dual quaternions with coefficients in $\mathbb{R}[\varepsilon]/(\varepsilon^{n+1})$; 2. Define temporal HMD transforms encoding poses and derivatives, establishing unitarity and deriving recursive coefficient constraints; 3. Extend ScLERP algebraically to unit HMD quaternions; 4. Analyze holonomicity of direct HMD-ScLERP; 5. Propose a holomorphic alternative using logarithmic dual-quaternion coordinates and degree-(2n+1) Hermite polynomials.

Result: 1. Temporal transforms and their relative descriptors are unitary; 2. Recursive coefficient constraints exist for temporal transforms; 3. Direct HMD-ScLERP is generically non-holonomic for arbitrary endpoint jets; 4. A holonomic alternative using logarithmic coordinates, Hermite polynomials, and exponential lifting successfully reproduces endpoint jets; 5. HMD arithmetic recovers higher-order rigid-motion acceleration fields without explicit differentiation of dexp.

Conclusion: While direct HMD-ScLERP fails to be holonomic in general, a holomorphic interpolation method can be constructed using logarithmic dual-quaternion coordinates and Hermite polynomials. This approach validates the theoretical analysis and demonstrates HMD arithmetic's efficiency for computing higher-order rigid-motion acceleration fields without explicit differentiation.
...
### [31] [Riemann-1.0: An Embodied World Action Model for Physical AI](https://arxiv.org/abs/2608.27033)
...
TL;DR: Riemann-1.0 is a fully causal autoregressive World Action Model (WAM) for embodied intelligence that unifies online robot policy execution and action-conditioned world simulation, trained via progressive pretraining across egocentric human videos, handheld-gripper demonstrations, and heterogeneous robot trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing World Action Models are limited by joint generation paradigms, video-first prediction approaches, or decoupled modeling strategies that prevent unified execution and simulation. A unified model capable of functioning as both an executable robot policy and a multi-embodiment visual world simulator is needed to effectively scale embodied experience across heterogeneous data sources.

Method: Riemann-1.0 adopts a fully causal autoregressive architecture that jointly models multi-view visual observations, robot states, and actions within a unified sequence, representing robot actions and world evolution as causal state transitions. The method includes a progressive embodied pretraining framework that unifies learning from egocentric human videos, handheld-gripper demonstrations, and heterogeneous robot trajectories under a shared World Action Modeling objective, built upon 200K+ hours of interaction data.

Result: Riemann-1.0 achieves state-of-the-art performance: 94.3% on RoboTwin2.0, 99.0% on LIBERO, 62.6% on RoboCasa-365 (8.4% improvement over previous best), 85.0% Success Rate and 94.4% Progress Success Rate on long-horizon real-world manipulation tasks (15% improvement over strongest open-source baseline).

Conclusion: Unified World Action Modeling combined with progressive embodied pretraining effectively transforms large-scale embodied experience into generalizable robot manipulation capabilities, demonstrating superior performance across simulation benchmarks and real-world tasks.
...
### [32] [GRAFT: Grounded and Efficient Online Reinforcement Adaptation for Fine-Grained Robot Manipulation](https://arxiv.org/abs/2608.27079)
...
### [33] [Pass the Bucket: Efficient, Robust, Local Load Balancing for Teams of Heterogeneous Robots](https://arxiv.org/abs/2608.27085)
*Tobias Wallner,Dominik Krupke,Arne Schmidt,Sándor P. Fekete*

Main category: cs.RO

TL;DR: This paper studies decentralized, self-organized task sharing for heterogeneous robot swarms using bucket brigade mechanisms in confined 1D spaces, introducing local stabilization tokens to prevent chaotic oscillations and achieve robust load balancing without central control.


<details>
  <summary>Details</summary>
Motivation: To develop a decentralized, self-organized mechanism for load balancing among heterogeneous robots that collaborate on tasks requiring coordinated motion planning, without relying on central control or information.

Method: The authors investigate bucket brigade mechanisms where robots share a spatial task in confined 1D spaces, using only local collision sensing. They develop a stabilization mechanism based on local "token" aid that temporarily decelerates robots after encounters, compare single boundary tokens to two-directional tokens, and optimize deceleration factors. Event-driven simulations validate the approach.

Result: The local token mechanism eliminates persistent oscillations and enables convergence to stable system states. Event-driven simulations demonstrate reliable convergence and robustness against various perturbations including robot deletion, position jittering, and velocity jittering.

Conclusion: The results demonstrate that simple local mechanisms can achieve effective, robust load balancing for heterogeneous robot teams, providing a practical foundation for more complex collaborative scenarios.
...
### [34] [Active sensing to characterize the heterogeneity of plant stress](https://arxiv.org/abs/2608.27088)
...
### [35] [Planning a Shared Modular Fixture Layout Across Robotic Disassembly Stages](https://arxiv.org/abs/2608.27151)
...
### [36] [Task-space model-based control of pneumatic soft actuators](https://arxiv.org/abs/2608.27186)
...
### [37] [Tensegrity Continuum Robots Enable Task-Adaptive Morphologies for Cooperative Behaviors](https://arxiv.org/abs/2608.27221)
*Mahmud Hasan Saikot,Sydney Spiegel,Sudheera Akalanka Kariyawasam,Andrew Stefka,Josh Chrisler,Jianguo Zhao*

Main category: cs.RO

TL;DR: A modular reconfigurable robot that combines tensegrity-based compliance with claw-based connections, enabling both individual manipulation/locomotion and self-reconfiguration into various morphologies for cooperative tasks.


<details>
  <summary>Details</summary>
Motivation: Robots with adaptable morphologies and behaviors are promising for multifunctional systems. Existing modular reconfigurable robots rely on rigid modules lacking compliance, while continuum robots offer compliance but cannot self-reconfigure. A unified architecture combining both advantages is needed.

Method: Introduces a modular reconfigurable robot combining a tensegrity-based compliant body with claw-based connection mechanisms. Each robot can manipulate and locomote independently, and multiple robots can self-reconfigure into different morphologies (chains, loops, branches) for cooperative tasks.

Result: Demonstrated capability across diverse tasks including coordinated object manipulation and transport, multimodal locomotion, and loco-manipulation in real-world scenarios. Robots successfully self-reconfigure into various morphologies.

Conclusion: These results lay a foundation for adaptable and multifunctional robotic collectives with broad potential applications in manufacturing, space exploration, and search-and-rescue operations.
...
### [38] [STEP: State-Aware Task Estimation and Planning with Multi-Modal LLMs for Human-Robot Collaboration](https://arxiv.org/abs/2608.27225)
...
### [39] [Embodied Scene Rearrangement Planning](https://arxiv.org/abs/2608.27371)
...
### [40] [FlashVLA: Streaming Action Decoding for Fast and Asynchronous VLA Inference](https://arxiv.org/abs/2608.27384)
...
### [41] [CLAP: Cross-Embodiment Video World Models are Zero-Shot Physical Simulators](https://arxiv.org/abs/2608.27406)
*Kechen Liu,Ola Shorinwa*

Main category: cs.RO

TL;DR: CLAP is a cross-embodiment action-conditioned video generation framework that leverages internet-scale heterogeneous video data across human and robotic agents to learn generalizable physics, achieving state-of-the-art performance through curriculum-based learning across multiple action spaces.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art action-conditioned video models are restricted to single robot embodiments, preventing them from leveraging vast heterogeneous video corpora containing rich signals for learning generalizable physics. Cross-embodiment learning faces the challenge that action representations vary sharply across robot platforms and are typically absent in human videos.

Method: CLAP reconciles disparate action spaces using end-effector poses, language instructions, and latent actions, and introduces a curriculum-based cross-embodiment learning recipe that first learns foundational physical priors across unlabeled video data using latent actions, then grounds them in end-effector action spaces for zero-shot deployment.

Result: CLAP approaches or surpasses state-of-the-art single-embodiment video models in challenging environments like DROID, and achieves performance advantages via few-shot adaptation, establishing a new paradigm for training single-embodiment video world models.

Conclusion: CLAP delivers the most comprehensive suite of action-conditioned video world models to date, spanning diverse action-conditioning spaces (end-effector, language, and latent) and robot morphologies (including cross-embodiment, DROID, Bridge, bimanual YAM robots, and G1 humanoids), with all code and models open-sourced.
