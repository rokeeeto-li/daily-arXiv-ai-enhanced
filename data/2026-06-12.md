...
TL;DR: SAIGuard proposes a proactive defense framework for LLM-based multi-agent systems (MAS) that performs communication-state simulation to detect and sanitize risky messages before propagation, reducing attack success rates while maintaining collaborative utility.


<details>
  <summary>Details</summary>
Motivation: LLM-based MAS face security risks due to their communication-driven nature; existing reactive defenses (detecting and isolating harmful agents) may cause irreversible damage and degrade collaborative utility.

Method: SAIGuard performs communication-state simulation over the MAS interaction graph, estimates the impact of incoming messages on local agent states and the global MAS state, and detects risky messages via reconstruction deviations from benign communication patterns. Instead of isolating agents, it sanitizes or regenerates suspicious messages before propagation.

Result: Experiments across diverse topologies and attack scenarios demonstrate that SAIGuard reduces attack success rates while maintaining MAS utility.

Conclusion: SAIGuard outperforms reactive defenses by proactively preventing security threats before they cause system-wide failures.
...
TL;DR: This paper investigates how model scale impacts security in linear multi-agent workflows, finding that larger LLMs are more vulnerable to malicious instructions but adding a lightweight terminal Fixer stage can restore resilience by collapsing the compliance gap from 53.7pp to 0.6pp.


<details>
  <summary>Details</summary>
Motivation: As LLM-based multi-agent systems (MAS) are deployed in production, understanding how model scaling affects resilience against adversarial compromise (e.g., prompt-injection, jailbreaking) is critical, yet the interaction between model scale and system-level security remains poorly understood.

Method: The authors investigated model scale effects on security in linear multi-agent workflows using experiments across scales of two open-weight model families on the HumanEval benchmark, examining compliance-correction symmetry with and without a lightweight terminal Fixer stage.

Result: Experiments revealed a compliance-correction symmetry: larger models are far more likely to execute malicious instructions (53.7pp drop from control to malicious at 27B in uncorrected pipelines), but appending a Fixer stage collapses this gap to 0.6pp, restoring statistical parity with control-level performance.

Conclusion: Linear collaboration structures can be viable and resilient to adversaries at this scale, suggesting that brittleness previously attributed to linear topology may stem from lack of correction mechanisms rather than the topology itself.
...
TL;DR: The paper introduces the Internet of Agentic AI (IoAI) as an open ecosystem enabling heterogeneous agents to discover, negotiate, exchange context, invoke tools, and execute workflows across cloud, edge, device, and cyber-physical environments.


<details>
  <summary>Details</summary>
Motivation: Autonomous AI agents are rapidly emerging, transforming AI from isolated model inference into distributed systems of reasoning, communication, and action.

Method: Synthesizing foundations from single-agent agentic AI, multi-agent systems, distributed computing, communication networks, game theory, and security engineering to characterize architectures and mechanisms for scalable agent ecosystems.

Result: Developed a comprehensive framework examining agent deployment models, workflow lifecycles, communication protocols, interoperability layers, resource-management challenges, and trust architectures with case studies in adaptive manufacturing and distributed operational coordination.

Conclusion: The framework identifies five central research challenges: controlled emergence, semantic interoperability, secure identity, incentive-compatible coordination, resource-aware orchestration, and governance for large-scale networks of autonomous agents.
...
TL;DR: This paper introduces a framework bridging α-fairness with Heterogeneous-Agent Trust Region Learning (HATRL) to ensure monotonic improvement and convergence to Nash Equilibria in multi-agent systems, proposing α-fair HATRPO and α-fair HAPPO algorithms.


<details>
  <summary>Details</summary>
Motivation: Utilitarian objectives in multi-agent systems maximize efficiency but ignore reward distribution, leading to inequitable leader-follower dynamics. While fairness-based approaches encourage pro-social behaviors, many current algorithms (including reward shaping) break stationarity of Markov Games or lack rigorous theoretical guarantees, creating a gap between fair objectives and safe learning frameworks.

Method: The proposed framework uses a fair advantage function that dynamically weights agent utilities based on expected returns, allowing the global objective to transition from utilitarian efficiency to α-fairness welfare via parameter α. Two practical algorithms are introduced: α-fair HATRPO and α-fair HAPPO.

Result: Experiments in sequential social dilemmas (CleanUp and CommonHarvest) demonstrate that the proposed algorithms perform better than HATRL's utilitarian algorithms from an efficiency perspective while achieving socially higher outcomes.

Conclusion: The framework ensures monotonic improvement and convergence toward Nash Equilibria, successfully bridging the gap between fairness-based approaches and theoretically safe learning frameworks in multi-agent reinforcement learning.
...
TL;DR: This paper introduces dense alignment for heterogeneous KV-cache communication between multi-agent systems via lightweight cross-model cache transformation and two-phase training (reconstruction + generation), achieving superior performance to prior methods and text communication at 2-3x lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems currently communicate through expensive, lossy text. While KV-cache communication is promising, prior work is either homogeneous (duplicate models) or restrictive (shared input, steering only). The authors ask whether heterogeneous agents can be aligned well enough for "mind reading" - transferring both what one agent sees and how it thinks.

Method: Information-structure analysis reveals two transfer modes: context-aware (sparse reasoning signals) and context-unaware (dense contextual knowledge preservation). The authors propose dense alignment for heterogeneous KV-cache communication using a lightweight cross-model cache transformation and two-phase training: reconstruction followed by generation.

Result: Experiments across six model sizes (Qwen3-4B, 8B, 14B) and six benchmarks (in-domain and out-of-domain) demonstrate that the method outperforms prior heterogeneous baselines, matches or exceeds text communication in context-aware settings at 2-3x lower compute, and remains effective in context-unaware transfer where prior methods collapse.

Conclusion: The proposed dense alignment approach enables effective cross-model knowledge transfer between heterogeneous agents, achieving comparable or better performance than text communication at significantly lower computational cost, particularly excelling in challenging context-unaware scenarios where existing approaches fail.
...
TL;DR: Large-scale agent-based predator-prey models using recurrent neural network controllers are optimized with a feature-based loss function to achieve classical Lotka-Volterra population cycles.


<details>
  <summary>Details</summary>
Motivation: Growing compute power enables large-scale agent-based modeling, but such models face challenges with many local rules/parameters that can cause runaway behavior, collapse, or artificial saturation. The authors aim to determine if environmental and demographic parameters can be tuned to produce classical Lotka-Volterra oscillations in a continuous predator-prey system with neural network controllers.

Method: A continuous predator-prey system featuring sheep and wolves as active agents with local sensing, internal energy, and RNN-based controllers. Parameter optimization uses a feature-based loss function rewarding sustained oscillations, phase lag, bounded populations, and long-term persistence, tested first with random controllers and then evolved controllers. The model is implemented in ABMax, a JAX-based agent-based modeling framework for efficient batched simulation on hardware accelerators.

Result: The abstract presents the methodology for parameter optimization to achieve desired population dynamics, demonstrating the feasibility of using feature-based loss functions to tune complex adaptive systems toward classical ecological patterns.

Conclusion: The work establishes a framework for tuning agent-based predator-prey models to exhibit classical Lotka-Volterra cycles through systematic parameter optimization with neural network controllers.
...
TL;DR: This paper demonstrates that vision-language-action (VLA) models can support human-robot collaboration (HRC), identifies a failure mode in action-chunking policies where demonstration action leakage causes premature assistive behavior, and proposes an inference-time steering method to mitigate this issue, resulting in faster collaboration and fewer failures in a 16-participant user study.


<details>
  <summary>Details</summary>
Motivation: Existing HRC systems rely on hand-engineered pipelines that limit scalability and flexibility for new tasks. The authors aim to show that end-to-end trained VLA models can effectively support collaborative manipulation and characterize the key factors affecting real-world performance.

Method: The authors evaluate two state-of-the-art VLA models, identify a failure mode in action-chunking policies for implicit HRC (demonstration action leakage), and propose an inference-time steering method to mitigate erroneous assistive actions while preserving policy performance.

Result: Action-chunking policies fail when action chunks cross latent task transitions, causing premature assistive behavior that increases with longer execution horizons. The steering method successfully mitigates these errors. In a 16-participant user study on a long-horizon collaborative assembly task, steering enabled longer execution horizons, faster collaboration, and fewer failures compared to a shorter-horizon policy.

Conclusion: Inference-time steering is an effective approach to enable longer execution horizons in collaborative VLA systems while reducing premature assistance and improving overall collaboration performance.
...
TL;DR: AEM is a pretraining framework that learns compact temporal representations from vision-action history for robot manipulation, improving performance over single-frame methods by modeling manipulation as an action-driven interaction process.


<details>
  <summary>Details</summary>
Motivation: Prior robot representation pretraining methods focus mainly on single-frame visual encoding, but current observation alone is often insufficient under partial observability. Manipulation has a temporal nature that requires capturing history rather than just individual frames.

Method: AEM models manipulation as an action-driven interaction process by interleaving visual and action features and applying masked modeling to recover missing content from incomplete histories. The Mamba-encoded output of the final vision token serves as a compact history representation, serving as global context for decoding and downstream control while preserving a single-vector temporal bottleneck for efficient inference.

Result: Evaluated with Diffusion Policy and Flow Policy, AEM consistently improves manipulation performance across clean scenes, cluttered and random scenes, and non-Markovian tasks. Ablation studies confirm history-aware pretraining surpasses single-frame pretraining and direct frame stacking while reducing inference latency and computational cost.

Conclusion: AEM effectively learns action-conditioned state evolution from incomplete histories, providing better temporal representations for robot manipulation than prior approaches, with improved performance and lower computational overhead.
...
### [10] [Foresight: Iterative Reasoning About Clues that Matter for Navigation](https://arxiv.org/abs/2606.12550)
*Arthur Zhang,Carl Qi,Donne Su,Xiangyun Meng,Amy Zhang,Joydeep Biswas*

Main category: cs.RO

TL;DR: Foresight is a test-time framework that uses pretrained Vision-Language Models (VLMs) to iteratively propose and critique motion plans for open-world mapless navigation, improving task success by 37% and reducing interventions by 52% compared to state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Open-world mapless navigation requires resolving underspecified goals and inferring relevant environmental cues. Prior works are limited by reliance on known navigation factors/closed-set categories or by identifying cues before motion planning, missing plan-dependent cues. Pretrained VLMs can discover novel instruction-relevant cues but require adaptation to focus on which cues matter and how they should influence motion planning.

Method: Foresight alternates a finetuned VLM between proposing image-space motion plans and critiquing them using language goals and visual context. Subsequent plans are conditioned on prior critiques for iterative refinement. A reward model from human feedback post-trains the VLM with reinforcement learning in the plan-critique loop to align with open-set behavior preferences.

Result: Foresight improves average task success by 37% and reduces interventions per mission by 52% relative to state-of-the-art test-time reasoning and foundation-model baselines in offline evaluations and 6 real-world environments, while running in real-time on a Jetson AGX Orin.

Conclusion: The plan-critique loop enables effective alignment with open-set behavior preferences, demonstrating that pretrained VLMs can discover novel instruction-relevant cues and adapt them to motion planning when properly fine-tuned with reinforcement learning from human feedback. The authors will release code, data, and training details to support future work on test-time reasoning for robot motion refinement.
...
### [11] [G-MAPP: GPU-accelerated Multi-Agent Planning and Perception for Reactive Motion Generation](https://arxiv.org/abs/2606.12579)
*Tanmay Bishnoi,Riddhiman Laha,Tobias Löw,Jose Alex Chandy,Luis F. C. Figueredo,Sami Haddadin*

Main category: cs.RO

TL;DR: Reactive motion generation in unstructured environments is accelerated using GPU-based parallel state exploration and tighter perception-action coupling, achieving up to 5x speedup over CPU implementations.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reactive motion generation in unstructured environments face computational complexity bottlenecks, either compromising on runtime performance by generating global trajectories for static scenarios or making conservative environmental assumptions. The primary challenges are the runtime performance demand of planning on high-fidelity environments and temporal integration between perception and planning modules.

Method: A GPU-accelerated framework that accelerates world modeling and vector-field based planning to enable faster parallel state exploration for quasi-global trajectory planning and tighter coupling of the perception-action loop in real-time for dynamic cluttered environments with off-the-shelf depth sensors.

Result: Quantitative evaluation shows up to 5x speedup over the CPU version, with both computation-time and success rate improvements. Qualitative experiments on a 7-DoF Franka Emika robot demonstrate successful collision avoidance across both trivial and challenging physical world scenarios.

Conclusion: The GPU-based framework successfully addresses the computational bottlenecks in reactive motion generation while maintaining high-fidelity world representations, achieving significant performance improvements without compromising safety or capability in unstructured environments.
...
### [12] [From Imitation to Alignment: Human-Preference Flow Policies for Long-Horizon Sidewalk Navigation](https://arxiv.org/abs/2606.12603)
*Honglin He,Zhizheng Liu,Yukai Ma,Bolei Zhou*

Main category: cs.RO

TL;DR: FlowPilot is a mapless navigation policy for autonomous sidewalk navigation using only a monocular RGB camera, combining anchored flow matching for action representation and human-in-the-loop preference learning to improve social compliance and counterfactual reasoning.


<details>
  <summary>Details</summary>
Motivation: Autonomous long-horizon sidewalk navigation for micro-mobility applications (robotic food delivery, assistive wheelchairs) requires precise maneuvering through unpredictable terrains and pedestrians with minimal perception (single monocular RGB camera). Existing imitation learning approaches suffer from compounding errors, lack of social compliance, and deficiencies in counterfactual reasoning to handle complex sidewalk situations.

Method: FlowPilot employs two key components: (1) Anchored flow matching as an action representation for policy pre-training on large-scale robot fleet data, capturing diverse, complex, multimodal distribution of sidewalk navigation behaviors; (2) Human-in-the-loop preference learning scheme to tune the policy on small amount of human intervention data, strengthening counterfactual reasoning and social compliance.

Result: In simulation, FlowPilot achieves 42% success rate and 66% route completion. With human preference tuning (FlowPilot-HP), real-world robustness and social compliance improve significantly, reducing interaction rate (IR) by 40.0% and non-interaction rate (NIR) by 52.1% relative to the base model.

Conclusion: FlowPilot demonstrates robust and efficient long-horizon navigation performance on sidewalks using only a monocular RGB camera, successfully bridging the gap between imitation learning and alignment through flow matching and preference learning approaches.
...
### [13] [EgoEngine: From Egocentric Human Videos to High-Fidelity Dexterous Robot Demonstrations](https://arxiv.org/abs/2606.12604)
...
### [14] [DARRMS -- An Efficient Algorithm for Dynamic Attention Radius in Resource-Constrained Multi-Agent Systems](https://arxiv.org/abs/2606.12614)
*Benjamin Alcorn,Eman Hammad*

Main category: cs.RO

TL;DR: This paper introduces an adaptive observation algorithm for multi-agent systems that limits each agent's observability to a configurable attention radius, reducing computational demand while maintaining performance through joint optimization of attention radius and decision-making.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems face computational resource constraints in robotics, cybersecurity, and autonomous vehicle domains. Traditional decision-making frameworks assume ideal conditions (full observability, unlimited computation) that don't align with real-world challenges, creating a gap between theoretical approaches and practical implementations.

Method: The proposed algorithm implements an attention mechanism where agents limit their observability to a configurable attention radius, intentionally ignoring parts of the environment deemed unnecessary for action planning. The method jointly optimizes both the attention radius parameters and the decision-making process to balance computational efficiency with information quality.

Result: Through theoretical analysis and empirical validation, the authors demonstrate that adaptive observation enhances coordination and scalability in uncertain environments. The approach maintains robust decision-making strategies while significantly reducing computational demands.

Conclusion: Adaptive observation effectively reduces computational resource requirements in multi-agent systems without substantial performance penalties. The joint optimization of attention radius and decision-making enables scalable, robust coordination in resource-constrained settings, making it suitable for practical applications in robotics, cybersecurity, and autonomous vehicle planning.
...
### [15] [EWAM: An Enhanced World Action Model for Closed-Loop Online Adaptation in Embodied Intelligence](https://arxiv.org/abs/2606.12690)
*Xin Zhou,Cong Miao*

Main category: cs.RO

TL;DR: EWAM introduces a closed-loop online adaptation architecture that adds four lightweight neural layers to a frozen Cosmos3 backbone, enabling zero-shot adaptation to new task layouts through inference-time co-reasoning with memory, anomaly detection, policy routing, and action correction mechanisms.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of reducing deployment data requirements when adapting to new task layouts. Most existing methods require fine-tuning the backbone network or collecting task-specific demonstration sets, which is impractical for many real-world scenarios. EWAM aims to enable adaptation without any additional training or demonstration data.

Method: EWAM builds upon a pretrained, fully frozen Cosmos3 backbone network and adds four lightweight neural layers at inference time:
1. Neural Experience Memory Layer (in DiT intermediate layers) - provides task-relevant execution context
2. Neural Anomaly Detection Layer (after state prediction head) - monitors divergence between predicted and actual states
3. Neural Policy Routing Layer - dynamically selects execution strategy based on anomaly severity (direct execution, conservative replanning, or rollback recovery)
4. Neural Action Correction Layer - refines action chunks using execution diagnostics

The memory, anomaly detection, and correction modules are deeply integrated into the forward path in a differentiable manner, while only the routing decision is discrete and supervised.

Result: The paper demonstrates that EWAM achieves performance gains through its inference-time co-reasoning mechanism, enabling effective adaptation to new task layouts without any fine-tuning or additional demonstration data. The modular design allows flexible deployment across different scenarios.

Conclusion: EWAM successfully demonstrates that effective adaptation to new task layouts can be achieved through well-designed inference-time mechanisms rather than fine-tuning the backbone network. The four-layer co-reasoning architecture provides a scalable approach for reducing deployment data requirements in practical applications, with the modular design allowing easy integration into existing systems.
...
### [16] [EquiDexFlow: Contact-Grounded SE(3)-Equivariant Dexterous Grasp Generative Flows](https://arxiv.org/abs/2606.12728)
*Clinton Enwerem,John S. Baras,Calin Belta*

Main category: cs.RO

TL;DR: EquiDexFlow is an SE(3)-equivariant flow-matching model that jointly predicts wrist pose, joint angles, fingertip contacts, surface normals, and contact forces from object point clouds, ensuring physically stable grasps by projecting contacts onto the object surface and forces into the Coulomb friction cone by construction.


<details>
  <summary>Details</summary>
Motivation: Most learned dexterous grasp generators only perform kinematic feasibility checks in downstream verification steps, allowing kinematically-plausible poses to violate conditions for stable physical grasps (e.g., contact force and friction constraints).

Method: EquiDexFlow employs an SE(3)-equivariant flow-matching architecture that simultaneously predicts wrist pose, joint angles, fingertip contacts, surface normals, and contact forces from object point clouds. The model projects contacts onto the object surface and forces into the Coulomb friction cone by construction, ensuring placement and friction compliance without loss penalties. The authors prove end-to-end SE(3) equivariance and verify it empirically.

Result: Trained on 8,100 force-closure grasps across 81 objects for a 16-DoF Allegro Hand, EquiDexFlow achieves zero friction violations, the best composite score, and the lowest wrench residual among all ablation variants. Hardware-feasible refinement places every joint at least 5% inside its actuator envelope. On physical robots, retargeted grasps successfully complete open-loop pick-and-hold trials on all six test objects, including asymmetric objects at both the canonical pose and 120° co-rotation.

Conclusion: EquiDexFlow successfully bridges the gap between kinematically-plausible grasps and physically stable grasps by incorporating contact force constraints directly into the generation process. The method demonstrates strong theoretical properties (SE(3) equivariance), excellent empirical performance, and practical success on physical robot hardware for real-world pick-and-hold tasks.
...
### [17] [Sparse2Act: Learning Action-Aligned Sparse 3D Representations for Cross-Domain Robot Manipulation](https://arxiv.org/abs/2606.12759)
*Yu Guo,Chang Yu,Siyu Ma,Yunuo Chen,Yin Yang,Ying Nian Wu,Chenfanfu Jiang*

Main category: cs.RO

TL;DR: Sparse2Act is an observation-action alignment framework for pretraining sparse point-cloud encoders, using task-space end-effector actions as geometric supervision to learn reusable 3D representations that support cross-domain transfer and sim-to-real deployment.


<details>
  <summary>Details</summary>
Motivation: Explicit 3D representations expose object shape, workspace geometry, and robot-object relations in metric coordinates, which are valuable for manipulation tasks. However, sparse 3D encoders are typically learned through downstream task objectives, tying the representation to specific data distributions, policy architectures, and action parameterizations, limiting their reusability.

Method: The authors introduce Sparse2Act, which treats task-space end-effector actions as geometric supervision. Masked sparse 3D tokens are trained to organize scene features around the workspace motion paired with the observation. The pretraining approach allows downstream policies to retain their own architectures and action spaces, including joint-space commands.

Result: On the LIBERO-10 benchmark, the method achieves 86.9% average success after 500 fine-tuning steps. Cross-domain transfer from LIBERO to Meta-World-5 achieves 73.4% average success. Real-world experiments show 72.5% average success across four tasks after simulation pretraining and limited real-data fine-tuning.

Conclusion: Robot actions can provide compact geometric supervision for reusable sparse 3D representations, enabling effective pretraining, cross-domain transfer, and sim-to-real deployment. The gains stem from the masked action-alignment signal and remain beneficial across different downstream action decoders.
...
### [18] [Stubborn: A Streamlined and Unified Reinforcement Learning Framework for Robust Motion Tracking and Fall Recovery for Humanoids](https://arxiv.org/abs/2606.12814)
*Xiao Ren,Yuhui Yang,Zongbiao Weng,Zhijie Liu,He Kong*

Main category: cs.RO

TL;DR: Stubborn is a unified reinforcement learning framework that integrates humanoid motion tracking and fall recovery via an asymmetric Actor-Critic architecture, using a yaw-aligned tracking representation, Bernoulli-based probabilistic termination mechanism, and tracking-error-driven adaptive sampling strategy to achieve robust performance.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods treat motion tracking and fall recovery as separate tasks requiring multi-stage training, and terminate episodes immediately after tracking failures, limiting recovery exploration in unstable states.

Method: Stubborn employs an asymmetric Actor-Critic architecture with a yaw-aligned tracking representation to reduce global drift sensitivity, a Bernoulli-based probabilistic termination mechanism to encourage fall-recovery exploration, and a tracking-error-driven adaptive sampling strategy to reshape the distribution for difficult segments.

Result: Extensive comparisons with SOTA methods and ablation studies show Stubborn achieves competitive performance, with the probabilistic termination and adaptive sampling contributing significantly to robustness gains.

Conclusion: Stubborn demonstrates that a unified RL framework with asymmetric architecture and adaptive sampling can effectively integrate motion tracking and fall recovery, improving both performance and robustness.
...
### [19] [AIR-VLA+: Decoupling Movement and Manipulation via Cascaded Dual-Action Decoders with Asymmetric MoE for Aerial Robots](https://arxiv.org/abs/2606.12859)
*Jianli Sun,Bin Tian,Qiyao Zhang,Zijian Liu,Yutong Wang,Zhiyong Cui,Bai Li,Yisheng Lv,Yonglin Tian*

Main category: cs.RO

TL;DR: AIR-VLA+ is a flow matching action generation architecture for aerial manipulation that uses cascaded dual-action decoders and asymmetric feature-level Mixture of Experts (MoE) to decouple UAV movement and arm manipulation, achieving 48.0 overall average score on AIR-VLA benchmark with 80.2% improvement over baseline policies.


<details>
  <summary>Details</summary>
Motivation: Aerial manipulation systems face representation coupling issues where UAV movement and arm manipulation have substantially different action scales, dynamics, and control objectives, leading to coordinated control conflicts.

Method: AIR-VLA+ proposes cascaded dual-action decoders (manipulation and movement) with asymmetric feature-level Mixture of Experts (MoE). The UAV movement decoder includes an input feature enhancement module with an implicit visual grasp projector to perceive gripper-object interaction and inject compressed global semantic features. An implicit MoE architecture enables different movement experts to adapt to various task stages through dense soft blending on the feature manifold.

Result: Experiments on the standardized AIR-VLA benchmark demonstrate comprehensive superiority over all baselines with an overall average score of 48.0, achieving 80.2% improvement in overall task completion score compared to the single-head π₀.₅ policy.

Conclusion: AIR-VLA+ effectively mitigates heterogeneous coordinated control conflicts of composite robots by decoupling UAV movement and arm manipulation, demonstrating strong task-stage adaptability and significantly improved performance in aerial manipulation tasks.
...
### [20] [Learning to Adapt: Representation-Based Reinforcement Learning for Multi-Task Skill Transfer](https://arxiv.org/abs/2606.12890)
*Aryan Naveen,Haitong Ma,Haldun Balim,Na Li*

Main category: cs.RO

TL;DR: RepMT-SAC is a multi-task reinforcement learning framework using spectral MDP decomposition to enable efficient knowledge sharing and robust transfer across tasks, achieving strong zero-shot and few-shot performance.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning has achieved remarkable success in complex control policies but remains limited by sample inefficiency and poor generalization across tasks.

Method: RepMT-SAC uses spectral MDP decomposition to capture transferable dynamics, structuring the value function into a task-agnostic core with minimal task-specific adjustment.

Result: Evaluated on quadcopter trajectory-following tasks across in-distribution and out-of-distribution contexts, RepMT-SAC outperforms baselines by up to 30%.

Conclusion: RepMT-SAC enables efficient knowledge sharing and robust transfer to new tasks, demonstrating strong zero-shot performance on in-distribution tasks and rapid few-shot adaptation to out-of-distribution tasks.
...
### [21] [Bounding Boxes as Goals: Language-Conditioned Grasping via Neuro-Symbolic Planning](https://arxiv.org/abs/2606.12910)
*Allison Andreyev,Landon Eum,Nestor Tiglao,Romel Gomez*

Main category: cs.RO

TL;DR: GRASP is a neuro-symbolic framework for open-vocabulary tabletop manipulation that translates natural-language queries into grounded goal states using pretrained vision-language models and bounding-box detection, achieving 73.3% success across 90 real-robot trials without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Current robotics systems require significant computational resources or extensive training on thousands of demonstrations to interpret natural language. The field needs more efficient approaches for real-time adaptation to natural-language prompts in household and industrial environments.

Method: GRASP leverages a pretrained Vision-Language Model (VLM) to convert natural-language queries into neuro-symbolic goal states, which are grounded in the physical world through a bounding-box detection pipeline. Unlike methods using fixed color lists or hard-coded coordinates, GRASP interprets abstract spatial concepts (e.g., "top shelf") and executes tasks without task-specific fine-tuning.

Result: 73.3% overall success rate across 90 real-robot trials at three difficulty levels. The framework demonstrated effective open-vocabulary manipulation capabilities without requiring task-specific training.

Conclusion: GRASP represents a significant step toward practical open-vocabulary tabletop manipulation, achieving strong real-world performance while eliminating the need for extensive task-specific training and reducing computational overhead compared to state-of-the-art approaches.
...
### [22] [An Embodied Simulation Platform, Benchmark, and Data-Efficient Augmentation Framework for Wet-Lab Robotics](https://arxiv.org/abs/2606.12936)
...
### [23] [Towards Reliable Sequential Object Picking in Clutter: The Runner-up Solution to RGMC 2025](https://arxiv.org/abs/2606.12954)
*Wei Yu,Xidan Zhang,Ziyi Zheng,Weijie Kong,Huixu Dong*

Main category: cs.RO

TL;DR: This paper presents a solution for sequential object picking in cluttered environments using an integrated hardware-software pipeline combining object recognition, decluttering, and multi-modal grasping, achieving second place in the 2025 RGMC competition.


<details>
  <summary>Details</summary>
Motivation: Sequential object search and sorting in cluttered environments is a challenging task with few mature solutions. The task requires robust grasping across diverse objects (rigid and deformable) and efficient search strategies with high success rates.

Method: An integrated hardware-software pipeline that combines object recognition, decluttering, and multi-modal grasping. Key contributions include a multifunctional gripper design and novel representations for object distribution and occlusion relationships in cluttered space.

Result: The solution achieved second place in the Pick-in-Clutter track of the 10th Robotic Grasping and Manipulation Competition (RGMC) at ICRA 2025, demonstrating strong performance in both laboratory tests and competition scenarios.

Conclusion: The proposed pipeline enables efficient recognition, search, and sequential grasping of objects in clutter, addressing key challenges of robust collision-aware grasping and efficient search strategies through hardware-software integration.
...
### [24] [SERF: Spatiotemporal Environment and Robot Feature Map for Long-Horizon Mobile Manipulation](https://arxiv.org/abs/2606.12956)
*Sunghwan Kim,Byeonghyun Pak,Kehan Long,Yulun Tian,Nikolay Atanasov*

Main category: cs.RO

TL;DR: A spatiotemporal feature map (SERF) that represents the environment and robot body as neural points in a shared latent space, conditioning vision-language-action policies to improve long-horizon mobile manipulation reasoning.


<details>
  <summary>Details</summary>
Motivation: Long-horizon robot mobile manipulation requires reasoning about localization, environment changes, and task progress, which is challenging to infer from image observations alone.

Method: Create a spatiotemporal feature map (SERF) representing the environment and articulated robot body as neural points in a shared latent space, updated online from egocentric observations and proprioceptive state. Environment neural points are updated via object-level rigid tracking, while robot neural points use forward kinematics. The map is extracted as tokens from multiple reference frames and spatial scales to provide local and global context to a vision-language-action policy.

Result: On the BEHAVIOR-1K benchmark, the SERF VLA policy outperforms image-only baselines, reaches subgoals faster by following more direct trajectories, improves robustness to scene-configuration shifts, and recovers from object-drop failures.

Conclusion: Conditioning mobile manipulation policies on spatiotemporal feature maps significantly improves reasoning over long horizons by providing both local and global context.
...
### [25] [EmbodiSteer: Steering Embodiment-Agnostic Visuomotor Policies with Joint-Space Guidance for Zero-Shot Cross-Embodiment Deployment](https://arxiv.org/abs/2606.12965)
*Shihefeng Wang,Kangchen Lv,Mingrui Yu,Xiang Li*

Main category: cs.RO

TL;DR: EmbodiSteer is a training-free framework that enhances embodiment-agnostic visuomotor policies by steering them toward embodiment-aware deployment through forward kinematics and collision-aware guidance in joint space during inference-time diffusion sampling.


<details>
  <summary>Details</summary>
Motivation: Scalable robot imitation learning relies on Cartesian end-effector actions, but this abstraction leaves policies unaware of robot-specific constraints like whole-body collision avoidance, making them brittle under robot-specific limitations.

Method: EmbodiSteer keeps policy learning in Cartesian space while lifting inference-time diffusion sampling into the target robot's joint space via forward kinematics and Jacobian-based updates. It applies whole-body collision-aware guidance over joint trajectories after each denoising step to steer the arm away from collisions while preserving learned end-effector behavior.

Result: Compared with Cartesian-only execution, EmbodiSteer reduces collision rate by 46.1% and improves task success rate by 28.5% across 9 simulated robots, and achieves 90.0% collision rate reduction and 36.7% success rate increase on two physical robots in highly constrained scenarios.

Conclusion: EmbodiSteer successfully bridges the gap between Cartesian policy learning and embodiment-aware deployment, enabling zero-shot adaptation to target robots with significantly improved safety and task performance.
...
### [26] [Trajectory-Level Redirection Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2606.12978)
*Gokul Puthumanaillam,Vardhan Dongre,Pranay Thangeda,Hooshang Nayyeri,Dilek Hakkani-Tür,Melkior Ornik*

Main category: cs.RO

TL;DR: This paper identifies a trajectory-level vulnerability in vision-language-action (VLA) policies where prompt-only attacks can redirect robot actions to attacker-specified targets while preserving the appearance of the original command.


<details>
  <summary>Details</summary>
Motivation: VLA policies enable robots to execute manipulation tasks from text instructions, but existing attacks focus only on low-level actions or persistence across frames. This work reveals a stronger failure mode where prompts can redirect the final physical outcome while still appearing to specify the intended task.

Method: The authors formalize "command-preserving trajectory redirection" as a prompt-only threat model and introduce an on-policy prompt search method using rollouts to discover perturbations that satisfy command-preserving constraints (staying close to benign instructions while omitting target words) while tracking target tasks in closed-loop behavior.

Result: Near-benign prompt perturbations can successfully redirect VLA rollouts to attacker-specified targets in both simulation and hardware experiments.

Conclusion: This exposes a critical trajectory-level vulnerability in VLA instruction grounding: text that appears to preserve the intended command can still give adversaries control over the robot's final physical outcome, suggesting a need for robustness guarantees against prompt-only trajectory redirection attacks.
...
### [27] [GenHOI: Contact-Aware Humanoid-Object Interaction by Imitating Generated Videos without Task-Specific Training](https://arxiv.org/abs/2606.12995)
...
TL;DR: GenHOI is a zero-shot framework that enables humanoid robots to perform diverse object-interaction tasks by imitating a single generated video, without requiring task-specific training or physical demonstration data.


<details>
  <summary>Details</summary>
Motivation: Humanoid-Object Interaction (HOI) is challenging due to the tight coupling between dynamic balance and stable interaction with diverse objects. Existing methods require time-consuming task-specific policy training or rely on rigid trajectory replay, which limits their ability to accommodate novel interaction scenarios.

Method: GenHOI first reconstructs the robot-object scene in simulation and renders a first-frame image conditioned on the language command to synthesize a task-oriented interaction video. The generated video is analyzed to identify interaction-relevant contact events and estimate hand-object contact regions, which are encoded as object-centric geometric constraints. These constraints convert visual interaction cues into physically grounded optimization priors that refine and smooth the reference motion recovered from the video, adapt a single reference trajectory to unseen robot-object relative poses, and are executed by a closed-loop tracking controller.

Result: Extensive simulation and real-world experiments across diverse object-interaction tasks, including box grasping, asymmetric bimanual chair carrying, table lifting from below, and cylindrical-object enveloping.

Conclusion: GenHOI enables humanoid robots to perform diverse object-interaction tasks in a zero-shot manner without task-specific training, effectively handling novel interaction scenarios by converting visual interaction cues into physically grounded optimization priors through object-centric geometric constraints derived from a single generated video.
...
### [28] [Comparing Commercial Depth Sensor Accuracy for Medical Applications](https://arxiv.org/abs/2606.13028)
...
### [29] [RoboProcessBench: Benchmarking Process-Aware Understanding in Vision-Language Robotic Manipulation](https://arxiv.org/abs/2606.13040)
...
### [30] [Y-BotFrame: An Extensible Embodied Agent Framework for Quadruped Robot Assistants](https://arxiv.org/abs/2606.13049)
*Luyao Zhang,Ke Li,Yuan Ding,Xulong Zhao,Guo Yu,Chengwei Yan,Fuyu Dong,Jiawei Hu,Di Wang,Nan Luo,Gang Liu,Quan Wang*

Main category: cs.RO

TL;DR: Y-BotFrame is an extensible embodied platform for quadruped robots that integrates multimodal perception (speech, vision, LiDAR) with a large language model as the cognitive core, enabling natural language interaction and task execution for intelligent ground assistance.


<details>
  <summary>Details</summary>
Motivation: Quadruped robots need to serve as real-world deployment platforms for algorithms with high mobility and flexibility across complex terrains. The paper aims to create an extensible embodied platform that transforms a robot into an intelligent ground assistant through multimodal perception and cognitive capabilities.

Method: Y-BotFrame employs a large language model as the cognitive core for environmental understanding, contextual reasoning, and task planning. The system integrates multimodal perception capabilities (speech, vision, LiDAR) and maps natural-language user instructions into executable embodied task units. It supports plug-and-play integration of functional modules.

Result: The system successfully enables natural interaction through voice commands and visual feedback, removing the need for remote controllers and facilitating efficient human-robot collaboration. The highly extensible framework supports modular upgrades and iterative development.

Conclusion: Y-BotFrame serves as a reference implementation for real-world deployment of general-purpose, instruction-driven embodied agents, offering a robust platform that can be continuously upgraded with new functional modules.
...
### [31] [EA-WM: Event-Aware World Models with Task-Specification Grounding for Long-Horizon Manipulation](https://arxiv.org/abs/2606.13053)
*Kailin Wang,Haoxiang Jie,Yaoyuan Yan,Jiacheng Zhou,Zhiyou Heng*

Main category: cs.RO

TL;DR: EA-WM is an event-aware world model framework that augments pretrained visual-feature dynamics with task-grounded event prediction and verification, enabling better alignment with task progress in long-horizon manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Pretrained visual-feature world models are useful for robot imagination, but visual/latent prediction alone cannot determine whether imagined futures satisfy task-relevant events. Long-horizon manipulation requires relational, predicate-level, and physically grounded progress signals (e.g., object movement, contact state changes, placement predicates, reliability) that are missing from pure feature predictions.

Method: EA-WM adds task-specification-grounded event prediction and verification to frozen visual-feature dynamics. It rolls out candidate futures in feature space, decodes them into structured event states, and scores candidates using task-progress, semantic-consistency, physical-feasibility, and uncertainty terms. The verifier guides sampling-based planning, gates candidate actions, and selects proposals in settings like LIBERO wine-rack.
...
Conclusion: Event-aware verification can enhance the utility of feature-space world models for long-horizon manipulation by ensuring that imagined futures are task-relevant, physically feasible, and semantically consistent.
...
### [32] [FTP-1: A Generalist Foundation Tactile Policy Across Tactile Sensors for Contact-Rich Manipulation](https://arxiv.org/abs/2606.13102)
...
TL;DR: FTP-1 is the first generalist foundation tactile policy that can transfer tactile manipulation abilities across diverse sensors and embodiments by using heterogeneous encoders to unify different tactile signals into shared latent tokens modeled by a tactile Transformer.


<details>
  <summary>Details</summary>
Motivation: Existing tactile-based policies are tied to fixed embodiments and sensor setups because tactile signals are highly heterogeneous across hardware, making cross-sensor generalization difficult.

Method: FTP-1 uses heterogeneous encoders to project varied tactile inputs (image-, array-, and state-based signals) into unified morphology-aware latent tokens, which are jointly modeled by a shared tactile Transformer expert. It is pretrained on ~3,000 hours of tactile manipulation data from 26 data sources across 21 sensors (human and robot demonstrations).

Result: FTP-1 improves contact-rich manipulation on seen sensor setups by +17.2% and successfully transfers to two previously unseen tactile-sensor setups with a +31% success rate gain across 5 hardware configurations.

Conclusion: FTP-1 establishes the first unified foundation baseline for tactile manipulation, providing future tactile policies with a shared model-level starting point for cross-sensor transferability.
...
### [33] [Redesigning Regularization for Effective Policy Smoothing](https://arxiv.org/abs/2606.13169)
*Taisuke Kobayashi,Naoto Yamanaka*

Main category: cs.RO

TL;DR: A novel regularization design for reinforcement learning that addresses implementation limitations to achieve effective policy smoothing, validated on multiple tasks and a quadruped robot sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Existing regularization for smoothing policy functions in RL is limited to local Lipschitz continuity due to tradeoffs between smoothness and expressiveness; the original implementation is cumbersome and insufficiently smooth, creating a theory-implementation gap.

Method: Identifies three reasons why the original implementation fails and provides targeted remedies; modifies the regularization approach to better align theory with practice.

Result: The modified regularization performs well across multiple tasks and algorithms, achieving smooth motion while improving control performance; applied to sim-to-real RL for a quadruped robot, demonstrating robustness against sudden velocity command changes.

Conclusion: A corrected regularization design effectively balances smoothness and expressiveness, closing the theory-implementation gap and providing practical benefits for both simulation and real-world robotic control.
...
### [34] [Multi-Modal Multi-Agent Robotic Cognitive Alignment enabled by Non-Invasive Consumer Brain Computer Interfaces: A Proof of Concept Exploration](https://arxiv.org/abs/2606.13190)
*Nataliya Kosmyna,Liz Jenkins,Anoop K. Sinha*

Main category: cs.RO

TL;DR: This paper introduces a cognitively aligned multi-agent interaction framework that uses a Brain-Computer Interface (BCI) to monitor human engagement levels and defer agent communications during high cognitive load, enabling seamless background processing of secondary tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems often interrupt humans at inopportune times, causing cognitive overload and decreased task performance. The authors argue that overlooking the human's internal cognitive state in human-robot interaction is a critical limitation that hinders natural and effective communication.

Method: A closed-loop architecture that integrates a consumer-grade BCI to continuously monitor EEG spectral band powers during engagement-inducing tasks. An HTTP-based signaling mechanism places a primary agent's sensory inputs and audio outputs into a holding state when high engagement is detected, allowing secondary agents to process delegated tasks in the background. The system releases queued messages once the human's cognitive state returns to baseline.

Result: Preliminary results demonstrate the feasibility of combining real-time signal processing, Large Language Models (LLMs), and physical robotic embodiments to create cognitively-aware, non-intrusive multi-agent systems that can dynamically adapt to human cognitive states.

Conclusion: The proposed framework enables robotic systems to contextually defer communications during high mental workload, reducing cognitive overload while maintaining system responsiveness. This approach represents a significant step toward creating more natural and effective human-robot interactions through cognitive alignment.
...
### [35] [Embedding ISO 10218 Safety Compliance in Robots via Control Barrier Functions for Human-Robot Collaboration](https://arxiv.org/abs/2606.13203)
*Federico Parma,Cesare Tonola,Nicola Pedrocchi,Manuel Beschi*

Main category: cs.RO

TL;DR: A Control Barrier Function (CBF) with human acceleration data prediction integrated into Sequential Quadratic Programming (SQP) is proposed for safer Human-Robot Collaboration, achieving 63% trajectory error reduction compared to standard SSM filters while maintaining ISO 10218 compliance.


<details>
  <summary>Details</summary>
Motivation: Standard Speed and Separation Monitoring (SSM) filters use conservative assumptions (constant human velocity) that prevent accurate minimum separation predictions, causing unnecessary operational halts in Human-Robot Collaboration systems requiring strict adherence to ISO 10218 safety standards.

Method: A Control Barrier Function (CBF) incorporating human acceleration data analytically forward-predicts minimum human-robot separation distances during worst-case robotic stopping trajectories, integrated as an inequality constraint within a Sequential Quadratic Programming (SQP) framework. Two methods are proposed: Method I (CBF-constrained PD safety filter) and Method II (task-scaling SQP controller with spatial tube constraint).

Result: Experiments on a UR10e robot compared against standard industrial SSM baseline show Method II dynamically modulates execution speed and confines spatial deviations, achieving 63% reduction in mean trajectory error compared to Method I while avoiding excessive evasive manoeuvres.

Conclusion: The proposed CBF-SQP approach outperforms standard SSM by providing accurate separation predictions that enable high task throughput while maintaining strict ISO 10218 safety compliance, with Method II demonstrating superior performance in trajectory accuracy and spatial constraint enforcement.
...
### [36] [Proprioceptive-visual correspondence enables self-other distinction in humanoid robots](https://arxiv.org/abs/2606.13222)
*Yurun Chen,Tianyuan Gao,Yizhong Ge,Shikun Ban,Yizhou Wang,Hongkai Xiong,Wenjun Zeng,Wentao Zhu*

Main category: cs.RO

TL;DR: A humanoid robot learns to distinguish itself from others using proprioceptive-visual correspondence, establishing a predictive self-model that maps joint configurations to 3D body occupancy for downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Humanoid robots working alongside humans lack the ability to distinguish self from others, which is essential for social intelligence and effective collaboration in shared physical environments.

Method: The robot learns self-other distinction through proprioceptive-visual correspondence without any identity labels or kinematic models. Once established, this distinction bootstraps a predictive self-model mapping joint configurations to 3D body occupancy.

Result: The system reliably identifies itself in multi-agent scenes with humans or morphologically identical robots, learns accurate 3D self-models, and successfully supports downstream tasks including target reaching, collision-aware motion planning, and human-to-robot motion retargeting.

Conclusion: This work outlines a viable route toward bodily self-representation in robots that act and coordinate alongside others, bridging the gap between current robots and the social intelligence required for collaborative workspaces.
...
### [37] [WT-UMI: Tactile-based Whole-Body Manipulation via Force-Supervised Contact-Aware Planning](https://arxiv.org/abs/2606.13232)
...
### [38] [Humor Style Drives Laughter, Topic Shapes Acceptability: Evaluating Bilingual Personal and Political Robot-Delivered AI Jokes](https://arxiv.org/abs/2606.13256)
*Anna-Maria Velentza,Anne-Gwenn Bosser*

Main category: cs.RO

TL;DR: This exploratory study investigates how humor type (Affiliative, Self-Enhancing, Aggressive, Self-Defeating) and joke content (person-related vs. political) influence perceptions of robot-delivered humor in group settings, examining effects on funniness, appropriateness, and language preference.


<details>
  <summary>Details</summary>
Motivation: Humor plays a central role in human social relationships and human-robot interaction (HRI). While large language models can generate diverse forms of humor, it remains unclear how humor style, joke content, and language preference shape perceptions of robot-delivered humor in group settings.

Method: Mixed factorial design study where university participants evaluated AI-generated jokes delivered by a robot in a classroom setting. Participants assessed humor type (Affiliative, Self-Enhancing, Aggressive, Self-Defeating) × joke content (person-related vs. political) across measures of perceived funniness, appropriateness, and preferred language.

Result: Humor type significantly influences funniness ratings, with Aggressive and Affiliative humor rated higher than other types. Joke content primarily affects appropriateness, with person-related jokes preferred over political ones. Language preference was shaped by both joke content and participants' self-reported fluency and humor practices.

Conclusion: Humor type and content have distinct, measurable effects on how humans perceive and respond to robot-delivered humor in group settings, with practical implications for designing effective robot humor systems in HRI applications.
...
### [39] [See Selectively, Act Adaptively: Dual-Level Structural Decomposition for Bimanual Robot Manipulation](https://arxiv.org/abs/2606.13279)
*Yoon-Ji Choi,Young-Chae Son,Soo-Chul Lim*

Main category: cs.RO

TL;DR: A bimanual manipulation VLA framework based on Dual-Level Structural Decomposition that dynamically adjusts visual processing via a View-Selective Visual Router and decomposes action generation into coordinated/arm-wise pathways using Interaction-Aware Action MoE, improving success rates by 27.7% in simulation and 43.3% in real-world tasks over monolithic baselines.


<details>
  <summary>Details</summary>
Motivation: In bimanual robotic manipulation, task-relevant visual information varies with task stage and context, while arm interactions shift between independent and coordinated modes, making policy learning challenging. Existing monolithic VLA policies fail to separately account for visual relevance and bimanual interaction structure.

Method: Proposes a bimanual manipulation VLA framework with Dual-Level Structural Decomposition: (1) View-Selective Visual Router dynamically adjusts wrist-view contributions to emphasize relevant visual cues, and (2) Interaction-Aware Action Mixture-of-Experts (MoE) decomposes action generation into coordinated and arm-wise pathways to adapt to varying bimanual interaction modes.

Result: The model improves overall average success rate over monolithic baseline by 27.7% in simulation (RoboTwin 2.0, 6 tasks) and 43.3% in real-world evaluation (3 long-horizon tasks), consistently outperforming single-module variants across both settings.
...
### [40] [EMG-Based Adaptation of Anisotropic Virtual Fixtures for Robot-Assisted Surgical Resection and Dissection](https://arxiv.org/abs/2606.13340)
*Dario Onfiani,Michael Dyck,Luigi Biagiotti,Julian Klodmann*

Main category: cs.RO

TL;DR: This paper proposes an adaptive and anisotropic virtual fixture system for robot-assisted laparoscopic surgery, using EMG signals to dynamically adjust constraints based on surgeon intent, enabling seamless transitions between guided motion and free repositioning.


<details>
  <summary>Details</summary>
Motivation: Conventional virtual fixtures have fixed geometries that lack flexibility to adapt to surgical workflows or surgeons' immediate intent, limiting their effectiveness for delicate tasks such as resection and dissection.

Method: The authors developed a novel adaptive and anisotropic virtual fixture framework with an intuitive EMG-based control interface. The system modulates fixture geometry in real-time based on surgeon intent inferred from forearm muscle contractions, allowing dynamic expansion or disengagement of constraints.

Result: A pilot user study on a standardized surgical training task demonstrated significant improvements in task accuracy and movement consistency, along with reductions in perceived cognitive load, effort, and frustration.

Conclusion: The proposed adaptive virtual fixture system with EMG-based control interface effectively enhances robot-assisted laparoscopic surgery by enabling seamless transitions between precise guided motion and free tool repositioning.
...
### [41] [Low cost, easily manufactured, highly flexible strain and touch sensitive fiber for robotics applications](https://arxiv.org/abs/2606.13352)
...
### [42] [Real-Time Execution with Autoregressive Policies](https://arxiv.org/abs/2606.13355)
*Sangkyu Lee,Seohyeon Park,Tackgeun You,Avi Caciularu,Idan Szpektor,Hwasup Lim,Youngjae Yu*

Main category: cs.RO

TL;DR: This paper demonstrates that autoregressive Vision-Language-Action models can achieve real-time execution by adjusting tokenization horizons and applying constrained decoding, outperforming flow-matching policies while maintaining strict latency bounds.


<details>
  <summary>Details</summary>
Motivation: Real-time execution is critical for large-scale Vision-Language-Action models. Recent work focused primarily on diffusion policies for real-time execution, but autoregressive policies have slower rollout speeds in synchronous inference and require attention for real-time performance.

Method: The researchers adjust the tokenization horizon and apply constrained decoding to autoregressive policies to achieve real-time execution with strict latency bounds, enabling multi-trajectory decoding to maximize performance.

Result: Autoregressive policies consistently outperform equivalent-level flow-matching policies across both simulated and real-world environments, achieving significantly improved task completion speeds from synchronous inference.

Conclusion: Coupled with inherent advantages of autoregressive policies such as faster convergence and better generalizability in instruction-following, these results confirm that autoregressive policies can remain competitive policy types supporting real-time execution.
...
### [43] [GeoHAT: Geometry-Adaptive Hybrid Action Transformer for Mobile Manipulation](https://arxiv.org/abs/2606.13394)
*Xiangyu Zhu,Renjun Wu,Luzhou Ge,Jinyan Liu,Xuesong Li*

Main category: cs.RO

TL;DR: GeoHAT is an end-to-end diffusion-based framework for whole-body mobile manipulation that selectively injects geometry into vision foundation models via a lightweight Fourier spatial encoder and gated fusion, while using a Hybrid Whole-Body Action Decoder with sparse cross-attention and causal temporal modeling to decompose arm and base control.


<details>
  <summary>Details</summary>
Motivation: Whole-body mobile manipulation faces challenges in geometric perception and action generation under shifting viewpoints. Current policies have limitations: reliance on 2D features or sparse 3D representations lacking dense spatial structure, single action vector encoding that ignores distinct control demands of arm and base, and dense fusion strategies that risk corrupting pretrained representations under noisy depth while incurring heavy computational overhead.

Method: GeoHAT employs a lightweight Fourier spatial encoder to map dense per-pixel 3D coordinates into geometric tokens without an additional 3D vision backbone. These tokens are selectively injected into vision foundation model features through per-token gated fusion modulated by depth validity. The Hybrid Whole-Body Action Decoder decomposes arm and base into distinct subspaces with sparse cross-attention for each action modality to attend to task-relevant visual context, while causal temporal modeling captures intra-timestep coordination and inter-timestep dependencies.

Result: On the ManiSkill-HAB simulation benchmark, GeoHAT achieves a 79.3% mean success rate, surpassing the strongest baseline by 23.7 percentage points. Real-world experiments on diverse tasks also demonstrate consistent improvements over all baselines.

Conclusion: GeoHAT successfully addresses the challenges of whole-body mobile manipulation through its selective geometry injection strategy that preserves semantic priors while enriching spatial understanding, and its hybrid action decoder that properly decomposes and coordinates arm and base control. The framework demonstrates significant performance improvements both in simulation and real-world settings.
...
### [44] [GIVE: Grounding Human Gestures in Vision-Language-Action Models](https://arxiv.org/abs/2606.13435)
*Pengfei Liu,Gen Li,Junqiao Fan,Boyu Ma,Jindou Jia,Yang Xiao,Jianfei Yang*

Main category: cs.RO

TL;DR: GIVE enhances Vision-Language-Action models with human gesture understanding through visual (hand skeleton overlay) and semantic (gesture description) pathways, improving target object recognition accuracy by 40% and task success rate by 80% in real-world HRI experiments.


<details>
  <summary>Details</summary>
Motivation: Current VLA models treat robotic manipulation as purely text-driven, overlooking the important role of gestures in conveying intentions. This leads to inaccurate intent grounding and unreliable manipulation when language instructions are ambiguous or underspecified.

Method: GIVE incorporates gesture information through two complementary pathways: (1) visual pathway that overlays hand skeletons and fingertip rays onto robot observations for explicit object grounding; (2) semantic pathway that generates high-level descriptions of human gestures and task instructions for robust intent grounding.

Result: In real-world HRI experiments, GIVE substantially outperforms baseline: improves target object recognition accuracy by 40% and overall task success rate by 80%. Demonstrates strong robustness and generalization to unseen spatial layouts and diverse participants.

Conclusion: By jointly leveraging visual and semantic guidance from gestures, GIVE enables VLA policies to better associate gestures with manipulation behaviors and adapt to dynamic interaction intents, bridging the gap between human communication and robotic manipulation.
...
### [45] [NavWAM: A Navigation World Action Model for Goal-Conditioned Visual Navigation](https://arxiv.org/abs/2606.13494)
...
TL;DR: NavWAM is a diffusion-transformer policy that converts navigation world-model predictions into executable robot actions by jointly learning future prediction, action, and value targets in a shared latent sequence, outperforming planning-based world-model baselines in both offline benchmarks and closed-loop real-robot navigation.


<details>
  <summary>Details</summary>
Motivation: Goal-conditioned visual navigation requires robots to act under partial observability by anticipating how their motion changes the future egocentric view and whether that change brings them closer to the goal. Navigation world models provide visual foresight but remain prediction modules that require an external planner to convert predicted futures into closed-loop control.

Method: NavWAM (Navigation World Action Model) is a diffusion-transformer policy that represents future observations, goal-progress values, and action chunks in a shared latent sequence. It learns future prediction jointly with action and value targets that determine closed-loop behavior, transforming navigation world-model prediction directly into executable action without requiring external planning.

Result: Across offline benchmarks and closed-loop real-robot deployment, NavWAM improves over planning-based world-model baselines in evaluations while using the default policy mode without CEM-style action search.

Conclusion: By learning future prediction directly with action and value targets, NavWAM demonstrates that visual foresight can be made directly usable for robot control, eliminating the need for external planners and action search mechanisms.
...
### [46] [SPARC: Reliable Spatial Annotations from Robot Demonstrations at Scale](https://arxiv.org/abs/2606.13497)
*Nils Blank,Paul Mattes,Maximilian Xiling Li,Jakub Suliga,Thomas Roth,Moritz Reuss,Pankhuri Vanjani,Rudolf Lioutikov*

Main category: cs.RO

TL;DR: SPARC is a risk-aware framework for automatically labeling robot demonstrations with structured spatial annotations and reliability scores, using spatio-temporal structure to reduce noisy labels while outperforming detection-only baselines in object localization accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing automated annotation pipelines generate spatial annotations at scale but lack reliable quality signals, as detector confidence is poorly calibrated. This forces a trade-off between accepting noisy labels or discarding useful samples.

Method: SPARC leverages the spatio-temporal structure inherent to robot tasks to generate a reliability signal for annotations. It introduces IA-Bench, a benchmark for measuring model accuracy in grounding interacted object locations in robot demonstrations.

Result: On 1.7k human-annotated demonstrations, SPARC significantly outperforms detection-only baselines in localization accuracy while retaining three times more samples at high-precision operating points. Models finetuned on SPARC annotations achieve state-of-the-art results on object-grounding and pointing benchmarks.

Conclusion: Policies trained on SPARC-generated annotations outperform baselines in cluttered, visually ambiguous real-world scenes, demonstrating that SPARC effectively reduces noisy labels while retaining useful samples for training robust robot policies.
...
### [47] [MCR-Bionic Hand: Anatomical Structural Priors for Dexterous Manipulation](https://arxiv.org/abs/2606.13601)
*Haosen Yang,Guowu Wei*

Main category: cs.RO

TL;DR: This paper introduces a biomimetic robotic hand (MCR-Bionic Hand) that encodes human hand dexterity into its physical architecture through two forms of structural intelligence: structural prior generation (wrist-to-finger tenodesis, tendon routing, and extensor hood mechanisms) and muscle-mediated modulation (extrinsic muscles, lumbricals, and interossei). The hand demonstrates functional capabilities in contact-rich tasks, showing that anatomical biomimetics can provide control advantages beyond visual similarity.


<details>
  <summary>Details</summary>
Motivation: Human hand dexterity is partly encoded in physical architecture (bones, ligaments, tendons, aponeuroses, intrinsic muscles) rather than just control algorithms. Existing robotic hands treat dexterity as high-dimensional active control systems, overlooking the value of anatomical structures in generating default grasp configurations and enabling fine modulation.

Method: The authors develop MCR-Bionic Hand as a 1:1 musculoskeletal biomimetic hand integrating: (1) two-row eight-bone wrist, cross-wrist tendons, anatomical flexor routing, volar plate and collateral ligament constraints, dorsal extensor hood, and intrinsic muscle pathways; (2) structural prior generation for wrist-to-finger tenodesis, FDS/FDP routing, and PIP-to-DIP coordination; (3) muscle-mediated modulation via extrinsic muscles, lumbricals, and interossei for MCP posture regulation, distal stability, and contact state control.

Result: Geometric mechanical models and functional demonstrations (coin rotation, pen transfer, dorsal coin flipping, cube manipulation) show that: (1) wrist posture induces multi-joint pre-shaping, (2) extensor hood maps PIP posture to coupled DIP response, (3) intrinsic muscle pathways modulate distal stability and action direction post-grasp formation, and (4) low-dimensional state generation links with fine post-contact modulation.

Conclusion: Anatomical biomimetics is valuable not for visual similarity but for identifying human hand structures that perform part of the control function. The physical architecture of the hand contributes to dexterity through structural priors and modulation capabilities, suggesting that robotic hands should incorporate such anatomical features for improved performance.
...
### [48] [$\texttt{WEAVER}$, Better, Faster, Longer: An Effective World Model for Robotic Manipulation](https://arxiv.org/abs/2606.13672)
*Arnav Kumar Jain,Yilin Wu,Jesse Farebrother,Gokul Swamy,Andrea Bajcsy*

Main category: cs.RO

TL;DR: WEAVER is a multi-view world model that simultaneously achieves fidelity, consistency, and efficiency for robotic manipulation by predicting future latents and rewards via flow-matching loss, outperforming prior methods in policy evaluation (ρ=0.87), policy improvement (38% gain), and test-time planning (14% gain with 5-10× speedup).


<details>
  <summary>Details</summary>
Motivation: World models (WMs) enable policy evaluation, policy improvement, and test-time planning with limited real-world interaction, but prior WMs struggle with long-horizon dynamic manipulation tasks. A WM must jointly satisfy three desiderata: fidelity (real-world correlation), consistency (long-horizon coherence), and efficiency (fast prediction).

Method: WEAVER (World Estimation Across Views for Embodied Reasoning) is a multi-view WM architecture trained to predict future latents and reward values using a flow-matching loss. Key design innovations include model architecture, memory management, and prediction objectives specifically designed for long-horizon tasks.

Result: State-of-the-art performance on robotic manipulation tasks. Policy evaluation achieved ρ=0.87 correlation with real-world success; policy improvement delivered 38% real-world success rate gain over π₀.₅ baseline; test-time planning achieved 14% success rate improvement with 5-10× speedup over prior WMs. Better performance on out-of-distribution scenarios as well.

Conclusion: WEAVER successfully unlocks all three critical desiderata (fidelity, consistency, efficiency) for world models, demonstrating substantial improvements across policy evaluation, policy improvement, and test-time planning in robotic hardware experiments, outperforming prior WMs on both in-distribution and out-of-distribution scenarios.
...
### [49] [Improving Robotic Generalist Policies via Flow Reversal Steering](https://arxiv.org/abs/2606.13675)
