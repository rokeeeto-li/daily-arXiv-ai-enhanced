<div id=toc></div>

# Table of Contents

- [cs.GR](#cs.GR) [Total: 1]
- [cs.MA](#cs.MA) [Total: 4]
- [cs.RO](#cs.RO) [Total: 66]


<div id='cs.GR'></div>

# cs.GR [[Back]](#toc)

### [1] [Self-supervised Garment Dynamics with Persistent Wrinkles](https://arxiv.org/abs/2606.25065)
*Xiaoyuan Yang,Deshan Gong,Taku Komura,He Wang*

Main category: cs.GR

TL;DR: The paper introduces the first self-supervised neural garment simulator that explicitly models persistent wrinkles by using a novel physics-inspired loss function and curriculum learning scheme that gradually transitions from pure elasticity to elasto-plasticity.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised neural garment simulators simplify the mechanical properties of fabrics, ignoring plasticity and persistent wrinkles, which reduces visual realism.

Method: A novel physics-inspired loss function that transforms learning into a moving energy minimization problem to mimic plasticity, combined with a curriculum learning scheme that gradually changes the target material from pure elasticity to elasto-plasticity.

Result: The proposed method can generate natural persistent wrinkles, outperforming existing methods on various garments, body shapes, and body motions according to multiple metrics.

Conclusion: This is the first self-supervised learning model capable of generating natural persistent wrinkles, significantly improving visual realism in garment simulation.

Abstract: The self-supervised neural garment simulator has become popular due to its high efficiency, good visual realism, and no reliance on training data. However, existing methods greatly simplify the mechanical properties of fabrics, ignoring persistent wrinkles caused by plasticity. Although this simplification allows for modeling of purely elastic material and simple training via energy minimization, the lack of believable wrinkles adversely affects the visual realism. Therefore, we introduce the first self-supervised neural garment simulator that explicitly models persistent wrinkles. This is achieved by a novel physics-inspired loss function, turning learning into a moving energy minimization problem to mimic plasticity. However, this requires learning to use a changing loss function, which causes difficulties in training i.e. the loss function changes during training. To this end, we propose a new physics-inspired curriculum learning scheme where the target material for learning gradually changes from pure elasticity to elasto-plasticity, allowing the loss function and the learnable parameters to jointly converge. Through a comprehensive evaluation, we show that for the first time, self-supervised learning models can generate natural persistent wrinkles, outperforming existing methods on a variety of garments, body shapes, and body motions, according to a range of metrics.

</details>


<div id='cs.MA'></div>

# cs.MA [[Back]](#toc)

### [2] [Bridging the Post-discharge Gap: A Traceable Multi-agent Framework for Safe and Continuous Care](https://arxiv.org/abs/2606.25334)
*Runwei Guan,Yi Zhou,Heyi Lin,Jinjing Zhu,Mingyuan Hou,Yang Yang,Fang Yuan,Xiaohong Lin,Shaofeng Liang,Xuming Hu,Tao Li,Tianbin Zhao,Yutao Yue,Zhiyuan Wang,Hui Xiong*

Main category: cs.MA

TL;DR: Healink is a memory-enhanced multi-agent framework for AI-assisted post-discharge follow-up that addresses healthcare workforce shortages and information silos by generating prescription-grounded, traceable responses that outperformed human physicians in clinical safety and authoritativeness evaluations.


<details>
  <summary>Details</summary>
Motivation: Traditional post-discharge follow-up suffers from workforce shortages, fragmented patient histories, and information silos. Existing large language models have limitations in medical QA including hallucination risks and inability to reason over longitudinal, patient-specific constraints.

Method: Healink integrates a triage routing mechanism, a unified memory enhancement module with a relational database for optimal latency, and a strict constraint-based retrieval-augmented generation engine. It employs vectorization of historical clinical records and weighted similarity functions across phenotypic and intervention dimensions to prevent cross-departmental drug conflicts.

Result: Evaluated on 400 continuous and 85 highly complex real-world follow-up cases plus webMedQA benchmark. In single-blind evaluation by clinical experts, Healink outperformed human physician baselines in both authoritativeness and clinical safety metrics.

Conclusion: Healink provides a scalable, safe, and highly effective paradigm for intelligent patient management through its traceable white-box evidence chain, ultimately enhancing societal healthcare outcomes by addressing workforce shortages and information fragmentation in post-discharge care.

Abstract: Post-discharge clinical follow-up is critical for maintaining continuity of care and mitigating long-term health risks. However, traditional follow-up paradigms suffer from shortage of health workforce, fragmented patient histories, and information silos across clinical departments. While large language models have demonstrated potential in medical question-answering, their deployment in continuous care is hindered by hallucination risks and a fundamental inability to reason over longitudinal, patient-specific constraints. Here we present Healink, a memory-enhanced multi-agent framework to support AI-assisted post-discharge follow-up by generating prescription-grounded, traceable responses that improved completeness and perceived clinical utility in retrospective and physician-blinded evaluations. The architecture seamlessly integrates a triage routing mechanism, a unified memory enhancement module utilizing a robust relational database for optimal latency, and a strict constraint-based retrieval-augmented generation engine. By vectorizing historical clinical records and employing weighted similarity functions across diverse phenotypic and intervention dimensions, Healink ensures precise inter-patient and intra-patient case matching while actively preventing cross-departmental drug conflicts. We evaluated Healink on a dataset comprising 400 continuous and 85 highly complex real-world follow-up cases, alongside the webMedQA benchmark. In a rigorous single-blind evaluation conducted by clinical experts, the framework outperformed human physician baselines in both authoritativeness and clinical safety. By generating a traceable, white-box evidence chain, Healink provides a scalable, safe, and highly effective paradigm for intelligent patient management, ultimately enhancing societal healthcare outcomes.

</details>


### [3] [Rate-Aware Quantum-Inspired Trajectory Learning for Interference-Limited Multi-UAV Networks](https://arxiv.org/abs/2606.25480)
*Khaoula Khaled,Muhammad Afaq,Ali Arshad Nasir,Zeeshan Kaleem*

Main category: cs.MA

TL;DR: RA-QAGC combines rate-aware graph abstraction with decentralized reinforcement learning to enable scalable, interference-aware UAV trajectory optimization, achieving 15% higher total throughput and 34% higher priority-user throughput over baseline schemes.


<details>
  <summary>Details</summary>
Motivation: UAVs provide on-demand connectivity but face curse of dimensionality in trajectory optimization, especially in interference-limited environments with vast search spaces that make real-time coordination computationally expensive.

Method: The proposed RA-QAGC scheme integrates rate-aware graph abstraction with decentralized reinforcement learning to identify high throughput locations and guide UAV trajectory adaptation toward throughput-optimal regions while maintaining QoS requirements.

Result: Simulation results demonstrate 59.4 Mbps total throughput and 23.9 Mbps priority-user throughput, representing approximately 15% and 34% improvements over baseline schemes respectively.

Conclusion: RA-QAGC effectively balances network capacity by maintaining QoS requirements and provides a scalable, interference-aware solution for UAV coordination in challenging environments.

Abstract: Unmanned aerial vehicle (UAV) can provide on-demand, high-capacity connectivity in disaster and normal situation. However, it faces a challenge of curse of dimensionality in trajectory optimization, where interference-limited environments and vast search spaces make real-time coordination computationally expensive. To overcome this challenge, we propose the Rate-Aware Quantum-Annealed Graph Condensation (RA-QAGC) scheme, which combines rate-aware graph abstraction with decentralized reinforcement learning to enable scalable, interference-aware UAV coordination. By identifying high throughput locations and guiding UAV trajectory adaptation toward throughput-optimal regions, RA-QAGC effectively balances network capacity by maintaining quality-of-service (QoS) requirements. Simulation results demonstrate the proposal outperformed over existing schemes by achieving 59.4 Mbps total throughput and 23.9 Mbps priority-user throughput, representing gains of approximately 15% and 34%, respectively, over the baseline schemes.

</details>


### [4] [Manipulation Is Task-Dependent: A Multi-Axis, Multi-Environment Evaluation of Frontier LLMs](https://arxiv.org/abs/2606.25899)
*Adeeb Zaman,Erik Nordby,Fred Heiding*

Main category: cs.MA

TL;DR: Researchers evaluated manipulative behavior in 6 frontier language models across 6 environments with 13,590 scenarios, measuring manipulation rates across three axes: framing, incentive structure, and task difficulty. They found low correlations (ρ=0.055) between manipulation tendencies across different environments, revealing that the drivers of manipulation vary contextually.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks typically vary only one axis within a single environment, which is insufficient for understanding manipulative behavior. The authors aim to provide rigorous multi-dimensional evaluations to better assess LLM manipulative tendencies.

Method: Evaluated 6 frontier language models across 6 diverse environments (negotiation tasks to agentic workflows), creating 13,590 individual scenarios. Measured manipulation rates across three axes: framing (mandate honesty vs permit manipulation), incentive structure (no to substantial incentives), and task difficulty. Calculated Spearman rank correlations across environments and identified patterns of manipulation drivers.

Result: Spearman rank correlations across environments averaged ρ=0.055, indicating manipulative tendencies in one task don't predict those in another. The axis driving manipulation varies by environment: instructional framing and structurally binding incentives dominate when incentivized to misrepresent future actions, while task difficulty dominates when incentivized to misrepresent ground truth. This pattern was validated across 5 environments and confirmed in a held-out 6th environment.

Conclusion: Multi-dimensional evaluations are essential for measuring manipulative propensities because manipulative behavior is context-dependent rather than consistent across tasks. Researchers should not assume that a model's manipulative tendencies in one domain predict its behavior in another.

Abstract: We evaluate manipulative behavior in six frontier language models across six environments, ranging from negotiation tasks to agentic workflows, resulting in 13{,}590 individual scenarios. Manipulation rates are measured across three axes: framing (mandate honesty or permit manipulation), incentive structure (from no incentives to substantial ones), and task difficulty. Existing benchmarks typically vary a single axis within a single environment, an approach our results show is insufficient. We rank models by manipulation rate and find Spearman rank correlations across environments average $ρ= 0.055$, indicating manipulative tendencies in one task do not necessarily predict those in another. Additionally, we find the axis that drives manipulation varies across different environments. In environments where models are incentivized to misrepresent future actions, instructional framing and structurally binding incentives are the primary drivers; in environments where models are incentivized to misrepresent a ground truth, task difficulty dominates. This split was identified in five environments and validated against a sixth held-out environment. Together, these findings illustrate the importance of rigorous multi-dimensional evaluations when measuring manipulative propensities.

</details>


### [5] [Multi-Agent Goal Recognition with Team- and Goal-Conditioned Reinforcement Learning and Factorized Branch-and-Bound](https://arxiv.org/abs/2606.25978)
*Thiago Thomas,Gabriel de Oliveira Ramos,Felipe Meneguzzi*

Main category: cs.MA

TL;DR: MAGR-BB introduces a branch-and-bound search framework with a shared policy scoring model for multi-agent goal recognition from trajectory data, achieving exhaustive search correctness while reducing hypothesis materialization by orders of magnitude.


<details>
  <summary>Details</summary>
Motivation: Real-world applications like drone surveillance and collaborative robotics expose only agent trajectories, requiring observers to rank team-goal hypotheses from behavior alone. Traditional exhaustive search becomes intractable due to the combinatorial explosion of team partitions and goals per team.

Method: MAGR-BB employs a shared team- and goal-conditioned policy as a scoring model within a factorized branch-and-bound search framework. It operates on a multi-agent Blocksworld benchmark to evaluate performance.

Result: On the Blocksworld benchmark, MAGR-BB returns the same top-ranked hypothesis as exhaustive search throughout the entire trajectory while cutting hypothesis materialization by orders of magnitude and substantially reducing cumulative recognition runtime.

Conclusion: MAGR-BB effectively solves the multi-agent goal recognition problem by maintaining correctness (identical top hypothesis to exhaustive search) while dramatically improving computational efficiency through intelligent pruning in branch-and-bound search.

Abstract: Multi-agent goal recognition asks an observer to jointly infer which agents act together and what each team is trying to achieve, so the hypothesis space grows combinatorially with the number of team partitions and goals per team. Real applications such as drone surveillance and collaborative robotics expose only the agents' trajectory, which forces the observer to rank team-goal hypotheses from behavior alone. Multi-Agent Goal Recognition with Branch-and-Bound (MAGR-BB) addresses this setting with a shared team- and goal-conditioned policy used as the scoring model inside a factorized branch-and-bound search. On a controlled multi-agent Blocksworld benchmark, MAGR-BB returns the same top-ranked hypothesis as exhaustive search throughout the trajectory while cutting hypothesis materialization by orders of magnitude and reducing cumulative recognition runtime substantially.

</details>


<div id='cs.RO'></div>

# cs.RO [[Back]](#toc)

### [6] [BFMTrack: Latent Sequence Optimization for Physics-Based Motion Tracking with Behavioral Foundation Models](https://arxiv.org/abs/2606.25056)
*Thomas Rupf,Agon Serifi,David Müller,Sammy Christen,Ruben Grandia,Espen Knoop,Moritz Bächer*

Main category: cs.RO

TL;DR: Latent Sequence Optimization (LSO) enables behavioral foundation models to perform precise motion tracking by combining simulation rollouts with policy gradient updates, guided by temporally correlated noise, validated across dense tracking, sparse keyframing, and real humanoid robot deployment.


<details>
  <summary>Details</summary>
Motivation: Behavioral foundation models (BFMs) excel at time-invariant tasks like goal-reaching but cannot directly support time-varying objectives like motion sequence tracking. Existing heuristics using moving-window-averaging fail to capture the nuances of highly dynamic motions.

Method: Latent Sequence Optimization (LSO) combines simulation rollouts with policy gradient updates to optimize over sequences of latents. The approach models the latent sequence using temporally correlated noise to guide optimization toward smooth, coherent trajectories, eliminating the need for reward engineering and tuning.

Result: LSO is validated across three scenarios: dense tracking, sparse keyframing, and direct deployment onto a real humanoid robot, demonstrating precise motion tracking capabilities.

Conclusion: LSO extends BFM capabilities toward precise motion tracking without requiring reward engineering and tuning, successfully bridging the gap between time-invariant and time-varying objective handling in physics-based character control.

Abstract: Behavioral Foundation Models (BFMs) offer a promising path toward universal physics-based character control by organizing a rich repertoire of physically plausible behaviors into a latent space, guided by a large-scale motion dataset. While these models excel at time-invariant tasks, such as goal-reaching and state-based reward optimization, their latent space does not directly support time-varying objectives, such as tracking a motion sequence. For tracking, existing heuristics rely on moving-window-averaging that fails to capture the nuances of highly dynamic motions. In this work, we propose a novel Latent Sequence Optimization (LSO) to address these shortcomings. Our approach combines simulation rollouts with a policy gradient update to optimize over a sequence of latents, extending the capabilities of BFMs toward precise motion tracking without requiring reward engineering and tuning. To guide the optimization toward smooth, coherent latent trajectories, we model the latent sequence using temporally correlated noise. We validate our approach across dense tracking, sparse keyframing, and direct deployment onto a real humanoid robot.

</details>


### [7] [Invariant Kalman filtering for extended pose estimation in multi-IMU articulated rigid-body systems](https://arxiv.org/abs/2606.25083)
*Sven Goffin,Cédric Schwartz,Silvère Bonnabel,Olivier Brüls,Pierre Sacré*

Main category: cs.RO

TL;DR: The paper introduces a relative L-extended pose representation and an iterated invariant extended Kalman filter (IterIEKF) for accurate pose estimation in IMU-instrumented articulated rigid-body systems, achieving at least 50% lower RMSE than baselines.


<details>
  <summary>Details</summary>
Motivation: Accurate extended pose estimation (orientation, velocity, position) for IMU-instrumented articulated rigid-body systems is challenging. The invariant EKF works well for single rigid bodies but extending to articulated systems is difficult due to inter-body pose coupling and joint kinematic constraints.

Method: The authors introduce a relative L-extended pose representation for kinematic-tree systems with one IMU per body. This yields group-affine dynamics and allows joint constraints to be expressed in invariant form. These constraints are incorporated as noise-free pseudo-measurements within an iterated IEKF (IterIEKF).

Result: Validated on a UR5e robot and a human leg, the IterIEKF outperforms all EKF, IterEKF, and absolute-pose IterIEKF baselines. It converges faster, exhibits lower run-to-run variability, and consistently achieves the lowest RMSE with at least 50% reduction compared to the second-best filter.

Conclusion: The proposed method successfully extends invariant filtering properties to articulated systems, providing faster convergence, consistency, and accuracy compared to traditional approaches, effectively addressing the open problem of incorporating joint kinematic constraints within the invariant framework.

Abstract: Accurate extended pose estimation (orientation, velocity, and position) for IMU-instrumented articulated rigid-body systems is a key challenge in robotics and human motion analysis. The invariant extended Kalman filter (IEKF) addresses this problem for a single rigid body with convergence guarantees and consistency under unobservability, but extending these properties to articulated systems is nontrivial: inter-body pose coupling prevents a direct application, and incorporating joint kinematic constraints within the invariant framework remains an open problem. To address this gap, we introduce the relative L-extended pose, a Lie group representation for kinematic-tree systems. With one IMU per body, it yields group-affine dynamics and allows joint constraints to be expressed in invariant form. We incorporate these constraints as noise-free pseudo-measurements within an iterated IEKF (IterIEKF), thereby preserving the convergence and consistency guarantees of invariant filtering. Validated on both a UR5e robot and a human leg, the proposed IterIEKF outperforms all EKF, IterEKF, and absolute-pose IterIEKF baselines. It converges faster, exhibits lower run-to-run variability, and consistently achieves the lowest RMSE, with reductions of at least 50% compared to the second-best filter across all scenarios considered in this work.

</details>


### [8] [ADM-Fusion: Adaptive Deep Multi-Sensor Fusion for Robust Ego-Motion Estimation in Diverse Conditions](https://arxiv.org/abs/2606.25111)
*Hasan Moughnieh,Ibrahim Ghaddar,Hadi Elham,Imad H. Elhajj,Daniel Asmar*

Main category: cs.RO

TL;DR: ADM-Fusion is an end-to-end adaptive multi-sensor fusion method for ego-motion estimation that employs a mixture-of-experts framework with content-aware routing and cross-task attention between separate translation and rotation branches, demonstrating effective simulation-to-real transfer and robust performance under degraded conditions.


<details>
  <summary>Details</summary>
Motivation: Robust multi-sensor fusion is essential for reliable autonomy in diverse and degraded environments where sensor reliability fluctuates rapidly. Since different modalities fail in distinct ways, effective fusion should adaptively balance complementary cues rather than rely on fixed weighting. This adaptability is particularly important for ego-motion estimation, where accurate updates depend on the consistent integration of complementary sensor information.

Method: ADM-Fusion uses an adaptive mixture-of-experts framework with content-aware routing to dynamically assign weights to sensor inputs in real time. The system incorporates separate translation and rotation branches coupled through a cross-task attention mechanism to preserve task-specific specialization while enabling information sharing. The method is trained end-to-end using deep learning.

Result: ADM-Fusion is trained on the CARLA-LOC simulated dataset and fine-tuned on KITTI real-world data, demonstrating effective simulation-to-real transfer. Experiments show that ADM-Fusion maintains competitive performance against existing methods while remaining robust under degraded conditions.

Conclusion: ADM-Fusion successfully adapts to environmental changes and sensor degradation, providing a robust solution for multi-sensor fusion in ego-motion estimation tasks.

Abstract: Robust multi-sensor fusion is essential for reliable autonomy in diverse and degraded environments, where sensor reliability can fluctuate rapidly. Because different modalities fail in distinct ways, effective fusion should adaptively balance complementary cues rather than rely on fixed weighting. This adaptability is particularly important for ego-motion estimation, since accurate updates depend on the consistent integration of complementary sensor information. We propose ADM-Fusion, an end-to-end deep learning based multi-sensor fusion method designed to adapt to environmental changes and sensor degradation. ADM-Fusion employs an adaptive sensor mixture-of-experts framework with content-aware routing to dynamically assign weights to sensor inputs in real time. The system further incorporates separate translation and rotation branches, coupled through a cross-task attention mechanism to preserve task-specific specialization while enabling information sharing. ADM-Fusion is trained on the CARLA-LOC simulated dataset and subsequently fine-tuned on KITTI real-world data, demonstrating effective simulation-to-real transfer. Experiments show that ADM-Fusion remains robust under degraded conditions while maintaining competitive performance against existing methods.

</details>


### [9] [SurveilNav: Collaborative Object Goal Navigation with Robot and Surveillance System](https://arxiv.org/abs/2606.25119)
*Ming-Ming Yu,Qunbo Wang,Rongtao Xu,Yanghong Mei,Yirong Yang,Longteng Guo,Wenjun Wu,Jing Liu*

Main category: cs.RO

TL;DR: SurveilNav is a collaborative navigation framework that integrates active camera scheduling, joint 2D/3D mapping, VLM-based value estimation, and collaborative target verification to enable efficient exploration and navigation in large-scale indoor environments by synergizing robot's dynamic local perception with static surveillance information.


<details>
  <summary>Details</summary>
Motivation: With growing surveillance deployment in factories, offices, and homes, integrating surveillance with robots offers promising collaborative task execution. Existing approaches focus on single-robot scenarios and struggle with multi-view collaboration in large-scale environments, facing limitations of single-robot perception range and fixed camera blind spots.

Method: SurveilNav integrates four key components: 1) Active camera scheduling, 2) Joint 2D/3D mapping, 3) VLM-based value estimation, and 4) Collaborative target verification. The framework synergizes the robot's dynamic local perception with the static global view of surveillance to overcome perception limitations.

Result: Built a novel indoor collaborative object navigation dataset on Habitat-Sim with 206 cameras across 74 floors. SurveilNav substantially outperforms existing methods, achieving state-of-the-art performance in both exploration efficiency and navigation success rate on the HM3D dataset.

Conclusion: The system effectively overcomes limited perception range of single agents and inherent blind spots of fixed cameras, resolving inefficient exploration. It demonstrates strong potential for applications in large-scale search, home environments, and rescue missions.

Abstract: With the growing deployment of surveillance systems in factories, offices, and homes, integrating them with robots offers a promising direction for collaborative and efficient task execution. However, existing approaches largely focus on single-robot scenarios and struggle with multi-view collaboration in large-scale environments. In this paper, we present a novel indoor collaborative object navigation dataset built on Habitat-Sim, featuring 206 cameras across 74 floors. The dataset enables systematic evaluation of an agent's ability to exploit multi-view surveillance information. To address the limitations of single-robot perception, we propose SurveilNav, a collaborative navigation framework that integrates active camera scheduling, joint 2D/3D mapping, VLM-based value estimation, and collaborative target verification. By synergizing the robot's dynamic local perception with the static global view of surveillance, this architecture effectively overcomes both the limited perception range of single agents and the inherent blind spots of fixed cameras, resolving inefficient exploration. Experimental results on the HM3D dataset demonstrate that SurveilNav substantially outperforms existing methods, achieving state-of-the-art performance in both exploration efficiency and navigation success rate. Moreover, the system shows strong potential for applications in large-scale search, home environments, and rescue missions.

</details>


### [10] [AeroCast: Probabilistic 3D Trajectory Prediction for Non-Cooperative Aerial Obstacles via Transformer-MDN Architecture](https://arxiv.org/abs/2606.25122)
*Syed Izzat Ullah,Jose Baca*

Main category: cs.RO

TL;DR: AeroCast: A probabilistic trajectory prediction framework for aerial vehicles using Transformer encoder + Mixture Density Network to predict multi-modal 3D obstacle trajectories with per-timestep Gaussian mixture distributions.


<details>
  <summary>Details</summary>
Motivation: Non-cooperative obstacles (birds, uncontrolled drones, debris) exhibit multi-modal motion that deterministic predictors cannot capture. Existing methods either use recurrent encoders that limit long-range precursor capture or produce point forecasts lacking distributional information for downstream planners.

Method: AeroCast combines a Transformer encoder with a Mixture Density Network (MDN) output head to predict per-timestep Gaussian mixture distributions over 3D displacements. Features: translation-invariant consecutive displacement encoding and calibration-oriented training objective to address input design and mode-degeneracy challenges.

Result: On hybrid real-and-synthetic quadrotor corpus spanning 9 motion categories, AeroCast reduces Average/Final Displacement Error by ~50% relative to baselines over 5-second horizon, achieves lowest negative log-likelihood and Continuous Ranked Probability Score. Inference: 0.1ms per sample (100Hz real-time compatible).

Conclusion: Ablation shows velocity input and model capacity are primary contributors to prediction quality, while positional encoding is essential for long-horizon trajectory coherence. AeroCast effectively addresses mode-degeneracy and calibration challenges in aerial trajectory prediction.

Abstract: Autonomous aerial vehicles operating in shared airspace must predict the future positions of non-cooperative obstacles to plan evasive maneuvers before a collision becomes unavoidable. Unlike cooperative systems that share intent, non-cooperative obstacles such as birds, uncontrolled drones, or debris exhibit multi-modal motion that deterministic predictors cannot adequately represent. Existing methods either rely on recurrent encoders that propagate temporal information sequentially, limiting their ability to capture long-range kinematic precursors of maneuver initiation, or produce point forecasts that provide no distributional information to downstream planners. This paper presents AeroCast, a probabilistic trajectory prediction framework that combines a Transformer encoder with a Mixture Density Network output head to predict per-timestep Gaussian mixture distributions over future three-dimensional displacements. A translation-invariant consecutive displacement encoding and a calibration-oriented training objective address the input design and mode-degeneracy challenges specific to mixture-based aerial trajectory prediction. On a hybrid real-and-synthetic quadrotor corpus spanning nine motion categories, AeroCast reduces Average Displacement Error and Final Displacement Error by approximately 50% relative to the baselines over a five-second horizon, and achieves the lowest negative log-likelihood and Continuous Ranked Probability Score among all compared methods. Ablation analysis identifies velocity input and model capacity as the primary contributors to prediction quality, and positional encoding as essential for long-horizon trajectory coherence. AeroCast inference completes in 0.1ms per sample, compatible with real-time onboard deployment at 100Hz.

</details>


### [11] [RGB: RL Guided Whole-Body MPPI for Humanoid Control](https://arxiv.org/abs/2606.25123)
*Yunsoo Seo,Sol Choi,Euncheol Im,Myo Taeg Lim,Yisoo Lee*

Main category: cs.RO

TL;DR: RL-guided whole-body MPPI framework using pretrained RL policy as sampling prior for MPPI rollouts, enabling precise task-level control without retraining


<details>
  <summary>Details</summary>
Motivation: Humanoid robots need whole-body controllers that are both robust and precise in contact-rich environments. Deep reinforcement learning (RL) achieves robust stability but its behavior is tightly coupled to training objective and command interface, making it difficult to add new feedback objectives without retraining.

Method: Propose an RL-guided whole-body model predictive path integral (MPPI) framework that uses a pretrained RL policy as a sampling prior to bias MPPI rollouts toward dynamically feasible behaviors. Task objectives are specified through modular MPPI cost terms, and MPPI closes the loop by continuously correcting the RL prior online without retraining.

Result: Simulations on a 29-DoF Unitree G1 humanoid in MuJoCo demonstrate stable high-rate control (average 280 Hz). The method improves task-level precision over a pure RL baseline under the same command interface, achieving this by correcting systematic drift during straight walking and tracking additional whole-body reference signals.

Conclusion: The proposed approach achieves precise task-level control without retraining by using RL as a prior and MPPI for online correction, demonstrating stable high-rate control in simulations for humanoid robots.

Abstract: Humanoid robots require whole-body controllers that are both robust and precise in contact-rich environments. While deep reinforcement learning (RL) achieves robust stability, its behavior is tightly coupled to the training objective and command interface, making it difficult to add new feedback objectives without retraining. In this study, we propose an RL guided whole-body model predictive path integral (MPPI) framework that acts as an add-on feedback controller on top of a pretrained RL policy. Instead of using RL policy as the final controller, we use it as a sampling prior that biases MPPI rollouts toward dynamically feasible behaviors. Task objectives are specified through modular MPPI cost terms, and MPPI closes the loop by continuously correcting the RL prior online to satisfy these objectives without retraining the policy. Simulations on a 29-DoF Unitree G1 humanoid in MuJoCo demonstrate stable high-rate control (average 280~Hz). The proposed method improves task-level precision over a pure RL baseline under the same command interface. This is achieved by correcting systematic drift during straight walking and tracking additional whole-body reference signals imposed through the cost.

</details>


### [12] [Causality-Based Parametric Control Barrier Function for Safe Multi-Vehicle Interaction](https://arxiv.org/abs/2606.25134)
*Yiwei Lyu,Caleb Chang,John M. Dolan*

Main category: cs.RO

TL;DR: Extends Parametric-Control Barrier Function (Parametric-CBF) with causality inference for multi-robot interactions, enabling adaptive safe control that accounts for inter-vehicle influence and improves task efficiency.


<details>
  <summary>Details</summary>
Motivation: Safe control is crucial for autonomous driving. Recent data-driven approaches model neighboring vehicles but face challenges: (1) inter-vehicle influence complicates causality determination, and (2) worst-case analysis leads to overly conservative behavior.

Method: Extends Parametric-CBF to multi-robot interactions with embedded causality inference to explicitly reason over inter-vehicle influence. Presents an adaptive safety-critical controller that leverages learned expectations for safe reaction to surrounding vehicles.

Result: Leveraging motion flexibility among multi-vehicle systems, task efficiency is greatly improved in various interaction-intensive scenarios.

Conclusion: The proposed causality-based Parametric-CBF approach addresses inter-vehicle influence while avoiding overly conservative behavior, enabling safer and more efficient autonomous driving in complex multi-vehicle scenarios.

Abstract: Safe control has been widely studied in various safety-critical applications, for instance, autonomous driving. In order to ensure the autonomous vehicle does not collide with other vehicles, it is essential to obtain an accurate expectation of surrounding vehicles' behavior and react adaptively. Instead of assuming fully cooperative and homogeneous vehicles using the same safety-critical controllers, recent works have been exploring different data-driven approaches to model the neighboring vehicles' underlying controllers with observed data. However, existing works either suffer from 1) the inter-vehicle influence during the multi-vehicle interaction, which makes it hard to determine the causality of surrounding vehicles' behavior in controller modeling, or 2) being dominated by the worst-case analysis, which may lead to overly conservative behavior. In this paper, we extend the prior work on Parametric-Control Barrier Function (Parametric-CBF) to multi-robot interactions with embedded causality inference to explicitly reason over the inter-vehicle influence. Given the learned Causality-based Parametric-CBF, we present an adaptive safety-critical controller that allows the ego vehicle to safely react to surrounding vehicles with the learned expectation. We demonstrate that by leveraging the motion flexibility among multi-vehicle systems, task efficiency can be greatly improved in various interaction-intensive scenarios.

</details>


### [13] [Memory Retrieval in Visuomotor Policies for Long-Horizon Robot Control](https://arxiv.org/abs/2606.25136)
*Rutav Shah,Yisu Li,Femi Bello,Yuke Zhu,Roberto Martín-Martín*

Main category: cs.RO

TL;DR: HALO introduces an attention-based memory retrieval mechanism for visuomotor policies in partially observable environments, using VLM priors and sparse attention to enable reliable long-horizon control from up to 8 minutes of past experience.


<details>
  <summary>Details</summary>
Motivation: General-purpose robots operating in partially observable environments (e.g., homes) require memory to recall diverse past information like object placements, task progress, and appliance states. While Transformer architectures with attention over long contexts offer a promising approach for learning retrieval from data, directly incorporating them into offline imitation learning faces two challenges: (1) spurious correlations between past information and predicted actions, and (2) error accumulation in memory causing model drift and cascading failures.

Method: HALO addresses these challenges through two key innovations: (1) Distilling vision-language model (VLM) priors into the policy by generating memory-dependent question-answer pairs from demonstration trajectories and training jointly with a video question-answering objective, steering retrieval toward task-relevant information; (2) Using sparse attention that restricts retrieval to only the most relevant parts of the history to reduce the impact of accumulated errors.

Result: HALO enables more reliable long-horizon control by guiding the policy to retrieve task-relevant information from up to eight minutes of past experience.

Conclusion: By combining VLM priors to suppress spurious correlations and sparse attention to mitigate error accumulation, HALO provides a robust memory retrieval mechanism that supports general-purpose robot autonomy in complex, partially observable environments.

Abstract: General-purpose robots operating in partially observable environments, such as homes, require memory to support autonomy. They must recall diverse information from the past, such as where objects were placed, which tasks a human partner has completed, and when an appliance was turned on. Achieving this versatility requires a general memory retrieval mechanism. Transformer architectures that use attention over long contexts for memory retrieval provide a promising approach, as they learn retrieval from data rather than relying on task-specific or hand-designed rules. However, directly incorporating them into imitation learning from offline data introduces two key challenges: (1) the policy may learn spurious correlations between past information and predicted actions, and (2) errors accumulate in memory due to prediction inaccuracies and their compounding interactions with the environment, causing model drift and cascading failures. To address both challenges, we introduce HALO, a visuomotor policy with an attention-based memory retrieval mechanism for long-horizon control. First, to suppress spurious correlations, HALO distills vision-language model (VLM) priors into the policy. It generates memory-dependent question--answer pairs from demonstration trajectories and trains jointly with a video question--answering objective, steering retrieval toward task-relevant information. Second, to reduce the impact of accumulated errors in memory during closed-loop control, HALO uses sparse attention that restricts retrieval to only the most relevant parts of the history. Together, these components enable more reliable long-horizon control by guiding the policy to retrieve task-relevant information from up to eight minutes of past experience. Project website: https://robin-lab.cs.utexas.edu/HALO

</details>


### [14] [SwarmFly: A simulation platform for UAV swarm experiment design and validation](https://arxiv.org/abs/2606.25146)
*Abhishek Phadke,Karthik Kumar Vasudeva,Abhishek Joshi*

Main category: cs.RO

TL;DR: SwarmFly is a modular MATLAB-based simulation platform for multi-UAV swarms that provides real-time operational maps, four coordination modes, simulated IMU telemetry, and IP-based geolocation via a plugin architecture, enabling comprehensive testing of swarm behaviors through eight experiments measuring formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance.


<details>
  <summary>Details</summary>
Motivation: Existing open-source UAV swarm simulation tools are unmaintained, have steep learning curves, and are limited to single fixed scenarios, creating a research gap for a comprehensive, modular simulation platform.

Method: SwarmFly combines a real-time operational map, four swarm coordination modes (leader-follower, decentralized, heterogeneous relay, heterogeneous speed), simulated IMU telemetry, and IP-based geolocation with a plugin architecture. Eight bundled plugins extend the base simulator. The platform runs entirely in MATLAB and supports hardware-in-the-loop testing, larger swarms, and higher-fidelity dynamics.

Result: The platform was verified and characterized through eight experiments measuring formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance, demonstrating its capability to expose swarms to internal and external disruptions for observation and quantification of control mechanisms.

Conclusion: SwarmFly provides an accessible, extensible open-source platform that addresses the limitations of existing tools, supports straightforward extension toward hardware-in-the-loop testing, larger swarms, and higher-fidelity dynamics, and is available at https://github.com/abhishekphadke/SwarmFly.git.

Abstract: The initial development phase of UAV swarms largely depends on simulation for experimental design and validation, yet existing open-source tools are often unmaintained, have steep learning curves, or are built around a single fixed scenario. The need for a comprehensive, modular simulation platform is a recognized research gap. This paper presents SwarmFly, a MATLAB-based simulation and test platform for multi- UAV swarms that addresses these gaps. SwarmFly combines a real-time operational map, four swarm coordination modes (leader-follower, decentralized, heterogeneous relay, and heterogeneous speed), simulated IMU telemetry, and IP-based geolocation with a plugin architecture that lets researchers add behaviors, fault models, and analysis tools without touching the core code. Eight bundled plugins extend the base simulator into a full test harness. The SwarmFly platform exposes multi-agent aerial swarms to a wide range of internal and external disruptions, enabling observation and quantification of underlying swarm control and behavioral mechanisms. This study verifies and characterizes each subsystem through eight experiments that measure formation accuracy, wind tolerance, fault recovery, energy endurance, and airspace compliance. The platform runs entirely in MATLAB. Its modular design supports straightforward extension toward hardware-in-the-loop testing, larger swarms, and higher-fidelity dynamics. An open-source release is available at [https://github.com/abhishekphadke/SwarmFly.git]

</details>


### [15] [Toward Low-Latency Vision-Language Models with Doubly-Correct Predictions in Egocentric Visual Understanding](https://arxiv.org/abs/2606.25160)
*Qitong Wang,Fan Du,Pranav Maneriker,Jihui Jin,Christopher Rasmussen*

Main category: cs.RO

TL;DR: This paper explores weight pruning techniques for Vision-Language Models (VLMs) in egocentric visual understanding tasks for human-robot collaboration. The authors propose a rationale-informed pruning strategy that preserves doubly-correct predictions (both accurate and evidentially grounded).


<details>
  <summary>Details</summary>
Motivation: VLMs are rapidly rising in egocentric visual understanding, making low-latency inference critical for human-robot collaborative (HRC) tasks. While weight pruning can satisfy efficiency demands of on-board processing, safe HRC demands pruning strategies that preserve doubly-correct predictions (outputs must be both accurate and evidentially grounded). Existing pruning methods surprisingly often preserve evidence localization but undermine correct predictions.

Method: A rationale-informed pruning strategy that better aligns evidence with decisions. The authors conduct experiments on egocentric video datasets to evaluate pruning effectiveness.

Result: Experiments reveal that existing pruning methods often preserve right evidence localization but undermine correct predictions. The proposed rationale-informed pruning strategy achieves the highest prediction accuracy and outperforms existing approaches in attaining doubly-correct predictions.

Conclusion: The method stimulates research on efficient and reliable VLMs, ensuring accuracy-driven advances align with the transparency, auditability, and safety required for responsible human-robot interaction and embodied intelligence.

Abstract: The rapid rise of Vision-Language Models (VLMs) in egocentric visual understanding has made low-latency inference in human-robot collaborative (HRC) tasks increasingly critical. Weight pruning techniques developed for VLMs to shrink model size and computation can be readily applied to satisfy the efficiency demands of on-board processing and real-time interactive robotics. Moreover, safe human-robot interaction demands pruning strategies that preserve doubly-correct predictions; outputs must be both accurate and evidentially grounded to mitigate risks and ensure user trust. In this paper, we present a new study of VLM pruning through the lens of doubly-correct prediction. Our experiments surprisingly show that existing pruning methods often preserve the right evidence localization but undermine correct prediction. To address this, we propose a rationale-informed pruning strategy that better aligns evidence with decisions. Benchmark results on egocentric video datasets demonstrate that our method not only achieves the highest prediction accuracy but also outperforms existing approaches in attaining doubly-correct predictions. We aim to stimulate research on efficient and reliable VLMs, ensuring accuracy-driven advances align with the transparency, auditability, and safety required for responsible human-robot interaction and embodied intelligence.

</details>


### [16] [fARfetch: Enabling Collocated AR-HRC in Large Visually Diverse Environments with VLM-Driven AR Content Adaptation](https://arxiv.org/abs/2606.25162)
*Christian Fronk,Hanting Ye,David Hunt,Miroslav Pajic,Maria Gorlatova*

Main category: cs.RO

TL;DR: fARfetch is an AR-HRC system for outdoor human-robot collaboration that integrates shared semantic mapping, world-in-miniature representation, and VLM-driven AR view management to maintain content legibility.


<details>
  <summary>Details</summary>
Motivation: AR can improve collocated human-robot collaboration by making robot state and intent visible and enabling intuitive control, yet large, visually diverse environments like outdoors challenge interaction and content legibility, especially at long distances and beyond visual line of sight.

Method: fARfetch integrates three components: (1) shared semantic environment mapping across AR headset and robot for landmark-grounded go-to commands, (2) context-aware world-in-miniature representation for fine-grained path authoring, and (3) vision-language-model driven AR view management adapting content color, size, and orientation. Implemented with Meta Quest 3 headset and Unitree Go2 quadruped robot, evaluated via within-subjects user study (N=13) on 30.5m outdoor inspection task.

Result: fARfetch yielded significantly faster completion times than non-AR baseline (66% improvement) and significantly lower workload in mental demand (-43%), temporal demand (-34%), and frustration (-66%). Custom legibility survey indicated fARfetch effectively maintained virtual content legibility in the large outdoor environment.

Conclusion: fARfetch effectively improves outdoor human-robot collaboration by maintaining content legibility while reducing user workload and improving task completion time through its integrated AR-HRC system design.

Abstract: Augmented Reality (AR) can improve collocated human-robot collaboration by making robot state and intent visible and enabling intuitive control, yet large, visually diverse environments like the outdoors challenge both interaction and content legibility, especially at long distances and beyond visual line of sight. We present fARfetch, an AR-HRC system that integrates (i) shared semantic environment mapping across an AR headset and robot that visualizes detected landmarks in AR to support landmark-grounded go-to commands, (ii) a context-aware world-in-miniature representation of the shared environment for fine-grained path authoring, and (iii) vision-language-model driven AR view management that jointly adapts virtual content color, size, and orientation to maintain legibility in large visually diverse environments. We implement fARfetch with a Meta Quest 3 headset and Unitree Go2 quadruped robot, and conduct a within-subjects user study (N=13) on a real-world large-scale (30.5m) outdoor inspection task. fARfetch yielded significantly faster completion times than a non-AR baseline (66%) and significantly lower workload in mental demand (-43%), temporal demand (-34%), and frustration (-66%). A custom legibility survey indicated fARfetch effectively maintained virtual content legibility in the large outdoor environment.

</details>


### [17] [Learning Perceptive Platform Adaptive Locomotion Controllers for Quadrupedal Robots](https://arxiv.org/abs/2606.25179)
*David Rytz,Kim Tien Ly,Ioannis Havoutis*

Main category: cs.RO

TL;DR: This paper investigates how to effectively integrate perception into morphology-aware reinforcement learning for quadrupedal locomotion, finding that critic-only perception improves robustness over blind baselines while remaining more stable than fully perceptive policies under noise.


<details>
  <summary>Details</summary>
Motivation: Universal quadrupedal locomotion is limited by the difficulty of integrating perception across diverse robot morphologies. Existing controllers rely on single-robot training or blind policies that lack real-time perception, leading to poor cross-embodiment generalization. Fully perceptive policies are sensitive to noise, while blind controllers lack terrain awareness.

Method: Building on MorAL, the authors train morphology-specialized universal controllers on multiple reference quadrupeds using adaptive terrain curricula. They compare three approaches: a blind baseline, a critic-perceptive variant (MorAL+), and a fully perceptive actor-critic (PPAL). Policies are evaluated in simulation on flat and rough terrains and deployed on ANYmal hardware.

Result: Critic-only perception improves robustness and tracking consistency over blind baselines while remaining more stable than fully perceptive policies under perception noise.

Conclusion: Perception placement and curriculum design are key factors for scalable, morphology-aware locomotion that balances robustness and stability.

Abstract: Universal quadrupedal locomotion remains limited by the difficulty of integrating perception across diverse robot morphologies. State-of-the-art controllers rely on single-robot training or blind policies that omit real-time perception, leading to poor cross-embodiment generalization. Designing locomotion policies that remain robust across related quadruped morphologies while incorporating perception is challenging. Moreover, fully perceptive policies are often sensitive to noise, whereas blind controllers lack terrain awareness. In this work, we study how perception should be integrated into morphology-aware reinforcement learning architectures for deployable quadrupedal control. Building on MorAL, we train morphology-specialized universal controllers on multiple reference quadrupeds using adaptive terrain curricula. We compare a blind baseline, a critic-perceptive variant (MorAL+), and a fully perceptive actor-critic (PPAL). Policies are evaluated in simulation on flat and rough terrains, and deployed on ANYmal hardware. Results show that critic-only perception improves robustness and tracking consistency over blind baselines while remaining more stable than fully perceptive policies under perception noise. These findings highlight that perception placement and curriculum design are key factors for scalable, morphology-aware locomotion.

</details>


### [18] [RAVEN: Long-Horizon Reasoning & Navigation with a Visuo-Spatio-Temporal Memory](https://arxiv.org/abs/2606.25206)
*Yixun Hu,Zhicheng Zheng,Lihan Zha,Chunwei Xing,Rajdeep Singh,Omar Hossain,Antonio Loquercio,Dhruv Shah*

Main category: cs.RO

TL;DR: RAVEN is a compact, scalable memory system for long-term robot deployment that stores visual embeddings with pose and time in a vector database, enabling efficient semantic, spatial, and temporal retrieval for robotic QA and navigation.


<details>
  <summary>Details</summary>
Motivation: Long-term robot deployment requires a memory system that preserves fine-grained visual semantics, grounds observations in space and time, and enables efficient storage and retrieval, while avoiding lossy image-to-text captioning.

Method: RAVEN stores visual embeddings with pose and time in a vector database, and grounds retrieval in a spatial map to answer queries and navigate to goals by operating directly on visual embeddings.

Result: Across several simulated and real-world video question-answering benchmarks, RAVEN consistently surpasses caption-based memory systems and matches frontier VLMs on long-horizon tasks at 10× lower retrieval cost. It also successfully deployed on a Unitree Go1 robot for long-horizon navigation tasks.

Conclusion: RAVEN demonstrates that operating directly on visual embeddings in a vector database with spatial grounding enables efficient long-horizon robotic tasks at a fraction of the retrieval cost compared to caption-based approaches.

Abstract: Long-term robot deployment requires a compact and scalable memory that preserves fine-grained visual semantics, grounds observations in space and time, and enables efficient storage and retrieval. In this paper, we propose RAVEN, an agentic memory system for long-horizon robotic question answering and navigation. RAVEN stores visual embeddings with pose and time in a vector database, and grounds retrieval in a spatial map to answer queries and navigate to goals. By operating directly on visual embeddings, RAVEN avoids lossy image-to-text captioning and enables accurate semantic, spatial, and temporal retrieval at scale. Across several simulated and real-world video question-answering benchmarks, RAVEN consistently surpasses caption-based memory systems and matches frontier VLMs on long-horizon tasks at 10$\times$ lower retrieval cost. Finally, we instantiate RAVEN on a Unitree Go1 robot for the task of long-horizon navigation for natural language goal-reaching, and show successful deployment over several large indoor environments.

</details>


### [19] [Wear-Clearance-Impact Coupling in the Jansen Linkage: A Gait-Durability-Optimized Design Slows Joint Loosening](https://arxiv.org/abs/2606.25208)
*Jichao Wang*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: A companion study introduced joint durability into the dimensional design of the Theo Jansen walking linkage and found its classical "holy numbers" Pareto-dominated, but it modelled the revolute joints as ideal, clearance-free pins, so its wear figures were relative rankings, not a prediction of in-service degradation. Here we relax that idealization. We build a forward-dynamic model of the Jansen leg in which a revolute joint becomes a clearance joint with a continuous normal contact law (Lankarani-Flores, hysteresis-damped) and Ambrosio friction, integrated as a constraint-stabilized differential-algebraic system, and couple it to the Archard law in a wear->clearance->impact feedback loop. Three findings emerge. First, neglecting clearance underestimates the peak joint load: the clearance model gives a peak contact force of ~104 N at the load-bearing pin against ~48 N for the ideal joint (~2x amplification), rising to ~426 N when two joints carry clearance at once. Second, the coupling is strongly impact-sensitive--single trajectories are non-monotonic and can reverse the design ranking, a chaos consistent with the literature--so designs must be compared statistically; over an ensemble of 16 randomized phases the optimized joint is robustly more durable, with per-cycle wear ~9-7x lower (peak force ~4x lower) at one clearance joint and still ~1.7x lower on both with two (p<0.01 throughout). Third, the wear is strongly non-uniform--it concentrates on a ~10 deg load arc--so assuming uniform clearance growth underestimates local clearance growth by ~36x. The clearance-free durability advantage thus survives the chaotic, multi-joint, non-uniformly-worn coupling in the ensemble mean. We deliver the first clearance-coupled forward-dynamic model of the Jansen leg and specify a falsifiable protocol to test each prediction.

</details>


### [20] [RigPI: Dynamic Parameter Identification of Rigid Body via VLM-Seeded Differentiable Simulation](https://arxiv.org/abs/2606.25212)
*Xincheng He,Rongrong Zhang,Wei Jiang,Wenqiang Xu*

Main category: cs.RO

TL;DR: RigPI is a differentiable simulation framework for identifying inertial and frictional parameters of manipulated objects by combining vision-language model priors, force-torque measurements, and motion observations through a two-stage optimization strategy.


<details>
  <summary>Details</summary>
Motivation: Accurate physical parameter identification is fundamental for robotic manipulation and digital twins, but acquiring consistent inertial and frictional properties is challenging due to sensing noise, modeling errors, and limited prior knowledge.

Method: RigPI integrates vision-based semantic priors (from a vision-language model), force-torque measurements, and motion observations within a differentiable simulation pipeline. It employs a two-stage optimization strategy: (1) VLM provides informed initialization and constrained search space, and (2) gradient-based parameter refinement using the differentiable physics simulator.

Result: Extensive real-world experiments on objects with revolute and prismatic joints demonstrate that RigPI achieves accurate and stable parameter estimates and successfully reproduces manipulation trajectories on a real robot.

Conclusion: RigPI is an effective and robust framework for real-world robotic system identification tasks, alleviating sensitivity to noise and avoiding physically implausible solutions.

Abstract: Accurate physical parameter identification of manipulated objects is fundamental to advanced robotic manipulation and the construction of faithful digital twins. However, acquiring physically consistent inertial and frictional properties from real-world interactions remains challenging due to sensing noise, modeling errors, and limited prior knowledge. This paper presents \textbf{RigPI}, a systematic framework for identifying dynamic parameters of both unconstrained rigid bodies and multi-link rigid bodies during robot-object interaction. RigPI integrates vision-based semantic priors, force-torque measurements, and motion observations within a differentiable simulation pipeline. A vision-language model (VLM) provides informed initialization and a constrained search space, while gradient information from a differentiable physics simulator enables efficient and stable parameter refinement. The proposed two-stage optimization strategy alleviates sensitivity to noise and avoids physically implausible solutions. Extensive real-world experiments on objects with revolute and prismatic joints demonstrate that RigPI achieves accurate and stable parameter estimates, and successfully reproduces manipulation trajectories on a real robot with parameter-aware predictive validity. These results highlight the effectiveness and robustness of RigPI for real-world robotic system identification tasks.

</details>


### [21] [Swazure: Swarm Measurement of Pose for Flying Light Specks](https://arxiv.org/abs/2606.25222)
*Hamed Alimohammadzadeh,Shahram Ghandeharizadeh*

Main category: cs.RO

TL;DR: Swazure is a novel cooperative technique that enables miniature drones with light sources (FLSs) to accurately track each other outside their sensor's sweet range for 3D multimedia display applications, using physical data independence and two heuristics (Move Obstructing and Move Source) to overcome sensor limitations.


<details>
  <summary>Details</summary>
Motivation: Miniature drones configured with light sources (FLSs) can create 3D multimedia displays by localizing to illuminate complex 3D shapes and animated sequences from point cloud data. However, FLSs must accurately measure their pose relative to each other using sensors like cameras, which have a limited "sweet range" for optimal accuracy. The challenge is tracking FLSs outside this sensor sweet range as dictated by point cloud data requirements.

Method: Swazure implements physical data independence by abstracting physical sensor characteristics, making point cloud data independent of sensor hardware. It uses cooperation among FLSs to solve missing sensor data. Two heuristics are proposed: Move Obstructing (which identifies and moves obstructing FLSs) and Move Source (which moves the source FLS). The approach works with medium-sized FLSs where the drone size is smaller than the minimum distance between point cloud points.

Result: With medium-sized FLSs, Swazure successfully positions 100% of FLS neighbors. The Move Obstructing heuristic resolves approximately 30% of obstructions in worst-case scenarios. The Move Source heuristic was also tested but showed less effectiveness. The technique demonstrates that cooperative approaches can overcome individual sensor limitations.

Conclusion: Swazure successfully addresses the challenge of tracking FLSs outside their sensor's sweet range through cooperative techniques and sensor abstraction. While the Move Obstructing heuristic resolves only ~30% of obstructions in worst cases, the approach shows promise for enabling robust 3D multimedia displays with drone swarms by overcoming individual sensor limitations through cooperative behavior.

Abstract: One may construct a 3D multimedia display using miniature drones configured with light sources, Flying Light Specks (FLSs). Swarms of FLSs localize to illuminate complex 3D shapes and animated sequences consistent with the coordinates of points in a point cloud. This requires FLSs to accurately measure their pose relative to one another using sensors such as cameras. Such sensors have a sweet range in which they provide the highest accuracy. A challenge is how an FLS tracks another FLS outside its sensor's sweet range, dictated by the point cloud data. We address this challenge by proposing a novel technique called Swazure that solves the missing sensor data using cooperation among FLSs. It implements physical data independence by abstracting the physical characteristics of the sensors, making point cloud data independent of the sensor hardware. The size of an FLS relative to the minimum distance between points of a point cloud is an important parameter. With medium-sized FLSs, Swazure is able to position 100% of the FLS's neighbors. Larger FLS sizes may result in potential obstructions that prevent Swazure from quantifying relative pose. We present two heuristics, Move Obstructing and Move Source, to address this limitation. Our experimental results show the superiority of the Move Obstructing heuristic which resolves approximately 30% of obstructions in the worst case scenario.

</details>


### [22] [Spatio-Temporal Retrieval-based Priors for Adaptive Computational Teaching in Driving](https://arxiv.org/abs/2606.25224)
*Deepak Edakkattil Gopinath,Xiongyi Cui,Jonathan DeCastro,Avinash Balachandran,Guy Rosman*

Main category: cs.RO

TL;DR: This paper proposes an imitation learning-based adaptive teaching model for complex motor tasks (specifically high-performance driving) that uses temporal reasoning, nearest neighbor retrieval, and cross-attention prior to effectively operate with limited training data.


<details>
  <summary>Details</summary>
Motivation: Current learning-based automated coaching systems are limited because they rely only on local, context-dependent reasoning, failing to account for the long-term temporal nature of student learning and the cumulative impact of repeated teacher-student interactions.

Method: The paper proposes an imitation learning computational model with a dedicated temporal reasoning module. To compensate for limited training data, the model uses nearest neighbor retrieval and cross-attention prior, reasoning only on semantically similar past interactions through an encoder-decoder based concurrent teaching model.

Result: Validation was performed using a novel semi-synthetic closed-loop longitudinal student-teacher interaction dataset based on Waymo Open Motion Dataset and a small-scale real-world naturalistic simulator race coaching dataset. Results show the model consistently outperforms non-adaptive baselines and adaptive models with different priors and temporal fusion mechanisms.

Conclusion: The adaptive teaching model with nearest neighbor retrieval and cross-attention prior demonstrates consistent advantages for adaptive teaching in complex motor tasks, effectively handling the long-term temporal nature of student learning with limited training data.

Abstract: Learning-based automated coaching systems for complex motor tasks such as high-performance driving remain limited in the ability to be adaptive by their reliance only on local, context-dependent reasoning, failing to account for the long-term temporal nature of student learning and the cumulative impact of repeated teacher-student interactions. In this paper, we propose an imitation learning based computational model for adaptive teaching with a dedicated temporal reasoning module that can reason over the interaction history under low-data regimes. To compensate for limited amounts of interactive training data, and based on the repetitive nature of the teaching process, the model relies on a nearest neighbor retrieval and cross attention prior, reasoning only on a narrowed-down set of semantically similar past interactions with an encoder-decoder based concurrent teaching model. We validate our approach with (i) a novel semi-synthetic closed-loop longitudinal student-teacher interaction dataset based on Waymo Open Motion Dataset and (ii) a small-scale real-world naturalistic simulator race coaching dataset. Our results reveal the consistent advantage of our adaptive teaching model with the nearest neighbor retrieval and cross-attention prior over a non-adaptive baseline as well as a suite of adaptive models that differ in their choice of priors and temporal fusion mechanisms.

</details>


### [23] [GRAFT: Graph-Based Affordance Transfer via Part Correspondence](https://arxiv.org/abs/2606.25241)
*Mengying Lin,Utkarsh Mishra,Ajay Mandlekar,Danfei Xu*

Main category: cs.RO

TL;DR: RAFT (Geometry-Aware Correspondence Framework) enables zero-shot robotic manipulation transfer using only one demonstration per object by representing objects as part-based graphs with part-level and vertex-level descriptors, combining functional and geometric similarity for instance retrieval and contact point propagation.


<details>
  <summary>Details</summary>
Motivation: Generalizing robotic manipulation to unseen objects is challenging because learning-based approaches require many demonstrations and fail in few-shot settings. Prior work transfers affordances through semantic retrieval alone, which neglects geometric similarity—a critical factor for successful manipulation.

Method: RAFT represents objects as part-based graphs with two types of descriptors: part-level descriptors for global instance retrieval and part correspondence, and vertex-level descriptors for fine-grained contact point matching. For an unseen object, it retrieves the most functionally and geometrically similar instance from the demonstration buffer with aligned functional parts, then propagates contact points through point-wise correspondence.

Result: The method achieves zero-shot manipulation transfer using only one demonstration per object by leveraging geometry-aware correspondence rather than relying solely on semantics.

Conclusion: Geometry-aware approaches that combine functional and geometric similarity are essential for effective manipulation transfer, and the proposed GRAFT framework demonstrates that zero-shot manipulation is achievable with minimal demonstrations by properly modeling object geometry through part-based graphs and correspondence matching.

Abstract: Generalizing robotic manipulation to unseen objects remains challenging, as learning-based approaches require many demonstrations and fail in few-shot settings. Prior work transfers affordances through semantic retrieval, but semantics alone neglect geometric similarity, which is critical for manipulation. We propose GRAFT, a geometry-aware correspondence framework for zero-shot manipulation transfer using only one demonstration per object. Objects are represented as part-based graphs, where part-level descriptors support global instance retrieval and part correspondence, and vertex-level descriptors enable fine-grained contact point matching. For an unseen object, our method first retrieves the most functionally and geometrically similar instance from the demonstration buffer with aligned functional parts, and finally propagates the contact points through point-wise correspondence.

</details>


### [24] [An Integrated Hardware-Software Design for Low-Data Spatial Defect Detection in Robotic Visual Inspection with Hybrid Optoelectronic Neural Networks](https://arxiv.org/abs/2606.25277)
*Chaoqing Tang,Jiaxuan Li,Huanze Zhuang,Guiyun Tian,Chao Wang,Yihao Ouyang,Wenzhong Liu*

Main category: cs.RO

TL;DR: A hardware-software integrated optoelectronic architecture for robotic visual inspection that employs a non-imaging, low-data paradigm. It combines a sensor-in-the-loop DMD reconfiguration strategy, block-based compressed sensing, and CLIP-guided natural language descriptions to detect defects with 90% data reduction and 60% computational workload reduction compared to traditional imaging.


<details>
  <summary>Details</summary>
Motivation: To address data overload and inefficient shape-level annotation in robotic visual inspection, where traditional methods require massive manual annotations which are labor-intensive and inefficient.

Method: 1. Sensor-in-the-loop strategy reconfiguring a Digital Micromirror Device (DMD) as a physical optical convolutional layer; 2. Block-based compressed sensing encoding spatial information into low-dimensional temporal signals; 3. Natural language descriptions guiding the network using CLIP for generalizable features; 4. Proposed Localization Accuracy for Attention (LAA) metric.

Result: Experiments on transparent material defect detection show: 90% data volume reduction compared to traditional imaging for Vision Transformers; 60% computational workload reduction for Convolutional Neural Networks; maintained equivalent accuracy; parametric analysis reveals effects of measurement matrices, compression ratios, and block sizes.

Conclusion: This low-data paradigm offers an efficient solution for industrial automation scenarios involving massive data streams, high acquisition costs, or constrained edge resources, effectively bypassing laborious manual defect shape annotation while maintaining accuracy.

Abstract: To address data overload and inefficient shape-level annotation in robotic visual inspection, this paper proposes a hardware-software integrated optoelectronic architecture. A non-imaging, low-data paradigm is established to minimize annotation dependency. First, a sensor-in-the-loop strategy reconfigures a Digital Micromirror Device (DMD) as a physical optical convolutional layer, enabling photonic-domain feature extraction that unifies sensing hardware and processing software. To suppress data volume at the source, a block-based compressed sensing strategy encodes spatial information into low-dimensional temporal signals, drastically reducing redundancy. Subsequently, to bypass laborious manual defect shape annotation, natural language descriptions guide the network to align with highly generalizable features from Contrastive Language-Image Pre-training (CLIP), steering the attention maps of the optoelectronic neural network toward defect shapes. Furthermore, a Localization Accuracy for Attention (LAA) metric is proposed to quantify shape-level defect localization performance. Experiments on transparent material defect detection validate the system's effectiveness. Parametric analysis reveals how measurement matrices, compression ratios, and block sizes affect accuracy. Results show that, compared to traditional imaging, the proposed architecture maintains equivalent accuracy while reducing data volume by 90% for Vision Transformers and computational workload by 60% for Convolutional Neural Networks. This low-data paradigm offers an efficient solution for industrial automation scenarios involving massive data streams, high acquisition costs, or constrained edge resources.

</details>


### [25] [DynaMOMA: Instantaneous Prediction of Grasp Poses for Mobile Manipulation of Dynamic Objects](https://arxiv.org/abs/2606.25295)
*Zhinan Yu,Junyan Xu,Jiazhao Zhang,Zheng Qin,Yijie Tang,Yuhang Huang,Yihan Cao,Zhiyuan Yu,Yongjun Wang,Renjiao Yi,Chenyang Zhu,Kai Xu*

Main category: cs.RO

TL;DR: A dynamic mobile manipulation framework that couples anchor-based diffusion model grasp trajectory prediction with whole-body reinforcement learning policy, enhanced by an anticipation-guided reward for dynamic object manipulation


<details>
  <summary>Details</summary>
Motivation: Mobile manipulation is fundamental but challenging with dynamic objects due to the need to coordinate mobile base and arm while adapting to continuously evolving target poses; key challenge is predicting temporally consistent short-horizon grasp trajectories from dynamic observations

Method: Proposed framework couples instantaneous grasp trajectory prediction with whole-body control policy. The predictor uses an anchor-based diffusion model to generate temporally consistent short-horizon grasp trajectories conditioned on historical observations. Predicted trajectories are encoded as compact features and fed to a whole-body reinforcement learning policy. Introduces an anticipation-guided reward that equips the policy with anticipatory grasping horizon by adaptively shifting the target from current grasp observation to the predicted grasp trajectory.

Result: Achieves strong performance in mobile manipulation of dynamic objects across diverse settings and grasping metrics. Demonstrates strong generalizability in real-world experiments.

Conclusion: The method effectively addresses the challenge of dynamic mobile manipulation by combining diffusion-based prediction with whole-body control and anticipatory rewards, demonstrating both simulation success and real-world generalizability.

Abstract: Mobile manipulation is a fundamental robotics task and has advanced rapidly in recent years, enabling robots to navigate, reach, and interact with objects in complex environments. However, mobile manipulation of dynamic objects remains highly challenging, as robots must coordinate the mobile base and arm while adapting to continuously evolving target poses. A key challenge lies in predicting temporally consistent short-horizon grasp trajectories from dynamic observations. In this work, we propose \ours{}, a dynamic mobile manipulation framework that couples instantaneous grasp trajectory prediction with whole-body control policy. Our predictor uses an anchor-based diffusion model to generate temporally consistent short-horizon grasp trajectories conditioned on historical observations. The predicted trajectories are then encoded as compact features and fed to a whole-body reinforcement learning policy, which controls the mobile manipulator for dynamic grasping. We further introduce a anticipation-guided reward that equips the policy with an anticipatory grasping horizon by adaptively shifting the target from the current grasp observation to the instantaneously predicted grasp trajectory. Through extensive experiments in Isaac Gym simulation, we show that our method achieves strong performance in mobile manipulation of dynamic objects across diverse settings and grasping metrics. Furthermore, our predictor and policy demonstrate strong generalizability in real-world experiments.

</details>


### [26] [WaveForward: An Omnidirectional Passive Wheeled Quadruped Robot with Casters](https://arxiv.org/abs/2606.25299)
*Chuanlin Zhao,Qifeng Zheng,Shuhan Wang,Tiancheng Ma,Weixian Lin,Xin Luo*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Wheeled-legged robots possess both agile mobility for traversing complex terrains and high efficiency, making them suitable for long-distance transportation applications. Conventional actuated wheeled robots require specialized hardware and electrical design due to the incorporation of wheel components. We propose a novel and low-cost passive wheeled legged robot equipped with standard casters on each leg to obtain omnidirectional mobility. The control method employs an asymmetric actor-critic structure, enabling the utilization of the privileged information of the passive caster's angles and velocities. We develop a caster base posture adjustment strategy based on velocity commands, utilizing actuated joints to modify the caster base joint axis posture and thereby adjust the propulsion direction of the casters. Moreover, we implemented multiple propulsion modes to achieve varying degrees of caster twisting oscillation, converting these into propulsive force. We conducted a slalom test and mode switch experience, which shows the passive wheeled quadruped could achieve omnidirectional movement versatility, and reduce the cost of transport (COT) by up to 89.1% with respect to legged motion.

</details>


### [27] [AI Coaching for Accelerating Human Skill Development with Reinforcement Learning](https://arxiv.org/abs/2606.25337)
*Wei Wang,Enlin Gu,Antonio Loquercio,Haimin Hu,Rahul Mangharam*

Main category: cs.RO

TL;DR: This paper develops an AI coaching framework that accelerates human motor-skill development through adaptive shared control, allowing productive failures while preventing over-reliance.


<details>
  <summary>Details</summary>
Motivation: AI copilots can boost performance but excessive assistance leads to over-reliance and skill atrophy. Effective coaching requires strategic scaffolding that aligns with learner capability and enables productive failures that drive learning.

Method: The interactive AI coaching process is formalized as a non-cooperative dynamic game where the learner optimizes task performance while the coach targets the learner's independent competence. A reinforcement learning framework combines adaptive shared control with probabilistic models of the coach's causal influence on skill evolution, enabling tractable training of coaching policies.

Result: A user study (N=33) on first-person-view drone racing demonstrates significant gains in human learning outcomes over state-of-the-art AI coaching baselines.

Conclusion: The proposed framework successfully balances AI assistance with learner independence, accelerating motor-skill development through strategic scaffolding and productive failures, validated by empirical results in drone racing.

Abstract: AI copilots can substantially boost human performance through shared control, but excessive assistance can induce over-reliance and skill atrophy. This paper studies how an embodied AI agent can act as a coach that accelerates human motor-skill development. We argue that effective coaching requires strategic scaffolding and stepping back that are aligned with the learner's capability, allowing productive failures that drive learning. We formalize the interactive AI coaching process as a non-cooperative dynamic game in which the learner optimizes task performance while the coach targets the learner's independent competence. Building on this formalism, we develop a reinforcement learning framework combining adaptive shared control with probabilistic models of the coach's causal influence on skill evolution, enabling tractable training of coaching policies. A comprehensive user study (N=33) on first-person-view drone racing shows significant gains in human learning outcomes over state-of-the-art AI coaching baselines.

</details>


### [28] [Self Capacitive Tactile Sensor System designed for Companion Robots](https://arxiv.org/abs/2606.25348)
*Mohsin Ali,Hidenobu Sumioka,Shuhei Ikemoto*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Tactile sensing is essential for humanoid robots to achieve safe physical interaction, dexterous manipulation, and truly human-like responsiveness. However, the design of such systems remains challenging. Conventional approaches often suffer from complex multilayer structures, intricate wiring, high cost, and poor scalability, making it difficult to realize full-body tactile sensing with real-time, low-latency detection while maintaining minimal computational load on the robot's main processor. In this work, we present a simple, scalable and hardware friendly tactile sensing system for a companion humanoid robot based on the self-capacitance principle. The proposed sensor system employs a single conductive fabric layer with a conductive fabric wire architecture and does not require intricate electrode patterning. Scalability was demonstrated by fabricating a 100-point sensor array on a flexible printed circuit (FPC). Evaluation across sampling frequencies showed that 10 Hz is insufficient and misses transient events, whereas 100 Hz and 1000 Hz reliably capture and clearly distinguish all interaction types: gentle touch, slow tapping, fast tapping, and hitting. A decision-tree classifier was implemented directly on the FPGA, offloading real-time inference from the Raspberry Pi 4 with minimal latency and negligible power overhead. This design fully meets the tactile sensing requirements of the HIRO-chan robot and is well-suited for full-body tactile sensing in HIRO-chan and other companion robots.

</details>


### [29] [Decoupling Semantics and Geometric Grounding: Spatial Visual Prompts for Language-Conditioned Imitation Learning](https://arxiv.org/abs/2606.25360)
*Yanzhe Tang,Xinyu Shao,Yuxuan Hu,Siyu Chen,Bowen Yang,Yajun Gao,Tongtong Cao,Xiu Li,Long Zeng*

Main category: cs.RO

TL;DR: SVP-IL is a decoupled architecture that separates spatial visual grounding from action generation in Vision-Language-Action models, using zero-shot geometric masks as Spatial Visual Prompts to guide the action generator with explicit spatial priors.


<details>
  <summary>Details</summary>
Motivation: End-to-end VLA models couple semantic reasoning and spatial control, creating alignment bottlenecks that limit precise target disambiguation in data-constrained imitation learning.

Method: SVP-IL extracts spatial visual grounding from the action generation loop using vision-language foundation models to parse instructions into zero-shot geometric masks (Spatial Visual Prompts - SVP). These priors are injected into a continuous action generator via a lightweight direct feature-level fusion mechanism.

Result: SVP-IL significantly outperforms state-of-the-art VLAs and pure visuomotor baselines. On highly ambiguous language-conditioned tasks, it improves average success rates from 24.0% to 39.5% with only 50-100 demonstrations, achieving 67.8% on standard benchmarks.

Conclusion: Real-world robotic experiments validate SVP-IL's robustness and data efficiency in unstructured physical environments, demonstrating effective performance with minimal training data.

Abstract: While end-to-end Vision-Language-Action (VLA) models show promise in robotic manipulation, their monolithic paradigm inherently couples semantic reasoning and spatial control. This creates a severe alignment bottleneck, limiting precise target disambiguation in data-constrained imitation learning. To overcome this, we propose SVP-IL, a decoupled architecture that explicitly extracts spatial visual grounding from the action generation loop. By leveraging vision-language foundation models, we parse instructions into zero-shot geometric masks, translating language into explicit Spatial Visual Prompts (SVP). These priors are injected into a continuous action generator via a lightweight direct feature-level fusion mechanism. This integration provides explicit and uncorrupted spatial gradient guidance while ensuring highly stable optimization under low-data regimes. Extensive experiments demonstrate that SVP-IL significantly outperforms state-of-the-art VLAs and pure visuomotor baselines. Trained on as few as 50 to 100 demonstrations, SVP-IL improves average success rates on highly ambiguous language-conditioned tasks from 24.0% to 39.5%, achieving 67.8% on standard benchmarks. Real-world robotic experiments further validate its robustness and data efficiency in unstructured physical environments.

</details>


### [30] [Reliability-Asymmetric Spacecraft Autonomy: Co-Designing a Capable Learned GNC Stack with a Verified, Adaptation-Aware Runtime Shield](https://arxiv.org/abs/2606.25366)
*Alireza Shojaei*

Main category: cs.RO

TL;DR: AMPLE-GNC is a three-tier autonomy stack for deep-space missions combining a foundation-model commander, constraint-screening verifier, and fault-adaptive controller, all bounded by a runtime shield with machine-checked LTL invariants to achieve both capability and certifiability.


<details>
  <summary>Details</summary>
Motivation: Deep-space missions require onboard autonomy that is both capable and certifiable. Traditional rule-based autonomy is certifiable but brittle, while learned autonomy is capable but hard to verify—creating a critical gap that AMPLE-GNC aims to bridge.

Method: The framework introduces a three-tier GNC stack: (1) a small foundation-model commander mapping natural language to PDDL+ with grammar-constrained decoding, (2) a constraint-screening verifier, and (3) a fault-adaptive controller using Rapid Motor Adaptation. All components are bounded by a runtime shield with nine linear-temporal-logic invariants whose soundness is machine-checked via Kind 2.

Result: Key results include: 84% planner-executable actions from the edge commander, 38% exact and 51% action-level generalization for novel phrasing; fault-adaptive controller recovers 97.8% of actuator-sign faults and 94.4% of continuous-gain faults within the training randomization envelope; and the safety shield maintains 94.5% autonomy while catching non-recovery scenarios.

Conclusion: AMPLE-GNC successfully demonstrates that capability and certifiability can coexist. The fault-adaptive controller shows superior fault recovery compared to classical baselines, and the safety shield with split-conformal recovery-deadline certificates maintains high autonomy while ensuring safety, proving the viability of machine-checked LTL constraints for certifying learned autonomy in safety-critical space applications.

Abstract: Deep-space missions need onboard autonomy that is both capable and certifiable. Rule-based autonomy is certifiable but brittle, while learned autonomy is capable but hard to verify. We present AMPLE-GNC, a three-tier guidance, navigation, and control stack. Its capability path combines a small foundation-model commander that maps natural language to PDDL+, a constraint-screening verifier, and a fault-adaptive controller. All three are bounded by a runtime shield with nine linear-temporal-logic invariants whose predictor soundness is machine-checked by the Kind 2 model checker. On a 6-DOF Basilisk testbed, we make three contributions. First, we deploy an edge commander. Fine-tuning a pretrained 360M model with grammar-constrained decoding gives a hard output-validity guarantee and 84% planner-executable actions. On a de-leaked test, novel-phrasing generalization is 38% exact and 51% action, rising to 48% exact after phrasing-diversity re-finetuning; we separate syntactic validity from semantic accuracy. Second, we introduce a fault-adaptive controller. Rapid Motor Adaptation infers latent actuator faults online and recovers 97.8% of actuator-sign faults and 94.4% of continuous-gain faults within the training randomization envelope. Fault-unaware PD and from-scratch end-to-end RL both score 0%, while the strongest classical-adaptive baseline reaches 55% on continuous gain. Beyond the envelope, a split-conformant retrain scores 57-67%, and adding 4x more in-regime data worsens performance, showing that randomization breadth, not data volume, drives generalization. Robustness is flat under star-tracker noise to 0.005. Third, we show that a latching safe-hold shield can suppress even a capable controller. A split-conformal recovery-deadline certificate with adaptation-aware engagement reconciles safety and recovery, keeping the controller 94.5% autonomous while still catching non-recovery.

</details>


### [31] [Commerge: Communication-Efficient, Robust, and Fast LiDAR Map Merging Framework for Multi-Robot Coordination in Resource-Constrained Scenarios](https://arxiv.org/abs/2606.25386)
*Hogyun Kim,Jiwon Choi,Juwon Kim,Geonmo Yang,Seokhwan Jeong,Hyungtae Lim,Younggun Cho*

Main category: cs.RO

TL;DR: Commerge is a communication-efficient multi-robot LiDAR map merging framework that reduces inter-robot communication by up to 5,000x through graph-theoretic selective data exchange, maintaining alignment accuracy across various environments.


<details>
  <summary>Details</summary>
Motivation: Map merging requires exchanging massive sensor data between servers and robots, making communication the bottleneck especially in communication-constrained environments, while existing approaches either transmit whole scans (GB-scale) or use naive downsampling.

Method: A three-stage cascaded optimization problem on an exchange graph where vertices represent robot keyframes and edges denote candidate inter-robot loops, selecting a sequentially overlapped, balanced-transmission-cost, and geometrically-perceptually optimal scan subset to minimize communication bandwidth.

Result: Commerge reduces inter-robot communication by up to 5,000x (99.98% data exchange reduction from 7,000MB to 1.3MB on HeLiPR dataset) while maintaining alignment accuracy across five public datasets and four in-house datasets covering diverse environments.

Conclusion: Only a small subset of carefully selected scans is sufficient for robust map merging, enabling significant communication bandwidth reduction without compromising alignment performance through graph-theoretic selective data exchange.

Abstract: By maintaining global consistency across robot teams, multi-robot LiDAR map merging enables faster exploration and efficient area coverage. However, map merging requires exchanging massive sensor data between the server and robots, making communication the bottleneck, especially in communication-constrained environments. Therefore, we present Commerge, a communication-efficient map merging framework that achieves bandwidth reduction through graph-theoretic selective data exchange. By doing so, our Commerge reduces inter-robot communication by up to 5,000x while maintaining alignment accuracy. Our key insight is that only a small subset of carefully selected scans is sufficient for robust map merging. We formulate this as a three-stage cascaded optimization problem on an exchange graph, where vertices represent robot keyframes and edges denote candidate inter-robot loops. Through three cascade stages, we select a sequentially overlapped, balanced-transmission-cost, and geometrically-perceptually optimal scan subset that preserves alignment quality while reducing communication. Unlike existing approaches that either transmit whole scans, which require GB-scale data exchange, or employ naive downsampling, our approach exchanges only MB-scale data while achieving comparable alignment accuracy. Extensive evaluation on five public datasets and four in-house datasets covering cave, planetary-analog, indoor, and outdoor campus environments shows up to 99.98% reduction in data exchange (e.g., from 7,000MB to 1.3MB on the HeLiPR dataset), while maintaining alignment performance across embedded to desktop platforms. The supplementary materials are available at https://sparolab.github.io/research/commerge.

</details>


### [32] [Large-Scale Tunnel Air--Ground Collaboration With FLISP: Fast LiDAR-IMU Synchronized Path Planne](https://arxiv.org/abs/2606.25393)
*Fenghe Guo,Runjie Shen,Chenyang Sun,Junrui Zhang,Quanxi Zhan,Yongchun Wang,Junjie Zhang*

Main category: cs.RO

TL;DR: FLISP is a mapless cooperative UGV-UAV planning framework for hydropower tunnel inspection that achieves 100% success rate with ~7ms latency, eliminating map-based method bottlenecks.


<details>
  <summary>Details</summary>
Motivation: Hydropower tunnel inspection is critical for infrastructure integrity but remains inefficient and hazardous using manual methods.

Method: FLISP features three core contributions: (1) unified architecture with single UGV-mounted LiDAR-IMU driving synchronized path generation for both platforms; (2) platform-specific solvers using enhanced Firefly Algorithm for UGV obstacle avoidance and dynamic iterative optimizer for UAV flight; (3) hierarchical refinement strategy ensuring kinematic feasibility without state estimation drift.

Result: In a 1.2 km operational tunnel, FLISP eliminates map rasterization overhead (Fast-LIO2 + A*) and sampling instability (LIO-SAM + RRT*), achieving 100% success rate with ~7ms latency—7-fold speedup over grid-based methods and three-order-of-magnitude improvement over sampling-based baselines.

Conclusion: Validated in operational hydropower tunnels, FLISP offers a scalable solution for robotic inspection in feature-degraded linear infrastructure.

Abstract: Hydropower tunnel inspection is critical for infrastructure integrity yet remains inefficient and hazardous using manual methods. We propose FLISP (Fast LiDAR-IMU Synchronized Path Planner), a mapless planning framework for cooperative UGV-UAV inspection. Unlike traditional map-based paradigms, FLISP features three core contributions: (1) a unified architecture where a single UGV-mounted LiDAR-IMU suite drives synchronized path generation for both platforms; (2) platform-specific solvers utilizing an enhanced Firefly Algorithm for UGV obstacle avoidance and a dynamic iterative optimizer for UAV flight; and (3) a hierarchical refinement strategy ensuring kinematic feasibility without state estimation drift. Benchmarks in a 1.2 km operational tunnel demonstrate that FLISP circumvents structural bottlenecks of map-based methods, eliminating map rasterization overhead (Fast-LIO2 + A*) and sampling instability (LIO-SAM + RRT*). FLISP achieves a 100% success rate with ~7 ms latency, representing a 7-fold speedup over grid-based and three-order-of-magnitude improvement over sampling-based baselines. Validated in operational hydropower tunnels, this approach offers a scalable solution for robotic inspection in feature-degraded linear infrastructure. A demonstration video is available at https://youtu.be/Y_ezs1PfLJ4, and the code at https://github.com/ArchibaldGuo/FLISP.git.

</details>


### [33] [MAPL: Multi-Objective Preference Learning for Robot Locomotion](https://arxiv.org/abs/2606.25398)
*Xiyue Chen,Muhan Lin,Shuyang Shi,Joseph Campbell*

Main category: cs.RO

TL;DR: MAPL uses LLM-generated preferences across multiple semantic criteria to learn locomotion rewards without manual engineering, achieving performance comparable to expert-designed rewards across quadruped environments.


<details>
  <summary>Details</summary>
Motivation: Reward design is a major bottleneck in RL for robot locomotion; existing LLM-based preference methods use single overall judgments, making it difficult to capture multiple competing objectives inherent in high-quality locomotion.

Method: MAPL prompts an LLM to compare trajectories independently along semantically meaningful, terrain-invariant criteria using natural language descriptions. These objective-wise preferences train a multi-head preference scoring model whose outputs are aggregated into a scalar reward for policy optimization.

Result: Across four quadruped locomotion environments, MAPL trains policies using only LLM-generated preferences and achieves performance comparable to or better than expert-designed rewards.

Conclusion: MAPL eliminates task-specific reward engineering while delivering competitive performance, demonstrating that high-level natural language objectives can effectively replace carefully tuned reward functions in robot locomotion tasks.

Abstract: Reward design remains a major bottleneck in reinforcement learning for robot locomotion, where successful policies often depend on carefully tuned, task-specific reward functions. Preference-based reinforcement learning offers an alternative, but existing LLM-based methods typically ask for a single overall judgment between behaviors, making it difficult to capture the multiple competing objectives that underlie high-quality locomotion. We present Multi-Objective AI-Informed Preference Learning (MAPL), a framework that learns locomotion rewards from high-level natural language objectives rather than manually engineered reward equations. MAPL prompts a large language model to compare trajectories independently along semantically meaningful criteria, using generic language descriptions that are terrain-invariant and require little domain expertise. These objective-wise preferences are used to train a multi-head preference scoring model, whose outputs are aggregated to form a scalar reward for policy optimization. Across four quadruped locomotion environments, MAPL trains policies using only LLM-generated preferences and achieves performance comparable to or better than expert-designed rewards, while eliminating task-specific reward engineering.

</details>


### [34] [HEART: Coordination of Heterogeneous Expert Agents for Physically Grounded Robotic Task Planning](https://arxiv.org/abs/2606.25404)
*Junho Lee,Seabin Lee,Wonjong Lee,Nayoung Kim,Moonjeong Kang,Changjoo Nam*

Main category: cs.RO

TL;DR: HEART is a heterogeneous multi-LLM framework for robotic task planning that decomposes instructions into atomic reasoning tasks allocated to specialized expert agents under token budgets, combining role-oriented reasoning with constraint-driven synthesis to produce physically executable plans.


<details>
  <summary>Details</summary>
Motivation: LLMs can reason over complex instructions but fail to satisfy physical and spatial constraints for robotic task planning. Recent LLM-based planners directly translate text to action sequences without structured reasoning about feasibility, reachability, and logical order, resulting in invalid or incomplete plans.

Method: HEART decomposes instructions into atomic reasoning tasks allocated to role-specialized expert agents under a token budget for real-world computational and communicational constraints. It combines role-oriented reasoning from heterogeneous agents with constraint-driven plan synthesis, validating capability, reachability, and constraint conditions before planning.

Result: Experiments across household benchmarks demonstrate that HEART consistently improves plan success compared to single-LLM and rule-based planners.

Conclusion: Heterogeneous LLM collaboration enables robust and scalable robotic task planning under resource constraints, producing physically executable plans while maintaining efficiency.

Abstract: Large Language Models (LLMs) can reason over complex instructions but often fail to satisfy the physical and spatial constraints required for robotic task planning. Recent LLM-based planners directly translate text into action sequences, yet they lack structured reasoning about feasibility, reachability, and logical order, resulting in invalid or incomplete plans. We present a heterogeneous multi-LLM framework that decomposes instructions into atomic reasoning tasks and allocates them to role-specialized expert agents under a token budget for real-world computational and communicational constraints. By combining role-oriented reasoning from heterogeneous agents followed by constraint-driven plan synthesis, HEART validates capability, reachability, and constraint conditions before planning and helps produce physically executable plans while maintaining efficiency. Experiments across different household benchmarks show that HEART consistently improves plan success compared to single-LLM and rule-based planners, demonstrating that heterogeneous LLM collaboration enables robust and scalable robotic task planning under resource constraints.

</details>


### [35] [Delta-Position Estimation-Based IMU Odometry: A Comparison of MLP and Kolmogorov-Arnold Networks](https://arxiv.org/abs/2606.25454)
*Osman Tokluoglu,Emin Keresteci*

Main category: cs.RO

TL;DR: This study compares a Multi-Layer Perceptron (MLP) and a Kolmogorov-Arnold Network (KAN) with learnable B-spline activations for learning-based inertial odometry using raw IMU measurements from the EuRoC MAV benchmark. KAN achieves 44% lower cumulative drift error (9.61 m vs 17.23 m) with 6.9x fewer parameters (8,444 vs 57,859), demonstrating superior performance and stability.


<details>
  <summary>Details</summary>
Motivation: Learning-based inertial odometry using raw IMU measurements; the authors aim to avoid absolute position regression, which can lead to large constant errors. Instead, they train models to estimate incremental displacement (Δp) over a fixed sliding window, reconstructing the full trajectory through numerical integration.

Method: The study compares a standard Multi-Layer Perceptron (MLP) with a Kolmogorov-Arnold Network (KAN) equipped with learnable B-spline activations. Both models are trained on raw IMU measurements from the EuRoC MAV benchmark dataset. KAN has 6.9× fewer parameters than MLP (8,444 vs 57,859). The models estimate incremental displacement (Δp) over a 50 ms sliding window, and trajectories are reconstructed via numerical integration.

Result: KAN achieves 44% lower final cumulative drift error on the test trajectory (9.61 m vs 17.23 m for MLP). KAN also exhibits more stable behavior with lower P_50 and P_90 cumulative drift values, indicating better long-term error accumulation characteristics.

Conclusion: Learnable B-spline-based activations have the potential to significantly reduce error accumulation in the inertial odometry problem, achieving superior performance with fewer parameters than traditional MLP architectures.

Abstract: In this study, the learning-based inertial odometry problem is investigated using raw IMU measurements obtained from the EuRoC MAV benchmark dataset. Instead of absolute position regression-a formulation that may lead to large constant errors-the models are trained to estimate the incremental displacement (Δp) over a fixed 50 ms sliding window, and the full trajectory is reconstructed through numerical integration. A standard Multi-Layer Perceptron (MLP) is compared with a Kolmogorov-Arnold Network (KAN) equipped with learnable B-spline activations. Although KAN has 6.9 times fewer parameters than MLP (8,444 versus 57,859), it produces a 44% lower error in terms of final cumulative drift on the test trajectory (9.61 m versus 17.23 m). In addition, KAN exhibits more stable behavior in terms of long-term error accumulation, with lower P_50 and P_90 cumulative drift values. These findings indicate that learnable B-spline-based activations have the potential to reduce error accumulation in the inertial odometry problem.

</details>


### [36] [Generative AI for Safe and Photorealistic Drone Light Shows](https://arxiv.org/abs/2606.25458)
*Pascal Reinhold,Alexander Gräfe,Sebastian Trimpe*

Main category: cs.RO

TL;DR: SWAN is an end-to-end pipeline that generates photorealistic, collision-free drone choreographies from text prompts, successfully demonstrated with 2000 simulated drones and 49 real quadcopters.


<details>
  <summary>Details</summary>
Motivation: Drone light shows require labor-intensive manual animation, and while generative AI could automate this, existing frameworks fail to provide photorealism with fluid, dynamic motion.

Method: SWAN converts text to realistic reference videos, uses a novel adaptive point-tracking algorithm to translate pixel-space dynamics into physical swarm kinematics (maintaining spatial coherence through occlusions and topological shifts), allocates trajectories via a dedicated planner, and applies a safety filter for collision-free execution.

Result: Successfully orchestrated 2000-drone formations in simulation and validated physical feasibility on 49 real quadcopters, operating entirely on standard consumer hardware.

Conclusion: Generative AI can be leveraged to automate multi-robot choreography design, providing an accessible new framework for drone light shows.

Abstract: Drone light shows are redefining aerial entertainment, yet their widespread adoption is bottlenecked by labor-intensive, manual animation. While generative AI promises an automated alternative, current frameworks fail to provide photorealism with fluid, dynamic motion. To address this limitation, we introduce SWAN, an end-to-end pipeline that synthesizes photorealistic, large-scale, and collision-free drone choreographies directly from text prompts. SWAN converts text into realistic reference videos and translates these pixel-space dynamics into physical swarm kinematics using a novel, adaptive point-tracking algorithm. Unlike existing trackers, this method maintains spatial coherence through severe occlusions and rapid topological shifts. A dedicated planner then allocates these trajectories to individual drones, while a subsequent safety filter ensures collision-free execution. We demonstrate scalability by safely orchestrating simulated 2,000-drone formations and validate physical feasibility on a dense real-world swarm of 49 quadcopters, operating everything entirely on standard consumer hardware. Combined, this work demonstrates how generative AI can be leveraged to automate multi-robot choreography design, providing an accessible new framework for drone light shows.

</details>


### [37] [SAGE-Nav: Leveraging LLM Planning and Alignment Fusion for Hierarchical Scene Graph-Guided Navigation](https://arxiv.org/abs/2606.25497)
*Hao Su,Yuehao Huang,Yukai Ma,Yong Liu,Jiajun Lv*

Main category: cs.RO

TL;DR: SAGE-Nav is a hierarchical framework combining LLMs with dynamic scene graphs for Object-Goal Navigation, achieving state-of-the-art performance in efficiency and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing monolithic methods struggle with long-horizon reasoning and generalize poorly to novel environments. The challenge is to balance global semantic planning with low-latency reactive control for embodied navigation.

Method: SAGE-Nav integrates LLMs as global planners with a Hierarchical Scene Graph Encoder (HSGE) and Goal-aware Alignment-Fusion Network (GAFN). The LLM decomposes instructions into waypoints, while HSGE produces structure-aware embeddings and GAFN uses adaptive gating to fuse real-time perception with structural priors.

Result: State-of-the-art performance in i-THOR and RoboTHOR environments with substantial gains in navigation efficiency and zero-shot generalization while maintaining low control latency.

Conclusion: The framework successfully decouples global semantic planning from reactive control, enabling robust navigation in novel environments suitable for physical robotic deployment.

Abstract: Object-Goal Navigation (ObjNav) requires embodied agents to autonomously locate specified targets using only egocentric visual observations. Existing monolithic methods struggle with long-horizon reasoning and generalize poorly to novel environments. To address these limitations, we propose SAGE-Nav, a novel hierarchical framework that integrates the reasoning capabilities of Large Language Models (LLMs) with dynamic scene graphs. Crucially, it decouples asynchronous global semantic planning from the high-frequency reactive control loop. The LLM serves as a global planner, decomposing abstract instructions into a sequence of semantically grounded waypoints. To translate these plans into dense multi-modal guidance, we design a Hierarchical Scene Graph Encoder (HSGE) that leverages relational graph convolutions to produce structure-aware embeddings preserving both semantic and spatial topology. Furthermore, we develop the Goal-aware Alignment-Fusion Network (GAFN) to dynamically fuse real-time perception with these structural priors. Using an adaptive gating mechanism with an explicit inductive bias, GAFN ensures robust visual-topological alignment for the low-level policy. Extensive evaluations in the i-THOR and RoboTHOR environments demonstrate that SAGE-Nav achieves state-of-the-art performance, delivering substantial gains in navigation efficiency and zero-shot generalization while maintaining the low control latency required for physical robotic deployment.

</details>


### [38] [AISPO: Enhancing Depth Reliability for Robotic Manipulation of Non-Lambertian Objects via Affine-Invariant Shape Prior](https://arxiv.org/abs/2606.25503)
*Zhiming Chen,Linfang Zheng,Kun Zhang,Hyung Jin Chang,Wei Zhang,Hongyu Yu,Hua Chen*

Main category: cs.RO

TL;DR: AISPO is a depth completion framework that improves depth reliability for robotic manipulation, especially for non-Lambertian objects like transparent or highly specular surfaces, by combining multi-scale RGB-D feature fusion with an affine-invariant shape prior.


<details>
  <summary>Details</summary>
Motivation: Reliable depth perception is critical for robotic manipulation, but raw depth measurements are often corrupted or missing for non-Lambertian objects (transparent or highly specular surfaces). These failures propagate to motion planning, resulting in invalid grasp poses and execution errors.

Method: AISPO combines multi-scale RGB-D feature fusion with an affine-invariant shape prior to enforce geometric consistency and mitigate catastrophic depth failures. Unlike methods that focus primarily on average depth accuracy, AISPO emphasizes physical plausibility and structural integrity of predicted depth maps.

Result: Extensive benchmark evaluations demonstrate competitive performance and strong generalization to unseen objects and novel scenes. Real-world grasping experiments show enhanced depth reliability significantly improves manipulation success rates, particularly for transparent objects where many existing methods fail to produce physically usable depth estimates.

Conclusion: The proposed approach effectively addresses depth completion challenges in challenging sensing conditions, providing physically plausible depth estimates that enable reliable robotic manipulation, especially for objects with non-Lambertian surface properties.

Abstract: Reliable depth perception is critical for robotic manipulation, especially for non-Lambertian objects such as transparent or highly specular surfaces, where raw depth measurements are often corrupted or missing. These failures frequently propagate to motion planning, resulting in invalid grasp poses and execution errors. We propose AISPO, a depth completion framework that improves depth reliability for manipulation in challenging sensing conditions. AISPO combines multi-scale RGB-D feature fusion with an affine-invariant shape prior to enforce geometric consistency and mitigate catastrophic depth failures. Unlike methods that focus primarily on average depth accuracy, our approach emphasizes physical plausibility and structural integrity of the predicted depth maps. Extensive benchmark evaluations demonstrate competitive performance and strong generalization to unseen objects and novel scenes. Real-world grasping experiments further show that enhanced depth reliability significantly improves manipulation success rates, particularly for transparent objects where many existing methods fail to produce physically usable depth estimates.

</details>


### [39] [GROVE: Grounded Pedestrian Simulation via Natural Language for Interactive Social Robot Navigation](https://arxiv.org/abs/2606.25504)
*Duc Tai Nguyen,Volodymyr Shcherbyna,Anh Do Duc,Zhengcheng Shen,Teham Buiyan,Linh Kästner*

Main category: cs.RO

TL;DR: GROVE is a text-to-scenario pedestrian simulation framework that integrates multiple state-of-the-art modules to generate realistic, socially challenging scenarios for social robot navigation.


<details>
  <summary>Details</summary>
Motivation: Pedestrian simulation is essential for training social robot navigation but remains rigid, requiring manual data generation for even simple scenarios.

Method: GROVE combines SotA approaches in three modules for long-horizon behavior, medium-horizon navigation, and short-horizon interactions, dynamically selecting algorithms based on scenario type. Users can select presets (emergency, queuing, normal) or custom prompts. The framework integrates with Isaac Sim, Gazebo, and RViz.

Result: Qualitative validation across residential, hospital, and office environments demonstrates high-fidelity simulation with complex, diverse, realistic human behaviors.

Conclusion: GROVE effectively narrows the simulation-to-real gap by capturing situational nuances of pedestrian behavior through dynamic module selection and customizable prompt-based tuning.

Abstract: Pedestrian simulation is a critical component for training and deploying social robot navigation approaches, yet it remains a largely rigid system that repeatedly requires manual data generation to define even simple scenarios. We propose GROVE, a text-to-scenario pedestrian simulation framework that combines state-of-the-art approaches to produce realistic, socially challenging scenarios for social robot navigation. Our framework allows users to customize one of several common presets (emergency, queuing, normal) or even enter a fully independent prompt to generate a highly customizable pedestrian simulation. Multiple modules separately ensure the realism and soundness of long-horizon human behavior, medium-horizon pedestrian navigation, and short-horizon robot/social interactions. Each module is tuned by the prompt in a way that reflects the user intent across all aspects of pedestrian simulation. By dynamically selecting one of several state-of-the-art (SotA) approaches in our modules based on the scenario, we capture many situational nuances of pedestrian behavior in order to narrow the simulation-to-real (sim2real) gap. The human simulation is directly integrated into Isaac Sim, Gazebo, and RViz simulators for robot deployment in highly social environments. We validate our approach through qualitative comparison against existing pedestrian simulation baselines across scenarios of varying complexity in residential, hospital, and office environments. The result is a high-fidelity pedestrian simulation that challenges social robot navigation with complex, diverse, realistic human behaviors.

</details>


### [40] [ASSCG: Just-Right Gating over Chattering for Fast-Slow LLM Planning in Autonomous Driving](https://arxiv.org/abs/2606.25509)
*Sining Ang,Yuan Chen,Liu Haiyan,Xuanyao Mao,Jason Bao,Xuliang,Bingchuan Sun,Yan Wang*

Main category: cs.RO

TL;DR: ASSCG is a resource-aware gating system for LLM-based autonomous driving that makes frame-level Query/Cache/Drop decisions using an RWKV backbone, trained via supervised fine-tuning and compute-aware reinforcement learning to optimize slow-system invocation.


<details>
  <summary>Details</summary>
Motivation: LLMs improve autonomous driving planning but are costly to query online. Existing fast-slow planners rely on hand-designed triggering rules that either over-call the slow system or invoke it at wrong times.

Method: Formulating slow-system invocation as a resource-aware sequential decision problem, ASSCG makes frame-level Query/Cache/Drop decisions using an RWKV backbone for efficient long-horizon gating, trained via supervised fine-tuning followed by GRPO-style compute-aware reinforcement fine-tuning.

Result: On nuPlan Hard20 with AsyncDriver: improves score to 67.28 (+2.28) while reducing average end-to-end inference latency by 60%. On NAVSIM with a RecogDrive-based dual system: achieves 91.4 PDMS (+0.6) and increases average speed by 25%.

Conclusion: ASSCG effectively balances performance and efficiency across different fast-slow architectures, demonstrating that adaptive gating significantly improves both driving performance and computational efficiency compared to hand-designed rules.

Abstract: Large language models (LLMs) can improve autonomous driving planning but are costly to query online, and existing fast-slow planners often rely on hand-designed triggering rules that either over-call the slow system or call it at the wrong times. We formulate slow-system invocation as a resource-aware sequential decision problem and propose the Adaptive Slow-System Control Gate (ASSCG), which makes frame-level Query/Cache/Drop decisions to refresh, reuse, or suppress slow guidance. ASSCG uses an RWKV backbone for efficient long-horizon gating and is trained with supervised fine-tuning followed by GRPO-style compute-aware reinforcement fine-tuning. We apply ASSCG to two different fast-slow architectures: (i) AsyncDriver on nuPlan Hard20 closed-loop evaluation, where ASSCG improves score to 67.28 (+2.28) while reducing average end-to-end inference latency by 60%; and (ii) a RecogDrive-based dual system that we build by replacing its original VLM-2B module with a lightweight ViT-based fast planner and adding an LLM slow planner, evaluated on NAVSIM, where ASSCG achieves 91.4 PDMS (+0.6) and increases average speed by 25%. The project page, including video visualizations and additional results, is available at https://williamxuanyu.github.io/asscg/.

</details>


### [41] [One Body, Two Minds: Variable Autonomy Approach for a Co-embodied Robotic Hand](https://arxiv.org/abs/2606.25575)
*Piotr Koczy,Yuchong Zhang,Danica Kragic,Michael C. Welle*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Assistive robotic systems face a fundamental trade-off: fully autonomous systems lack user agency, while fully user-controlled systems demand continuous cognitive effort. Existing shared autonomy approaches blend human and robot commands but are mostly deployed in separate physical bodies. We introduce co-embodiment with variable autonomy, where human and robot share a single physical body and operate at different autonomy levels across task phases, from mutual autonomy during object search and grasping to human-dominant control during actuation.
  We present a co-embodied, wearable robotic hand that has its own ``mind'' and operates with variable autonomy levels. A learning-from-demonstration visuomotor diffusion policy enables autonomous grasping when the user positions the hand near known objects. Once grasped, the system signals completion and the human can actuate the grasped tool (drill, spray bottle, infrared thermometer, lighter, and ice-cream scoop) via hands-free head gestures. The human retains veto authority at all times through a release gesture that returns the system to the initial phase. Unlike blended autonomy, where control is continuously negotiated, our co-embodied approach consists of variable autonomy from full human control to full independent actions while maintaining physical coupling, realizing a one body, two minds paradigm.
  In a user study with 44 participants performing five bimanual tasks, users rapidly adapted to this ``two minds'' paradigm: completion times improved by 23.3% across trials ($p < 0.001$, Cohen's $d = 0.94$), the best-performing policy variant reached a 93.6% task success rate, and acceptance ratings were high (5.70/7 overall impression, 5.52/7 daily use willingness). This work establishes co-embodiment with variable autonomy as a viable approach for assistive robotics, enabling human-robot collaboration through co-embodiment.

</details>


### [42] [WOLF-VLA: Whole-Body Humanoid Optimal Locomotion Framework for Vision-Language-Action Learning](https://arxiv.org/abs/2606.25591)
*Melya Boukheddimi,Omar Adjali,Daniel Sontag,Frank Kirchner*

Main category: cs.RO

TL;DR: WOLF-VLA is a unified framework that integrates whole-body optimal-control motion synthesis with large-scale multi-modal datasets to train Vision-Language-Action models capable of generating humanoid locomotion policies from natural language instructions.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models have shown strong generalization in robotic manipulation but remain underexplored for whole-body, contact-rich humanoid locomotion due to data scarcity, absence of dynamically consistent demonstrations, and difficulty in encoding optimality and safety in learning-based pipelines.

Method: The framework constructs a comprehensive dataset of dynamically feasible humanoid trajectories across six locomotion-related task families parameterized by environmental variations, object colors, placements, and visual distractors. It trains a VLA model using joint trajectories, ego-centric visual observations, and natural language instructions.

Result: The trained policy exhibits strong reasoning and robustness to initial-condition variability, achieving competitive performance across several tasks and environment settings. A systematic ablation study demonstrates the impact of each modality on model performance.

Conclusion: The authors will release the full dataset, model checkpoints, and benchmarking simulation suite to establish a reproducible dynamically consistent benchmark for whole-body humanoid locomotion rich in VLA control, enabling future research in scalable transfer of instruction-driven locomotion policies.

Abstract: Vision-Language-Action (VLA) models have recently demonstrated strong generalization in robotic manipulation, yet their applicability to whole-body, contact-rich humanoid locomotion remains severely underexplored due to data scarcity, the absence of dynamically consistent demonstrations, and the difficulty of encoding optimality and safety in learning-based pipelines. This work introduces a unified framework WOLF-VLA that integrates whole-body optimal-control (OC) motion synthesis with large-scale multi-modal dataset to train VLAs capable of generating humanoid locomotion policies directly from natural-language instructions. We construct a comprehensive dataset of dynamically feasible humanoid trajectories across six locomotion-related task families, each parameterized by environmental variations, object colors, placements, and visual distractors. We train a VLA model using the collected joint trajectories, ego-centric visual observations and natural language instruction, yielding a policy that exhibits strong reasoning and robustness to initial-condition variability, and competitive performance across several tasks and environment settings. A systematic ablation study demonstrates the impact of each modality on the model performance. The full dataset, model checkpoints, and benchmarking simulation suite will be openly released, establishing a reproducible dynamically consistent benchmark for whole-body humanoid locomotion rich VLA control and enabling future research in scalable transfer of instruction-driven locomotion policies.

</details>


### [43] [1000 Rallies: An Event-Camera Dataset and Real-Time Learned Ball-State Estimation for Robotic Table Tennis](https://arxiv.org/abs/2606.25620)
*Raphaela Kreiser,Asude Aydin,Yin Bi,Claudio Fanconi,Peter Dürr,Naoya Takahashi*

Main category: cs.RO

TL;DR: A large-scale event-camera dataset for table tennis is introduced, along with a CNN-based ball estimation system integrated with a robotic arm to enable real-time human-robot rallies.


<details>
  <summary>Details</summary>
Motivation: Table tennis is a challenging benchmark for real-time robotic perception due to its fast ball dynamics and stringent timing requirements. Traditional frame-based cameras face a trade-off between low frame rates (missing fast objects) and high frame rates (increased data/computational cost). Event cameras offer microsecond temporal resolution and are less prone to motion blur, but the community lacks large-scale datasets to develop and benchmark event-based perception in realistic sports scenarios.

Method: The authors created the first large-scale event-camera dataset for table tennis comprising over 1000 rallies from amateur to elite players. Each recording captures the event stream alongside 14 synchronized high-speed frame-based cameras at 200 FPS to produce 1 kHz pseudo ground-truth labels for ball position, velocity, and spin. They trained a CNN robust to background player motion that jointly estimates ball position and velocity from events. The predicted velocity was used as an additional measurement in the Kalman filter, and the system was integrated with a Stäubli robotic arm to close the perception-action loop.

Result: The event-based system reduced bounce-point prediction error by 36% compared to a position-only baseline. The authors successfully demonstrated the first real-time human-robot table tennis rallies driven by event-based perception.

Conclusion: The large-scale event-camera dataset and integrated system demonstrate the viability of event-based perception for fast-moving sports scenarios, showing significant improvements in prediction accuracy and enabling real-time human-robot interaction in table tennis. This work addresses the critical gap in event-based perception datasets for realistic sports scenarios.

Abstract: Robotic table tennis has emerged as a compelling benchmark for real-time robotic perception due to its fast ball dynamics and stringent timing requirements. Accurate, high-frequency, and low-latency ball state estimation is critical for reliable trajectory prediction and timely control. Traditional frame-based cameras face an inherent trade-off: low frame rates leave temporal blind spots that miss fast-moving objects and high frame rates raise data and computational cost. Event cameras instead offer microsecond temporal resolution and, under sufficient illumination, remain largely free of motion blur even at high ball speeds. However, the community lacks large-scale datasets to develop and benchmark event-based perception in realistic sports scenarios. We address this gap by introducing the first large-scale event-camera dataset for table tennis, comprising over 1000 rallies from a diverse group of players ranging from amateurs to elite-level athletes. Each recording captures the event stream alongside 14 synchronized high-speed frame-based cameras at 200 FPS, which we use to produce 1 kHz pseudo ground-truth labels for ball position, velocity, and spin. Building on this dataset, we train a convolutional neural network robust to background player motion that jointly estimates the ball's position and velocity in the image-plane from events. Treating the predicted velocity as an additional measurement in the Kalman filter reduces bounce-point prediction error by 36% relative to a position-only baseline. Finally, we close the perception-action loop by integrating the event-based system with a Stäubli robotic arm, enabling the first real-time human-robot table tennis rallies driven by event-based perception.

</details>


### [44] [Event-Adaptive Motion Planning with Distilled Vision-Language Model in Safety-Critical Situations](https://arxiv.org/abs/2606.25629)
*Zhenwei Huang,Changsheng You,Shuai Wang,Chao Zhou,Wei Xu,Yi Gong*

Main category: cs.RO

TL;DR: EAMP proposes an efficient VLM-based robot navigation framework for safety-critical scenarios, using a prompt-configurable semantic event trigger to selectively activate semantic reasoning and combine it with semantic model predictive control for improved dynamic safety margins while preserving real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: Robot navigation in safety-critical environments faces challenges from unforeseen semantic events where collisions primarily arise from unpredictable behaviors of dynamic agents rather than unseen objects. While large vision-language models (VLMs) offer powerful commonsense reasoning capabilities, invoking them within continuous control loops introduces severe computational latency that destabilizes physical execution.

Method: EAMP framework with three components: (1) Prompt-configurable semantic event trigger (PC-SET) that continuously monitors short temporal clips for behavioral anomalies and selectively activates semantic intervention; (2) Event-triggered distilled SemNav-VLM fine-tuned via physically verified semantic distillation, which maps detected anomalies into discrete strategy-level decisions; (3) Semantic model predictive control (SMPC) module that translates these strategies into dynamic reconfigurations of optimization objectives and geometric references.

Result: Extensive experiments in safety-critical logistics scenarios demonstrate that EAMP effectively aligns high-level reasoning with low-level control, significantly improving dynamic safety margins over existing baselines while preserving real-time efficiency.

Conclusion: The proposed event-adaptive motion planning framework successfully addresses the fundamental challenge of combining VLM-based semantic reasoning with real-time control in safety-critical navigation scenarios, achieving both improved safety performance and computational efficiency.

Abstract: Robot navigation in safety-critical scenarios faces significant challenges from unforeseen semantic events, where collisions arise primarily from the unpredictable behaviors of dynamic agents rather than unseen objects. While large vision-language models (VLMs) offer remarkable capabilities in commonsense reasoning, frequently invoking them within the continuous control loop introduces severe computational latency, fundamentally destabilizing physical execution. To address these challenges, we propose event-adaptive motion planning (EAMP), an efficient framework for VLM-based robot navigation. Specifically, a prompt-configurable semantic event trigger (PC-SET) selectively activates semantic intervention by continuously monitoring short temporal clips for behavioral anomalies. Upon triggering, an event-triggered distilled SemNav-VLM, fine-tuned via physically verified semantic distillation, maps detected anomalies into discrete strategy-level decisions. Subsequently, a semantic model predictive control (SMPC) module translates these strategies into dynamic reconfigurations of optimization objectives and geometric references. Extensive experiments in safety-critical logistics scenarios demonstrate that EAMP effectively aligns high-level reasoning with low-level control, significantly improving dynamic safety margins over existing baselines while preserving real-time efficiency.

</details>


### [45] [Calousel: Extrinsic Calibration of Non-overlapping Multi-camera Systems from Pure Rotation](https://arxiv.org/abs/2606.25646)
*Gwanhyeong Song,Chaehyeon Song,Ayoung Kim*

Main category: cs.RO

TL;DR: A novel multi-camera extrinsic calibration method for non-overlapping FOVs using pure rotational motion with a single static board, formulated via a latent turntable frame and SE(3) global optimization.


<details>
  <summary>Details</summary>
Motivation: Multi-camera extrinsic calibration with non-overlapping FOVs is challenging: target-based methods have high setup overhead, while motion-based approaches suffer from drift, scale ambiguity, and motion degeneracy. The authors seek to balance accuracy and usability.

Method: A method leveraging pure rotational motion with only a single static calibration board. All cameras sequentially observe the same target under a shared geometric reference without overlapping views. The problem is formulated using a latent turntable frame and 3D error on SE(3) within a global optimization framework.

Result: Validated on a controlled camera rig and a full-scale vehicle platform with heterogeneous cameras. The approach maintains competitive accuracy without specialized precision hardware and shows robustness under non-ideal turntable motion.

Conclusion: Extensive experiments demonstrate strong suitability for realistic on-site deployments, achieving competitive accuracy with improved usability compared to conventional approaches.

Abstract: Extrinsic calibration of multi-camera systems with non-overlapping FOVs has been a challenging problem in the robotics literature. Conventional target-based methods impose substantial target setup overhead, either deploying large calibration targets or requiring pre-measured multi-target poses. Motion-based approaches instead suffer from drift error, scale ambiguity, and motion degeneracy. Securing both accuracy and usability, we propose a novel calibration method that leverages pure rotational motion, requiring only a single static calibration board. The key idea is to make all cameras sequentially observe the same target under a shared geometric reference, even without overlapping views. To integrate these time-separated observations, we formulate the problem using a latent turntable frame and a 3D error on SE(3) within a global optimization framework. We validate the proposed method on both a controlled camera rig and a full-scale vehicle platform with heterogeneous cameras, and analyze robustness under non-ideal turntable motion. Extensive experiments show that our approach maintains competitive accuracy without specialized precision hardware, proving its strong suitability for realistic on-site deployments. Our code is publicly available here.

</details>


### [46] [Learning to Adapt: Reptile-D-Learning for Robust and Efficient Control Under Parametric Uncertainty](https://arxiv.org/abs/2606.25659)
*Haipeng Cao,Zhaolong Shen,Quan Quan*

Main category: cs.RO

TL;DR: Reptile-D-learning leverages Reptile meta-learning to capture shared dynamical structures across systems with different parameters, enabling learning of a generalizable Lyapunov network initialization and a high-performance controller that significantly improves generalization and rapid adaptation to unseen parameter configurations.


<details>
  <summary>Details</summary>
Motivation: Learning-based Lyapunov Control (LLC) provides formal stability guarantees but relies heavily on accurate system models; parameter variations and uncertainties may invalidate stability constraints, leading to costly retraining. While D-learning can estimate Lyapunov derivatives without explicit dynamics models, it remains limited by single-task dynamics and degrades under large parameter shifts.

Method: The authors propose Reptile-D-learning, a framework that leverages the Reptile meta-learning algorithm to capture shared dynamical structures across systems with different parameters, thereby learning a generalizable Lyapunov network initialization and a high-performance controller.

Result: Experiments on multiple nonlinear control systems demonstrate that Reptile-D-learning significantly improves both generalization and rapid adaptation to unseen parameter configurations.

Conclusion: Reptile-D-learning effectively addresses the limitations of traditional D-learning by using meta-learning to learn shared dynamical structures, enabling better generalization and adaptation to parameter variations in nonlinear control systems.

Abstract: Learning-based Lyapunov Control (LLC) provides formal stability guarantees for nonlinear systems, but its validity relies heavily on accurate system models. Parameter variations and uncertainties may invalidate stability constraints, leading to costly retraining. Although D-learning can estimate Lyapunov derivatives without relying on explicit dynamics models, it remains limited by single-task dynamics and degrades under large parameter shifts. We propose Reptile-D-learning, a framework that leverages the Reptile meta-learning algorithm to capture shared dynamical structures across systems with different parameters, thereby learning a generalizable Lyapunov network initialization and a high-performance controller. Experiments on multiple nonlinear control systems demonstrate that Reptile-D-learning significantly improves both generalization and rapid adaptation to unseen parameter configurations.

</details>


### [47] [Power-Budgeted Underwater Vehicle Control via Constrained Reinforcement Learning](https://arxiv.org/abs/2606.25680)
*Yinuo Wang,Gavin Tao,Yuze Liu*

Main category: cs.RO

TL;DR: This paper formulates energy-efficient underwater vehicle control as a constrained Markov decision process (CMDP) with an explicit power budget constraint, solved via PPO-Lagrangian algorithm, achieving 14-65% power reduction without sacrificing task accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional RL controllers optimize task accuracy alone, leading to oscillatory, energy-wasting actuation. Existing remedies subtract energy penalties from rewards, but this approach lacks physical units for power specification, requires per-vehicle/task weight re-tuning, and can even increase power when weights are mismatched.

Method: Formulates energy-efficient underwater control as a constrained Markov decision process where average thruster power is explicitly bounded. Solved using PPO-Lagrangian algorithm with a dual variable updated online to meet the power budget without manual weight search.

Result: Across three vehicles and four tasks in MarineGym simulator, the energy-constrained policy draws the least power in all settings (14-65% reduction over task-only baseline, below energy-reward baseline), is smoothest in ten settings, and preserves task accuracy except in one power-limited regime.

Conclusion: Imposing energy as an explicit constraint provides a tuning-free route to energy-efficient underwater control that eliminates the need for per-vehicle and per-task weight search.

Abstract: Underwater vehicles operate from a fixed onboard energy budget that propulsion rapidly depletes, so a controller that completes its task while drawing less thruster power directly extends mission range and endurance. Reinforcement learning yields capable model-free controllers for station-keeping and trajectory tracking, but optimizing task accuracy alone drives the policy toward oscillatory, energy-wasting actuation. The established remedy subtracts an energy penalty from the reward, yet this sets the task-power trade-off through a single weight with no physical units: a target power level cannot be specified, the weight must be re-tuned for every vehicle and task, and a mismatched weight can even raise power. This paper instead formulates energy-efficient underwater control as a constrained Markov decision process in which average thruster power is subject to an explicit budget, solved with a PPO-Lagrangian algorithm. The power level is set by declaring a budget in physical units, and a single dual variable is updated online to meet it for each vehicle and task, without manual weight search. Across three vehicles and four tasks in the MarineGym simulator, the energy-constrained policy draws the least power in all twelve settings, reducing it by 14--65\% (up to 64.9\%) over a task-only baseline and below an energy-reward baseline everywhere, while remaining the smoothest in ten settings and preserving task accuracy except in one deliberately power-limited regime. Imposing energy as an explicit constraint thus offers a tuning-free route to energy-efficient underwater control that needs no per-vehicle, per-task weight search.

</details>


### [48] [SA-LIVO: Efficient LiDAR-Inertial-Visual Odometry with Subspace-Aware Degeneracy Handling](https://arxiv.org/abs/2606.25699)
*Yinong Cao,Xin He,Yuwei Chen,Shijie Liu,Chunlai Li,Jianyu Wang*

Main category: cs.RO

TL;DR: SA-LIVO is a LiDAR-inertial-visual odometry system that uses Subspace-Aware Information Fusion (SAIF) to handle sensor failures by eigendecomposing the joint information matrix and applying direction-selective fusion, achieving competitive accuracy with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Tightly coupled LiDAR-visual-inertial odometry fuses precise geometric depth with complementary visual measurements, but sensors face independent failure modes: LiDAR degenerates under constrained scan geometry, while visual measurements degrade under adverse illumination or texture absence. Existing countermeasures operate at the modality level and fail to address the direction-dependent structure of the joint information matrix, causing visual residuals to enter pose directions where LiDAR is well-constrained or disperse visual compensation across the full state space in deficient directions.

Method: The Subspace-Aware Information Fusion (SAIF) framework eigendecomposes the joint LiDAR-visual information matrix and applies a linear-clamp soft gate per eigendirection to attenuate degenerate directions while preserving observable ones. LiDAR and visual residuals are jointly optimized in one InEKF loop at a shared linearization point. Photometric Jacobians are assembled once before the loop and reused across iterations to avoid per-iteration computational costs.

Result: Experiments on 29 sequences from HILTI'22, New College, and Oxford Spires benchmarks show competitive accuracy with strongest baselines and bounded drift where competing systems diverge. SA-LIVO achieves 12.3 ms per frame on a laptop CPU and 26.8 ms on an embedded ARM board without GPU, with 3.6-6.3x lower peak memory.

Conclusion: By addressing the direction-dependent structure of the joint information matrix through eigendirection-selective fusion, SA-LIVO overcomes limitations of existing modality-level countermeasures. The system achieves robust performance across diverse benchmarks while demonstrating significant computational efficiency improvements, making it suitable for both laptop and embedded deployment.

Abstract: Tightly coupled LiDAR-visual-inertial odometry (LIVO) fuses precise geometric depth with complementary visual measurements, yet its exteroceptive sensors face independent failure modes: LiDAR degenerates when scan geometry is under-constrained, while visual measurements degrade under adverse illumination or texture absence. Existing countermeasures, including binary degeneracy detection, covariance inflation, and scene-level quality gating, operate at the modality level and leave the direction-dependent structure of the joint information matrix unaddressed. Consequently, visual residuals enter pose directions where LiDAR is well-constrained, while in deficient directions visual compensation disperses across the full state space rather than concentrating where needed. We propose SA-LIVO, a LiDAR-inertial-visual odometry system addressing these limitations through direction-selective fusion and information-efficient processing. The Subspace-Aware Information Fusion (SAIF) framework eigendecomposes the joint LiDAR-visual information matrix and applies a linear-clamp soft gate per eigendirection, attenuating degenerate directions while preserving observable ones at full strength. LiDAR and visual residuals are then jointly optimized in one InEKF loop at a shared linearization point. Since visual information contributes only where LiDAR is deficient, photometric Jacobians are assembled once before the loop and reused across iterations, avoiding the per-iteration cost of conventional iterated filters. Experiments on 29 sequences from three benchmarks (HILTI'22, New College, Oxford Spires) and concurrent-degradation scenarios show accuracy competitive with the strongest baselines and bounded drift where competing systems diverge. SA-LIVO averages 12.3 ms per frame on a laptop CPU and 26.8 ms on an embedded ARM board without GPU, with 3.6-6.3x lower peak memory. The code will be open-sourced.

</details>


### [49] [Learning Asynchronous Upper-body Task-space Trajectory Tracking Policy for Humanoid Robots](https://arxiv.org/abs/2606.25706)
*Yumeng Liu,Dongqi Wang,Jiyu Yu,Yijun Fan,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: An asynchronous upper body task-space tracking framework for humanoids that reduces frame drift and temporal asynchrony between sparse, low-rate planners and high-frequency controllers using teacher-student distillation, MPC completion, and self-guidance mechanisms.


<details>
  <summary>Details</summary>
Motivation: High-level humanoid planners output sparse, low-rate task-space trajectories, while whole-body controllers operate at high frequency, creating temporal asynchrony and structural incompleteness that degrades tracking performance.

Method: A student policy is initialized via teacher-student distillation, conditioned on cached future trajectories and execution-time indices, and trained with sliding-window global rewards to minimize frame drift. For task-specific adaptation, an MPC module completes sparse references into floating-base and upper-body guidance, while action- and FK-level self-guidance constrains policy drift during execution.

Result: The proposed framework achieves improved tracking under low update rates, outperforms both synchronous and decoupled baselines, and demonstrates safer adaptation to out-of-distribution motions in both simulation and Unitree G1 hardware experiments.

Conclusion: The asynchronous framework effectively resolves the temporal mismatch between planning and execution, providing a robust solution for humanoid control that maintains performance even with sparse, low-rate reference trajectories.

Abstract: High-level humanoid planners often output sparse task-space, low-rate trajectories, whereas whole-body controllers run at high frequency. This creates temporal asynchrony between the planning and execution, and structural incompleteness for full-body control. We propose an asynchronous upper body task-space tracking framework for humanoids. A student policy is initialized by teacher-student distillation, conditioned on the full cached future trajectory and an execution-time index, and trained with a sliding-window global reward to reduce frame drift without explicit frame estimation. For task-specific post-training, an MPC module completes sparse references into floating-base and upper-body guidance, while action- and FK level self-guidance constrain policy drift. Simulation and Unitree G1 hardware experiments show improved tracking under low update rates, stronger performance than synchronous and decoupled baselines, and safer adaptation to out-of-distribution motions.

</details>


### [50] [Stage-Aware and Roughness-Constrained Diffusion Policy for Multi-Stage Robotic Polishing](https://arxiv.org/abs/2606.25754)
*Shuai Ke,Jiexin Zhang,Huan Zhao,Zhiao Wei,Yikun Guo,Tiange Wu,Guoqiang Guo,Haoyuan Zhou,Jie Pan,Han Ding*

Main category: cs.RO

TL;DR: A Stage-Aware and Roughness-Constrained Diffusion Policy (SRDP) is proposed for robotic polishing in high-end manufacturing, which infers process stages from multimodal observations and uses roughness-constrained diffusion sampling to generate consistent action trajectories without external stage labels.


<details>
  <summary>Details</summary>
Motivation: Robotic imitation learning for industrial polishing faces challenges including long-horizon dependencies, uncertain stage transitions, and difficulty in modeling and regulating coupled process parameters, limiting its application in high-end manufacturing fields like aerospace.

Method: SRDP infers process-stage posterior from multimodal observation histories to condition a shared reverse denoising process, enabling stage-consistent action generation without external stage labels. A roughness-oriented process-constrained diffusion sampling method generates constrained feed speed and normal contact force under stage-wise preset spindle speeds.

Result: Systematic experiments on spacecraft cabin coating-surface polishing and inner-cavity structural surface finishing show SRDP improves stage-transition stability, process-parameter consistency, and final surface quality compared to advanced baselines, as validated through ablation studies and real-robot experiments.

Conclusion: SRDP effectively addresses the limitations of current robotic polishing methods by achieving better stage-transition stability, process-parameter consistency, and surface quality across different polishing scenarios, demonstrating practical value for high-end manufacturing applications.

Abstract: Polishing is a critical finishing process in high-end manufacturing fields such as aerospace, where surface quality directly affects the service performance and reliability of components. Robotic imitation learning provides a flexible solution for such tasks, but current methods remain limited in industrial polishing because of long-horizon dependencies, uncertain stage transitions, and the difficulty of modeling and regulating coupled process parameters. To address these issues, this paper proposes a Stage-Aware and Roughness-Constrained Diffusion Policy (SRDP) for robotic polishing. SRDP infers the process-stage posterior from multimodal observation histories and uses it to condition the shared reverse denoising process, enabling stage-consistent action generation without external stage labels during execution. Furthermore, a roughness-oriented process-constrained diffusion sampling method is incorporated to generate constrained feed speed and normal contact force under stage-wise preset spindle speeds, thereby improving process consistency and physical feasibility. Systematic experiments are conducted on two representative scenarios, namely spacecraft cabin coating-surface polishing and inner-cavity structural surface finishing. Comparisons with advanced baselines, ablation studies, and real-robot validations comprehensively evaluate the proposed method. The results show that SRD improves stage-transition stability, process-parameter consistency, and final surface quality across different polishing scenarios.

</details>


### [51] [StairMaster: Learning to Conquer Risky Hollow Stairs for Agile Quadrupedal Robots](https://arxiv.org/abs/2606.25765)
*Xincheng Tang,Youhan Xie,Zhengjie Shu,Wanyu Li,Lai Jiang,Wenkang Hu,Yitong Li,Ruigang Yang*

Main category: cs.RO

TL;DR: StairMaster is a three-stage reinforcement learning framework that enables quadruped robots to climb hollow stairs with up to 55° incline by integrating Cross-Attention mechanisms, Spatial-aware Recurrent Units, high-fidelity depth sensor modeling, and proactive sensing rewards.


<details>
  <summary>Details</summary>
Motivation: Climbing hollow stairs is extremely challenging for quadrupeds due to leg trapping risk, severe depth sparsity, and high-frequency depth-sensing noise.

Method: A three-stage RL framework featuring Cross-Attention for structural feature extraction from noisy depth data, Spatial-aware Recurrent Unit (SRU) for spatio-temporal memory, high-fidelity sim-to-real depth sensor modeling pipeline, 3D waypoint-guided active perception reward, and hollow gap/stair edge penalties.

Result: Successfully deployed on a Unitree Go2 robot, achieving zero-shot transfer and unprecedented 55° incline hollow stair climbing capability.

Conclusion: This is the first RL-based policy to achieve such steep hollow stair climbing in real-world environments.

Abstract: Climbing hollow stairs remains a challenging problem for quadruped robots due to the high risk of leg trapping, severe depth sparsity, and high-frequency depth-sensing noise. In this paper, we propose StairMaster, a novel three-stage reinforcement learning framework for stable locomotion on such extreme discontinuous terrains. Our architecture integrates a Cross-Attention mechanism to extract structural features from noisy depth data, alongside a Spatial-aware Recurrent Unit (SRU) that maintains robust spatio-temporal memory to mitigate perception blind spots. To bridge the sim-to-real gap in depth perception, we propose a high-fidelity sim-to-real depth sensor modeling pipeline that faithfully replicates real-world sensor artifacts. Additionally, we employ a 3D waypoint-guided active perception reward for proactive sensing, alongside hollow gap kinematic and stair edge penalties to ensure precise foothold placement. We successfully deployed StairMaster on a Unitree Go2 robot, demonstrating its ability to conquer hollow stairs with an unprecedented incline of up to 55$^\circ$ through zero-shot transfer. To the best of our knowledge, this is the first RL-based policy to achieve such steep hollow stair climbing in real-world environments.
  Project Website: https://sivan666666.github.io/StairMaster/.

</details>


### [52] [MIL-LC: A Robust Magnetometer-Inertial-LiDAR Fusion Multimodal Localization Framework](https://arxiv.org/abs/2606.25796)
*Qiyang Lyu,Zhenyu Wu,Wei Wang,Hongming Shen,Danwei Wang*

Main category: cs.RO

TL;DR: This paper proposes a magnetometer-inertial-LiDAR fused localization framework (MIL-LC) for autonomous mobile robots in challenging GNSS-denied and textureless environments, using a custom-designed sensor suite that maintains robust performance despite LiDAR geometric degeneration or magnetic map changes.


<details>
  <summary>Details</summary>
Motivation: Localization in challenging environments (GNSS-denied, repetitive, textureless) is an open problem. Single-modality methods are limited, while existing multimodal fusion relies on geometric/texture features or infrastructure. Ambient magnetic field (AMF) is promising but only explored in pedestrian scenarios. Practical solutions for AMR systems remain unexplored.

Method: Proposes MIL-LC: a magnetometer-inertial-LiDAR fused multimodal localization framework with a custom-designed sensor suite, designed to handle LiDAR geometric degeneration and magnetic map changes during long-term deployment.

Result: Extensive experiments in both simulation and real-world environments demonstrate that the MIL-LC framework achieves robust and accurate localization performance.

Conclusion: The proposed MIL-LC framework provides reliable localization capabilities for AMR systems even under challenging conditions where LiDAR suffers from geometric degeneration or when magnetic maps change, validated through comprehensive experimental results.

Abstract: Localization in challenging environments, such as GNSS-denied, geometrically repetitive, or textureless scenes commonly found in offices, hotels, and underground parking facilities, remains an open problem for reliable autonomous mobile robot (AMR) deployment. Single-modality localization methods are inherently limited by the constraints of individual sensors. Although multimodal fusion frameworks have shown improved robustness, most existing approaches still rely heavily on geometric or texture features, or on infrastructure-based beacons, which increase installation and maintenance costs while reducing deployment flexibility. Recently, ambient magnetic field (AMF)-based localization has attracted growing attention because it does not depend on geometric or texture features, nor does it require additional infrastructure, making it a promising complementary modality for AMR localization. However, existing studies have only explored such fusion in pedestrian scenarios using smartphone-mounted sensor suites, and practical solutions for AMR systems remain largely unexplored. To address this gap, this article proposes a magnetometer-inertial-LiDAR fused multimodal localization framework with a custom-designed sensor suite, termed MIL-LC, which provides reliable localization even when LiDAR suffers from geometric degeneration or when the magnetic map changes during long-term deployment. Extensive experiments in both simulation and real-world environments demonstrate that the proposed MIL-LC framework achieves robust and accurate localization performance.

</details>


### [53] [Beyond a Shadow of a Doubt: Close Proximity Geometry Reconstruction Using FMCW Radar Shadow Effects](https://arxiv.org/abs/2606.25829)
*Felix de Trogoff du Boisguezennec,Benjamin Ramtoula,Daniele De Martini*

Main category: cs.RO

TL;DR: This paper proposes a novel method to recover the 3D in-plane inclination of slender vertical objects using millimetre-wave FMCW radar by exploiting vehicle chassis shadows as a geometric cue.


<details>
  <summary>Details</summary>
Motivation: Reliable perception in adverse conditions remains challenging for autonomous systems due to camera and LiDAR degradation in poor lighting or weather. While millimetre-wave FMCW radar is robust to such conditions, its elevation collapse limits geometric reasoning. The researchers observe that vehicle chassis occlude radar rays and form distinctive geometric shadows, which can be leveraged to infer information about objects whose returns intersect these shadows.

Method: The method establishes an analytical, closed-form mapping between the radar return boundaries and the opening angle to retrieve the object inclination without assumptions about the wider scene. Validation is conducted through simulation and experimentation with a Navtech CTS350-X radar.

Result: Inclinations can be estimated under practical conditions, with segmentation of the object in the radar scan identified as the main bottleneck.

Conclusion: This work highlights chassis shadows as a novel geometric cue, extending the role of 2D rotating radar beyond localisation toward 3D scene reconstruction.

Abstract: Reliable perception in adverse conditions remains challenging for autonomous systems, as cameras and LiDAR degrade in poor lighting or weather. Millimetre-wave FMCW radar is robust to such conditions, but its elevation collapse limits geometric reasoning. We observe that vehicle chassis occlude radar rays and form a distinctive geometric shadow, and its consistency can enable us to infer useful information about objects whose returns intersect this shadow. Motivated by this observation, we propose a method to recover the 3D, in-plane inclination of nearby slender vertical objects from this cue. The object inclination is retrieved without assumptions about the wider scene, but through an analytical, closed-form mapping between its radar return boundaries and the opening angle. Validation in simulation and experimentation on a Navtech CTS350-X radar shows that inclinations can be estimated under practical conditions, with segmentation of the object in the radar scan emerging as the main bottleneck. This work highlights chassis shadows as a novel geometric cue, extending the role of 2D rotating radar beyond localisation and toward 3D scene reconstruction.

</details>


### [54] [TacVerse: A Multi-Sensor Dataset and Benchmark for Cross-Sensor Vision-Based Tactile Perception](https://arxiv.org/abs/2606.25877)
*Lan Wei,Gurmeher Khurana,Sirine Bhouri,Wenhao Hong,Zeyuan Xin,Qingzheng Cong,Wen Fan,Yanzheng Xiang,Dandan Zhang*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Vision-based tactile sensors (VBTSs) enable robots to infer contact geometry and force-related cues by imaging deformation through an internal camera, yet generalisation across sensor designs remains poorly understood. We present TacVerse, a multi-sensor dataset and benchmark for cross-sensor vision-based tactile perception. The dataset contains 106,800 tactile images from seven VBTSs and supports three downstream tasks: shape classification, grating classification, and force regression. Experiments are conducted under three settings: within-sensor training, zero-shot cross-sensor transfer, and few-shot adaptation. Strong within-sensor performance across all tasks indicates that the collected tactile observations are informative for the target objectives. Direct cross-sensor transfer, however, leads to substantial degradation. Shape classification is comparatively robust, whereas grating classification and force regression are more sensitive to sensor shift. Few-shot adaptation for force regression consistently improves performance on unseen target sensors but does not fully close the gap to within-sensor upper bounds. A representation study further shows that MAE (Masked Autoencoder) pretraining provides the most consistent gains across tasks and sensors. TacVerse provides a controlled testbed for studying sensor shift, data-efficient adaptation, and self-supervised learning in tactile perception.

</details>


### [55] [A 3D-Printable Dataset for Fair Testing and Comparisons of Tactile Sensors](https://arxiv.org/abs/2606.25886)
*Dexter R. Shepherd,Nicolas Herzig,Phil Husbands,Andrew Philippides,Chris Johnson,William Kimbell*

Main category: cs.RO

TL;DR: A 3D-printable tactile texture dataset with six mathematically defined patterns to enable fair, reproducible comparison of tactile sensors across different 3D printers and filaments.


<details>
  <summary>Details</summary>
Motivation: Existing tactile texture datasets use sensor-specific readings from arbitrary surfaces, limiting fair sensor comparison and reproducibility. The authors aim to create a benchmark that can be physically reproduced across different hardware configurations.

Method: Six parametric surface patterns generated from sine-wave and Fourier functions with controlled spatial frequency, amplitude, and directional variation. Evaluated across three 3D printers and multiple filaments using optical TacTip sensor images. Classification experiments used neural networks and PCA-based models to assess generalisation.

Result: Print quality (peak sharpness, stringing) significantly affects tactile variance; higher-end printers produce more consistent signatures. High-quality prints support strong within-printer generalisation, but cross-printer generalisation remains challenging due to geometric inconsistencies.

Conclusion: Establishes the first openly available, physically reproducible 3D-printed texture benchmark, providing a foundation for fair tactile sensor comparison and enabling reproducible research in tactile sensing.

Abstract: Existing texture datasets for tactile sensing primarily consist of sensor readings from a specific sensor interacting with available surfaces/objects rather than describing the textures themselves, limiting fair comparison between tactile sensors and hindering reproducible research. In this work, we introduce a 3D-printable dataset of mathematically defined textures designed to be fabricated reliably across different printers and filament types. The dataset consists of six parametrically generated surface patterns derived from combinations of sine-wave and Fourier-based functions, giving controlled variation in spatial frequency, amplitude, and directional structure. We evaluate the reproducibility of these textures across three popular 3D printers and multiple filament types by measuring variance in images captured using an optical TacTip sensor under controlled contact conditions. Our results show that print quality, particularly peak sharpness and stringing, affects tactile variance, with higher-end printers producing significantly more consistent signatures. Classification experiments using neural networks and PCA-based models further demonstrate that high-quality prints support strong within-printer generalisation, while cross-printer generalisation remains challenging due to geometric inconsistencies. This work establishes the first openly available, physically reproducible 3D-printed texture benchmark, providing a foundation for fair comparison of tactile sensors.

</details>


### [56] [DeformGen: Dynamics-Based Topology Augmentation for Deformable Manipulation Policy Learning](https://arxiv.org/abs/2606.25939)
*Zili Lin,Wenyao Zhang,Yuyang Zhang,Zekun Qi,Junyan Lin,Hanxin Zhu,Jiaolong Yang,Zhibo Chen,Yao Mu,Xiaokang Yang,Xin Jin,Wenjun Zeng*

Main category: cs.RO

TL;DR: DeformGen is a dynamics-based augmentation framework that addresses state space and trajectory transfer challenges in deformable manipulation by expanding valid state distributions via localized physical disturbances and forward simulation, while enabling topology-coherent trajectory transfer through deformation-field warping, demonstrating improved policy learning over rigid-style augmentation baselines.


<details>
  <summary>Details</summary>
Motivation: Existing demonstration augmentation methods are fundamentally limited for deformable manipulation due to: (1) high-dimensional state space with physics-induced constraints making valid configurations unreachable via low-dimensional pose perturbations, and (2) trajectory transfer non-equivariance where material points no longer move rigidly together under deformation.

Method: DeformGen tackles both challenges through: (1) state augmentation - applying localized physical disturbances and forward-simulating dynamics to obtain topology-coherent, physically plausible deformable states, expanding the valid state distribution; (2) trajectory augmentation - transferring source manipulation trajectories via deformation-field warping, lifting per-particle displacements into continuous spatial functions to adapt end-effector trajectories consistently with deformed geometry.

Result: Experiments on high-fidelity deformable manipulation benchmarks demonstrate that DeformGen generally improves policy learning compared to training on original demonstrations alone and rigid-style augmentation baselines.

Conclusion: By jointly augmenting the state distribution and its associated manipulation behavior through physics-based state expansion and deformation-field warping, DeformGen achieves topological diversity for deformable objects, demonstrating the effectiveness of dynamics-aware augmentation approaches.

Abstract: Demonstration augmentation is proposed for cost-efficient data acquisition, but existing methods are fundamentally limited in deformable manipulation due to two challenges: (1) the state space is high-dimensional with physics-induced constraints, making valid configurations impossible to reach via low-dimensional pose perturbations; and (2) trajectory transfer is non-equivariant, as material points no longer move rigidly together under deformation. We present DeformGen, a dynamics-based augmentation framework that achieves topological diversity for deformable objects. For the state challenge, DeformGen expands the valid state distribution by applying localized physical disturbances and forward-simulating the dynamics to obtain topology-coherent, physically plausible deformable states. For the trajectory challenge, DeformGen transfers source manipulation trajectories via deformation-field warping, which lifts per-particle displacements into a continuous spatial function to adapt the end-effector trajectory consistently with the deformed geometry. In this way, our method jointly augments the state distribution and its associated manipulation behavior. Experiments on high-fidelity deformable manipulation benchmarks show that DeformGen generally improves policy learning compared with training on the original demonstrations alone and with rigid-style augmentation baselines.

</details>


### [57] [DSP-SLAM++: A Unified Framework for Multi-Class, High-Fidelity Object SLAM in the Wild](https://arxiv.org/abs/2606.25953)
*Ahmad Kourani,Ghina Daoud,Daniel Asmar,Imad Elhajj*

Main category: cs.RO

TL;DR: DSP-SLAM++ extends the DSP-SLAM framework with an asynchronous mapping pipeline and sensor fusion adaptations for monocular fisheye-LiDAR, enabling real-time multi-class object SLAM with reduced processing latency.


<details>
  <summary>Details</summary>
Motivation: Existing object-aware SLAM systems face a difficult trade-off between real-time performance, multi-class support, and high-fidelity semantically coherent object model generation.

Method: The authors extended the DSP-SLAM framework by adding an asynchronous mapping pipeline for real-time performance and dedicated sensor fusion adaptations specifically for a monocular fisheye-LiDAR sensor suite.

Result: The system generates fine-grained, geometrically-complete shapes for multiple object classes while reducing maximum object processing latency by up to 70% compared to state-of-the-art baselines, achieving robust real-time performance on a 25 Hz multi-class dataset.

Conclusion: This work makes high-fidelity, multi-class object SLAM more practical for real-world applications such as autonomous driving and robotic manipulation on platforms with common fisheye-LiDAR sensor setups.

Abstract: Existing object-aware SLAM systems force a trade-off between real-time performance, multi-class support, and the generation of high-fidelity, semantically coherent object models. To address this trade-off, we present DSP-SLAM++, which extends the DSP-SLAM framework with an asynchronous mapping pipeline for real-time performance and dedicated sensor fusion adaptations for a monocular fisheye-LiDAR suite. Experiments demonstrate that our system generates fine-grained, geometrically-complete shapes for multiple object classes while eliminating severe mapping thread bottlenecks by reducing maximum object processing latency by up to 70\% compared to the state-of-the-art baseline, enabling robust, real-time performance on a challenging 25 Hz multi-class datasets. This work makes high-fidelity, multi-class object SLAM more practical for real-world applications like autonomous driving and robotic manipulation by enabling its use on platforms with common fisheye-LiDAR sensor setups. The open-source code is available at: [github.com/AUBVRL/DSP-SLAMpp].

</details>


### [58] [From Rubble Simulation to Active Magnetic Mapping: Quantum Sensing for Disaster Response](https://arxiv.org/abs/2606.25957)
*Samuel Tovey,Stefan Prestel,Hiroshi Yamauchi*

Main category: cs.RO

TL;DR: This paper proposes drone-based quantum magnetometry as a complementary modality for locating survivors in building collapses, using a simulation pipeline to demonstrate recoverable magnetic structures from rubble debris.


<details>
  <summary>Details</summary>
Motivation: Locating survivors within the first 72 hours of building collapses is critical, but existing sensing modalities provide only partial information about the structure beneath rubble.

Method: Develops a simulation pipeline spanning rubble physics, sensor-array deployment, and active spatial reconstruction using Unreal Engine to generate a steel-reinforced concrete parking-garage collapse, computing induced magnetic fields via per-triangle dipole approximation, and employing Gaussian Process Regression with Bayesian active sampling for sparse multi-sensor data processing.

Result: Meaningful magnetic structure is recoverable in the sub-pT to sub-nN range from roughly 1 m above the roofline; a three-sensor array optimizes the trade-off between gradient resolution and UAV payload constraints; active sampling reaches peak structural correlation in approximately 100 samples.

Conclusion: Quantum-grade sensing could become a useful tool for drone-based structural analysis and potentially void detection in collapsed buildings.

Abstract: Locating survivors of building collapses within the first 72 hours is a critical challenge in disaster response, and existing sensing modalities provide only partial information about the structure beneath the rubble.
  This paper proposes drone-based quantum magnetometry as a complementary modality and develops a simulation pipeline spanning rubble physics, sensor-array deployment, and active spatial reconstruction. We use Unreal Engine to generate a steel-reinforced concrete parking-garage collapse and compute the induced magnetic field via a per-triangle dipole approximation, establishing that meaningful magnetic structure is recoverable in the sub-pT to sub-nT range from roughly 1 m above the roofline. Then, we feed sparse multi-sensor samples into a Gaussian Process Regression back-end driven by Bayesian active sampling and validate the pipeline across multiple independent collapse realizations; a three-sensor array optimizes the trade-off between gradient resolution and UAV payload constraints, and active sampling reaches peak structural correlation in roughly $100$ samples. Together, these results indicate that quantum-grade sensing could become a useful tool for drone-based structural analysis and potentially void detection in collapsed buildings.

</details>


### [59] [Mixture-of-Experts RL for Fault-Tolerant Legged Locomotion](https://arxiv.org/abs/2606.25965)
*Giulio Turrisi,Ozan Pali,Luca Oneto,Claudio Semini*

Main category: cs.RO

TL;DR: This paper proposes a fault-aware modular control architecture for legged robots that uses fault-diagnosis information to activate specialized control experts for different actuator failure modes. The modular approach outperforms monolithic policies while being more efficient and suitable for compute-constrained platforms.


<details>
  <summary>Details</summary>
Motivation: Legged robots deployed in planetary exploration and remote environments must maintain reliable locomotion despite actuator failures and challenging terrain conditions. Monolithic reinforcement learning policies struggle to efficiently represent the diverse control strategies required to compensate for different fault conditions.

Method: The authors propose a fault-aware modular control architecture that explicitly leverages fault-diagnosis information to activate specialized control experts associated with distinct actuator failure modes, creating fault-conditioned modular policies.

Result: Experimental results show that explicit fault-conditioned modular policies consistently outperform monolithic policies of comparable size, achieving higher locomotion performance across failure scenarios. The modular architecture retains competitive performance even under significantly reduced network capacity.

Conclusion: The proposed modular architecture is more efficient and effective than monolithic policies for handling actuator failures, making it particularly suitable for compute-constrained robotic platforms such as those typically employed in space applications.

Abstract: Legged robots deployed in planetary exploration and other remote environments must maintain reliable locomotion despite actuator failures and challenging terrain conditions. Although reinforcement learning has achieved strong results in legged locomotion, monolithic policies can struggle to efficiently represent the diverse control strategies required to compensate for different fault conditions. In this work, we propose a fault-aware modular control architecture that explicitly leverages fault-diagnosis information to activate specialized control experts associated with distinct actuator failure modes. Experimental results show that explicit fault-conditioned modular policies consistently outperform monolithic policies of comparable size, achieving higher locomotion performance across failure scenarios. Moreover, the proposed modular architecture retains competitive performance even under significantly reduced network capacity, highlighting its suitability for compute-constrained robotic platforms, such as those typically employed in space applications. The code associated with this work is available at: https://github.com/iit-DLSLab/fault-locomotion-isaaclab.

</details>


### [60] [A Sensorised Lattice Footplate for a Semi-Active Prosthetic Foot](https://arxiv.org/abs/2606.25966)
*Jinze Ge,Jingcheng Sun,Chengxu Zhou*

Main category: cs.RO

TL;DR: The paper investigates embedding magnetic plantar sensing directly into the load-bearing compliant element of a low-cost semi-active prosthetic foot. A prototype integrating a sensorized 3D-printed lattice footplate, servo-adjustable hydraulic damper, and reduced-order ankle model demonstrates tunable lattice stiffness, accurate plantar-force estimation, separable forefoot/rearfoot loading distributions, and a feedforward damping schedule approximating dorsiflexion trends (though active push-off cannot be generated with purely dissipative mechanisms).


<details>
  <summary>Details</summary>
Motivation: To investigate whether magnetic plantar sensing can be embedded directly inside the load-bearing compliant element of a low-cost semi-active prosthetic foot, exploring an integrated approach that combines sensing with mechanical compliance.

Method: Developing a prototype with a sensorized 3D-printed lattice footplate, servo-adjustable hydraulic damper, and reduced-order ankle model. Experimental characterization of damper (adjustment angle to damping coefficient), controlled compression tests for tunable lattice stiffness, cyclic normal loading to verify sensor accuracy, static-posture trials at body-weight loading, and testing a feedforward damping schedule.

Result: The system demonstrates tunable lattice stiffness, embedded sensor accurately tracks reference force during cyclic loading, forefoot/rearfoot loading distributions are separable across stance configurations, and a feedforward damping schedule approximates dorsiflexion trends through early-to-mid stance. A limitation is that purely dissipative mechanisms cannot generate active push-off.

Conclusion: Sensing can be embedded inside the load-bearing compliant element of a prosthetic foot and used to drive semi-active damping, demonstrating a viable approach for integrated prosthetic foot design where sensing and mechanical function are combined.

Abstract: This paper investigates whether magnetic plantar sensing can be embedded directly inside the load-bearing compliant element of a low-cost semi-active prosthetic foot. We present a prototype integrating a sensorised 3D-printed lattice footplate, a servo-adjustable hydraulic damper, and a reduced-order ankle model. The damper is experimentally characterised to relate adjustment angle to damping coefficient. Controlled compression tests show tunable lattice stiffness, while cyclic normal loading shows that the embedded sensor tracks the testing-machine reference force, supporting plantar-force estimation without an external insole layer. Static-posture trials under approximately body-weight loading show that forefoot and rearfoot loading distributions are separable across four prescribed stance configurations, providing a preliminary check of the sensing pipeline. A feedforward damping schedule approximates the dorsiflexion trend of a reference ankle trajectory through early-to-mid stance, while exposing the expected limitation that a purely dissipative mechanism cannot generate active push-off. Together, these results demonstrate that sensing can be embedded inside the load-bearing compliant element of a prosthetic foot and used to drive semi-active damping.

</details>


### [61] [Action ControlNet: A Lightweight Delay-Aware Adapter for Smooth Asynchronous Control in Vision-Language-Action Models](https://arxiv.org/abs/2606.25985)
*Tiecheng Guo,Meng Guo*

Main category: cs.RO

TL;DR: Action ControlNet (ACNet) introduces a lightweight delay-aware adapter for vision-language-action models that uses executed motion suffix as a residual condition for a frozen action head, improving robustness and smoothness in asynchronous control under inference delay.


<details>
  <summary>Details</summary>
Motivation: Vision-language-action (VLA) models show strong potential for general-purpose robot manipulation, but inference latency is a major obstacle to stable high-frequency control. Asynchronous execution mitigates this by overlapping policy inference with action execution, but the next action chunk is still predicted from stale observations, causing handoff discontinuities, action jitter, and failures in contact-rich manipulation. Existing remedies typically require full-policy retraining or architecture-specific runtime logic, which are computationally expensive.

Method: ACNet is a lightweight delay-aware adapter that uses the executed motion suffix as a residual condition for a mostly frozen action head. It leaves the pretrained backbone unchanged, introduces few trainable parameters, and remains compatible with generative action heads such as diffusion and flow matching.

Result: On Kinetix, Meta-World MT50, and a real-world SO-ARM101 platform, ACNet improves robustness under inference delay and yields smoother asynchronous trajectories than direct chunk stitching, while remaining more lightweight than full delay-conditioned retraining.

Conclusion: ACNet provides an effective solution to the inference latency problem in VLA models by balancing performance improvement with computational efficiency, and it maintains compatibility with various generative action heads without requiring full-policy retraining or architecture-specific runtime logic.

Abstract: Vision-language-action (VLA) models have shown strong potential for general-purpose robot manipulation, but their inference latency remains a major obstacle to stable high-frequency control. Asynchronous execution mitigates this bottleneck by overlapping policy inference with action execution, yet the next action chunk is still predicted from stale observations while the robot continues to move. Direct chunk stitching therefore introduces handoff discontinuities, action jitter, and failures in contact-rich manipulation. Existing remedies typically require either full-policy retraining or architecture-specific runtime logic. This work proposes Action ControlNet (ACNet), a lightweight delay-aware adapter that uses the executed motion suffix as a residual condition for a mostly frozen action head. ACNet leaves the pretrained backbone unchanged, introduces few trainable parameters, and remains compatible with generative action heads such as diffusion and flow matching. On Kinetix, Meta-World MT50, and a real-world SO-ARM101 platform, ACNet improves robustness under inference delay and yields smoother asynchronous trajectories than direct chunk stitching, while remaining more lightweight than full delay-conditioned retraining.

</details>


### [62] [FORCE: Efficient VLA Reinforcement Fine-Tuning via Value-Calibrated Warm-up and Self-Distillation](https://arxiv.org/abs/2606.26006)
*Shuyi Zhang,Yunfan Lou,Hongyang Cheng,Yichen Guo,Chuyao Fu,Yaoxu Lyu,Xiaojie Zhang,Haoran Li,Pengwei Wang,Zhongyuan Wang,Shanghang Zhang*

Main category: cs.RO

TL;DR: FORCE is a 3-stage framework that stabilizes RL fine-tuning of Vision-Language-Action models by improving Q-function stability and filtering low-quality data, achieving significant improvements in success rates and training speed without human intervention.


<details>
  <summary>Details</summary>
Motivation: VLA models are constrained by imitation ceiling from sub-optimal data. RL fine-tuning can surpass this but is sample inefficient due to catastrophic initial unlearning from unstable Q-function and inefficient policy updates from low-quality exploration data, often requiring costly human interventions.

Method: FORCE introduces two key mechanisms: (1) Value-Calibrated Warm-Up phase using on-policy rollouts to mitigate Q-function distributional shift, and (2) Online stage where the calibrated Q-function filters both the policy's own action proposals and expert data to ensure only high-value actions are used for policy updates.

Result: FORCE achieves 79% absolute improvement in success rates, outperforms prior RL methods by 10%, accelerates training by 32.5%, mitigates common success rate drop, and achieves robust performance without human intervention across various simulation and real-world tasks.

Conclusion: FORCE marks a significant step towards deploying capable and autonomous robotic agents by enabling stable RL fine-tuning of VLA models without the need for human intervention.

Abstract: Vision-Language-Action (VLA) models are often constrained by the imitation ceiling imposed by sub-optimal data. While Reinforcement Learning (RL) fine-tuning can surpass this limit, it is notoriously sample inefficient. This challenge arises from two core issues: (1) catastrophic initial unlearning due to an unstable Q-function and (2) inefficient policy updates caused by low-quality exploration data, often forcing a reliance on costly human interventions. We introduce FORCE, a 3-stage framework that stabilizes fine-tuning by tackling both issues. FORCE first incorporates a Value-Calibrated Warm-Up phase, utilizing on-policy rollouts to mitigate the distributional shift of the Q-function. Subsequently, during the online stage, this calibrated Q-function acts as a filter for both the policy's own action proposals and expert data, ensuring only high-value actions are used for the policy update. We evaluate FORCE on various simulation and real-world tasks, and the result shows that FORCE achieves a 79% absolute improvement in success rates and outperform prior RL methods by 10%, while accelerating training by 32.5%. Critically, it mitigates the common success rate drop and achieves this robust performance without human intervention, marking a significant step towards deploying capable and autonomous robotic agents.

</details>


### [63] [Emcar: Embodied Controller for Animating Robots](https://arxiv.org/abs/2606.26008)
*Carlos Gomez Cubero,Elizabeth Jochum*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: This chapter describes EMCAR, a novel software tool for programming robot motion that leverages the unique affordances of artistic practices such as puppetry and drawing to conceive, design, and program novel interactions and realize new use cases for HRI. The advantage of this no-code platform is that it expands creative applications for collaborative robots - putting robots directly in the hands of artists - and provides an inclusive environment that enables individuals with little or no technical backgrounds to engage meaningfully in collaborations and robotics research.

</details>


### [64] [FAR-LIO: Enabling High-Speed Autonomy through Fast, Accurate, and Robust LiDAR-Inertial Odometry](https://arxiv.org/abs/2606.26010)
*Maximilian Leitenstern,Marcel Weinmann,Patrick Haft,Tobias Lasser,Dominik Kulmer,Markus Lienkamp*

Main category: cs.RO

TL;DR: FAR-LIO is a CUDA-accelerated LiDAR-inertial odometry framework designed for autonomous racing environments, achieving 6.9% reduction in positional error and 38.4% lower runtime compared to state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Robust and accurate odometry estimation is critical in robotics, especially in highly dynamic environments with sensor noise. Autonomous racing combines these challenges in unstructured settings where minimizing odometry latency is essential for stable closed-loop control.

Method: The system employs a novel CUDA-based voxel hashmap for parallelized nearest-neighbor search and efficient map updates, combined with a sparsity-aware Generalized Iterative Closest Point algorithm with adaptive thresholding. An Extended Kalman Filter serves as the backend with upsampling and delay compensation to fuse LiDAR odometry with high-frequency IMU data.

Result: Evaluated across four sensor setups on public datasets and two autonomous racecars (up to 250 km/h), FAR-LIO achieved a 6.9% reduction in positional error and 38.4% lower runtime compared to state-of-the-art baselines using a single parameter set.

Conclusion: FAR-LIO demonstrates high computational efficiency and broad applicability, enabling low-latency accurate odometry suitable for fast autonomous racing while maintaining robustness in dynamic environments. The code is available open-source for further development.

Abstract: Robust and accurate odometry estimation is essential in modern robotics. In environments characterized by highly dynamic motion and sensor noise, odometry estimation becomes increasingly challenging. Autonomous racing combines both factors in an unstructured setting, where minimizing odometry latency is essential for stable closed-loop control. This paper introduces FAR-LIO, a highly optimized CUDA-accelerated LiDAR-inertial odometry framework developed for Fast, Accurate, and Robust performance. Our system leverages a novel CUDA-based voxel hashmap to enable parallelized nearest-neighbor search and efficient map updates. We employ a sparsity-aware Generalized Iterative Closest Point algorithm with adaptive thresholding on top of the CUDA-based voxel hashmap with adaptive density to achieve low-latency without compromising accuracy. An Extended Kalman Filter serves as a robust backend. It utilizes an upsampling and delay compensation strategy to fuse the LiDAR odometry with high-frequency IMU data, thereby ensuring a robust and smooth odometry output. We evaluate FAR-LIO across four different sensor setups, using both public datasets and data from two autonomous racecars driving at speeds of up to 250 km/h. FAR-LIO achieves an average 6.9% reduction in the positional error and 38.4% lower runtime compared to state-of-the-art baselines on target hardware using a single parameter set. This demonstrates its computational efficiency and broad applicability. To build upon our work, our code is available open-source on https://github.com/TUMFTM/FAR-LIO.

</details>


### [65] [G2DP: Diffusion Planning with Spatio-Temporal Grid Guidance](https://arxiv.org/abs/2606.26017)
*Hang Yu,Ye Jin,Alessandro Canevaro,Julian Schmidt,Julian Jordan,Peizheng Li,Marc Kaufeld,Silvan Lindner,Johannes Betz,Wilhelm Stork*

Main category: cs.RO

TL;DR: G2DP is a grid-guided diffusion planner that enforces dense environmental constraints through spatio-temporal cost volumes, achieving state-of-the-art performance in autonomous driving motion planning.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based planners for autonomous driving have stochastic nature requiring explicit guidance for safety and route adherence. Existing guidance methods rely on sparse, entity-centric geometric queries or post-hoc refinement, which yield limited situational awareness and fragile performance in interactive scenes.

Method: G2DP constructs a differentiable spatio-temporal cost volume by fusing probabilistic future occupancy distributions with a route-progress map. This volume is formulated as a continuous safety energy functional that injects dense gradients directly into the denoising loop, actively steering trajectory generation toward collision-free and progress-optimal regions.

Result: G2DP achieves state-of-the-art performance on nuPlan, outperforming the strongest imitation-learning baseline by +7.2 points in reactive score. It maintains top scores in zero-shot transfers to interPlan and DeepScenario benchmarks, with collision avoidance improving by +10.15 over the unguided approach on interPlan.

Conclusion: Spatio-temporal cost grids serve as an effective representation for robust guidance in diffusion-based planning, demonstrating that dense environmental constraints can be effectively enforced during inference-time guidance for safe and efficient autonomous driving.

Abstract: In autonomous driving, diffusion-based planners have emerged as a promising paradigm for robust motion planning in dense and interactive traffic, as they can effectively model diverse driving behaviors. However, their inherent stochasticity often requires explicit guidance during denoising to ensure safety and route adherence for robust closed-loop execution. Existing guidance typically relies on sparse, entity-centric geometric queries or post-hoc refinement, yielding limited situational awareness and fragile performance in interactive scenes. To address this issue, we propose G2DP (Grid-Guided Diffusion Planning), a diffusion-based planner that directly enforces dense environmental constraints through inference-time guidance. Specifically, G2DP constructs a differentiable spatio-temporal cost volume by fusing probabilistic future occupancy distributions with a route-progress map. By formulating this volume as a continuous safety energy functional, it injects dense gradients directly into the denoising loop, actively steering trajectory generation toward collision-free and progress-optimal regions. Extensive closed-loop evaluations show that G2DP achieves state-of-the-art performance on nuPlan, outperforming the strongest imitation-learning baseline by +7.2 points in reactive score. It further maintains top scores in zero-shot transfers to interPlan and DeepScenario benchmarks, with collision avoidance improving by +10.15 over the unguided approach on interPlan. These results demonstrate that spatio-temporal cost grids serve as an effective representation for robust guidance in diffusion-based planning.

</details>


### [66] [In-Context World Modeling for Robotic Control](https://arxiv.org/abs/2606.26025)
*Siyin Wang,Junhao Shi,Senyu Fei,Zhaoyang Fu,Li Ji,Jingjing Gong,Xipeng Qiu*

Main category: cs.RO

TL;DR: ICWM is an in-context adaptation framework that enables robot policies to infer system variables from self-generated interactions, allowing VLA models to adapt to novel camera viewpoints and configurations without parameter updates.


<details>
  <summary>Details</summary>
Motivation: Modern VLA models fail to generalize to novel setups (altered camera viewpoints or robot morphologies) because they only condition on current observations and language instructions, implicitly assuming a fixed execution context encountered during training.

Method: ICWM treats system identification as an in-context adaptation problem. The model processes a short history of self-generated, task-agnostic interactions before task execution to capture world dynamics, enabling adaptation without parameter updates.

Result: Extensive experiments in simulation and on real-world robot platforms demonstrate that ICWM significantly outperforms standard VLA baselines on novel camera viewpoints.

Conclusion: By leveraging the context window to understand how the system operates rather than just what task to perform, ICWM enables robot policies to autonomously infer essential system variables and adapt to novel configurations without requiring data-intensive fine-tuning.

Abstract: Modern Vision-Language-Action (VLA) models often fail to generalize to novel setups, such as altered camera viewpoints or robot morphologies, because they are typically conditioned only on current observations and language instructions. By ignoring the underlying system configuration as a variable, these models implicitly assume a fixed execution context encountered during training, necessitating data-intensive fine-tuning for any new environment. In this work, we introduce In-Context World Modeling (ICWM), a framework that treats system identification as an in-context adaptation problem. ICWM enables robot policies to autonomously infer essential system variables from a short history of self-generated, task-agnostic interactions. Unlike traditional In-Context Learning that uses demonstrations to specify what task to perform, ICWM leverages the context window to understand how the system operates. By processing these interactions before task execution, the model implicitly captures the world dynamics of the current system, enabling adaptation to novel configurations without parameter updates. Extensive experiments in simulation and on real-world robot platforms demonstrate that ICWM significantly outperforms standard VLA baselines on novel camera viewpoints.

</details>


### [67] [RoboAtlas: Contextual Active SLAM](https://arxiv.org/abs/2606.26046)
*Alexander Schperberg,Shivam K. Panda,Abraham P. Vinod,M. K. Jawed,Stefano Di Cairano*

Main category: cs.RO

TL;DR: RoboAtlas is a contextual Active SLAM framework that adaptively balances geometric exploration and semantic reasoning using frontier exploration, global semantic-map reasoning, and egocentric VLM-based reasoning through a contextual multi-armed bandit. It achieves state-of-the-art 90.6% success rate on GOAT-Bench "Val Unseen" using GPT-4o, and 88.8% using the smaller Qwen2.5-VL-7B model.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of balancing geometric exploration and semantic reasoning in Active SLAM, and to demonstrate that grounding foundation models with large-scale 3D semantic maps enables robust and efficient contextual Active SLAM, showing that semantic mapping provides more value than simply replacing the underlying foundation model.

Method: RoboAtlas integrates frontier exploration, global semantic-map reasoning, and egocentric VLM-based reasoning through a contextual multi-armed bandit that transitions from exploration to semantically guided navigation as scene understanding improves. It uses OpenRoboVox, a scalable 3D semantic mapping system, and evaluates in simulation and on a Unitree Go2 robot.

Result: Evaluated in large-scale real-world environments exceeding 1800 m² with ~30k mapped semantic instances, achieving 100% task success rate. On GOAT-Bench "Val Unseen", achieves 90.6% SR with GPT-4o (improving over strongest prior baseline by 17.8 percentage points). Using Qwen2.5-VL-7B, achieves 88.8% SR, outperforming all baselines using GPT-4o.

Conclusion: Grounding foundation models with large-scale 3D semantic maps enables robust and efficient contextual Active SLAM, revealing that information gained from semantic mapping is more valuable than simply using a more powerful foundation model.

Abstract: We present RoboAtlas, a contextual Active SLAM framework that adaptively balances geometric exploration and semantic reasoning using a scalable 3D semantic mapping system, OpenRoboVox. RoboAtlas integrates frontier exploration, global semantic-map reasoning, and egocentric VLM-based reasoning through a contextual multi-armed bandit that transitions from exploration to semantically guided navigation as scene understanding improves. We evaluate the system in simulation and on a Unitree Go2 robot in large-scale real-world environments exceeding 1800 m2 with approx. 30k mapped semantic instances, achieving a 100% task success rate. On the GOAT-Bench "Val Unseen" benchmark, RoboAtlas achieves state-of-the-art performance with highest reported success rate (SR) of 90.6%, using GPT-4o, improving over the strongest prior baseline by 17.8 percentage points in SR. Using the much smaller Qwen2.5-VL-7B model, it still achieves 88.8% SR, outperforming all baselines using GPT-4o in SR, and revealing the importance of the information gained by our semantic mapping framework over simply replacing the underlying foundation model. The results demonstrate that grounding foundation models with large-scale 3D semantic maps enables robust and efficient contextual Active SLAM.

</details>


### [68] [Learning Robot Visual Navigation in Crowds via Intention-Aware Scene Representations](https://arxiv.org/abs/2606.26047)
*Han Bao,Bingyi Xia,Hanjing Ye,Yu Zhan,Hao Cheng,Baozhi Jia,Wenjun Xu,Jiankun Wang*

Main category: cs.RO

TL;DR: Summary generation failed


<details>
  <summary>Details</summary>
Motivation: Motivation analysis unavailable

Method: Method extraction failed

Result: Result analysis unavailable

Conclusion: Conclusion extraction failed

Abstract: Robot crowd navigation requires the ability to infer human intentions while accounting for the structural constraints of the environment. Currently, deep reinforcement learning (DRL) provides a promising method for learning navigation policies that understand human intentions. However, most of them rely on limited scene representations, treating pedestrians as simple 2D points and ignoring rich visual cues from both humans and the environment. To address this issue, we introduce iCrowdNav, a novel visual crowd navigation method with intention-aware scene representations, to encode behavioral and structural context from egocentric visual observations. Our method employs two key components: a spatio-temporal encoder for extracting occupancy features of the scene, and Intent-Interact Former (I$^2$ Former), an attention-based module that encodes human poses to infer pedestrians' motion intentions. These features are integrated into a compact state embedding that supports effective DRL policy training. Extensive experiments show that our method achieves superior performance over baselines, and real-world deployment demonstrates vision-based crowd navigation.

</details>


### [69] [Deep Reinforcement Learning-Enhanced Event-Triggered Data-Driven Predictive Control for a 3D Cable-Driven Soft Robotic Arm](https://arxiv.org/abs/2606.26048)
*Cheng Ouyang,Moeen Ul Islam,Kaixiang Zhang,Zhaojian Li,Xiaobo Tan,Dong Chen*

Main category: cs.RO

TL;DR: This paper proposes an adaptive reinforcement-learning-based event-triggered DeePC framework for soft robotic control, which reduces optimization frequency while maintaining comparable tracking accuracy.


<details>
  <summary>Details</summary>
Motivation: Soft robots have nonlinear and time-varying dynamics that are challenging to control. DeePC offers a model-free alternative but requires solving constrained optimization problems at every sampling instant, which is computationally demanding for real-time deployment on resource-limited robotic platforms.

Method: The authors propose an adaptive reinforcement-learning-based event-triggered DeePC (RL-ET-DeePC) framework, where a model-free RL policy determines when to invoke the DeePC optimizer based on the current system state representation, thereby reducing unnecessary optimization calls.

Result: Simulation results show RL-ET-DeePC reduces optimization frequency by up to 66% compared to periodic DeePC while maintaining comparable tracking accuracy. Hardware experiments on a 3D cable-driven soft robotic arm demonstrate zero-shot transfer, achieving a 34% reduction in optimization frequency with tracking accuracy comparable to periodic DeePC and more consistent performance than a static threshold-based event-triggered baseline.

Conclusion: The RL-ET-DeePC framework effectively reduces the computational burden of DeePC while preserving closed-loop performance for soft robotic systems, demonstrating practical viability for resource-constrained platforms.

Abstract: Soft robots are challenging to control due to their nonlinear and time-varying dynamics. Data-enabled predictive control (DeePC) offers a model-free alternative by directly leveraging measured input-output trajectories to construct a predictive controller. However, its receding-horizon formulation requires solving a constrained optimization problem at every sampling instant, which can be computationally demanding for real-time deployment on resource-limited robotic platforms.To address this limitation, we propose an adaptive reinforcement-learning-based event-triggered DeePC (RL-ET-DeePC) framework for soft robotic control. A model-free RL policy is trained to determine when to invoke the DeePC optimizer based on the current system state representation, thereby reducing unnecessary optimization calls while preserving closed-loop performance.Simulation results show that RL-ET-DeePC reduces optimization frequency by up to 66% compared to periodic DeePC, while maintaining comparable tracking accuracy. Hardware experiments on a three-dimensional cable-driven soft robotic arm demonstrate zero-shot transfer, achieving a 34% reduction in optimization frequency with tracking accuracy comparable to periodic DeePC and more consistent performance than a static threshold-based event-triggered baseline.

</details>


### [70] [ForceBand: Learning Forceful Manipulation with sEMG](https://arxiv.org/abs/2606.26093)
*Botao He,Zhi Wang,Linna Kuang,Ishaan Ghosh,Jitendra Malik,Cornelia Fermuller,Tingfan Wu,Jiayuan Mao,Ruoshi Liu,Haozhi Qi,Yiannis Aloimonos*

Main category: cs.RO

TL;DR: ForceBand is a low-cost wrist-worn sEMG system that converts human muscle activity into force-enriched demonstrations for robot manipulation policies. Using sEMG and IMU signals to predict per-finger forces, it enables force-augmented demonstrations for robot learning with over 50% lower force prediction error than vision-based baselines and 87% success rate on pick, squeeze, and place tasks.


<details>
  <summary>Details</summary>
Motivation: Human demonstrations are scalable for robot manipulation learning, but common sources like motion-capture and internet videos capture motion and appearance while missing critical contact forces needed for force-sensitive manipulation.

Method: Collects a 10-hour multimodal dataset (egocentric video, sEMG, IMU, fingertip force) and pre-trains an EMG2Force model to predict per-finger forces from sEMG and IMU signals. After user calibration, demonstrations are collected with ForceBand and video, then labeled with per-finger force traces.

Result: ForceBand recovers fine-grained fingertip interactions with over 50% lower force prediction error than vision-based baselines. Achieves 87% success rate on pick, squeeze, and place tasks requiring object-specific force control across diverse objects.

Conclusion: ForceBand enables force-enriched demonstrations from low-cost sEMG systems, bridging the gap between motion/visual data and force-sensitive manipulation requirements, making force data collection scalable and accessible for robot policy learning.

Abstract: Human demonstrations are a scalable data source for learning robot manipulation policies. However, common sources of human demonstration data, such as motion-capture trajectories and internet videos, capture mostly motion and appearance while missing the contact forces that are critical for force-sensitive manipulation. In this paper, we introduce ForceBand, a low-cost wrist-worn sEMG system that turns human muscle activity into force-enriched demonstrations. We first collect a 10-hour multimodal dataset containing egocentric video, sEMG, IMU, and fingertip force measurements across diverse actions and objects. Using this dataset, we pre-train an EMG2Force model that predicts per-finger forces from sEMG and IMU signals. After a short user-specific calibration, users can collect target-task demonstrations using only ForceBand and video; EMG2Force then labels these demonstrations with per-finger force traces, producing force-augmented demonstrations for robot policy learning. Experiments show that ForceBand recovers fine-grained fingertip interactions with over 50% lower force prediction error than vision-based baselines and achieves an 87% success rate on pick, squeeze, and place tasks that require object-specific force control across objects with diverse shapes, sizes, and weights. Project website: https://forceband-emg.github.io

</details>


### [71] [Learning Action Priors for Cross-embodiment Robot Manipulation](https://arxiv.org/abs/2606.26095)
*Dong Jing,Tianqi Zhang,Jiaqi Liu,Jinman Zhao,Zelong Sun,Li Erran Li,Zhiwu Lu,Mingyu Ding*

Main category: cs.RO

TL;DR: A two-stage training framework that pretrains a Vision-Language-Action (VLA) model's action module with motion priors before cross-modal alignment, enabling faster convergence and stronger performance across cross-embodiment tasks.


<details>
  <summary>Details</summary>
Motivation: Most VLA models attach action modules to VLM backbones, but this leaves the action module to learn physical motion from scratch. This results in a lack of explicit motion prior, forcing early optimization to simultaneously discover temporal action dynamics and cross-modal alignment—a challenge particularly amplified in cross-embodiment settings.

Method: A two-stage training framework: Stage 1 pretrains a lightweight flow-matching-based encoder-decoder action module with unconditioned action trajectories to learn temporal motion structure. Stage 2 transfers this prior to VLA training through decoder reuse and early-stage latent distillation, while the encoder serves as a compact history compressor for state-action histories.

Result: Achieves faster convergence, higher success rates, and substantially stronger performance on data-scarce real-world tasks across 13 diverse cross-embodiment tasks on both simulated and real-world platforms. Scaling action data in Stage 1 yields a more generalizable action prior that directly improves downstream VLA performance.

Conclusion: The proposed approach effectively equips VLA models with motion priors before cross-modal alignment, demonstrating that pretraining the action module with temporal motion structure significantly improves policy learning efficiency and generalization across different embodiments.

Abstract: Most Vision-Language-Action (VLA) models build on a Vision-Language Model (VLM) backbone by attaching an action module and optimizing the full policy jointly. This design inherits strong visual and linguistic priors from the VLM, but leaves the action module to learn physical motion almost from scratch. As a result, the policy lacks an explicit motion prior, forcing early optimization to simultaneously discover temporal action dynamics and cross-modal alignment, a challenge further amplified in cross-embodiment settings. In this work, we propose to pretrain the action module with motion priors before cross-modal VLA alignment. Specifically, we introduce a two-stage training framework that equips the action module with cross-embodiment temporal motion structure before VLA training begins. In Stage~1, a lightweight flow-matching-based encoder-decoder action module efficiently learns temporal motion structure solely from unconditioned action trajectories, without processing visual or language tokens. In Stage~2, this learned prior is transferred to VLA training through decoder reuse and early-stage latent distillation, aligning visual-language features with the action embedding space while still allowing end-to-end policy refinement. In addition, the trained encoder serves as a compact history compressor, summarizing state-action histories into a single temporal context token for history-aware modeling at negligible cost. Extensive experiments across 13 diverse cross-embodiment tasks on both simulated and real-world platforms validate the effectiveness of our approach. Compared with VLA training without action priors, our model achieves faster convergence, higher success rates, and substantially stronger performance on data-scarce real-world tasks. Moreover, scaling up the action data in Stage~1 yields a more generalizable action prior that directly improves downstream VLA performance.

</details>
